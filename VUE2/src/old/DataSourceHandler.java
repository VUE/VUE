...
// TODO: in order to keep backward compat with old DataSourceViewer code (unless
// we're
// ready to just throw the swtich), we'll need an interface that both
// DataSourceViewer
// and DataSourceHandler can implement, that includes at least a
// setActiveDataSource for
// the two types, plus addOrdered / getModelContents, or some new pairs of
// clearear and
...
public class DataSourceHandler
  extends JPanel
  implements
    edu.tufts.vue.dsm.DataSourceListener,
    edu.tufts.vue.fsm.event.SearchListener,
    KeyListener,
    ActionListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DataSourceHandler.class);
  ...
  private final org.osid.shared.Type searchType = new edu.tufts.vue.util.Type(
    "mit.edu",
    "search",
    "keyword"
  );
  private final org.osid.shared.Type thumbnailType =
    new edu.tufts.vue.util.Type("mit.edu", "partStructure", "thumbnail");
  ...
  private class BasicSourcesList extends JList {
    ...
    // private static final org.apache.log4j.Logger Log =
    // org.apache.log4j.Logger.getLogger(BasicSourcesList.class);
    ...
    // todo: does not maintain groupings on later OSID add update should list
    // contain both OSIDs & VUE local DataSources
    public boolean addOrdered(Object o) {
      if (excludedSources.contains(o.getClass())) return false;
      if (
        includedSources.size() > 0 && !includedSources.contains(o.getClass())
      ) return false;
  ...
  public DataSourceHandler(
    ...
    edu.tufts.vue.dsm.impl.VueDataSourceManager
      .getInstance()
  ...
  private static final JMenuItem aboutResource = new JMenuItem(
    VueResources.getString("datasourcehandle.menu.aboutresource")
  );
  ...
  private static final JMenuItem deleteResource = new JMenuItem(
    VueResources.getString("datasourcehandle.menu.deleteresource")
  );
  ...
  private static final JLabel StatusLabel = new JLabel(
    VueResources.getString("addLibrary.loading.label"),
    JLabel.CENTER
  );
  ...
  static {
    ...
    if (DEBUG.BOXES) StatusLabel.setBorder(
  ...
  private JComponent produceViewer(
    final tufts.vue.BrowseDataSource ds,
    final boolean caching
  ) {
    if (!SwingUtilities.isEventDispatchThread()) throw new Error(
      "not threadsafe except for AWT"
    );
    ...
    final Thread buildViewerThread = new Thread(
      String.format("VBLD-%02d %s", vCount++, name)
    ) {
      {
        setDaemon(true);
      ...
      public void run() {
        ...
        if (isInterrupted()) {
          if (DEBUG.DR && newViewer != null) Log.debug(
            "produced; but not needed: aborting"
          );
          return;
        }
  ...
  private class AWTAcceptViewerTask implements Runnable {
    ...
    public void run() {
      if (serviceThread.isInterrupted()) {
        ...
        Log.warn(
          name +
          "; in AWT; but viewer no longer needed: aborting result for " +
          serviceThread,
          new Throwable("FYI")
        );
        return;
      ...
      if (DEBUG.Enabled) Log.debug(
        "accepting viewer & setting into VueDataSource"
      );
      ...
      // if (DataSourceViewer.this.browserDS == ds) { // important to check this in
      // AWT;
      if (browserDS == ds) { // important to check this in AWT;
        if (DEBUG.Enabled) Log.debug(
          "currently displayed data-source wants this viewer; displaying"
        );
        displayInBrowsePane(newViewer, false); // important to do this in AWT;
      } else if (DEBUG.DR) Log.debug(
        "display: skipping; user looking at something else"
      );
  ...
  private JComponent buildViewer(final tufts.vue.BrowseDataSource ds) {
    ...
    if (exception == null && viewer == null) exception =
      new Exception("no viewer available");
  ...
  private String prettyException(Throwable t) {
    String txt;
    if (t.getClass().getName().startsWith("java")) txt =
      t.getClass().getSimpleName(); else txt = t.getClass().getName();
          ...
          if (o != null) {
            ...
            if (o instanceof edu.tufts.vue.dsm.DataSource) {
              ...
              if (
                VueUtil.confirm(
                  VUE.getDialogParent(),
                  VueResources.getString("dialog.confirmdelete.resource") +
                  displayName +
                  "?",
                ...
                if (DRB.resultsPane != null) widgetStack =
                  (WidgetStack) DRB.resultsPane.getComponent(0);
  ...
  private static class StatusLabel extends JPanel {
    ...
    StatusLabel(String s, boolean center, boolean useIcon) {
      ...
      if (useIcon) {
        waitIcon =
          new JLabel(VueResources.getImageIcon("dsv.statuspanel.waitIcon"));
        this.add(waitIcon);
      }
      label = new JLabel(s);
      this.add(label);
    ...
    private void processResultsAndDisplay(
      ...
      if (assetIterator != null) {
        try {
          while (assetIterator.hasNextAsset()) {
            ...
            try {
              // resourceList.add(Resource.instance(mRepository, asset,
      ...
      if (resourceList.size() == 0) {
        if (assetIterator == null) results =
          new StatusLabel(
            "Empty results for " + mSearchString,
            false,
            false
          ); else results =
          new StatusLabel("No results for " + mSearchString, false, false);
      } else {
  ...
  private void positionEditInfoWindow() {
    ...
    if (
      (w.getX() + w.getWidth() + editInfoDockWindow.getWidth()) <
      screenSize.getWidth()
    ) editInfoDockWindow.setLocation(
      w.getX() + w.getWidth(),
      w.getY()
    ); else editInfoDockWindow.setLocation(
      w.getX() - editInfoDockWindow.getWidth(),
      w.getY()
    );
  }
  ...
  private void refreshEditInfo(edu.tufts.vue.dsm.DataSource ds, boolean force) {
    ...
    if (force || editInfoDockWindow.isVisible()) {
      ...
      if (ds.hasConfiguration()) {
        ...
        editInfoStack.addPane(
          name,
          new JLabel("UNIMPLEMENTED: EditLibraryPanel " + ds.getClass())
        );
      } else {
  ...
  public static void saveDataSourceViewer() {
    Log.error(
      new Throwable(
        "SAVE DISABLED -- NEED TO MAKE SURE LISTS ARE JOINED TO SAVE"
      )
    );
    // if (dataSourceList == null) {
    ...
    // File f = new
    // File(VueUtil.getDefaultUserFolder().getAbsolutePath()+File.separatorChar+VueResources.getString("save.datasources"));
    ...
    // }
    // try {
    // if (DEBUG.DR) Log.debug("saveDataSourceViewer: creating new
    // SaveDataSourceViewer");
  ...
  public static void marshallMap(
    File file,
    SaveDataSourceViewer dataSourceViewer
  ) {
    ...
    try {
      ...
      if (DEBUG.DR) Log.debug(
        "marshallMap: marshalling " + dataSourceViewer + " to " + file + "..."
      );
  ...
  public SaveDataSourceViewer unMarshallMap(File file)
    throws java.io.IOException, org.exolab.castor.xml.MarshalException, org.exolab.castor.mapping.MappingException, org.exolab.castor.xml.ValidationException {
    Unmarshaller unmarshaller =
      tufts.vue.action.ActionUtil.getDefaultUnmarshaller(file.toString());
    FileReader reader = new FileReader(file);
    SaveDataSourceViewer sviewer =
      (SaveDataSourceViewer) unmarshaller.unmarshal(new InputSource(reader));
    reader.close();
    return sviewer;
  }
