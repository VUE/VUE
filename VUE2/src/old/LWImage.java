...
public class LWImage
  ...
  implements Images.Listener // ,LWSelection.ControlListener
// ,edu.tufts.vue.preferences.VuePrefListener
{

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWImage.class);
  ...
  // static int MaxRenderSize =
  // PreferencesManager.getIntegerPrefValue(ImageSizePreference.getInstance());
  // private static VueIntegerPreference PrefImageSize =
  // ImageSizePreference.getInstance(); // is failing for some reason
  // static int MaxRenderSize = PrefImageSize.getValue();
  public static final int DefaultMaxDimension = 128;

  private static final int MinWidth = 16;
  ...
  // either just auto-handle the special case of width/height for everything, or
  // add
  // generic properties based on content type for the URLResource (we sort of
  // already
  // have this with stuff that comes from MapDropTarget). Tho we only need this
  // for
  // content that has a needed/useful in-memory representation that's different
  // than
  // it's on-disk content. Currently, this only applies to images. If we were to
  // support, say, dynamically generating icons (or even a document model) for
  // HTML or
  ...
  public LWImage(Resource r) {
    ...
    if (r == null) throw new IllegalArgumentException(
      "resource is not image content: " + r
    );
    if (!r.isImage()) Log.warn(
      "making LWImage: may not be image content: " + r
    );
    setFillColor(null);
    setResource(r);
    // edu.tufts.vue.preferences.implementations.ImageSizePreference.getInstance().addVuePrefListener(this);
  ...
  // // TODO: it isn't such a good idea to have every single LWImage ever created
  // during the runtime
  // // be a listiner to the VUE preferences sub-system -- that means a pref event
  // will be driving
  // // global, cross-map, cross-undo-queue events, which would be better managed
  // from a single
  // // place in the model code.
  // public void preferenceChanged(VuePrefEvent prefEvent)
  // {
  // if (DEBUG.IMAGE) out("new pref value is " +
  // ((Integer)ImageSizePreference.getInstance().getValue()).intValue());
  // DefaultMaxDimension = ((Integer)prefEvent.getNewValue()).intValue();
  // if (DEBUG.Enabled) System.out.println("New DefaultMaxDimension : " +
  // DefaultMaxDimension + " in " + this);
  ...
  public boolean isAutoSized() {
    if (
      getClass().isAssignableFrom(LWNode.class)
    ) return super.isAutoSized(); else return false;
  }
  ...
  public boolean isTransparent() {
    if (
      false
    ) return super.isTransparent(); else return false; // && this instanceof LWNode)
  }
  ...
  public boolean isTranslucent() {
    if (
      false
    ) return super.isTranslucent(); else { // && this instanceof LWNode) {
  ...
  /**
   * This currently makes LWImages invisible to selection (they're locked in their
   * parent node
   */
  // @Override
  protected LWComponent defaultPickImpl(PickContext pc) {
    if (getClass().isAssignableFrom(LWNode.class)) {
      ...
      if (!hasFlag(Flag.SLIDE_STYLE) && isNodeIcon()) return pc.pickDepth > 0
        ? this
        : getParent(); else return this;
    }
  }
  ...
  /**
   * this for backward compat with old save files to establish the image as a
   * special "node" image
   */
  @Override
  public void XML_addNotify(Object context, String name, Object parent) {
    super.XML_addNotify(context, name, parent);
    if (parent instanceof LWNode) updateNodeIconStatus((LWNode) parent);
  }
  ...
  private void updateNodeIconStatus(LWContainer parent) {
    // tufts.Util.printStackTrace("updateNodeIconStatus, mImage=" + mImage + "
    // parent=" + parent);
    if (DEBUG.IMAGE) out(
      "updateNodeIconStatus, mImage=" + Util.tags(mImage) + " parent=" + parent
    );

    if (parent == null) return;

    if (
      parent instanceof LWNode &&
  ...
  public void setMaxDimension(final double max) {
    ...
    // if (DEBUG.IMAGE) out("setMaxDimension newSize " + newWidth + "x" +
    // newHeight);
  ...
  public void layoutImpl(Object triggerKey) {
    if (false && getClass().isAssignableFrom(LWNode.class)) {
      ...
      // mIconBlock.layout();
      // if (super.labelBox != null) {
      // out("layoutImpl " + triggerKey + "; SET BOX LOCATION AT Y " + getHeight() + "
      // in " + this);
  ...
  public void setSelected(boolean selected) {
    ...
    if (
      selected && !wasSelected && mImageStatus == Status.ERROR && hasResource()
    ) {
      ...
      addCleanupTask(
        new Runnable() {
          public void run() {
            if (VUE.getSelection().only() == LWImage.this) loadResourceImage(
              getResource(),
              null
            );
          }
  ...
  private void setImageResource(Resource r, boolean isNodeIconSync) {
    if (DEBUG.IMAGE) Log.debug(
      "setImageResource " + r + "; isNodeIconSync=" + isNodeIconSync
    );
  ...
  // todo: find a better way to do this than passing in an undo manager, which is
  // dead ugly
  ...
  private void loadResourceImage(final Resource r, final UndoManager um) {
    ...
    synchronized (this) {
      ...
      if (immediatelyAvailable) mUndoMarkForThread =
        null; else mUndoMarkForThread = UndoManager.getKeyForNextMark(this);
    }
  }
  ...
  public synchronized void gotImageSize(
    ...
    if (DEBUG.IMAGE) out(
      "gotImageSize " + width + "x" + height + " bytes=" + byteSize
    );
    mDataSize = byteSize;
    mImageStatus = Status.LOADING;

    if (mUndoMarkForThread == null) {
      if (DEBUG.IMAGE || DEBUG.UNDO || DEBUG.THREAD) out(
        "gotImageSize: no undo key"
      );
    }

    // For the events triggered by the setSize below, make sure they go
    // to the right point in the undo queue.
    // The mark was generated synchronously in the main model accessing thread (AWT
    // EDT),
    ...
    // If we're interrupted before this happens, and this is the drop of a new
    // image,
    // we'll see a zombie event complaint from this setSize which is safely
    // ignorable.
    ...
    // if (isCropped() == false) {
    // if (super.width == NEEDS_DEFAULT) {
    // // if this is a new image object, set it's size to the image size (natural
    // size)
  ...
  public void gotBytes(Object imageSrc, long bytesSoFar) {
    ...
    if (mDataSize > 0 && mDataSoFar > 0) { // don't bother if we don't know the whole size yet...
      ...
      if (pctEven > mLastPctEven) {
        // todo: if last update was more than, say 100ms ago (10fps) (statically: for
        // ANY image),
  ...
  /** @see Images.Listener */
  public synchronized void gotImage(
    Object imageSrc,
    Image image,
    int w,
    int h
  ...
  // super.setSize(w, h);
  // // Even if we don't have an image yet, we need to keep these set in case user
  // attemps to resize the frame.
  // // They can still crop down if they like, but this prevents them from making
  // it any bigger.
  // if (mImageWidth < 0)
  // mImageWidth = (int) getWidth();
  // if (mImageHeight < 0)
  ...
  void setImageSize(int w, int h) {
    ...
    // todo: may want to just always update the node status here -- covers most
    // cases, plus better when the drop code calls this?
    if (DEBUG.IMAGE) out(
      "setImageSize " + w + "x" + h + " aspect=" + mImageAspect
    );

    // SMF 2008-05-06: can't make sense of this: if we replace the image content,
    // and the
    // old content aspect != 1, and the new content aspect == 1, this prevents us
    // from
    // updating our shape to the new 1.0 aspect (and squeezes/stretches the 1.0
    // aspect image
  ...
  private void autoShapeToAspect() {
    if (mImageAspect > 0) {
      if (this.width == NEEDS_DEFAULT || this.height == NEEDS_DEFAULT) {
        // Log.error("cannot auto-shape without request size: " + this, new
        // Throwable("HERE"));
        if (DEBUG.WORK || DEBUG.IMAGE) out(
          "autoshaping from scratch to " + DefaultMaxDimension
        );
        setMaxDimension(DefaultMaxDimension);
        return;
      }
      ...
      final Size newSize = ConstrainToAspect(
        mImageAspect,
        this.width,
        this.height
      );
       ...
       */
      if (
        (DEBUG.WORK || DEBUG.IMAGE) &&
        (newSize.width != width || newSize.height != height)
      ) out(
        String.format(
          "autoShapeToAspect: a=%.2f dw=%g dh=%g; %.1fx%.1f -> %s",
          mImageAspect,
          dw,
          dh,
          width,
      ...
      // out("autoShapeToAspect: a=" + mImageAspect + "; dw=" + dw + ", dh=" + dh + ";
      // " + width + "," + height + " -> adj " + newSize);
      // out("autoShapeToAspect: " + width + "," + height + " -> newSize: " +
      // newSize.width + "," + newSize.height);

      if (Math.abs(dw) > 1 || Math.abs(dh) > 1) {
        // above check helps reduce needless tweaks, which make things messy during map
        // loading
  ...
  protected void userSetSize(float width, float height, MapMouseEvent e) {
    if (DEBUG.IMAGE) out(
      "userSetSize " + Util.fmt(new Point2D.Float(width, height)) + "; e=" + e
    );
  ...
  private void croppingSetSize(float width, float height) {
    ...
    if (mImageWidth + mOffset.x < width) width = mImageWidth + mOffset.x;
    if (mImageHeight + mOffset.y < height) height = mImageHeight + mOffset.y;
   ...
   */
  public void X_RESIZE_CONTROL_HACK_userSetFrame(
    float x,
    float y,
    float w,
    float h,
    ...
    if (DEBUG.IMAGE) out(
      "userSetFrame0 " + VueUtil.out(new Rectangle2D.Float(x, y, w, h))
    );
    if (w < MinWidth) {
      if (
        x > getX()
      ) x -= MinWidth - w; // dragging left edge right: hold it back
      w = MinWidth;
    }
    if (h < MinHeight) {
      if (
        y > getY()
      ) y -= MinHeight - h; // dragging top edge down: hold it back
      h = MinHeight;
    }
    ...
    setOffset(off);
    if (DEBUG.IMAGE) out(
      "userSetFrame1 " + VueUtil.out(new Rectangle2D.Float(x, y, w, h))
    );
    userSetSize(w, h, e);
    setLocation(x, y);
  }

  public static final Key KEY_Rotation = new Key(
    "image.rotation",
    KeyType.STYLE
  ) { // rotation in radians
    public void setValue(LWComponent c, Object val) {
      ((LWImage) c).setRotation(((Double) val).doubleValue());
    }

    public Object getValue(LWComponent c) {
      return new Double(((LWImage) c).getRotation());
    }
  };
  ...
  public static final Key Key_ImageOffset = new Key(
    "image.pan",
    KeyType.STYLE
  ) {
    public void setValue(LWComponent c, Object val) {
      ((LWImage) c).setOffset((Point2D) val);
    }

    public Object getValue(LWComponent c) {
      return ((LWImage) c).getOffset();
    }
  };

  public void setOffset(Point2D p) {
    if (p.getX() == mOffset.x && p.getY() == mOffset.y) return;
  ...
  private static final AlphaComposite HudTransparency =
  ...
  // private static final Color IconBorderColor = new Color(0,0,0,64); // screwing
  // up in composite drawing
  ...
  protected void drawImpl(DrawContext dc) {
    ...
    // ((grandparent = parent.parent) == null || !grandparent.isSelected())))) {
    ...
    // System.err.println("ALPHA RULE: " + a.getRule() + " " +
    // DrawContext.AlphaRuleNames[a.getRule()] + " " + this);
    // }
  ...
  private boolean isIndicatedIn(DrawContext dc) {
    if (dc.hasIndicated()) {
      ...
      return indication == this || (isNodeIcon() && getParent() == indication);
    } else {
      return false;
    }
  }
  ...
  public void drawWithoutShape(DrawContext dc) {
    // see comments on VUE-892 - this code does not seem to present the right
    // behavior for search
     ...
     * // this is a hack because images are currently special cased as tied to their
     * parent node
     * return;
     * }
     * }
    ...
    if (indicated && dc.focal != this) {
      ...
      if (VueUtil.isTranslucent(c)) c = Color.gray;
      dc.g.setColor(c);
      dc.g.fill(shape);
    }

    if (isNodeIcon && dc.focal != this) {
      if (!indicated && DefaultMaxDimension > 0) drawImageBox(dc);

      // Forced border for node-icon's:
      if (
        (mImage != null || indicated) &&
        !getParent().isTransparent() &&
        DefaultMaxDimension > 0
      ) {
        // this is somehow making itext PDF generation through a GC worse... (probably
        // just a bad tickle)
  ...
  private void drawImageBox(DrawContext dc) {
    if (mImage == null && !dc.isIndicated(this)) drawImageStatus(
      dc
    ); else drawImage(dc);
    ...
    if (mImageStatus == Status.UNLOADED && getResource() != null) {
      ...
      synchronized (this) {
        if (mImageStatus == Status.UNLOADED) {
          ...
          if (DEBUG.IMAGE) out(
            "invokeLater loadResourceImage " + getResource()
          );
          tufts.vue.gui.GUI.invokeAfterAWT(
            new Runnable() {
              public void run() {
  ...
  public void XML_completed(Object context) {
    ...
    if (super.width < MinWidth || super.height < MinHeight) {
      Log.info(
        String.format(
          "bad size: adjusting to minimum %dx%d: %s",
          MinWidth,
          MinHeight,
          this
        )
    ...
    // // This will cause images to start loading during parsing of persisted map
    // files:
    // if (mImageStatus == Status.UNLOADED) {
    // mImageStatus = Status.LOADING;
    // loadResourceImage(getResource(), null);
  ...
  private void drawImage(DrawContext dc) {
    ...
    // private static final AlphaComposite MatteTransparency =
    // AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f);
    // Todo: when/if put this back in, see if we can handle it in the ImageTool so
    // we don't need active tool in the DrawContext
    // if (isSelected() && dc.isInteractive() && dc.getActiveTool() instanceof
    // ImageTool) {
  ...
  private Color getEmptyColor(DrawContext dc) {
    ...
    // } else if (dc.getBackgroundFill() != null &&
    // dc.getBackgroundFill().equals(Color.black))
  ...
  // private static final float LoadingWidth = (float)
  // tufts.vue.gui.GUI.stringWidth(StatusFont, LoadingText);
  ...
  private void drawImageStatus(DrawContext dc) {
    ...
    // final float statusWidth = (float) tufts.vue.gui.GUI.stringWidth(StatusFont,
    // mStatusMsg);
    ...
    // } else {
    // dc.g.drawString("Loading...", (width-LoadingWidth)/2,
    ...
    } else drawStatusLine(dc, status1, 0);
  }

  private void drawStatusLine(DrawContext dc, String text, int yoff) {
    final float textWidth = (float) tufts.vue.gui.GUI.stringWidth(
      StatusFont,
      text
    );
    dc.g.drawString(
   ...
   * }
   *
   * AffineTransform transform = AffineTransform.getTranslateInstance(mOffset.x,
   * mOffset.y);
  ...
  // Holy shit: if we somehow defined all this control-point stuff as a property
  // editor,
  ...
  public void controlPointMoved(int index, MapMouseEvent e) {
    if (index == 0) {
      if (
        mImageStatus == Status.ERROR
      ) return; // don't let user play with offset if no image visible
    ...
    } else throw new IllegalArgumentException(this + " no such control point");
  }
  ...
  /**
   * Keep LWImage filled with image bits (never display "area" outside of the
   * image)
   */
  private void constrainOffset(Point2D.Float off) {
    if (off.x > 0) off.x = 0;
    if (off.y > 0) off.y = 0;
    if (off.x + getImageWidth() < getWidth()) off.x =
      getWidth() - getImageWidth();
    if (off.y + getImageHeight() < getHeight()) off.y =
   ...
   * Keep LWImage filled with image bits (never display "area" outside of the
   * image)
   * Used for constraining the clipped region to the underlying image, which we
   * keep
  ...
  // private LWSelection.Controller[] controlPoints = new
  // LWSelection.Controller[1];
  // /** interface ControlListener */
  // public LWSelection.Controller[] X_getControlPoints() // DEIMPLEMENTED
  // {
  // controlPoints[0] = new LWSelection.Controller(getMapCenterX(),
  // getMapCenterY());
   ...
   * // [ What?? ] -- todo: this doesn't work if we're here because the resource
   * was changed...
   ...
   *
   * Image image = XgetImage(mr); // this will immediately block if host not
   * responding
   * // todo: okay, we can skip the rest of this code as getImage now uses the
   * ImageIO
   * // fetch
   *
   * if (image == null) {
   ...
   * }
   *
   * // don't bother to set mImage here: JVM's no longer do drawing of available
   * bits
   *
   ...
   * if (s.endsWith(".tif") || s.endsWith(".tiff") || s.endsWith(".bmp") ||
   * s.endsWith(".ico"))
   ...
   * // will apparently ALWAYS eventually get an Image object, but if it failed,
   * we
   * // eventually get callback to imageUpdate (once prepareImage is called) with
   * an
   * // error code. In any case, if you don't want to block, this has to be done
   * in
   * // a thread.
   *
   * String s = mr.getSpec();
   ...
   * public boolean XimageUpdate(Image img, int flags, int x, int y, int width,
   * int height)
   * {
   * if ((DEBUG.IMAGE||DEBUG.THREAD) && (DEBUG.META || (flags &
   * ImageObserver.SOMEBITS) == 0)) {
   ...
   * }
   *
   * if ((flags & ImageObserver.WIDTH) != 0 && (flags & ImageObserver.HEIGHT) !=
   * 0) {
   * //XsetRawImageSize(width, height);
   * if (DEBUG.IMAGE || DEBUG.THREAD) out("imageUpdate; got size " + width + "x" +
   * height);
   *
   * if (mUndoMarkForThread == null) {
   ...
   * // todo: suspend events if our thread was interrupted
   * if (isCropped() == false) {
   * // don't set size if we are cropped: we're probably reloading from a saved
   * .vue
   * setSize(width, height);
   * }
   * layout();
   ...
   * System.err.println("\n" + getResource() + " *** INTERRUPTED *** " +
   * Thread.currentThread());
   * //System.err.println("\n" + getResource() +
   * " *** INTERRUPTED *** (lowering priority) " + thread);
   * // Changing priority of the Image Fetcher will prob slow down all subsequent
   * loads
   ...
   * notify(LWKey.RepaintAsync);
   * } else {
   * UndoManager.detachCurrentThread(mUndoMarkForThread); // in case our
   * ImageFetcher get's re-used
  ...
  public static void main(String args[]) throws Exception {
    // GUI init required for fully loading all image codecs (tiff gets left behind
    // otherwise)
     ...
     * tho we would need to convert it from the resulting RenderedImage /
     * PlanarImage
     *
     */
     ...
     * com.sun.media.jai.codec.SeekableStream stream = new
     * com.sun.media.jai.codec.FileSeekableStream(filename);
     * String[] names = com.sun.media.jai.codec.ImageCodec.getDecoderNames(stream);
     * System.out.println("ImageCodec API's found decoders: " +
     * java.util.Arrays.asList(names));
     * com.sun.media.jai.codec.ImageDecoder dec =
     * com.sun.media.jai.codec.ImageCodec.createImageDecoder(names[0], stream,
     * null);
     ...
     * }
     *
     * // We're not magically getting any new codec's added to ImageIO after the
     * above code
     * // finds the .fpx codec...
     *
     * System.out.println(java.util.Arrays.asList(javax.imageio.ImageIO.
  ...
  // */
  // // TODO: no longer meaninful -- was only used to determine if the old
  // preference
