...
// ,edu.tufts.vue.preferences.VuePrefListener
{
    ...
    enum Status {
        UNLOADED, LOADING, LOADED, ERROR, EMPTY
    };
    ...
    private void updateNodeIconStatus(LWContainer parent) {
        ...
        // parent=" + parent);
        if (DEBUG.IMAGE)
            out("updateNodeIconStatus, mImage=" + Util.tags(mImage) + " parent=" + parent);
    ...
    public void setMaxDimension(final double max) {
        ...
        if (DEBUG.IMAGE)
            out("setMaxDimension curSize " + width + "x" + height);
        ...
        // newHeight);
        if (DEBUG.IMAGE)
            out("setMaxDimension newSize " + w + "x" + h);
    ...
    private void setImageResource(Resource r, boolean isNodeIconSync) {
        if (DEBUG.IMAGE)
            Log.debug("setImageResource " + r + "; isNodeIconSync=" + isNodeIconSync);
    ...
    public synchronized void gotImageSize(Object imageSrc, int width, int height, long byteSize) {
        if (DEBUG.IMAGE)
            out("gotImageSize " + width + "x" + height + " bytes=" + byteSize);
        mDataSize = byteSize;
        mImageStatus = Status.LOADING;

        if (mUndoMarkForThread == null) {
            if (DEBUG.IMAGE || DEBUG.UNDO || DEBUG.THREAD)
                out("gotImageSize: no undo key");
        }
    ...
    void setImageSize(int w, int h) {
        ...
        if (DEBUG.IMAGE)
            out("setImageSize " + w + "x" + h + " aspect=" + mImageAspect);
    ...
    private void autoShapeToAspect() {
        ...
        if (mImageAspect > 0) {
            ...
            if (this.width == NEEDS_DEFAULT || this.height == NEEDS_DEFAULT) {
                ...
                // Throwable("HERE"));
                if (DEBUG.WORK || DEBUG.IMAGE)
                    out("autoshaping from scratch to " + DefaultMaxDimension);
                setMaxDimension(DefaultMaxDimension);
                return;
            }
    ...
    protected void userSetSize(float width, float height, MapMouseEvent e) {
        if (DEBUG.IMAGE)
            out("userSetSize " + Util.fmt(new Point2D.Float(width, height)) + "; e=" + e);
    ...
    private void scalingSetSize(float width, float height) {
        ...
        if (DEBUG.IMAGE)
            out("scalingSetSize1 " + width + "x" + height);
    ...
    private void croppingSetSize(float width, float height) {
        ...
        if (height < MinHeight)
            height = MinHeight;
        if (DEBUG.IMAGE)
            out("croppingSetSize1 " + width + "x" + height);
    ...
    public void X_RESIZE_CONTROL_HACK_userSetFrame(float x, float y, float w, float h, MapMouseEvent e) {
        if (DEBUG.IMAGE)
            out("userSetFrame0 " + VueUtil.out(new Rectangle2D.Float(x, y, w, h)));
        if (w < MinWidth) {
        ...
        setOffset(off);
        if (DEBUG.IMAGE)
            out("userSetFrame1 " + VueUtil.out(new Rectangle2D.Float(x, y, w, h)));
        userSetSize(w, h, e);
        setLocation(x, y);
    }
    ...
    public void setOffset(Point2D p) {
        ...
        if (DEBUG.IMAGE)
            out("LWImage setOffset " + VueUtil.out(p));
        ...
        if (mImageStatus == Status.UNLOADED && getResource() != null) {
            ...
            synchronized (this) {
                if (mImageStatus == Status.UNLOADED) {
                    ...
                    if (DEBUG.IMAGE)
                        out("invokeLater loadResourceImage " + getResource());
    ...
    public String paramString() {
        return super.paramString() + " " + mImageStatus + " raw=" + mImageWidth + "x" + mImageHeight
                + (isNodeIcon ? " <NodeIcon>" : "");
    }
