...
public class ObjectInspectorPanel
  ...
  public class InfoPanel extends JPanel {
    ...
    public void updatePanel(LWComponent pComponent) {
      mComponent = pComponent;
      if (DEBUG.Enabled) objectLabel.setText(
        mComponent.getUniqueComponentTypeLabel()
      ); else objectLabel.setText(mComponent.getComponentTypeLabel());
  ...
  public class TreePanel extends JPanel {
    ...
    public TreePanel() {
      ...
      mTreeScrollPane.setVerticalScrollBarPolicy(
        JScrollPane.VERTICAL_SCROLLBAR_ALWAYS
      );
      mTreeScrollPane.setHorizontalScrollBarPolicy(
    ...
    public void updatePanel(LWComponent pComponent) {
      ...
      if (
        !tree.isInitialized() || !this.isVisible() || !tree.contains(pComponent)
      ) {
        // panelLabel.setText("Node: " + pNode.getLabel());
        if (pComponent instanceof LWContainer) tree.switchContainer(
          (LWContainer) pComponent
        ); else if (pComponent instanceof LWLink) tree.switchContainer(null);
      }
  ...
  public class NodeFilterPanel extends JPanel implements ActionListener {
    ...
    public void updatePanel(LWComponent pComponent) {
      ...
      if (DEBUG.SELECTION) System.out.println(
        "NodeFilterPanel.updatePanel: " + pComponent
      );
      if (nodeFilterEditor != null) {
        ...
        if (VUE.getActiveMap() != null && pComponent.getNodeFilter() != null) {
          ...
          nodeFilterEditor =
            new NodeFilterEditor(pComponent.getNodeFilter(), true);
  ...
  /**
   * setTab
   * Sets the selected Tab for teh panel to the specifided ObjectInspector panel
   * key
   **/
  public void setTab(int pTabKey) {
    ...
    if (picked != null) {
      if (WidgetStackImpl) Widget.setExpanded(
        picked,
        true
      ); else mTabbedPane.setSelectedComponent(picked);
    }
  ...
  public static void main(String args[]) {
    ...
    for (int i = 10; i < 31; i++) r.setProperty("field_" + i, "value_" + i);
    // map.addLWC(node);
    tufts.vue.gui.DockWindow w = tufts.vue.gui.GUI.createDockWindow(
      "Object Inspector"
    );
    w.setContent(p);
    w.setVisible(true);
