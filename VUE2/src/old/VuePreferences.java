...
public class VuePreferences {
  ...
  static final String PropertyKey = "vuePreferenceProperty";

  static class Property implements ActionListener {
    ...
    public JComponent makeEditor() {
      ...
      if (mDefault instanceof Boolean) {
        ...
        JTextField text;
        if (mDefault instanceof Number) text = new JTextField(4); else {
          text = new JTextField();
          text.setText((String) mDefault);
        }
        text.addActionListener(this);
        editor.add(text);
        if (mUnits.length() > 0) editor.add(new JLabel(mUnits));
      }
    ...
    public void actionPerformed(ActionEvent e) {
      Property p = null;
      if (e.getSource() instanceof JComponent) p =
        (Property) ((JComponent) e.getSource()).getClientProperty(PropertyKey);
  ...
  static class PreferencePane extends JTabbedPane {
    ...
    public PreferencePane() {
      ...
      addTab(
        "Fedora Prefs",
        new EditPane(Preferences.userRoot().node("tufts/oki/dr/fedora"))
      );
    }
  }
