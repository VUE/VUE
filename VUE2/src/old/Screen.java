...
public class Screen {
    ...
    private static void cycleAlpha(NSWindow w, float start, float end, final int steps) {
        System.out.println("cycleAlpha " + start + " -> " + end + " in " + steps + " steps");
        float alpha;
     ...
     *                     VUE.
     */
    public static void keepWindowsOnTop(String ensuredWindow, boolean fullScreen) {
        ...
        if (true || MainWindow == null || FullWindow == null) {
            for (int i = 0; i < windows.count(); i++) {
                ...
                if (w.title().startsWith("VUE-FULL-WORKING"))
                    FullWindow = w;
                else if (w.title().startsWith("VUE:"))
                    MainWindow = w;
            }
        }
        for (int i = 0; i < windows.count(); i++) {
            w = (NSWindow) windows.objectAtIndex(i);
            if (w == MainWindow || w == FullWindow || w.title().startsWith("OLD-"))
                continue;
            ...
            if ((ensuredWindow != null && ensuredWindow.equals(w.title())) ||
                ...
                if (fullScreen && FullWindow != null) {
                    System.out.println("ADDING AS CHILD OF FULL-SCREEN: #" + i + " [" + w.title() + "]");
                    FullWindow.addChildWindow(w, NSWindow.Above);
                } else {
                    System.out.println("ADDING AS CHILD OF MAIN-WINDOW: #" + i + " [" + w.title() + "]");
    ...
    public static void dumpWindows() {
        ...
        for (int i = 0; i < windows.count(); i++) {
            ...
            System.out.println("Window #" + (i > 9 ? "" : " ") + i + ": "
                    + " visible=" + w.isVisible() + (w.isVisible() ? " " : "")
                    + " [" + w.title() + "]\t"
                    + w
                    // + " level=[" + w.level() + "] "
