...
public class Screen {
  ...
  private static void cycleAlpha(
    NSWindow w,
    float start,
    float end,
    final int steps
    ...
    System.out.println(
      "cycleAlpha " + start + " -> " + end + " in " + steps + " steps"
    );
    float alpha;
    ...
    w.setAlphaValue(end);
    System.out.println("cycleAlpha complete");
  }
  ...
  private static NSWindow getFullScreenWindow() {
    if (sFullScreen == null) {
      ...
      sFullScreen =
        new NSWindow(
          new NSRect(0, 0, screen.width, screen.height),
          0,
          NSWindow.Buffered,
          true
        );
   ...
   *
   *                     Not that making a window a child also brings it to the
   *                     front, if it's NOT already at the front, java will have
   *                     no idea the window is visible, and it won't be pretty.
   *
   *                     This code is very specific to the frame title used in
   *                     VUE.
   */
  public static void keepWindowsOnTop(
    String ensuredWindow,
    boolean fullScreen
  ) {
    // dumpWindows();
    ...
    if (true || MainWindow == null || FullWindow == null) {
      for (int i = 0; i < windows.count(); i++) {
        ...
        if (w.title().startsWith("VUE-FULL-WORKING")) FullWindow = w; else if (
          w.title().startsWith("VUE:")
        ) MainWindow = w;
      }
    }
    for (int i = 0; i < windows.count(); i++) {
      w = (NSWindow) windows.objectAtIndex(i);
      if (
        w == MainWindow || w == FullWindow || w.title().startsWith("OLD-")
      ) continue;
      // Ordering also forces the window visible! (and doesn't tell java, of course)
      // Even when checking if visible mac java impl is putting other frames them
      // right back
      ...
      if (
        (ensuredWindow != null && ensuredWindow.equals(w.title())) ||
        (w.isVisible() && w.title().length() > 1)
      ) {
        if (fullScreen && FullWindow != null) {
          System.out.println(
            "ADDING AS CHILD OF FULL-SCREEN: #" + i + " [" + w.title() + "]"
          );
          FullWindow.addChildWindow(w, NSWindow.Above);
        } else {
          System.out.println(
            "ADDING AS CHILD OF MAIN-WINDOW: #" + i + " [" + w.title() + "]"
          );
  ...
  public static void main(String args[]) {
    ...
    fadeToBlack();
    if (args.length > 0) w.show(); // leaves us in AWT event loop at end of main instead of exiting
    // goBlack();
    showColorPicker();
  ...
  public static void x_main(String args[]) {
    ...
    final NSWindow nsw = new NSWindow(
      new NSRect(0, 0, 1600, 1024),
    ...
    new Thread() {
      public void run() {
        ...
        while (true) {
          if (cycle) {
            if (black) nsw.setBackgroundColor(
              NSColor.whiteColor()
            ); else nsw.setBackgroundColor(NSColor.blackColor());
            nsw.setAlphaValue(0);
