...
class LWCInfoPanel
  ...
  private void addLabelTextRows(
    ...
    for (int i = 0; i < num; i += 2) {
      ...
      if (labelAbove) c.insets = new Insets(0, 0, 0, 0); else c.insets =
        new Insets(0, 0, 1, 0);
      c.weightx = 1.0;
      ...
      if (readOnly) {
        ...
        if (b != null) {
          ...
          System.out.println(
            "LWCInfoPanel: got border insets " + borderInsets + " for " + field
          );
  ...
  public void LWCChanged(LWCEvent e) {
    ...
    if (e.key == LWKey.Deleting) {
      this.lwc = null;
      setAllEnabled(false);
    } else if (e.getSource() != this) loadItem(this.lwc);
  }
  ...
  public void activeChanged(ActiveEvent e, LWComponent c) {
    if (c == null) setAllEnabled(false); else loadItem(c);
  }
  ...
  private void loadItem(LWComponent lwc) {
    ...
    if (r != null) loadText(resourceField, r.getSpec()); else loadText(
      resourceField,
      ""
    );
    ...
    if (c.getResource() != null) {
      ...
      if (properties != null) {
        if (
          c.getResource().getClientType() == Resource.ASSET_FEDORA
        ) propertiesEditor.setProperties(
          properties,
          false
        ); else propertiesEditor.setProperties(properties, true);
      }
    } else {
  ...
  public void updateLWComponent(String text, Object src, LWComponent c) {
    try {
      boolean set = true;
      if (src == labelField) c.setLabel(text); else if (
        src == resourceField
      ) c.setResource(text); else set = false;
      if (set) VUE.getUndoManager().mark(); else return;
    } catch (Exception ex) {
      System.err.println(ex);
      System.err.println(
        "LWCInfoPanel: error setting property value [" + text + "] on " + src
      );
    }
  }
