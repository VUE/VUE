...
import tufts.vue.CabinetNode;
import tufts.vue.CabinetResource;
import tufts.vue.DEBUG;
import tufts.vue.FavoritesNode;
import tufts.vue.LocalFileDataSource;
import tufts.vue.MapResource;
import tufts.vue.Resource;
import tufts.vue.ResourceNode;
import tufts.vue.VUE;
import tufts.vue.VueDandDTree;
import tufts.vue.VueDragTree;
import tufts.vue.VueDragTree.VueDragTreeCellRenderer;
import tufts.vue.VueResources;
import tufts.vue.VueUtil;

/**
 *
 * @version $Revision: 1.1 $ / $Date: 2009-08-09 15:08:15 $ / $Author: mike $
 * @author  mikek
 */
public class ZoteroDandDTree extends VueDragTree implements DropTargetListener {
  ...
  public ZoteroDandDTree(ResourceNode root) {
    super(root);
  ...
  public void drop(DropTargetDropEvent e) {
    ...
    if (debug) System.out.println(
      "drop: found " + dataFlavors.length + dataFlavors.toString()
    );
    try {
      ...
      ex.printStackTrace();
      System.err.println(
        "TRANSFER: Transfer lied about supporting " + foundFlavor
      );
      ...
      ex.printStackTrace();
      System.err.println(
        "TRANSFER: Transfer data did not match declared type! flavor=" +
        foundFlavor +
        " data=" +
        foundData.getClass()
      );
    ...
    if (dropLocation.x < 4) {
      ...
      (this.getPathForLocation(dropLocation.x, dropLocation.y) == null)
    ) {
      rootNode = (ResourceNode) model.getRoot();
      System.out.println("loc" + "x" + dropLocation.x + "y" + dropLocation.y);
    } else {
      rootNode =
        (ResourceNode) this.getPathForLocation(dropLocation.x, dropLocation.y)
      ...
      if (rootNode.getParent() == tufts.vue.VueDragTree.oldnode) {
        System.out.println(
          "Cannot move a parent node into a child.. Can cause infinite loops"
        );
        return;
      }
    }
    if (rootNode.getResource().getClientType() == FAVORITES) {
      if (resourceList != null) {
        ...
        while (iter.hasNext()) {
          ...
          try {
            final LocalFilingManager manager =
              LocalFileDataSource.getLocalFilingManager();
            osid.shared.Agent agent = null;
            LocalCabinet cab = LocalCabinet.instance(
            ...
            if (entry instanceof RemoteCabinetEntry) cabNode =
              new CabinetNode(res, CabinetNode.REMOTE); else cabNode =
              new CabinetNode(res, CabinetNode.LOCAL);
            this.setRootVisible(true);
        ...
        ResourceNode newNode = new ResourceNode(
          Resource.getFactory().get(droppedText)
        );
        this.setRootVisible(true);
        model.insertNodeInto(newNode, rootNode, (rootNode.getChildCount()));
        this.expandPath(new TreePath(rootNode.getPath()));
      ...
      VueUtil.alert(
        null,
        VueResources.getString("dialog.resourceadd.message"),
        VueResources.getString("dialog.resourceadd.title")
      );
      //.dropComplete(false);
    }
  }
  ...
  class VueTreeModelListener implements TreeModelListener {
    ...
    public void treeNodesChanged(TreeModelEvent e) {
      ResourceNode node;
      node = (ResourceNode) (e.getTreePath().getLastPathComponent());
      ...
      try {
        ...
        node = (ResourceNode) (node.getChildAt(index));
  ...
  class VueDandDTreeCellRenderer extends VueDragTreeCellRenderer {
    ...
    private final Dimension imageIconPrefSize = new Dimension(
      Short.MAX_VALUE,
      32 + 3
    );
    private final Dimension defaultPrefSize = new Dimension(
      Short.MAX_VALUE,
      20
    ); // todo: common default for VueDragTree

    private final Border lineBorder = new MatteBorder(
      1,
      0,
      0,
      0,
      tufts.vue.ui.ResourceList.DividerColor
    );
    ...
    public VueDandDTreeCellRenderer(VueDragTree pTree) {
      super(pTree);
      setAlignmentY(0.5f);
    ...
    public Dimension getPreferredSize() {
      if (hasImageIcon) return imageIconPrefSize; else return defaultPrefSize;
    }
    ...
    public Component getTreeCellRendererComponent(
      ...
      if (level <= 1 && r != null) {
        ...
        if (icon != null) {
          hasImageIcon = true;
          superGetTreeCellRendererComponent(
            tree,
            value,
            sel,
            expanded,
  ...
  public void addResource(Resource resource) {
    ...
    ((DefaultTreeModel) getModel()).insertNodeInto(
        ...
        ((DefaultMutableTreeNode) rootNode).getChildCount()
      );
    this.expandPath(new TreePath(rootNode.getPath()));
  }
}
