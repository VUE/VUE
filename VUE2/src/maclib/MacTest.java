...
public class MacTest extends MacOSX {
  ...
  static void test_movie(String args[]) {
    ...
    String viewHex = viewStr.substring(
      viewStr.indexOf("0x") + 2,
      viewStr.length() - 1
    );
      ...
      public void run() {
        ...
        while (true) {
          ...
          for (alpha = 0.1; alpha <= 1; alpha += 0.02) {
            ...
            try {
              sleep(10);
            } catch (Exception e) {} // give CPU a break
          }
          out("alpha=" + (float) alpha);
  ...
  public static class NoticeListener {
    ...
    // new String().getClass() });
    private static NSSelector selector = new NSSelector(
      "notice",
    ...
    public static void addObserver(Object toWatch, String message) {
      System.err.println(
        "Adding observer of " + toWatch + " for messages: " + message
      );
      Object listener = new NoticeListener();
      KEEP.add(listener);
      NSNotificationCenter
        .defaultCenter()
        .addObserver(listener, getSelector(), message, toWatch);
    ...
    public void notice(NSNotification n) {
      if (
        !n.name().equals(NSWindow.WindowDidUpdateNotification)
    ...
    public static void test() {
      try {
        ...
        selector.invoke(
          new NoticeListener(),
          new Object[] { new NSNotification("TestNotice", "AnObject") }
        );
      } catch (Exception e) {
        e.printStackTrace();
        System.exit(0);
  ...
  public static class MacPanel extends NSPanel {
    ...
    MacPanel(boolean textured) {
      super(
        new NSRect(100 * cnt, 100 * cnt, 200, 100),
        NSWindow.TitledWindowMask +
        NSWindow.ClosableWindowMask +
      ...
      awtNS.setBackgroundColor(
        NSColor.whiteColor().colorWithAlphaComponent(0.02f)
      );
    ...
    public void testInvoke() {
      ...
      NSSelector method2 = new NSSelector(
        "test",
        new Class[] { new String().getClass() }
      );
      NSSelector method3 = new NSSelector(
        "windowNotice",
        new Class[] { new String().getClass() }
      );
    ...
    public void setFrame(
      com.apple.cocoa.foundation.NSRect rect,
      boolean display,
      boolean animate
    ) {
    ...
    public void setFrame(
      final com.apple.cocoa.foundation.NSRect r,
      boolean display
    ) {
