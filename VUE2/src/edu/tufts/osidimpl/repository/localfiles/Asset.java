...
public class Asset implements org.osid.repository.Asset {
  ...
  private org.osid.shared.Type recordStructureType = new Type(
    "edu.tufts",
    "recordStructure",
    "file",
    "File Record"
  ...
  private CabinetEntry cabinetEntry = null;
  private String home = VUE.getSystemProperty("user.home");
  private int homeLength = home.length() + 1;
  private String volume = null;
  private int volumeLength = 0;

  protected Asset(CabinetEntry cabinetEntry, org.osid.shared.Id repositoryId)
    throws org.osid.repository.RepositoryException {
    try {
      this.cabinetEntry = cabinetEntry;
      this.displayName = cabinetEntry.getDisplayName();
      ...
      if (this.displayName.startsWith(".")) {
        // do nothing, but maybe we want to type these differently;
      } else if (displayName.startsWith(this.home)) {
        try {
          this.displayName = displayName.substring(this.homeLength);
        } catch (Exception ex) {}
      } else if (displayName.startsWith(this.volume)) {
        try {
          this.displayName = displayName.substring(this.volumeLength);
        } catch (Exception ex) {}
      }

      String lastAccessedTime = cabinetEntry
        .getLastAccessedTime()
        .getTime()
        .toString();
      String lastModifiedTime = cabinetEntry
        .getLastModifiedTime()
        .getTime()
        .toString();
      String createdTime = cabinetEntry.getCreatedTime().getTime().toString();

      org.osid.repository.Record record = createRecord(
        RecordStructure.getInstance().getId()
      );
      org.osid.repository.Part part = record.createPart(
        LastAccessedTimePartStructure.getInstance().getId(),
        lastAccessedTime
      );
      part =
        record.createPart(
          LastModifiedTimePartStructure.getInstance().getId(),
          lastModifiedTime
        );
      part =
        record.createPart(
          CreatedTimePartStructure.getInstance().getId(),
          createdTime
        );

      String url = "";
      //  Check for each of the four possible cases.
      if (cabinetEntry instanceof tufts.oki.remoteFiling.RemoteByteStore) url =
        ((RemoteByteStore) cabinetEntry).getUrl();
      if (cabinetEntry instanceof tufts.oki.remoteFiling.RemoteCabinet) url =
        ((RemoteCabinet) cabinetEntry).getUrl();
      if (cabinetEntry instanceof tufts.oki.localFiling.LocalByteStore) url =
        ((LocalByteStore) cabinetEntry).getUrl();
      if (cabinetEntry instanceof tufts.oki.localFiling.LocalCabinet) url =
        ((LocalCabinet) cabinetEntry).getUrl();
      part = record.createPart(URLPartStructure.getInstance().getId(), url);

      //System.out.println(displayName);
      this.description = displayName; // note no description provided by the Filing OSID
      osid.shared.Id oldStyleId = cabinetEntry.getId();
      this.id = Utilities.getIdManager().getId(oldStyleId.getIdString());
  ...
  public void removeAsset(org.osid.shared.Id assetId, boolean includeChildren)
    throws org.osid.repository.RepositoryException {
    throw new org.osid.repository.RepositoryException(
      org.osid.OsidException.UNIMPLEMENTED
    );
  ...
  public org.osid.repository.Record createRecord(
  ...
  public void copyRecordStructure(
    org.osid.shared.Id assetId,
    org.osid.shared.Id recordStructureId
  ) throws org.osid.repository.RepositoryException {
    throw new org.osid.repository.RepositoryException(
  ...
  public org.osid.repository.RecordIterator getRecordsByRecordStructure(
    org.osid.shared.Id recordStructureId
  ) throws org.osid.repository.RepositoryException {
    if (recordStructureId == null) {
      throw new org.osid.repository.RepositoryException(
  ...
  public org.osid.repository.PartIterator getPartByPart(
  ...
  public org.osid.shared.ObjectIterator getPartValueByPart(
    org.osid.shared.Id partStructureId
  ) throws org.osid.repository.RepositoryException {
    java.util.Vector results = new java.util.Vector();
    org.osid.repository.PartIterator partIterator = getPartByPart(
      partStructureId
    );
    while (partIterator.hasNextPart()) {
  ...
  public org.osid.shared.ObjectIterator getPartValuesByPartStructure(
    ...
    try {
      java.util.Vector results = new java.util.Vector();
      org.osid.repository.PartIterator partIterator = getPartsByPartStructure(
        partStructureId
      );
      while (partIterator.hasNextPart()) {
        org.osid.repository.Part part = partIterator.nextPart();
