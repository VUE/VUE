...
implements org.osid.repository.Asset
{
    ...
    private org.osid.shared.Type recordStructureType = new Type("edu.tufts","recordStructure","file","File Record");
    ...
    protected Asset(CabinetEntry cabinetEntry,
    ...
    throws org.osid.repository.RepositoryException
    {
		try {
			...
			String lastAccessedTime = cabinetEntry.getLastAccessedTime().getTime().toString();
			String lastModifiedTime = cabinetEntry.getLastModifiedTime().getTime().toString();
			...
			org.osid.repository.Part part = record.createPart(LastAccessedTimePartStructure.getInstance().getId(), lastAccessedTime);
			part = record.createPart(LastModifiedTimePartStructure.getInstance().getId(), lastModifiedTime);
			part = record.createPart(CreatedTimePartStructure.getInstance().getId(), createdTime);
			
			String url = "";
            //  Check for each of the four possible cases.
            if (cabinetEntry instanceof tufts.oki.remoteFiling.RemoteByteStore)
                url = ((RemoteByteStore)cabinetEntry).getUrl();
            if (cabinetEntry instanceof tufts.oki.remoteFiling.RemoteCabinet)
                url = ((RemoteCabinet)cabinetEntry).getUrl();
            if (cabinetEntry instanceof tufts.oki.localFiling.LocalByteStore)
                url = ((LocalByteStore)cabinetEntry).getUrl();
            if (cabinetEntry instanceof tufts.oki.localFiling.LocalCabinet)
                url = ((LocalCabinet)cabinetEntry).getUrl();
