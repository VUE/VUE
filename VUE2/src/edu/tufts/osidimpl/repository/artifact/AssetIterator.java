...
public class AssetIterator implements org.osid.repository.AssetIterator {
  ...
  private org.osid.shared.Id recordStructureId = RecordStructure
    .getInstance()
    .getId();
  ...
  protected AssetIterator(String query, org.osid.shared.Id repositoryId)
    ...
    try {
      ...
      for (int i = 0, size = hitList.size(); i < size; i++) {
        ...
        org.osid.repository.Asset asset = new Asset(
          hit.getTitle(),
          "",
          Utilities.getIdManager().createId(),
          repositoryId
        );
        org.osid.repository.Record record = asset.createRecord(
          this.recordStructureId
        );

        record.createPart(
          ArtifactPartStructure.getInstance().getId(),
          hit.getArtifact()
        );
        record.createPart(
          ThumbnailPartStructure.getInstance().getId(),
          hit.getThumb()
        );
        record.createPart(
        ...
        if (hit.artistList != null && hit.artistList.size() > 0) {
          for (String artist : hit.getArtistList()) {
            record.createPart(
              ArtistPartStructure.getInstance().getId(),
              artist
            );
          }
        }
        record.createPart(
          CulturePartStructure.getInstance().getId(),
          hit.culture
        );
        record.createPart(
        ...
        if (hit.materialList != null && hit.materialList.size() > 0) {
          for (String material : hit.materialList) {
            record.createPart(
              MaterialPartStructure.getInstance().getId(),
              material
            );
          }
        }
        record.createPart(
          OriginPartStructure.getInstance().getId(),
          hit.origin
        );
        record.createPart(
          PeriodPartStructure.getInstance().getId(),
          hit.period
        );
        record.createPart(
  ...
  public ArtifactResult loadArtifactResult(String query) {
    try {
      ...
      ArtifactResult artifactResult = (ArtifactResult) unmarshaller.unmarshal(
