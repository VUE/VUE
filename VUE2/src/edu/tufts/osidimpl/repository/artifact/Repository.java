...
public class Repository implements org.osid.repository.Repository {
  ...
  private org.osid.shared.Type artifactSearchType = new Type(
    "tufts.edu",
    "search",
    "artifact"
  );
  ...
  private org.osid.shared.Type artifactMultiFieldSearchType = new Type(
    "tufts.edu",
    "search",
    "artifact-multifield"
  );
  ...
  private static final String SEARCH_URL_PREFIX =
    DEFAULT_ADDRESS + "vue_xml/search2.asp?searchType=Any&max_return=10&query=";
  ...
  public org.osid.repository.AssetIterator getAssetsBySearch(
    ...
    try {
      ...
      if (searchType.isEqual(this.artifactSearchType)) {
        ...
        searchType.isEqual(this.keywordSearchType) && this.address != null
      ) {
  ...
  private String xmlCriteriaToStringUrl(String criteria) throws Exception {
    ...
    Document doc = factory
      .newDocumentBuilder()
      .parse(new StringBufferInputStream(criteria));
    ...
    for (int i = 0; i < list.getLength(); i++) {
      ...
      if (typeNode.getFirstChild() instanceof Text) {
        ...
        if (key.equalsIgnoreCase("keywords")) key = ""; else key =
          "&" + key + "=";
      }
  ...
  public org.osid.repository.RecordStructureIterator getRecordStructuresByType(
    ...
    if (
      recordStructureType.isEqual(
        new Type("tufts.edu", "recordStructure", "artifact")
      )
    ) {
      java.util.Vector results = new java.util.Vector();
      results.addElement(RecordStructure.getInstance());
