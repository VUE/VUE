...
public class VUERecordStructure implements org.osid.repository.RecordStructure {
  ...
  public static final String[] dcFields = {
    "title",
    "creator",
    "subject",
    "date",
  ...
  protected VUERecordStructure(Repository repository)
    ...
    for (int i = 0; i < Repository.DC_FIELDS.length; i++) {
      org.osid.repository.PartStructure metadataElementPartStructure =
        new MetadataElementPartStructure(
          Repository.DC_FIELDS[i],
          this,
          repository,
          new Type("tufts.edu", "partStructure", Repository.DC_FIELDS[i])
        );
  ...
  public org.osid.repository.PartStructure getMetadataElementPartStructure(
    String element
  ) throws org.osid.repository.RepositoryException {
    ...
    try {
      while (i.hasNext()) {
        org.osid.repository.PartStructure partStructure =
          (org.osid.repository.PartStructure) i.next();
        if (
          element.equals(partStructure.getId().getIdString())
  ...
  public static Record createVUERecord(
    String pid,
    ...
    try {
      ...
      if (
        ...
        record.createPart(
          recordStructure.getVUEDefaultViewPartStructure().getId(),
          Utilities.formatObjectUrl(
            objectId.getIdString(),
            repository.getFedoraProperties().getProperty("assetDef.fullView"),
            repository
          )
        );
      } else {
      ...
      if (
        ...
        String url = Utilities.formatObjectUrl(
          objectId.getIdString(),
          repository.getFedoraProperties().getProperty("dissemination.dc"),
          repository
        );
        ...
        if (dcUrl != null) {
          ...
          for (int i = 0; i < Repository.DC_FIELDS.length; i++) {
            NodeList list = document.getElementsByTagName(
              Repository.DC_NAMESPACE + dcFields[i]
            );
            if (list != null && list.getLength() != 0) {
              // only picks the first element
              if (list.item(0).getFirstChild() != null) record.createPart(
                recordStructure
                  .getMetadataElementPartStructure(Repository.DC_FIELDS[i])
                  .getId(),
