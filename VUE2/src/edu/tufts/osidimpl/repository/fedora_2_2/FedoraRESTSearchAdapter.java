...
public class FedoraRESTSearchAdapter {
  ...
  public static final String SEARCH_STRING =
    "/fedora/search?pid=true&title=true&xml=true&cModel=true&description=true&maxResults=100&terms=";
  public static final String SEARCH_ADVANCED =
    "/fedora/search?pid=true&title=true&xml=true&cModel=true&description=true&maxResults=100&query=";
  public static final String SEARCH_RESUME =
    "/fedora/search?xml=true&sessionToken=";
  ...
  public static org.osid.repository.AssetIterator search(
    ...
    try {
      ...
      if (lSearchCriteria.getSearchOperation() == SearchCriteria.FIND_OBJECTS) {
        URL url = new URL(
          ...
          SEARCH_STRING +
          URLEncoder.encode(
            lSearchCriteria.getKeywords() + WILDCARD,
            "ISO-8859-1"
          )
        );
        ...
        fieldNode =
          (NodeList) xPath.evaluate(
            "/pre:result/pre:resultList/pre:objectFields",
            inputSource,
  ...
  public static org.osid.repository.AssetIterator advancedSearch(
    Repository repository,
    SearchCriteria lSearchCriteria
  ) throws org.osid.repository.RepositoryException {
    try {
  ...
  public static String getQueryFromConditions(Condition[] conds)
    throws org.osid.repository.RepositoryException {
    try {
      ...
      throw wrappedException(
        "FedoraRESTSearchAdapter.getQueryFromConditions",
        t
      );
    }
  }

  private static org.osid.repository.AssetIterator getAssetIterator(
    Repository repository,
    NodeList fieldNode
  ) throws org.osid.repository.RepositoryException {
    ...
    try {
      ...
      for (int i = 0; i < fieldNode.getLength(); i++) {
        ...
        resultList.add(
          new Asset(repository, pid, title, repository.getAssetType(cModel))
        );
      }
  ...
  public static List<String> getDataStreams(String dSUrl)
    throws org.osid.repository.RepositoryException {
    ...
    try {
      ...
      XPath xPath = factory.newXPath();
      InputSource inputSource = new InputSource(url.openStream());
      NodeList dSNodes = (NodeList) xPath.evaluate(
        "/objectDatastreams/datastream",
        inputSource,
        XPathConstants.NODESET
      );
      for (int i = 0; i < dSNodes.getLength(); i++) {
        ...
        dataStreams.add(
          n.getAttributes().getNamedItem(DS_ID_ATTRIBUTE).getNodeValue()
        );
      }
    } catch (Throwable t) {
  ...
  private static org.osid.repository.RepositoryException wrappedException(
    String method,
    Throwable cause
  ) {
    cause.printStackTrace();
    org.osid.repository.RepositoryException re =
      new org.osid.repository.RepositoryException(
        "FedoraRESTSearchAdapter." + method + "; cause is " + cause
      );
    re.initCause(cause);
    return re;
  }
  ...
  public static void testSearch() throws Exception {
    URL url = new URL(
      "http://dl.tufts.edu:8080/" +
      SEARCH_STRING +
      URLEncoder.encode("street", "ISO-8859-1")
    );
    ...
    inputSource = new InputSource(url.openStream());
    NodeList fieldNode = (NodeList) xPath.evaluate(
      "/pre:result/pre:resultList/pre:objectFields",
      inputSource,
      XPathConstants.NODESET
    );
...
class FedoraNamespaceContext implements NamespaceContext {
  ...
  public String getNamespaceURI(String prefix) {
    if (prefix == null) throw new NullPointerException("Null prefix"); else if (
      "pre".equals(prefix)
    ) return "http://www.fedora.info/definitions/1/0/types/"; else if (
      "xml".equals(prefix)
