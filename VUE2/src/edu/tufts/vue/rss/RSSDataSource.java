...
public class RSSDataSource extends BrowseDataSource {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(RSSDataSource.class);
  ...
  public RSSDataSource(String displayName, String address)
  ...
  private JComponent loadContentAndBuildViewer() {
    ...
    if (mItems.size() == 0) throw new DataSourceException("[Empty RSS feed]");
    ...
    if (DEBUG.Enabled) {
      if (DEBUG.WORK) {
        ...
        fr.addPropertyIfContent(
          "feed-supported-types",
      ...
      for (Map.Entry<String, List<String>> e : headers.entrySet()) {
        ...
        if (
          value instanceof Collection && ((Collection) value).size() == 1
        ) value = ((Collection) value).toArray()[0];
        if (e.getKey() == null) fr.setProperty(
          "HTTP-response",
          value
        ); else fr.setProperty("HTTP:" + e.getKey(), value);
      }
    }
    ...
    if (
      "This file is an XML representation of some issues".equalsIgnoreCase(desc)
    ) // ignore JIRA standard meaningless comment
    ((URLResource) fr).setSpec(getAddress()); else fr.addPropertyIfContent(
      "Description",
      desc
    );
    ...
    for (SyndEntry item : mItems) {
      ...
      if (r == null) {
        Log.warn(
          "failed to create resource from rss feed item, skipping; link=" +
          item.getLink()
        );
        continue;
      }
