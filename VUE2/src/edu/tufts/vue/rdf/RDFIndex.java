...
public class RDFIndex extends com.hp.hpl.jena.rdf.model.impl.ModelCom {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(RDFIndex.class);
  ...
  public static final String INDEX_FILE =
    ...
    File.separator +
    VueResources.getString("rdf.index.file");
  ...
  public static final String VUE_ONTOLOGY =
    Constants.ONTOLOGY_URL + ONT_SEPARATOR;
  ...
  private static final String VUE_GENERAL_NAMESPACE =
    VueResources.getString("metadata.vue.url") + ONT_SEPARATOR;
  ...
  private static final String VUE_OSID_UNKNOWN_NAMESPACE =
    VUE_OSID_PREFIX + "#";

  /** i.e.: http://vue.tufts.edu/vue.rdfs#none */
  public static final String VueTermOntologyNone =
    VUE_GENERAL_NAMESPACE + "none";
  ...
  static {
    ...
    Log.info(
      Util.tags(VueTermOntologyNone) +
      " <- ONTOLOGY_NONE " +
      Util.tag(VueTermOntologyNone)
    );
  }
  ...
  final com.hp.hpl.jena.rdf.model.Property _propertyNone = createProperty(
    VUE_GENERAL_NAMESPACE,
    "none"
  );

  final com.hp.hpl.jena.rdf.model.Property idOf = createProperty(
    VUE_ONTOLOGY,
    Constants.ID
  );
  final com.hp.hpl.jena.rdf.model.Property labelOf = createProperty(
    VUE_ONTOLOGY,
    Constants.LABEL
  );
  final com.hp.hpl.jena.rdf.model.Property childOf = createProperty(
    VUE_ONTOLOGY,
    Constants.CHILD
  );
  final com.hp.hpl.jena.rdf.model.Property authorOf = createProperty(
    VUE_ONTOLOGY,
    Constants.AUTHOR
  );
  final com.hp.hpl.jena.rdf.model.Property colorOf = createProperty(
    VUE_ONTOLOGY,
    Constants.COLOR
  );
  final com.hp.hpl.jena.rdf.model.Property notesOf = createProperty(
    VUE_ONTOLOGY,
    Constants.NOTES
  );

  final com.hp.hpl.jena.rdf.model.Property contentPropertyOf = createProperty(
    VUE_ONTOLOGY,
    Constants.CONTENT_INFO_PROPERTY
  );
  ...
  public Collection<LWComponent> decodeVueResults(Collection<URI> results) {
    final Collection<LWComponent> hits = new ArrayList<LWComponent>(
      results.size() + 2
    );
    for (URI uri : results) {
      final LWComponent c = vueComponentMap.get(uri);
      if (c != null) hits.add(c); else Log.error(
        "*** Internal error: couldn't find URI in results-index: " +
        uri +
        "; " +
        Util.tags(vueComponentMap)
      );
    }
    return hits;
  }
  ...
  public void indexAdd(
    ...
    final boolean searchEverything_IS_IGNORED
  ) {
    if (DEBUG.Enabled && searchEverything_IS_IGNORED) Log.debug(
      "Note: \"search-everything\" bit is now ignored."
    );
    ...
    if (DEBUG.RDF) Log.debug(
      "indexAdd: begin; freeMem=: " + Runtime.getRuntime().freeMemory()
    );

    final com.hp.hpl.jena.rdf.model.Resource mapRoot =
      this.createResource(map.getURI().toString());

    if (DEBUG.RDF) Log.debug(
      "index: create resource for map; freeMem=: " +
    ...
    try {
      ...
      if (map.hasLabel()) addProperty(mapRoot, labelOf, map.getLabel());

      if (DEBUG.RDF) Log.debug(
        "index: added properties for map; freeMem=" +
        Runtime.getRuntime().freeMemory()
      );
      ...
      for (LWComponent c : searchSet) {
        if (c instanceof LWPathway || c instanceof LWMap.Layer) continue;
        if (!INDEX_SLIDES && c instanceof LWSlide) continue;
        ...
        if (size() > MAX_SIZE) {
          Log.warn(
            "Maximum fail-safe search capacity reached: not all nodes will be searchable. (See property rdf.index.size)"
          );
          break;
        }
      }
      ...
      if (DEBUG.RDF) Log.debug(
        "index: after indexing all components; freeMem=" +
        Runtime.getRuntime().freeMemory()
      );
    } catch (Exception ex) {
      Log.error("index", ex);
  ...
  public Collection<URI> searchWithSPARQL(final String queryString) {
    ...
    if (DEBUG.SEARCH) Log.debug(
      "created QEF " + qe + "; memory=" + Runtime.getRuntime().freeMemory()
    );

    final ResultSet results = qe.execSelect();
    if (DEBUG.SEARCH) Log.debug(
      "execSelect returned; memory=" + Runtime.getRuntime().freeMemory()
    );
    ...
    while (results.hasNext()) {
      ...
      if (DEBUG.SEARCH) {
        final String qss = qs
          .toString()
          .replaceAll("<http://vue.tufts.edu", "..."); // shorten debug output
        Log.debug(
          "qSol " +
          String.format(
            "%.190s%s",
            qss,
            qss.length() > 190 ? ("...x" + qss.length()) : ""
          )
        );
      }
      if (false) {
  ...
  public Collection<URI> searchAllValues(String substring) {
    if (DEBUG.SEARCH || DEBUG.RDF) Log.debug(
      "searchAllValues:   " + Util.tags(substring)
    );
  ...
  private static final boolean RDFIZE_COLOR = VueResources.getBool(
    "rdf.rdfize.color"
  );
  ...
  private void load_VUE_component_to_RDF_index(
    ...
    final com.hp.hpl.jena.rdf.model.Resource r =
      this.createResource(uri.toString());
    ...
    if (includeNodeData) {
      ...
      //------------------------------------------------------------------
      if (component.hasLabel()) addProperty(r, labelOf, component.getLabel());

      if (component.hasNotes()) addProperty(r, notesOf, component.getNotes());

      if (RDFIZE_COLOR && component.getXMLfillColor() != null) addProperty(
        r,
        colorOf,
        component.getXMLfillColor()
      );
        ...
        if (INDEX_VUE_RESOURCE_KEYWORDS) {
          ...
          if (osid != null) {
            if (osid.endsWith(".Repository")) {
              ...
              osidNS =
                VUE_OSID_PREFIX +
                osid.substring(osid.lastIndexOf('.') + 1) +
                "#";
            } else {
              // note: could use provider id...  probably even more unique than impl class
              Log.warn(
                "Couldn't understand @osid.impl: " + Util.tags(osidProp)
              );
            }
          ...
          if (value != null) {
            ...
            if (INDEX_VUE_RESOURCE_KEYWORDS) {
              ...
              if (!tufts.vue.Resource.isHiddenPropertyKey(key.toString())) {
                addProperty(
                  r,
                  createProperty(osidNS, key.toString()),
                  strValue
                );
              }
            } else {
    ...
    final List<VueMetadataElement> metadata = component
      .getMetadataList()
      .getMetadata();
  ...
  public void addStatement(com.hp.hpl.jena.rdf.model.Statement statement) {
    if (size() < MAX_SIZE) {
      ...
      throw new RuntimeException(
        "Size of index: " + size() + " exceeds MAX_SIZE: " + MAX_SIZE
      );
    }
  }
  ...
  public void regenerate() {
    ...
    for (int i = 0; i < stmtList.size(); i++) {
      com.hp.hpl.jena.rdf.model.Statement statementI =
        (com.hp.hpl.jena.rdf.model.Statement) stmtList.get(i);
      for (int j = i + 1; j < stmtList.size(); j++) {
        com.hp.hpl.jena.rdf.model.Statement statementJ =
          (com.hp.hpl.jena.rdf.model.Statement) stmtList.get(j);
  ...
  private boolean compareStatements(
    com.hp.hpl.jena.rdf.model.Statement stmt1,
    com.hp.hpl.jena.rdf.model.Statement stmt2
  ) {
    if (
      !stmt1.getSubject().toString().equals(stmt2.getSubject().toString())
    ) return false; else if (
      !stmt1.getObject().toString().equals(stmt2.getObject().toString())
    ) return false; else if (
      !stmt1.getPredicate().toString().equals(stmt2.getPredicate().toString())
    ) return false; else return true;
  }

  /** This is overriden to add debugging only */
  @Override
  public com.hp.hpl.jena.rdf.model.Property createProperty(
    final String nameSpace,
    final String localName
  ) {
    final com.hp.hpl.jena.rdf.model.Property property = super.createProperty(
      nameSpace,
      localName
    );
    ...
    if (DEBUG.SEARCH && DEBUG.RDF) {
      ...
      if (
        property instanceof com.hp.hpl.jena.rdf.model.impl.PropertyImpl
      ) propName = "PropertyImpl"; else propName =
  ...
  public com.hp.hpl.jena.rdf.model.Property getPropertyFromKey(String key) {
    ...
    if (words.length == 1) {
      ...
      throw new RuntimeException(
        "createPropertyFromKey: The key format is wrong. key - " + key
      );
    } else {
    ...
    try {
      ...
      if (ontEnd != -1) {
        ...
        if (name != encodedName) {
          ...
          if (DEBUG.RDF) Log.debug(
            "encoded " + Util.tags(key) + " to " + Util.tags(encodedKey)
          );
        }
      } else {
        encodedKey = java.net.URLEncoder.encode(key, "UTF-8");
        if (DEBUG.RDF && encodedKey != key) Log.debug(
          "encoded " + Util.tags(key) + " to " + Util.tags(encodedKey)
        );
      }
