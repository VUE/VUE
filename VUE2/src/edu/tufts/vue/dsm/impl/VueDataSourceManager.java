...
public class VueDataSourceManager
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueDataSourceManager.class);
  ...
  private static final File userFolder =
    tufts.vue.VueUtil.getDefaultUserFolder();
  private static final String xmlFilename =
    userFolder.getAbsolutePath() +
    "/" +
    tufts.vue.VueResources.getString("dataSourceSaveToXmlFilename");

  private static final VueDataSourceManager singleton =
    new VueDataSourceManager(true);
  ...
  public synchronized void startRepositoryConfiguration(
    final java.awt.Component clientUI
  ) {
    ...
    for (final edu.tufts.vue.dsm.DataSource ds : dataSources) {
      if (ds instanceof VueDataSource == false) {
        Log.warn(
          "unhandled DataSource impl, cannot configure: " + Util.tags(ds)
        );
    ...
    for (final edu.tufts.vue.dsm.DataSource ds : dataSources) {
      if (ds instanceof VueDataSource == false) continue;
        ...
        public void run() {
          try {
            try {
              ...
              if (tufts.vue.DEBUG.DR) Log.info(
                String.format(
                  "configured %-33s %s",
                  '"' + ds.getRepositoryDisplayName() + '"',
              ...
              if (ds.getRepository() == null) notifyDataSourceListeners(
                DS_ERROR,
                ds
              ); else notifyDataSourceListeners(DS_CONFIGURED, ds);
            } catch (Throwable t) {
            ...
            if (clientUI != null) clientUI.repaint();
  ...
  public void remove(org.osid.shared.Id id) {
    ...
    synchronized (this) {
      ...
      try {
        ...
        while (i.hasNext()) {
          ...
          if (id.isEqual(ds.getId())) {
            i.remove();
            if (removed) Log.warn("removed again: " + ds); else Log.info(
              "removed: " + ds
            );
            removed = true;
      ...
      if (removed && !isMarshalling) save();
    }
  }

  /**
   */
  public edu.tufts.vue.dsm.DataSource getDataSource(
    org.osid.shared.Id dataSourceId
  ) {
    try {
      synchronized (this) {
        for (DataSource ds : DataSources) if (
          dataSourceId.isEqual(ds.getId())
        ) return ds;
      }
    } catch (Throwable t) {
  ...
  public org.osid.repository.Repository[] getIncludedRepositories() {
    ...
    synchronized (this) {
      for (DataSource ds : DataSources) if (ds.isIncludedInSearch()) {
        Repository r = ds.getRepository();
        if (r == null) {
          if (tufts.vue.DEBUG.DR) Log.debug(
            "has no repository; not included: " + ds
          );
        } else included.add(r);
      }
    }
  ...
  /**
   */
  public java.awt.Image getImageForRepositoryType(
  ...
  public void addDataSourceListener(
    edu.tufts.vue.dsm.DataSourceListener listener
  ) {
  ...
  public synchronized void notifyDataSourceListeners(
    Object state,
    DataSource changed
  ) {
  ...
  private synchronized void marshall(File file) {
    ...
    for (DataSource ds : marshallingVector) {
      try {
        Log.info(
          "marshalling:  " + ds + "; RepositoryID=" + getRepositoryID(ds)
        );
      } catch (Throwable t) {
  ...
  static final String CRIMSON_ENCODING_EXCEPTION_PREFIX =
    "Declared encoding \"";
  ...
  private static synchronized VueDataSourceManager unMarshalVDSM(File file)
    ...
    try {
      ...
      if (
        e.getCause() instanceof org.xml.sax.SAXParseException &&
        e.getMessage() != null &&
        ...
        try {
          declared =
            e
              .getMessage()
              .substring(CRIMSON_ENCODING_EXCEPTION_PREFIX.length());
          declared = declared.substring(0, declared.indexOf('"'));
          Log.info(
            String.format("Attempting alternate encoding [%s]", declared)
          );
        } catch (Throwable t) {
          Log.error(t);
          throw e;
  ...
  private static VueDataSourceManager unmarshalWithEncoding(
    File file,
    String encoding
  )
    throws java.io.IOException, org.exolab.castor.xml.MarshalException, org.exolab.castor.xml.ValidationException, org.exolab.castor.mapping.MappingException {
    ...
    if (encoding != null) reader =
      new InputStreamReader(new FileInputStream(file), encoding); else reader =
      new InputStreamReader(new FileInputStream(file));

    Log.debug(
      String.format("reader actual encoding name: [%s]", reader.getEncoding())
    );
    ...
    final VueDataSourceManager vdsm =
      (VueDataSourceManager) unmarshaller.unmarshal(inputSource);
...
class PropertyEntryMarshalListener
  ...
  public boolean preMarshal(java.lang.Object object) {
    if (object instanceof tufts.vue.PropertyEntry) {
      ...
      if (pe.getEntryKey().toLowerCase().contains(VueDataSourceManager.PASS)) {
        pe.setEntryValue(
          edu.tufts.vue.util.Encryption.encrypt(pe.getEntryValue().toString())
        );
      }
    }
    return true;
  ...
  public void postMarshal(java.lang.Object object) {
    if (object instanceof tufts.vue.PropertyEntry) {
      ...
      if (pe.getEntryKey().toLowerCase().contains(VueDataSourceManager.PASS)) {
        pe.setEntryValue(
          edu.tufts.vue.util.Encryption.decrypt(pe.getEntryValue().toString())
        );
      }
    }
  }
...
class PropertyEntryUnMarshalListener
