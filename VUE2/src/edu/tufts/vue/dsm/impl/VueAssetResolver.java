...
public class VueAssetResolver implements edu.tufts.vue.dsm.AssetResolver {

  /**
   */
  public edu.tufts.vue.dsm.AssetReference getAssetReference(
    org.osid.shared.Id assetId,
    org.osid.shared.Id repositoryId
  ) {
    try {
      edu.tufts.vue.dsm.DataSourceManager dsm =
        VueDataSourceManager.getInstance();
      edu.tufts.vue.dsm.DataSource[] dataSources = dsm.getDataSources();
      for (int i = 0; i < dataSources.length; i++) {
        if (dataSources[i].getRepositoryId().isEqual(repositoryId)) {
          return new VueAssetReference(
            dataSources[i].getOsidLoadKey(),
            assetId.getIdString()
          );
        }
      }
  ...
  public org.osid.repository.Asset getAsset(
    ...
    try {
      org.osid.id.IdManager idManager = VueOsidFactory
        .getInstance()
        .getIdManagerInstance();
      org.osid.repository.RepositoryManager repositoryManager = VueOsidFactory
        .getInstance()
        .getRepositoryManagerInstance(osidLoadKey);
