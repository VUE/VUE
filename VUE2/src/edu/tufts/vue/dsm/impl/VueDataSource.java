...
public class VueDataSource implements edu.tufts.vue.dsm.DataSource {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DataSourceImp.class);
  ...
  public VueDataSource(
    ...
    try {
      ...
      throw new org.osid.provider.ProviderException(
  ...
  private boolean setProviderValues()
    ...
    this.requestsLicenseAcknowledgement =
      provider.requestsLicenseAcknowledgement();
  ...
  private void setRepositoryManager()
    throws org.osid.provider.ProviderException {
    this.repositoryId =
      edu.tufts.vue.util.Utilities.getRepositoryIdFromLoadKey(this.osidLoadKey);
    this.repositoryManager =
      factory.getRepositoryManagerInstance(this.osidLoadKey);
  }

  private void setRelatedValues() throws org.osid.provider.ProviderException {
    ...
    try {
      ...
      Log.warn(
        String.format(
          "repositoryManager.getRepository(ID) failed; loadKey[%s]; manually searching",
          this.osidLoadKey
        )
      );
      // special case for when the Manager implementation doesn't offer this method
      try {
        org.osid.repository.RepositoryIterator repositoryIterator =
        ...
        while (repositoryIterator.hasNextRepository()) {
          final org.osid.repository.Repository r =
            repositoryIterator.nextRepository();
          ...
          } else Log.info("  no match: " + idString(r.getId()));
        }
        if (found == null) {
          if (last != null) {
            Log.warn(
              "using last: " + idString(last.getId()) + " (no match found)"
            );
            found = last;
          } else Log.error("no match found for " + idString(repositoryId));
        }
    ...
    try {
      ...
      edu.tufts.vue.util.Logger.log(
        t,
        "in method edu.tufts.vue.dsm.VueDataSource calling Repository.getDescription()"
      );
    }
    ...
    try {
      ...
      edu.tufts.vue.util.Logger.log(
        t,
        "in method edu.tufts.vue.dsm.VueDataSource calling Repository.getType()"
      );
    }
    ...
    try {
      ...
      edu.tufts.vue.util.Logger.log(
        t,
        "in method edu.tufts.vue.dsm.VueDataSource calling Repository.getId()"
      );
    }
    ...
    try {
      ...
      edu.tufts.vue.util.Logger.log(
        t,
        "in method edu.tufts.vue.dsm.VueDataSource calling Repository.getAssetTypes()"
      );
    }
    ...
    try {
      ...
      edu.tufts.vue.util.Logger.log(
        t,
        "in method edu.tufts.vue.dsm.VueDataSource calling Repository.getSearchTypes()"
      );
    }
    ...
    try {
      ...
      edu.tufts.vue.util.Logger.log(
        t,
        "in method edu.tufts.vue.dsm.VueDataSource calling Repository.supportsUpdating()"
      );
    }
  }
  ...
  public String getRepositoryDisplayName() {
    if (
      this.repositoryDisplayName == null ||
      this.repositoryDisplayName.trim().length() < 1
    ) {
      if (this.providerDisplayName == null) {
        return "(repository-name-unknown)";
      } else return "(" + providerDisplayName + ": unconfigured)";
      //return "(Name Unknown; Provider: " + providerDisplayName + ")";
    } else return this.repositoryDisplayName;
  }

  public boolean isOnline() {
    if (
      repositoryDisplayName == null || repositoryDisplayName.trim().length() < 1
    ) {
      return false;
    } else {
      return true;
  ...
  public void setProviderIdString(String providerIdString) {
    try {
      ...
      this.providerId =
        edu.tufts.vue.dsm.impl.VueOsidFactory
          .getInstance()
          .getIdManagerInstance()
          .getId(providerIdString);
      if (
        !setProviderValues()
  ...
  public void setDataSourceIdString(String dataSourceIdString) {
    try {
      this.dataSourceId =
        factory.getIdManagerInstance().getId(dataSourceIdString);
      if (
        !setProviderValues()
      ) return; // must come first
      setRepositoryManager();
      ...
      Log.error(
        this + " setDataSourceIdString[" + dataSourceIdString + "];",
        t
      );
    }
  }
  ...
  public String getDataSourceIdString() {
    if (this.dataSourceIdString == null) {
      try {
        ...
        Log.error(
          "loading data sources from XML; dataSourceId[" + dataSourceId + "];",
          t
        );
        //edu.tufts.vue.util.Logger.log(t,"loading data sources from XML");
      }
  ...
  private static Properties getPropertyConfiguration(
    Vector<PropertyEntry> propertyList
  ) {
    ...
    for (PropertyEntry pe : propertyList) {
      properties.put(pe.getEntryKey(), pe.getEntryValue());
    }
