...
public class VueOsidFactory implements edu.tufts.vue.dsm.OsidFactory {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueOsidFactory.class);
  ...
  private static final edu.tufts.vue.dsm.OsidFactory SingletonOsidFactory =
    new VueOsidFactory();
  ...
  private static void initializeVOF()
    ...
    try {
      ...
      edu.tufts.vue.util.Logger.log(
        "Assigning to context: this error should never happen"
      );
    ...
    // }
      ...
      edu.tufts.vue.util.Logger.log(
        "Cannot load ProviderInvocationManager: " + e.getMessage()
      );
      throw new org.osid.provider.ProviderException(
        org.osid.OsidException.CONFIGURATION_ERROR
      );
  ...
  private static void initializeOSIDs() throws OsidException {
    ...
    VueOsidFactory.PCM =
      (org.osid.provider.ProviderControlManager) // tufts.vue.OsidLoader.getManager("org.osid.provider.ProviderControlManager",  // tufts.vue.OsidLoader impl never completed...
      edu.mit.osidimpl.OsidLoader.getManager(
    ...
    Log.info(
      "OsidLoader found ProviderControlManager " +
      PCM +
  ...
  public org.osid.repository.RepositoryManager getRepositoryManagerInstance(
    String osidLoadKey
  ) {
    ...
    try {
      managerImplementation =
        edu.tufts.vue.util.Utilities.getManagerStringFromLoadKey(osidLoadKey);
      //System.out.println("Manager implementation is " + managerImplementation);
      manager =
        (org.osid.repository.RepositoryManager) providerInvocationManager.getManager(
      ...
      edu.tufts.vue.util.Logger.log(
        t,
        "Trying to load Repository Manager in factory with key " + osidLoadKey
      );
    }
    return manager;
  }
  ...
  public org.osid.provider.Provider[] checkRegistryForNew(
    ...
    try {
      ...
      while (providerIterator.hasNextProvider()) {
        ...
        if (index == -1) {
          System.out.println(
            "A new provider is available " + nextProvider.getDisplayName()
          );
          results.addElement(nextProvider);
        }
      }
      int size = results.size();
      org.osid.provider.Provider providers[] =
        new org.osid.provider.Provider[size];
  ...
  public org.osid.provider.Provider[] checkRegistryForUpdates(
    edu.tufts.vue.dsm.DataSource[] dataSources
  ) throws org.osid.provider.ProviderException {
    java.util.Vector results = new java.util.Vector();
    try {
      ...
      ProviderIterator providerIterator =
        providerInstallationManager.getInstalledProvidersNeedingUpdate();
      while (providerIterator.hasNextProvider()) {
        org.osid.provider.Provider nextProvider =
          providerIterator.getNextProvider();
        String providerIdString = nextProvider.getId().getIdString();
        ...
        if (index == -1) {
          System.out.println(
            "A provider update is available " + nextProvider.getDisplayName()
          );
          results.addElement(nextProvider);
        }
      }
