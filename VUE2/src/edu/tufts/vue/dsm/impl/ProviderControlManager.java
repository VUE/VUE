...
public class ProviderControlManager
  ...
  public static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VuePCM.class);
  ...
  @Override
  public void assignConfiguration(java.util.Properties conf)
  ...
  protected void initialize() throws org.osid.OsidException {
    ...
    if (tufts.Util.isMacPlatform()) {
      try {
        ...
        if (runtimeRoot != null) Log.info(
          "Runtime configured OSID provider install dir: " + runtimeRoot
        );
      } catch (Throwable t) {
    ...
    final String installRoot = runtimeRoot == null
      ? getActiveRoot()
      : runtimeRoot;
    ...
    if (oldDir.exists() && oldDir.isDirectory() && oldDir.canRead()) {
      ...
      if (newDir.exists() && newDir.isDirectory() && newDir.canRead()) {
        ...
        if (oldDir.canWrite() && newContents.length > oldContents.length) {
          Log.info(
            "prioritizing new dir over old based on contents: " + NewLibraryDir
          );
      ...
      if (oldDir.canWrite() && oldContents.length > 0) {
        ...
        return null;
      } else if (oldContents.length > 0) Log.warn(
        "Old providers dir exists, but we cannot write to it: " +
        oldContents.length +
        " providers may need re-download for " +
        NewLibraryDir
      );
    }
  ...
  private void inspectProperties() {
    try {
      ...
      final java.io.InputStream in =
        VueOsidFactory.class.getResourceAsStream("/osid.properties");
