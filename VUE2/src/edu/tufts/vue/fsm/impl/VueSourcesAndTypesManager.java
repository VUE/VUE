...
public class VueSourcesAndTypesManager
  ...
  private static edu.tufts.vue.fsm.SourcesAndTypesManager vueSourcesAndTypesManager =
    new edu.tufts.vue.fsm.impl.VueSourcesAndTypesManager();
  ...
  public org.osid.shared.Type[] getSearchTypes(int rule) {
    try {
      org.osid.repository.Repository[] repositories =
        dataSourceManager.getIncludedRepositories();
      ...
      if (rule == edu.tufts.vue.fsm.SourcesAndTypesManager.ALL_TYPES) {
        ...
        for (int i = 0; i < size; i++) {
          types[i] =
            edu.tufts.vue.util.Utilities.stringToType(
        ...
        rule == edu.tufts.vue.fsm.SourcesAndTypesManager.TYPES_IN_COMMON
      ) {
        // start with the first repository
        java.util.Vector vector = new java.util.Vector();
        org.osid.shared.TypeIterator typeIterator =
          repositories[0].getSearchTypes();
        while (typeIterator.hasNextType()) {
          vector.addElement(
