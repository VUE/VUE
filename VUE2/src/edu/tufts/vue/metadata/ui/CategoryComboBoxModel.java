...
public class CategoryComboBoxModel
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(CategoryComboBoxModel.class);
  ...
  /** @interface ListModel */public int getSize() {
    return contents.size();
  }

  /** @interface ListModel */public Object getElementAt(int i) {
    return contents.get(i);
  }

  /** @interface ComboBoxModel */public void setSelectedItem(Object o) {
    selected = o;
  }

  /** @interface ComboBoxModel */public Object getSelectedItem() {
    return selected;
  }
  ...
  private void rebuild(final List list) {
    ...
    for (Ontology ont : VUE_Ontologies) {
      ...
      if (ont.getOntTypes().size() > 0) {
        for (OntType ot : ont.getOntTypes()) list.add(ot);
        list.add(SEPARATOR);
      }
    }
  ...
  public int indexOfCategoryKey(final String key) {
    ...
    if (key.indexOf('#') < 0) {
      ...
      for (Object item : contents) {
        index++;
        if (
          item instanceof OntType && key.equals(((OntType) item).getLabel())
        ) return index;
      }
    } else {
      for (Object item : contents) {
