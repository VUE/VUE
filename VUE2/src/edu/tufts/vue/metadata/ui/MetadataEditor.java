...
public class MetadataEditor
  extends JPanel
  implements
    ActiveListener, MetadataList.MetadataListListener, LWSelection.Listener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MetadataEditor.class);
  ...
  public static final String CC_ADD_LOCATION_MESSAGE =
    "<html>Click + to permanently add this<br>custom category to your own list.";

  //public final static int ROW_HEIGHT = 31;
  public static final int ROW_HEIGHT = 27;
  public static final int CELL_VERT_INSET = 4;
  public static final int CELL_HORZ_INSET = 6; // Left aligns with JTextField editor text in Mac Aqua -- todo: check Windows
  public static final int BUTTON_COL_WIDTH = 27;

  private static final java.awt.Color GRID_COLOR = (DEBUG.BOXES)
    ? Color.black
    : new Color(196, 196, 196);

  public static final Object CC_ADD_LEFT = "left-edge";
  public static final Object CC_ADD_RIGHT = "right-edge";

  public static final Border insetBorder = GUI.makeSpace(
    CELL_VERT_INSET,
    CELL_HORZ_INSET,
    CELL_VERT_INSET,
    CELL_HORZ_INSET / 2
  );
  public static final Border buttonBorder = GUI.makeSpace(
    CELL_VERT_INSET,
    CELL_HORZ_INSET - 1,
    CELL_VERT_INSET,
    CELL_HORZ_INSET - 1
  );
  public static final Border insetBorderNoBottom = GUI.makeSpace(
    CELL_VERT_INSET,
    CELL_HORZ_INSET,
    CELL_VERT_INSET - 1,
    CELL_HORZ_INSET
  );
  public static final Border fullBox = BorderFactory.createMatteBorder(
    1,
    1,
    1,
    1,
  ...
  public static final Border noBottomBox = BorderFactory.createMatteBorder(
    1,
    1,
    0,
    1,
  ...
  public static final Border topLeftBotBox = BorderFactory.createMatteBorder(
    1,
    1,
    1,
    0,
  ...
  public static final Border topLeftBox = BorderFactory.createMatteBorder(
    1,
    1,
    0,
    0,
  ...
  public static final Border oneLeftBox = GUI.makeSpace(0, 1, 0, 0);

  private static final Border DebugBorder = BorderFactory.createMatteBorder(
    1,
    1,
    1,
    1,
  ...
  private final JButton autoTagButton = new JButton(
    VueResources.local("keywordPanel.autotag")
  );

  private static final CategoryModel OntologiesList = VUE.getCategoryModel();
  private static final CategoryComboBoxModel CategoryMenuModel =
    new CategoryComboBoxModel();

  /** @see ensureModelBulge() */
  private int PlusOneWhenAdding = 0;

  private static final String EmptyValue = "";
  private static final String[] DefaultVMEObject = new String[] {
    VueMetadataElement.ONTOLOGY_NONE,
    EmptyValue,
  };

  /** for newly added items, not existing items */
  private static final VueMetadataElement InputVME = new VueMetadataElement(
    -1,
    DefaultVMEObject[0],
    DefaultVMEObject[1],
    DefaultVMEObject.clone()
  ) {
    @Override
    public void setType(int t) {
      throw new Error("setType " + t);
    }

    @Override
    public void setKey(String k) {
      throw new Error("setKey " + k);
    }

    @Override
    public void setValue(String v) {
      throw new Error("setValue " + v);
    }

    @Override
    public void setObject(Object o) {
      throw new Error("setObject " + Util.tags(o));
    }
  };

  private static final java.util.EventObject EDIT_REQUEST =
    new java.util.EventObject("<md-edit-request>");
  ...
  private final class MDTable extends JTable {
    ...
    MDTable(AbstractTableModel m) {
      ...
      ((DefaultCellEditor) getDefaultEditor(
    ...
    public String getToolTipText(MouseEvent me) {
      if (
        getEventIsOverAddLocation(me)
      ) return CC_ADD_LOCATION_MESSAGE; else return null;
  ...
  private static String shortKey(String k) {
    ...
    if (localPart > 1 && localPart < (k.length() - 1)) return k.substring(
      localPart + 1
    ); else return k;
  }

  public MetadataEditor(
    tufts.vue.LWComponent oneOff,
    boolean showOntologicalMembership,
    boolean followAllActive
  ) {
    //VUE-846 -- special case related to VUE-845 (see comment on opening from keyword item in menu)
    if (getSize().width < 100) setSize(new java.awt.Dimension(300, 200));
      ...
      .addMouseListener(
        new java.awt.event.MouseAdapter() {
          public void mouseReleased(java.awt.event.MouseEvent evt) {
            java.awt.event.MouseEvent me =
              javax.swing.SwingUtilities.convertMouseEvent(
                mdTable.getTableHeader(),
                evt,
                headerAddButtonPanel
              );
            //javax.swing.SwingUtilities.convertMouseEvent(mdTable.getTableHeader(), evt, headerAddButton);
            me.translatePoint(
              evt.getX() - me.getX() - mdTable.getWidth() + BUTTON_COL_WIDTH,
              evt.getY() - me.getY()
            );
          ...
          public void mousePressed(java.awt.event.MouseEvent evt) {
            ...
            if (DEBUG.PAIN) {
              Log.debug("header mousePressed complete.");
              VUE.diagPop();
            }
          }
        }
    ...
    mdTable.addMouseListener(
      new tufts.vue.MouseAdapter("MetadataEditor:JTable") {
        ...
        public void mouseReleased(MouseEvent e) {
          if (getEventIsOverAddLocation(e)) {
            ...
            final OntType existing = OntologiesList.getCustomCategoryByLabel(
              shortKey(vme.getKey())
            );
            final OntType ontType;
            if (existing == null) {
              ontType =
                OntologiesList.addCustomCategory(shortKey(vme.getKey()));
              if (DEBUG.Enabled) Log.debug(
                "added custom cat: " + Util.tags(ontType)
              );
            } else {
              ontType = existing;
              if (DEBUG.Enabled) Log.debug(
                "found custom cat: " + Util.tags(ontType)
              );
            }

            final VueMetadataElement customCategory_VME = freshVME(
              ontType.getAsKey(),
              vme.getValue()
            );

            if (DEBUG.PAIN) Log.debug(
              "updated VME with new key: " + customCategory_VME
            );
            ...
            GUI.invokeAfterAWT(
              new Runnable() {
                public void run() {
                  refreshAll();
                  if (existing == null) OntologiesList.saveCustomOntology();
                }
              }
            );
          }
    ...
    } else mdTable.getTableHeader().setOpaque(false);
    ...
    final JLabel optionsLabel = new JLabel(
      VueResources.getString("jlabel.assigncategory")
    );
    ...
    headerAddButtonPanel.addMouseListener(
      new java.awt.event.MouseAdapter() {
        private boolean dispatchedInButton(MouseEvent e) {
          ...
          final tufts.vue.gui.VueButton button =
            (tufts.vue.gui.VueButton) headerAddButtonPanel.getComponents()[0];
          final java.awt.event.MouseEvent me =
            javax.swing.SwingUtilities.convertMouseEvent(
              headerAddButtonPanel,
              e,
              button
    ...
    headerAddButton.addMouseListener(
      new tufts.vue.MouseAdapter("<headerAddButton>") {
        public void mousePressed(MouseEvent e) {
  ...
  public MDTableModel getModel() {
    return model;
  }

  public CellEditor getActiveTextEdit() {
    return activeTextEdit;
  }

  private static VueMetadataElement emptyVME() {
    return freshVME(EmptyValue);
  }
  ...
  public int getCustomCategoryAddLocation(Object ccLocationConstant) {
    if (mdTable.getModel().getColumnCount() == 2) return -1;

    if (ccLocationConstant == CC_ADD_LEFT) return (
      mdTable.getColumnModel().getColumn(0).getWidth() - 20
    ); else if (ccLocationConstant == CC_ADD_RIGHT) return mdTable
      .getColumnModel()
      .getColumn(0)
      .getWidth(); else return -1;
  }
  ...
  public void adjustColumnModel() {
    ...
    if (mdTable.getModel().getColumnCount() == 2) {
      ...
      mdTable
        .getColumnModel()
        .getColumn(0)
        .setWidth(editorWidth - BUTTON_COL_WIDTH);
      ...
      mdTable
        .getColumnModel()
        .getColumn(0)
        .setWidth(editorWidth / 2 - BUTTON_COL_WIDTH / 2);
      mdTable
        .getColumnModel()
        .getColumn(1)
        .setWidth(editorWidth / 2 - BUTTON_COL_WIDTH / 2);
  ...
  public void selectionChanged(final LWSelection s) {
    ...
    if (s.size() > 1) loadMultiSelection(s); else loadSingleSelection(
      s.first()
    ); // 0 or 1 elements in selection
  ...
  private void loadSingleSelection(LWComponent selected) {
    ...
    if (single.getMetadataList().getCategoryListSize() == 0) changed =
      model.ensureBulge("selected-has-no-meta"); else changed =
      model.clearBulge("selected: reset for new md");
  ...
  private void loadMultiSelection(final LWSelection selection) {
    ...
    for (LWComponent c : group.getAllDescendents()) if (
      !c.hasFlag(LWComponent.Flag.ICON)
    ) groupContents.add(c);
    ...
    for (LWComponent c : groupContents) {
      ...
      final Collection<VueMetadataElement> data = c
        .getMetadataList()
        .getCategoryList()
        .getAsList();
    ...
    if (DEBUG.Enabled) {
      Log.debug("shared: " + Util.tags(shared));
      Util.dumpCollection(shared);
    }
    ...
    if (shared.isEmpty()) {
      ...
      final List<VueMetadataElement> shareTarget = group
        .getMetadataList()
        .getAll();
      for (VueMetadataElement vme : shared) shareTarget.add(vme);
  ...
  private final tufts.vue.gui.VueButton headerAddButton =
    new tufts.vue.gui.VueButton("keywords.button.add");
  private final JPanel headerAddButtonPanel = new JPanel();
  private final JLabel LabelKeyword = new JLabel(
    VueResources.local("jlabel.keyword")
  );
  private final JLabel LabelKeywordShared = new JLabel(
    "Shared " + VueResources.local("jlabel.keyword")
  ); // todo: localize
  private final JLabel LabelCategory = new JLabel(
    VueResources.local("jlabel.category")
  );
  private final JLabel LabelEmpty = new JLabel("");
  private final Border HeaderCellBorder = GUI.makeSpace(
    0,
    CELL_HORZ_INSET + 2,
    0,
    0
  ...
  private class MDTableHeaderRenderer extends DefaultTableCellRenderer {
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      } else comp = LabelEmpty;
      if (col == TC_BUTTON) comp.setBorder(buttonBorder); else comp.setBorder(
        HeaderCellBorder
      );
      comp.setOpaque(true);
  ...
  private class MDCellRenderer extends DefaultTableCellRenderer {
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      if (TC_COMBO == col) {
        ...
        } else if (!knownCategory) box.add(addLabel, BorderLayout.EAST);
        ...
        if (vme != null && vme.hasValue()) {
          ...
          if (!model.keyColumnVisible()) {
            ...
            if (vme.isKeyNone()) faintKeyLabel.setText(
              "None"
            ); else faintKeyLabel.setText(shortKey(vme.getKey()));
            box.add(faintKeyLabel);
          } // else just render the empty box
        }
      } else if (
        TC_BUTTON == col && (isPlusOneEditRow || _vme != null)
      ) return deleteButton;
  ...
  /**/public final class ComboCE extends MDCellEditor {
  ...
  /**/public final class TextCE extends MDCellEditor {
  ...
  /**/public final class ButtonCE extends MDCellEditor {
  ...
  private class MDCellEditor extends DefaultCellEditor {
          ...
          new FocusAdapter() {
            public void focusLost(FocusEvent fe) {
              if (DEBUG.Enabled) {
                ...
                debug(
                  "focusLost (" +
                  (editWasCanceled ? "CANCELLED" : "natural") +
                  ") source_VME=" +
                  source_VME
                );
                if (DEBUG.PAIN && fe != null) debug(
                  "opposite: " + GUI.name(fe.getOppositeComponent())
                );
              }
              activeTextEdit = null;
              if (editWasCanceled) {
              ...
              activeEditRow = -1;
              if (
                DisplayChangesDuringEdit && model.keyColumnVisible()
      ...
      catCombo.addItemListener(
        new ItemListener() {
          public void itemStateChanged(final ItemEvent ie) {
            ...
            if (
              ignoreCategorySelectEvents ||
              ie.getStateChange() != ItemEvent.SELECTED
            ) return;

            if (DEBUG.PAIN) debug(
              "catCombo itemStateChanged " + Util.tags(ie.getItem())
            );

            if (
              ie.getItem() instanceof
              edu.tufts.vue.metadata.gui.EditCategoryItem
            ) {
              if (mdTable.getCellEditor() == null) {
                ...
                if (DEBUG.Enabled) debug(
                  "ignoring itemStateChanged: no active editor"
                );
              } else {
    ...
    private void popEditCategoryDialog() {
      ...
      if (mdTable.getCellEditor() != MDCellEditor.this) throw new Error(
        "what the hell in " + GUI.name(MDCellEditor.this)
      );

      final JDialog ecd = new JDialog(
        VUE.getApplicationFrame(),
        VueResources.local("dialog.editcat.title")
      );
      ecd.setModal(true);
      ecd.add(
        new CategoryEditor(
    ...
    private void catComboHasMadeSelection(final ItemEvent ie) {
      ...
      if (tableVME == null) {
        if (DEBUG.Enabled) debug(
          "aborting catCombo state change on null VME: probably model refresh"
        );
        return;
      }
      publishEdit(
    ...
    private boolean stopEditing(boolean cancel) {
      if (DEBUG.PAIN) {
        debug("stopCellEditing, " + (cancel ? "CANCELLING" : "saving"));
        if (DEBUG.META) Util.printClassTrace("!java");
      }
      editWasCanceled = cancel;
      if (cancel) {
        super.cancelCellEditing();
        return true; // ignored
      } else return super.stopCellEditing();
    }

    private void load_UI_text(VueMetadataElement vme) {
      source_list = null;
      if (group != null) source_list =
        group.getMetadataList().getCategoryList(); else if (
        single != null
      ) source_list =
        single.getMetadataList().getCategoryList(); else throw new Error(
        "can't load VME w/out source list: " + vme
      );
      ...
      source_VME = vme;
      if (vme != InputVME) field.setText(vme.getValue()); else if (
        DEBUG.PAIN
      ) debug(
        "re-using any old input text on InputVME: " + Util.tags(field.getText())
      );
    ...
    public java.awt.Component getTableCellEditorComponent(
      ...
      if (DEBUG.PAIN) debug(
        "getTableCellEditorComponent ic" +
        INSTANCE_COLUMN +
        ": " +
        row +
        "," +
      ...
      if (col == TC_COMBO) {
        ...
        GUI.invokeAfterAWT(
          new Runnable() {
            public void run() {
              ...
              field.requestFocus();
              if (vme == InputVME) field.selectAll();
            }
          }
        );
        ...
        if (getActiveTextEdit() != null) deleteButton.setBackground(
          Color.white
        ); // we'll cancel the edit instead of delete
        else deleteButton.setBackground(Color.yellow);
        return deleteButton;
      }
    ...
    private void saveChangesOnFocusLoss(final FocusEvent fe) {
      ...
      if (DEBUG.PAIN) {
        debug(
          "getCellEd: " +
          Util.tags(editor) +
          (editor == null ? " (no longer editing)" : "")
        );
        if (editor != null && editor != MDCellEditor.this) {
          debug("is not us: " + Util.tags(MDCellEditor.this));
          if (editor instanceof MDCellEditor) debug(
            "remote row: " + activeEditRow
          );
        }
      }
      ...
      if (single == null && group == null) return;
      ...
      if (source_VME == InputVME) {
        if (DEBUG.Enabled) debug(
          "at bottom with InputVME -- creating fresh -- ignoring any category menu selection"
        );
      ...
      if (source_VME == InputVME) {
        ...
        if (
          !model.clearBulge("success: filled bulge with new data")
        ) model.refresh(); // should never happen, but just in case
      } else {
        if (DEBUG.PAIN) debug(
          "inserting new_VME at row " +
          row +
          " " +
          new_VME +
          " -> " +
      ...
      if (groupContents != null) {
        ...
        for (LWComponent c : groupContents) {
          ...
          if (source_VME == InputVME) md.addElement(
            new_VME
          ); else md.replaceValueForKey(new_VME);
          c.layout();
        }
      }

      if (single != null) single.layout();
    ...
    public boolean isCellEditable(java.util.EventObject object) {
      if (DEBUG.PAIN) {
        VUE.diagPush("isCellEditable");
        Log.debug(object/*,new Throwable("HERE")*/);
      }
    ...
    private boolean canEdit(java.util.EventObject object) {
      if (object == EDIT_REQUEST) return true; else if (
        object instanceof MouseEvent == false
      ) {
        if (DEBUG.Enabled) Log.debug(
          "unexpected event input for isCellEditable: " + Util.tags(object)
        );
        return false;
      }
      ...
      if (col == TC_BUTTON) {
        if (DEBUG.PAIN) Log.debug(
          "button-detect in isCellEditable, row " + row + ", col " + col
        );
        ...
        if (DEBUG.PAIN) Log.debug(
          "double-click detect in isCellEditable, row " + row + ", col " + col
        );
        return true;
      }
      return false;
  ...
  private void publishEdit(
    ...
    if (DEBUG.Enabled) {
      Log.debug(
        Util.tags(description) +
        "; publishing change to row " +
        row +
        ": " +
        oldVME
      );
    ...
    if (group != null) {
      for (LWComponent c : groupContents) {
        final MetadataList.SubsetList md = c
          .getMetadataList()
          .getCategoryList();
  ...
  public class MDTableModel extends AbstractTableModel {
    ...
    private final java.util.List<Boolean> categoryFound =
      new java.util.ArrayList<Boolean>(32);
    ...
    public Class getColumnClass(int col) {
      if (col == TC_TEXT) return JTextField.class; else if (
        col == TC_COMBO
      ) return JComboBox.class; else return Object.class;
    }
    ...
    private boolean wasCategoryFound(int row) {
      return (row >= 0 && row < categoryFound.size())
        ? categoryFound.get(row)
        : false;
    }
    ...
    private void reportCategoryFound(int row, boolean found) {
      if (
        categoryFound.size() <= row
      ) for ( // expand valid porition of capacity
        int i = categoryFound.size();
        i < row + 1;
        i++
    ...
    private int getDataRowCount() {
      return source == null
        ? 0
        : source.getMetadataList().getCategoryListSize();
    }
    ...
    boolean keyColumnVisible() {
      return cols == 3;
    }

    private void toggleCategoryKeyColumn() {
      if (DEBUG.PAIN) Log.debug(
        "model: toggleCategoryKeyColumn: cols <- " + cols
      );

      if (this.cols == 2) this.cols = initCols(3); else this.cols = initCols(2);
    ...
    public void refresh() {
      ...
      if (DEBUG.Enabled) Log.debug(
        getClass().getName() + ": refresh; src==" + source
      );
      fireTableDataChanged();
    }
    ...
    public VueMetadataElement getValueAt(int row, int _column_ignored_) {
      ...
      if (DEBUG.PAIN) Log.debug(
        ...
        " size=" +
        size +
        (PlusOneWhenAdding != 0 ? "+1" : "")
      );
      if (source != null) {
        if (size == 0 || row == size) return InputVME; else if (
          row >= size
        ) return null;
        return source.getMetadataList().getCategoryListElement(row);
      } else Log.error(
        " getValueAt " + row + "," + _column_ignored_ + ": no data in model"
      );
      return null;
    }
    ...
    private boolean ensureBulge(String reason) {
      if (DEBUG.PAIN) {
        ...
        Log.debug(
          Util.color(
            "temporary model bulge to +1: " + reason + noneed,
            Util.TERM_CYAN
          )
        );
      }
    ...
    private boolean clearBulge(String reason) {
      if (DEBUG.PAIN) {
        ...
        Log.debug(
          Util.color(
            "clearing the +1 model bulge: " + reason + noneed,
            Util.TERM_CYAN
          )
        );
      }
    ...
    public void deleteAtRow(final int row) {
      ...
      if (groupContents != null) {
        for (LWComponent c : groupContents) {
          ...
          if (md.remove(vme)) {
            if (DEBUG.Enabled) Log.debug(
              "multiple deleted " + vme + "; " + c.getDiagnosticLabel()
            );
            if (md.size() == 0) c.layout(); // no more meta-data: icon display may change
          } else {
            Log.warn(
              "multiple failed to remove " + vme + "; " + c.getDiagnosticLabel()
            );
          }
        }
      }
      ...
      final VueMetadataElement removed = source
        .getMetadataList()
        .getAll()
        .remove(row);

      if (!vme.equals(removed)) Log.error(
        "BAD DELETE: " + removed + " != " + vme
      );
