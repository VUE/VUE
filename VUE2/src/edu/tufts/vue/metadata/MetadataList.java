...
public class MetadataList implements tufts.vue.XMLUnmarshalListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MetadataList.class);
  ...
  static {
    Log.info(
      "ONTOLOGY_NONE " +
      Util.tags(ONTOLOGY_NONE) +
      " " +
      Util.tag(ONTOLOGY_NONE)
    );
  }
  ...
  private final SubsetList resourceList = new SubsetList(
    VueMetadataElement.RESOURCE_CATEGORY
  );
  private final SubsetList categoryList = new SubsetList(
    VueMetadataElement.CATEGORY
  );
  private final SubsetList ontologyList = new SubsetList(
    VueMetadataElement.ONTO_TYPE
  );
  ...
  public List<VueMetadataElement> getAll() {
    return dataList;
  }
  ...
  public boolean isEmpty() {
    return (
      dataList.size() <= 0 ||
      (dataList.size() == 1 && dataList.get(0).isEmpty())
    );
  }
  ...
  private int countType(int type) {
    int count = 0;
    for (VueMetadataElement vme : dataList) if (vme.getType() == type) count++;
    return count;
  }
  ...
  private int sz(int type) {
    return sizeForTypeActual(type);
  }
  ...
  public boolean intersects(MetadataList remote) {
    ...
    for (VueMetadataElement e : dataList) if (
      remoteList.contains(e)
    ) return true;
    return false;
  }
  ...
  public int getCategoryListSize() { // publicly called InspectorPane
    return dataList.getCategoryEndIndex() - dataList.getResourceEndIndex();
  }

  public int getOntologyListSize() { // publicly called
    return dataList.getOntologyEndIndex() - dataList.getCategoryEndIndex();
  }

  /*hide*/private int getOtherListSize() {
    return dataList.getOtherEndIndex() - dataList.getOntologyEndIndex();
  }
  ...
  public void replaceValueForKey(VueMetadataElement hasNewValue) {
    final int index;
    if (hasNewValue.type == VueMetadataElement.CATEGORY) index =
      findCategory(hasNewValue.key); else if (
      hasNewValue.type == VueMetadataElement.RESOURCE_CATEGORY
    ) index =
      findRCategory(
        hasNewValue.key
      ); else throw new UnsupportedOperationException("type of " + hasNewValue);
    ...
    if (index >= 0) {
      ...
      if (
        hasOldValue.obj instanceof String[]
      ) ((String[]) hasOldValue.obj)[1] = hasNewValue.value; // should always be the case
  ...
  public VueMetadataElement get(String key) {
    ...
    if (rcindex != -1) return getResourceListElement(rcindex);
    if (cindex != -1) return getCategoryListElement(cindex); else return null;
  }
  ...
  /*hide*/private int findCategory(String key) {
    ...
    try {
      for (i = 0; i < getCategoryListSize(); i++) {
        ...
        if (key.equals(vme.key)) return i;
      }
    } catch (Throwable t) {
      Log.error(
        "searching for " + Util.tags(key) + " at index " + i + " with " + vme,
        t
      );
    }
    return -1;
  ...
  /*public*/public int findRCategory(String key) {
    for (int i = 0; i < getResourceListSize(); i++) {
      ...
      if (vme.getKey().equals(key)) return i;
    }
    return -1;
  }
  ...
  public boolean contains(String key, String value) {
    for (int i = 0; i < getCategoryListSize(); i++) {
      ...
      if (vme.getKey().equals(key) && vme.getValue().equals(value)) return true;
    }
    return false;
  }
  ...
  /*hide*/private boolean containsOntologicalType(String ontType) {
    for (int i = 0; i < getOntologyListSize(); i++) {
      ...
      if (DEBUG_LOCAL) Log.debug(
        "containsOntologicalType - vme.getValue() " +
        vme.getValue() +
        " --- ontType from properties " +
        ontType
      );
  ...
  /*public*/public VueMetadataElement getCategoryListElement(int index) {
    try {
      if (
        getCategoryListSize() > 0 && index < dataList.size()
      ) return dataList.get(
        index + ((CategoryFirstList) dataList).getResourceEndIndex()
      );
      //else
  ...
  /*hide*/private VueMetadataElement getResourceListElement(int index) {
    try {
      if (
        getResourceListSize() > 0 && index < dataList.size()
      ) return dataList.get(index); else return new VueMetadataElement();
    } catch (Exception e) {
  ...
  /*hide*/private void setCategoryListElement(int i, VueMetadataElement vme) {
    ...
    try {
      ...
      Log.warn(
        "setCategoryListElement " + i + ", real=" + index + "; " + vme,
        e
      );
      return;
    }
  ...
  /*hide*/private void setResourceListElement(
    int index,
    VueMetadataElement ele
  ) {
    try {
      if (getResourceListSize() > 0 && index < dataList.size()) dataList.set(
        index,
        ele
      ); else return;
    } catch (Exception e) {
  ...
  /*public*/public VueMetadataElement getOntologyListElement(int i) {
    ...
    try {
      if (
        getOntologyListSize() > 0 && index < dataList.size()
      ) return dataList.get(index); else return new VueMetadataElement();
    } catch (Exception e) {
      return new VueMetadataElement();
    }
  ...
  /*hide*/private void setOntologyListElement(int i, VueMetadataElement ele) {
    ...
    int index = i + cf.getCategoryEndIndex();
    try {
      if (getOntologyListSize() > 0 && index < dataList.size()) dataList.set(
        index,
        ele
      ); else return;
    } catch (Exception e) {
  ...
  /*hide*/private VueMetadataElement getOtherListElement(int i) {
    ...
    try {
      if (
        getOtherListSize() > 0 && index < dataList.size()
      ) return dataList.get(index); else return new VueMetadataElement(); // NOTE VME CREATION!
    } catch (Exception e) {
  ...
  /*hide*/private void setOtherListElement(int i, VueMetadataElement ele) {
    int index = i + ((CategoryFirstList) dataList).getOntologyEndIndex();
    try {
      if (getOtherListSize() > 0 && index < dataList.size()) dataList.set(
        index,
        ele
      ); else return;
    } catch (Exception e) {
  ...
  public String getMetadataAsHTML(int type) {
    if (
      DEBUG.DATA || DEBUG.DR
    ) return buildDebugHTML(type).toString(); else return buildHTMLForType( // DR so can turn on with '^' key
      type
    );
  }
  ...
  private StringBuilder buildDebugHTML(int type) {
    ...
    if (type >= VueMetadataElement._Types.length) typeName =
      "<unknown:" + type + ">"; else typeName = VueMetadataElement._Types[type];
    ...
    for (int i = 0; i <= 5; i++) {
      ...
      if (sz >= 0) b
        .append(VueMetadataElement._Types[i].toLowerCase())
        .append('=')
        .append(sz)
        .append(" ");
    }
    ...
    for (VueMetadataElement md : dataList) {
      ...
      if (dataList.size() > 1) for (int i = key.length(); i < 22; i++) b.append(
        "&nbsp;"
      );
      ...
      if (val == null) {
        ...
        if (o != null) {
          ...
          if (o instanceof String[]) {
            String s[] = (String[]) o;
            if (s.length == 2 && s[0] == key && s[1] == val) dumpObj = false;
          }
          if (dumpObj) b.append(" ").append(Util.tags(o)); // [nice to pass in a "StringPainter" to tags]
        }
      } else {
        b.append("<font color=red>");
        b.append('"');
        if (val.length() > 64) b
          .append(val.substring(0, 64))
          .append("\"...x")
          .append(val.length()); else b.append(val).append('"');
      }
      b.append("</font>");
    }
    return b;
  ...
  private void buildHTMLFilteredAndOrganized(
    final StringBuilder b,
    final int typeRequest
  ) {
    final List<VueMetadataElement> haveKey = new ArrayList(dataList.size());
    final List<VueMetadataElement> haveOnlyValue = new ArrayList(
      dataList.size()
    );
    ...
    for (VueMetadataElement md : dataList) {
      ...
      if (
        md.type != typeRequest || !md.hasValue() || "<missing>".equals(md.value)
      ) continue;
    ...
    if (haveKey.size() + haveOnlyValue.size() > 0) return;
    ...
    for (VueMetadataElement md : dataList) {
      if (md.type != typeRequest) continue;
      if (DEBUG.Enabled) b.append(NEWLINE); // newline only for debugging the output string
      final boolean hasKey =
        !(md.key == null || md.key == ONTOLOGY_NONE || md.key == KEY_TAG);

      b.append("&nbsp;");
      if (isSingle) b.append("<font size=+0>"); // odd -- this still makes it bigger
      else if (hasKey) b.append("<font color=gray>");

      if (hasKey) b.append(shortKey(md.key)).append(':'); else b.append(
        "&bull;"
      );
      if (hasKey && !isSingle) b.append("</font>");
      b.append(SPACE);
      if (md.value != null) {
        if (!hasKey || isSingle) appendBold(
          b,
          truncate(md.value)
        ); else b.append(truncate(md.value));
      }
      b.append("&nbsp;<br>");
    }
  }
      ...
      if (md.type == OTHER || md.type == TAG) {
        ...
        if (md.value != null && md.value.startsWith("source:")) {
          ...
          if (part.length > 3) {
            if (
              part[1].charAt(0) == 'L'
            ) b.append("&harr; "); // link (also: N=node, I=image, etc) // horizontal double-ended arrow
            else b.append("&thinsp;&bull; "); // bullet: align under 'M' of "Merge sources"
            b.append(part[2]).append(SLASH); // map name
            appendItalic(b, truncate(part[3])); // node/component name
          }
        }
        if (b.length() == startLine) b.append(md.value); // dump all if didn't parse
        b.append("&nbsp;");
      }
    }
  }
  ...
  private StringBuilder getOldMetadataHTML(int type) {
    ...
    if (type == VueMetadataElement.CATEGORY) mdList =
      getCategoryList(); else if (type == VueMetadataElement.ONTO_TYPE) mdList =
      getOntologyList(); else mdList = getOtherList();
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (DEBUG_LOCAL) Log.debug(
        "HTML loop -- value for " + i + " type: " + type + " value: " + value
      );

      if (value == null) {
        if (DEBUG.MEGA) Log.debug(
          "GET-AS-HTML: null meta-data value for " + mdList.get(i)
        );
      ...
      if (value.length() > 0) {
        ...
        if (type == VueMetadataElement.ONTO_TYPE) {
          final int nameLocation = value.indexOf(
            VueMetadataElement.ONT_SEPARATOR
          );
          ...
          if (colon > -1 && value.length() > colon + 1) cursor =
    ...
    if (b.length() > 0) {
      if (type == CATEGORY) b.insert(0, "Keywords: "); else if (
        type == ONTO_TYPE
      ) b.insert(0, "Ontological Membership: "); else if (
        type == OTHER
  ...
  public class SubsetList {
    ...
    private int getSizeForNonResourceType(final int t) {
      ...
      if (
        t == VueMetadataElement.CATEGORY
      ) return getCategoryListSize(); else if (
        t == VueMetadataElement.ONTO_TYPE
      ) return getOntologyListSize(); else return getOtherListSize();
    }
    ...
    public int indexOf(VueMetadataElement vme) {
      ...
      for (int i = 0; i < size; i++) {
        if (
          type == VueMetadataElement.CATEGORY &&
          getCategoryListElement(i).equals(vme)
        ) return i; else if (
          type == VueMetadataElement.ONTO_TYPE &&
          getOntologyListElement(i).equals(vme)
        ) return i; else if (getOtherListElement(i).equals(vme)) return i;
      }
    ...
    public boolean contains(VueMetadataElement vme) {
      ...
      for (int i = 0; i < size; i++) {
        if (
          ...
          type == VueMetadataElement.ONTO_TYPE &&
          getOntologyListElement(i).equals(vme)
        ) return true; else if (getOtherListElement(i).equals(vme)) return true;
      }
    ...
    public VueMetadataElement get(int i) {
      if (
        type == VueMetadataElement.RESOURCE_CATEGORY
      ) return getResourceListElement(i); else if (
        type == VueMetadataElement.CATEGORY
      ) return getCategoryListElement(i); else if (
        type == VueMetadataElement.ONTO_TYPE
      ) return getOntologyListElement(i); else return getOtherListElement(i);
    }

    // Used in MetadataEdtior.java
    public void set(int i, VueMetadataElement ele) {
      if (type == VueMetadataElement.RESOURCE_CATEGORY) setResourceListElement(
        i,
        ele
      ); else if (type == VueMetadataElement.CATEGORY) setCategoryListElement(
        i,
        ele
      ); else if (type == VueMetadataElement.ONTO_TYPE) setOntologyListElement(
        i,
        ele
      ); else setOtherListElement(i, ele);
    }

    public java.util.List getAsList() {
      final java.util.List returnList = new java.util.ArrayList();
      final int size = size();
      for (int i = 0; i < size; i++) returnList.add(get(i));
      return returnList;
    }
  }
  ...
  public class CategoryFirstList
    extends java.util.ArrayList<VueMetadataElement> {
    ...
    private int getResourceEndIndex() {
      return rCategoryEndIndex;
    }

    private int getOntologyEndIndex() {
      return ontologyEndIndex;
    }

    private int getOtherEndIndex() {
    ...
    public boolean add(final VueMetadataElement vme) {
      if (DEBUG.Enabled) {
        if (DEBUG.CASTOR || DEBUG.XML || DEBUG.DATA || DEBUG.PAIN) Log.debug(
          Util.tag(this) + ":add: " + Util.tags(vme)
        );
      }
      ...
      if (vme.getObject() == null) {
        if (DEBUG_LOCAL) Log.debug(
          "VME object is null, (re)setting type..." + vme.getType()
        );
        vme.setType(vme.getType()); // ick ick ick
      }

      if (DEBUG_LOCAL) Log.debug(
        "categoryFirstList add - catEnd,ontEnd,size(): " +
        categoryEndIndex +
        "," +
        ontologyEndIndex +
        "," +
      ...
      if (
        vme.type == VueMetadataElement.ONTO_TYPE &&
        vme.getObject() instanceof OntType
      ) {
        otherEndIndex++;
        super.add(ontologyEndIndex++, vme);
      } else if (
        vme.type == VueMetadataElement.RESOURCE_CATEGORY &&
        vme.getObject() instanceof String[]
      ) {
        ontologyEndIndex++;
        categoryEndIndex++;
        otherEndIndex++;
        ...
        if (DEBUG_LOCAL) Log.debug(
          "rCategoryIndex is now: " + rCategoryEndIndex
        );
      } else if (
        vme.type == VueMetadataElement.CATEGORY &&
        vme.getObject() instanceof String[]
      ) {
        ontologyEndIndex++;
        otherEndIndex++;
    ...
    public VueMetadataElement remove(final int i) {
      ...
      if (DEBUG.PAIN) Log.debug(
        Util.tag(this) + ": remove index " + i + ": " + removed
      );
  ...
  public static void addListener(MetadataListListener listener) {
    ...
    if (DEBUG.Enabled) Log.debug(
      "listeners no longer allowed, will not report to: " + Util.tags(listener)
    );
