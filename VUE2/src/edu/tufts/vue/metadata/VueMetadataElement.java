...
public class VueMetadataElement implements tufts.vue.XMLUnmarshalListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueMetadataElement.class);

  public static final String ONTOLOGY_NONE =
  ...
  private static final Object UniversalOkayObject = new String[] {
    "[keyPlaceHolder]",
    "[valuePlaceHolder]",
  };
  ...
  public boolean isEmpty() {
    return key == ONTOLOGY_NONE && !hasValue();
  }

  public boolean isKeyNone() {
    return key == ONTOLOGY_NONE;
  }
  ...
  /** for mapping backward compat*/public String getSetterKey() {
    return null;
  }

  /** for mapping backward compat*/public String getSetterValue() {
    return null;
  }

  /** for mapping backward compat*/public String getSetterType() {
    return null;
  }

  /** for mapping backward compat*/public void setSetterKey(String k) {
    setKey(k);
  }

  /** for mapping backward compat*/public void setSetterValue(String v) {
    setValue(v);
  }
  ...
  /** for mapping backward compat*/public void setSetterType(String t) {
    ...
    if (t == null || t.length() < 1) {
      if (DEBUG.Enabled) Log.debug(
        "empty type in old save file: " + Util.tags(t)
      );
      this.type = 0;
    } else {
  ...
  public void XML_completed(Object context) {
    ...
    if (isGood) setType(this.type); else this.type = VME_EMPTY_IGNORE; // mark for ignoring in MetadataList.SubsetList.add
  ...
  /** castor persist only */public void setXMLtype(int type) {
    this.type = type;
  }

  /** castor persist only */public int getXMLtype() {
    return type;
  }
  ...
  public void setKey(final String key) {
    ...
    if (key != null && key.startsWith("http://vue.")) {
      if (ONTOLOGY_NONE.equals(key)) this.key = ONTOLOGY_NONE; else if (
        KEY_SOURCE.equals(key)
      ) this.key = KEY_SOURCE; else if (KEY_TAG.equals(key)) this.key =
        KEY_TAG; else this.key = key;
    } else this.key = key;
  }
  ...
  public void setType(int type) {
    ...
    if (
      obj == null &&
      (type == CATEGORY || type == RESOURCE_CATEGORY || type == ONTO_TYPE)
    ) {
      try {
        ...
        if (
          key != null &&
          key.indexOf("file") != -1 &&
          key.indexOf("custom.rdfs") != -1
        ) {
          if (key.indexOf("#") != -1) {
            Log.info("patching key in: " + key + "=" + value);
            this.key =
              "http://vue.tufts.edu/custom.rdfs" +
              key.substring(key.indexOf("#"), key.length());
  ...
  public void setObject(final Object obj) {
    if (DEBUG.Enabled) Log.warn(
      ...
      this +
      "; obj=" +
      Util.color(Util.tags(obj), Util.TERM_GREEN)
    );
  ...
  @Override
  public int hashCode() {
    if (value == null) return key.hashCode(); else return (
  ...
  @Override
  public boolean equals(final Object vme) {
    if (vme instanceof VueMetadataElement) {
      final VueMetadataElement other = (VueMetadataElement) vme;
      if (this.key == other.key && this.value == other.value) return true;
      try {
  ...
  public String toString() {
    final String tname = (type >= 0 && type < _Types.length)
      ? _Types[type]
      : ("type" + type + "?");

    final String id = String
      .format("%08x", System.identityHashCode(this))
      .substring(4);
