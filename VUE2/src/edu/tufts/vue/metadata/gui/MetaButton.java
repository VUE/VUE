...
public class MetaButton extends javax.swing.JPanel {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MetaButton.class);
  ...
  public MetaButton(
    edu.tufts.vue.metadata.ui.MetadataEditor editor,
    String type
  ) {
  ...
  private final tufts.vue.MouseAdapter MouseListener =
    new tufts.vue.MouseAdapter("MB:mouseListener") {
      public void mousePressed(final java.awt.event.MouseEvent me) {
        if (DEBUG.PAIN) {
          VUE.diagPush("MB-pressIn" + clickState);
          Log.debug("mousePressed " + GUI.name(MetaButton.this));
        }

        if (DEBUG.PAIN) Log.debug(
          "active cell editor: " + Util.tags(table.getCellEditor())
        ); // should be ButtonCE
        ...
        if (mdEditor.getActiveTextEdit() != null) {
          if (DEBUG.PAIN) Log.debug(
            "no delete: just cancel active edit and return; " +
            Util.tags(mdEditor.getActiveTextEdit())
          );
  ...
  private final java.awt.event.ActionListener ButtonActionListener =
      ...
      public void actionPerformed(final java.awt.event.ActionEvent ae) {
        if (DEBUG.PAIN) {
          ...
          Log.debug(
            "actionPerformed " +
            MetaButton.this +
            (ignoreNextAction ? " (ignored-on-cancel)" : "")
          );
        }
        // if (clickState < 3) return;

        if (ignoreNextAction) ignoreNextAction = false; else mdEditor
          .getModel()
          .deleteAtRow(rowOfLastRequestedEditor);
