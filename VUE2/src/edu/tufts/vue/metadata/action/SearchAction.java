...
public class SearchAction extends AbstractAction {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(SearchAction.class);

  //----------------------------------------------------------------------------------------
  // constants
  //----------------------------------------------------------------------------------------

  public static final ComboKey SEARCH_SCOPE_CURRENT_MAP = new ComboKey(
    "searchgui.currentmap"
  );
  public static final ComboKey SEARCH_SCOPE_ALL_OPEN_MAPS = new ComboKey(
    "searchgui.allopenmaps"
  );
  ...
  public static final SearchAction.SearchType SEARCH_EVERYTHING =
    new SearchType("searchgui.searcheverything"), SEARCH_ONLY_LABELS =
    new SearchType("searchgui.labels"), SEARCH_ONLY_KEYWORDS = new SearchType(
    "searchgui.keywords"
  ), SEARCH_WITH_CATEGORIES = // "VUE" keywords, which means just the VueMetadataElements / Keywords Widget
    new SearchType("searchgui.categories_keywords"); // search keywords with specific field labels named to search

  /** result-actions -- will appear in combo-boxes in order of declaration.  */
  public static final SearchAction.ResultOp RA_SELECT = new ResultOp(
    "searchgui.select"
  ), RA_SHOW = new ResultOp("searchgui.show"), RA_HIDE = new ResultOp(
    "searchgui.hide"
  ), RA_CLUSTER = new ResultOp("searchgui.cluster"), RA_LINK = new ResultOp(
    "searchgui.link"
  ...
  public enum Operator {
    ...
    Operator(String k) {
      key = k;
      localized = VueResources.local(key);
    }
  ...
  public SearchAction(
    List<edu.tufts.vue.metadata.VueMetadataElement> searchTerms,
    SearchType type
  ) {
    super(VueResources.getString("searchgui.search"));
    //searchType = TYPE_QUERY;
    this.searchTerms = searchTerms;
    if (type != null) setParamsByType(type);
  }
  ...
  public void fire(Object source, String sourceTag) {
    actionPerformed(
      new ActionEvent(this, ActionEvent.ACTION_PERFORMED, sourceTag)
    );
  }

  public void setLocationType(Object scope) {
    if (
      scope == SEARCH_SCOPE_CURRENT_MAP || scope == SEARCH_SCOPE_ALL_OPEN_MAPS
    ) this.searchScope =
      (ComboKey) scope; else throw new IllegalArgumentException(
      GUI.name(scope)
    );
  }
  ...
  public void setParamsByType(SearchType type) {
    ...
    if (type == SEARCH_ONLY_LABELS) setBasic = true; else if (
      type == SEARCH_ONLY_KEYWORDS
    ) textOnly = metadataOnly = true; else if (
      type == SEARCH_EVERYTHING
    ) textOnly = everything = true; else; //if(type == SEARCH_WITH_CATEGORIES) // default: all-false
  }

  /** note: everything bit is currently ignored -- we never index slides or slide content */
  public void setEverything(boolean set) {
    everything = set;
  }

  public void setBasic(boolean set) {
    setBasic = set;
  }
  ...
  public void setResultAction(ResultOp ra) {
  ...
  public void setResultAction(Object ra) {
    if (ra instanceof ResultOp) setResultAction(
      (ResultOp) ra
    ); else throw new IllegalArgumentException(
      "not a ResultOp: " + Util.tags(ra)
    );
  }
  ...
  private List<Query> makeQueryFromCriteria(
    ...
    if (queryType == AS_ONE_QUERY) queryList = null; else queryList =
      new ArrayList<Query>();
    ...
    for (VueMetadataElement criteria : terms) {
      ...
      if (setBasic != true) {
        if (DEBUG.SEARCH) {
          Log.debug(
            "query setBasic != true -- more than simple label-contains"
          );
          Log.debug(
            "RDFIndex.VUE_ONTOLOGY+none=[" + RDFIndex.VUE_ONTOLOGY + "none]"
          );
        }
        if (
          this.treatNoneSpecially &&
          criteria.getKey().equals(RDFIndex.VueTermOntologyNone)
        ) {
          // Note that above is only place where treatNoneSpecially is tested
          if (DEBUG.SEARCH) Log.debug(
            "using manual textToFind: #none is special & no term (query will be empty)"
          );
          ...
          if (DEBUG.SEARCH) Log.debug(
            "using manual textToFind: textOnly=true (query will be empty)"
          );
          textToFind.add(criteria.getValue());
          // The Query object won't even be used!
        } else {
        ...
        query.addCriteria(
          RDFIndex.VUE_ONTOLOGY + Constants.LABEL,
          criteria.getValue(),
          "CONTAINS"
        );
    ...
    if (DEBUG.SEARCH) {
      ...
      if (queryList != null) {
        int i = 0;
        for (Query q : queryList) Log.debug(
          "query " + (i++) + ": " + q.createSPARQLQuery()
        );
      } else {
        Log.debug(
          "query: " + (query == null ? "<NO QUERY>" : query.createSPARQLQuery())
        );
      }
    }
  ...
  private static final String INDEX_WITH_WAIT_CURSOR =
    "<index-with-wait-cursor>";
  ...
  private RDFIndex getIndexForScope(final ComboKey scope) {
    ...
    if (scope == SEARCH_SCOPE_ALL_OPEN_MAPS) {
      ...
      for (LWMap map : VUE.getAllMaps()) {
        if (DEBUG.SEARCH || DEBUG.RDF) Log.debug(
          "adding to global index: " + map
        );
        final RDFIndex mapIndex = getIndexForMap(
          map,
          !this.metadataOnly,
          INDEX_NO_WAIT_CURSOR
        );
      ...
      return getIndexForMap(
        VUE.getActiveMap(),
        !this.metadataOnly,
        INDEX_WITH_WAIT_CURSOR
      );
    }
  }
   ...
   * should be cleared.
   */
  private RDFIndex getIndexForMap(
    final LWMap map,
    boolean include_LWC_fields,
    String activateWait
  ) {
    ...
    if (cache != null && cache.isStillValid(include_LWC_fields)) {
      if (DEBUG.SEARCH) Log.debug(
        "found valid cached index " + Util.tags(cache)
      );
      index = cache.index;
    } else {
      if (DEBUG.SEARCH || DEBUG.RDF) {
        if (cache != null) Log.debug(
          "saw cached index " + Util.tags(cache) + "; but was invalid"
        );
      ...
      map.setClientData(
        CachedIndex.class,
        new CachedIndex(index, map, include_LWC_fields)
      );
  ...
  private static class CachedIndex {
    ...
    boolean isStillValid(boolean want_LWC_fields) {
      return (
        map.getChangeState() == mapChangeStateAtIndex &&
        indexHad_LWC_fields == want_LWC_fields
      );
    }
  }
  ...
  ) // add actualCriteraAdded & crossTermOperator as explicit inputs
  {
    if (DEBUG.SEARCH) {
      Log.debug(
        "runSearch: term(s): " + (terms.size() > 1 ? terms.size() : terms)
      );
      if (terms.size() > 1) Util.dump(terms);
    }
    ...
    if (true) {
      ...
      if (actualCriteriaAdded) {
        ...
        if (textToFind.size() > 0) Log.error(
          "CRITERIA + TEXT-TO-FIND! " + Util.tags(textToFind)
        );
        if (DEBUG.SEARCH) Log.debug(
          "Query objects were used, operator=" + crossTermOperator
        );
        if (crossTermOperator == Operator.AND) {
          ...
          for (Query q : queryList) results.addAll(index.search(q));
        } else {
          // should never happen:
          Log.error(
            "UNHANDLED CROSS TERM OPERATOR " + Util.tags(crossTermOperator)
          );
        }
      }

      if (actualCriteriaAdded && textToFind.size() > 0) throw new Error(
        "I THOUGHT ACTUAL QUERY & TEXT-TO-FIND WERE EXCLUSIVE CASES"
      );
      ...
      for (String textInput : textToFind) {
        ...
        if (textTerm.length() > 0) {
          if (firstTerm || crossTermOperator == Operator.OR) results.addAll(
            index.searchAllValues(textTerm)
          ); else if (crossTermOperator == Operator.AND) results.retainAll(
            index.searchAllValues(textTerm)
          ); else Log.error(
            "Unhandled operator: " + Util.tags(crossTermOperator)
          );
        }
        firstTerm = false;
      }
  ...
  public void actionPerformed(ActionEvent ae) {
    if (DEBUG.SEARCH || DEBUG.RDF) {
      ...
      Log.debug(
        "AE: " +
        "srcTag=" +
        Util.tags(ae.getActionCommand()) +
        " src=" +
        (s instanceof java.awt.Component ? GUI.name(s) : Util.tags(s))
      );
    }
    ...
    if (DEBUG.SEARCH) {
      Log.debug(
        "results are in for scope " +
        Util.tags(searchScope) +
        "; willProcessAs: " +
        GUI.name(resultAction)
      );
      if (DEBUG.RDF) {
  ...
  private void processResultsToMap(
    ...
    if (hits.isEmpty()) {
      ...
      final Collection<LWComponent> toSelect = targetNodesForAction(
        action,
        hits,
        map
      );
      ...
      if (toSelect != null) selection.setWithDescription(
        toSelect,
        "*** Search Results ***"
      ); // TODO: something descriptive here
  ...
  private static final LWComponent.ChildKind SearchRelevant =
    LWComponent.ChildKind.PROPER;

  private static final int TARGETING_DENIED =
    Flag.FILTERED.bit |
    Flag.LOCKED.bit |
    // | Flag.ICON.bit // handling separately
    // We shouldn't ever see any these, but just in case:Flag.STYLE.bit | // should never see unless index is bad (e.g., indexed ChildKind.ANY v.s. PROPER)
   ...
   */
  private Collection<LWComponent> targetNodesForAction(
    final ResultOp action,
    final Collection<LWComponent> hits,
    final LWMap map
  ) {
    if (DEBUG.SEARCH) Log.debug(
      Util.color(GUI.name(resultAction), Util.TERM_GREEN)
    );
    ...
    final boolean doTargeting =
      (action == RA_SELECT || action == RA_LINK || action == RA_CLUSTER);
    final boolean doFiltering = (action == RA_HIDE || action == RA_SHOW);

    if (doFiltering == doTargeting) throw new Error("impossible");

    final Collection<LWComponent> targets = new HashSet<LWComponent>(
      hits.size()
    );
    ...
    if (doTargeting) {
      ...
      for (LWComponent c : hits) {
        ...
        if (c.isHidden() || c.hasAnyFlag(TARGETING_DENIED)) continue;
        final LWMap.Layer layer = c.getLayer();
        if (layer == null || layer.isHidden() || layer.isLocked()) continue;
        // todo: what about being hidden via ancestors?
        if (c.hasFlag(Flag.ICON)) targets.add(c.getParent()); else targets.add(
          c
        );
      }
      ...
      if (action == RA_HIDE) {
        ...
        final Collection<LWComponent> allOnMap = map.getAllDescendents(
          SearchRelevant
        );
        final Collection<LWComponent> toHide = new ArrayList(32);

        for (LWComponent c : allOnMap) if (!targets.contains(c)) toHide.add(c);

        hideComponents(map, toHide);
      } else throw new Error(action.toString());
    }
    return null;
  }
  ...
  private static void revertPreviouslyHiddenToVisible(boolean repaint) {
    ...
    for (LWComponent c : FilteredBySearch) c.setFiltered(false);
  ...
  private class LinkResultAction implements Runnable {
    ...
    public void run() {
      ...
      if (selectedNodes.size() > 0) { // wierd test, given knowns, but used ot exist for the doClusterAction call
        ...
        centralNode.setCenterAt(
          selectionBounds.getCenterX(),
          selectionBounds.getCenterY()
        );
        map.addChildren(newComps);
        if (undoManager != null) undoManager.mark(
          VueResources.getString("searchgui.link")
        );
  ...
  private void displayAllMapsSearchResults(
    final Collection<LWComponent> comps
  ) {
    final LWMap searchResultMap = new LWMap(
      "Search Result #" + ResultMapCount++
    );
    ...
    for (LWMap map : VUE.getAllMaps()) {
      ...
      for (LWComponent next : map.getAllDescendents(
          ...
          if (parent != null && !comps.contains(parent)) {
            ...
            if (LWNode.isImageNode(parent)) {
              if (!comps.contains(parent)) {
                ...
                if (
                  !(dup instanceof LWSlide) &&
                  !dup.hasFlag(LWComponent.Flag.SLIDE_STYLE) &&
                  (!(dup.hasAncestorOfType(LWSlide.class)))
                ) searchResultMap.add(dup);
              }
            } else {
              if (
                !(duplicate instanceof LWSlide) &&
                !duplicate.hasFlag(LWComponent.Flag.SLIDE_STYLE) &&
  ...
  private void produceSearchResultCopyMap(final Collection<LWComponent> comps) {
    ...
    Iterator<LWComponent> components = VUE
      .getActiveMap()
      ...
      if (comps.contains(next)) {
        ...
        if (parent != null && !comps.contains(parent)) {
          ...
          if (LWNode.isImageNode(parent)) {
            if (!comps.contains(parent)) {
              if (
                !(parent instanceof LWSlide) &&
                !parent.hasFlag(LWComponent.Flag.SLIDE_STYLE) &&
    ...
    Iterator<LWComponent> components2 = VUE
      .getActiveMap()
      .getAllDescendents(LWComponent.ChildKind.EDITABLE)
      .iterator();
    while (components2.hasNext()) {
      ...
      if (next instanceof LWLink && duplicates.get(next) != null) {
        LWLink link = (LWLink) next;
        LWComponent head = link.getHead();
        if (head != null && comps.contains(head)) ((LWLink) duplicates.get(
            next
          )).setHead(duplicates.get(head));
        LWComponent tail = link.getTail();
        if (tail != null && comps.contains(tail)) ((LWLink) duplicates.get(
            next
          )).setTail(duplicates.get(tail));
      }
    }
  ...
  private static void revertSearchState(boolean repaint) {
    if (
      LastAction == RA_HIDE || LastAction == RA_SHOW
  ...
  public static void revertGlobalSearchSelectionFromMSGUI() {
    if (
      LastAction == RA_SELECT ||
      LastAction == RA_CLUSTER ||
      LastAction == RA_LINK
    ) {
    ...
    if (viewer != null) {
      ...
      if (focal != null && focal != map) focal.notify(
        SearchAction.class,
        LWKey.Repaint
      );
      if (map != null) map.notify(SearchAction.class, LWKey.Repaint);
  ...
  public static final class /*enum*/SearchType extends ComboKey {
    ...
    public static final Map<String, SearchType> KeyMap =
      new LinkedHashMap(), ValueMap = new HashMap();

    private SearchType(String s) {
      super(s, KeyMap, ValueMap);
    }
  ...
  public static final class /*enum*/ResultOp extends ComboKey {
    ...
    public static final Map<String, ResultOp> KeyMap =
      new LinkedHashMap(), ValueMap = new HashMap();

    private ResultOp(String s) {
      super(s, KeyMap, ValueMap);
    }
