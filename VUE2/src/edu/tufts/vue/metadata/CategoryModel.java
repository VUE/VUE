...
public class CategoryModel extends ArrayList<edu.tufts.vue.ontology.Ontology> {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(CategoryModel.class);

  public static final String CUSTOM_METADATA_FILE =
    tufts.vue.VueUtil.getDefaultUserFolder() +
    File.separator +
    tufts.vue.VueResources.getString("metadata.custom.file");
  ...
  private static final Map<URL, edu.tufts.vue.ontology.Ontology> ontCache =
    new HashMap<URL, edu.tufts.vue.ontology.Ontology>(); // is never cleared
  ...
  public CategoryModel() {
    ...
    Runnable ontologyLoader = new Runnable() {
      public void run() {
        ...
        edu.tufts.vue.preferences.implementations.MetadataSchemaPreference msp =
          edu.tufts.vue.preferences.implementations.MetadataSchemaPreference.getInstance();

        if (!msp.getDublinCoreValue()) removeDefaultOntology(DUBLIN_CORE);

        if (!msp.getVRAValue()) removeDefaultOntology(VRA);
  ...
  @Override
  public boolean add(edu.tufts.vue.ontology.Ontology o) {
    if (DEBUG.Enabled) Log.debug(
      "add[" + size() + "] " + o.getBase() + " " + Util.tags(o.getLabel())
    );
    scanOntTypes(o);
    return super.add(o);
  }
  ...
  @Override
  public edu.tufts.vue.ontology.Ontology set(
    int index,
    edu.tufts.vue.ontology.Ontology o
  ) {
    if (DEBUG.Enabled) Log.debug(
      "set[" + index + "] " + o.getBase() + " " + Util.tags(o.getLabel())
    );
  ...
  public boolean remove(Object _o) {
    ...
    if (DEBUG.Enabled) Log.debug(
      "remove " + o.getBase() + " " + Util.tags(o.getLabel())
    );
    // todo: flush cache
  ...
  public void loadDefaultVUEOntologies() {
    defaultOntologyUrls =
      tufts.vue.VueResources.getStringArray("metadata.load.files");
    for (int i = 0; i < defaultOntologyUrls.length; i++) {
      try {
        loadOntology(
          /*tufts.vue.VueResources.getBundle().getClass()*/tufts.vue.VueResources.class.getResource(
              defaultOntologyUrls[i]
            )
        );
      } catch (Throwable t) {
        Log.error(
          "Problem loading metadata: " +
          defaultOntologyUrls[i] +
          " Error:" +
          t.getMessage()
        );
  ...
  public void loadDefaultVUEOntology(int location) {
    ...
    try {
      loadOntology(
        location,
        tufts.vue.VueResources.class.getResource(defaultOntologyUrls[location])
      );
    } catch (Throwable t) {
      Log.error(
        "Problem loading metadata: " +
        defaultOntologyUrls[location] +
        " Error:" +
        t.getMessage()
      );
    }
  ...
  public OntType addCustomCategory(String name, OntType value) {
    ...
    ontType.setBase(customOntology.getBase());
    ontType.setId(customOntology.getBase() + name);
    if (customOntology.getOntTypes().indexOf(value) > -1) customOntology
      .getOntTypes()
      .set(customOntology.getOntTypes().indexOf(value), ontType);
    //customOntology.getOntTypes().add(ontType);
    return ontType;
  }
  ...
  public void removeDefaultOntology(int location) {
    if (location == DUBLIN_CORE) {
      ...
      edu.tufts.vue.ontology.Ontology dcOntology = ontCache.get(
        tufts.vue.VueResources
          .getBundle()
          .getClass()
          .getResource(defaultOntologyUrls[DUBLIN_CORE])
      );
    ...
    if (location == VRA) {
      edu.tufts.vue.ontology.Ontology vraOntology = ontCache.get(
        tufts.vue.VueResources
          .getBundle()
          .getClass()
          .getResource(defaultOntologyUrls[VRA])
      );
      RDFSOntology emptyOntology = new RDFSOntology();
  ...
  public synchronized void saveCustomOntology() {
    try {
      ...
      for (OntType ontType : customOntology.getOntTypes()) {
        m.createClass(
          customOntology.getBase() + ONT_SEPARATOR + ontType.getLabel()
        );
      }
      ...
      writer.write(
        m,
        new BufferedWriter(new FileWriter(CUSTOM_METADATA_FILE)),
        customOntology.getBase()
      );
    } catch (Throwable t) {
  ...
  private void loadCustomOntology(boolean flag) {
    try {
      if (customOntology == null && !flag) {
        customOntology =
          new RDFSOntology(new File(CUSTOM_METADATA_FILE).toURI().toURL());
      ...
      if (t.getCause() instanceof FileNotFoundException) {
        ...
        Log.info(
          "Didn't load custom metadata, probably just hasn't been created yet - users can enter custom " +
          "metadata through the info window GUI to create file in user directory, if needed. " +
          "Additional details on specific condition follow: " +
          t.getMessage()
        );
      }
    }
