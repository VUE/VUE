...
public class AlchemyAnalyzer implements LWComponentAnalyzer {
  ...
  public List<AnalyzerResult> analyze(LWComponent c, boolean tryFallback) {
    if (isEmpty(alchemyAPIKey)) throw new RuntimeException(
      "AlchemyAPI Key is not specified"
    );
    if (null == c) throw new IllegalArgumentException(
      "Illegal data specified to analyze."
    );

    StringBuilder strBuilder = new StringBuilder();

    if (!isEmpty(c.getLabel())) strBuilder.append(c.getLabel()).append(". ");
    if (!isEmpty(c.getNotes())) strBuilder.append(c.getNotes()).append(". ");

    if (
      null != c.getMetadataList() && !isEmpty(c.getMetadataList().getMetadata())
    ) for (VueMetadataElement element : c.getMetadataList().getMetadata()) if (
      null != element && !isEmpty(element.getValue())
    ) strBuilder.append(element.getValue()).append(". ");

    if (
      null != c.getResource() &&
      null != c.getResource().getProperties() &&
      !isEmpty(c.getResource().getProperties().entries())
    ) for (Entry entry : c.getResource().getProperties().entries()) if (
      null != entry && entry instanceof AbstractMapEntry
    ) {
  ...
  public Multimap<String, AnalyzerResult> analyzeResource(LWComponent c)
    ...
    if (
      null == c || null == c.getResource() || null == c.getResource().getSpec()
    ) throw new IllegalArgumentException(
      "Illegal resource specified to analyze."
    );
    ...
    Document doc = null;
    try {
      AlchemyAPI_NamedEntityParams entityParams =
        new AlchemyAPI_NamedEntityParams();
      entityParams.setSourceText(AlchemyAPI_NamedEntityParams.RAW);
      doc =
        alchemy.URLGetRankedNamedEntities(
          c.getResource().getSpec(),
          entityParams
        );
    } catch (Exception e) {
      log.error(
        "Alchemy XXXX URLGetRankedNamedEntities request failed  -----",
        e
      );
      throw new RuntimeException(e);
    }
    ...
    NodeList nodeList = doc.getElementsByTagName("entity");
    if (null != nodeList && nodeList.getLength() > 0) for (
      int i = 0;
      i < nodeList.getLength();
      ++i
    ) {
      AnalyzerResult res = parseEntity(nodeList.item(i), null);
      if (null != res) result.put(res.getType(), res);
    }
  ...
  public boolean IsAlchemyAPIKeySet() {
    return !isEmpty(alchemyAPIKey);
  }

  public void SetAlchemyAPIKey(String key) {
    if (isEmpty(key)) throw new RuntimeException(
      "AlchemyAPI Key is not specified"
    );
  ...
  private AnalyzerResult parseEntity(Node entity, String defaultType) {
    ...
    for (
      ...
      if (Node.ELEMENT_NODE == child.getNodeType()) {
        if ("type".equals(child.getNodeName())) type =
          getElementValue(child); else if (
          ...
          "disambiguated".equals(child.getNodeName())
        ) disambiguatedNode = (Element) child; else try {
          if ("relevance".equals(child.getNodeName())) relevance =
            Double.valueOf(getElementValue(child)); else if (
            "count".equals(child.getNodeName())
    ...
    if (!isEmpty(type) && !isEmpty(value)) {
      AnalyzerResult retResults = new AnalyzerResult(
        type,
        value,
        relevance,
        count
  ...
  private String getElementValue(Node node) {
    ...
    if (nodes.getLength() == 1) {
      ...
      if (null != child && child.getNodeType() == Node.TEXT_NODE) value =
        child.getNodeValue();
    }
