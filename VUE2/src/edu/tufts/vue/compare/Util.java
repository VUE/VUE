...
public class Util {
  ...
  public static MP getMergeProperty() {
  ...
  public static Object getMergeProperty(final LWComponent c) {
    ...
    switch (MergeProperty) {
      case BOTH:
        return c.getLabel() + "|" + c.getMetadataList().getOntologyListString(); // "BOTH" is historical name
      case TYPE:
        return c.getMetadataList().getOntologyListString(); // concats, SLOWLY, any Onto VME's?
      ...
      // way.
      case RESOURCE:
        return c.hasResource() ? c.getResource().getSpec() : c.getLabel();
      case LABEL:
      default:
        return c.getLabel();
    }
