...
public class WeightVisualizationSettingsPanel
  extends JPanel
  implements ActionListener {

  public static final int DEFAULT_STYLE_CHOICES = 5;

  public static final String parameterChoiceMessageString =
    "Set parameters for:";
  //public static final String parameterChoiceMessageString = "Select a vizualization mode:";
  public static final String intervalChoiceMessageString =
    "Set number of intervals:";
  public static final String paletteChoiceMessageString =
    VueResources.getString("dialog.visualizationsettings.selectcolor");
  ...
  public WeightVisualizationSettingsPanel(
    boolean parameterChoiceDisplayedInOtherPanel
  ) {
    this.parameterChoiceDisplayedInOtherPanel =
      parameterChoiceDisplayedInOtherPanel;
    ...
    parameterChoiceMessage =
      new JLabel(parameterChoiceMessageString, JLabel.RIGHT) {
  ...
  public void setUpGui() {
    ...
    String[] parameterChoices = {
      VueResources.getString("dialog.visualizationsettings.nodes"),
      VueResources.getString("dialog.visualizationsettings.links"),
    };

    tufts.vue.PolygonIcon lineIcon = new tufts.vue.PolygonIcon(
    ...
    parameterChoice =
      new JComboBox(parameterChoices) {
        public java.awt.Dimension getMinimumSize() {
          if (Util.isMacLeopard()) return new java.awt.Dimension(
            /*getGraphics().getFontMetrics().charsWidth(choices[0].toCharArray(),0,choices[0].length())+*/100,
            super.getPreferredSize().height
          ); else {
            return new java.awt.Dimension(
              /*getGraphics().getFontMetrics().charsWidth(choices[0].toCharArray(),0,choices[0].length())+*/80,
              super.getPreferredSize().height
            );
          }
        }
      };
    JLabel intervalNumberChoiceMessage = new JLabel(
      intervalChoiceMessageString,
      JLabel.RIGHT
    );
    Integer[] intervalNumberChoices = { 3, 4, 5, 6, 7, 8, 9, 10 };
    intervalNumberChoice =
      new JComboBox(intervalNumberChoices) {
        public java.awt.Dimension getPreferredSize() {
          return new java.awt.Dimension(30, 30);
        }
      };
    intervalNumberChoice.setSelectedItem(5);
    JLabel paletteChoiceMessage = new JLabel(
      paletteChoiceMessageString,
      JLabel.RIGHT
    );
    ...
    intervalList.setDefaultRenderer(
      PercentageInterval.class,
      new PercentageIntervalRenderer()
    );
    intervalList.setDefaultEditor(
      PercentageInterval.class,
      new PercentageIntervalEditor()
    );
    intervalList.setDefaultRenderer(
      IntervalStylePreview.class,
      new IntervalStylePreviewRenderer()
    );
    intervalList.setDefaultEditor(
      IntervalStylePreview.class,
      new IntervalStylePreviewEditor()
    );
    ...
    if (!parameterChoiceDisplayedInOtherPanel) add(parameterChoiceMessage);
    ...
    if (!parameterChoiceDisplayedInOtherPanel) add(parameterChoice);
    ...
    intervalList.setPreferredScrollableViewportSize(
      new java.awt.Dimension(200, 300)
    );
  ...
  public ArrayList<Double> getNodeIntervalBoundaries() {
    ...
    if (DEBUG_LOCAL) System.out.println("WVSP: node boundaries: " + boundaries);
  ...
  public ArrayList<Double> getLinkIntervalBoundaries() {
    ...
    if (DEBUG_LOCAL) System.out.println("WVSP: link boundaries: " + boundaries);
    ...
    for (int si = 0; si < DEFAULT_STYLE_CHOICES; si++) {
      ...
      if (currNodeStyle != null) {
        ...
        if (currNodeStyle.getAttribute("font-color") != null) {
          foregroundColor =
            Style.hexToColor(currNodeStyle.getAttribute("font-color"));
        } else {
          foregroundColor = Color.BLACK;
        }

        nodeModel.addRow(
          (int) (si * (100.0 / DEFAULT_STYLE_CHOICES)),
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == paletteChoice) {
      ...
      if (skipPaletteApplication) return;
      int i = 1;
      Iterator<Color> colors =
        ((Colors) (paletteChoice.getSelectedItem())).getColors().iterator();
      ...
      while (colors.hasNext()) {
        ...
        // change model
        IntervalStylePreview isp = (IntervalStylePreview) intervalList
          .getModel()
          .getValueAt(count - 1, 1);
        isp.setBackground(color);
        ((IntervalListModel) (intervalList.getModel())).setValueAt(
            isp,
            count - 1,
            1
          );
  ...
  class PercentageIntervalRenderer extends JPanel implements TableCellRenderer {
    ...
    private JLabel percentageLabel = new JLabel(
      VueResources.getString("jlabel.percentage")
    );
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      if (EDITABLE_INTERVALS) {
        ...
        if (row == 4) endField.setEnabled(false); else endField.setEnabled(
          true
        );
      } else {
        startField.setEnabled(false);
  ...
  class PercentageIntervalEditor
    ...
    public PercentageIntervalEditor() {
      ...
      endField.addKeyListener(
        new java.awt.event.KeyAdapter() {
          public void keyTyped(java.awt.event.KeyEvent e) {
            if (!Character.isDigit(e.getKeyChar())) {
              if (DEBUG_LOCAL) {
                System.out.println(
                  "WVSP: character is not a digit..." + e.getKeyChar()
                );
              }
              e.consume();
            }
            ...
            if (
              value > 100 && (endField.getSelectedText() == null)
            ) e.consume();
          }
        }
      );

      endField.addFocusListener(
          ...
          public void focusLost(java.awt.event.FocusEvent evt) {
            if (currentRow < 4) {
              ...
              if (endField.getText().equals("")) {
                endField.setText(
                  "" +
                  ((PercentageInterval) intervalList
                      .getModel()
                      .getValueAt(currentRow, 0)).getEnd()
                );
              }
              ...
              int nextEndValue =
                ((PercentageInterval) intervalList
                    .getModel()
                    .getValueAt(currentRow + 1, 0)).getEnd();
              ...
              if (DEBUG_LOCAL) {
                ...
                System.out.println(
                  "WVSP focus lost -- currentRow: " + currentRow
                );
                System.out.println("--------------");
              }
              ...
              try {
                intervalList
                  .getModel()
                  .setValueAt(
                    new PercentageInterval(
                      startValue,
                      Integer.parseInt(nextStartField)
                    ),
                    currentRow,
                    0
                  );
  ...
  class IntervalStylePreviewRenderer implements TableCellRenderer {
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      javax.swing.BoxLayout boxLayout = new javax.swing.BoxLayout(
        renderer,
        javax.swing.BoxLayout.X_AXIS
      );
      renderer.setLayout(boxLayout);
      final JLabel buttonImage = new JLabel(
        VueResources.getString("jlabel.label")
      );
      ...
      if (intervalList.getModel() == nodeModel) linkOrNode =
        "Node "; else linkOrNode = "Link ";
  ...
  class IntervalStylePreviewEditor
    extends javax.swing.AbstractCellEditor
    implements TableCellEditor {
    ...
    public IntervalStylePreviewEditor() {
      ...
      final java.awt.event.ActionListener okListener =
          ...
          public void actionPerformed(java.awt.event.ActionEvent e) {
            ...
            if (/*current!=null &&*/(parameterChoice.getSelectedIndex() == 0)) {
              ...
              if (currStyle != null) {
                ...
                currStyle.setAttribute(
                  "background",
                  Style.colorToHex(chooser.getColor())
                );
              }
            }
            if (
              /*current!=null && */(parameterChoice.getSelectedIndex() == 1)
            ) {
          ...
          public void mousePressed(java.awt.event.MouseEvent e) {
            ...
            try {
              javax.swing.JColorChooser
                .createDialog(panel, "Color", true, chooser, okListener, null)
                .setVisible(true);
            } catch (Exception ex) {
              ex.printStackTrace();
            }
  ...
  class IntervalListModel extends javax.swing.table.DefaultTableModel { //implements TableModel {
    ...
    public String getColumnName(int col) {
      if (col == 0) return VueResources.getString(
        "dialog.visualizationsettings.colheaderint"
      ); else return VueResources.getString(
        "dialog.visualizationsettings.colheaderpre"
      );
    }

    public int getRowCount() {
      if (piList != null) return piList.size(); else return 0;
    }
