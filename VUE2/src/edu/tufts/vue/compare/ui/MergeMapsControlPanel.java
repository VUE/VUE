...
public class MergeMapsControlPanel
  extends JPanel/* implements ActiveListener<LWMap> */ {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MergeMapsControlPanel.class);

  /** This was for code to re-load slider settings and vote v.s weight choice from existing LWMergeMaps */
  private static final boolean DynamicImpl = VueResources.getBool(
    "merge.view.settings.dynamic"
  );
  ...
  public MergeMapsControlPanel(final DockWindow dw) {
    ...
    tabs.addTab(
      VueResources.local("dialog.mergemap.selectmaps"),
      mapSelectionPanel
    );
    tabs.addTab(
      VueResources.local("dialog.mergemap.visualizationsettings"),
    ...
    generateButton =
      new JButton(VueResources.local("dialog.mergemap.generatenewmap"));
  ...
  private void runMergeProcess() throws java.lang.InterruptedException {
    generateButton.setEnabled(false);

    final JPanel loadingPanel = new JPanel(new BorderLayout());
    final JLabel loadingLabelImage = new JLabel(
    ...
    if (noMapsSelected) {
      ...
      progressLabel.setText(
        VueResources.local("dialog.mergemap.nomapsselected")
      );
    } else {
    ...
    if (!noMapsSelected) loadingLabel.add(loadingLabelImage);
    ...
    final JButton stopButton = new JButton(
      Util.upCaseInitial(VueResources.local("dialog.mergemap.cancel"))
    );
  ...
  private LWMap createMergedMap() {
    ...
    final MergeMapFactory mm = new MergeMapFactory(
      baseMap,
      mapList,
      mapSelectionPanel.getCheckList()
    );
    ...
    mm.setNodeIntervalBoundaries(
      visualizationSettingsPanel.getNodeIntervalBoundaries()
    );
    mm.setLinkIntervalBoundaries(
      visualizationSettingsPanel.getLinkIntervalBoundaries()
    );
    mm.setNodeThresholdSliderValue(
      visualizationSettingsPanel.getNodeThresholdSliderValue()
    );
    mm.setLinkThresholdSliderValue(
      visualizationSettingsPanel.getLinkThresholdSliderValue()
    );
    mm.setFilterOnBaseMap(mapSelectionPanel.getFilterOnBaseMap());
    mm.setExcludeNodesFromBaseMap(
      mapSelectionPanel.getExcludeNodesFromBaseMap()
    );

    if (
      visualizationSettingsPanel.getVisualizationSettingsType() ==
      VisualizationSettingsPanel.VOTE
    ) return mm.createAsVoteMerge(); else return mm.createAsWeightMerge();
  }
