...
public class MapsSelectionPanel extends JPanel {
  ...
  private static final String[] MergePropertyChoices = {
    ...
    VueResources.local(
      "combobox.mergepropertychoices.ontologicalmembershiplabel"
    ),
  };
  ...
  public String stepOneMessage = VueResources.local(
    "dialog.selectionpane.stepOneMessage"
  );
  public String stepTwoMessage = VueResources.local(
    "dialog.selectionpane.stepTwoMessage"
  );
  public String stepThreeMessage = VueResources.local(
    "dialog.selectionpane.stepThreeMessage"
  );

  public String showAllMessage = VueResources.local(
    "dialog.selectionpane.showAllMessage"
  );
  public String filterOnPrimaryMapMessage = VueResources.local(
    "dialog.selectionpane.filterOnPrimaryMapMessage"
  );
  public String excluePrimaryMapNodesMessage = VueResources.local(
    "dialog.selectionpane.excluePrimaryMapNodesMessage"
  );

  public String filterOnBaseMapMessageString = VueResources.local(
    "dialog.selectionpane.filterOnBaseMapMessageString"
  );

  private String AddMaps = VueResources.local("dialog.selectionpane.AddMaps");

  private String MergeProrerty = VueResources.local(
    "dialog.selectionpane.MergeProrerty"
  );
  ...
  private String[] mapFilterChoices = {
    showAllMessage,
    filterOnPrimaryMapMessage,
    excluePrimaryMapNodesMessage,
  };
  ...
  private MapsSelectionPanel() {
    ...
    browseButton.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          ...
          if (lastDirectory != null) choose.setCurrentDirectory(lastDirectory);
          ...
          if (cancel == VueFileChooser.CANCEL_OPTION) return;
          File choice = choose.getSelectedFile();
          if (
            choose.getSelectedFile().getParentFile().isDirectory()
    ...
    stepOneLabel.setBorder(
      BorderFactory.createEmptyBorder(5 + 2, 15, 5 + 2, 7)
    );
    add(stepOneLabel);
    ...
    stepTwoLabel.setBorder(
      BorderFactory.createEmptyBorder(5 + 2, 15, 5 + 2, 5)
    );
    add(stepTwoLabel);
    ...
    mergePropertyChoice.addItemListener(
      new ItemListener() {
        public void itemStateChanged(ItemEvent ie) {
          if (ie.getStateChange() == ItemEvent.SELECTED) {
            if (
              ie.getItem().equals(MergePropertyChoices[0])
            ) Util.setMergeProperty(MP.LABEL); else if (
              ie.getItem().equals(MergePropertyChoices[1])
            ) Util.setMergeProperty(MP.RESOURCE); else if (
              ie.getItem().equals(MergePropertyChoices[2])
            ) Util.setMergeProperty(MP.TYPE); else if (
              ie.getItem().equals(MergePropertyChoices[3])
            ) Util.setMergeProperty(MP.BOTH); // historical name
            else {
    ...
    mergePropertyLabel.setBorder(
      BorderFactory.createEmptyBorder(5 + 2, 15, 5 + 2, 5)
    );
    if (
      !VueResources
        .getString("merge.ontologyType.gui")
        .equals(VueResources.getString("merge.ontologyType.guioff"))
    ) {
    ...
    ((DefaultTableCellRenderer) (maps
          .getTableHeader()
          .getDefaultRenderer())).setHorizontalAlignment(SwingConstants.LEFT);

    maps.addMouseListener(
      new java.awt.event.MouseAdapter() {
        public void mousePressed(java.awt.event.MouseEvent e) {
          ...
          if (maps.getSelectedColumn() == 0) {
            if (model.isSelected(maps.getSelectedRow())) model.setSelected(
              false,
              maps.getSelectedRow()
            ); else model.setSelected(true, maps.getSelectedRow());
          }
        ...
        public void mouseReleased(java.awt.event.MouseEvent e) {
          if (
            maps.getSelectedColumn() == 3 &&
            ((MapTableModel) maps.getModel()).getMapType(
                  maps.getSelectedRow()
                ) ==
              LOCAL_FILE
          ) {
            ((MapTableModel) maps.getModel()).localFiles.remove(
                maps.getSelectedRow()
              );
            deleteDown = false;
          }
  ...
  public int getBaseMapIndex() {
    ...
    if (baseIndex > numberOfMaps) return Math.max(numberOfMaps - 1, 0);
    return baseIndex;
  }
  ...
  public boolean getFilterOnBaseMap() {
    ...
    if (
      mapFilterChoice.getSelectedItem().equals(filterOnPrimaryMapMessage)
    ) return true; else return false;
  }

  public boolean getExcludeNodesFromBaseMap() {
    if (
      mapFilterChoice.getSelectedItem().equals(excluePrimaryMapNodesMessage)
    ) return true; else return false;
  }
  ...
  class MapTableModel implements TableModel {
    ...
    public boolean isSelected(int row) {
      if (getMapType(row) == OPEN_MAP) {
        ...
        if (
          map.hasClientData(tufts.vue.MergeMapFactory.class) &&
          (selected == null || !selected.equals("Selected"))
        ) {
          return false;
        }
    ...
    public void setSelected(boolean select, int row) {
      if (getMapType(row) == OPEN_MAP) {
        ...
        if (!select) openMapSelections.put(
          getMap(row),
          "Not Selected"
        ); else openMapSelections.put(getMap(row), "Selected");
      } else {
        if (select) localFileSelectionStates.set(
          row,
          "Selected"
        ); else localFileSelectionStates.set(row, "UnSelected");
      }
    }
    ...
    public LWMap getMap(int row) {
      ...
      if (
        openMapIndex < VUE.getLeftTabbedPane().getTabCount() &&
        openMapIndex > -1
    ...
    public int getMapType(int row) {
      if (row < getFirstOpenMapRow()) return LOCAL_FILE; else return OPEN_MAP;
    }
    ...
    public Object getValueAt(int row, int col) {
      ...
      if (col == 1) {
        if (row < getFirstOpenMapRow()) {
          ...
          return VUE
            .getLeftTabbedPane()
            .getMapAt(row - getFirstOpenMapRow())
            .getLabel();
        }
      } else if (col == 2) {
    ...
    public Class getColumnClass(int col) {
      if (col == 0) return JCheckBox.class; else if (
        col == 3
      ) return JPanel.class; else return String.class;
    }

    public String getColumnName(int col) {
      if (col == 0) return VueResources.getString("dialog.mapselection.colset");
      if (col == 1) return VueResources.getString(
        "dialog.mapselection.colname"
      );
      if (col == 2) return VueResources.getString(
        "dialog.mapselection.colprimary"
      );
      if (col == 3) return VueResources.getString(
        "dialog.mapselection.collocation"
      ); else return "";
    }
  ...
  class MapTableCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (col == 0) {
        ...
        if (model.isSelected(row)) checkBox.setSelected(
          true
      ...
      if (col == 2) {
        ...
        if (model.getMapType(row) == LOCAL_FILE) {
          if (isSelected && deleteDown) imageLabel.setIcon(
            VueResources.getImageIcon("merge.selectmaps.delete.down")
          ); else imageLabel.setIcon(
            VueResources.getImageIcon("merge.selectmaps.delete.up")
          );
