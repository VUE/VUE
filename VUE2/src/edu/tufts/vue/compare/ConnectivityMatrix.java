...
public class ConnectivityMatrix {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ConnectivityMatrix.class);
  ...
  protected static class IndexedCountingSet<T> implements Iterable<T> {
    ...
    void add(T s) {
      ...
      if (entry != null) {
        ...
        if (ts.length() > maxLen) maxLen = ts.length();
      }
    }

    //int count(T s)          { try{return map.get(s)[COUNT];}catch(Throwable t){return 0;}}
    int count(T s) {
      return map.get(s)[COUNT];
    }

    int indexOf(T s) {
      return map.get(s)[INDEX];
    }

    T get(int atIndex) {
      return values.get(atIndex);
    }
    ...
    void addAll(IndexedCountingSet<T> mergeIn) {
      for (T val : mergeIn.values) add(val);
    }
  ...
  public ConnectivityMatrix(LWMap map) {
    ...
    final Collection<LWComponent> allInMap = map.getAllDescendents(
      ChildKind.PROPER
    );
  ...
  public IndexedCountingSet getKeys() {
    return keys;
  }
  ...
  public static final boolean isValidTarget(LWComponent c) {
    if (c != null) {
      if (c.hasFlag(LWComponent.Flag.ICON)) {
        ...
        return (
          clazz == tufts.vue.LWNode.class || clazz == tufts.vue.LWImage.class
        );
      }
    } else {
      return false;
  ...
  private void indexMergeKeys(final Collection<LWComponent> allInMap) {
    if (DEBUG.MERGE) Log.debug(
      "indexing [" + Util.getMergeProperty() + "] merge keys for map " + map
    );
    ...
    if (DEBUG.MERGE) Log.debug(
      String.format(
        "merge keys: %d valid targets scanned, %d keys found, %d unique.",
        scanCount,
        hitCount,
        size()
      )
  ...
  public int getConnection(Object key1, Object key2) {
    ...
    if (row >= 0 && col >= 0) return this.cx[row][col]; else return 0;
  }
  ...
  public void setConnection(Object key1, Object key2, int value) {
    ...
    if (index1 >= 0 && index2 >= 0) this.cx[index1][index2] = value;
  }
  ...
  public boolean compare(ConnectivityMatrix c2) {
    final int size = size();
    if (c2.size() != size) return false;
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        if (this.cx[i][j] != c2.cx[i][j]) return false;
      }
    }
    return true;
  }
  ...
  private static final boolean LABELS_TOP_BIG =
    (LABELS_TOP && !LABELS_TOP_CHAR);
  ...
  public String asString() {
    ...
    if (LABELS_TOP) capacity += size * (LABELS_TOP_CHAR ? 2 : 8);
    if (LABELS_LEFT) capacity += size * (maxLeftLen + 1);
    ...
    if (DEBUG.MERGE) {
      ...
      Log.debug(
        "toString: keys.size=" + keys.size() + " (unique property values)"
      );
      ...
      for (Object key : this.keys) {
        if (key != null) {
          if (LABELS_TOP_CHAR) {
            ...
            if (txt.length() > 7) b.append(txt, 0, 7); else b.append(txt);
            b.append(TAB);
          }
        }
      }
    ...
    for (int row = 0; row < size; row++) {
      if (LABELS_LEFT) {
        ...
        final String txt =
          this.keys.get(row).toString().replace('\n', ' ').trim();
        if (txt.length() > maxLeftLen) {
          b.append(txt, 0, maxLeftLen);
        } else {
          for (int i = txt.length(); i < maxLeftLen; i++) b.append(SPACE);
          b.append(txt);
        }
      ...
      for (int col = 0; col < size; col++) {
        if (col != 0) b.append(LABELS_TOP_BIG ? TAB : SPACE);
        final int val = cx[row][col];
        if (val == 0) b.append((row == col) ? '0' : '.'); else b.append(val);
      }
      b.append(NewLine);
    }
