...
public class SearchLayout extends Layout {
  ...
  public static final double MIN_AVOID_DIST = VueResources.getDouble(
    "layout.avoid_distance"
  );
  public static final double RIPPLE_RANGE = VueResources.getDouble(
    "layout.ripple_range"
  );
  ...
  public void layout(LWSelection selection) {
    ...
    double radius = Math.sqrt(
      (FACTOR * total * meanNodeWidth * meanNodeWidth) / Math.PI
    );
    ...
    while (i.hasNext()) {
      ...
      if (!selection.contains(c) && c instanceof LWNode) {
        ...
        if (dist < rippleRange && dist > MIN_AVOID_DIST) {
          double newDist =
            dist + ((rippleRange - dist) * maxShift) / rippleRange;
    ...
    while (i.hasNext()) {
      ...
      if (c instanceof LWNode) {
        ...
        while (flag && col_count < MAX_COLLISION_CHECK) {
          ...
          if (
            (viewer.pickNode((float) x, (float) y) != null) ||
            (viewer.pickNode(
                (float) x + node.getWidth(),
                (float) y + node.getHeight()
              ) !=
              null) ||
            (viewer.pickNode((float) x, (float) y + node.getHeight()) !=
              null) ||
            (viewer.pickNode((float) x + node.getWidth(), (float) y) != null)
          ) {
          ...
          col_count++;
          System.out.println(
            "Checking collision for " + c.getLabel() + " count:" + col_count
          );
        }
