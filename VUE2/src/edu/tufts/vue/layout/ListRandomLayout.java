...
public class ListRandomLayout extends Layout {
    ...
    public void layout(LWSelection selection) {
        ...
        while (i.hasNext()) {
            ...
            if (c instanceof LWNode) {
                LWNode node = (LWNode) c;
                double x = minX + xSize * (Math.random() - 0.5);
                double y = minY + ySize * (Math.random() - 0.5);
                int col_count = 0;
                boolean flag = true;
                while (flag && col_count < MAX_COLLISION_CHECK) {

                    LWComponent overlapComponent;
                    if (VUE.getActiveViewer().pickNode((float) x, (float) y) != null) {
                         x = minX + xSize * (Math.random() - 0.5);
                         y = minY + ySize * (Math.random() - 0.5);
 //                       overlapComponent = VUE.getActiveViewer().pickNode((float) x, (float) y);
//                        x = overlapComponent.getX() + overlapComponent.getWidth() + X_SPACING;
//                        y = overlapComponent.getY() + overlapComponent.getHeight() + Y_SPACING;
//                         System.out.println(node.getLabel() + " collided with " + overlapComponent.getLabel() + " set:" + x + "," + y);
                    } else if (VUE.getActiveViewer().pickNode((float) x + node.getWidth(), (float) y + node.getHeight()) != null) {
                          x = minX + xSize * (Math.random() - 0.5);
                         y = minY + ySize * (Math.random() - 0.5);
//                         overlapComponent = VUE.getActiveViewer().pickNode((float) x + node.getWidth(), (float) y + node.getHeight());
//                        x = overlapComponent.getX() + overlapComponent.getWidth() + X_SPACING;
//                        y = overlapComponent.getY() + overlapComponent.getHeight() + Y_SPACING;
 //                       System.out.println(node.getLabel() + " collided with " + overlapComponent.getLabel() + " set:" + x + "," + y);

                    }   else if (VUE.getActiveViewer().pickNode((float) x + node.getWidth(), (float) y ) != null) {
                          x = minX + xSize * (Math.random() - 0.5);
                         y = minY + ySize * (Math.random() - 0.5);
//                       overlapComponent = VUE.getActiveViewer().pickNode((float) x + node.getWidth(), (float) y );
//                        x = overlapComponent.getX() + overlapComponent.getWidth() + X_SPACING;
//                        y = overlapComponent.getY() + overlapComponent.getHeight() + Y_SPACING;
 //                       System.out.println(node.getLabel() + " collided with " + overlapComponent.getLabel() + " set:" + x + "," + y);

                    }   else if (VUE.getActiveViewer().pickNode((float) x , (float) y + node.getHeight()) != null) {
                          x = minX + xSize * (Math.random() - 0.5);
                         y = minY + ySize * (Math.random() - 0.5);
//                        overlapComponent = VUE.getActiveViewer().pickNode((float) x, (float) y + node.getHeight());
//                        x = overlapComponent.getX() + overlapComponent.getWidth() + X_SPACING;
//                        y = overlapComponent.getY() + overlapComponent.getHeight() + Y_SPACING;
 //                      System.out.println(node.getLabel() + " collided with " + overlapComponent.getLabel() + " set:" + x + "," + y);
