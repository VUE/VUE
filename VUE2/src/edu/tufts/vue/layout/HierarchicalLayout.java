...
public class HierarchicalLayout extends Layout {
  ...
  public void layout(LWSelection selection) throws Exception {
    ...
    while (iter.hasNext()) {
      ...
      if (c instanceof LWNode) {
        ...
        double radius =
          (relatedNodes.size() * (c.getHeight() + c.getWidth())) / 2 / Math.PI;
        if (radius < 3 * c.getWidth()) radius = 3 * c.getWidth(); // want to make the radius
        // atleast 3 times the width
        if (radius < MIN_RADIUS_INCREASE) radius = MIN_RADIUS_INCREASE;
        int count = 0;
        for (LWNode related : relatedNodes) {
          related.setLocation(
            centerX + radius * Math.cos(angle) - related.getWidth() / 2,
            centerY + radius * Math.sin(angle) - related.getHeight() / 2
          );
          layoutChildren(node, related, relatedNodes.size(), processedNodes);
          count++;
  ...
  private void layoutChildren(
    ...
    LWNode currentNode,
    int size,
    HashMap<String, LWNode> processedNodes
  ) {
    ...
    double angle = Math.atan2(centerY - centerParentY, centerX - centerParentX);
    ...
    if (relatedNodes.size() < 1) return;

    double radius =
      (relatedNodes.size() *
        (currentNode.getHeight() + currentNode.getWidth())) /
      2 /
      Math.PI;
    if (radius < 2 * currentNode.getWidth()) radius =
      2 * currentNode.getWidth(); // want to make the radius
    ...
    if (relatedNodes.size() == 1) {
      ...
      double dist2 =
        Math.pow(centerX - centerParentX, 2) +
        Math.pow(centerY - centerParentY, 2);
      double r1 = Math.sqrt(dist2);
      double alpha = Math.PI / (size + 1);
      ...
      for (LWNode related : relatedNodes) {
        ...
        double childAngle =
          angle -
          (beta * (2 * count + 1 - relatedNodes.size())) / relatedNodes.size();
        count++;
        ...
        related.setLocation(
          centerX + radius * Math.cos(childAngle) - related.getWidth() / 2,
          centerY + radius * Math.sin(childAngle) - related.getHeight() / 2
        );
        if (relatedNodes.size() > 0) {
          layoutChildren(
            currentNode,
  ...
  private List<LWNode> getRelated(
    LWNode node,
    HashMap<String, LWNode> processedNodes
  ) {
    ...
    while (i.hasNext()) {
      ...
      if (head instanceof LWNode && tail instanceof LWNode) {
        ...
        if (
          (processedNodes.get(tailNode.getID()) == null) && headNode == node
        ) {
        ...
        if (
          (processedNodes.get(headNode.getID()) == null) && tailNode == node
        ) {
