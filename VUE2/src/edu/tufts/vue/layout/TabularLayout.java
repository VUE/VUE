...
public class TabularLayout extends Layout {
  ...
  public void layout(LWSelection selection) throws Exception {
    ...
    if (STRICT_COL_COUNT) mod = LIMIT_COLS;
    ...
    final java.util.List<LWComponent> toLayout = new ArrayList(
      selection.size()
    );
    ...
    for (LWComponent c : selection) {
      ...
      if (c instanceof LWNode || c instanceof LWText) {
        ...
        if (!image.isNodeIcon()) toLayout.add(c);
      }
    }
    ...
    for (LWComponent node : toLayout) {
      ...
      if (IM_LAYOUT) {
        minX =
          node.getLocation().getX() != 0.0 && node.getLocation().getX() < minX
            ? node.getLocation().getX()
            : minX;
        minY =
          node.getLocation().getX() != 0.0 && node.getLocation().getY() < minY
    ...
    double x = minX;
    double y = minY;
    if (!STRICT_COL_COUNT) mod = (int) Math.ceil(Math.sqrt((double) total));
