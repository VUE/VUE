...
public class ClusterLayout extends Layout {
  ...
  public static final int MINX_RADIUS = VueResources.getInt(
    "layout.minx_radius"
  );
  public static final int MINY_RADIUS = VueResources.getInt(
    "layout.miny_radius"
  );
  ...
  public static final double FACTOR = VueResources.getDouble(
    "layout.space_ratio"
  );
  public static final int MAX_COLLISION_CHECK = VueResources.getInt(
    "layout.check_overlap_number"
  );
  ...
  public LWMap createMap(Dataset ds, String mapName) throws Exception {
    ...
    for (ArrayList<String> row : ds.getRowList()) {
      ...
      for (int i = 1; i < row.size(); i++) {
        ...
        String key = ((ds.getHeading() == null) || ds.getHeading().size() < i)
      ...
      if (
        ds.getHeading().size() > 1 && ds.getHeading().get(1).equals("resource")
      ) {
        Resource resource = node1
          .getResourceFactory()
          .get(new File(row.get(1)));
        node1.setResource(resource);
      }
      // special hack to demo the dataset laurie baise dataset
      if (
        ds.getHeading().size() > 6 && ds.getHeading().get(6).equals("Actual")
      ) {
      ...
      if (
        ds.getHeading().size() > 9 && ds.getHeading().get(9).contains("Total")
      ) {
      ...
      if (!clusterColumnList.contains(clusterElement)) clusterColumnList.add(
        clusterElement
      );
      ...
      double x =
        (clusterColumnList.indexOf(clusterElement) % COLUMNS) * Q_SIZE -
        Q_SIZE / 2;
      double y =
        (clusterColumnList.indexOf(clusterElement) / COLUMNS) * Q_SIZE -
        Q_SIZE / 2;
      ...
      node1.setLocation(
        x + (Math.cos(angle) * Q_SIZE) / 3,
        y + (Math.sin(angle) * Q_SIZE) / 3
      );
  ...
  public void layout(LWSelection selection) {
    ...
    while (i.hasNext()) {
      ...
      if (c instanceof LWLink) {
        ...
        maxNodeWidth =
          maxNodeWidth > c.getWidth() ? maxNodeWidth : c.getWidth();
        maxNodeHeight =
          maxNodeHeight > c.getHeight() ? maxNodeHeight : c.getHeight();
    ...
    while (iter.hasNext()) {
      ...
      if (c instanceof LWNode) {
        ...
        minX =
          node.getLocation().getX() < minX ? node.getLocation().getX() : minX;
        minY =
          node.getLocation().getY() < minY ? node.getLocation().getY() : minY;
        total++;
        maxNodeWidth =
          maxNodeWidth > c.getWidth() ? maxNodeWidth : c.getWidth();
        maxNodeHeight =
    ...
    for (LWComponent c : clusterMap.keySet()) {
      ...
      double clusterArea =
        FACTOR * clusterMap.get(c).size() * meanNodeWidth * meanNodeHeight;
      area += clusterArea;
      // assuming width> height
      //			System.out.println("Total Area Needed: "+area+" "+c.getLabel()+" "+clusterMap.get(c).size());
      double radius =
        1.05 * (c.getWidth() / 2 + Math.sqrt(clusterArea / Math.PI)); // increase the radius for computing cluster centers by 5%
      ...
      if (c instanceof LWNode) {
        ...
        if (clusterMap.get(node) != null && clusterMap.get(node).size() > 0) {
          ...
          for (LWComponent linkedNode : clusterMap.get(node)) {
            ...
            y = node.getY();
            double radiusX =
              node.getWidth() / 2 + radius * (1 - Math.pow(Math.random(), 2.0));
            double radiusY =
              node.getWidth() / 2 + radius * (1 - Math.pow(Math.random(), 2.0));
            double xLinkedNode =
              x +
              node.getWidth() / 2 -
              linkedNode.getWidth() / 2 +
            ...
            while (flag && col_count < MAX_COLLISION_CHECK) {
              if (
                (VUE.getActiveViewer().pickNode((float) x, (float) y) !=
                  null) ||
                (VUE
                    .getActiveViewer()
                    .pickNode(
                      (float) x + node.getWidth(),
                      (float) y + node.getHeight()
                    ) !=
                  null) ||
                (VUE
                    .getActiveViewer()
                    .pickNode((float) x, (float) y + node.getHeight()) !=
                  null) ||
                (VUE
                    .getActiveViewer()
                    .pickNode((float) x + node.getWidth(), (float) y) !=
                  null)
              ) {
                angle = Math.PI * 2 * Math.random();
                radiusX =
                  node.getWidth() / 2 +
                  radius * (1 - Math.pow(Math.random(), 2.0));
  ...
  private void packCircles(
    Map<LWComponent, Double> componentRadiusMap,
    double area,
    double minX,
    double minY
  ) {
            ...
            if (checkCollision(node1, node2, r1, r2)) {
              ...
              if (r1 > r2) {
                if (
                  ...
                  node2.setLocation(
                    minX + Math.random() * side,
                    minY + Math.random() * side
                  );
                } else {
                  if (Math.random() > 0.5) {
                    ...
                    node2.setLocation(
                      node1.getX() + distance * Math.cos(angle),
                      node1.getY() + distance * Math.sin(angle)
                    );
                  } else {
                    node2.setLocation(
                      minX + Math.random() * side,
                ...
                if (
                  ...
                  if (Math.random() > 0.5) {
                    ...
                    node1.setLocation(
                      node2.getX() + distance * Math.cos(angle),
                      node2.getY() + distance * Math.sin(angle)
                    );
                  } else {
                    node1.setLocation(
                      minX + Math.random() * side,
