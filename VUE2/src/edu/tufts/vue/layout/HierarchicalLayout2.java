...
public class HierarchicalLayout2 extends HierarchicalLayout {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(HierarchicalLayout2.class);
  ...
  public void layout(LWSelection selection) throws Exception {
    ...
    while (iter.hasNext()) {
      ...
      if (
        ...
        !comp.isManagedLocation() &&
        (processedNodes.get(comp.getID()) == null)
      ) {
  ...
  protected void findChildrenAndParents(
    Vector<LWNode> nodes,
    ...
    for (LWNode node : nodes) {
      ...
      Vector<LWNode> children = new Vector<LWNode>(), parents = new Vector<
        LWNode
      >();
      ...
      while (linkIter.hasNext()) {
        ...
        LWComponent head = link.getHead(), tail = link.getTail();
        int arrowState = link.getArrowState();
        LWNode child = null, parent = null;
        ...
        if (child != null) {
          ...
          if (processedNodes.get(childID) == null) {
            ...
            if (DEBUG_LOCAL) {
              Log.info(
                node.getLabel() + " has child " + child.getLabel() + "."
              );
            }
          }
        } else if (parent != null) {
          ...
          if (processedNodes.get(parentID) == null) {
            ...
            if (DEBUG_LOCAL) {
              Log.info(
                node.getLabel() + " has parent " + parent.getLabel() + "."
              );
            }
          }
        }
  ...
  protected void layoutLayers(LWNode node, HierarchyLayer layer) {
    ...
    float layerCenterX =
      node.getX() + (layer.getPaddedWidth() / 2), layerCenterY =
    ...
    float childLayerCenterY = layerCenterY, previousLayerHeight =
      layer.getHeight(), invert = (mInverted ? -1 : 1);
  ...
  class HierarchyLayer {
    ...
    HierarchyLayer mChildren = null, mParents = null;
    ...
    public float getPaddedWidth() {
      ...
      float horizontalPadding =
        (mWidth / HORIZONTAL_PADDING_FRACTION) / nodeCount;
    ...
    public void layout(float centerX, float centerY) {
      int nodeCount = mNodes.size();
      float x = centerX - (getPaddedWidth() / 2), horizontalPadding =
        (mWidth / HORIZONTAL_PADDING_FRACTION) / nodeCount;
    ...
    protected void printLayerAndChildren(int level) {
      System.out.print(
        level + " (width " + mWidth + ", height " + mHeight + "): "
      );
