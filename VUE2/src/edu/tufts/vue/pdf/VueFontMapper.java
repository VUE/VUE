...
public class VueFontMapper implements FontMapper {
  ...
  public BaseFont awtToPdf(Font font) {
    try {
      ...
      if (p != null) return BaseFont.createFont(
        p.fontName,
        p.encoding,
        p.embedded,
        p.cached,
      ...
      if (
        logicalName.equalsIgnoreCase("DialogInput") ||
        ...
        logicalName.equalsIgnoreCase("Serif") ||
  ...
  public Font pdfToAwt(BaseFont font, int size) {
    ...
    if (names.length == 1) return new Font(names[0][3], 0, size);
    String name10 = null;
    String name3x = null;
    for (int k = 0; k < names.length; ++k) {
      String name[] = names[k];
      if (name[0].equals("1") && name[1].equals("0")) name10 =
        name[3]; else if (name[2].equals("1033")) {
        name3x = name[3];
        break;
      }
    }
    String finalName = name3x;
    if (finalName == null) finalName = name10;
    if (finalName == null) finalName = names[0][3];
  ...
  public BaseFontParameters getBaseFontParameters(String name) {
    ...
    if (alias == null) return (BaseFontParameters) mapper.get(name);
    BaseFontParameters p = (BaseFontParameters) mapper.get(alias);
    if (p == null) return (BaseFontParameters) mapper.get(name); else return p;
  }
    ...
    for (int k = 0; k < files.length; ++k) {
      ...
      try {
        if (
          ...
          Object allNames[] = BaseFont.getAllFontNames(
            file.getPath(),
            BaseFont.CP1252,
            null
          );
