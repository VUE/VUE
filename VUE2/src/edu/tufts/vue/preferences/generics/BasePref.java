...
public abstract class BasePref<T> implements VuePreference<T> {
  ...
  public synchronized void addVuePrefListener(
    VuePrefListener l,
    boolean callbackWithInitialValue
  ) {
    _listeners.add(l);
    if (callbackWithInitialValue) l.preferenceChanged(
      new VuePrefEvent(this, null, getValue())
    );
  }
  ...
  protected synchronized void _fireVuePrefEvent() {
    final VuePrefEvent event = new VuePrefEvent(
      this,
      getPreviousValue(),
      getValue()
    );
    ...
    for (VuePrefListener target : _listeners) {
      try {
        ...
        System.err.println(
          "exception delivering preferenceChanged event " +
          event +
          " to " +
          tufts.Util.tags(target)
        );
        t.printStackTrace();
      }
    }
