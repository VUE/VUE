 ...
 *
 */
public class WindowPropertiesPreference
  ...
  private JButton resetButton = new JButton(
    VueResources.getString("button.reset.label")
  );
   ...
   * -MK
   */
  protected WindowPropertiesPreference(
    String category,
    String key,
    String name,
    String desc,
  ...
  public void itemStateChanged(ItemEvent e) {
    ...
    if (p2.get("enabledWinPos", "null").equals("null")) p2.put(
      "enabledWinPos",
      defaultEnabledVal
    ); else p2.put("enabledWinPos", defaultEnabledVal);
  ...
  public static WindowPropertiesPreference create(
    String category,
    String key,
    String name,
    String desc,
  ...
  public Dimension getWindowSize() {
    return new Dimension(
      Integer.valueOf((String) table.get(WIDTH_KEY)),
      Integer.valueOf((String) table.get(HEIGHT_KEY))
    );
  }
  ...
  public void actionPerformed(ActionEvent e) {
    try {
      ...
      for (int i = 0; i < keys.length; i++) {
        if (keys[i].startsWith("windows.")) p2.remove(keys[i]);
      }

      String s = p2.get("enabledWinPos", "true");

      if (s.equals("true")) p2.put("enabledWinPos", "reset");

      VueUtil.alert(
        PreferencesDialog.getDialog(),
        VueResources.getString("dialog.preference.message"),
        VueResources.getString("dialog.preference.title"),
  ...
  public JComponent getPreferenceUI() {
    ...
    resetPanel.add(
      new JLabel(VueResources.getString("jlabel.resetdefaultwindow"))
    );
  ...
  private void configureResetPanel() {
    if (
      p2.get("enabledWinPos", "true").equals("true") ||
      p2.get("enabledWinPos", "true").equals("reset")
    ) {
  ...
  public Point getWindowLocationOnScreen() {
    return new Point(
      Integer.valueOf((String) table.get(X_POS_KEY)),
      Integer.valueOf((String) table.get(Y_POS_KEY))
    );
  }
