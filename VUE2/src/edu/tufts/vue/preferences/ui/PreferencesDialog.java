...
public class PreferencesDialog extends JDialog {
  ...
  public PreferencesDialog(
    ...
    array[0] =
      new String(VueResources.getString("preferencedailog.vuepreference"));
    array[1] =
      new String(VueResources.getString("preferences.category.mapdisplay"));
    ...
    try {
      ...
      PrefCategoryTreeNode node =
        (PrefCategoryTreeNode) path.getLastPathComponent();
  ...
  private void createTree() {
    ...
    while (i.hasNext()) {
      ...
      String category = VueResources.getString(
        "preferences.category." + prefKey
      );
      System.out.println(category);
      //System.out.println(category);
      PrefCategoryTreeNode categoryNode = new PrefCategoryTreeNode(category);
  ...
  public static void expandAll(JTree tree, TreePath path) {
    ...
    for (int i = 0; i < num; i++) expandAll(
      tree,
      path.pathByAddingChild(model.getChild(node, i))
    );
  }
  ...
  private void addPrefsToCats(DefaultMutableTreeNode node) {
    ...
    for (int i = 0; i < classesA.length; i++) {
      ...
      if (classesA[i] instanceof VuePreference) vp =
        (VuePreference) classesA[i]; else vp =
        getPreferenceObject((Class) classesA[i]);
      while (nodes.hasMoreElements()) {
        DefaultMutableTreeNode n = (DefaultMutableTreeNode) nodes.nextElement();
        if (
          (n instanceof PrefCategoryTreeNode) &&
          (((PrefCategoryTreeNode) n).toString()
              .equals(
                PreferencesManager.mapCategoryKeyToName(vp.getCategoryKey())
              ))
        ) {
          try {
