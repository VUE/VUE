...
public class OntType implements java.io.Serializable {
  ...
  public boolean matchesKey(String key) {
    if (!key.startsWith(baseURL)) return false;
  ...
  public void setBase(final String base) {
    ...
    if (base.indexOf("/edu/tufts/vue/metadata/dces_1_1.rdf") != -1) baseURL =
      VueResources.local("metadata.dublincore.url");
    // else if (base.indexOf(edu.tufts.vue.metadata.CategoryModel.CUSTOM_METADATA_FILE) != -1)
    else if (
      base.indexOf("edu/tufts/vue/metadata/categories.rdf") != -1
    ) baseURL = VueResources.local("metadata.vue.url"); else if (
      base.indexOf("custom.rdfs") != -1
    ) // return "file:///Users/dhelle01/.vue_2/custom.rdfs";
    baseURL = "http://vue.tufts.edu/custom.rdfs"; else baseURL = base;
  }
  ...
  public String toString() {
    ...
    if (baseURL != base) return (
      baseURL +
      " for " +
      base +
      " id=" +
      id +
      (!xid.equals(label) ? (" label=" + label) : "")
    ); else return (
      base + " id=" + id + (!xid.equals(label) ? (" label=" + label) : "")
    );
