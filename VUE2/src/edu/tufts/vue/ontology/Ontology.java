...
public abstract class Ontology {
  ...
  public static final String DEFAULT_ONT_LABEL = VueResources.getString(
    "ontology.label"
  );
  ...
  public OntType getOntTypeByLabel(String label) {
    for (OntType ontType : types) if (
      label.equals(ontType.getLabel())
    ) return ontType;
    return null;
  }
  ...
  public static String getLabelFromUrl(String url) {
    ...
    if (start == -1 || (start == url.length() - 1)) return url; else url =
      url.substring(start + 1);

    int stop = url.lastIndexOf(".");

    if (stop == -1) return url;
  ...
  public boolean equals(Object o) {
    if (o instanceof Ontology) {
      ...
      if (
        this.base.equals(ontology.getBase()) &&
        this.label.equals(ontology.getLabel())
      ) {
        return true;
      } else {
        return false;
  ...
  public abstract void readAllSupportedOntTypes();

  public abstract void readAllSupportedOntTypesWithCss();
}
