...
public class CSSParser {
  ...
  private void parseAtRule() throws IOException {
    ...
    while (!done) {
      ...
      switch (nextToken) {
        case IDENTIFIER:
          if (
            tokenBufferLength > 0 && tokenBuffer[tokenBufferLength - 1] == ';'
          ) {
            --tokenBufferLength;
            done = true;
          }
  ...
  private void parseTillClosed(int openToken) throws IOException {
    ...
    if (isParseToMap) styleMap.put(
      currentStyle.getName(),
      currentStyle
    ); else StyleMap.addStyle(currentStyle);
  }
  ...
  private int readWS() throws IOException {
    int nextChar;
    while (
      (nextChar = readChar()) != -1 && Character.isWhitespace((char) nextChar)
    ) {
      readWS = true;
    }
    return nextChar;
