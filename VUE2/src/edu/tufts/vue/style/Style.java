...
public abstract class Style {
  ...
  public static final String[] DEFAULT_FONT_KEYS = {
    "font-family",
    "font-size",
    "font-color",
  };
  public static final String[] DEFAULT_FONT_VALUES = {
    "Arial",
    "12pt",
    "#000000",
  };
  ...
  public Font getFont() {
    Font f = new Font(
      DEFAULT_FONT_VALUES[DEFAULT_FONT_FAMILY_KEY],
      Font.PLAIN,
      10
    );
    ...
    if (attributes.get(FONT_SIZE_KEY) != null) {
      fa.put(
        TextAttribute.SIZE,
        ShorthandParser.parseFontSize(attributes.get(FONT_SIZE_KEY))
      );
    }
    ...
    if (attributes.get(FONT_STYLE_KEY) != null) {
      fa.put(
        ...
        ShorthandParser.parseFontStyle(attributes.get(FONT_STYLE_KEY))
      );
    }
    if (attributes.get(FONT_WEIGHT_KEY) != null) {
      fa.put(
        TextAttribute.WEIGHT,
        ShorthandParser.parseFontWeight(attributes.get(FONT_WEIGHT_KEY))
      );
    }
    f = f.deriveFont(fa);
    return f;
  ...
  public static String colorToHex(Color color) {
    ...
    if (str.length() > 2) str = str.substring(0, 2); else if (
      str.length() < 2
    ) colorstr += "0" + str; else colorstr += str;
  ...
  public static Color stringToColor(String str) {
    ...
    if (str.length() == 0) color = Color.black; else if (
      str.startsWith("rgb(")
    ) {
      color = parseRGB(str);
    } else if (str.charAt(0) == '#') color = hexToColor(str); else if (
      str.equalsIgnoreCase("Black")
    ) color = hexToColor("#000000"); else if (
      str.equalsIgnoreCase("Silver")
    ) color = hexToColor("#C0C0C0"); else if (
      str.equalsIgnoreCase("Gray")
    ) color = hexToColor("#808080"); else if (
      str.equalsIgnoreCase("White")
    ) color = hexToColor("#FFFFFF"); else if (
      str.equalsIgnoreCase("Maroon")
    ) color = hexToColor("#800000"); else if (
      str.equalsIgnoreCase("Red")
    ) color = hexToColor("#FF0000"); else if (
      str.equalsIgnoreCase("Purple")
    ) color = hexToColor("#800080"); else if (
      str.equalsIgnoreCase("Fuchsia")
    ) color = hexToColor("#FF00FF"); else if (
      str.equalsIgnoreCase("Green")
    ) color = hexToColor("#008000"); else if (
      str.equalsIgnoreCase("Lime")
    ) color = hexToColor("#00FF00"); else if (
      str.equalsIgnoreCase("Olive")
    ) color = hexToColor("#808000"); else if (
      str.equalsIgnoreCase("Yellow")
    ) color = hexToColor("#FFFF00"); else if (
      str.equalsIgnoreCase("Navy")
    ) color = hexToColor("#000080"); else if (
      str.equalsIgnoreCase("Blue")
    ) color = hexToColor("#0000FF"); else if (
      str.equalsIgnoreCase("Teal")
    ) color = hexToColor("#008080"); else if (
      str.equalsIgnoreCase("Aqua")
    ) color = hexToColor("#00FFFF"); else color = hexToColor(str); // sometimes get specified without leading #
    return color;
  }
