...
public class QuickImportAction extends VueAction {
  ...
  protected static final int COLUMNS = 30, GUTTER = 4;
  protected static final boolean DEBUG_LOCAL = false;

  protected JDialog dialog = null;
  protected JRadioButton fileRadioButton = new JRadioButton(
    VueResources.getString("quickImport.file")
  ), URLRadioButton = new JRadioButton(
    VueResources.getString("quickImport.url")
  );
  protected JButton browseButton = new JButton(
    VueResources.getString("quickImport.browse")
  ), cancelButton = new JButton(
    VueResources.getString("quickImport.cancel")
  ), addButton = new JButton(VueResources.getString("quickImport.add"));
  protected JTextField fileTextField = new JTextField();
  protected File file = null, lastDirectory = null;
  ...
  protected void showDialog() {
    if (dialog == null) {
      JPanel outerPanel = new JPanel(new GridBagLayout()), innerPanel =
        new JPanel(new GridBagLayout()), locationPanel = new JPanel(
        new GridBagLayout()
      ), locationRadioPanel = new JPanel(
        new GridBagLayout()
      ), locationRemainderPanel = new JPanel(), selectFilePanel = new JPanel(
        new GridBagLayout()
      ), cancelAddPanel = new JPanel(new GridBagLayout());
      JLabel infoLabel = new JLabel(
        tufts.vue.VueResources.getIcon("helpIcon.raw")
      ), locationLabel = new JLabel(
        VueResources.getString("quickImport.location")
      ), selectFileLabel = new JLabel(
        VueResources.getString("quickImport.selectfile")
      );
      ButtonGroup locationGroup = new ButtonGroup();
      Insets gutter = new Insets(GUTTER, GUTTER, GUTTER, GUTTER), noGutter =
        new Insets(0, 0, 0, 0);
      ...
      dialog =
        new JDialog(
          tufts.vue.VUE.getApplicationFrame(),
          VueResources.getString("quickImport.dialogtitle")
        );
    ...
    synchronized (LOCK) {
      if (!openUnderway) {
        ...
        try {
          ...
          if (
            locationLower.endsWith(".owl") || locationLower.endsWith(".rdfs")
          ) {
            edu.tufts.vue.ontology.ui.TypeList list =
              new edu.tufts.vue.ontology.ui.TypeList();
            ...
            tufts.vue.gui.Widget w = browser.addTypeList(
              list,
              edu.tufts.vue.ontology.Ontology.getLabelFromUrl(ontURL.getFile()),
              ontURL
            );
            list.loadOntology(
              ontURL,
              null,
              edu.tufts.vue.ontology.ui.OntologyChooser2.getOntType(ontURL),
              browser,
              w
            );
          }
          ...
          if (
            locationLower.endsWith(".rdf") || locationLower.endsWith(".rdfs")
          ) {
            ...
            Dataset dataset =
              (isFolder ? new FolderDataset() : new ListDataset());
  ...
  protected void addToGridBag(
    ...
    constraints.weightx =
      (fill == GridBagConstraints.BOTH || fill == GridBagConstraints.HORIZONTAL
          ? 1.0
          : 0.0);
    constraints.weighty =
