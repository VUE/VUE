...
public class AuthenticationManager
   ...
   */
  public osid.shared.Agent authenticate(osid.shared.Type authType)
   ...
   *  @author Mark Norton
   */
  public void authenticateUser(osid.shared.Type authType)
    throws osid.authentication.AuthenticationException {
    Agent temp = null;

    //  Check authNType against all known authorization types.
    if (authType.isEqual(new AuthenticationLDAP())) am_agent =
      this.authenticateViaLDAP(); else //  Type is not known.
    throw new osid.authentication.AuthenticationException(
      osid.authentication.AuthenticationException.UNKNOWN_TYPE
    );
  }
   ...
   *  @author Mark Norton
   */
  public void destroyAuthenticationForType(osid.shared.Type authenticationType)
    throws osid.authentication.AuthenticationException {
  ...
  public osid.shared.TypeIterator getAuthenticationTypes() {
    tufts.oki.shared.TypeIterator it = new tufts.oki.shared.TypeIterator(
      am_types
    );
    return it;
  }
   ...
   *  @author Mark Norton
   */
  public boolean isUserAuthenticated(osid.shared.Type authenticationType)
    throws osid.authentication.AuthenticationException {
  ...
  private boolean isValidAuthNType(osid.shared.Type authNType) {
    ...
    //  Check authNType against all known authorization types.
    if (authNType.isEqual(new AuthenticationLDAP())) val = true;
    return val;
  }
   ...
   */
  private String getAuthNTypeKey(osid.shared.Type authNType)
    throws osid.authentication.AuthenticationException {
    String key;
    //  Check authNType against all known authorization types.
    if (authNType.isEqual(new AuthenticationLDAP())) key =
      AuthenticationLDAP.AUTHN_LDAP_KEY; else throw new osid.authentication.AuthenticationException(
      osid.authentication.AuthenticationException.UNKNOWN_TYPE
    );
   ...
   *  @return A valid Agent object intialized with any known propertites.
   */
  private osid.shared.Agent authenticateViaLDAP()
    throws osid.authentication.AuthenticationException {
    ...
    if ((am_username == null)) {
      throw new osid.authentication.AuthenticationException(
        osid.authentication.AuthenticationException.OPERATION_FAILED
      );
    }
    ...
    try {
      if (sharedMgr != null) am_agent =
        sharedMgr.createAgent(
          am_username,
          new tufts.oki.shared.AgentPersonType()
        ); else am_agent =
        new tufts.oki.shared.Agent(
          am_username,
          new tufts.oki.shared.AgentPersonType()
        );
  ...
  private osid.shared.Agent authenticateViaLDAP3()
    throws osid.authentication.AuthenticationException {
    java.util.Properties props = new java.util.Properties();
    ...
    props.put(
      ...
      "ldaps://ldap.tufts.edu/o=Central Administration (Affiliate)"
    );
    ...
    props.put(
      ...
      "uid=" +
      am_username +
      ", ou=Computer Services, o=Central Administration (Affiliate)"
    );
