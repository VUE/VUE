 ...
 *
 */
public class LocalFilingManager
  extends tufts.oki.OsidManager
  implements osid.filing.FilingManager {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LocalFilingManager.class);
  ...
  public void addRoot(String path) throws osid.filing.FilingException {
    ...
    try {
      ...
      throw new osid.filing.FilingException(
        osid.filing.FilingException.OPERATION_FAILED
      );
  ...
  private void initializeRoots() throws osid.filing.FilingException {
    ...
    if (tufts.Util.isWindowsPlatform()) {
      if (rootCabinets.size() == 0) {
        ...
        for (int i = 0; i < f.length; i++) {
          File file = f[i];
          LocalCabinet newRoot = LocalCabinet.instance(
            file.toString(),
            agent,
            null
          );
          rootCabinets.add(newRoot);
          if (drives[i].compareTo("C:") == 0) cwd = newRoot;
        }
      }
    } else {
      if (rootCabinets.size() == 0) {
    ...
    if ((cwd == null) && rootCabinets.size() > 0) cwd =
      (LocalCabinet) rootCabinets.first();

    if (Log.isDebugEnabled()) Log.debug(
      "initializeRoots; in " + tufts.Util.tags(this) + ": completed."
    );
  }
   ...
   *
   */
  public osid.filing.CabinetEntry getCabinetEntry(osid.shared.Id id)
    ...
    while (it.hasNext()) {
      ...
      try {
        if (entry_id.isEqual(id)) found =
          entry; /*  Otherwise, check sub-cabinets to see if it's in there.  */
        else {
          try {
    ...
    if (found == null) throw new osid.filing.FilingException(
      osid.filing.FilingException.ITEM_DOES_NOT_EXIST
    );
    return found;
  }
  ...
  public void delete(osid.shared.Id cabinetEntryId)
    throws osid.filing.FilingException {
    throw new osid.filing.FilingException(
      osid.filing.FilingException.UNIMPLEMENTED
    );
  }
   ...
   */
  public osid.filing.CabinetEntryIterator oldlistRoots()
    throws osid.filing.FilingException {
    Vector vect = new Vector(10);
    SortedSet set = new TreeSet();
    //vect.add (root);
    osid.filing.CabinetEntryIterator it =
      (osid.filing.CabinetEntryIterator) new LocalCabinetEntryIterator(set);
    return it;
  }

  public osid.filing.CabinetEntryIterator listRoots()
    throws osid.filing.FilingException {
   ...
   */
  public void openDirectory(LocalCabinet dir)
    throws osid.filing.FilingException {
    dir.entries(); //  executed for the side effect of opening it.
  }

  public void XXopenDirectory(LocalCabinet dir)
    throws osid.filing.FilingException {
    ...
    if (trace) System.out.println(
      "openDirectory - files to open: " +
      files.length +
      " in " +
      dir.getFile().getAbsoluteFile()
    );
    ...
    String path = rootBase + dir.getFullName();
    if (trace) System.out.println("openDirectory - path name: " + path);
    for (int i = 0; i < files.length; i++) {
      //File temp = new File (cwd.getPath(), files[i]);
      File temp = new File(rootBase + dir.getFullName(), files[i]);
      if (trace) System.out.println(
        "openDirectory - new file: " + rootBase + dir.getFullName() + files[i]
      );

      String absolute = null;
      if (dir.isRootCabinet()) absolute = path + temp.getName(); else absolute =
        path + dir.separator() + temp.getName();

      if (temp.isDirectory()) {
        if (trace) System.out.println(
          "\tDir " + i + ": " + temp.getName() + "\t" + absolute
        );
        //cwd.createCabinet (temp.getName());
        cwd.createCabinet(absolute);
      } else if (temp.isFile()) {
        if (trace) System.out.println(
          "\tFile " + i + ": " + temp.getName() + "\t" + absolute
        );
   ...
   *  @author Mark Norton
   */
  public void setWorkingDirectory(LocalCabinet cabinet)
    throws osid.filing.FilingException {
    if (trace) System.out.println(
      "setWorkingDirectory.1 - cabinet to open: " + cabinet.getDisplayName()
    );
   ...
   *  @author Mark Norton
   */
  public void setWorkingDirectory(String cabName)
    throws osid.filing.FilingException {
    osid.filing.CabinetEntry entry = null;

    if (trace) System.out.println(
      "setWorkingDirectory.2 - cabinet to open: " + cabName
    );

    String parts[] = this.explodePath(cabName, java.io.File.separatorChar);
    if (trace) System.out.println(
      "setWorkingDirectory.2 - node count is: " + parts.length
    );
    ...
    if (!(entry instanceof LocalCabinet)) {
      throw new osid.filing.FilingException(
        osid.filing.FilingException.NOT_A_CABINET
      );
    }
   ...
   *  @author Mark Norton
   */
  public osid.filing.CabinetEntryIterator list()
    throws osid.filing.FilingException {
    return cwd.entries();
  }
   ...
   *  @author Mark Norton
   */
  public InputStream openForInput(String name)
    throws osid.filing.FilingException {
    ...
    if (trace) System.out.println(
      "openForInput - file to open: " + file.getAbsolutePath()
    );
    try {
      stream = new FileInputStream(file);
    } catch (java.io.IOException ex1) {
      throw new osid.filing.FilingException(
        osid.filing.FilingException.IO_ERROR
      );
    }
   ...
   *  @author Mark Norton
   */
  public OutputStream openForOutput(String name)
    throws osid.filing.FilingException {
    ...
    if (trace) System.out.println(
      "openForOutput - new file: " + rootBase + cwd.getFullName() + name
    );

    //  Create the absolute file name.
    String absolute = null;
    if (cwd.isRootCabinet()) absolute = path + temp.getName(); else absolute =
      path + cwd.separator() + temp.getName();

    //  Check to see if the file already exists.
    try {
      cwd.getCabinetEntryByName(absolute);
      new osid.filing.FilingException(
        osid.filing.FilingException.ITEM_ALREADY_EXISTS
      );
  ...
  public void copy(InputStream in, OutputStream out)
    ...
    BufferedInputStream bin = new BufferedInputStream(
      in,
      LocalFilingManager.BUFFER_SIZE
    );
  ...
  public void rename(
    ...
    } else throw new osid.filing.FilingException(
      osid.filing.FilingException.IO_ERROR
    );
  }
  ...
  public void rename(String oldName, String newName)
    ...
    if (trace) System.out.println("rename - new name: " + absolute);
   ...
   */
  public void delete(osid.filing.CabinetEntry entry)
    throws osid.filing.FilingException {
    cwd.remove(entry);
  }
  ...
  public void createDirectory(String name) throws osid.filing.FilingException {
    String rootBase = cwd.getRootBase();
    String path = rootBase + cwd.getFullName();
    if (trace) System.out.println("createDirectory - path: " + path);
    ...
    if (trace) System.out.println(
      "createDirectory - named " + ": " + temp.getName() + "\t" + absolute
    );

    File fin = new File(absolute);
    if (fin.mkdir()) {
      cwd.createCabinet(absolute);
      if (trace) System.out.println("createDirectory - mkdir result: false");
    } else {
      if (trace) System.out.println("createDirectory - mkdir result: false");
    }
  }
  ...
  public String[] explodePath(String path, char separator) {
    ...
    if (node.length() != 0) parts.add(node.toString());
    ...
    for (int j = 0; j < parts.size(); j++) strParts[j] = (String) parts.get(j);
  ...
  public LocalCabinet walkPath(String path) throws osid.filing.FilingException {
    ...
    for (int i = 1; i < parts.length; i++) {
      ...
      if (!((LocalCabinet) ptr).isOpen()) openDirectory((LocalCabinet) ptr);
