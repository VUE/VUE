 ...
 *
 */
public class LocalCabinet
  extends LocalCabinetEntry
  implements osid.filing.Cabinet {

  private static final boolean UseCache = false;
  private static final Map<String, LocalCabinet> Cache;

  static {
    Cache = UseCache ? new java.util.concurrent.ConcurrentHashMap() : null;
  }
  ...
  public static LocalCabinet instance(
    String path,
    osid.shared.Agent owner,
    osid.filing.Cabinet parent
  ) {
    if (UseCache) {
      ...
      return cabinet;
    } else return new LocalCabinet(path, owner, parent);
  }
  ...
  private void debug(String fmt, Object... args) {
    if (Log.isDebugEnabled()) Log.debug(this + ": " + String.format(fmt, args));
  }
  ...
  public LocalCabinet(
    ...
    if (tufts.vue.DEBUG.IO && Log.isDebugEnabled()) {
      if (parent != null) Log.debug(
        "CREATED in " + parent + ": " + this
      ); else Log.debug("CREATED " + this);
    }
  }
   ...
   *   @author Mark Norton
   */
  public void add(osid.filing.CabinetEntry entry, java.lang.String name)
   ...
   *   @author Mark Norton
   */
  public void add(osid.filing.CabinetEntry entry)
    throws osid.filing.FilingException {
    /*  Add the element to the Vector array.  */
    children.add(entry);
  }
  ...
  public void list() throws osid.filing.FilingException {
    ...
    while (i.hasNext()) {
      ...
      if (entry instanceof LocalCabinet) System.out.println(
        "Cabinet " + i + ":  " + entry.getDisplayName()
      ); else if (entry instanceof LocalByteStore) System.out.println(
        "Byte Store " + i + ":  " + entry.getDisplayName()
      );
    }
    //      }
   ...
   *  @return A new ByteStore with the name provided and this cabinet as parent.
   */
  public osid.filing.ByteStore copyByteStore(
    String name,
    osid.filing.ByteStore oldByteStore
  ) throws osid.filing.FilingException {
   ...
   */
  public osid.filing.Cabinet createCabinet(String displayName)
    throws osid.filing.FilingException {
   ...
   */
  public synchronized osid.filing.CabinetEntryIterator entries()
    throws osid.filing.FilingException {
    if (!initialized) loadChildren();
  ...
  private void loadChildren() throws osid.filing.FilingException {
    ...
    files = dir.list();
    if (files == null) throw new osid.filing.FilingException(
      osid.filing.FilingException.NOT_A_CABINET + ": no files"
    );
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      if (isRootCabinet()) absolute = path + temp.getName(); else absolute =
        path + separator() + temp.getName();
   ...
   */
  public osid.filing.CabinetEntry getCabinetEntryByName(String name)
    throws osid.filing.FilingException {
    Iterator i = children.iterator();
    while (i.hasNext()) {
   ...
   */
  public osid.filing.CabinetEntry getCabinetEntryById(osid.shared.Id id)
    throws osid.filing.FilingException {
    Iterator i = children.iterator();
    while (i.hasNext()) {
      //   for (int i = 0; i < children.size(); i++) {
  ...
  public osid.filing.Cabinet getRootCabinet() {
    ...
    while (cab.getParent() != null) cab = (LocalCabinet) cab.getParent();
    return cab;
  }
  ...
  public String getRootBase() {
    if (super.getParent() == null) return rootBase; else {
   ...
   *  @author Mark Norton
   */
  public void remove(osid.filing.CabinetEntry entry)
    throws osid.filing.FilingException {
   ...
   *  @author Mark Norton
   */
  public void rename(String absolute, String newName)
    throws osid.filing.FilingException {
