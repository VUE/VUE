...
public class VueAuthorization implements osid.authorization.Authorization {
   ...
   *   @author Mark Norton
   */
  public VueAuthorization(
    osid.shared.Id authId,
    osid.shared.Id agent,
    osid.authorization.Function function,
    osid.authorization.Qualifier qualifier,
  ...
  public osid.shared.Agent getAgent()
    ...
    osid.shared.SharedManager mgr =
      tufts.oki.authorization.AuthorizationManager.sharedMgr;
    ...
    try {
      ...
      throw new osid.authorization.AuthorizationException(
        "Agent not found in Shared Manager"
      );
    }
    return ag;
  }
  ...
  public osid.shared.Agent getModifiedBy()
    throws osid.authorization.AuthorizationException {
    if (this.modifiedById == null) return null;
    osid.shared.SharedManager mgr =
      tufts.oki.authorization.AuthorizationManager.sharedMgr;
    osid.shared.Agent ag = null;
    try {
  ...
  public boolean isActiveNow() {
    ...
    if (
      now.after(effectiveDate) && now.before(expirationDate)
  ...
  public void updateEffectiveDate(java.util.Calendar effectiveDate)
    ...
    if (
      ...
      osid.authorization.AuthorizationException.PERMISSION_DENIED
    );
    this.effectiveDate = effectiveDate;
  ...
  public boolean hasAgent(osid.shared.Id agentId) {
    try {
      if (agentId.isEqual(this.agentId)) return true;
    } catch (osid.shared.SharedException ex) {}
    return false;
  }
  ...
  public boolean hasFunction(osid.shared.Id functionId) {
    try {
      if (functionId.isEqual(function.getId())) return true;
    } catch (osid.shared.SharedException ex) {}
    return false;
  }
  ...
  public boolean hasFunctionType(osid.shared.Type functionType) {
    if (functionType.isEqual(function.getFunctionType())) return true;
    return false;
  }
  ...
  public boolean hasQualifier(osid.shared.Id qualifierId) {
    try {
      if (qualifierId.isEqual(qualifier.getId())) return true;
    } catch (osid.shared.SharedException ex) {}
    return false;
  }
  ...
  public boolean hasQualifierOrAncestor(osid.shared.Id qualifierId) {
    ...
    if (hasQualifier(qualifierId)) return true;
    //  Otherwise check to see if qualifierId is an ancestor of this qualifier.
    else return this.qualifier.isDescendantOf(qualifierId);
  }
}
