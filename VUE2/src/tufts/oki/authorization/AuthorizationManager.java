...
public class AuthorizationManager
  extends tufts.oki.OsidManager
  implements osid.authorization.AuthorizationManager {
  ...
  public static osid.shared.SharedManager sharedMgr = null; //  SharedManager is cached.
   ...
   */
  public AuthorizationManager(
    osid.OsidOwner owner,
    osid.shared.Agent user,
    osid.shared.SharedManager sharedMgr
  ) {
    super(owner);
    this.user = user;
    this.auths = new Vector(100);
  ...
  public boolean agentExists(osid.shared.Id agentId) {
    for (int i = 0; i < auths.size(); i++) {
      ...
      try {
        if (agentId.isEqual(az.getAgent().getId())) return true;
   ...
   *  @author Mark Norton
   */
  public osid.authorization.Authorization createAuthorization(
    osid.shared.Id agentId,
    osid.shared.Id functionId,
    osid.shared.Id qualifierId
  ) {
    return createDatedAuthorization(
      agentId,
      functionId,
      qualifierId,
      null,
   ...
   *  @author Mark Norton
   */
  public osid.authorization.Authorization createDatedAuthorization(
    osid.shared.Id agentId,
    osid.shared.Id functionId,
    osid.shared.Id qualifierId,
    ...
    VueAuthorization az = new VueAuthorization(
      authId,
      agentId,
      ftn,
      qual,
   ...
   *  @author Mark Norton
   */
  public osid.authorization.Function createFunction(
    osid.shared.Id functionId,
    String displayName,
    String description,
    osid.shared.Type functionType,
    osid.shared.Id qualifierHierarchyId
  ) {
    VueFunction ftn = new VueFunction(
      displayName,
      description,
      functionType,
      functionId,
   ...
   *  @author Mark Norton
   */
  public osid.authorization.Qualifier createQualifier(
    osid.shared.Id qualifierId,
    String displayName,
    String description,
    osid.shared.Type qualifierType,
    osid.shared.Id parentId
  ) {
    VueQualifier parent = (VueQualifier) this.getQualifier(parentId);
    VueQualifier qual = new VueQualifier(
      displayName,
      description,
      qualifierId,
      qualifierType,
   ...
   *  @author Mark Norton
   */
  public osid.authorization.Qualifier createRootQualifier(
    osid.shared.Id qualifierId,
    String displayName,
    String description,
    osid.shared.Type qualifierType,
  ...
  public void deleteFunction(osid.shared.Id functionId) {
    ...
    for (int i = 0; i < auths.size(); i++) {
      ...
      try {
        if (functionId.isEqual(ftn.getId())) auths.remove(i);
      } catch (osid.shared.SharedException ex) {}
    }
    ...
    for (int j = 0; j < functions.size(); j++) {
      ...
      try {
        if (functionId.isEqual(ftn.getId())) functions.remove(j);
  ...
  public void deleteQualifier(osid.shared.Id qualifierId)
    ...
    throw new osid.authorization.AuthorizationException(
      osid.authorization.AuthorizationException.UNIMPLEMENTED
    );
  }
  ...
  public osid.authorization.AuthorizationIterator getAllAZs(
    osid.shared.Id agentId,
    osid.shared.Id functionId,
    osid.shared.Id qualifierId,
    boolean isActiveNow
    ...
    for (int i = 0; i < auths.size(); i++) {
      ...
      if (
        az.hasAgent(agentId) &&
        az.hasFunction(functionId) &&
        az.hasQualifierOrAncestor(qualifierId)
      ) {
    ...
    return (osid.authorization.AuthorizationIterator) new VueAuthorizationIterator(
      authz
    );
  }
  ...
  public osid.authorization.AuthorizationIterator getAllAZsByFuncType(
    osid.shared.Id agentId,
    osid.shared.Type functionType,
    osid.shared.Id qualifierId,
    boolean isActiveNow
    ...
    for (int i = 0; i < auths.size(); i++) {
      ...
      if (
        az.hasAgent(agentId) &&
        az.hasFunctionType(functionType) &&
        az.hasQualifierOrAncestor(qualifierId)
      ) {
        if (isActiveNow && az.isActiveNow()) {
          VueAuthorization azCopy = (VueAuthorization) az.clone();
  ...
  public osid.authorization.AuthorizationIterator getAllUserAZs(
    osid.shared.Id functionId,
    osid.shared.Id qualifierId,
    boolean isActiveNow
  ) {
    ...
    for (int i = 0; i < auths.size(); i++) {
      ...
      if (
        az.hasFunction(functionId) && az.hasQualifierOrAncestor(qualifierId)
      ) {
        if (isActiveNow && az.isActiveNow()) {
          VueAuthorization azCopy = (VueAuthorization) az.clone();
          authz.add(azCopy);
  ...
  public osid.authorization.AuthorizationIterator getAllUserAZsByFuncType(
    osid.shared.Type functionType,
    osid.shared.Id qualifierId,
    boolean isActiveNow
  ) {
  ...
  public osid.authorization.AuthorizationIterator getExplicitAZs(
    ...
    for (int i = 0; i < auths.size(); i++) {
      ...
      if (
        az.hasAgent(agentId) &&
        az.hasFunction(functionId) &&
        az.hasQualifier(qualifierId)
      ) {
        if (isActiveNow && az.isActiveNow()) authz.add(az); else if (
          isActiveNow == false
        ) authz.add(az);
      }
    }
  ...
  public osid.authorization.AuthorizationIterator getExplicitUserAZs(
    osid.shared.Id functionId,
    osid.shared.Id qualifierId,
    boolean isActiveNow
  ) {
   ...
   *  @author Mark Norton
   */
  public osid.authorization.AuthorizationIterator getAllExplicitAZs(
    osid.shared.Id agentId,
    boolean isActiveNow
  ) {
    Vector authz = new Vector(100);
    ...
    //  Iterate over all known authorizations.
    System.out.println(
      "getAllExplicitAZs:  Authorization count: " + auths.size()
    );
    for (int i = 0; i < auths.size(); i++) {
      VueAuthorization az = (VueAuthorization) auths.elementAt(i);
  ...
  public osid.shared.TypeIterator getFunctionTypes() {
    return new tufts.oki.shared.TypeIterator(
      tufts.oki.authorization.VueFunctionType.functionTypes
    );
  }
   ...
   *  @author Mark Norton
   */
  public osid.authorization.FunctionIterator getFunctions(
    osid.shared.Type functionType
  ) {
   ...
   *  @author Mark Norton
   */
  public osid.authorization.QualifierIterator getQualifierChildren(
    osid.shared.Id qualifierId
  ) {
    ...
    return (osid.authorization.QualifierIterator) new VueQualifierIterator(
      children
    );
  }
   ...
   *  @author Mark Norton
   */
  public osid.authorization.QualifierIterator getQualifierDescendants(
    osid.shared.Id qualifierId
  ) {
    Vector children = new Vector(10);
    for (int i = 0; i < this.qualifiers.size(); i++) {
  ...
  public osid.shared.TypeIterator getQualifierTypes() {
    return new tufts.oki.shared.TypeIterator(
      tufts.oki.authorization.VueQualifierType.qualifierTypes
    );
  }
   ...
   *  @author Mark Norton
   */
  public osid.authorization.QualifierIterator getRootQualifiers(
    osid.shared.Id qualifierHierarchyId
  ) {
    return (osid.authorization.QualifierIterator) new VueQualifierIterator(
      this.rootQualifiers
  ...
  public osid.shared.AgentIterator getWhoCanDo(
    ...
    VueAuthorizationIterator it = (VueAuthorizationIterator) this.getAllUserAZs(
        functionId,
        qualifierId,
        isActiveNow
      );
    ...
    while (it.hasNext()) {
      ...
      if (agents.indexOf(ag) == -1) agents.add(ag);
    }
  ...
  public boolean isAuthorized(
    ...
    VueAuthorizationIterator it = (VueAuthorizationIterator) this.getAllAZs(
        agentId,
        functionId,
        qualifierId,
        true
    ...
    if (it.hasNext()) return true; else return false;
  }
  ...
  public boolean isUserAuthorized(
    ...
    if (this.user == null) throw new osid.authorization.AuthorizationException(
