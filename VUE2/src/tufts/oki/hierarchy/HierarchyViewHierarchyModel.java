...
public class HierarchyViewHierarchyModel extends HierarchyModel {
  ...
  public HierarchyViewHierarchyModel(
    LWNode node,
    LWHierarchyMap hierarchyMap,
    String name,
    String description
  ) {
  ...
  public void computeShortestPath(LWNode rootNode) {
    ...
    hierarchyHash.put(
      (LWNode) nodeHash.get(rootNode),
      new ShortestPathData(null, 0)
    );
    ...
    while (!nodesVector.isEmpty()) {
      ...
      int totalDistance =
        ((ShortestPathData) hierarchyHash.get(currentNodeCopy)).getDistance();
      ...
      for (Iterator i = currentNode.getLinks().iterator(); i.hasNext();) {
        ...
        int length = (int) connectedLink
          .getPoint1()
          .distance(connectedLink.getPoint2());
        totalDistance += length;

        //gets the component associated with the given link
        if (
          (nextNode = (LWNode) connectedLink.getComponent1()) == currentNode
        ...
        if (
          !hierarchyHash.containsKey(nextNodeCopy) ||
          totalDistance <
            ((ShortestPathData) hierarchyHash.get(nextNodeCopy)).getDistance()
        ) {
          //updates the distance and parent hashtables and adds to the vector
          nodesVector.add(nextNode);
          hierarchyHash.put(
            nextNodeCopy,
            new ShortestPathData(currentNodeCopy, totalDistance)
          );
        }
      }
    }
  ...
  public void layout(LWNode currentNode, LWNode previousNode) {
    ...
    LWNode parentNode =
      ((ShortestPathData) hierarchyHash.get(currentNode)).getParent();
    ...
    for (Iterator i = currentNode.getLinks().iterator(); i.hasNext();) {
      ...
      if ((nextNode = (LWNode) link.getComponent1()) == currentNode) nextNode =
        (LWNode) link.getComponent2();
  ...
  public void setUpHierarchyNodes(LWNode node, HierarchyNode parentNode) {
    try {
      ...
      if (parentNode == null) {
        ...
        if ((label = node.getLabel()) == null) label =
          new String("Node:" + node.getID());

        if ((description = node.getNotes()) == null) description =
      ...
      else hierarchyNode = createHierarchyNode(parentNode, node);
      ...
      for (Iterator i = node.getLinks().iterator(); i.hasNext();) {
        ...
        if (link.getComponent1() != node) nextNode =
          link.getComponent1(); else if (
          link.getComponent2() != node
        ) nextNode = link.getComponent2();

        if (
          nextNode != null &&
          nextNode instanceof LWNode &&
          !originalNodes.contains(nextNode)
    ...
    try {
      ...
      if ((label = component.getLabel()) == null) {
        if (component instanceof LWLink) label =
          new String("Link:" + component.getID()); else if (
