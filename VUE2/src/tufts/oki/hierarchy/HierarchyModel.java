...
public class HierarchyModel implements osid.hierarchy.Hierarchy {
  ...
  public void revalidateTree(HierarchyNode node) {
    if (treeModel != null && node != null) treeModel.nodeChanged(
      node.getTreeNode()
    );
  }
  ...
  protected HierarchyNode getRootNode()
    ...
    try {
      ...
      throw new osid.hierarchy.HierarchyException(
        "getRootNode caused a problem"
      );
    }
  }
  ...
  public void addNodeType(osid.shared.Type type)
    ...
    if (!availableTypes.contains(type)) availableTypes.add(
      type
    ); else throw new osid.hierarchy.HierarchyException("type already exists");
  }

  /**A method that removes a given type from the list of the node types*/
  public void removeNodeType(osid.shared.Type nodeType)
    throws osid.hierarchy.HierarchyException {
    if (nodeType == null) throw new osid.hierarchy.HierarchyException(
      "type can't be null"
    );
    ...
    try {
      ...
      for (
        osid.hierarchy.NodeIterator nodeIterator = getAllNodes();
        nodeIterator.hasNext();
      ) {
        if (
          nodeType.isEqual(nodeIterator.next().getType())
  ...
  public osid.hierarchy.Node createNode(
    osid.shared.Id nodeId,
    osid.shared.Id parentId,
    osid.shared.Type type,
    String name,
    String description
  ) throws osid.hierarchy.HierarchyException {
    if (
    ...
    try {
      HierarchyNode parentNode = (HierarchyNode) map.get(
        parentId.getIdString()
      );
      ...
      } else throw new osid.hierarchy.HierarchyException(
  ...
  public osid.hierarchy.Node createRootNode(
    osid.shared.Id nodeId,
    osid.shared.Type type,
    String name,
    String description
  ...
  public void deleteNode(osid.shared.Id nodeId)
    ...
    try {
      ...
      if (node != null && !node.isRoot()) {
        ...
        if (treeNode != null) treeModel.removeNodeFromParent(treeNode);
  ...
  public osid.hierarchy.NodeIterator getAllNodes()
    throws osid.hierarchy.HierarchyException {
    Vector list = new Vector();

    for (Iterator i = map.values().iterator(); i.hasNext();) list.add(
      (osid.hierarchy.Node) i.next()
    );
  ...
  public osid.hierarchy.Node getNode(osid.shared.Id nodeId)
    throws osid.hierarchy.HierarchyException {
    try {
      osid.hierarchy.Node node = (osid.hierarchy.Node) map.get(
        nodeId.getIdString()
      );
      return node;
    } catch (osid.shared.SharedException se) {
  ...
  public osid.shared.TypeIterator getNodeTypes()
    throws osid.hierarchy.HierarchyException {
    return (osid.shared.TypeIterator) (new tufts.oki.shared.TypeIterator(
        availableTypes
      ));
  }

  /**A method that returns an iterator of root nodes*/
  public osid.hierarchy.NodeIterator getRootNodes()
    throws osid.hierarchy.HierarchyException {
    ...
    for (Iterator i = map.values().iterator(); i.hasNext();) {
      ...
      if (nextNode.isRoot()) rootNodes.add(nextNode);
    }

    return (osid.hierarchy.NodeIterator) (new HierarchyNodeIterator(rootNodes));
  }

  public osid.hierarchy.TraversalInfoIterator traverse(
    osid.shared.Id startId,
    int mode,
    int direction,
    int levels
  ) throws osid.hierarchy.HierarchyException {
    if (
      (mode != osid.hierarchy.Hierarchy.TRAVERSE_MODE_DEPTH_FIRST) &&
      (mode != osid.hierarchy.Hierarchy.TRAVERSE_MODE_BREADTH_FIRST)
    ) throw new osid.hierarchy.HierarchyException("unknown mode");

    if (
      (direction != osid.hierarchy.Hierarchy.TRAVERSE_DIRECTION_UP) &&
      (direction != osid.hierarchy.Hierarchy.TRAVERSE_DIRECTION_DOWN)
    ...
    if (startingNode == null) throw new osid.hierarchy.HierarchyException(
      "unknown node"
    ); else {
      Enumeration e;
      ...
      if (direction == osid.hierarchy.Hierarchy.TRAVERSE_DIRECTION_DOWN) {
        if (mode == osid.hierarchy.Hierarchy.TRAVERSE_MODE_DEPTH_FIRST) e =
          startingTreeNode.depthFirstEnumeration(); else e =
          startingTreeNode.breadthFirstEnumeration();

        int maxLevel;
        int currentLevel = startingTreeNode.getLevel();
        int startLevel = currentLevel;

        if (levels >= 0) maxLevel = currentLevel + levels; else maxLevel =
          Integer.MAX_VALUE;
        ...
        while (e.hasMoreElements()) {
          ...
          if (nextNode.getLevel() > maxLevel) break;

          osid.hierarchy.Node node =
            (osid.hierarchy.Node) nextNode.getUserObject();
          traversalInfoList.add(
            (osid.hierarchy.TraversalInfo) (new HierarchyTraversalInfo(
                node.getId(),
                node.getDisplayName(),
        ...
        if (mode == osid.hierarchy.Hierarchy.TRAVERSE_MODE_DEPTH_FIRST) e =
          rootTreeNode.depthFirstEnumeration(); else e =
        ...
        if (levels >= 0) maxLevel = currentLevel; else maxLevel =
          Integer.MAX_VALUE;
        ...
        while (e.hasMoreElements()) {
          ...
          if (nextNode == startingTreeNode) break;
        }

        Vector reverseVector = new Vector();

        for (int j = traversalInfoList.size(); j >= 0; j--) reverseVector.add(
          traversalInfoList.elementAt(j)
        );
    ...
    HierarchyTraversalInfoIterator iterator =
      new HierarchyTraversalInfoIterator(traversalInfoList);
