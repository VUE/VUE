...
public class OutlineViewHierarchyModel
  extends HierarchyModel
  implements LWComponent.Listener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(OutlineViewHierarchyModel.class);
  ...
  public OutlineViewHierarchyModel(
  ...
  private void _setUpOutlineView(
    LWContainer container,
    HierarchyNode parentNode
    ...
    if (parentNode == null) {
      ...
      if ((label = container.getLabel()) == null) label =
        new String("Container:" + container.getID());

      if ((description = container.getNotes()) == null) description =
        new String("No description for " + label);

      hierarchyNode =
        (HierarchyNode) createRootNode(
          new tufts.oki.shared.Id(getNextID()),
    ...
    for (Iterator i = container.getChildIterator(); i.hasNext();) {
      ...
      if (component instanceof LWLink); // ignore
      else if (component instanceof LWContainer) setUpOutlineView(
        (LWContainer) component,
        hierarchyNode
      ); else createHierarchyNode(hierarchyNode, component);
    }
  }
  ...
  /**A method which finds a tree node representing the given component under the given tree node
     ...
     */
  public HierarchyNode findHierarchyNode(
    HierarchyNode hierarchyNode,
    ...
    for (
      osid.hierarchy.NodeIterator i = hierarchyNode.getChildren();
      i.hasNext();
    ) {
   ...
   */
  public ArrayList findHierarchyNodeByComponentID(
    HierarchyNode parentNode,
    String id
  ) throws osid.hierarchy.HierarchyException {
    ...
    for (
      osid.hierarchy.NodeIterator i = parentNode.getChildren();
      i.hasNext();
    ) {
      HierarchyNode childNode = (HierarchyNode) i.next();
  ...
  public void updateHierarchyNodeLabel(String newLabel, String id) {
    try {
      ...
      if (rootNode.getLWComponent().getID().equals(id)) nodes.add(rootNode);

      nodes.addAll(findHierarchyNodeByComponentID(rootNode, id));

      if (nodes == null) {
        System.out.println(
          "OutlineViewHierarchyModel: unhandled case, nodes is null"
        );
        return;
      }
  ...
  public void addHierarchyTreeNode(LWContainer parent, LWComponent addedChild)
    ...
    if (addedChild instanceof LWLink) {
      ...
      new Throwable(
        this + " FYI: deprecated use of addHierarchyTreeNode for " + addedChild
      )
        .printStackTrace();
    }
    //if it is a LWNode, LWImage, LWGroup, etc
      ...
      if (parent instanceof LWMap) parentNode = getRootNode(); else parentNode =
        findHierarchyNode(getRootNode(), parent, true);

      if (parentNode == null) {
        if (Log.isDebugEnabled()) Log.debug(
          "null parentNode when adding hierarchy node: " + addedChild
        );
        return;
      }
      ...
      if (addedChild.hasChildren()) {
        for (
          Iterator childIterator =
            ((LWContainer) addedChild).getChildIterator();
          childIterator.hasNext();
        ) {
          LWComponent child = (LWComponent) childIterator.next();
          if (child instanceof LWContainer) addHierarchyTreeNode(
            (LWContainer) addedChild,
            (LWContainer) child
          );
        }
      }
  ...
  public void deleteHierarchyTreeNode(
    LWContainer parent,
    LWComponent deletedChild
  ) throws osid.hierarchy.HierarchyException {
    // if it is a LWLink
    if (deletedChild instanceof LWLink) {
      ...
      new Throwable(
        this +
        " FYI: unexpected use of deleteHierarchyTreeNode for " +
        deletedChild
      )
        .printStackTrace();
    }
    //if it is anything except a link
      ...
      if (parentNode == null) {
        System.err.println(
          this + " *** NULL parentNode when deleting a hierarchy node"
        );
        return;
      }
  ...
  private void removeLinkConnection(LWComponent linkEndpoint, LWLink link)
    throws osid.hierarchy.HierarchyException {
    if (linkEndpoint == null) return;
    HierarchyNode hierParent = findHierarchyNode(
      getRootNode(),
      linkEndpoint,
      true
    );
  ...
  public void LWCChanged(LWCEvent e) {
    ...
    try {
      if (message == LWKey.LinkAdded) {
        addLinkConnection(
          (LWComponent) e.getSource(),
          (LWLink) e.getComponent()
        );
        ...
        for (LWComponent c : e.getComponents()) {
          if (c instanceof LWLink) continue; // will be handled by a LinkAdded event
          addHierarchyTreeNode((LWContainer) e.getSource(), c);
        }
      } else if (message == LWKey.ChildrenRemoved) {
        for (LWComponent c : e.getComponents()) {
          if (c instanceof LWLink) continue; // will be handled by a LinkRemoved event
  ...
  private HierarchyNode createHierarchyNode(
    HierarchyNode parentNode,
    LWComponent component
  ) throws osid.hierarchy.HierarchyException {
    ...
    try {
      ...
      if ((description = component.getNotes()) == null) description =
        new String("No description for " + label);
  ...
  private void deleteHierarchyNode(HierarchyNode childNode)
    throws osid.hierarchy.HierarchyException {
    try {
      for (
        osid.hierarchy.NodeIterator ni = childNode.getChildren();
        ni.hasNext();
      ) {
        HierarchyNode node = (HierarchyNode) ni.next();
  ...
  public boolean contains(LWComponent component) {
    ...
    try {
      if (
        getRootNode().getLWComponent() == component ||
        findHierarchyNode(getRootNode(), component, true) != null
      ) result = true;
    } catch (osid.hierarchy.HierarchyException he) {
  ...
  public String getNodeLabel(LWComponent component) {
    ...
    if ((label = component.getLabel()) == null) {
      if ((label = component.getDisplayLabel()) == null) {
        if (component instanceof LWLink) label =
          new String("Link-ID# " + component.getID()); else if (
          component instanceof LWNode
        ) label = new String("Node-ID# " + component.getID()); else label =
          "no label";
      }
    }
