...
public class Group extends Agent implements org.osid.agent.Group {
  ...
  public boolean contains(
    org.osid.agent.Agent memberOrGroup,
    boolean searchSubgroups
  ) throws AgentException {
    if (agents.contains(memberOrGroup)) return true; else if (searchSubgroups) {
      for (int i = 0; i < subgroups.size(); i++) {
        org.osid.agent.Group sg = (org.osid.agent.Group) subgroups.elementAt(i);
        return sg.contains(memberOrGroup, searchSubgroups);
      }
    } else return subgroups.contains(memberOrGroup);
    return false;
  }
  ...
  public org.osid.agent.AgentIterator getGroups(boolean includeSubgroups) {
    ...
    for (int i = 0; i < subgroups.size(); i++) {
      Group sub = (Group) subgroups.elementAt(i);
      temp.add(sub);
      if (includeSubgroups) sub.makeGroupsVector(temp, includeSubgroups);
    }
   ...
   *  passed in.
   */
  private void makeGroupsWithAgentVector(
    org.osid.agent.Agent member,
    Vector master
  ) throws AgentException {
    for (int i = 0; i < subgroups.size(); i++) {
      Group sub = (Group) subgroups.elementAt(i);
      if (sub.contains(member, true)) master.add(sub);
   ...
   *  @author Mark Norton
   */
  public org.osid.agent.AgentIterator getGroupsContainingMember(
    org.osid.agent.Agent member
  ) throws AgentException {
    Vector temp = new Vector(100);
    for (int i = 0; i < subgroups.size(); i++) {
      Group sub = (Group) subgroups.elementAt(i);
      if (sub.contains(member, true)) temp.add(sub);
  ...
  public void remove(org.osid.agent.Agent memberOrGroup) {
    if (memberOrGroup instanceof Group) subgroups.remove(
      memberOrGroup
    ); else agents.remove(memberOrGroup);
  }
  ...
  public org.osid.shared.TypeIterator getPropertyTypes()
    ...
    for (int i = 0; i < ag_props.size(); i++) {
      tufts.oki.shared2.Properties prop =
        (tufts.oki.shared2.Properties) ag_props.elementAt(i);
    }
  ...
  public org.osid.shared.Properties getPropertiesByType(
    ...
    try {
      for (int i = 0; i < ag_props.size(); i++) {
        org.osid.shared.Properties prop =
          (org.osid.shared.Properties) ag_props.elementAt(i);
        if ((prop.getType()).isEqual(propertiesType)) return prop;
      }
    } catch (Throwable t) {
