...
public class DR implements osid.dr.DigitalRepository {
  ...
  public void setFedoraProperties(java.net.URL conf) {
    ...
    try {
      ...
      fedoraProperties.setProperty(
        "url.fedora.api",
        prefs.get("url.fedora.api", "")
      );
      fedoraProperties.setProperty(
        "url.fedora.type",
        prefs.get("url.fedora.type", "")
      );
      fedoraProperties.setProperty(
        "url.fedora.soap.access",
        url + prefs.get("url.fedora.soap.access", "")
      );
      fedoraProperties.setProperty(
        "url.fedora.get",
        url + prefs.get("url.fedora.get", "")
      );
      fedoraProperties.setProperty(
        "fedora.types",
        prefs.get("fedora.types", "")
      );
    } catch (Exception ex) {
  ...
  private void loadFedoraObjectAssetTypes() {
    try {
      Vector fedoraTypesVector = FedoraUtils.stringToVector(
        fedoraProperties.getProperty("fedora.types")
      );
  ...
  public FedoraObjectAssetType createFedoraObjectAssetType(String type)
    ...
    while (i.hasNext()) {
      FedoraObjectAssetType fedoraObjectAssetType =
        (FedoraObjectAssetType) i.next();
      if (
        fedoraObjectAssetType.getType().equals(type)
      ) return fedoraObjectAssetType;
    }
    FedoraObjectAssetType fedoraObjectAssetType = new FedoraObjectAssetType(
      this,
      type
    );
    osid.dr.InfoStructureIterator iter =
      fedoraObjectAssetType.getInfoStructures();
    while (iter.hasNext()) {
      osid.dr.InfoStructure infoStructure = (osid.dr.InfoStructure) iter.next();
      if (infoStructures.indexOf(infoStructure) < 0) infoStructures.add(
        infoStructure
      );
    }
  ...
  public FedoraObjectAssetType getAssetType(String type)
    throws osid.dr.DigitalRepositoryException {
    java.util.Iterator i = assetTypes.iterator();
    while (i.hasNext()) {
      FedoraObjectAssetType fedoraObjectAssetType =
  ...
  public osid.dr.InfoStructureIterator getInfoStructures()
    throws osid.dr.DigitalRepositoryException {
    return (osid.dr.InfoStructureIterator) new InfoStructureIterator(
      infoStructures
    );
  }
   ...
   *     @throws DigitalRepositoryException if there is a general failure
   */
  public osid.dr.InfoStructureIterator getMandatoryInfoStructures(
    osid.shared.Type assetType
  ) throws osid.dr.DigitalRepositoryException {
    return null;
  }
  ...
  public Asset getAsset(osid.shared.Id assetId)
    ...
    osid.dr.AssetIterator mAssetIterator = FedoraSoapFactory.advancedSearch(
      this,
      searchCriteria
    );
    if (
      mAssetIterator.hasNext()
    ) return mAssetIterator.next(); else throw new osid.dr.DigitalRepositoryException(
      "Object not found"
    );
  }
  ...
  public osid.dr.AssetIterator getAssets(
    ...
    if (searchCriteria instanceof String) {
      ...
      throw new osid.dr.DigitalRepositoryException(
        osid.dr.DigitalRepositoryException.UNKNOWN_TYPE
      );
    }
    ...
    if ((searchType.getKeyword().equals("Search"))) {
      ...
      try {
        ...
        while (st.hasMoreTokens()) {
          ...
          lSearchCriteria.setSearchOperation(SearchCriteria.FIND_OBJECTS);
          lSearchCriteria.setResults(0);
          osid.dr.AssetIterator ai = FedoraSoapFactory.search(
            this,
            lSearchCriteria
          );
          while (ai.hasNext()) {
  ...
  public osid.shared.Type getType() throws osid.dr.DigitalRepositoryException {
    throw new osid.dr.DigitalRepositoryException(
      osid.dr.DigitalRepositoryException.UNIMPLEMENTED
    );
  }
  ...
  public osid.dr.AssetIterator getAssetsBySearch(
  ...
  public osid.dr.AssetIterator getAssetsByType(osid.shared.Type type)
    throws osid.dr.DigitalRepositoryException {
    return null;
  }
  ...
  public osid.shared.Id ingest(
    ...
    throws osid.dr.DigitalRepositoryException, java.net.SocketException, java.io.IOException, osid.shared.SharedException, javax.xml.rpc.ServiceException {
    long sTime = System.currentTimeMillis();
    if (DEBUG) System.out.println(
      "INGESTING FILE TO FEDORA:fileName =" +
      fileName +
      "fileType =" +
      fileType +
      "t = 0"
    ...
    int port = Integer.parseInt(
      FedoraUtils.getFedoraProperty(this, "admin.ftp.port")
    );
    ...
    String directory = FedoraUtils.getFedoraProperty(
      this,
      "admin.ftp.directory"
    );
    ...
    client.storeFile(
      fileName,
      new FileInputStream(file.getAbsolutePath().replaceAll("%20", " "))
    );
    client.logout();
    client.disconnect();
    if (DEBUG) System.out.println(
      "INGESTING FILE TO FEDORA: Writting to FTP Server:" +
    ...
    BufferedInputStream fis = new BufferedInputStream(
      new FileInputStream(
        new File(getResource(templateFileName).getFile().replaceAll("%20", " "))
      )
    );
    ...
    fis.close();
    if (DEBUG) System.out.println(
      "INGESTING FILE TO FEDORA: Read Mets File:" +
      (System.currentTimeMillis() - sTime)
    );
    ...
    if (DEBUG) System.out.println(
      "INGESTING FILE TO FEDORA: Resplaced Metadata:" +
      (System.currentTimeMillis() - sTime)
    );
    ...
    if (DEBUG) System.out.println(
      "INGESTING FILE TO FEDORA: Ingest complete:" +
      (System.currentTimeMillis() - sTime)
    );
  ...
  public static String getMetadataString(Properties dcFields) {
    ...
    while (e.hasMoreElements()) {
      ...
      if (isSupportedMetadataField(field)) metadata +=
      "<" +
      DC_NAMESPACE +
      field +
      ">" +
      dcFields.getProperty(field) +
      "</" +
      DC_NAMESPACE +
      field +
      ">";
