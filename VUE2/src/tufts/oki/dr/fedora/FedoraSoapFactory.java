...
public class FedoraSoapFactory {
  ...
  public static Vector getDissemintionInfoRecords(
    String pid,
    osid.dr.InfoStructure infoStructure,
    DR dr
    ...
    try {
      ...
      ObjectMethodsDef[] objMethods = (ObjectMethodsDef[]) call.invoke(
        new Object[] { pid }
      );
      if (objMethods == null) throw new osid.dr.DigitalRepositoryException(
        "tufts.dr.FedoraObject():No Disseminations  returned"
      ); else {
        for (int i = 0; i < objMethods.length; i++) {
          InfoRecord infoRecord = new InfoRecord(
            new PID(objMethods[i].getMethodName()),
            infoStructure
          );
          infoRecord.createInfoField(
            ((DisseminationInfoStructure) infoStructure).getBDEFInfoPart()
              .getId(),
            objMethods[i].getBDefPID()
          );
          String disseminationURL =
            dr.getFedoraProperties().getProperty("url.fedora.get") +
            pid +
            "/" +
            objMethods[i].getBDefPID() +
            "/" +
            objMethods[i].getMethodName();
          infoRecord.createInfoField(
            ((DisseminationInfoStructure) infoStructure).getDisseminationURLInfoPart()
              .getId(),
            disseminationURL
          );
      ...
      throw new osid.dr.DigitalRepositoryException(
        "FedoraSoapFactory.getDisseminators " + ex.getMessage()
      );
    }
    return disseminationList;
  }

  public static FedoraObjectIterator search(
    DR dr,
    ...
    Call call;
    String fedoraApiUrl = dr
      .getFedoraProperties()
      .getProperty("url.fedora.api");
    ...
    try {
      ...
      if (searchOperation == SearchCriteria.FIND_OBJECTS) {
        methodDefs =
          (FieldSearchResult) call.invoke(
            new Object[] { resField, maxRes, query }
          );
        ListSession listSession = methodDefs.getListSession();
        if (listSession != null) lSearchCriteria.setToken(
          listSession.getToken()
        ); else lSearchCriteria.setToken(null);
      } else {
        if (lSearchCriteria.getToken() != null) {
          methodDefs =
            (FieldSearchResult) call.invoke(
              new Object[] { lSearchCriteria.getToken() }
            );
          ListSession listSession = methodDefs.getListSession();
          if (listSession != null) lSearchCriteria.setToken(
            listSession.getToken()
      ...
      if (methodDefs != null && methodDefs.getResultList().length > 0) {
        ...
        for (int i = 0; i < fields.length; i++) {
          ...
          if (fields[i].getTitle() != null) title = fields[i].getTitle()[0];
          resultObjects.add(
            new FedoraObject(
              dr,
              fields[i].getPid(),
              title,
              dr.getAssetType(fields[i].getCModel())
            )
          );
        }
      } else {
      ...
      ex.printStackTrace();
      throw new osid.dr.DigitalRepositoryException(
        "FedoraSoapFactory.search" + ex.getMessage()
      );
    }
  }

  public static FedoraObjectIterator advancedSearch(
    DR dr,
    SearchCriteria lSearchCriteria
  ) throws osid.dr.DigitalRepositoryException {
    ...
    try {
      ...
      java.util.Vector resultObjects = new java.util.Vector();
      FieldSearchResult methodDefs = (FieldSearchResult) call.invoke(
        new Object[] { resField, maxRes, query }
      );
      if (methodDefs != null) {
        ObjectFields[] fields = methodDefs.getResultList();
      ...
      throw new osid.dr.DigitalRepositoryException(
        "FedoraSoapFactory.advancedSearch" + ex.getMessage()
      );
    }
  }

  private static Call getCallMethods(DR dr)
    throws osid.dr.DigitalRepositoryException {
    //creates the new service and call instance
    Call call;
    try {
      String fedoraTypeUrl = dr
        .getFedoraProperties()
        .getProperty("url.fedora.type");
      String fedoraApiUrl = dr
        .getFedoraProperties()
        .getProperty("url.fedora.api");
      ...
      call.setTargetEndpointAddress(
        new URL(dr.getFedoraProperties().getProperty("url.fedora.soap.access"))
      );
      ...
      call.registerTypeMapping(
        ObjectMethodsDef.class,
        qn1,
        new BeanSerializerFactory(ObjectMethodsDef.class, qn1),
        new BeanDeserializerFactory(ObjectMethodsDef.class, qn1)
      );
      call.registerTypeMapping(
        ObjectProfile.class,
        qn2,
        new BeanSerializerFactory(ObjectProfile.class, qn2),
        new BeanDeserializerFactory(ObjectProfile.class, qn2)
      );
      call.registerTypeMapping(
        MethodParmDef.class,
        qn3,
        new BeanSerializerFactory(MethodParmDef.class, qn3),
        new BeanDeserializerFactory(MethodParmDef.class, qn3)
      );
    } catch (Exception ex) {
      throw new DigitalRepositoryException(
        "FedoraSoapFactory.getCallMethods " + ex.getMessage()
      );
    }
    return call;
  }

  private static Call getCallSearch(DR dr)
    throws osid.dr.DigitalRepositoryException {
    Call call;
    try {
      String fedoraTypeUrl = dr
      ...
      System.out.println(
        "FEDORA ACCESS URL = " +
        dr.getFedoraProperties().getProperty("url.fedora.soap.access")
      );
      call.setTargetEndpointAddress(
        new URL(dr.getFedoraProperties().getProperty("url.fedora.soap.access"))
      );
      ...
      throw new DigitalRepositoryException(
        "FedoraSoapFactory.getCallSearch " + ex
      );
    }
  }

  private static Call getCallAdvancedSearch(DR dr)
    throws osid.dr.DigitalRepositoryException {
    Call call;
    try {
      String fedoraTypeUrl = dr
      ...
      throw new DigitalRepositoryException(
        "FedoraSoapFactory.getCallSearch " + ex.getMessage()
      );
    }
  }
}
