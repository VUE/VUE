...
public class VUEInfoStructure implements osid.dr.InfoStructure {
  ...
  public osid.dr.InfoPart getVUEDefaultViewInfoPart()
    throws osid.dr.DigitalRepositoryException {
    if (
      this.sVUEDefaultViewInfoPart == null
  ...
  public static InfoRecord createVUEInfoRecord(
    String pid,
    VUEInfoStructure infoStructure,
    DR dr,
    PID objectId,
    ...
    if (
      assetType.getKeyword().equals("TUFTS_STD_IMAGE")
    ) infoRecord.createInfoField(
      infoStructure.getVUEDefaultViewInfoPart().getId(),
      dr.getFedoraProperties().getProperty("url.fedora.get") +
      "/" +
      objectId.getIdString() +
      "/bdef:11/getDefaultView/"
    ); else if (
      assetType.getKeyword().equals("XML_TO_HTMLDOC")
    ) infoRecord.createInfoField(
      infoStructure.getVUEDefaultViewInfoPart().getId(),
      dr.getFedoraProperties().getProperty("url.fedora.get") +
      "/" +
      objectId.getIdString() +
      "/bdef:11/getDefaultView/"
    ); else infoRecord.createInfoField(
      infoStructure.getVUEDefaultViewInfoPart().getId(),
      dr.getFedoraProperties().getProperty("url.fedora.get") +
      "/" +
      objectId.getIdString()
