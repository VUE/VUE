...
public class SyncDialog extends JDialog implements ActionListener, KeyListener {
  ...
  private final JRadioButton nodeToSlideButton = new JRadioButton(
    VueResources.getString("jbutton.nodetoslide.label")
  );
  private final JRadioButton slideToNodeButton = new JRadioButton(
    VueResources.getString("jbutton.slidetonode.label")
  );
  private final JRadioButton syncAllButton = new JRadioButton(
    VueResources.getString("jbutton.bothways.label")
  );

  public SyncDialog(Frame parentFrame, Point location) {
    super(
      parentFrame,
      VueResources.getString("dialog.syncresource.title"),
      true
    );
    setSize(390, 200);
  ...
  private boolean fireAction() {
    ...
    if (entry == null || entry.isMapView()) {
      tufts.Util.printStackTrace(
        this +
        ": no entry or not allowed to sync virtual slides; action should have been disabled; entry=" +
        entry
      );
      return false;
    }
    ...
    if (nodeToSlideButton.isSelected()) Actions.SyncToSlide.fire(
      this
    ); else if (slideToNodeButton.isSelected()) Actions.SyncToNode.fire(
      this
    ); else if (syncAllButton.isSelected()) Actions.SyncAll.fire(
      this
    ); else return false;
