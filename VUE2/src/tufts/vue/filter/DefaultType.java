...
public class DefaultType implements tufts.vue.filter.Type {
  ...
  public java.util.List getSettableOperators() {
    ...
    while (i.hasNext()) {
      ...
      if (operator.isSettable()) settableOperatorList.add(operator);
    }
  ...
  public Operator getDefaultOperator() {
    if (operatorList.size() > 0) return (Operator) operatorList.get(
      0
    ); else throw new RuntimeException(displayName + ": Type has no operators");
  }
