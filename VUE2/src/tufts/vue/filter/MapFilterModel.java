...
public class MapFilterModel extends AbstractTableModel {
  ...
  public synchronized void add(Key key) {
    keyVector.add(key);
    notifyListeners(
      new MapFilterModelEvent(this, key, MapFilterModelEvent.KEY_ADDED)
    );
  }
  ...
  public synchronized void remove(Key key) {
    ...
    while (iter.hasNext()) ((tufts.vue.LWComponent) iter.next()).getNodeFilter()
      .removeStatements(key);
    tufts.vue.VUE.getActiveMap().getLWCFilter().removeStatements(key);
    notifyListeners(
      new MapFilterModelEvent(this, key, MapFilterModelEvent.KEY_DELETED)
    );
  }
  ...
  public Object getValueAt(int row, int col) {
    ...
    if (col == 0) return key.getKey().toString(); else return key
      .getType()
      .getDisplayName();
  }
  ...
  public void setValueAt(Object value, int row, int col) {
    Key key = (Key) this.get(row);
    if (col == 0) key.setKey((String) value);
