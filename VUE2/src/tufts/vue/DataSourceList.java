...
// TODO: remove constructor reference to DataSourceViewer: handle via globally
// posted events
...
public class DataSourceList extends JList implements DropTargetListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DataSourceList.class);
      ...
      .addDataSourceListener(
        new edu.tufts.vue.dsm.DataSourceListener() {
          public void changed(
            ...
            if (DEBUG.DATA || DEBUG.EVENTS) Log.debug(
              "data sources changed: repaint"
            );
            repaint();
          }
        }
  ...
  /*
   * We are going to add some cleverness to the way data sources are ordered.
  ...
  public void addOrdered(Object o) {
    ...
    if (o instanceof edu.tufts.vue.dsm.DataSource) {
      if (
        ((edu.tufts.vue.dsm.DataSource) o).getRepositoryDisplayName()
          .trim()
          .length() ==
        0
      ) {
        Log.debug(
          "ignoring repository with empty display name: " + Util.tags(o)
        );
        return;
      }
      ...
      for (int i = 0; i < size; i++) {
        Object obj = model.elementAt(i);
        if (
          (!(obj instanceof edu.tufts.vue.dsm.DataSource)) &&
          (!(obj instanceof LocalFileDataSource))
        ) {
          model.insertElementAt(o, i);
          return;
        }
  ...
  public void dragOver(DropTargetDragEvent e) {
    ...
    if (over instanceof FavoritesDataSource) {
      ...
      setIndicated(null);
      if (ALLOW_FEED_DROPS) e.acceptDrag(
        DnDConstants.ACTION_LINK
      ); else e.rejectDrag();
    }
  }
  ...
  public void drop(DropTargetDropEvent e) {
    ...
    if (over instanceof FavoritesDataSource) {
      ...
      if (ALLOW_FEED_DROPS) {
        ...
        if (DEBUG.DND) try {
          new MapDropTarget(null).processTransferable(transfer, null);
        } catch (Throwable t) {}
        if (transfer.isDataFlavorSupported(DataFlavor.stringFlavor)) {
          String txt = MapDropTarget.extractData(
            transfer,
            DataFlavor.stringFlavor,
            String.class
          );
          if (txt.startsWith("feed:")) {
            BrowseDataSource feed = new edu.tufts.vue.rss.RSSDataSource(
              txt,
              txt
            );
    ...
    try {
        ...
        if (transfer.isDataFlavorSupported(Resource.DataFlavor)) {
          ...
          if (data instanceof Resource) {
            ...
            Util.printStackTrace(
              "Unhandled drop type: " + Util.tag(data) + "; " + data
            );
            return;
          }
            ...
            if (file.isDirectory()) {
              try {
                ...
                if (file.getPath().toLowerCase().endsWith(".url")) {
                  String url = MapDropTarget.convertWindowsURLShortCutToURL(
                    file
                  );
                  if (url != null) {
                    res.setSpec(url);
                    String resName;
                    if (file.getName().length() > 4) resName =
                      file
                        .getName()
                        .substring(
                          0,
                          file.getName().length() - 4
                        ); else resName = file.getName();
              ...
              // //tufts.Util.printStackTrace("Unsupported DROP onto " + ds + "; of " +
              // transfer);
              ...
              try {
                ...
                // So dropping directories in works fine, but drop anything else and you're
                // hosed.
                ...
                CabinetNode cabNode = null;
                if (oldentry instanceof RemoteCabinetEntry) cabNode =
                  new CabinetNode(res, CabinetNode.REMOTE); else cabNode =
                  new CabinetNode(res, CabinetNode.LOCAL);
                model.insertNodeInto(cabNode, rootNode, 0);
                favoritesTree.expandPath(new TreePath(rootNode.getPath()));
          ...
          String dataString = (String) transfer.getTransferData(
            DataFlavor.stringFlavor
          );
      ...
      // Very annoying if you want to drag items from search results into My
      // Favorites:
      ...
      VueUtil.alert(
        null,
        VueResources.getString("dialog.addfav.message"),
        VueResources.getString("dialog.addfav.message")
      );
    }
  }
