 ...
 *
 **/
public class VueAimPanel
  extends JPanel
  implements ActionListener, ClientConnListener, ItemListener {
  ...
  public VueAimPanel() {
    ...
    assignColorCheckbox =
      new JCheckBox(VueResources.getString("im.button.assigncolor"));
    ignoreCheckbox = new JCheckBox(VueResources.getString("im.button.ignore"));
    approveCheckbox =
      new JCheckBox(VueResources.getString("im.button.approve"), true);
    nodeWidthCheckbox =
      new JCheckBox(VueResources.getString("im.button.nodewidth"), true);
    makeTableCheckbox =
      new JCheckBox(VueResources.getString("im.button.maketable"), true);
    zoomCheckbox =
      new JCheckBox(VueResources.getString("im.button.zoom"), true);
    enforceColCheckbox =
      new JCheckBox(VueResources.getString("im.button.enforce"), false);

    resetApproveButton =
      new JButton(VueResources.getString("im.approve.reset"));
    ...
    mPropPanel.addProperty(
      VueResources.getString("im.username"),
      mUsernameEditor
    ); //added through metadata
    mPropPanel.addProperty(
      VueResources.getString("im.password"),
      mPasswordEditor
    );
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource().equals(loginButton)) {
      ...
      if (aim.isConnected()) aim.disconnect(); else {
        aim.connect();
      ...
      if (aim != null) aim.resetApprovalList();
    }
  }

  public void stateChanged(ClientConnEvent arg0) {
    if (
      arg0.getNewState().equals(BasicConn.STATE_CONNECTED)
    ) loginButton.setText(VueResources.getString("im.button.logout")); else if (
      arg0.getNewState().equals(BasicConn.STATE_NOT_CONNECTED)
