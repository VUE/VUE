...
public class DataSetViewer extends ContentViewer {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DataSetViewer.class);
  ...
  protected void loadDataSets() {
    ...
    if (!file.exists()) {
      if (DEBUG.DR) System.out.println(
        "Loading Datasets (does not exist: " + file + ")"
      );
    } else {
      try {
        SaveDataSourceViewer dataSourceViewer = unMarshallMap(file);
        Vector<BrowseDataSource> dataSources =
          dataSourceViewer.getSaveDataSources();
    ...
    dataSourceList.addMouseListener(
      new MouseAdapter() {
        public void mouseClicked(MouseEvent event) {
          ...
          DataSource ds = (DataSource) dataSourceList
            .getModel()
            .getElementAt(index);
  ...
  public static void saveDataSetViewer() {
    if (dataSourceList == null) {
      ...
      if (DEBUG.DR) Log.debug(
        "saveDataSetViewer: found " +
        size +
        " dataSets: scanning for local's to save..."
      );

      for (int i = 0; i < size; i++) {
        Object item = dataSourceList.getModel().getElementAt(i);

        if (DEBUG.DR) System.err.print(
          "\tsaveDataSetViewer: item " +
          i +
          " is " +
          tufts.Util.tag(item) +
          "[" +
      ...
      if (DataSourceViewer.dataSourceList != null) {
        ...
        for (int i = 0; i < size; i++) {
          Object item = DataSourceViewer.dataSourceList
            .getModel()
            .getElementAt(i);
      ...
      try {
        if (DEBUG.DR) Log.debug(
          "saveDataSetViewer: creating new SaveDataSourceViewer"
        );
        ...
        if (DEBUG.DR) Log.debug(
          "saveDataSourceViewer: marshallMap: saving " + sViewer + " to " + file
        );
  ...
  private void positionEditInfoWindow() {
    ...
    if (
      (DSB.dockWindow.getX() +
        DSB.dockWindow.getWidth() +
        editInfoDockWindow.getWidth()) <
      screenSize.getWidth()
    ) editInfoDockWindow.setLocation(
      DSB.dockWindow.getX() + DSB.dockWindow.getWidth(),
      DSB.dockWindow.getY()
    ); else editInfoDockWindow.setLocation(
      DSB.dockWindow.getX() - editInfoDockWindow.getWidth(),
      DSB.dockWindow.getY()
    );
  }
  ...
  private static DockWindow _buildWindow() {
    final DockWindow dw = GUI.createDockWindow(
      VueResources.getString("dockWindow.dataset.title")
    );
