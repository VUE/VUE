 ...
 *
 * @author Scott Fraize
 * @version $Revision: 1.118 $ / $Date: 2010-02-03 19:17:41 $ / $Author: mike $
 */
public class LWSlide extends LWContainer {

  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWSlide.class);
  ...
  public static final float SlideAspect =
  ...
  private static final Rectangle2D SlideZeroShape = new Rectangle2D.Float(
    0,
    0,
    SlideWidth,
    SlideHeight
  ...
  public final java.awt.Shape getZeroShape() {
    if (mEntry == null) return super.getZeroShape(); // if entry is null, is an on-map slide that may have been resized
    else return SlideZeroShape; // as slides all same size for now, can use a constant zeroShape
  }
  ...
  protected void setParent(LWContainer parent) {
    ...
    if (mEntry == null && parent instanceof LWPathway == false) { // parent will be pathway for master slides
      ...
      if (getFillColor() == null) setFillColor(Color.gray);
      takeScale(0.5);
  ...
  public String getLabel() {
    if (supportsProperty(LWKey.Label)) return super.getLabel();

    if (mEntry != null) {
      if (getSourceNode() == null) return (
        "Slide in " + mEntry.pathway.getDisplayLabel()
      ); else return (
        "Slide for " +
        getSourceNode().getDisplayLabel() +
        " in " +
        mEntry.pathway.getDisplayLabel()
      );
    } else return "<LWSlide:initializing>"; // true during persist restore
  }
  ...
  private abstract static class SlideStylingTraversal extends LWTraversal {
  ...
  private static void applyMasterStyle(
    MasterSlide master,
    LWComponent c,
    boolean resetStyle
  ) {
    ...
    if (c.hasResource() && !c.getResource().isImage()) style =
      master.getLinkStyle(); else if (c instanceof LWText) style =
      master.getTextStyle(); else if (c instanceof LWNode) style =
      master.getTextStyle();
    ...
    if (style != null) {
      if (KEEP_DIVERGENT_STYLES_ON_DUPLICATE) {
        if (resetStyle) c.setStyle(style); else c.takeStyle(style);
      } else {
        c.setStyle(style);
      }
    }
    ...
    if (DEBUG.Enabled) {
      if (c.getStyle() == null) track(
        "skip-style",
        String.format(
          "%-14s %s",
          "(" +
          (c.getTypeToken() instanceof Class
              ? ((Class) c.getTypeToken()).getSimpleName()
              : c.getTypeToken()) +
          ")",
          c
        )
        ...
        "styled-as",
        String.format("%-14s %s", c.getStyle().getLabel() + ";", c)
      );
    }
  }
  ...
  private void applyStyle(LWComponent c, boolean resetStyle) {
    ...
    c.setFlag(Flag.SLIDE_STYLE);
    if (c.getStyle() == null) applyMasterStyle(c, resetStyle);
  ...
  public boolean inPathway(LWPathway p) {
    if (mEntry == null) return super.inPathway(p); else return (
      mEntry.pathway == p
    );
  ...
  public MasterSlide getMasterSlide() {
    if (
      mEntry == null
    ) return null; else return mEntry.pathway.getMasterSlide();
  }
  ...
  public Color getRenderFillColor(DrawContext dc) {
    if (mFillColor.isTransparent()) {
      ...
      if (
        master == null
      ) return getFillColor(); else return master.getFillColor();
  ...
  protected void drawImpl(DrawContext dc) {
    ...
    final boolean onMapSlideIcon =
      isSlideIcon() && (dc.focal != this || dc.focused == this);
    ...
    if (DEBUG.BOXES) {
      ...
      onMapSlideIcon &&
      !drewBorder &&
      /*&& !dc.isAnimating()*/!dc.isPrintQuality()
    ) { // need: isPrinting
    ...
    if (dc.focal != this) dc.g.clip(getZeroShape());
  ...
  public LWSlide duplicate(CopyContext cc) {
    ...
    if (
      newSlide.mEntry == null &&
      newSlide.isTransparent() &&
      getMasterSlide() != null
    ) {
  ...
  public boolean handleDoubleClick(MapMouseEvent e) {
    ...
    if (e == null) { // special case message from MapViewer
      ...
      if (DEBUG.Enabled) Actions.LaunchPresentation.fire(
        "MapViewerHandToolDoubleClick"
      );
  ...
  protected LWComponent pickChild(PickContext pc, LWComponent c) {
    ...
    if (
      pc.root == this || (!SwapFocalOnSlideZoom && pc.dc.zoom > 4)
    ) return c; else return this;
  }
