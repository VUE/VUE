...
*/
  ...
  static void setMapIndicationIfOverValidTarget(
    ...
    if (DEBUG.CONTAINMENT || DEBUG.PICK || DEBUG.LINK) System.out.println(
      "LINK-TARGET-POTENTIAL: " +
      over +
      "; src=" +
      linkSource +
      "; link=" +
    ...
    if (over != null && isValidLinkTarget(link, linkSource, over)) {
      ...
      if (
        currentIndication != null && currentIndication != over
  ...
  private static LWComponent pickLinkTarget(LWLink link, MapMouseEvent e) {
    ...
    if (hit == e.getViewer().getFocal()) return null; else return hit;
  }
   ...
   */
  static boolean isValidLinkTarget(
    LWLink link,
    LWComponent linkSource,
    LWComponent linkTarget
  ) {
    ...
    if (linkSource != null) {
      ...
      if (linkTarget != null) {
        if (!linkSource.canLinkTo(linkTarget)) {
          if (DEBUG.LINK) reject(
            linkTarget,
            "source denies target; src=" + linkSource
          );
          return false;
        }
      }
    ...
    if (
      link != null &&
      linkTarget instanceof LWLink &&
      linkTarget.isConnectedTo(link)
    ) {
      // Don't permit a link to link back to a link that's connected to it.
      if (DEBUG.LINK) reject(
        linkTarget,
        "this link already connected to target"
      );
      return false;
    }
  ...
  public static class OntologyLinkModeTool
    extends LinkModeTool
    implements edu.tufts.vue.ontology.ui.OntologySelectionListener {
    ...
    public OntologyLinkModeTool() {
      edu.tufts.vue.ontology.ui.OntologyBrowser
        .getBrowser()
        .addOntologySelectionListener(this);
    ...
    public boolean handleMousePressed(MapMouseEvent e) {
      if (creationLink == null) {
        VueUtil.alert(
          VueResources.getString("ontologyLinkError.message"),
          VueResources.getString("ontologyLinkError.title")
        );
        return true;
      } else return false;
    }
    ...
    public boolean handleMouseReleased(MapMouseEvent e) {
      ...
      if (
        Math.abs(e.getDeltaPressX()) > 10 || Math.abs(e.getDeltaPressY()) > 10
      ) { // todo: config min dragout distance
        //repaintMapRegionAdjusted(creationLink.getBounds());
        LWComponent linkDest = e.getViewer().getIndication();
        if (linkDest != linkSource) makeLink(
          e,
          linkSource,
          linkDest,
          !e.isShiftDown(),
    ...
    private void makeLink(
      ...
      LWContainer commonParent = e.getMap();
      if (pLinkDest == null) commonParent = pLinkSource.getParent(); else if (
        pLinkSource.getParent() == pLinkDest.getParent() &&
        pLinkSource.getParent() != commonParent
      ) {
        // todo: if parents different, add to the upper most parent
        commonParent = pLinkSource.getParent();
      ...
      if (pMakeConnection && (comboMode || pLinkDest != null)) {
        ...
        if (existingStraightLinks > 0) link.setControlCount(1);
      } else {
      ...
      // is currently drawn to it's center, which might paint over
      // a label.
      if (pLinkSource instanceof LWLink) commonParent.ensurePaintSequence(
        link,
        pLinkSource
      );
      if (pLinkDest instanceof LWLink) commonParent.ensurePaintSequence(
        link,
        pLinkDest
      );
      VUE.getSelection().setTo(link);
      if (pMakeConnection && comboMode) e
        .getViewer()
        .activateLabelEdit(createdNode ? pLinkDest : link);
    }
    ...
    public boolean handleComponentPressed(MapMouseEvent e) {
      ...
      if (hit != null && hit.canLinkTo(null)) {
        ...
        if (
          creationLink.getStrokeWidth() < minStrokeWidth
  ...
  static class LinkModeTool extends VueTool {
    ...
    public void handlePostDraw(DrawContext dc, MapViewer viewer) {
      if (linkSource != null) creationLink.draw(dc);
    }
  ...
  public static class SubTool extends VueSimpleTool {
    ...
    public VueAction getSetterAction() {
      if (setterAction == null) {
        setterAction =
          new Actions.LWCAction(getToolName(), getIcon()) {
            void act(LWLink c) {
              c.setControlCount(curveCount);
            }
          };
