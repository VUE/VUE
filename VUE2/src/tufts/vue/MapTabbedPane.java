...
public class MapTabbedPane
  extends JTabbedPane //extends DnDTabbedPane
  implements LWComponent.Listener, FocusListener, MapViewer.Listener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MapTabbedPane.class);
  ...
  protected void fireStateChanged() {
    try {
      if (DEBUG.FOCUS) out(
        "fireStateChanged, selectedIndex=" +
        getSelectedIndex() +
        "; viewerAtIndex=" +
        getViewerAt(getSelectedIndex())
      );
    ...
    if (GUI.isMacAqua() == false) {
      ...
      if (mWasSelected >= 0) setForegroundAt(mWasSelected, Color.darkGray);
    ...
    if (viewer != null/*&& !VUE.isStartupUnderway()*/) {
      ...
      GUI.invokeAfterAWT(
        new Runnable() {
          public void run() {
            if (DEBUG.FOCUS) out(
              "after AWT focus jibber-jabber (now at end of AWT event queue), focus is being demanded by: " +
              viewer
            );
            ...
            viewer.grabVueApplicationFocus(
              toString() + ".setSelectedIndex=" + index + " " + viewer,
              null
            );
          }
        }
  ...
  public void advanceToNextTab() {
    if (getSelectedIndex() >= getTabCount() - 1) setSelectedIndex(
      0
  ...
  private String mapToTabTitle(LWMap map) {
    ...
    if (title.toLowerCase().endsWith(".vue") && title.length() > 4) title =
      title.substring(0, title.length() - 4);
    if (map.isCurrentlyFiltered()) title += "*";
    return title;
  }

  private String viewerToTabTitle(MapViewer viewer) {
    if (DEBUG.WORK) return mapToTabTitle(viewer.getMap()); else return (
      mapToTabTitle(viewer.getMap()) +
      " (" +
  ...
  private void updateTitleTextAt(int i) {
    if (i >= 0) {
      ...
      String tooltip = null;
      if (map.getFile() != null) tooltip =
        map.getFile().toString(); else tooltip =
        "<html>&nbsp;<i>Unsaved</i> &nbsp;";
      if (map.isCurrentlyFiltered()) tooltip += " (filtered)";
      setToolTipTextAt(i, tooltip);
    }
  }

  static final int TitleChangeMask =
    MapViewer.Event.DISPLAYED | MapViewer.Event.FOCUSED | MapViewer.Event.ZOOM; // title includes zoom
  ...
  public void LWCChanged(LWCEvent e) {
    if (e.getSource() instanceof LWMap) updateTitleTextAt(
      findTabWithMap((LWMap) e.getSource())
    );
  }
  ...
  public void addViewer(MapViewer viewer) {
    ...
    if (false) {
      ...
      if (tabTitle == null) tabTitle = "unknown";
  ...
  public int indexOfComponent(Component component) {
    for (int i = 0; i < getTabCount(); i++) {
      ...
      if (c instanceof JScrollPane) {
        if (component == ((JScrollPane) c).getViewport().getView()) return i;
      }
    }
    return -1;
  }
  ...
  public MapViewer getViewerAt(int index) {
    ...
    if (c instanceof MapViewer) viewer = (MapViewer) c; else if (
  ...
  public void closeMap(LWMap map) {
    ...
    if (forceFocusTransfer) {
      ...
      if (DEBUG.FOCUS) out(
        "closeMap force focus transfer to new selected tab index: " +
        selectedIndex
      );
      if (selectedIndex >= 0) getViewerAt(selectedIndex)
        .grabVueApplicationFocus("closeMap", null); else VUE.setActive(
        MapViewer.class,
        this,
        null
      ); // no open viewers
