...
public class LWMap extends LWContainer
{
    ...
    public LWMap() {
        ...
        // HO 15/09/2010 BEGIN *****************
		addLWCListener(new LWComponent.Listener() {
...
// //         addChild(mInternalLayer);
    ...
    public void markAsSaved() {
    	// HO 07/09/2010 BEGIN ******************
    	// this is where we can reliably check and see if any wormholes
    	// need to be reconstructed
    	//constructWormholesFollowingSave(this.getFile());
    	// HO 07/09/2010 END ******************
    ...
    // HO 10/08/2010 BEGIN **********************
    /**
     * @return all instances of WormholeResource objects found in the map, even if some of them point to the same destination.
     */
    public Collection<WormholeResource> getAllWormholeResources() {

        final Map<WormholeResource,Boolean> wormholeResources = new IdentityHashMap(); // really, want IdentityHashSet

        for (LWComponent c : getAllDescendents(ChildKind.ANY))
            if (c.hasResource()) {
            	Resource r = c.getResource();
            	if (r.getClass().equals(tufts.vue.WormholeResource.class))
                wormholeResources.put((WormholeResource)r, Boolean.TRUE);
            }
        return wormholeResources.keySet();
    }
    // HO 10/08/2010 END **********************
    
    // HO 23/08/2010 BEGIN **********************
    /**
     * @return all instances of WormholeNode objects found in the map
     */
    public Collection<LWWormholeNode> getAllWormholeNodes() {

        final Map<LWWormholeNode,Boolean> wormholeNodes = new IdentityHashMap(); // really, want IdentityHashSet

        for (LWComponent c : getAllDescendents(ChildKind.ANY))
            //if (c.hasResource()) {
            	//Resource r = c.getResource();
            	if (c.getClass().equals(tufts.vue.LWWormholeNode.class))
                wormholeNodes.put((LWWormholeNode)c, Boolean.TRUE);
            //}
        return wormholeNodes.keySet();
    }
    // HO 23/08/2010 END **********************    
...
//                 recordRelativeLocations(getAllResources(), getFile().getParentFile());
    ...
    class ResourceFactory extends Resource.DefaultFactory {
        ...
        private void track(Resource r) {
            ...
            // HO 16/07/2010 BEGIN
            // the trouble with using the spec to see if a resource has
            // been duplicated is that you can have two WormholeResources
            // with the same spec but different Components in them
            // and if that's the case, this will treat them as the same
            // resource, chucking out bidirectionality.
            // There must be a better way of handling it than this hack, but...
            //if (already != null) {
            // basically, unless it's a WormholeResource, pass it over if the spec 
            // is already in the map
            if ((already != null) && (!r.getClass().equals(tufts.vue.WormholeResource.class))) {
            	// HO 16/07/2010 END 
...
//             + "]";
	...
	// HO 20/07/2010 BEGIN 
	public LWComponent findChildByURI(URI theURI) {
	...
	// HO 20/07/2010 BEGIN 
	public LWComponent findChildByID(String theID) {
        Iterator iter = this.getAllDescendents().iterator();
        while(iter.hasNext()) {
        	LWComponent nextComponent = (LWComponent)iter.next();
        	String nextID = nextComponent.getID();
        	if (nextID.equals(theID)) {
        		return nextComponent;
        	}
        }
        // if we got this far there's nothing to return
