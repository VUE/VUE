...
public class LWMap extends LWContainer {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWMap.class);
  ...
  LWMap(LWComponent c) {
    ...
    if (c instanceof LWGroup && c.getFillColor() != null) setFillColor(
      c.getFillColor()
    );
    mChildren.add(c);
  ...
  @Override
  protected final double getMapXPrecise() {
    return 0;
  }

  @Override
  protected final double getMapYPrecise() {
    return 0;
  }
  ...
  @Override
  public final boolean isAncestorCollapsed() {
    return false;
  }
  ...
  public void setModelVersion(int version) {
    if (DEBUG.Enabled) {
      ...
      ) Log.debug("setModelVersion " + version + "; current=" + mModelVersion); // don't bother with this message on construction
    }
  ...
  public void setUndoManager(UndoManager um) {
    ...
    if (mUndoManager != null) throw new IllegalStateException(
      this + " already has undo manager " + mUndoManager
    );
    mUndoManager = um;
    markAsSaved();
  }
  ...
  public void markAsSaved() {
    if (DEBUG.Enabled && mChanges != 0) Log.debug(
      "marking " + mChanges + " modifications as current: " + this
    );
    mChanges = 0;
  }
  ...
  public final boolean isCurrentlyFiltered() {
    return false;
  }

  /** @deprecated this api / LWCFilter is no longer used -- always return null */
  public final LWCFilter getLWCFilter() {
    return null;
  }

  /** @deprecated this api / LWCFilter is no longer used -- always return null */
  public final UserMapType[] getUserMapTypes() {
    return null;
  }

  /** @deprecated this api / LWCFilter is no longer used -- always dump exception */
  public final void setUserMapTypes(UserMapType[] x) {
    Log.warn("UserMapType deprecated: " + Util.tags(x), new Throwable("HERE"));
  }
  ...
  public void setDescription(String pDescription) {
    ...
    if (!hasNotes()) setNotes(pDescription);
  }
  ...
  protected int mNodeCount, mLinkCount, mGroupCount, mConnectionCount, mMinConnections, mMaxConnections;
    ...
    for (LWComponent child : c.getChildren()) {
      if (child instanceof LWGroup) {
        ...
        for (LWLink link : child.getLinks()) {
          LWComponent head = link.getPersistHead(), tail =
            link.getPersistTail();

          if (
            head != null &&
            tail != null &&
            head instanceof LWNode &&
            tail instanceof LWNode
          ) {
            connections++;
          }
        }
  ...
  public String getObjectStatistics() {
    ...
    return String.format(
      Locale.getDefault(),
      VueResources.getString("mapinspectorpanel.objectStats.format"),
      VueResources.getString("mapinspectorpanel.objectStats.nodes"),
      mNodeCount,
      VueResources.getString("mapinspectorpanel.objectStats.links"),
      mLinkCount,
      VueResources.getString("mapinspectorpanel.objectStats.groups"),
      mGroupCount
    );
  }
  ...
  public String getConnectivityStatistics() {
    ...
    double avg =
      (mNodeCount == 0
          ? 0.0
          : ((double) mConnectionCount) / (double) mNodeCount);

    return String.format(
      Locale.getDefault(),
      VueResources.getString("mapinspectorpanel.connectivityStats.format"),
      VueResources.getString("mapinspectorpanel.connectivityStats.min"),
      mMinConnections,
      VueResources.getString("mapinspectorpanel.connectivityStats.max"),
      mMaxConnections,
      VueResources.getString("mapinspectorpanel.connectivityStats.avg"),
      avg
    );
  }
  ...
  private Collection<LWComponent> getAllLayerDescendents(
    ChildKind kind,
    Collection bag,
    Order order,
    ...
    for (LWComponent layer : getChildren()) {
      ...
      if (
        (kind == ChildKind.VISIBLE || kind == ChildKind.EDITABLE) &&
        layer.isHidden()
      ) {
        // exclude invisible
  ...
  public Collection<Resource> getAllResources() {
    ...
    for (LWComponent c : getAllDescendents(ChildKind.ANY)) if (
      c.hasResource()
  ...
  public Collection<Resource> getAllUniqueResources() {
    ...
    for (Resource r : getAllResources()) {
      if (resources.add(r)) {
        if (DEBUG.RESOURCE) Log.debug(
          "getAllUniqueResources: Found resource " + r
        );
      } else {
        if (DEBUG.RESOURCE) Log.debug(
          "getAllUniqueResources:      duplicate " + r
        );
      }
    }
  ...
  public Collection getArchiveManifest() {
    if (mXMLRestoreUnderway) {
      if (mArchiveManifest == null) mArchiveManifest = new ArrayList();
    }
  ...
  private final java.util.concurrent.atomic.AtomicInteger mNextID =
  ...
  private void upgradeAbsoluteToRelativeCoords(
    Collection<LWComponent> children
  ) {
    ...
    for (LWComponent c : children) {
      if (c instanceof LWContainer && c.hasChildren()) {
        if (c instanceof LWGroup) {
          ...
          if (java.awt.Color.white.equals(c.getFillColor())) c.setFillColor(
            null
          );
          if (isGroupRelative(getModelVersion())) {
            if (DEBUG.Enabled) System.out.println(
              " DM#1 ALREADY RELATIVE EXCEPT LINKS: " + c
            );
  ...
  private void upgradeLinksToParentRelative(Collection<LWComponent> nodes) {
    for (LWComponent c : nodes) {
      if (c instanceof LWLink == false) continue;
  ...
  private void upgradeChildrenFromAbsoluteToRelative(LWContainer container) {
    if (DEBUG.Enabled) System.out.println(
      "CONVERTING TO RELATIVE in " + this + "; container: " + container
    );
    for (LWComponent c : container.getChildren()) {
      ...
      // of operations here...
      c.takeLocation(c.getX() - container.getX(), c.getY() - container.getY());
    }
  }
  ...
  public static final class Layer extends LWContainer {
    ...
    public double getMapScale() {
      return 1;
    } // currently always true: for performance
    ...
    public void setFiltered(boolean filtered) {
      ...
      if (DEBUG.Enabled) Log.debug(
        "layers ignore filtered; (request=" + filtered + ") " + this
      );
    }
    ...
    protected void setParent(LWContainer p) {
      if (p instanceof LWMap) {
        ...
        Util.printStackTrace(
          "Layers can only be parented to map; attempted to add " +
          this +
          " to " +
          p
        );
    ...
    private void add(LWComponent c) {
      if (mChildren == NO_CHILDREN) mChildren = new ArrayList();
      mChildren.add(c);
    }
    ...
    protected void setAsChildAndLocalize(LWComponent c) {
      if (c.getParent() instanceof Layer) {
        ...
        if (
          DEBUG.Enabled && getMap() != null && c.getMap() != getMap()
        ) Util.printStackTrace(
          "cross-map reparenting! " + c.getMap() + " -> " + getMap()
        );
        c.setParent(this);
      } else {
    ...
    @Override
    public List<LWComponent> getXMLChildList() {
      return null;
    }
    ...
    public Layer duplicate(CopyContext cc) {
      ...
      if (hasLabel()) c.setLabel(getLabel() + " Copy");
      return (Layer) c;
    }
  ...
  public void setActiveLayer(LWComponent layer) {
    ...
    if (layer == null || layer instanceof Layer) mActiveLayer =
      (Layer) layer; else Util.printStackTrace(
      "not a layer: " + Util.tags(layer)
    );
  }
      ...
      if (hasChildren()) {
        ...
        for (LWComponent c : getChildren()) {
          if (c instanceof Layer) {
            ...
            Log.warn(
              "getXMLChildList: non-layer direct child of map; " +
              this +
              ": " +
              c
            );
        ...
        Log.debug(
          "getXMLChildList: built integrated list of all layer members, n=" +
          childrenInAllLayers.size()
        );
  ...
  private boolean reparentAllToLayers() {
    ...
    if (mLayers.size() > 0) {
      if (DEBUG.Enabled) Log.debug(
        "restoring: found existing layers in " + this
      );

      for (Layer layer : mLayers) {
        for (LWComponent c : mChildren) {
          if (c.getParent() == layer) layer.add(c);
        }
      }
    ...
    if (!addedLayers) {
      ...
      if (defaultLayer != null) setActiveLayer(defaultLayer); else if (
        mLayers.size() > 1
      ) setActiveLayer(mLayers.get(1)); else setActiveLayer(mLayers.get(0));
    }
  ...
  private void installDefaultLayers() {
    ...
    if (true) {
      ...
      for (LWComponent c : this.mChildren) c.setParent(layer0);
      ...
      for (LWComponent c : LWMap.this.mChildren) c.setParent(layer1);
  ...
  @Override
  public void layoutAll(Object trigger) {
   ...
   */
  private void layoutAllAfterRestore(
    final Collection<LWComponent> components,
    final Object key
  ) {
    Object layoutKey;

    if (key == INIT_LAYOUT) layoutKey = NODE_INIT_LAYOUT; else layoutKey = key;
    ...
    for (LWComponent c : components) {
      ...
      if (c instanceof LWLink) continue;
      if (DEBUG.LAYOUT || DEBUG.INIT) out(
        "LAYOUT NODE: in " + c.getParent() + ": " + c
      );
      try {
        c.layout(layoutKey);
      } catch (Throwable t) {
    ...
    if (key == INIT_LAYOUT) layoutKey = LINK_INIT_LAYOUT; else layoutKey = key;
    ...
    for (LWComponent c : components) {
      try {
        if (c instanceof LWGroup) ((LWGroup) c).normalize();
      } catch (Throwable t) {
  ...
  public void completeXMLRestore(Object context) {
    if (DEBUG.INIT || DEBUG.IO || DEBUG.XML) Log.debug(
      getLabel() + ": completing restore..."
    );
    ...
    final Collection<LWComponent> allRestored = getAllDescendents(
      ChildKind.ANY,
    ...
    if (mPathways == null) mPathways = new LWPathwayList(this);
    ...
    if (addedLayers) {
      ...
      for (LWComponent c : getChildren()) c.ensureID(c, false);
    }
    ...
    if (getModelVersion() < getCurrentModelVersion()) {
      ...
      if (isGroupAbsolute(getModelVersion())) upgradeAbsoluteToRelativeCoords(
        getChildren()
      );

      if (
        getCurrentModelVersion() >= 4 && getModelVersion() < 4
      ) upgradeLinksToParentRelative(allRestored);

      Log.info(
        this +
        " Updated from model version " +
        getModelVersion() +
        " to " +
    ...
    if (isArchiveMap()) {
      ...
      if (mSaveLocationURI == null) {
        Log.info(
          "unrooted map (no setFile) -- skipping search for relative resources; " +
          this
        );
      } else {
    ...
    if (DEBUG.INIT || DEBUG.IO || DEBUG.XML) Log.debug(
      "RESTORE COMPLETED; nextID=" + mNextID.get()
    );
  ...
  class ResourceFactory extends Resource.DefaultFactory {
    ...
    void loadResources(Collection<Resource> resourceBag) {
      for (Resource r : resourceBag) track(r);
    }
    ...
    private void recordInode(Resource r, Object source) {
      if (
        Util.isMacPlatform() &&
        (source instanceof java.io.File || source instanceof String)
      ) {
        String inode = Util.getSystemCommandOutput(
          new String[] { "/usr/bin/stat", "-f", "%i", "" + source },
          getSaveLocation()
        );

        if (inode != null) r.setHiddenProperty("file.MacOSX.inode", inode);
      }
    }
  }
  ...
  public Collection<Schema> getIncludedSchemas() {
    if (
      mXMLRestoreUnderway
    ) return mRestoredSchemas; else return findAllSchemas();
  }
  ...
  private void recordRelativeLocations(
    Collection<Resource> resources,
    File mapSaveDirectory
  ) {
    final URI root = Resource.toCanonicalFile(mapSaveDirectory).toURI();

    if (DEBUG.Enabled) Log.debug(
      "relativizing any resources local to: " + root
    );
    ...
    for (Resource r : resources) {
      try {
        ...
        Log.warn(
          this +
          "; recordRelativeLocations failure " +
          root +
          ": " +
          t +
          "; " +
  ...
  private void restoreRelativeLocations(
    Collection<Resource> resources,
    URI root
  ) {
    if (DEBUG.IO || DEBUG.INIT || DEBUG.RESOURCE) {
      Resource.dumpURI(
        root,
        Util.TERM_GREEN + "resolving resources to map root;"
      );
    ...
    for (Resource r : resources) {
      try {
        ...
        Log.warn(
          this +
          "; restoreRelativeLocations failure " +
          root +
          ": " +
          t +
          "; " +
  ...
  public void runResourceDeserializeInits(Collection<Resource> resources) {
    if (DEBUG.RESOURCE || DEBUG.IO) Log.debug(
      Util.TERM_CYAN +
      "initAfterDerserialize for all resources; " +
      Util.tags(resources) +
      Util.TERM_CLEAR
    );
    for (Resource r : resources) {
  ...
  LWComponent findByID(Collection<LWComponent> allRestored, String id) {
    for (LWComponent c : allRestored) if (id.equals(c.getID())) return c;
  ...
  private int findGreatestID(final Collection<LWComponent> allRestored) {
    ...
    if (mostRecent != null) return mostRecent.getNumericID(); else return -1;
  }
  ...
  public LWComponent findMostRecentlyCreated(
    ...
    for (LWComponent c : components) {
      ...
      if (typeToken != null && c.getTypeToken() != typeToken) continue;
  ...
  @Override
  public final java.util.List<LWComponent> getPickList(
    PickContext pc,
  ...
  private void drawPathways(DrawContext dc) {
    if (mPathways != null && dc.drawPathways()) {
      ...
      for (LWPathway path : mPathways) {
        ...
        } else if (path == active) active = null; // active isn't being drawn
      }
  ...
  public java.awt.image.BufferedImage createImage(
    ...
    return super.createImage(
      alpha,
      maxSize,
      fillColor == null ? getFillColor() : fillColor,
      mapZoom
    );
  }
  ...
  public LWPathway getActivePathway() {
    if (getPathwayList() != null) return getPathwayList()
      .getActivePathway(); else return null;
  }
  ...
  public void addChildren(
    ...
    if (
      children.size() == 1 && Util.getFirst(children) instanceof LWMap.Layer
    ) {
      //isLayered = true;
  ...
  protected void addChildImpl(LWComponent c, Object context) {
    if (c instanceof LWPathway) throw new IllegalArgumentException(
      "LWPathways not added as direct children of map: use addPathway " + c
    );

    if (c instanceof Layer == false && !mXMLRestoreUnderway) {
      if (mActiveLayer != null) {
        Log.warn(
          "addChildImpl: forcing to active layer: " +
          mActiveLayer +
          " in " +
          this
        );
  ...
  protected void notifyLWCListeners(LWCEvent e) {
    ...
    if (mCachedBounds != null) {
      ...
      // }
        ...
        e.component != null &&
        mCachedBounds.contains(outside = e.component.getPaintBounds())
      ) {
        ...
        // full-screen mode.
        if (outside != null) mCachedBounds.add(outside); else flushBounds();
      }
    }
   ...
   ** i digress). */
  private final Runnable EnsureBoundsCacheIsFilled = new Runnable() {
    public void run() {
      getMapBounds();
    }
  };
  ...
  private void markChange(LWCEvent e) {
    if (!javax.swing.SwingUtilities.isEventDispatchThread()) {
      ...
      if (DEBUG.WORK || DEBUG.EVENTS || DEBUG.INIT) Log.debug(
        "staying clean for non-AWT event: " + e
      );
      return;
    }
    ...
    if (DEBUG.UNDO) {
      ...
      if (true || mXMLRestoreUnderway) Log.debug(msg); else Log.debug(
        msg,
        new Throwable("HERE")
      );
    }
  ...
  public Rectangle2D.Float getMapBounds() {
    ...
    if (mCachedBounds == null) {
      ...
      if (DEBUG.CONTAINMENT && DEBUG.META) Log.debug(
        "COMPUTED BOUNDS " + Util.fmt(mCachedBounds) + "; for " + this
      );
    }
  ...
  public Rectangle2D.Float getPaintBounds() {
    if (mChildren == NO_CHILDREN) return EmptyBounds;
  ...
  public static void accruePaintBounds(
    Iterable<LWComponent> iterable,
    Rectangle2D.Float rect
  ) {
    for (LWComponent c : iterable) if (c.isDrawn()) accumulate(
      rect,
      c.getPaintBounds(),
      c,
      "paintBounds"
  ...
  public static Rectangle2D.Float getBounds(Iterator<LWComponent> i) {
    ...
    while (i.hasNext()) {
      final LWComponent c = i.next();
      if (c.isDrawn()) accumulate(rect, c.getBounds(), c, "bounds");
    }
    return rect.isEmpty() ? EmptyBounds : rect;
  }

  public static Rectangle2D.Float getBorderBounds(
    Iterable<LWComponent> iterable
  ) {
    final Rectangle2D.Float rect = new Rectangle2D.Float();
    for (LWComponent c : iterable) {
      if (c.isDrawn()) accumulate(rect, c.getBorderBounds(), c, "borderBounds");
    }
    return rect.isEmpty() ? EmptyBounds : rect;
  }

  public static Rectangle2D.Float getLocalBorderBounds(
    Iterable<LWComponent> iterable
  ) {
    final Rectangle2D.Float rect = new Rectangle2D.Float();
    for (LWComponent c : iterable) {
      if (c.isDrawn()) accumulate(
        rect,
        c.getLocalBorderBounds(),
        c,
        "localBorderBounds"
      );
    }
  ...
  public static Rectangle2D.Float getLocalBounds(
    Iterable<LWComponent> iterable
  ) {
    final Rectangle2D.Float rect = new Rectangle2D.Float();
    for (LWComponent c : iterable) {
      if (c.isDrawn()) accumulate(rect, c.getLocalBounds(), c, "localBounds");
    }
    return rect.isEmpty() ? EmptyBounds : rect;
  }

  public static Rectangle2D.Float getLayoutBounds(
    Iterable<LWComponent> iterable
  ) {
    final Rectangle2D.Float rect = new Rectangle2D.Float();
    for (LWComponent c : iterable) {
      if (c.isDrawn()) accumulate(rect, c.getLayoutBounds(), c, "layoutBounds");
    }
    return rect.isEmpty() ? EmptyBounds : rect;
  }
  ...
  public static Rectangle2D.Float getULCBounds(java.util.Iterator i) {
    ...
    if (i.hasNext()) {
      ...
      rect.x = c.getX();
      rect.y = c.getY();
      while (i.hasNext()) rect.add(((LWComponent) i.next()).getLocation());
    }
    return rect;
  }
  ...
  public static Rectangle2D.Float getLRCBounds(java.util.Iterator i) {
    ...
    if (i.hasNext()) {
      ...
      while (i.hasNext()) {
        ...
        rect.add(c.getX() + c.getWidth(), c.getY() + c.getHeight());
      }
    }
    return rect;
  }
   ...
   */
  @Override
  public final Point2D transformMapToZeroPoint(
  ...
  public String toString() {
    ...
    if (DEBUG.DATA && mFile != null) buf.append(" <" + mFile + ">");
  ...
  public Object clone() throws CloneNotSupportedException {
    try {
      ...
      File tempFile = File.createTempFile(
        prefix,
        suffix,
        VueUtil.getDefaultUserFolder()
      );
      tufts.vue.action.ActionUtil.marshallMap(tempFile, this);
      LWMap cloneMap = tufts.vue.action.OpenAction.loadMap(
  ...
  private final ColorProperty mPresentationColor = new ColorProperty(
    KEY_PresentationColor,
    new java.awt.Color(32, 32, 32)
  );
  public static final Key KEY_PresentationColor = new Key(
    "presentation.color",
    KeyType.STYLE
  ) {
    final Property getSlot(LWMap c) {
      return c.mPresentationColor;
    }
  };
   ...
   */
  @Override
  public final void addObject(Object o) {
    if (
      o instanceof org.exolab.castor.types.AnyNode &&
      ((org.exolab.castor.types.AnyNode) o).getLocalName()
        .endsWith("Boundaries")
    ) {
      ...
      if (_mergeMapBugCount++ % 100000 == 0) Log.info(
        "restoring: " + _mergeMapBugCount + " XML <*Boundaries> ignored..."
      );
    } else super.addObject(o);
  }
}
