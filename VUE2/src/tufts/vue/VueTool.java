...
public abstract class VueTool
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueTool.class);
  ...
  /** tool display order by keyname **/protected Vector mSubToolIDs =
    new Vector();
  /** the sub tool map with toolname as key **/protected Map<
    String,
    VueTool
  > mSubToolMap = new HashMap();
  /** the parent tool -- if this is a subtool **/protected VueTool mParentTool =
    null;
  /** the currently active subtool name **/protected VueTool mSelectedSubTool =
    null;

  private static final Map<Class<? extends VueTool>, VueTool> InstanceMap =
    new HashMap();

  /** for storing all the tools by name, including sub-tools */
  private static final java.util.List<VueTool> mAllTools =
  ...
  public VueTool() {
    ...
    if (DEBUG.TOOL) {
      ...
      } else System.out.println("instanced " + this);
    }
  ...
  public static VueTool getInstance(Class<? extends VueTool> clazz) {
    ...
    if (tool == null) tufts.Util.printStackTrace(
      "failed to find tool instance of type: " + clazz
    );
    return tool;
  }
  ...
  void initFromResources() {
    ...
    if (rawIcon != null) {
      ...
      if ((i = VueResources.getImageIcon(mID + ".down")) != null) setDownIcon(
        i
      );
      if (
        (i = VueResources.getImageIcon(mID + ".selected")) != null
      ) setSelectedIcon(i);
      if (
        (i = VueResources.getImageIcon(mID + ".disabled")) != null
      ) setDisabledIcon(i);
      if (
        (i = VueResources.getImageIcon(mID + ".rollover")) != null
      ) setRolloverIcon(i);
      if (
        (i = VueResources.getImageIcon(mID + ".menu")) != null
      ) setMenuItemIcon(i);
      if (
        (i = VueResources.getImageIcon(mID + ".menuselected")) != null
      ) setMenuItemSelectedIcon(i);
    }
  ...
  public String getToolTipText() {
    if (mToolTipText == null) return null; else if (
      getShortcutKey() == 0
    ) return mToolTipText; else return (
      mToolTipText + " (" + getShortcutKey() + ")"
    );
  }
  ...
  public void setCursorByID(int id) {
    if (id < 0 || id > 13) throw new IllegalArgumentException(
      "cursor id outside range 0-13: " + id
    );
  ...
  public Cursor getCursor() {
    Cursor cursor = null;
    if (hasSubTools()) cursor = getSelectedSubTool().getCursor();
  ...
  public boolean accept(PickContext pc, LWComponent c) {
    if (pc.isRegionPick()) {
      if (getSelectionType() == null) return true; else return (
        getSelectionType() == c.getTypeToken()
      );
    } else {
  ...
  public boolean supportsEditLabel() {
    return supportsEditActions();
  }
  ...
  public void handleToolSelection(boolean selected, VueTool otherTool) {
    if (DEBUG.TOOL) Log.debug(
      "handleToolSelection: " + this + ": " + selected + "; from=" + otherTool
    );
  }
  ...
  public boolean handleMouseMoved(MapMouseEvent e) {
    if (DEBUG.MOUSE && DEBUG.META) System.out.println(
  ...
  public void handleSelectionChange(LWSelection s) {
    ...
    if (
      ...
      VUE.multipleMapsVisible() &&
      VUE.isActiveViewerOnLeft() &&
      VUE.getRightTabbedPane().getSelectedViewer().getMap() ==
        VUE.getActiveMap()
    ) {
  ...
  public void setSelectedSubTool(VueTool pTool) {
    ...
    if (DEBUG.TOOL && DEBUG.META) tufts.Util.printStackTrace(
      "setSelectedSubTool: " + pTool
    );
    mSelectedSubTool = pTool;

    if (VUE.getActiveViewer() != null) VUE
      .getActiveViewer()
      .setCursor(mSelectedSubTool.getCursor());
  }
  ...
  public String toString() {
    ...
    if (getSelectedSubTool() != null) params += " sub=" + getSelectedSubTool();
    if (getSelectionType() != null) {
    ...
    if (params != null) return name + "[" + params + "]"; else return name;
  }
  ...
  public void actionPerformed(ActionEvent pEvent) {
    ...
    if (parent != null) parent.setSelectedSubTool(this);
    ...
    if (parent != null) {
      ...
      parent.handleToolSelection(
        true,
        VueToolbarController.getController().getActiveTool()
      );
    }
    if (this.hasSubTools()) VueToolbarController
      .getController()
      .handleToolSelection(this.getSelectedSubTool()); else VueToolbarController
      .getController()
      .handleToolSelection(this);
  }

  public JPanel getContextualPanel() {
    if (mToolPanel == null) mToolPanel = createToolPanel();
    return mToolPanel;
  }
