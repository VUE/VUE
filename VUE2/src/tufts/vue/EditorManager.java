...
public class EditorManager
  implements
    LWSelection.Listener, LWComponent.Listener, PropertyChangeListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(EditorManager.class);
  ...
  // now that we're tracking these, we may be able to do away with the
  // provisionals:
  ...
  private static final long PERMITTED_TEXT_BITS =
    ~(LWKey.FillColor.bit | LWKey.Shape.bit);
  ...
  private static class StyleType {
    ...
    void resolveToProvisional(long freeBits) {
      if (
        DEBUG.STYLE && (DEBUG.META/* || DEBUG.WORK */)
      ) tufts.Util.printStackTrace(this + " RESOLVING TO " + provisional);
      ...
      if (token == LWNode.TYPE_TEXT || token == LWText.TYPE_RICHTEXT) {
        ...
        if (freeBits != 0) style.copyProperties(
          provisional,
          freeBits
        ); else style.copyStyle(provisional);
      }
    }

    /**
     * @return the style ready for use to be applied to something -- does sanity
     *         checking
  ...
  private static synchronized void _install() {
    if (singleton != null) {
      ...
      preLoadStyle(
        tufts.vue.NodeTool.NodeModeTool.createDefaultNode("editor-node-style")
      );
      preLoadStyle(
        tufts.vue.NodeTool.NodeModeTool.createDefaultTextNode(
          "editor-text-style"
        )
      );
  ...
  public void selectionChanged(LWSelection s) {
    ...
    if (s.size() == 1) {
      if (singleSelection != null) singleSelection.removeLWCListener(this);
  ...
  public void activeChanged(ActiveEvent e, VueTool tool) {
    ...
    if (tool == null || VUE.getSelection().size() > 0 || tool.isTemporary()) {
      if (tool == null) CurrentToolStyle = null;
      return;
    }
    ...
    if (CurrentStyle != null && CurrentStyle != oldStyle) loadAllEditorValues(
      CurrentStyle.style
    );
  }
  ...
  public void activeChanged(ActiveEvent e, LWMap map) {
    if (map != null && map.hasChildren() && !extractedDefaultTypesFromMap) {
      ...
      if (CurrentStyle != null) CurrentStyle =
        StylesByType.get(CurrentStyle.token);
      if (CurrentToolStyle != null) CurrentToolStyle =
        StylesByType.get(CurrentToolStyle.token);
      if (CurrentStyle != null) loadAllEditorValues(CurrentStyle.style);
    }
  }

  /**
   * If the single object in the selection has a property change that was NOT due
   * to an editor,
   * (e.g., a menu) we detect this here, and re-load the editors as needed.
   */
  public void LWCChanged(LWCEvent e) {
    if (EditorLoadingUnderway || PropertySettingUnderway); // ignore
    ...
    if (!EditorLoadingUnderway && e instanceof LWPropertyChangeEvent) {
      ...
      try {
        ...
        tufts.Util.printStackTrace(
          t,
          this + ": failed to handle property change event: " + e
        );
      }
    }
  }
  ...
  private void loadAllEditors(LWSelection selection) {
    ...
    if (selection.size() == 1) editorStateSource = selection.first();
  ...
  private void loadAllEditors(
    ...
    if (DEBUG.TOOL || DEBUG.STYLE) {
      String msg =
        ...
        " " +
        selection;
      if (DEBUG.META/* ||DEBUG.WORK */) tufts.Util.printStackTrace(
        msg
      ); else out(msg);
    }
    ...
    try {
      for (LWEditor editor : mEditors) {
        try {
          ...
          tufts.Util.printStackTrace(
            t,
            this + ": general failure processing LWEditor: " + editor
          );
        }
      }
    } finally {
  ...
  private void setEditorState(
    ...
    if (DEBUG.TOOL) out(
      "SET-ENABLED " + (supported ? "YES" : " NO") + ": " + editor
    );
    ...
    if (setEnabledState) {
      try {
        ...
        tufts.Util.printStackTrace(
          t,
          this + ": LWEditor.setEnabled failed on: " + editor
        );
      }

      if (mLabels.containsKey(editor)) mLabels
        .get(editor)
    ...
    // if (DEBUG.TOOL&&DEBUG.META) out("loadEditor: " + editor + " loading " +
    // editor.getPropertyKey() + " from " + source);

    // if (supported && propertySource != null &&
    // propertySource.supportsProperty(editor.getPropertyKey()))
    if (supported && propertySource != null) loadEditorWithValue(
      editor,
      propertySource.getPropertyValue(key)
    );

    // TODO: a bit overkill to do this no matter what -- do we need to if no
    // property source?
    if (propertySource != null) recordPropertyChangeInStyles(
      "load",
      editor.getPropertyKey(),
      editor.produceValue(),
      selection.isEmpty()
    );
    // if (editor instanceof Component) ((Component)editor).repaint(); // not
    // helping ShapeIcon's repaint when disabled...
  }

  private void loadEditorWithValue(LWEditor editor, Object value) {
    if (DEBUG.TOOL) out(
      "     loadEditor: " + editor + " <- value[" + value + "]"
    );
    try {
      ...
      tufts.Util.printStackTrace(
        t,
        this +
        ": LWEditor.displayValue failed on: " +
        editor +
        " with value [" +
        value +
        "]"
    ...
    // } else if (DEBUG.TOOL) out("\tloadEditor: " + source + " -> " + editor + "
    // skipped; null value for " + key);
  }
  ...
  public static void firePropertyChange(LWEditor editor, Object source) {
    try {
      ...
      Log.error(
        "failed to fire property for LWEditor " +
        editor +
        " for source " +
        source,
        t
  ...
  /**
   * Will either modifiy the active selection, or if it's empty, modify the
   * default state (creation state) for this tool panel
   */
  private static void applySinglePropertyChange(
    final Object key,
    ...
    if (EditorLoadingUnderway) {
      if (DEBUG.TOOL) out(
        "applySinglePropertyChange: " + key + " " + newValue + " (skipping)"
      );
      return;
    }

    if (DEBUG.TOOL || DEBUG.STYLE) out(
      "applySinglePropertyChange: " + key + " " + newValue
    );

    if (!components.isEmpty()) {
      consumeFreeProperties();
      // As setting these properties in the model will trigger notify events from the
      // selected objects
      // back up to the tools, we want to ignore those events while this is underway
      // -- the tools
      // already have their state set to this.
      PropertySettingUnderway = true;
      try {
        for (tufts.vue.LWComponent c : components) applyPropertyValue(
          source,
          key,
          newValue,
          c
      ...
      if (selection.getStyleRecord() != null) applyPropertyValue(
        source,
        key,
        newValue,
        selection.getStyleRecord()
      );

      if (VUE.getUndoManager() != null) VUE
        .getUndoManager()
        .markChangesAsUndo(key.toString());
  ...
  private static void declareFreeProperty(Object k) {
    if (k instanceof Key) {
      ...
      // Note that if any LWEditors are created that are handling non KeyType.STYLE
      // properties,
      // we could get some odd effects (e.g., we wouldn't want label or notes being
      // tagged as having been free property bit, then applying it to newly created
      // objects!)

      if (key.type != KeyType.STYLE && key.type != KeyType.SUB_STYLE) {
        if (DEBUG.Enabled) Log.debug(
          "note: free property of non-style type being ignored: " +
          key +
          "; type=" +
          key.type
        );
        // we can safely ignore this, but debug for now so we know if we get into this
        // situation
        return;
      }
      ...
      if (DEBUG.STYLE) out(
        "declaring free (unused) property change: " + key + "; type=" + key.type
      );
  ...
  private static void consumeFreeProperties() {
    if (DEBUG.STYLE) out(
      "consuming free property bits: " + Long.bitCount(FreePropertyBits)
    );
    FreePropertyBits = 0;
  }
  ...
  private static void recordPropertyChangeInStyles(
    ...
    // provisionals should be true when there is no selection, and the tools are all
    // enabled in their "free" state
    if (provisionals) {
      ...
      for (StyleType styleType : StylesByType.values()) {
        applyPropertyValue(
          ...
          key,
          newValue,
          styleType.provisional
        );
        if (CurrentToolStyle == styleType) applyPropertyValue(
          "<" + debugSrc + ":provSync>",
          key,
          newValue,
          styleType.style
      ...
      if (DEBUG.STYLE) out(
        "NO CURRENT STYLE FOR " + debugSrc + " " + key + " " + newValue
      );
    } else {
  ...
  /**
   * Apply the current appropriate properties to the given newly created object
   * based on it's type.
   */
  ...
  private static void applyCurrentProperties(LWComponent c, long freeBits) {
    ...
    try {
      ...
      if (styleType != null) {
        if (freeBits != 0) c.copyProperties(
          styleType.provisional,
          freeBits
        ); else c.copyStyle(styleType.provisional);
      }
    } catch (Throwable t) {
      tufts.Util.printStackTrace(
        t,
        "failed to apply current properties to: " + c
      );
    }
  }
   ...
   * Apply the current appropriate properties to the given newly created object
   * based
   * on it's type. If nothing was selected, and the the editors were all enabled
   * in
   * their "free" state (not tied to the property of something selected) resolve
   * that
  ...
  public static void targetAndApplyCurrentProperties(LWComponent c) {
    try {
      ...
      consumeFreeProperties();
      if (styleForType != null) c.copyStyle(styleForType);
    } catch (Throwable t) {
      tufts.Util.printStackTrace(
        t,
        "failed to target and apply current properties to: " + c
      );
    }
  }
  ...
  private static void applyPropertyValue(
    ...
    // if (DEBUG.STYLE) System.out.println("APPLY " + debugSrc + " " + key + "[" +
    // newValue + "] -> " + target);
  ...
  private synchronized StyleType getStyleForType(LWComponent c) {
    ...
    if (DEBUG.STYLE) out(
      "got styleHolder for type token (" + token + "): " + Util.tags(styleType)
    );
    return styleType;
  }

  private static StyleType putStyle(Object token, LWComponent style) {
    StyleType newType = new StyleType(
      token,
      style, // style
      createStyle(style.getClass(), style, token, "provi")
    ); // provisional
  ...
  // }
  // private static void preLoadStyle(Class<? extends LWComponent> clazz, Object
  // typeToken) {
  // final LWComponent preLoadStyle = createStyle(clazz, null, typeToken,
  // "INIT-");
  // putStyle(typeToken, preLoadStyle);
  // }

  private static final LWComponent.CopyContext DUPE_WITHOUT_CHILDREN =
    new LWComponent.CopyContext(false);

  private static LWComponent createStyle(
    LWComponent styleSource,
    Object typeToken
  ) {
  ...
  // private static LWComponent createStyle(LWComponent styleSource, Object
  // typeToken, String version)
  private static LWComponent createStyle(
    ...
    // if (DEBUG.STYLE || DEBUG.WORK) out("creating style holder based on " +
    // styleSource + " for type (" + typeToken + ")");
    ...
    if (styleSource != null) {
      if (styleSource.getClass() != clazz) Log.warn(
        "class mis-match: " + clazz + " != " + Util.tags(styleSource),
        new Throwable("HERE")
      );
    ...
    // out("created new styleHolder for type token [" + token + "]: " +
    // styleHolder);
  ...
  /**
   * @return the current style for type type of the given component only if we
   *         already have one
   *         -- do not auto-create a new style for the type if we don't already
   *         have one
   */
  private static LWComponent resolveToProvisionalStyle(
    Object typeToken,
    long freeBits
  ) {
    ...
    if (resolver != null) {
      ...
      if (DEBUG.STYLE || DEBUG.WORK) out(
        "Resolved provisional type to final applied type: " + resolver
      );
      consumeFreeProperties();
    }
    ...
    for (StyleType styleType : StylesByType.values()) if (
      styleType != resolver
  ...
  /*
   * Apply the current value of all selected tools that are applicable to the
   * given component.
   * E.g., used for setting the properties of newly created objects.
   *
   * @param keyBits -- only apply keys whose bit is represented in keyBits (@see
   * LWComonent.Key.bit)
   *
   ...
   * else {
   * key = null;
   * out("ApplyProperties: skipping non proper key: " + k.getClass().getName() +
   * "[" + k + "]");
   * continue;
   * }
   *
  ...
  private void extractMostRecentlyUsedStyles(LWMap map) {
    ...
    for (Map.Entry<Object, LWComponent> e : foundStyles.entrySet()) {
      ...
      if (DEBUG.STYLE || DEBUG.INIT) out(
        "extracted style for " + token + " from " + lastCreated
      );
  ...
  private static Object typeToken(LWComponent c) {
    ...
    // This won't work for keeping separate slide styles until new objects created
    // on the slide get
    // their SLIDE_STYLE bit immediately set before they're auto-styled: will
    // probably need a
    // NewItem factory (may replace code that NodeTool currently has) fetchable from
    // any LWContainer.
    // Also, anywhere we use token == value would need to be updated, at least using
    // this method
    // of token differentiation, as it creates a new string object each time a
    // SLIDE_STYLE token is
  ...
  static void unregisterEditor(LWEditor editor) {
    if (editor.getPropertyKey() == null) {
      // if (DEBUG.Enabled) System.out.println("EditorManager: registration ignoring
      // editor w/null key: " + dumpEditor(editor));
      Log.debug(
        "unregistration ignoring editor w/null key: " + dumpEditor(editor)
      );
      return;
    }

    if (mEditors.remove(editor)) {
      if (DEBUG.TOOL || DEBUG.INIT || !VUE.isStartupUnderway()) out(
        "UNREGISTERED EDITOR: " + editor
      );
      if (
        editor instanceof java.awt.Component
  ...
  static boolean isRegistered(LWEditor editor) {
    ...
    if (mEditors.contains(editor)) return true; else return false;
  }
  ...
  static void registerEditor(LWEditor editor) {
    if (editor.getPropertyKey() == null) {
      ...
      // editor w/null key: " + dumpEditor(editor));
      Log.debug(
        "registration ignoring editor w/null key: " + dumpEditor(editor)
      );
      return;
    }

    if (mEditors.add(editor)) {
      // apparently things have changed such that starutp underway is done before most
      // all
      // the editors are registered -- don't think this should be a problem, but
      // marking with DEBUG.WORK just in case
      if (
        DEBUG.TOOL || DEBUG.INIT || (DEBUG.WORK && !VUE.isStartupUnderway())
      ) out("REGISTERED EDITOR: " + editor);
      ...
      try {
        ...
        Log.warn(
          "editor not ready to produce value: " + Util.tags(editor) + "; " + t
        );
      }
      if (curVal != null) {
        // if (curVal != null && VUE.isStartupUnderway()) {
        // doing this at runtime this can break us badly (LWText 0 font size props
        // leaking to LWNodes!)
        // Mike appears to have fixed the above problem, so we can re-enable this fix
        // for VUE-1529 (aka VUE-408)
        recordPropertyChangeInStyles(
          "register",
          editor.getPropertyKey(),
          curVal,
          true
    ...
    } else System.out.println(" REGISTERED AGAIN: " + editor);
  }
  ...
  private void findEditors() {
    ...
    new EventRaiser<JLabel>(this, JLabel.class) {
      public void dispatch(JLabel label) {
        ...
        if (gui != null && gui instanceof LWEditor) mLabels.put(
          (LWEditor) gui,
          label
        );
      }
  ...
  static void out(Object o) {
    // Log.forcedLog("FOO", org.apache.log4j.Level.ALL, "EditorManager: " + o,
    // null);
    Log.debug(o);
  }
 ...
 *
 * Holy crap -- I think there's a way to really "do what I mean" with this, but
 * it's
 * freakin hairy. So every type has it's own style stored which can be used to
 * style newly created objects of that type, as well as become the default
 * editor
 * states if a VueTool that is tied to that object type is enabled (and
 * something
 * isn't selected -- that's an interesting case -- lets say a link is selected
 * and
 * you load the node tool -- we probably want the selection to have priority in
 * the
 * editor states, and just ignore toe VueTool in this case -- tho actually,
 * hell, it
 * would be MUCH cleaner to just de-select what's selected when you load a
 * VueTool
 ...
 * use the current VueTool type if there is one, or more accurately, leave it
 * with
 * the last type that was selected. Tho, say, if the node tool was selected, and
 * we
 * leave that in place, and they change the stroke color, then they create a
 * link,
 * the link will have the properties of the last time we had the link style
 * state,
 * not the current state. So, to deal with this, if NOTHING is selected (and all
 * editors get enabled), we could apply any editor state changes to ALL the
 * current
 * typed styles, which is the only way we could capture all the changes anyway,
 * as
 ...
 * when nothing is selected, update every one of these second "provisional"
 * styles
 * for each type, and then the next object that's created, we know that's what
 * they
 * wanted, and then the provisional style for that type is copied over the
 * in-use
 * style for that type, and the provisional styles for all other types are
 * thrown
 * out (well, actually, just ignored -- they'll always be updating when nothing
 * is
 ...
 *
 * So now lets say we select the node tool: currently, the tools still allow you
 * to
 * single select objects of any type (the only limit the drag selection) --
 * would
 * could change that for simplicity. So lets say node tool is selected -- we
 * want
 * to load the node style... crap -- whan happens if they load the node style,
 * but
 * then forget the node tool is active, change the stroke style, and create a
 * new
 ...
 * the editors.
 *
 * (2) When nothing is selected, we're ALWAYS working with the provisional
 * style,
 * even if a tool with a type is selected -- so changing the active tool is
 * going to
 * have NO EFFECT on the editor states, because even if the node tool is active,
 * they could still create a link, and if they change the provisional style, we
 * want
 ...
 * create a node: the selection is cleared, the provisional style is returned to
 * the
 ...
 *
 * More complexity: if we want to load the typed style with the tool, which
 * nicely
 * communicates the avail properties for that type, yet it has properties that
 * overlap with other tools, the typed style for the current tool could itself
 * serve
 ...
 *
 * So all of this also means that all new on-map user object creation will have
 * to
 * go through the editor manager, tho actually the generic
 * applyCurrentProperties is
 * what does this, tho it would benefit from a more powerful name to suggest
 * what
 ...
 * provisional styles, which will cause mass confusion if we ever create an
 * object
 * without telling the editor manager that this is where the provisional style
 * went.
 *
 * WHEN THE ACTIVE TOOL CHANGES, AND NOTHING IS SELECTED: We still leave
 * everything
 * enabled, as they might still want to style and create anything, however, we
 * CAN
