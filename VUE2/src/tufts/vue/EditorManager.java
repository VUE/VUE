...
public class EditorManager
    ...
    private static class StyleType {
        ...
        void resolveToProvisional(long freeBits) {
            if (DEBUG.STYLE && (DEBUG.META /* || DEBUG.WORK */))
    ...
    private void setEditorState(LWEditor editor, LWSelection selection, LWComponent propertySource,
        ...
        if (DEBUG.TOOL)
            out("SET-ENABLED " + (supported ? "YES" : " NO") + ": " + editor);
    ...
    private void loadEditorWithValue(LWEditor editor, Object value) {
        if (DEBUG.TOOL)
            out("     loadEditor: " + editor + " <- value[" + value + "]");
        try {
            editor.displayValue(value);
        } catch (Throwable t) {
            tufts.Util.printStackTrace(t,
                    this + ": LWEditor.displayValue failed on: " + editor + " with value [" + value + "]");
        }
    ...
    private static void applySinglePropertyChange(final Object key,
        ...
        if (EditorLoadingUnderway) {
            if (DEBUG.TOOL)
                out("applySinglePropertyChange: " + key + " " + newValue + " (skipping)");
            return;
        }

        if (DEBUG.TOOL || DEBUG.STYLE)
            out("applySinglePropertyChange: " + key + " " + newValue);
    ...
    private static void declareFreeProperty(Object k) {
        if (k instanceof Key) {
            ...
            if (key.type != KeyType.STYLE && key.type != KeyType.SUB_STYLE) {
                if (DEBUG.Enabled)
                    Log.debug("note: free property of non-style type being ignored: " + key + "; type=" + key.type);
            ...
            if (DEBUG.STYLE)
                out("declaring free (unused) property change: " + key + "; type=" + key.type);
    ...
    private static void consumeFreeProperties() {
        if (DEBUG.STYLE)
            out("consuming free property bits: " + Long.bitCount(FreePropertyBits));
        FreePropertyBits = 0;
    }

    private static void recordPropertyChangeInStyles(String debugSrc,
            Object key,
            Object newValue,
        ...
        if (provisionals) {
            ...
            if (DEBUG.STYLE)
                out("NO CURRENT STYLE FOR " + debugSrc + " " + key + " " + newValue);
        } else {
    ...
    private static void applyPropertyValue(Object debugSrc, Object key, Object newValue, LWComponent target) {
        ...
        if (target.supportsProperty(key)) {
            if (DEBUG.STYLE)
                out(String.format("APPLY %s %-15s %-40s -> %s",
                        debugSrc,
                        key,
                        "(" + newValue + ")",
                        target));
            ...
            try {
                ...
                tufts.Util.printStackTrace(t,
                        debugSrc + " failed to set property " + key + "; value=" + newValue + " on " + target);
            }
        }
    }
    ...
    private synchronized StyleType getStyleForType(LWComponent c) {
        ...
        if (DEBUG.STYLE)
            out("got styleHolder for type token (" + token + "): " + Util.tags(styleType));
        return styleType;
    }
    ...
    private static LWComponent createStyle(Class<? extends LWComponent> clazz,
        ...
        if (DEBUG.STYLE)
            out("CREATED STYLE: type=" + typeToken + "; store=" + style + " based on " + styleSource);
        return style;
    }
    ...
    private static LWComponent resolveToProvisionalStyle(Object typeToken, long freeBits) {
        if (DEBUG.STYLE)
            out("resolveToProvisionalStyle: " + typeToken);
        ...
        if (resolver != null) {
            ...
            if (DEBUG.STYLE || DEBUG.WORK)
                out("Resolved provisional type to final applied type: " + resolver);
            consumeFreeProperties();
    ...
    private void extractMostRecentlyUsedStyles(LWMap map) {
        ...
        for (Map.Entry<Object, LWComponent> e : foundStyles.entrySet()) {
            ...
            if (DEBUG.STYLE || DEBUG.INIT)
                out("extracted style for " + token + " from " + lastCreated);
    ...
    static void unregisterEditor(LWEditor editor) {
        ...
        if (mEditors.remove(editor)) {
            if (DEBUG.TOOL || DEBUG.INIT || !VUE.isStartupUnderway())
                out("UNREGISTERED EDITOR: " + editor);
            if (editor instanceof java.awt.Component)
    ...
    static void registerEditor(LWEditor editor) {
        ...
        if (mEditors.add(editor)) {
            ...
            if (DEBUG.TOOL || DEBUG.INIT || (DEBUG.WORK && !VUE.isStartupUnderway()))
