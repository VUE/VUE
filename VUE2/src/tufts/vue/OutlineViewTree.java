...
public class OutlineViewTree
      ...
      new TreeSelectionListener() {
        public void valueChanged(TreeSelectionEvent e) {
          ...
          for (int i = 0; i < paths.length; i++) {
            ...
            tufts.oki.hierarchy.HierarchyNode hierarchyNode =
              (tufts.oki.hierarchy.HierarchyNode) o;
  ...
  public boolean isInitialized() {
    if (hierarchyModel != null) return true; else return false;
  }

  /**A method for handling a LWC event*/
  public void LWCChanged(LWCEvent e) {
    if (e.getComponent() instanceof LWPathway) return;
    ...
    if (!labelChangeState) {
      ...
      hierarchyModel.updateHierarchyNodeLabel(
  ...
  public void selectionChanged(LWSelection selection) {
    if (
      !isShowing()
    ) return; // TODO: now we should do an auto-update when made visible

    if (!valueChangedState) {
      selectionFromVUE = true;

      if (!selection.isEmpty()) setSelectionPaths(selection);
      //else deselect
      else super.setSelectionPath(null);
  ...
  private class OutlineViewTreeRenderer
    extends OutlineViewRenderElement
    ...
    public Component getTreeCellRendererComponent(
      ...
      if (
        ((DefaultMutableTreeNode) value).getUserObject() instanceof
        tufts.oki.hierarchy.HierarchyNode
      ) {
        tufts.oki.hierarchy.HierarchyNode hierarchyNode =
          (tufts.oki.hierarchy.HierarchyNode) (((DefaultMutableTreeNode) value).getUserObject());
        LWComponent component = hierarchyNode.getLWComponent();

        if (component instanceof LWMap) setIcon(mapIcon); else if (
          component instanceof LWNode
        ) setIcon(nodeIcon); else if (component instanceof LWText) setIcon(
          textIcon
        ); else if (component instanceof LWLink) setIcon(linkIcon);
      ...
      if (sel) setIsSelected(true); else setIsSelected(false);

      if (hasFocus) setIsFocused(true); else setIsFocused(false);
  ...
  private class OutlineViewTreeEditor
    ...
    public Component getTreeCellEditorComponent(
      ...
      editorElement.setBorder(
        UIManager.getBorder("Table.focusCellHighlightBorder")
      );

      // Configure the component with the specified value
      String label = tree.convertValueToText(
        value,
        isSelected,
        expanded,
        leaf,
      ...
      if (selectedLWComponent instanceof LWText) editorElement.setIcon(
        nodeIcon
      ); else if (selectedLWComponent instanceof LWText) editorElement.setIcon(
        textIcon
      ); else if (selectedLWComponent instanceof LWLink) editorElement.setIcon(
        linkIcon
      ); else editorElement.setIcon(mapIcon);
    ...
    public Object getCellEditorValue() {
      try {
        ...
        if (DEBUG.FOCUS) System.out.println(
          this + " getCellEditorValue returns [" + text + "]"
        );
    ...
    public void focusGained(FocusEvent e) {
      if (DEBUG.FOCUS) System.out.println(
        this + " focusGained from " + e.getOppositeComponent()
      );
    }
  ...
  private static class OutlineViewRenderElement extends JPanel {
    ...
    private static final Color selectedColor = VueResources.getColor(
      "outlineTreeSelectionColor"
    );
    ...
    public void setIsSelected(boolean value) {
      if (value) label.setBackground(selectedColor); else label.setBackground(
        nonSelectedColor
      );
    }
