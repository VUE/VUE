...
public class EventHandler<E> {
            ...
            synchronized (AllEventHandlers) {
                if (AllEventHandlers.containsKey(_itemType)) {
                    ...
                    tufts.Util.printStackTrace(
                            "ignoring prior active change handler for " + getClass() + " and taking over listeners");
    ...
    public synchronized void raise(final Object source, final E newEvent) {
        if (DEBUG.EVENTS) {
            ...
            final String debug = _itemTypeName
    ...
    public void addListener(Listener listener) {
        if (mListeners.addIfAbsent(listener)) {
            if (DEBUG.EVENTS)
                Log.debug(String.format(TERM_GREEN + "%-50s added listener %s" + TERM_CLEAR, _itemTypeName, listener));
        } else {
    ...
    protected static void lock(Object o, String msg) {
        if (DEBUG.THREAD)
            System.err.println((o == null ? "EventHandler" : o) + " " + msg + " LOCK");
    }

    protected static void unlock(Object o, String msg) {
        if (DEBUG.THREAD)
            System.err.println((o == null ? "EventHandler" : o) + " " + msg + " UNLOCK");
    }

    protected static void outf(String fmt, Object... args) {
        for (int x = 0; x < ActiveInstance.depth; x++)
            System.out.print("    ");
