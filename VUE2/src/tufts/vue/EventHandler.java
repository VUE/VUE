 ...
 * <p>
 * E.g.:
 * <code>interface <i>MyListener</i> extends EventHandler.Listener&lt;<i>MyEvent</i>&gt;{}</code>
 *
 * <p>
 * Or when the listener and event are static inner classes to an implementation
 * class defining
 ...
 * </code></blockquote>
 *
 * Using inner classes, this standard pattern defines <i>MyClass</i>.Listener,
 * and
 * all that remains is to define <i>MyClass</i>.Event.
 *
 * <p>
 * Note that bothering to define the Listener class is uneeded in some cases:
 * implementing classes can implement EventHandler.Listener&lt;<i>type</i>&gt;,
 * although if done
 * this way, due to type-erasure, a single class may only declare itself as
 * implementing
 * a single EventHandler.Listener of any kind. If a class is declared as
 * implementing
 * multiple EventHandler.Listener's, even with multiple <i>types</i>, once the
 * type information is
 * thrown away, it will look like a class being declared as implementing the
 * same
 * interface multiple times.
 *
 * <p>
 * Instances of handlers for any new event type can be obtained or created on
 * the fly
 * by calling the static method getHandler for a given class/type, which will
 * automatically create a new handler for the given event type if one doesn't
 * exist.
 *
...
public class EventHandler<E> {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(EventHandler.class);

  public interface Listener<E> {
    public void eventRaised(E event);
  }

  private static final Map<Class, EventHandler> AllEventHandlers =
    new HashMap();

  // private static final List<Listener> ListenersForAllEvents = new
  ...
  public static EventHandler getHandler(Class type) {
    ...
    synchronized (AllEventHandlers) {
      ...
      if (handler == null) handler = new EventHandler(type, true);
    }
  ...
  protected final CopyOnWriteArrayList<Listener> mListeners =
      ...
      synchronized (AllEventHandlers) {
        if (AllEventHandlers.containsKey(_itemType)) {
          ...
          tufts.Util.printStackTrace(
            "ignoring prior active change handler for " +
            getClass() +
            " and taking over listeners"
          );
  ...
  protected void notifyListeners(final Object source, final E event) {
    if (_inNotify) {
      tufts.Util.printStackTrace(
        this + " event loop! aborting delivery of: " + event
      );
      return;
    }
    ...
    try {
      ...
      if (mListeners.size() > 0) notifyListenerList(source, event, mListeners);
  ...
  protected void notifyListenerList(
    final Object source,
    final E event,
    ...
    for (Listener target : listenerList) {
      ...
      try {
        ...
        Util.printStackTrace(
          t,
          this + " exception notifying " + target + " with " + event
        );
      }
    }
  }
  ...
  public void addListener(Listener listener) {
    if (mListeners.addIfAbsent(listener)) {
      if (DEBUG.EVENTS) Log.debug(
        String.format(
          TERM_GREEN + "%-50s added listener %s" + TERM_CLEAR,
          _itemTypeName,
          listener
        )
      );
      ...
      if (DEBUG.EVENTS) Util.printStackTrace(
        this + "; FYI: already listening: " + listener
      );
    }
  }
  ...
  public void removeListener(Listener listener) {
    if (mListeners.remove(listener)) {
      if (DEBUG.Enabled) outf(
        TERM_GREEN + "%-50s removed listener %s\n" + TERM_CLEAR,
        this,
        listener
      );
  ...
  protected static String sourceName(Object s) {
    if (s == null) return "null"; else if (
      s instanceof ActiveEvent || s instanceof EventHandler
    ) return s.toString(); else return Util.tags(s);
    // return s.getClass().getName() + ":" + s;
  }

  protected static void lock(Object o, String msg) {
    if (DEBUG.THREAD) System.err.println(
      (o == null ? "EventHandler" : o) + " " + msg + " LOCK"
    );
  }

  protected static void unlock(Object o, String msg) {
    if (DEBUG.THREAD) System.err.println(
      (o == null ? "EventHandler" : o) + " " + msg + " UNLOCK"
    );
  }
