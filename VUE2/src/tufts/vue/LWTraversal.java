...
public class LWTraversal {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWTraversal.class);
  ...
  LWTraversal(boolean preOrder, PickContext pc) {
    ...
    this.pc = pc;
    if (DEBUG.PICK && DEBUG.WORK) Log.debug(
      "pick cache: " + Util.tags(pickCache)
    );
  }
    ...
    if (!acceptTraversal(c)) {
      ...
      if (acceptChildren(c)) {
        ...
        if (iteratingPickCache) {
          ...
          if (DEBUG.PICK && DEBUG.WORK) Log.debug(
            "allocating tmp pick cache for " + c
          );
        ...
        try {
          ...
          if (pc != null) pickables =
            c.getPickList(pc, curCache); else pickables = c.getChildList();
        } catch (Throwable t) {
          Util.printStackTrace(
            t,
            "pickList fetch failed for " + c + "; pickables=" + pickables
          );
        }

        if (depth > 15) {
          Util.printStackTrace(
            "aborting pick at depth " +
            depth +
            " in case of loop; pickables: " +
            pickables
          );
          done = true;
          return;
        ...
        if (pickables == pickCache) iteratingPickCache = true;
        ...
        try {
          ...
          Util.printStackTrace(
            t,
            "traversal failure on " + c + "; pickables=" + pickables
          );
        } finally {
          depth--;
          if (painted) depthVisible--;
          if (pickables == pickCache) iteratingPickCache = false;
        }
  ...
  public void traversePicks(
    LWComponent curTop,
    java.util.List<LWComponent> children
  ) {
    ...
    for (
      ListIterator<LWComponent> i = children.listIterator(children.size());
      i.hasPrevious();
    ) {
      final LWComponent c = i.previous();
      if (c == curTop) {
        Util.printStackTrace(
          "found local root in pick list (loop!), skipping: " + c
        );
        continue;
      }
    ...
    if (zoomedFocus != null) traverse(zoomedFocus);
  }
   ...
   */
  public abstract static class Picker extends LWTraversal {
    ...
    Picker(PickContext pc) {
      ...
      if (DEBUG.PICK) System.out.println(
        "Picker created: " + getClass().getName() + "; " + pc
      );
    }
    ...
    public boolean acceptChildren(LWComponent c) {
      ...
      if (
        c.isPathwayOwned() && c instanceof LWSlide && pc.root != c
      ) return false; else return c.hasPicks();
    }
    ...
    @Override
    public boolean accept(LWComponent c) {
      if (c == pc.excluded) return false; else if (
        c.isFiltered()
      ) return false; else if ( // note: children may NOT be filtered out, but default acceptTraversal will get us there
        pc.ignoreSelected && c.isSelected()
      ) return false;
      //else if (pc.pickType != null && !pc.pickType.isInstance(c))
      else if (
        pc.acceptor != null && !pc.acceptor.accept(pc, c)
  ...
  public static class PointPick extends LWTraversal.Picker {
    ...
    public void visit(LWComponent c) {
      ...
      final float hitResult = c.pickDistance(zeroPoint.x, zeroPoint.y, pc);

      //if (DEBUG.PICK && DEBUG.META) {
      if (DEBUG.PICK) {
        if (hitResult > 0) System.out.format(
          "; distance=%.2f\n",
          Math.sqrt(hitResult)
    ...
    public LWComponent getPicked() {
      if (DEBUG.PICK) {
        if (pc.dropping != null) eoutln(
          "PointPick:   DROPPING: " + Util.tags(pc.dropping)
        );
        eoutln("PointPick: DIRECT-HIT: " + hit);
        eout(
          String.format(
            "PointPick:  CLOSE-HIT: %s; distance=%.2f;",
            closeHit,
      ...
      if (hit == null || (closeHit != null && closeHit.hasAncestor(hit))) {
        ...
        } else closeEnoughSq = 8 * 8;
        if (DEBUG.PICK) System.out.format(
          " closeEnough=%.2f;",
      ...
      if (hit != null && hit.isPathwayOwned() && hit instanceof LWSlide) {
        ...
        if (parent != null) {
          ...
          final LWGroup topGroupAncestor =
            (LWGroup) parent.getTopMostAncestorOfType(LWGroup.class, pc.root);
          ...
          if (topGroupAncestor != null) {
            if (pc.pickDepth > 0) {
              ...
              final LWGroup groupAncestor = (LWGroup) parent.getAncestorOfType(
                LWGroup.class,
                pc.root
              );
              if (groupAncestor != topGroupAncestor) picked = groupAncestor;
            } else {
              // SHALLOW PICK:
        ...
        if (picked == hit) {
          ...
          if (picked != null) picked = picked.defaultPick(pc);
        }

        if (picked != null && pc.dropping != null) picked =
          picked.defaultDropTarget(pc);
      }
      ...
      if (picked != null && picked != pc.root && !picked.hasAncestor(pc.root)) {
        ...
        if (DEBUG.Enabled) Log.warn(
          "PointPick: DENIED: " + picked + "; is above pick root: " + pc.root
        );
        picked = null;
      }
      ...
      if (DEBUG.PICK) {
        eoutln(
          Util.TERM_GREEN + "PointPick:     PICKED: " + picked + Util.TERM_CLEAR
        );
        if (DEBUG.PICK) System.out.println("");
      }
  ...
  public static class RegionPick extends LWTraversal.Picker {
    ...
    public java.util.List<LWComponent> traverseAndPick(LWComponent root) {
      if (DEBUG.PERF) {
        ...
        final long delta = System.nanoTime() - start;
        Log.debug(
          String.format("RegionPick elapsed: %.1fms", delta / 100000.0)
        );
        return hits;
      } else {
        traverse(root);
