...
public class DEBUG {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DEBUG.class);
  ...
  public static void setAllEnabled(boolean enabled) {
    for (Field f : Fields) setFlag(f, enabled);

    // only turn META & WORK off, not on
    if (enabled == false) META = WORK = false;
  }

  public static void parseArg(String arg) {
    if (arg == null || !arg.toLowerCase().startsWith("-debug")) return;
    ...
    if (arg.charAt(0) == ':') args = arg.substring(1).split(","); else if (
      arg.charAt(0) == '_'
    ) args = new String[] { arg.substring(1) }; else return;
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (symbol.charAt(0) == '+') {
        ...
        try {
          ...
          try {
            ...
            if (setFlag(debugField, true)) handled = true;
          } catch (Exception e) {
            Log.info(e);
          }
          if (false) {
            ...
            ((org.apache.log4j.Logger) logField.get(null)).setLevel(
        ...
        for (Field f : Fields) {
          if (
            f.getName().toUpperCase().startsWith(symbol) && setFlag(f, true)
          ) {
            handled = true;
            break;
          }
      ...
      if (!handled) Log.info(
        String.format("couldn't handle debug flag \"%s\"", args[i])
      );
    }
  }
  ...
  private static boolean setFlag(Field f, boolean enabled) {
    ...
    try {
      ...
      success = true;
      Log.info(
        f.getDeclaringClass().getName() + "/" + f.getName() + " = " + enabled
      );
