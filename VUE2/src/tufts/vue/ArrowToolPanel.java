...
public class ArrowToolPanel extends ToolPanel {
  ...
  private static class ArrowStyleButton extends JComboBox {
    ...
    public ArrowStyleButton(Object[] a, boolean isHead) {
      ...
      // setButtonIcon(new LineIcon(ButtonWidth-18, 3)); // height really only needs
      // to be 1 pixel
      ComboBoxRenderer renderer = new ComboBoxRenderer();
      setRenderer(renderer);

      imageIcons = new Icon[1];
      if (isHead) imageIcons[0] =
        new GUI.ProxyEnabledIcon(
          (Icon) VueResources.getIcon("leftarrow.raw"),
          this
        ); else imageIcons[0] =
        new GUI.ProxyEnabledIcon(
          (Icon) VueResources.getIcon("rightarrow.raw"),
          this
        );
  ...
  protected void buildBox() {
    // final AbstractButton mArrowStartButton = new
    // VueButton.Toggle("link.button.arrow.start");
    // final AbstractButton mArrowEndButton = new
    // VueButton.Toggle("link.button.arrow.end");
    ...
    // AbstractButton linkTypeMenu = new VuePopupMenu(LWKey.LinkCurves,
    // LinkTypeActions);
    ...
    final JLabel headLabel = new JLabel(
      VueResources.getString("arrowtoolpanel.start")
    );
    ...
    gbc.fill = GridBagConstraints.VERTICAL; // the label never grows
    gbc.anchor = GridBagConstraints.EAST;
    final JLabel tailLabel = new JLabel(
      VueResources.getString("arrowtoolpanel.end")
    );
    ...
    final LWPropertyHandler arrowPropertyHandler = new LWPropertyHandler<
      Integer
    >(LWKey.LinkArrows, arrowHeadCombo, arrowTailCombo) {
      public Integer produceValue() {
        int arrowState = 0;
        if (arrowHeadCombo.getSelectedIndex() > 0) arrowState |=
        LWLink.ARROW_HEAD;
        if (arrowTailCombo.getSelectedIndex() > 0) arrowState |=
        LWLink.ARROW_TAIL;
        return arrowState;
      }
      ...
      public void displayValue(Integer arrowState) {
        if (
          (arrowState & LWLink.ARROW_HEAD) == 0
        ) arrowHeadCombo.setSelectedIndex(
          0
        ); else arrowHeadCombo.setSelectedIndex(1);
        if (
          (arrowState & LWLink.ARROW_TAIL) == 0
        ) arrowTailCombo.setSelectedIndex(
          0
        ); else arrowTailCombo.setSelectedIndex(1);
      }
  ...
  // protected VueBeanState getDefaultState() { return
  // VueBeans.getState(LWLink.setDefaults(new LWLink())); }
