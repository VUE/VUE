...
public class MapViewport extends JViewport implements VueConstants {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MapViewport.class);
  ...
  private void placeMapLocationAtCanvasOrigin(float mapX, float mapY) {
    viewer.setMapOriginOffset(
      (float) (mapX * viewer.mZoomFactor),
  ...
  public void setViewPosition(Point p) {
    ...
    if (!p.equals(lastPosition)) {
      ...
      viewer.fireViewerEvent(
        MapViewer.Event.PAN,
        "MapViewport:setViewPosition0"
      );
    }
    lastPosition = p;
    super.setViewPosition(p);

    if (!isAdjusting()) viewer.trackViewChanges("MapViewport:setViewPosition1");
  }
  ...
  public void zoomAdjust(Point2D mapAnchor, Point2D screenPositionOfMapAnchor) {
    ...
    if (
      centerZoomStyle || screenPositionOfMapAnchor == null
    ) placeMapLocationAtViewCenter(
      mapAnchor
    ); else placeMapLocationAtViewLocation(
      mapAnchor,
      screenPositionOfMapAnchor
    );
  }

  public void placeMapLocationAtViewLocation(
    Point2D mapAnchor,
    Point2D viewLocation
  ) {
    Point2D canvasAnchor = viewer.mapToScreenPoint2D(mapAnchor);
    if (DEBUG.SCROLL) System.out.println(
      "  ZOOM CANVAS ANCHOR: " + out(canvasAnchor)
    );
  ...
  private void adjustCanvasSize(
    ...
    if (DEBUG.SCROLL && DEBUG.META) new Throwable("adjustSize")
      .printStackTrace();
    ...
    if (trimNorthWest) {
      ...
      if (DEBUG.SCROLL) out(
        "     is map loc: " + out(mapLocationAtCanvasOrigin)
      );
    ...
    if (!trimNorthWest) {
      ...
      if (originGrew) placeMapLocationAtCanvasOrigin(ox, oy);
    }
    ...
    if (!intermediate) {
      if (newCanvas.width < curView.width) newCanvas.width = curView.width;
      if (newCanvas.height < curView.height) newCanvas.height = curView.height;
    }

    if (!trimSouthEast) {
      // don't let new size be less than current canvas
      if (newCanvas.width < curCanvas.width) newCanvas.width = curCanvas.width;
      if (newCanvas.height < curCanvas.height) newCanvas.height =
        curCanvas.height;
    }
    ...
    if (DEBUG.SCROLL) {
      ...
      if (!vp.equals(curView)) out("!!cur view size: " + out(curView)); // same as above
      out(" current canvas: " + out(curCanvas));
      if (!curCanvas.equals(viewer.getSize())) out(
        "!!!!actual size: " + out(viewer.getSize())
      ); // same as above
  ...
  public void reshape(int x, int y, int w, int h) {
    ...
    if (viewer.isEditingLabel()) repaint();
  }
  ...
  private void setCanvasPosition(Point2D p, boolean allowGrowth) {
    if (DEBUG.SCROLL) out(
      "setCanvasPosition " + out(p) + " allowGrowth=" + allowGrowth
    );
    ...
    if (floorx != px || floory != py) {
      ...
      moved = true;
      if (!allowGrowth) out(
        "setCanvasPosition: growth disallowed, losing compensation " +
        out(decimal)
      );
    }

    if (allowGrowth) {
      if (grew) setCanvasSize(canvas);
      if (moved) viewer.setMapOriginOffset(ox, oy);
    }
