...
public class ActiveInstance<T> {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ActiveInstance.class);

  private static final Map<Class, ActiveInstance> AllActiveHandlers =
    new HashMap();
  private static final List<ActiveListener> ListenersForAllActiveEvents =
    new CopyOnWriteArrayList();
  protected static int depth = -1; // event delivery depth

  private final CopyOnWriteArrayList<ActiveListener> mListeners =
    new CopyOnWriteArrayList();
  private final Set<T> allInstances = Collections.synchronizedSet(
    new HashSet()
  );
  ...
  public static ActiveInstance getHandler(Class type) {
    ...
    synchronized (AllActiveHandlers) {
      ...
      if (handler == null) handler = new ActiveInstance(type);
    }
  ...
  private static String sourceName(Object s) {
    if (s == null) return "null"; else if (
      s instanceof ActiveEvent || s instanceof ActiveInstance
    ) return s.toString(); else return s.getClass().getName() + ":" + s;
  }
  ...
  private synchronized void setActiveAndNotify(
    final Object source,
    final T oldActive
  ) {
    if (DEBUG.EVENTS) {
    ...
    final ActiveEvent e = new ActiveEvent(
      itemType,
      source,
      oldActive,
      nowActive
    ...
    try {
      ...
      tufts.Util.printStackTrace(
        t,
        this + " onChange failed in implementation subclass: " + getClass()
      );
    }
    this.lastEvent = e;
  }
  ...
  private void markActive(Markable markableItem, boolean active) {
    try {
      ...
      tufts.Util.printStackTrace(
        t,
        this + " marking active state to " + active + " on " + markableItem
      );
    }
  }
  ...
  protected void notifyListeners(ActiveEvent<T> e) {
    if (inNotify) {
      String msg = this + " event loop: skipping delivery of: " + e;
      if (DEBUG.EVENTS) Log.warn(msg, new Throwable("HERE")); else Log.warn(
        msg
      );
      return;
    }
    ...
    try {
      depth++;
      if (mListeners.size() > 0) notifyListenerList(this, e, mListeners);
      if (ListenersForAllActiveEvents.size() > 0) notifyListenerList(
        this,
        e,
        ListenersForAllActiveEvents
      );
    } finally {
      depth--;
      inNotify = false;
  ...
  protected static void notifyListenerList(
    ...
    for (ActiveListener listener : listenerList) {
      ...
      try {
        if (method != null) method.invoke(
          ...
          e,
          e.active
        ); else listener.activeChanged(e);
      } catch (java.lang.reflect.InvocationTargetException ex) {
        tufts.Util.printStackTrace(
          ex.getCause(),
          handler + " exception notifying " + target + " with " + e
        );
      } catch (Throwable t) {
        tufts.Util.printStackTrace(
          t,
  ...
  public void addListener(ActiveListener listener) {
    if (mListeners.addIfAbsent(listener)) {
      if (DEBUG.EVENTS) Log.debug(
        String.format(
          TERM_YELLOW + "%-50s added listener %s" + TERM_CLEAR,
          itemTypeName,
          listener
        )
      );
  ...
  public void addListener(Object reflectedListener) {
    ...
    try {
      ...
      method =
        reflectedListener
          .getClass()
          .getMethod("activeChanged", ActiveEvent.class, itemType);
    } catch (Throwable t) {
      tufts.Util.printStackTrace(
        t,
        this +
        ...
        " must implement activeChanged(ActiveEvent, " +
        itemType +
        ")" +
        " to be a listener for the active instance of " +
        itemType
      );
      return;
    }
  ...
  public void removeListener(ActiveListener listener) {
    if (mListeners.remove(listener)) {
      if (DEBUG.EVENTS) outf(
        TERM_YELLOW + "%-50s removed listener %s\n" + TERM_CLEAR,
        this,
        listener
      );
  ...
  private static void lock(Object o, String msg) {
    if (DEBUG.THREAD) System.err.println(
      (o == null ? "ActiveInstance" : o) + " " + msg + " LOCK"
    );
  }

  private static void unlock(Object o, String msg) {
    if (DEBUG.THREAD) System.err.println(
      (o == null ? "ActiveInstance" : o) + " " + msg + " UNLOCK"
    );
  }
  ...
  private static final class MethodProxy implements ActiveListener {
    ...
    public boolean equals(Object o) {
      if (o instanceof MethodProxy) return (
        target == ((MethodProxy) o).target
      ); else return false;
    }

    @Override
    public String toString() {
      if (method == null) return String.format(
        "%s[%s]",
