...
public class Field implements tufts.vue.XMLUnmarshalListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Field.class);
  ...
  public void XML_completed(Object context) {
    ...
    } else mRelatedFields = Collections.EMPTY_LIST;
  }

  /** Wrapper for display of special values: e.g., EMPTY_VALUE ("") to "(no value)" */
  public static String valueText(Object value) {
    if (value == null) return null; else if (
      value == EMPTY_VALUE
    ) return "(no value)"; else return value.toString();
  }
  ...
  public String valueDisplay(Object value) {
    ...
    if (isQuantile()) {
      display =
        getName() + ": " + StringEscapeUtils.escapeHtml(valueText(value));
    } else {
  ...
  void annotateIncludedValues(final Collection<LWComponent> nodes) {
    if (mValues == null || count(mValues) < 1) {
      if (mContextValues != null) mContextValues.clear();
      return;
    }
    if (mContextValues == null) mContextValues =
      HashMultiset.create(); else mContextValues.clear();
    if (DEBUG.META) Log.debug(
      "MARKING INCLUDED VALUES AGAINST " + nodes.size() + " NODES for " + this
    );
  ...
  public void setStyleNode(LWComponent style) {
    if (DEBUG.SCHEMA) Log.debug(
      String.format("setStyleNode %-22s%s", this, style)
    );
    //         if (mNodeStyle != null)
  ...
  private void takeType(String type, String cause) {
    if (DEBUG.Enabled) Log.debug(
      toTerm() + " type=>" + type + " on " + Util.tags(cause)
    );
    mType = type;
  }
  ...
  @Override
  public String toString() {
    if (schema == null) return String.format(
      "<?>.%s",
      getName()
    ); else return String.format("%s.%s", schema.getName(), getName());
  }
  ...
  protected int uniqueValueCount() {
    if (mValues == null) {
      if (mMaxValueLen == 0) return 0; else return valueCount();
    } else {
  ...
  private static final Multiset EMPTY_MULTISET = Multisets.unmodifiableMultiset(
  ...
  public Multiset<String> getValueSet() {
    return mValues == null
      ? EMPTY_MULTISET
      : Multisets.unmodifiableMultiset(mValues);
  ...
  void trackValue(String value) {
    ...
    if (valueLen > mMaxValueLen) mMaxValueLen = valueLen;

    if (mValueTrackDisabled) return;
    ...
    if (mValues.contains(value)) mAllValuesUnique = false;
    ...
    if (!mTypeDetermined) trackForTypeInference(value);
  }
  ...
  private void trackForTypeInference(final String text) {
    if (text.indexOf(':') > 0) {
      ...
      if (Double.isNaN(number)) {
        ...
        if (number < mMinValue) mMinValue = number; else if (
          number > mMaxValue
        ) mMaxValue = number;
        mValuesTotal += number;
  ...
  private double getNumericValue(final String text, final boolean tryCurrency) {
    ...
    if (
      ...
      tryCurrency &&
      text.length() > 1 &&
      isCurrencySymbol(text.codePointAt(0))
    ) {
    ...
    if (DEBUG.SCHEMA || DEBUG.DATA) Log.debug(
      Util.tags(text) + " = " + Util.tags(value)
    );
  ...
  private static boolean isDateValue(String value) {
    ...
    try {
      date = new Date(value);
      if (DEBUG.Enabled) Log.debug(
        "PARSED DATE: " + Util.tags(date) + " from " + value
      );
    } catch (Throwable t) {
      if (DEBUG.DATA) Log.debug(
        "Failed to parse [" + value + "] as date: " + t
      );
    }
  ...
  private static double computeQuantiles(
    ...
    if (DEBUG.Enabled) Log.debug(
      "count of all possible values: " + values.length
    );
    ...
    if (DEBUG.Enabled) Log.debug(
      "each of " +
      regions +
      " quantile regions has an approx sample size of: " +
      range +
      " samples"
    );
    ...
    for (int i = 0; i < quantiles.length; i++) {
      ...
      if (DEBUG.Enabled) Log.debug(
        String.format(
          "quantile %d index %3.2f (%d) value = " + values[index],
          i,
          rawIndex,
          index
        )
    ...
    if (values.length % 2 == 0) {
      ...
      if (DEBUG.Enabled) Log.debug(
        String.format(
          "AVERAGED MEDIAN: %g from %g+%g halfIndex=%d",
          median,
          belowMedian,
          aboveMedian,
          halfIndex
      ...
      if (DEBUG.Enabled) Log.debug(
        String.format(
          "PICKED MEDIAN: %g from exact middle index=%d",
          median,
          halfIndex
        )
      );
    ...
    if (EVEN_REGIONS) {
      if (quantiles[quantiles.length / 2] != median) {
        if (DEBUG.Enabled) Log.info(
          String.format(
            "PATCHING MIDDLE QUANTILE TO ABSOLUTE MEDIAN; %g -> %g",
            quantiles[quantiles.length / 2],
            median
          )
        );
  ...
  private static void computeValueRangeQuantiles(
    final double[] quantiles,
    ...
    if (DEBUG.Enabled) Log.debug(
      String.format(
        "computing value-based quantiles for values (%g-%g) range=%g, quantileRange=%g",
        minValue,
        maxValue,
        allValueRange,
        quantileValueRange
    ...
    for (int i = 0; i < quantiles.length; i++) {
      ...
      if (DEBUG.Enabled) Log.debug(
        String.format("quantile %d value = %g", i, quantiles[i])
      );
    }
  }
  ...
  private void computeQuantiles(final double[] allValues) {
    ...
    if (USE_STANDARD_QUANTILES) {
      ...
      if (mValues.contains(EMPTY_VALUE)) validCount--;

      final double[] uniqueValues = new double[validCount];

      int i = 0;
      for (String s : mValues.elementSet()) if (
        s != EMPTY_VALUE
  ...
  private int getQuantile(final double value) {
    ...
    if (SKEW_QUANTILES_LOW) {
      for (int i = 0; i < mQuantiles.length; i++) if (
        value <= mQuantiles[i]
      ) return i;
    } else {
      for (int i = 0; i < mQuantiles.length; i++) if (
  ...
  private String getQuantileName(int i) {
    ...
    if (
      mAllValuesAreIntegers && SKEW_QUANTILES_HIGH
    ) TOP_RANGE_ADJUSTMENT = 1.0; else TOP_RANGE_ADJUSTMENT = 0.0; // could still adjust low, but would need different adjustment

    final double min, max;

    if (i == 0) min = mMinValue; else min = mQuantiles[i - 1];

    if (i == mQuantiles.length) max = mMaxValue; else max =
      mQuantiles[i] - TOP_RANGE_ADJUSTMENT;

    if (mAllValuesAreIntegers) return String.format(
      "Q%d: %,.0f-%,.0f",
      i + 1,
      min,
      max
    ); else return String.format("Q%d: %,g-%,g", i + 1, min, max);
  }
  ...
  void performFinalAnalysis() {
    ...
    if (!isNumeric() || uniqueValueCount() <= (QUANTILE_BUCKETS * 3)) return;
    ...
    if (USE_STANDARD_QUANTILES) allValues =
      new double[mValuesSeen]; else allValues = null;
    ...
    for (DataRow row : schema.getRows()) {
      ...
      if (USE_STANDARD_QUANTILES) allValues[count] = value;
      count++;
    ...
    final Field quantileField = schema.addFieldBefore(
      this,
    ...
    quantileField.setStyleNode(
      DataAction.initNewStyleNode(getStyleNode().duplicate())
    );
    ...
    if (mAllValuesAreIntegers) {
      ...
      if (!Double.isNaN(mMedianValue)) quantileField.addDataComment(
        String.format("Median: %.1f", mMedianValue)
      );
      ...
      if (!Double.isNaN(mMedianValue)) quantileField.addDataComment(
        String.format("Median: %g", mMedianValue)
      );
      quantileField.addDataComment(
        String.format(
          "Std Dev: %g x %.1f",
  ...
  private String sampleValues(boolean unique) {
    if (count(mValues) <= 20) return unique
      ? mValues.elementSet().toString()
      : mValues.toString();
  ...
  public String valuesDebug() {
    if (mValues == null) {
      if (mValuesSeen == 0) return "(empty)"; else return String.format(
        "%5d values (un-tracked; max-len%6d)",
        mValuesSeen,
        mMaxValueLen
      );
    } else if (isSingleton()) {
      ...
      if (count(mValues) > 1) {
        return String.format(
          "%5d unique, single-instance values; %s",
          count(mValues),
          sampleValues(true)
        );
