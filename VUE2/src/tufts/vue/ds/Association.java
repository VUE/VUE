...
public final class Association {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Association.class);

  private static final Multimap<Field, Association> AllByField =
  ...
  private static final EventHandler<Event> EventSource =
    EventHandler.getHandler(Event.class);

  private Association(Field f1, Field f2, boolean isOn) {
    if (f1 == f2) throw new IllegalArgumentException(
      "field can't associate to itself: " + f1
    );
    if (f1 == null || f2 == null) throw new IllegalArgumentException(
      "null field: " + f1 + "; " + f2
    );
    field1 = f1;
    field2 = f2;
    ...
    enabled = isOn;
    if (schema1 == schema2) throw new IllegalArgumentException(
      "can't associate fields from the same schema: " +
      f1 +
      "; " +
      f2 +
      "; " +
  ...
  public static void add(Field f1, Field f2) {
    ...
    if (a != null) EventSource.raise(
      Association.class,
      new Event(a, Event.ADDED)
    );
  }

  private static synchronized Association addImpl(
    Field f1,
    Field f2,
    int index
  ) {
    ...
    for (Association scan : AllPairsList) {
      if (
        (scan.field1 == f1 && scan.field2 == f2) ||
        (scan.field1 == f2 && scan.field2 == f1)
      ) {
        alreadyExists = true;
        if (DEBUG.SCHEMA) Log.debug(
          "an association already exists between " + f1 + " and " + f2
        );
        break;
      }
    }
    ...
    if (alreadyExists) {
      ...
      AllByField.put(f1, a);
      AllByField.put(f2, a);
      if (index >= 0) AllPairsList.add(index, a); else AllPairsList.add(a);
      if (DEBUG.SCHEMA) {
  ...
  static void updateForNewAuthoritativeSchema(final Schema newAuthority) {
    if (verifyAll(newAuthority)) EventSource.raise(
      Association.class,
      new Event(null, Event.CHANGED)
    );
  }
  ...
  private static synchronized boolean verifyAll(final Schema newAuthority) {
    ...
    for (Association a : Util.copy(AllPairsList)) { // list may change during iteration
      ...
      try {
        if (verifyAndUpdate(a, newAuthority)) changed = true;
      } catch (Throwable t) {
        Log.error(
  ...
  private static boolean verifyAndUpdate(
    final Association a,
    ...
    if (s1 == a.schema1) f1 = a.field1; else f1 = replaceField(a.field1, s1);

    if (s2 == a.schema2) f2 = a.field2; else f2 = replaceField(a.field2, s2);
  ...
  private static Schema replaceSchema(
    ...
    if (s != old) {
      if (s != newAuthority) {
        Log.warn(
          "ASSOCIATION NEEDED PATCHING UNRELATED TO NEW AUTHORITY REPORT;" +
          "\n\t    old: " +
          old +
          "\n\t lookup: " +
          s +
      ...
      if (!old.isDiscarded()) {
        Log.warn(
          "REPLACED SCHEMA WASN'T DISCARDED; " +
          "\n\t    old: " +
          old +
          "\n\t lookup: " +
          s +
  ...
  public static Association get(int n) {
    if (n < 0 || n > getCount()) return null; else return AllPairsList.get(n);
  }
  ...
  public static List<Association> getBetweens(Schema s1, Schema s2) {
    ...
    for (Association a : getAll()) {
      if (a.isEnabled() && a.isBetween(s1, s2)) betweens.add(a);
    }
    //if (DEBUG.SCHEMA && DEBUG.META)
    if (DEBUG.Enabled) dumpSmart(
      betweens,
      "betweens for schemas: " + s1.getName() + " x " + s2.getName()
    );
    return betweens;
  }
  ...
  public static List<Association> getJoins(Schema s1, Field field) {
    ...
    if (s1 == null) throw new NullPointerException("null Schema");
    if (field == null) throw new NullPointerException("null Field");

    for (Association a : getAll()) {
      if (
        a.isEnabled() && a.isBetween(s1, s2) && !a.contains(field)
      ) betweens.add(a);
    }
    if (DEBUG.Enabled) {
      dumpSmart(
  ...
  private static void dumpSmart(List list, String msg) {
    String s = "";
    if (list.size() <= 1) {
      if (list.size() == 1) s = ": " + list.get(0).toString(); else s =
        ": none";
      //s = ": " + Util.tags(list);
    }
    Log.debug(msg + s);
    if (list.size() > 1) Util.dump(list);
  }
  ...
  public static Collection<Association> getAliases(Field f) {
    ...
    if (DEBUG.Enabled) {
      final Object gots;
      if (got.size() == 0) gots = null; else if (got.size() == 1) gots =
        Util.getFirst(got); else gots = "";
  ...
  public Field getPairedField(Field f) {
    if (f == field1) return field2; else if (
      f == field2
    ) return field1; else throw new Error("field not in association: " + f);
  }

  public Field getFieldForSchema(Schema s) {
    if (s == schema1) return field1; else if (
      s == schema2
    ) return field2; else throw new Error(
      "field for schema not in association: " + s
    );
  }
  ...
  public boolean isBetween(Schema s1, Schema s2) {
    if (schema1 == s1 && schema2 == s2) return true; else if (
      schema1 == s2 && schema2 == s1
