...
public class Schema implements tufts.vue.XMLUnmarshalListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Schema.class);
  ...
  private static final Multimap<String, Field> AllFieldsByLowerName =
  ...
  private static final java.util.concurrent.atomic.AtomicInteger NextLocalId =
  ...
  private static final Map<Resource, Schema> _SchemaByResource =
    Collections.synchronizedMap(new HashMap());
  private static final Map<String, Schema> _SchemaByDSGUID =
    Collections.synchronizedMap(new HashMap());
  ...
  private static synchronized void registerAsAuthority(Schema schema) {
    if (schema.DSGUID != null) registerByDSGUID(schema, schema.DSGUID);
  ...
  private static synchronized void registerByDSGUID(Schema s, String dsGUID) {
    ...
    if (prev != null && prev != s) {
      Log.warn(
        "REPLACING EXISTING DSGUID; orphaning: " + prev,
        new Throwable("HERE")
      );
    }
    if (prev == s) Log.info(
      s + "; was already AUTHORITATIVE for DSGUID " + dsGUID
    ); else Log.info(s + "; recorded as AUTHORITATIVE for DSGUID " + dsGUID);
    if (s.getDSGUID() != dsGUID) Log.warn(
      "" + s,
      new Throwable("INCONSISTENT DSGUID STATE; " + dsGUID)
    );
  }
  ...
  private static synchronized void registerByResource(Schema s, Resource r) {
    ...
    if (prev != null) {
      Log.warn(
        "REPLACING EXISTING byRESOURCE; orphaning" + prev,
        new Throwable("HERE")
      );
    }
    if (prev == s) Log.info(
      s + "; was already AUTHORITATIVE for Resource: " + r
    ); else Log.info(s + "; recorded as AUTHORITATIVE for Resource: " + r);
    if (s.getResource() != r) Log.warn(
      "" + s,
      new Throwable(
        "INCONSISTENT RESOURCE STATE " + r + " != " + s.getResource()
      )
    );
  }
  ...
  private void copyStyles(Schema s) {
    if (s.mPersistFields.size() > 0) {
      if (DEBUG.SCHEMA) Log.debug(
        "copyStyles; from persisted handles:" +
        "\n\tsource: " +
        s +
        "\n\ttarget: " +
        this
  ...
  private void copyStyles(Collection<Field> fields) {
    for (Field src : fields) {
      ...
      if (target != null) target.setStyleNode(
        runtimeInitStyleNode(src.getStyleNode())
      );
    }
  }
  ...
  public static synchronized void dumpAuthorities() {
  ...
  public static Schema getNewAuthorityInstance(
    Resource r,
    String dsGUID,
    String displayName
  ) {
  ...
  private static synchronized Schema getInstance(
    ...
    if (true) {
      return getInstanceAuthoritiesAreNew(
        r,
        dsGUID,
        displayName,
        infoIsAuthority
      ...
      return getInstanceAuthoritiesAreReloaded(
        r,
        dsGUID,
        displayName,
        infoIsAuthority
    ...
    if (s == null) {
      ...
      if (infoIsAuthority) {
        ...
        if (!r.equals(s.getResource())) {
          Log.info(
            "updating Resource in authority schema:" +
            "\n\told: " +
            s.getResource() +
            "\n\tnew: " +
            r
        ...
        if (dsGUID != null && !dsGUID.equals(s.getDSGUID())) {
          Log.info(
            "updating DSGUID in authority schema",
            new Throwable("HERE")
          );
          s.setDSGUID(dsGUID);
        }
  ...
  private static synchronized boolean updateRestoredMapToAuthorities(
    ...
    if (existing != null) {
      if (DEBUG.Enabled) Log.debug(
        "DUMPING THIS SCHEMA FOR EXISTING:\n\t  dumped: " +
        handle +
        "\n\texisting: " +
        existing
      );
  ...
  private static synchronized void replaceSchemaReferences(
    ...
    Log.info(
      String.format(
        "updated %d schema handle references in %s",
        updateCount,
        sourceMap
      )
    );
  ...
  private static synchronized void makeSchemaReferencesAuthoritative(
    ...
    if (DEBUG.Enabled) {
      Log.debug(
        "makeSchemaReferencesAuthoritative; " + newAuthority + "; maps: " + maps
      );
      dumpAuthorities();
    }
    ...
    if (!newAuthority.isLoaded()) {
      Log.warn(
        "newly loaded schema is empty: " + newAuthority,
        new Throwable("FYI")
      );
      return;
    }
    ...
    for (tufts.vue.LWMap map : maps) {
      ...
      if (updateCount > countAtMapStart) updateMapCount++;
      scanMapCount++;
    }
    Log.info(
      String.format(
        "scanned %d schema references in %d maps",
        scanCount,
        scanMapCount
      )
    );
    Log.info(
      String.format(
        "replaced %d schema references in %d maps",
        updateCount,
        updateMapCount
      )
    );
  ...
  private static synchronized void createAttachableAssociations(
    Schema schema,
    Collection<Field> restoredFields
  ) {
    ...
    for (final Field field : restoredFields) {
      for (Field.PersistRef ref : field.getRelatedFieldRefs()) {
        if (DEBUG.SCHEMA) Log.debug(
          schema + "; persisted relation for " + field + ": " + ref
        );
        for (Field possibleMatch : AllFieldsByLowerName.get(
          ref.fieldName.toLowerCase()
        )) {
          ...
          if (ref.schemaGuid.equals(guid)) {
            ...
            if (match.getSchema().isDiscarded()) {
              Log.debug(
                "ignoring association match for discarded schema: " + match
              );
            } else {
              Log.debug(
                "found live field to match ref: " +
                ref +
                " = " +
                Util.tags(match)
              );
  ...
  private synchronized Field addField(
    final Field newField,
    final Field nearField,
    boolean before
  ) {
    ...
    if (nearField != null) mFieldList.add(
      mFieldList.indexOf(nearField) + (before ? 0 : 1),
      newField
    ); else mFieldList.add(newField);
  ...
  public void setImageField(String name) {
    if (name == null) mImageField = null; else mImageField = findField(name);
  }
  ...
  public synchronized void annotateFor(Collection<LWComponent> nodes) {
    for (Field field : getFields()) field.annotateIncludedValues(nodes);
  ...
  public synchronized DataRow findRow(Field field, String value) {
    for (DataRow row : mRows) if (row.contains(field, value)) return row;
    return null;
  }
  ...
  public synchronized Field getKeyField() {
    if (mKeyField == null) mKeyField = getKeyFieldGuess();
    return mKeyField;
  }

  public synchronized Field getEncodingField() {
    if (mEncodingField == null) return new Field(
      "win",
      this
    ); else return mEncodingField;
  }
  ...
  public synchronized Field getField(String name) {
    if (name == null) {
      Log.warn(
        "searching for null Field name in " + this,
        new Throwable("HERE")
      );
      return null;
    }
    final Field f = findField(name);
    if (f == null && DEBUG.Enabled) Log.debug(
      String.format(
        "%s; no field named '%s' in %s",
        this,
        name,
        //mFields.keySet()
  ...
  public synchronized Field findField(String name) {
    ...
    if (
      ...
      if (DEBUG.SCHEMA && f != null) Log.debug(
        "found lowerized [" + name + "]"
      );
    }
    return f;
  }
  ...
  public synchronized String getName() {
    if (mName != null) {
      ...
      String s = r.getTitle();
      if (s == null) return r.getSpec(); else return s;
    }
  ...
  public void ensureFields(
    ...
    if (isMatrixData) addField(matrixNameField);

    int index = 0;
    for (String name : names) {
      name = name.trim();
      if (isMatrixData) {
        String[] matrixEntities = {
          ds.getMatrixRowField(),
          ds.getMatrixColField(),
          ds.getMatrixRelField(),
          ds.getMatrixPivotField(),
        };

        if (
          isMatrixData &&
          org.apache.commons.lang.ArrayUtils.contains(matrixEntities, name)
        ) {
  ...
  private static boolean isUnlikelyKeyField(Field f) {
    ...
    return (
      f.isSingleValue() ||
      (f.getName().startsWith("dc:") && !f.getName().equals("dc:identifier"))
    );
  }
  ...
  public synchronized Vector<String> getPossibleKeyFieldNames() {
    ...
    for (Field field : getFields()) if (
      field.isPossibleKeyField()
  ...
  public synchronized Vector<String> getFieldNames() {
    ...
    for (Field field : getFields()) names.add(field.getName());
  ...
  public synchronized Field getKeyFieldGuess() {
    if (getFieldCount() == 0) throw new Error("no fields in " + this);

    // Many RSS feeds can be covered by checking "guid" and "link"

    Field f;
    if ((f = findField("guid")) != null && f.isPossibleKeyField()) return f;
    if ((f = findField("key")) != null && f.isPossibleKeyField()) return f;
    //if ((f = getField("link")) != null && f.isPossibleKeyField()) // some rss news feeds have dupe entries
    if ((f = findField("link")) != null && !f.isSingleValue()) return f;
  ...
  protected void addMatrixRow(XmlDataSource ds, String[] values) {
    ...
    for (Field field : getFields()) {
      if (field.getName().equals(MATRIX_NAME_FIELD)) continue;
    ...
    if (existingRows.get(values[matrixColNums.get(rowName)]) == null) {
      addRow(fromRow);
      existingRows.put(
        values[matrixColNums.get(rowName)],
        new Integer(fromRow.mmap.size())
      );
    } else {
      ...
      if (valCount < fromRow.mmap.size()) {
        ...
        for (DataRow r : getRows()) {
          if (
            r
              .getValue(MATRIX_NAME_FIELD)
              .equals(values[matrixColNums.get(rowName)])
          ) {
            getRows().remove(r);
            break;
          }
    ...
    if (
      !values[matrixColNums.get(rowName)].equals(
          values[matrixColNums.get(colName)]
        )
    ) {
      ...
      if (
        ...
        new MatrixRelationship(
          values[matrixColNums.get(rowName)],
          values[matrixColNums.get(colName)],
          values[matrixColNums.get(relName)]
        )
      );
    }
    return;
  ...
  protected void addWideMatrixRow(XmlDataSource ds, String[] values) {
    ...
    //skip partial rows
    if (values.length < matrixSize) return;
    ...
    if (rowCount < matrixSize) {
      ...
      for (int i = pivotNum + 1; i < pivotNum + matrixSize + 1; i++) {
        if (thisCol.equals(ds.headerValues[i])) continue;
        ...
        if (!(values[i].equals(ds.getMatrixIgnoreField()))) {
          ...
          matrixRelations.add(
            new MatrixRelationship(thisCol, ds.headerValues[i], res.toString())
          );
        }
      }
  ...
  public Collection<DataRow> getMatchingRows(Field field, String fieldValue) {
    if (DEBUG.Enabled) Log.debug(
      "getMatchingRows: " + Relation.quoteKV(field, fieldValue)
    );
    ...
    if (hasField(field)) {
      ...
      if (Association.hasJoins(this, field)) {
        for (DataRow row : getRows()) {
          if (
            Relation.getCrossSchemaRelation(field, row.getData(), fieldValue) !=
            null
          ) results.add(row);
        }
      }
  ...
  public synchronized String getSingletonValue(String name) {
    Field f = mFields.get(name);
    if (f != null && f.isSingleton()) return f
      .getValueSet()
      .iterator()
...
final class DataRow implements Relation.Scannable {
  ...
  String takeValue(Field f, String value) {
    ...
    if (value.length() == 0) value = Field.EMPTY_VALUE;
