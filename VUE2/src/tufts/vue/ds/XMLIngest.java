...
public class XMLIngest {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(XMLIngest.class);
  ...
  public static class XmlSchema extends tufts.vue.ds.Schema {
    ...
    public XmlSchema(tufts.vue.Resource source, String itemPath) {
      ...
      if (itemPath == null || itemPath.length() == 0) itemPathLen =
        0; else itemPathLen = itemPath.length() + 1; // add one for dot
    ...
    void trackFieldValuePair(String name, String value) {
      ...
      if (
        itemPath != null &&
        name.startsWith(itemPath) &&
        name.length() > itemPathLen
      ...
      if (field == null) {
        field = addField(name);
        if (name.length() > mLongestFieldName) mLongestFieldName =
          name.length();
      }
      if (curRow != null) curRow.addValue(field, value); else field.trackValue(
        value
      );
    }
  ...
  static void XPathExtract(XmlSchema schema, Document document) {
    try {
      ...
      Node nodeValue = (Node) xpath.evaluate(
        expression,
        document,
        XPathConstants.NODE
      );
      ...
      String stringValue = (String) xpath.evaluate(
        expression,
        document,
  ...
  public static Schema ingestXML(
    XmlSchema schema,
    ...
    if (schema == null) schema =
      new XmlSchema(
        tufts.vue.Resource.instance(input),
        itemKey
      ); else schema.flushData();

    if (false) XPathExtract(schema, doc); else scanNode(
      schema,
  ...
  private static void scanNode(
    ...
    final boolean hasAttributes =
      (!isAttribute && node != null && node.hasAttributes());
    ...
    if (isAttribute) XMLName =
      parentName + ATTR_SEPARATOR + nodeName; else XMLName = nodeName;

    final String fullName;

    if (parentPath != null) { // should only be null first time in at the top root
      if (isMergedText) fullName = parentPath; else if (isAttribute) fullName =
        parentPath + ATTR_SEPARATOR + nodeName; else fullName =
        parentPath + '.' + nodeName;
    } else {
      fullName = nodeName;
    }
    ...
    if (hasAttributes && ATTRIBUTES_IMMEDIATE) scanAttributes(
      schema,
      fullName,
      nodeName,
      node.getAttributes()
    ...
    if (DO_TAG) {
      ...
      if (
        firstChild == null || (isText(firstChild) && firstChild == lastChild)
      ) {
      ...
      if (
        FOLD_TEXT && (type != Node.ELEMENT_NODE && type != Node.ATTRIBUTE_NODE)
      ) {
    ...
    if (!isAttribute && node != null) {
      ...
      if (FOLD_KEYS || schema.isXMLKeyFold()) {
        ...
        for (int i = 0; i < children.getLength(); i++) scanNode(
          schema,
          children.item(i),
          fullName,
          nodeName
    ...
    if (DO_TAG) {
      if (closeOnSameLine) outln("</" + XMLName + ">"); else ioutln(
        "</" + XMLName + ">"
      );
    }
  ...
  private static void scanFoldedChildren(
    XmlSchema schema,
  ...
  private static org.w3c.dom.Document parseXML(
    Object input,
    boolean validating
  ) {
    try {
  ...
  static final boolean ATTRIBUTES_IMMEDIATE = false; // false better for clearer XML output, true better for schema output (e.g., rss.version 1st, not last)
  static final boolean FOLD_TEXT = true; // default true: fold Node.TEXT_NODE(#text) and CDATA items into parent node

  static final boolean FOLD_KEYS = false; // auto-enabled if top-level item is "plist" (current breaks on JIRA XML if true)
  ...
  private static final String JIRA_VUE_URL =
    "http://bugs.atech.tufts.edu/secure/IssueNavigator.jspa?view=rss&pid=10001&tempMax=9999&reset=true&decorator=none";
  private static final String JIRA_SFRAIZE_COOKIE =
    "seraph.os.cookie=LkPlQkOlJlHkHiEpGiOiGjJjFi";
    ...
    org.apache.log4j.Logger
      ...
      .addAppender(
        new org.apache.log4j.ConsoleAppender(
          tufts.vue.VUE.MasterLogPattern,
          "System.err"
        )
      );
