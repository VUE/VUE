...
public class DataTree extends javax.swing.JTree implements DragGestureListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DataTree.class);
  ...
  private final AbstractButton mUpdateButton = new JButton(
    VueResources.getString("dockWindow.contentPanel.sync.updateMap")
  );
  private final AbstractButton mSendToMapButton = new JButton("Send to Map");
  private final DefaultTreeModel mTreeModel;
  private static final Color MEDIUM_DARK_GRAY = new Color(96, 96, 96);
  private static final boolean DEBUG_LOCAL = false;

  private Thread mAnnotateThread;

  private static final MapListener ActiveMapListener = new MapListener();
  private static final java.util.concurrent.atomic.AtomicBoolean FirstInstance =
    new java.util.concurrent.atomic.AtomicBoolean(true);

  private static final Collection<DataTree> ActiveTrees =
  ...
  private static volatile Collection<LWComponent> ActiveMapDataNodes =
    Collections.EMPTY_LIST;

  public static final class MapListener
    implements LWComponent.Listener, LWSelection.Listener {
    ...
    public void LWCChanged(tufts.vue.LWCEvent e) {
      ...
      if (e.key == LWKey.UserActionCompleted && mDataEventWasSeen) {
        ...
        if (DEBUG.ANNOTATE) Log.debug(
          "   FOUND DATA EVENT: " + e + "; seenOne=" + mDataEventWasSeen
        );
      }
    }
    ...
    private void kickOffAnnotations() {
      if (DEBUG.ANNOTATE) Log.debug(
        "kicking off annotations for: " + Util.tags(ActiveTrees)
      );

      if (ActiveTrees.size() > 0) {
        loadGlobalDataForAnnotations();
        for (DataTree tree : ActiveTrees) tree.kickAnnotate();
      }
    }
    ...
    public void selectionChanged(LWSelection selection) {
      if (selection.isEmpty()) {
        if (ActiveTrees.size() > 0) {
          for (DataTree tree : ActiveTrees) tree.setSelectionPath(null);
        }
      }
    }
  }
  ...
  private static void loadGlobalDataForAnnotations() {
    ...
    if (DEBUG.Enabled) Log.debug(
      "loading global annotation data from map " + mActiveMap
    );
    ...
    if (mActiveMap != null) {
      ...
      for (LWComponent c : allNodes) {
        if (
          c.isDataNode() && c instanceof LWNode
        ) dataNodes.add(c); // leave out data-links for now
      }
  ...
  private void updateMap() {
    ...
    try {
      ...
      if (mChangedRowsCheckBox.isSelected()) {
        ...
        if (mSchema.isMatrixDataSet) GUI.invokeAfterAWT(
          new Runnable() {
            public void run() {
              applyMatrixRelations(newNodes);
            }
  ...
  public synchronized void applyMatrixRelations(List<LWComponent> newNodes) {
    ...
    for (LWComponent newNode : newNodes) {
      String trueName = newNode
        .getRawData()
        .getString(Schema.MATRIX_NAME_FIELD);

      for (MatrixRelationship relation : relations) {
        if (
          relation.getFromLabel().equals(trueName) ||
          relation.getToLabel().equals(trueName)
        ) {
          for (TreeNode n : mAllRowsNode.getChildren()) {
            RowNode rn = (RowNode) n;

            if (rn.isMapPresent()) {
              String potentialTargetName = rn
                .getRow()
                .getValue(Schema.MATRIX_NAME_FIELD);
              if (
                potentialTargetName.equals(relation.getToLabel()) ||
                potentialTargetName.equals(relation.getFromLabel())
              ) {
                //System.out.println("Relation : " + relation.getFromLabel() + "," + relation.getToLabel());
                //try to find a place to draw it.
                final Collection<LWComponent> searchSet = VUE
                  .getActiveViewer()
                  .getMap()
                  .getAllDescendents(LWComponent.ChildKind.EDITABLE);
                ...
                for (LWComponent hit : hits) {
                  ...
                  if (newNode.getLabel().equals(hit.getLabel())) continue;

                  if (
                    relation.getFromLabel().equals(trueName) &&
                    !newNode.hasDirectedLinkTo(hit)
                  ) link = new LWLink(newNode, hit);

                  if (
                    relation.getToLabel().equals(trueName) &&
                    !hit.hasDirectedLinkTo(newNode)
                  ...
                  if (
                    hit.hasMultipleLinksTo(newNode) &&
                    VUE.getSelection().size() > 0
  ...
  private void enableUpdateButton() {
    mUpdateButton.setEnabled(
      (mNewRowsCheckBox.isEnabled() && mNewRowsCheckBox.isSelected()) ||
  ...
  private void applyDataUpdatesToMap(final LWMap map) {
    ...
    if (DEBUG.Enabled) Log.debug(
      "Found " + freshData.size() + " data rows with newer data for map"
    );
    ...
    if (DEBUG.Enabled) Log.debug(
      "Updated " + patched.size() + " nodes with fresh data"
    );
    ...
    map
      .getUndoManager()
      .mark(String.format("Update %d Data Nodes", patched.size()));
  }
  ...
  private static JComponent buildControllerUI(final DataTree tree) {
    ...
    tree.mNewRowsCheckBox.addActionListener(
      new ActionListener() {
    ...
    tree.mChangedRowsCheckBox.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          tree.enableUpdateButton();
        }
    ...
    tree.mUpdateButton.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          tree.updateMap();
        }
    ...
    tree.mSendToMapButton.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
    ...
    if (imagePath == null) imagePath =
      schema.getSingletonValue("rdf:RDF.image.url");
    if (imagePath != null) {
    ...
    final int GUTTER = 4;
    JPanel toolbar = new JPanel(), remainderPanel = new JPanel(), newRowsPanel =
      new JPanel(), changedRowsPanel = new JPanel();
    Insets noInset = new Insets(0, 0, 0, 0), panelInset = //top, left, bottom, right
      new Insets(0, 0, 0, GUTTER), buttonInset = new Insets(
      0,
      0,
      GUTTER,
      GUTTER
    ...
    if (dataSourceLabel != null) wrap.add(dataSourceLabel, BorderLayout.SOUTH);
  ...
  protected void setExpandedState(final TreePath path, final boolean expanded) {
    final DataNode treeNode = path == null
      ? null
      : (DataNode) path.getLastPathComponent();

    if (DEBUG.Enabled) Log.debug(
      "setExpandedState " + path + " = " + expanded + "; " + Util.tags(treeNode)
    );
    ...
    GUI.invokeAfterAWT(
      new Runnable() {
        public void run() {
          if (!inDoubleClick) {
            if (DEBUG.FOCUS) Log.debug(
              "setExpandedState " + path + " = " + expanded + " RELAYING"
            );
            DataTree.super.setExpandedState(path, expanded);
          } else if (DEBUG.FOCUS) Log.debug(
            "setExpandedState " + path + " = " + expanded + " SKIPPING"
          );
          inDoubleClick = false;
        }
      }
  ...
  private class ClickHandler extends tufts.vue.MouseAdapter {
    ...
    public void mousePressed(java.awt.event.MouseEvent e) {
      ...
      if (DEBUG.Enabled) {
        ...
        if (mClickPath != null) Log.debug(
          "MOUSE PRESSED ON: " + Util.tags(mClickPath.getLastPathComponent())
        ); else Log.debug("MOUSE PRESSED ON: nothing");
      }
    ...
    public void mouseClicked(java.awt.event.MouseEvent e) {
      ...
      if (!GUI.isDoubleClick(e)) return;

      if (DEBUG.Enabled) Log.debug(
        "ACTIONABLE DOUBLE CLICK ON " + Util.tags(treeNode)
      );
      ...
      if (
        treeNode.isRow() ||
        (treeNode.getField() != null &&
          treeNode.getField().isPossibleKeyField())
      ) {
        selectMatchingNodes(treeNode, false);
      }
    }
  ...
  private void selectMatchingNodes(
    final DataNode treeNode,
    final boolean extendSearch
  ) {
    if (mActiveMap == null) return;
    ...
    final Collection<LWComponent> searchSet = mActiveMap.getAllDescendents(
      LWComponent.ChildKind.EDITABLE
    );

    if (DEBUG.Enabled) Log.debug(
      "SEARCH:\n\nSEARCHING ALL EDITABLE DESCENDENTS of " +
      mActiveMap +
      "; count=" +
      searchSet.size() +
      "; treeNode=" +
   ...
   */
  private void findAndSelectMatchingNodes(
    ...
    if (field == null) {
      if (treeNode == mAllRowsNode || treeNode instanceof RowNode) {
        ...
        if (treeNode == mAllRowsNode) styleRecord = mSchema.getRowNodeStyle();
      } else {
    ...
    if (extendSearch) {
      ...
      if (treeNode.isField() && styleRecord == null) styleRecord =
        field.getStyleNode();
    }
    ...
    if (extendSearch) {
      ...
      for (LWComponent c : searchSet) if (criteria.matches(c)) hits.add(c);
    ...
    if (DEBUG.Enabled) {
      if (hits.size() == 1) Log.debug(
        "hits=" + hits.get(0) + " [single hit]"
      ); else Log.debug("hits=" + hits.size());
  ...
  public Criteria dataNodeToSearchCriteria(final DataNode treeNode) {
    ...
    if (treeNode == mAllRowsNode) {
      ...
      if (DEBUG.Enabled) Log.debug(
        "searching for all data records in schema " + mSchema
      );
      ...
      final String keyField =
        ((RowNode) treeNode).getSchema().getKeyFieldName();
      ...
      if (DEBUG.Enabled) Log.debug(
        "searching for any enumerated value from a field named " + fieldName
      );
      ...
      if (DEBUG.Enabled) Log.debug(
        String.format("searching for %s=[%s]", fieldName, fieldValue)
      );
  ...
  public abstract static class Criteria {
    ...
    public List<LWComponent> search(final Collection<LWComponent> searchSet) {
      ...
      for (LWComponent c : searchSet) if (matches(c)) hits.add(c);
  ...
  public static final class SchemaMatch extends Criteria {
    ...
    @Override
    public boolean matches(LWComponent c) {
  ...
  public static final class SmartSearch /*extends Crtieria*/{
    ...
    final Multimap<String, Criteria> criteriaByKey =
    ...
    public List<LWComponent> search(final Collection<LWComponent> searchSet) {
      ...
      if (DEBUG.Enabled) {
        for (Criteria c : globalBooleanOr) Log.debug("GlobalOR: " + c);

        final Collection<
          Map.Entry<String, Collection<Criteria>>
        > allKeyEntries = criteriaByKey.asMap().entrySet();

        for (Map.Entry e : allKeyEntries) {
          Log.debug(
            String.format("Key: %-12s criteria=%s", e.getKey(), e.getValue())
          );
        }
      }
      ...
      if (criteriaByKey.size() > 0) keyBasedCriteria =
        criteriaByKey.asMap().values(); else keyBasedCriteria = null;

      if (globalBooleanOr.size() > 0) {
        // search method 1: includes the global OR tests
        for (LWComponent c : searchSet) {
          if (anyCriteriaMatches(c, globalBooleanOr)) hits.add(c); else if (
            keyBasedCriteria != null && allGroupsMatch(c, keyBasedCriteria)
          ) hits.add(c);
        }
    ...
    private boolean allGroupsMatch(
      final LWComponent c,
    ...
    private boolean anyCriteriaMatches(
      final LWComponent c,
      final Collection<Criteria> booleanOrGroup
    ) {
      for (Criteria criteria : booleanOrGroup) {
        if (criteria.matches(c)) {
    ...
    @Override
    public String toString() {
      final Collection<Collection<Criteria>> groupedByKey = criteriaByKey
        .asMap()
        .values();
  ...
  private void kickAnnotate() {
    ...
    if (DEBUG.THREAD || DEBUG.ANNOTATE) debug(
      "NOTIFIED ANNOTATION THREAD " + mAnnotateThread
    );
  }
  ...
  private boolean annotateForMap(final LWMap map) {
    if (DEBUG.THREAD || DEBUG.SCHEMA || DEBUG.ANNOTATE) Log.debug(
      "ANNOTATING against " + map + "; " + Util.tags(ActiveMapDataNodes)
    );
    ...
    if (map != null) {
      ...
      for (TreeNode n : mRootNode.getChildren()) {
        ...
        n2.annotate(map);
        if (!n.isLeaf()) for (TreeNode cn : n2.getChildren()) {
    ...
    for (TreeNode n : mAllRowsNode.getChildren()) {
      DataNode n2 = (DataNode) n;
      if (!n2.isMapPresent()) _newRowCount++;
      if (n2.isContextChanged()) _changedRowCount++;
    }

    final int newRowCount = _newRowCount;
    final int changedRowCount = _changedRowCount;

    if (DEBUG.THREAD || DEBUG.SCHEMA) Log.debug(
      "annotateForMap: newRows " +
      newRowCount +
      "; changedRows " +
      changedRowCount
    );
    ...
    GUI.invokeOnEDT(
      new Runnable() {
        public void run() {
          String newRowsMessage = "", changedRowsMessage = "";

          newRowsMessage =
            (newRowCount == 0
                ? VueResources.getString(
                  "dockWindow.contentPanel.sync.noNewRecords"
                )
                : (newRowCount == 1
                    ? VueResources.getString(
                      "dockWindow.contentPanel.sync.oneNewRecord"
                    )
                    : String.format(
                      VueResources.getString(
                        "dockWindow.contentPanel.sync.manyNewRecords"
                      ),
                      newRowCount
                    )));
          changedRowsMessage =
            (changedRowCount == 0
                ? VueResources.getString(
                  "dockWindow.contentPanel.sync.noChangedRecords"
                )
                : (changedRowCount == 1
                    ? VueResources.getString(
                      "dockWindow.contentPanel.sync.oneChangedRecord"
                    )
                    : String.format(
                      VueResources.getString(
                        "dockWindow.contentPanel.sync.manyChangedRecords"
                      ),
                      changedRowCount
                    )));
          ...
          mNewRowsLabel.setForeground(
            newRowCount != 0 ? Color.BLACK : MEDIUM_DARK_GRAY
          );
          ...
          mChangedRowsLabel.setForeground(
            changedRowCount != 0 ? Color.BLACK : MEDIUM_DARK_GRAY
          );
  ...
  private void refreshAll() {
    ...
    if (DEBUG.THREAD) Log.debug(
      "REFRESHING " + Util.tags(mRootNode.getChildren())
    );
    refreshTopLevel();
    for (TreeNode n : mRootNode.getChildren()) if (
      !n.isLeaf()
    ) refreshAllChildren(n);
    if (DEBUG.THREAD) Log.debug(
      " REFRESHED " + Util.tags(mRootNode.getChildren())
    );
      ...
      if (n instanceof FieldNode) {
        ...
        if (f != null) {
          ...
          if (style != null && style.isVisible()) {
            ...
            if (DEBUG.Enabled) Log.debug(
              "EXPAND " + Util.tags(n) + " " + Util.tags(path)
            );
            GUI.invokeOnEDT(
              new Runnable() {
                public void run() {
  ...
  private void refreshAllChildren(TreeNode node) {
    ...
    for (int i = 0; i < childIndexes.length; i++) childIndexes[i] = i; // why there's isn't an API to do this automatically, i don't know...

    // using nodesChanged instead of mTreeModel.reload preserves the expanded state of nodes in the tree
    if (DEBUG.META) Log.debug(
      "refreshing " + childIndexes.length + " children of " + node
    );
  ...
  private DataTree(final Schema schema) {
    ...
    mAnnotateThread =
      new Thread(String.format("Annotate%d: %-20.20s", ac, schema.getName())) {
        {
        ...
        public synchronized void run() {
          while (true) {
            try {
              ...
              if (DEBUG.THREAD || DEBUG.ANNOTATE) Log.debug(
                "annotation thread sleeping, pri=" + getPriority()
              );
              wait();
            ...
            if (
              DataTree.this == ForegroundTree
            ) setPriority(NORM_PRIORITY - 1); else setPriority(MIN_PRIORITY); // referring to NORM_PRIORITY can fail in Applets
            if (DEBUG.THREAD || DEBUG.ANNOTATE) Log.debug(
              "annotation thread woke, pri=" + getPriority() + "; running..."
            );
            final boolean interrupted = annotateForMap();
            if (DEBUG.Enabled) {
              if (interrupted) Log.debug("annotation aborted"); else if (
                DEBUG.THREAD
    ...
    if (DEBUG.THREAD) Log.debug(
      "STARTING " + mAnnotateThread + "; (tree constructing)"
    );
    ...
    addTreeSelectionListener(
      new javax.swing.event.TreeSelectionListener() {
        public void valueChanged(javax.swing.event.TreeSelectionEvent e) {
          ...
          if (DEBUG.Enabled) Log.debug(
            "valueChanged: isAddedPath=" + e.isAddedPath() + "; PATHS:"
          );
          ...
          final DataNode treeNode = (DataNode) e
            .getPath()
            .getLastPathComponent();
          if (treeNode instanceof RowNode) {
            VUE.setActive(
              tufts.vue.MetaMap.class,
              DataTree.this,
              treeNode.getRow().getData()
            );
          } else if (
            treeNode instanceof ValueNode &&
            treeNode.getField().isPossibleKeyField()
          ) {
            DataRow row = mSchema.findRow(
              treeNode.getField(),
              treeNode.getValue()
            );
            if (row != null) {
              ...
              Log.warn(
                "Row is NULL while trying to set active in tree selection value changed"
              );
            }
          }
          ...
          //                         }
            ...
            if (paths.length == 1) mSelectedSearchNode =
              node; else mSelectedSearchNode = null;
          } else {
            if (DEBUG.Enabled) Log.warn(
              "null search path from selection model"
            );
          }
  ...
  public void removeNotify() {
    if (ForegroundTree == this) ForegroundTree = null;
    super.removeNotify();
  }
  ...
  private static int sortPriority(Field f) {
    if (f.isSingleton()) return -4; else if (
      f.isSingleValue()
    ) return -3; else if (f.isUntrackedValue()) return -2; else if (
      f.getName().contains(":") && !f.getName().startsWith("dc:")
  ...
  private TreeNode buildTree(final Schema schema) {
    ...
    final DataNode root = new DataNode("Data Set: " + schema.getName());
    ...
    if (labelField == null) labelField = keyField;
    ...
    if (
      !DataTree.this.mSchema.isMatrixDataSet
  ...
  private static void buildValueChildren(Field field, DataNode fieldNode) {
    ...
    if (field.isQuantile() || (SORT_BY_COUNT && field.isPossibleKeyField())) {
      ...
      final ArrayList<Multiset.Entry<String>> sortedValues = new ArrayList(
        entrySet
      );
      ...
      Collections.sort(
        ...
        new Comparator<Multiset.Entry<String>>() {
          public int compare(
            final Multiset.Entry<String> e1,
            final Multiset.Entry<String> e2
          ) {
            // always put any empty value item last, otherwise sort on frequency
            if (e1.getElement() == Field.EMPTY_VALUE) return 1; else if (
              e2.getElement() == Field.EMPTY_VALUE
            ) return -1; else if (SORT_BY_COUNT) return (
              e2.getCount() - e1.getCount()
            ); else return tufts.Strings.compareNaturalIgnoreCaseAscii( // SORT_BY_VALUE
              e1.getElement(),
              e2.getElement()
            );
          }
    ...
    for (Multiset.Entry<String> e : valueEntries) {
      ...
      if (field.isPossibleKeyField()) {
        ...
        final String countTxt = String
          .format("%3d", nValues)
          .replaceAll(" ", "&nbsp;");

        final String color;

        if (nValues <= 0) display =
          String.format(
            HTML("<font color=#AAAAAA><code>%s</code> %s"),
            countTxt,
            valueText(value)
          ); else display =
          String.format(
            HTML("<code><font color=#888888>%s</font></code> %s"),
            countTxt,
            valueText(value)
          );
      }
  ...
  public void dragGestureRecognized(DragGestureEvent e) {
    ...
    Log.debug(
      "SELECTED: " + Util.tags(getSelectionPath().getLastPathComponent())
    );
    final DataNode treeNode = (DataNode) getSelectionPath()
      .getLastPathComponent();
    //if (resource != null)
    ...
    if (treeNode.isValue()) {
      if (treeNode.getCount() <= 0) dragNode = null; else dragNode =
      ...
      final List<LWComponent> nodes = DataAction.makeSingleRowNode(
        treeNode.getSchema(),
        row
      );
    ...
    dragNode.setClientData(
      tufts.vue.MapDropTarget.DropHandler.class,
  ...
  private void sendToMap(final DataNode treeNode, final LWMap map) {
    if (map == null || treeNode == null) return;
    Log.debug("SENDING TO MAP: " + treeNode);
  }

  private static final Ordering<Multiset.Entry> ByDecreasingFrequency =
  ...
  private void addMissingRowsToMap(final LWMap map) {
    ...
    final List<LWComponent> newRowNodes = DataAction.makeRowNodes(
      mSchema,
      newRows
    );
    ...
    try {
      ...
      Log.error(
        "problem creating links on " +
        map +
        " for new nodes: " +
        Util.tags(newRowNodes),
        t
      );
    }
  ...
  private static String makeFieldLabel(final Field field) {
    if (field.isQuantile()) return HTML(field.getName());
    ...
    if (values.size() == 0) {
      if (field.getMaxValueLength() == 0) {
        ...
        label =
          String.format(
            HTML("%s (max size: %d bytes)"),
            field.getName(),
            field.getMaxValueLength()
          );
      }
    } else if (values.size() == 1) {
      label =
        String.format(
          HTML("%s: <font color=green>%s"),
          field.getName(),
          field.getValues().toArray()[0]
        );
    } else if (values.size() > 1) {
  ...
  private static class FieldNode extends DataNode {
    ...
    FieldNode(Field field, LWComponent.Listener repainter, String description) {
      ...
      if (description == null) {
        if (field != null) setDisplay(makeFieldLabel(field));
      } else setDisplay(description);
  ...
  private static final class ValueNode extends FieldNode {
    ...
    void annotate(LWMap map) {
      ...
      if (mapCount > 0) {
        isMapPresent = true;
        if (mapCount != dataSetCount) setPostfix(
          String.format("[%+d]", mapCount - dataSetCount)
        ); else setPostfix(null);
        //setPrefix("=");
      } else {
        isMapPresent = false;
        setPostfix(null);
  ...
  final class AllRowsNode extends FieldNode {
    ...
    AllRowsNode(Schema schema, Object _repainter_ignored) {
      ...
      this.schema = schema;
      if (schema.getRowNodeStyle() == null) schema.setRowNodeStyle(
        DataAction.makeStyleNode(schema)
      );
      schema
        .getRowNodeStyle()
        .addLWCListener(
    ...
    private void updateLabel(boolean refresh) {
      ...
      if (refresh) DataTree.this.refreshRootNode();
    }
  ...
  private static final Border TopBorderCollapsed = new CompoundBorder(
    new CompoundBorder(
  ...
  private static final Border LeafBorder = GUI.makeSpace(
    0,
    IconWidth - 16,
    2,
    0
  ...
  private static final Icon RowHasChangedIcon = makeIcon(
    0x229B,
    RIPS,
    Color.green.darker(),
    -2,
  ...
  private static final Icon RowOnMapIcon = makeIcon(
    0x229B,
    RIPS,
  ...
  private static final Icon RowOffMapIcon = makeIcon(
    0x229B,
    RIPS,
    Color.lightGray,
    -2,
  ...
  private static final Icon ValueOnMapIcon = makeIcon(
    0x25C9,
    12,
    VueConstants.COLOR_SELECTION,
    0,
  ...
  private static final Icon ValueOffMapIcon = makeIcon(
    0x25C9,
    12,
    Color.lightGray,
    0,
  ...
  private static final Icon UniqueValueOnMapIcon = makeIcon(
    0x229A,
    16,
    VueConstants.COLOR_SELECTION,
    0,
  ...
  private static final Icon UniqueValueOffMapIcon = makeIcon(
    0x229A,
    16,
    Color.lightGray,
    0,
  ...
  private static final Icon TestIcon = VueResources.getImageIcon(
    "dataSourceRSS"
  );
  ...
  private class DataRenderer extends DefaultTreeCellRenderer {
    {
      ...
      setBackgroundSelectionColor(
        VueResources.getColor("dataTree.selected.background", Color.blue)
      );
    ...
    public Component getTreeCellRendererComponent(
      ...
      if (treeNode.hasStyle()) {
        ...
        if (field != null && field.isSingleton()) {
          ...
          if (treeNode.isRow()) {
            if (treeNode.isContextChanged()) setIcon(
              RowHasChangedIcon
            ); else if (treeNode.isMapPresent()) setIcon(
              RowOnMapIcon
            ); else setIcon(RowOffMapIcon);
          } else {
            if (field != null && field.isPossibleKeyField()) {
              if (treeNode.isMapPresent()) setIcon(
                UniqueValueOnMapIcon
              ); else setIcon(UniqueValueOffMapIcon);
              //                         } else if (field != null && field.isQuantile() && treeNode.getCount() <= 0) {
              //                             setIcon(null);
            } else {
              if (treeNode.isMapPresent()) setIcon(
                ValueOnMapIcon
              ); else setIcon(ValueOffMapIcon);
            }
          }
        }
      }
      ...
      if (row == 0) {
        if (expanded) setBorder(TopBorderExpanded); else setBorder(
          TopBorderCollapsed
        );
        //setBorder(TopBorder);
        ...
        if (leaf) {
          if (
            treeNode.isField() && treeNode.getField().isSingleton()
          ) setBorder(TopTierBorder); else setBorder(LeafBorder);
        } else {
          setBorder(null);
        }
      }
  ...
  private static final Icon EmptyIcon = new GUI.EmptyIcon(
    IconWidth,
    IconHeight
  );
