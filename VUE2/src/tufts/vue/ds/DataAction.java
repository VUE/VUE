...
public final class DataAction {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DataAction.class);

  public static final Object ClusterTimeKey =
    new tufts.vue.LWComponent.PersistClientDataKey("clusterTime");
  ...
  public static void centroidCluster(
    ...
    final Collection<LWComponent> pushable = map.getTopLevelItems(
      tufts.vue.LWComponent.ChildKind.EDITABLE
    );
    final Collection<LWComponent> toPush = new ArrayList(
      pushable.size() + nodes.size()
    );
    toPush.addAll(nodes); // also push other new nodes (they're not on the map yet)
    for (LWComponent c : map.getTopLevelItems(
      tufts.vue.LWComponent.ChildKind.EDITABLE
    )) {
      if (c instanceof tufts.vue.LWLink); // performance filter
      else toPush.add(c);
    }
    ...
    for (LWComponent node : nodes) {
      if (moveToCentroid(node, node.getLinked())) { // could also use getClustered(), which will leave out any count links
        ...
        if (deformMap) pushers.add(node);
      } else {
        unpositioned.add(node);
      }
    }

    if (unpositioned.size() > 0) tufts.vue.LayoutAction.filledCircle.act(
      unpositioned,
      DataDropHandler.AUTO_FIT
    );
  ...
  public static boolean moveToCentroid(
    LWComponent mover,
    Collection<LWComponent> related
  ) {
    if (related == null || related.isEmpty()) {
      ...
      mover.setCenterAt(
        onTopOf.getMapCenterX() + (2 - Math.random() * 4),
        onTopOf.getMapCenterY() - (mover.getHeight() + 12)
      );
      return true;
    } else {
      java.awt.geom.Point2D.Float centroid = tufts.vue.VueUtil.computeCentroid(
        related
      );
      if (centroid != null) {
  ...
  private static List<LWLink> makeDataLinksForNode(
    ...
    if (linkTargets.size() > 0) {
      return makeLinks(
        ...
        targetsUsed
      ); // seems wrong to be providing this last argumen
    } else {
      return Collections.EMPTY_LIST;
    }
  }
  ...
  public static Object[] addDataLinksForNodes(
    ...
    if (DEBUG.Enabled) Log.debug(
      "addDataLinksForNodes; field=" + quoteKey(field)
    );

    final Multiset<LWComponent> targetsUsed =
      com.google.common.collect.HashMultiset.create();
    final List<LWLink> links = makeDataLinksForNodes(
      map,
      nodes,
      field,
      targetsUsed
  ...
  private static List<LWLink> makeLinks(
    ...
    if (field == null) return makeRowNodeLinks(
      linkTargets,
      node,
      targetsUsed
    ); else return makeValueNodeLinks(linkTargets, node, field, targetsUsed);
  }

  public static List<LWComponent> makeRelatedNodes(
    Field dragField,
    LWComponent dropTarget
  ) {
    if (DEBUG.Enabled) Log.debug(
      "makeRelatedNodes: " + quoteKey(dragField) + "; target=" + dropTarget
    );
   ...
   */
  static List<LWComponent> makeRelatedValueNodes(
    Field dragField,
    MetaMap onMapRowData
  ) {
    if (DEBUG.Enabled) Log.debug(
      "makeRelatedValueNodes: " + quoteKey(dragField) + "; row=" + onMapRowData
    );
    ...
    //             nodes.add(newNode);
    //         }
    for (Relation join : Relation.getCrossSchemaJoinedValues(
      dragField,
      onMapRowData,
      ALL_VALUES
    )) {
  ...
  private static List<LWLink> makeValueNodeLinks(
    ...
    if (DEBUG.SCHEMA || DEBUG.WORK) {
      Log.debug(
        ...
        node +
        "\n\ttargets: " +
        Util.tags(linkTargets)
      );

      if (node.getDataValueField() != field) Util.printStackTrace(
        "field mis-match: nodeField=" +
        node.getDataValueField() +
        "; argField=" +
        field +
        "; node=" +
    ...
    for (LWComponent target : linkTargets) {
      ...
      try {
        ...
        if (target.hasDataValue(fieldName, fieldValue)) {
          ...
          links.add(
            makeLink(
              ...
              fieldName,
              fieldValue,
              sameField ? Color.red : null
            )
          );
          if (targetsUsed != null) targetsUsed.add(target);
        }

        final Relation relation = Relation.getCrossSchemaRelation(
          field,
          target.getRawData(),
          fieldValue
        );
        ...
        Log.error(
          "exception scanning for links from " + field + " to " + target + ":",
          t
        );
      }
    }
  ...
  private static List<LWLink> makeRowNodeLinks(
    ...
    if (!rowNode.isDataRowNode()) Log.warn(
      "making row links to non-row node: " + rowNode,
      new Throwable("FYI")
    );
    ...
    if (DEBUG.Enabled) {
      String targets;
      if (linkTargets.size() == 1) targets =
        Util.getFirst(linkTargets).toString(); else targets =
        Util.tags(linkTargets);
      Log.debug(
        "makeRowNodeLinks: " +
        rowNode +
    ...
    for (LWComponent target : linkTargets) {
      if (
        target == rowNode
      ) continue; // never link to ourself
      ...
      try {
        ...
        if (singleValueField != null) {
          ...
          final List<LWLink> valueLinks = makeValueNodeLinks(
            singletonTargetList,
            target,
            singleValueField,
            null
          ); // do NOT accrue reverse targets!
          //= makeValueNodeLinks(singletonTargetList, target, singleValueField, targetsUsed);
          if (valueLinks.size() > 0) {
            targetsUsed.add(target);
            if (valueLinks.size() > 1) Log.warn(
              "more than 1 link added for single value node: " +
              Util.tags(valueLinks),
              new Throwable("HERE")
            );
          }
  ...
  private static LWLink makeLink(
    ...
    if (r.type == Relation.AUTOMATIC) {
      ...
      //if (true) return null;
      color = Color.lightGray;
      if (r.count == 1) link.setStrokeWidth(0.3f); else link.setStrokeWidth(
        (float) Math.log(r.count)
      );
      link.setTextColor(color);
      link.setLabel(String.format(" %d ", r.getCount()));
    } else if (r.type == Relation.JOIN) {
      color = Color.orange;
    } else color = Color.magenta; // unknown type!
  ...
  private static LWLink makeLink(
    ...
    if (fieldName == null) relationship = fieldValue; else relationship =
      String.format(
        "%s=%s",
        fieldName,
  ...
  private static final int DataNodeLabelLength = VueResources.getInt(
  ...
  public static LWComponent makeValueNode(Field field, String value) {
    ...
    if (field.hasStyleNode()) node.setStyle(field.getStyleNode());
  ...
  public static List<LWComponent> makeSingleRowNode(
    Schema schema,
    DataRow singleRow
  ) {
  ...
  public static List<LWComponent> makeRelatedRowNodes(
    Schema schema,
    LWComponent dropTargetFilter
  ) {
  ...
  public static List<LWComponent> makeRowNodes(
    ...
    if (rows.isEmpty()) return Collections.EMPTY_LIST;

    if (schema.getRowNodeStyle() == null) {
      schema.setRowNodeStyle(makeStyleNode(schema));
      Log.info(
        "auto-applied row-style to " + schema + ": " + schema.getRowNodeStyle()
      );
    }
    ...
    if (mediaDescription == null) mediaDescription =
      schema.findField("media:content.media:description");
      ...
      try {
        ...
        if (link != null) {
          ...
          if (mediaField != null) {
            ...
            if (DEBUG.WORK) Log.debug(
              "attempting to set thumbnail " + Util.tags(media)
            );
        ...
        if (imageField != null) {
          ...
          if (Resource.looksLikeURLorFile(image)) {
            if (
              !image.equals("n/a")
            ) IR = Resource.instance(image); // tmp hack for one of our old test data sets
          }
        }

        if (IR == null && mediaField != null) {
          String media = row.getValue(mediaField);
          if (Resource.looksLikeURLorFile(media)) IR = Resource.instance(media);
        }

        if (IR != null) {
          String mediaDesc = null;

          if (mediaDescription != null) mediaDesc =
            row.getValue(mediaDescription);

          if (mediaDesc == null && titleField != null) mediaDesc =
            row.getValue(titleField);
    ...
    Log.debug(
      "makeRowNodes: PRODUCED NODE(S) FOR " + schema + "; count=" + nodes.size()
    );

    return nodes;
  }

  private static final Color DataNodeColor = VueResources.getColor(
    "node.dataRow.color",
    Color.gray
  );
  private static final float DataNodeStrokeWidth = VueResources.getInt(
    "node.dataRow.stroke.width",
    0
  );
  private static final Color DataNodeStrokeColor = VueResources.getColor(
    "node.dataRow.stroke.color",
    Color.black
  );
  private static final Font DataNodeFont = VueResources.getFont(
    "node.dataRow.font"
  );

  private static final Color ValueNodeTextColor = VueResources.getColor(
    "node.dataValue.text.color",
    Color.black
  );
  private static final Font ValueNodeFont = VueResources.getFont(
    "node.dataValue.font"
  );
  private static final Color[] ValueNodeDataColors = VueResources.getColorArray(
    "node.dataValue.color.cycle"
  );
  ...
  public static LWComponent makeStyleNode(Schema schema) {
    ...
    if (
      schema.getRowCount() <= 42 &&
      (schema.hasField("title") || schema.hasField("Title"))
    ) {
    ...
    String notes = String.format(
      "Style for row nodes in Schema '%s'",
      schema.getName()
    );
  ...
  public static LWComponent makeStyleNode(final Field field) {
    ...
    if (field.isPossibleKeyField()) {
      ...
      if (++NextColor >= ValueNodeDataColors.length) NextColor = 0;
      //             NextColor += 8;
