...
class ResizeControl implements LWSelection.ControlListener, VueConstants {
  ...
  ResizeControl() {
    for (int i = 0; i < handles.length; i++) handles[i] =
      new LWSelection.Controller();
  }
  ...
  private boolean isTopCtrl(int i) {
    return i == 0 || i == 1 || i == 2;
  }

  private boolean isLeftCtrl(int i) {
    return i == 0 || i == 6 || i == 7;
  }

  private boolean isRightCtrl(int i) {
    return i == 2 || i == 3 || i == 4;
  }

  private boolean isBottomCtrl(int i) {
    return i == 4 || i == 5 || i == 6;
  }

  /** this ctrl point can effect only the size, never the location */
  private boolean isSizeOnlyCtrl(int i) {
    return i >= 3 && i <= 5;
  }
  ...
  public void controlPointPressed(int index, MapMouseEvent e) {
    if (DEBUG.LAYOUT || DEBUG.MOUSE) out(
      "resize control point " + index + " pressed"
    );
    ...
    if (LOCAL_RESIZE) {
      if (!selection.allHaveSameParent()) Util.printStackTrace(
        this +
        "; selection includes multiple parentage -- unpredicatable results"
      );
    ...
    if (mOriginalGroup_bounds == null) {
      ...
      System.err.println(
        this + " control point pressed with empty selection " + selection
      );
    ...
    for (LWComponent c : selection) {
      ...
      if (c instanceof LWLink) {
        ...
        mOriginal_each_bounds[idx] =
          ((LWLink) c).getControlsWithCurrentDragEffect().clone();
        ...
        if (LOCAL_RESIZE) mOriginal_each_bounds[idx] =
          c.getLocalBounds(); else mOriginal_each_bounds[idx] = c.getBounds();
  ...
  void drawDebug(DrawContext dc) { // this only called if viewer or layout debug is on
    if (mNewDraggedBounds != null) {
      ...
      if (LOCAL_RESIZE) {
        Rectangle2D localOriginal = mLocalParent.transformZeroToMapRect(
          mOriginalGroup_bounds,
          null
        );
        ...
        Rectangle2D localNew = mLocalParent.transformZeroToMapRect(
          mNewDraggedBounds,
          null
        );
  ...
  public void controlPointMoved(int i, MapMouseEvent e) {
    ...
    if (VUE.getSelection().size() == 1) {
      ...
      if (c.supportsUserResize()) dragReshape(i, c, mNewDraggedBounds, e);
    } else {
      final double scaleX =
        mNewDraggedBounds.width / mOriginalGroup_bounds.width;
      final double scaleY =
        mNewDraggedBounds.height / mOriginalGroup_bounds.height;

      dragReshapeSelection(
        i,
        VUE.getSelection(),
        scaleX,
        scaleY,
        e.isAltDown()
      ); // resize if ALT is down, otherwise reposition only
    }
  }
   ...
   */
  private void dragReshape(
    final int controlPoint,
    final LWComponent c,
    Rectangle2D.Float request,
    MapMouseEvent e
  ) {
    ...
    if (
      ...
      c.getParent() instanceof LWSlide &&
      !e.isShiftDown()
    ) {
      // TODO: fix
    ...
    if (MAPPED_RESIZE) {
      ...
      request =
        (Rectangle2D.Float) c.getParent().transformMapToZeroRect(request);
    ...
    if (lockedLocation || isSizeOnlyCtrl(controlPoint)) {
      ...
      if (moved) {
        if (LOCAL_RESIZE == false) {
          if (DEBUG.WORK) System.out.format(
            "RC: new absolute loc: %6.1f,%-6.1f; %s\n",
            newX,
            newY,
            c
          );
          ...
          c
            .getParent()
            .transformMapToZeroPoint(new Point2D.Float(newX, newY), p);
          newX = p.x;
          newY = p.y;
          //newX -= c.getParent().getMapX();
          //newY -= c.getParent().getMapY();
          if (DEBUG.WORK) System.out.format(
            "RC: new relative loc: %6.1f,%-6.1f; %s\n",
            newX,
            newY,
            c
          );
  ...
  // dragReshape above.
  private void dragReshapeSelection(
    final int cpi,
    ...
    for (LWComponent c : selection) {
      if (
        c.isManagedLocation()
      ) continue; // must match conditinal aboice where we collect mOriginal_each_bounds[] -- OVERKILL, allow reshaping of child (managed loc) objects
      if (
        false && c.getParent().isSelected()
      ) continue; // skip if our parent also being resized -- race conditions possible -- todo: deeper nesting???
      ...
      if (c instanceof LWLink) {
        ...
        for (Point2D.Float originalPoint : ((Point2D.Float[]) mOriginal_each_bounds[idx++])) {
          controlIndex++;
          if (originalPoint == null) continue;
      ...
      Rectangle2D.Float c_original_bounds =
        (Rectangle2D.Float) mOriginal_each_bounds[idx++];
      ...
      if (true) { // if "reposition allowed"
        ...
        if (reshapeObjects) {
          ...
          c_new_x =
            mNewDraggedBounds.x +
            (c_original_bounds.x - mOriginalGroup_bounds.x) * scaleX;
          c_new_y =
            mNewDraggedBounds.y +
            (c_original_bounds.y - mOriginalGroup_bounds.y) * scaleY;
        } else {
        ...
        if (reshapeObjects) {
          if (isLeftCtrl(cpi)) {
            float c_width = resized
              ? c_new_width * c.getMapScaleF()
              : c.getWidth();
            if (c_new_x + c_width > resize_box.lr.x) c_new_x =
              (float) resize_box.lr.x - c_width;
          }
          if (isTopCtrl(cpi)) {
            float c_height = resized
              ? c_new_height * c.getMapScaleF()
              : c.getHeight();
            if (c_new_y + c_height > resize_box.lr.y) c_new_y =
              (float) resize_box.lr.y - c_height;
          }
        }
        repositioned = true;
      }
      ...
      if (repositioned) {
        ...
        if (DEBUG.WORK) System.out.format(
          "new absolute loc: %6.1f,%6.1f for %s\n",
          c_new_x,
          c_new_y,
          c
        );
        if (c.atTopLevel()) {
          ...
          if (DEBUG.WORK) System.out.format(
            "new relative loc: %6.1f,%6.1f for %s\n",
            c_new_x,
            c_new_y,
            c
          );
      ...
      if (resized && repositioned) {
        ...
        if (centerPoint != null) c.setCenterAt(
          centerPoint
        ); else c.userSetLocation(c_new_x, c_new_y);
      } else throw new IllegalStateException("Unhandled dragResizeReshape");
    }
  }
  ...
  private Point2D.Float translatePoint(
    Point2D.Float originalPoint,
