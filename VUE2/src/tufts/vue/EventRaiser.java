...
public abstract class EventRaiser<T> {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(EventRaiser.class);
  ...
  private void traverseFrames() {
    ...
    if (DEBUG.EVENTS && DEBUG.META) out(
      "FRAMES: " + java.util.Arrays.asList(frames)
    );
  ...
  protected void dispatchSafely(Component target) {
    if (DEBUG.EVENTS) out(
      "DISPATCHING " + this + " to " + tufts.vue.gui.GUI.name(target)
    ...
    try {
      ...
      Util.printStackTrace(
        e,
        EventRaiser.class.getName() +
        " exception during dispatch: " +
        "\n\t  event source: " +
        source +
        " (" +
        Util.objectTag(source) +
        ")" +
        "\n\t  event raised: " +
        this +
        "\n\t   targetClass: " +
  ...
  protected void traverseContainer(Container parent) {
    ...
    if (parent.getComponentCount() > 0) traverseChildren(
  ...
  protected void traverseChildren(Component[] children) {
    ...
    for (int i = 0; i < children.length; i++) {
      if (DEBUG.EVENTS && DEBUG.META && DEBUG.FOCUS) eoutln(
        getType(children[i]) + i + " " + GUI.name(children[i])
      );
      if (children[i] instanceof Container) traverseContainer(
        (Container) children[i]
      ); else visit(children[i]);
    }
  ...
  private static String getType(Component c) {
    if (c instanceof Frame) return "FRAME";
    if (c instanceof Window) return "owned";
    return "child";
  }
