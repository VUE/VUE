...
public class LWText extends LWComponent {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWText.class);
  ...
  public String getRichText() {
    if (richLabelBox == null) {
      ...
      return "";
    } else return richLabelBox.getRichText();
  }
  ...
  public void copyStyle(LWComponent styleSource) {
    ...
    if (styleSource == null) return; else this.getRichLabelBox()
      .copyStyle(styleSource);
  }
  ...
  public RichTextBox getRichLabelBox(boolean overrideStyleSheet) {
    if (this.richLabelBox == null) {
      synchronized (this) {
        if (this.richLabelBox == null) this.richLabelBox =
          new RichTextBox(
            this,
            this.richLabel != null ? this.richLabel : this.label
          );
      }
    }
    ...
    if (
      ...
      String fontName = (String) ((LWSlide) VUE
          .getActivePathway()
          .getMasterSlide()).getMasterSlide()
        .getTextStyle()
        .getPropertyValue(LWKey.FontName);
      Integer fontSize = (Integer) ((LWSlide) VUE
          .getActivePathway()
          .getMasterSlide()).getMasterSlide()
        .getTextStyle()
        .getPropertyValue(LWKey.FontSize);
      VueStyleSheet ss =
        (VueStyleSheet) ((SHTMLDocument) richLabelBox.getDocument()).getStyleSheet();
      Color color =
      ...
      ss.addRule(
        "body {margin-top:0px;margin-bottom:0px;margin-left:0px;margin-right:0px;font-size:" +
        fontSize +
        ";font-family:" +
        fontName +
        ";color: " +
      ...
      ss.addRule(
        "ol { margin-top:6;font-family:" +
        fontName +
        ";vertical-align: middle;margin-left:30;font-size:" +
        fontSize +
        ";list-style-position:outside;}"
      );
      ss.addRule(
        "p  { margin-top:0;margin-left:0;margin-right:0;margin-bottom:0;color: " +
        colorString +
        ";}"
      );
      ss.addRule(
        "ul { margin-top:6;font-size:" +
        fontSize +
        ";margin-left:30;vertical-align: middle;list-style-position:outside;font-family:" +
        fontName +
        ";}"
      );
    } else if (VUE.getActiveViewer() != null && overrideStyleSheet) {
      FontEditorPanel fep = VUE
        .getFormattingPanel()
        .getTextPropsPane()
        .getFontEditorPanel();
    ...
    if (isSelected() && dc.isInteractive() && dc.focal != this) {
      ...
      if (p != null && p.isVisible() && p.getCurrentNode() == this) {
        ...
        if (isTransparent()) {
          dc.g.fill(
            grow(
              (Rectangle2D.Float) mZeroBounds.clone(),
  ...
  private static final Line2D.Float xline = new Line2D.Float();

  private void drawTextWithReducedLOD(
  ...
  private void drawLODTextLine(final DrawContext dc) {
    ...
    if (VUE.getActiveViewer() != null) zoomFactor =
      VUE.getActiveViewer().getZoomFactor(); else zoomFactor = 1;
    ...
    while (line < scaledHeight) {
      dc.g.drawLine(
        0,
        line,
        (int) (getLabelBox().getWidth() * zoomFactor),
        line
      );
      line += 100;
    }
  ...
  public float getWidth() {
    if (richLabelBox == null) return super.getWidth(); else {
      SHTMLDocument doc = (SHTMLDocument) richLabelBox.getDocument();

      if (
        doc != null && doc.isEditing()
      ) return (float) richLabelBox.getUnscaledWidth(); else return richLabelBox.getWidth();
    }
  }
  ...
  public float getHeight() {
    ...
    if (richLabelBox == null) {
      ...
      if (
        doc != null && doc.isEditing()
      ) return (float) richLabelBox.getUnscaledHeight(); else return richLabelBox.getHeight();
    }
  }

  @Override
  public float getLocalWidth() {
    return (float) (getWidth() * getScale());
  }

  @Override
  public float getLocalHeight() {
    return (float) (getHeight() * getScale());
  }

  @Override
  public float getMapWidth() {
    return (float) (getWidth() * getMapScale());
  }

  @Override
  public float getMapHeight() {
    return (float) (getHeight() * getMapScale());
  }

  @Override
  public float getLocalBorderWidth() {
    return (float) ((getWidth() + mStrokeWidth.get()) * getScale());
  }

  @Override
  public float getLocalBorderHeight() {
    return (float) ((getHeight() + mStrokeWidth.get()) * getScale());
  }
  ...
  public void setStyle(LWComponent parentStyle) {
    if (richLabelBox != null) richLabelBox.overrideTextColor(
  ...
  void setLabel0(String newLabel, boolean setDocument) {
    ...
    if (newLabel == null || newLabel.length() == 0) {
      this.label = null;
      if (richLabelBox != null) richLabelBox.setText("");
    } else {
      this.label = newLabel;
  ...
  private void setAutomaticAutoSized(boolean tv) {
    if (
      isOrphan()
    ) // auto-size computations // if this is during a restore, don't do any automatic
    return;
  ...
  protected void layout(Object triggerKey, Size curSize, Size request) {
    if (inLayout) {
      if (DEBUG.Enabled) {
        if (DEBUG.META || DEBUG.LAYOUT) new Throwable(
          "ALREADY IN LAYOUT " + this
        )
          .printStackTrace(); else Log.warn(
          "already in layout: " + Util.tag(this) + " id#" + getID()
        );
      }
      return;
    }
    ...
    if (
      DEBUG.LAYOUT &&
      getLabelBox().getHeight() != getLabelBox().getPreferredSize().height
    ) {
    ...
    if (richLabelBox != null) richLabelBox.setBoxLocation(0, 0);
    ...
    if (!isAutoSized()) notify(LWKey.Size, min); // todo perf: can we optimize this event out?
  ...
  public String toString() {
    ...
    if (richLabelBox == null) txt = "<null-RichTextBox>"; else {
      txt = richLabelBox.getText();
      txt = txt.replaceAll("\\s+", " ");
    }
