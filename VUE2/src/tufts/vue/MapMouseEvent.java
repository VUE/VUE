...
public class MapMouseEvent extends MouseEvent {
  ...
  private static final int COMMAND_MASK = VueUtil.isMacPlatform()
  ...
  public int getDeltaPressX() {
    return getX() - pressX;
  }

  public int getDeltaPressY() {
    return getY() - pressY;
  }

  public void setDragRequest(LWComponent c) {
    this.dragRequest = c;
  }

  public LWComponent getDragRequest() {
    return this.dragRequest;
  }
  ...
  public MapViewer getViewer() {
    ...
    if (
      src instanceof JViewport
    ) return (MapViewer) ((JViewport) src).getView(); else if (
      src instanceof JScrollPane
    ) return (MapViewer) ((JScrollPane) src).getViewport()
      .getView(); else return (MapViewer) src;
  }
  ...
  public Point2D.Float getFocalPoint() {
    if (getFocal() == getMap()) return getMapPoint(); else return getFocal()
      .transformMapToZeroPoint(new Point2D.Float(mapX, mapY));
  }
  ...
  public LWComponent getPicked() {
    ...
    } else if (DEBUG.PICK) out("picked is already: " + picked);
    return picked;
  }
