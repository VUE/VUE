...
public final class DrawContext {
    ...
    public void fillArea(final Shape shape, final Color fill) {
        if (fill == null || fill.equals(fillColor)) {
            if (DEBUG.PRESENT && DEBUG.META)
                Util.printStackTrace("skipping fillArea matching existing bg fill " + fill + " " + fmt(shape));
            return;
        }
        g.setColor(fill);
        g.fill(shape);
    ...
    public void setMasterClip(Shape clip) {
        ...
        if (clip instanceof Rectangle2D) {
            if (DEBUG.PAINT)
                out("SET MASTER CLIP RECT2D " + fmt(clip));
    ...
    public static final String[] AlphaRuleNames = {
            ...
            // 1997:
            "<none>",
            "CLEAR", "SRC", "SRC_OVER", "DST_OVER", "SRC_IN", "DST_IN", "SRC_OUT", "DST_OUT", "DST", "SRC_ATOP",
            "DST_ATOP", "XOR"
    };
    ...
    private static void setQuality(final Graphics g,
