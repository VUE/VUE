...
public final class DrawContext {
  ...
  public DrawContext(Graphics g, LWComponent focal) {
    ...
    // setBackgroundFill(focal.getRenderFillColor(null)); // caller should do
    // manually
  }
  ...
  public DrawContext push() {
    if (lastPush != null) Util.printStackTrace("Unpopped DC: " + lastPush);
  ...
  public void fillBackground(Color c) {
    if (fillColor != null) Util.printStackTrace(
      this + " already filled with " + fillColor
    );
    setBackgroundFill(c);
    g.setColor(c);
  ...
  /**
   * Fill the given shape will the given color: will be a noop if the fill color
   * matches the current BG fill
   */
  public void fillArea(final Shape shape, final Color fill) {
    if (fill == null || fill.equals(fillColor)) {
      if (DEBUG.PRESENT && DEBUG.META) Util.printStackTrace(
        "skipping fillArea matching existing bg fill " + fill + " " + fmt(shape)
      );
      return;
    }
    g.setColor(fill);
   ...
   * if (rawTransform == null)
   * throw new
   * IllegalStateException("attempt to revert to raw draw in a derivative DrawContext"
   * );
  ...
  public boolean isClipOptimized() {
    if (
      DEBUG.CONTAINMENT && DEBUG.META && DEBUG.WORK
    ) return false; else return isClipOptimized; // TODO: temporary weird case
  }
  ...
  public void setMasterClip(Shape clip) {
    ...
    if (clip instanceof Rectangle2D) {
      ...
      // we've set the shaped clip in the gc, now extract the master clip rectangle
      // from the gc
  ...
  public void setAlpha(double p_alpha, int alphaRule) {
    ...
    if (alpha == 1f) g.setComposite(AlphaComposite.Src); else g.setComposite(
      AlphaComposite.getInstance(alphaRule, this.alpha)
    );
  ...
  public static final String[] AlphaRuleNames = {
    // Index based on coded values in java.awt.AlphaComposite.java @version 10 Feb
    // 1997:
    "<none>",
    "CLEAR",
    "SRC",
  ...
  public void checkComposite(LWComponent c) {
    if (false && alpha != 1f) {
      ...
      if (c instanceof LWImage) {
        ...
        setAlpha(alpha, AlphaComposite.SRC_OVER);
        // System.out.println("COMPOSITE: SRC_OVER (has fill with partial transparency)
        // " + c);
      } else {
  ...
  public void setInteractiveQuality() {
    ...
    // even best quality under Java 1.6 appears to be crappy compared to best under
    // 1.5 on Mac OS X 10.5.8
  ...
  public void setPrintQuality() {
    ...
    // improve quality of printed text (high DPI's are needed to take advantage of
    // fractional metrics)
  ...
  public void setAbsoluteScale(double absScale) {
    ...
    if (adjScale > 0) g.scale(adjScale, adjScale);
  }
  ...
  public void setAbsoluteDrawing(boolean unZoom) {
    if (unZoom) g.scale(1 / zoom, 1 / zoom); else g.scale(zoom, zoom);
  }
  ...
  public String toString() {
    // return String.format("DrawContext@%x[zoom=%.2f mapOffset=%.1f,%.1f
    // focal=%s]",
    return String.format(
      "DrawContext@%06X[%.1f%% %s %s%s]",
      hashCode(),
      zoom * 100,
      // offsetX, offsetY,
      focal == null ? "null-focal" : focal.getUniqueComponentTypeLabel(),
      fmt(masterClipRect),
      isClipOptimized ? " CLIPPING" : " DRAW-ALL"
    );
  }
  ...
  private static void setQuality(
    ...
    if (on == DEFAULT) hint = defaultValue; else if (on) hint =
      qualityValue; else hint = speedValue;

    if (DEBUG.PAINT) {
      System.out.format(
        TERM_PURPLE + "%s %7s for %-41s = %s\n" + TERM_CLEAR,
        Util.tag(g),
  ...
  protected void out(String s) {
    System.err.println(Util.TERM_PURPLE + this + Util.TERM_CLEAR + " " + s);
  }

  private static void msg(String s) {
    System.err.println(Util.TERM_PURPLE + s + Util.TERM_CLEAR);
  }
  ...
  public DrawContext(DrawContext dc, LWComponent newFocal) {
    ...
    // This helps itext (tho is not a workaround) -- it looks like the GC provided
    // by
    // itext doesn't handle create properly -- it almost looks as if a fill is
    // required
    // every time a new GC is created for fill's in subsequent child fill's or even
    // text to work
    ...
    // g.fillRect(-Short.MAX_VALUE/2, -Short.MAX_VALUE/2, Short.MAX_VALUE,
    // Short.MAX_VALUE);
  ...
  private static final Multiset DebugRecording =
  ...
  public static void recordDebug(LWComponent c) {
    ...
    if (
      c.getTypeToken() != null && c.getTypeToken() != c.getClass()
    ) recordDebug(type + ":" + c.getTypeToken()); else recordDebug(type);
  }
  ...
  private static final boolean PlatformQualityIsSlow;

  public static boolean drawingMayBeSlow(final LWComponent focal) {
    // could check focal for the presence of images
    // if (true) return false;
    if (PlatformQualityIsSlow && isImageQualityRequested()) return (
      focal instanceof LWMap
    ); else return false;
  }
  ...
  static {
    if (
      Util.isMacLeopard() && Util.getJavaVersion() < 1.6f
    ) PlatformQualityIsSlow = true; // slow, but the only one truly effective!
    else PlatformQualityIsSlow = false;
