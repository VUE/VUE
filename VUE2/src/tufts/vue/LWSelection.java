...
public class LWSelection extends java.util.ArrayList<LWComponent> {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWSelection.class);
  ...
  private final Set<LWContainer> mParents = new java.util.HashSet() {
    ...
    public boolean add(Object parent) {
      if (parent != null) return super.add(parent); else return false;
    }
  };
  ...
  public List<LWComponent> contents() {
    if (mSecureList == null) mSecureList =
  ...
  private void removeControlListener(ControlListener listener) {
    ...
    if (!controlListeners.remove(listener)) throw new IllegalStateException(
      this + " didn't contain control listener " + listener
    );
  }
  ...
  private synchronized void notifyListeners() {
    ...
    if (notifyUnderway()) return;

    try {
      inNotify = true;

      if (DEBUG.SELECTION || DEBUG.EVENTS) {
        if (DEBUG.SELECTION) System.out.println(
          "\n-----------------------------------------------------------------------------"
        );
      ...
      for (int i = 0; i < nlistener; i++) {
        if (DEBUG.SELECTION && DEBUG.META) debug(
          "notifying: #" + (i + 1) + " " + (i < 9 ? " " : "")
        );
        ...
        try {
          if (DEBUG.SELECTION || DEBUG.PERF) {
            ...
            if (l instanceof java.awt.Component) System.err.format(
              "%70s...",
              tufts.vue.gui.GUI.name(l)
            ); else System.err.format("%70s...", Util.tags(l));
          ...
          System.err.println(
            this +
            " notifyListeners: exception during selection change notification:" +
            "\n\tselection: " +
            this +
            "\n\tfailing listener: " +
            l
  ...
  protected void postNotify() {
    if (size() == 1) VUE.setActive(
      LWComponent.class,
      this,
      first()
    ); else VUE.setActive(LWComponent.class, this, null);
  }
  ...
  private boolean notifyUnderway() {
    if (inNotify) {
      Util.printStackTrace(
        this +
        " attempt to change selection during selection change notification: denied. src=" +
        source
      );
      return true;
    } else {
  ...
  public void setWithStyle(
    Iterable bag,
    String description,
    LWComponent styleRecord
  ) {
  ...
  public synchronized boolean add(LWComponent c) {
    if (notifyUnderway()) return false;
    if (!c.isSelected()) {
  ...
  private synchronized boolean addImpl(Iterator<LWComponent> i) {
    ...
    while (i.hasNext()) {
      ...
      if (!c.isSelected() && c.isDrawn()) {
        if (addSilent(c)) changed = true;
      }
    }
    if (DEBUG.SELECTION || DEBUG.PERF) Log.debug(
      "addImpl: " + Util.tags(i) + "; completed"
    );
  ...
  private synchronized boolean addSilent(LWComponent c) {
    if (DEBUG.SELECTION && DEBUG.META/*|| DEBUG.EVENTS*/) debug(
      "addSilent " + c + " src=" + source
    );
    ...
    if (!isClone && size() > 0) { // clones can break the rules if they want
      ...
      if (!first().supportsMultiSelection()) clearSilent(); else if (
        !c.supportsMultiSelection()
    ...
    if (!c.isSelected()) {
      ...
      if (!isClone && c instanceof ControlListener) addControlListener(
        (ControlListener) c
      );
      return true;
    } else throw new RuntimeException(
      this + " attempt to add already selected component " + c
    );
  }
  ...
  public synchronized void clearHidden() {
    ...
    for (LWComponent c : toArray()) {
      try {
        if (
          c.isHidden() || c.isAncestorCollapsed() || c.getLayer().isHidden()
        ) {
  ...
  private synchronized void removeSilent(LWComponent c) {
    ...
    if (!isClone && c instanceof ControlListener) removeControlListener(
      (ControlListener) c
    );

    if (!super.remove(c)) throw new RuntimeException(
      this + " remove: list doesn't contain " + c
    );
  }
  ...
  public synchronized void clearAndNotify() {
    clearSilent();
    if (DEBUG.SELECTION) debug(
      "clearAndNotify: forced notification after clear"
    );
    notifyListeners();
  }

  public synchronized void clear() {
    if (clearSilent()) notifyListeners();
  }
  ...
  private synchronized boolean clearSilent() {
    ...
    if (!isClone) {
      for (LWComponent c : this) c.setSelected(false);
      lastSelection = clone();
    }

    if (controlListeners != null) controlListeners.clear();
  ...
  public void resetStatistics() {
    ...
    if (size() > 0) {
      ...
      for (LWComponent c : this) {
        ...
        mTypes.add(c.getClass());
        // todo: more efficient:
        if (c.isDataValueNode()) mDataValueCount++; else if (
          c.isDataRowNode()
        ) mDataRowCount++;
      }
    }
  ...
  public boolean containsType(Class clazz) {
    for (Class contentClass : getTypes()) if (
      clazz.isAssignableFrom(contentClass)
    ) return true;
  ...
  public boolean allHaveSameParentOfType(Class<? extends LWComponent> clazz) {
    return (
      size() > 0 && allHaveSameParent() && clazz.isInstance(first().getParent())
    );
  }

  public boolean allHaveTopLevelParent() {
    for (LWContainer parent : mParents) if (!parent.isTopLevel()) return false;
    return true;
  }
  ...
  public long getEditablePropertyBits() {
    if (mEditablePropertyKeys == 0 && !isEmpty()) {
      ...
      for (LWComponent c : this) mEditablePropertyKeys |=
      c.getSupportedPropertyBits();
    }
  ...
  public boolean hasEditableProperty(Object key) {
    if (key instanceof LWComponent.Key) return (
      (getEditablePropertyBits() & ((LWComponent.Key) key).bit) != 0
    ); else return true;
  }
