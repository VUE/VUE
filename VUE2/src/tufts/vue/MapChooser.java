...
public class MapChooser extends JDialog implements ActionListener {
  ...
  public static final String generateMessage = VueResources.getString(
    "dialog.connanalysis"
  );
  public static final String browseMessage = VueResources.getString(
    "dialog.browsmap"
  );
  ...
  public MapChooser(Frame owner, String title) {
    ...
    cancelButton =
      new JButton(VueResources.getString("dialog.connanalysis.cancel"));
    generateButton =
      new JButton(VueResources.getString("dialog.connanalysis.generate"));
    browseButton =
      new JButton(VueResources.getString("dialog.connanalysis.browse"));
  ...
  public void setUpLocationPanel() {
    ...
    String[] choices = {
      VueResources.getString("dialog.connanalysis.activemap"),
      VueResources.getString("dialog.connanalysis.mapinfolder"),
    };
        ...
        if (getSelectedMap() != null) {
          ...
          for (int ji = 0; ji < jmax; ji++) {
            for (int ki = 0; ki < nodeCount; ki++) {
              ...
              if (ki != (nodeCount - 1)) md += ",";
            }
            if (ji < jmax - 1) md += System.getProperty("line.separator");
          }
          ...
          if (md.length() > 0) {
            ...
            PrintWriter writer3 = new PrintWriter(
              new BufferedWriter(new FileWriter(fileName3))
            );
            writer3.write(md);
            writer3.close();
    ...
    if (e.getSource() == browseButton) {
      try {
        ...
        chooseFile.showDialog(
          this,
          VueResources.getString("dialog.select.title")
        );
