...
public abstract class Curve {
  ...
  public static void insertCubic(
    Vector curves,
    double x0,
    double y0,
    double coords[]
  ...
  public static int crossingsForLine(
    double x,
    double y,
    double x0,
    double y0,
  ...
  public static int crossingsForQuad(
    double x,
    double y,
    double x0,
    double y0,
  ...
  public static int crossingsForCubic(
    double x,
    double y,
    double x0,
    double y0,
    ...
    while (!pi.isDone()) {
      switch (pi.currentSegment(coords)) {
        case PathIterator.SEG_MOVETO:
          if (cury != movy) {
            cross += Curve.crossingsForLine(x, y, curx, cury, movx, movy);
          }
          movx = curx = coords[0];
          movy = cury = coords[1];
          break;
        case PathIterator.SEG_LINETO:
          newx = coords[0];
          newy = coords[1];
          cross += Curve.crossingsForLine(x, y, curx, cury, newx, newy);
          curx = newx;
          cury = newy;
          break;
        case PathIterator.SEG_QUADTO:
          newx = coords[2];
          newy = coords[3];
          cross += Curve.crossingsForQuad(x, y, curx, cury, coords);
          curx = newx;
          cury = newy;
          break;
        case PathIterator.SEG_CUBICTO:
          newx = coords[4];
          newy = coords[5];
          cross += Curve.crossingsForCubic(x, y, curx, cury, coords);
          curx = newx;
          cury = newy;
          break;
  ...
  public boolean findIntersect(
	...
	*/
    ...
    if (
      Math.min(xs0, xs1) > Math.max(xt0, xt1) ||
