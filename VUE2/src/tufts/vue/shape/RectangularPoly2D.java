...
public abstract class RectangularPoly2D extends RectangularShape {
  ...
  public void setSides(int sides) {
    if (sides < 3 || sides > 8) throw new IllegalArgumentException(
      "RectangularPoly2D: sides not >=3 && <=8: " + sides
    );
    this.sides = sides;
  ...
  private Crossings getCrossings(
    double xlo,
    double ylo,
    double xhi,
    double yhi
  ...
  class PolyIterator implements PathIterator {
    ...
    public int currentSegment(double[] coords) {
      ...
      if (index == 0) return SEG_MOVETO; else if (
        index == sides
      ) return SEG_CLOSE; else return SEG_LINETO;
    }
