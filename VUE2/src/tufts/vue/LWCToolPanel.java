...
public class LWCToolPanel
  ...
  public LWCToolPanel() {
    ...
    if (debug) setBorder(new LineBorder(Color.pink, 2)); else setBorder(
      new EmptyBorder(2, 1, 2, 1)
    ); //t,l,b,r

    if (debug) setBackground(Color.blue); else GUI.applyToolbarColor(this);
    setOpaque(false);
  ...
  private void loadAllEditors(LWSelection selection) {
    ...
    try {
      for (LWEditor editor : mEditors) {
        boolean supported = selection.hasEditableProperty(
        ...
        if (supported) loadEditor(propertySource, editor);
      }
    } finally {
  ...
  private void loadEditor(LWComponent source, LWEditor editor) {
    ...
    if (source.supportsProperty(key)) value =
      source.getPropertyValue(key); else value = null;
    if (value != null) {
      if (DEBUG.TOOL) out("loadEditor1 [" + value + "] -> " + editor);
      editor.displayValue(value);
    } else if (DEBUG.TOOL) out(
      "loadEditor1 " +
      source +
      " -> " +
      editor +
      " skipped; unsupported property " +
      key
    );
  }

  private void loadEditorsMatchingKey(
    final Object key,
    final LWComponent source
  ) {
    ...
    for (LWEditor editor : mEditors) {
      try {
        ...
        if (editor.getPropertyKey() == key && source.supportsProperty(key)) {
          if (DEBUG.TOOL) out(
            "loadEditorsMatchingKey: found producer for key [" +
            key +
            "]: " +
            editor.getClass()
          );
        ...
        tufts.Util.printStackTrace(
          new Throwable(t),
          "exception loading editor: " + editor
        );
      }
    }
    if (DEBUG.TOOL && DEBUG.META) {
      if (!loaded) System.out.println(
        this +
        " loadEditorsMatchingKey: FYI, no LWEditor for '" +
        key +
        "' in " +
        source
      );
  ...
  public void propertyChange(PropertyChangeEvent e) {
    ...
    ApplyPropertyChangeToSelection(
      VUE.getSelection(),
  ...
  private static void ApplyPropertyChangeToSelection(
    ...
    if (selection.isEmpty()) {
      ...
      try {
        for (tufts.vue.LWComponent c : selection) {
          if (c.supportsProperty(key)) c.setProperty(key, newValue);
        }
      } finally {
  ...
  public void stateChanged(ChangeEvent e) {
    ...
    for (LWEditor editor : mEditors) {
      if (source == editor) {
        if (true || DEBUG.TOOL) out(
          "matched ChangeEvent source to editor " + editor.getClass()
        );
        propertyChange(
          new LWPropertyChangeEvent(
            editor,
            editor.getPropertyKey(),
            editor.produceValue()
          )
...
*/
  ...
  public static void main(String[] args) {
    ...
    //DEBUG.BOXES = true;
    FontEditorPanel.sFontNames =
      new String[] { "Lucida Sans Typewriter", "Courier", "Arial" }; // so doesn't bother to load system fonts
