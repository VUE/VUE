...
public class LWTransfer implements Transferable {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWTransfer.class);
  ...
  public boolean isDataFlavorSupported(final DataFlavor flavor) {
    ...
    if (
      flavor == DropHandler.DataFlavor && LWC.hasClientData(DropHandler.class)
    ) {
      return true;
    }
    ...
    if (
      flavor == Resource.DataFlavor && LWC.getResource() == null
    ) return false;

    // TODO BUG: support for TypeList is being incorrectly reported as true here
    // even if there is no old-style meta-data!

    for (int i = 0; i < supportedFlavors.length; i++) if (
      flavor.equals(supportedFlavors[i])
    ) return true;
  ...
  public synchronized Object getTransferData(DataFlavor flavor)
    ...
    if (DEBUG.DND && DEBUG.META) System.err.print(
      "<LWTransfer.getTransferData(" + flavor.getHumanPresentableName() + ")>"
    );
    ...
    if (TypeList.DataFlavor.equals(flavor)) {
      ...
      String s = null;
      if (LWC instanceof LWMap && ((LWMap) LWC).getFile() != null) s =
        ((LWMap) LWC).getFile().getAbsolutePath();
      if (LWC.hasResource()) s = LWC.getResource().getSpec();
      if (s == null && LWC.hasLabel()) s = LWC.getLabel();
      if (s == null && LWC.hasNotes()) s = LWC.getNotes();
      if (s == null) s = LWC.getDisplayLabel();
      //if (s != null) s += "\n";
      data = s;
      ...
      if (isSelection) {
        duplicates =
          tufts.vue.Actions.duplicatePreservingLinks(LWC.getChildren());
      ...
      //             }
        ...
        LWC.hasFlag(LWComponent.Flag.INTERNAL)
        /*&& LWC.getClientProperty(Field.class) != null*/
      ) {
