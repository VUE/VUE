...
public class LWNode extends LWContainer {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWNode.class);

  public static final Object TYPE_TEXT = "textNode";

  static final boolean WrapText = false; // under development

  // todo: all this stuff should be in a factory some day
  public static final Font DEFAULT_NODE_FONT = VueResources.getFont(
    "node.font"
  );
  public static final Color DEFAULT_NODE_FILL = VueResources.getColor(
    "node.fillColor"
  );
  public static final int DEFAULT_NODE_STROKE_WIDTH = VueResources.getInt(
    "node.strokeWidth"
  );
  public static final Color DEFAULT_NODE_STROKE_COLOR = VueResources.getColor(
    "node.strokeColor"
  );
  public static final Font DEFAULT_TEXT_FONT = VueResources.getFont(
    "text.font"
  );

  /** how much smaller children are than their immediately enclosing parent (is cumulative) */
  static final float ChildScale =
    VueResources.getInt("node.child.scale", 75) / 100f;
  ...
  private final LWIcon.Block mIconBlock = new LWIcon.Block(
    this,
    IconWidth,
    IconHeight,
    null,
  ...
  LWNode(String label, float x, float y, RectangularShape shape) {
    ...
    if (shape == null) setShape(tufts.vue.shape.RoundRect2D.class);
    //setShape(new RoundRectangle2D.Float(0,0, 10,10, 20,20));
    else if (shape != null) setShapeInstance(shape);
  ...
  public static final Key KEY_Shape = new Key<
    LWNode,
    Class<? extends RectangularShape>
  >("node.shape", "shape") {
    @Override
    ...
    public void setValue(
      LWNode c,
      Class<? extends RectangularShape> shapeClass
    ) {
    ...
    boolean valueEquals(LWNode c, Object other) {
      ...
      if (other instanceof RectangularShape) {
        ...
        if (DEBUG.Enabled) Log.warn(
          this + "; valueEquals against unexpected type: " + Util.tags(other)
        );
        return false;
  ...
  protected void setShapeInstance(RectangularShape shape) {
    if (DEBUG.CASTOR) System.out.println(
      "SETSHAPE " + shape.getClass() + " in " + this + " " + shape
    );
    ...
    isRectShape =
      (shape instanceof Rectangle2D || shape instanceof RoundRectangle2D);
    mShape = shape;
  ...
  protected Point2D getZeroSouthEastCorner() {
    ...
    final float[] corner = VueUtil.computeIntersection(
      getWidth() / 2,
      getHeight() / 2,
      getWidth(),
      getHeight(),
   ...
   */
  private static final Point2D RoundRectCorner = new Point2D.Double(
    2.928932,
    2.928932
  );
  ...
  public LWNode duplicate(CopyContext cc) {
    ...
    if (DEBUG.STYLE) out(
      "re-adjusting size during duplicate to set shape size"
    );
  ...
  public boolean supportsUserResize() {
    if (isTextNode()) return !isAutoSized(); // could be confusing, as once is shrunk down, can't resize again w/out undo
    else return true;
  }
  ...
  public boolean handleDoubleClick(MapMouseEvent e) {
    ...
    if (
      this instanceof LWPortal
    ) return super.handleDoubleClick(e); // hack: clean this up -- maybe move all below to LWComponent...
  ...
  public static boolean isImageNode(LWComponent c) {
    if (c instanceof LWNode) {
      ...
      return (
        childZero instanceof LWImage &&
        childZero.hasResource() &&
        childZero.getResource().equals(node.getResource())
      );
    } else return false;
  }
  ...
  public static boolean isTextNode(LWComponent c) {
    if (c instanceof LWNode) return (
      (LWNode) c
    ).isTextNode(); else return false;
  }
  ...
  public boolean isLikelyTextNode() {
    ...
    return (
      getClass() == LWNode.class && // sub-classes don't count
      isTranslucent() &&
      !hasChildren() &&
      !inPathway()
    ); // heuristic to exclude LWNode portals (not likely to just put a piece of text alone on a pathway)
  }

  @Override
  public boolean isExternalResourceLinkForPresentations() {
    return (
      hasResource() && !hasChildren() && !iconShowing() && getStyle() != null
    );
  ...
  public boolean isAutoSized() {
    if (WrapText) return false; // LAYOUT-NEW
    else return isAutoSized;
  }
  ...
  private void setAutomaticAutoSized(boolean tv) {
    if (
      isOrphan()
    ) return; // if this is during a restore, don't do any automatic auto-size computations
  ...
  private static boolean IsSameShape(
    ...
    if (c1 == c2) {
      if (
        java.awt.geom.RoundRectangle2D.class.isAssignableFrom(c1)
  ...
  public void XML_completed(Object context) {
    ...
    if (hasChildren()) {
      if (DEBUG.WORK || DEBUG.XML || DEBUG.LAYOUT) Log.debug(
        "XML_completed: scaling down LWNode children in: " + this
      );
  ...
  public void addChildren(
    java.util.Collection<? extends LWComponent> children,
    Object context
  ) {
    if (
      !mXMLRestoreUnderway &&
  ...
  @Override
  protected List<LWComponent> sortForIncomingZOrder(
    Collection<? extends LWComponent> toAdd
  ) {
  ...
  public void setResource(final Resource r) {
    ...
    if (getChild(0) instanceof LWImage) {
      ...
      if (DEBUG.IMAGE) out(
        "checking for resource sync to image @child(0): " + image0
      );
      if (r.isImage()) {
  ...
  private void loadAssetToVueMetadata(Osid2AssetResource r)
    ...
    if (asset == null) {
      Log.warn(
        this + "; can't load asset meta-data: Resource has no asset: " + r
      );
    ...
    while (recordIterator.hasNextRecord()) {
      ...
      while (partIterator.hasNextPart()) {
        ...
        if ((part != null) && (partStructure != null)) {
          ...
          if (
            !key.startsWith(VueResources.getString("metadata.dublincore.url"))
          ) continue;
          if (key == null) {
          ...
          if (value instanceof String) {
            ...
            edu.tufts.vue.metadata.VueMetadataElement vueMDE =
  ...
  protected final void setSizeImpl(float w, float h, boolean internal) {
    ...
    if (
      isAutoSized() && (w > this.width || h > this.height)
    ) setAutomaticAutoSized(false); // does this handle scaling?
  ...
  private void layoutNode(Object triggerKey, Size curSize, Size request) {
    if (inLayout) {
      if (DEBUG.Enabled) {
        if (DEBUG.LAYOUT) new Throwable("ALREADY IN LAYOUT " + this)
          .printStackTrace(); else Log.warn("already in layout: " + this);
      }
      return;
    }
    inLayout = true;
    if (DEBUG.LAYOUT) {
      String msg =
        ...
        " isAutoSized=" +
        isAutoSized();
      if (DEBUG.META) Util.printClassTrace(
        "tufts.vue.LW",
        msg + " " + this
      ); else out(msg);
    }
    ...
    if (
      request.height <= min.height && request.width <= min.width
    ...
    if (isAutoSized()) {
      ...
      if (request.width > min.width) newWidth = request.width; else newWidth =
        min.width;
      if (request.height > min.height) newHeight =
        request.height; else newHeight = min.height;
    }
    ...
    if (isCenterLayout == false) {
      ...
      mIconDivider.setLine(
        IconMargin,
        MarginLinePadY,
        IconMargin,
        newHeight - MarginLinePadY
      );
      // mIconDivider set by layoutCentered in the other case
    }

    if (labelBox != null) labelBox.setBoxLocation(
      relativeLabelX(),
      relativeLabelY()
    );
  ...
  private int getTextWidth() {
    if (WrapText) return labelBox.getWidth(); else return Math.round(
      getTextSize().width
    );
  }
  ...
  private Size layoutCentered(Size request) {
    ...
    if (
      ...
      growShapeUntilContainsContent(nodeShape, content)
    ) {
    ...
    if (DEBUG.LAYOUT) out(
      "*** content placed at " + content + " in " + nodeShape
    );
   ...
   */
  private boolean growShapeUntilContainsContent(
    RectangularShape shape,
    NodeContent content
  ) {
    final int MaxTries = 1000; // in case of loops (e.g., a broke shape class whose contains() never succeeds)
    final float increment;
    if (content.width > content.height) increment =
      content.width * 0.1f; else increment = content.height * 0.1f;
    ...
    if (tries > 0) {
      ...
      do {
        shape.setFrame(
          0,
          0,
          shape.getWidth() - shrink,
          shape.getHeight() - shrink
        );
        //System.out.println("trying size " + shape + " for content " + content);
        layoutContentInShape(shape, content);
        tries++;
    ...
    } else if (DEBUG.LAYOUT) System.out.println(
      "Contents of " + shape + " already contains " + content
    );
   ...
   */
  private void layoutContentInShape(
    RectangularShape shape,
    NodeContent content
  ) {
  ...
  private class NodeContent extends Rectangle2D.Float {
    ...
    NodeContent() {
      ...
      if (iconShowing()) {
        rIcons =
          new Rectangle2D.Float(0, 0, mIconBlock.width, mIconBlock.height);
    ...
    private void copyTranslate(
      Rectangle2D.Float src,
      Rectangle2D.Float dest,
      float xoff,
      float yoff
    ) {
  ...
  private Size layoutBoxed(Size request, Size oldSize, Object triggerKey) {
    ...
    if (WrapText) min =
      layoutBoxed_floating_text(request, oldSize, triggerKey); else min =
      layoutBoxed_vanilla(request);
  ...
  private void layoutBoxed_label() {
    ...
    if (iconShowing()) {
      ...
      //-------------------------------------------------------
      if (WrapText) mLabelPos.x =
        (this.width - text.width) / 2 + 1; else mLabelPos.x = 200; // marked bad because unused in this case
    }
  }
  ...
  private void layoutBoxed_children(Size min, Size labelText) {
    if (DEBUG.LAYOUT) out(
      "*** layoutBoxed_children; min=" + min + " text=" + labelText
    );
  ...
  private void layoutBoxed_icon(Size request, Size min, Size text) {
    ...
    if (min.height < iconPillarHeight) {
      ...
      if (centerY > IconPillarPadY + IconPillarFudgeY) centerY =
        IconPillarPadY + IconPillarFudgeY;
      iconPillarY = centerY;
    }

    if (!isRectShape) {
      float height;
      if (isAutoSized()) height = min.height; else height =
        Math.max(min.height, request.height);
  ...
  private Size layoutBoxed_floating_text(
    Size request,
    Size curSize,
    Object triggerKey
  ) {
    if (DEBUG.LAYOUT) out(
      "*** layoutBoxed_floating_text, req=" +
      request +
      " cur=" +
      curSize +
      " trigger=" +
    ...
    if (iconShowing()) min.width =
      LabelPositionXWhenIconShowing; else min.width = LabelPadLeft;
    ...
    if (curSize.width == NEEDS_DEFAULT) { // NEEDS_DEFAULT meaningless now: will never be true (oh, only on restore?)
      ...
      if (resizeRequest) {
        ...
        if (true) {
          ...
          if (
            curWidth == labelBox.getPreferredSize().width
          ) keepPreferredWidth = true; else if (
            curWidth == labelBox.getMaxWordWidth()
  ...
  private Size layoutChildren(
    ...
    if (DEBUG.LAYOUT && DEBUG.META) Util.printClassTrace(
      "tufts.vue.LW",
      "*** layoutChildren"
    );
    ...
    //         else
    if (
      hasFlag(Flag.SLIDE_STYLE) &&
      mAlignment.get() != Alignment.LEFT &&
      isImageNode(this)
    ) layoutChildrenColumnAligned(
      baseX,
      baseY,
      result
    ); else layoutChildrenSingleColumn(baseX, baseY, result);
  ...
  protected void layoutChildrenColumnAligned(
    float baseX,
    float baseY,
    Size result
  ) {
    ...
    for (LWComponent c : getChildren()) {
      if (
        c instanceof LWLink
      ) continue; // todo: don't allow adding of links into a manged layout node!
      float w = c.getLocalBorderWidth();
      if (w > maxWidth) maxWidth = w;
    }
    ...
    for (LWComponent c : getChildren()) {
      ...
      if (mAlignment.get() == Alignment.RIGHT) c.setLocation(
        baseX + maxLayoutWidth - c.getLocalWidth(),
        y
      ); else if (mAlignment.get() == Alignment.CENTER) c.setLocation(
        baseX + (maxLayoutWidth - c.getLocalWidth()) / 2,
        y
      ); else c.setLocation(baseX, y);
      y += c.getLocalHeight();
    }
  ...
  protected void layoutChildrenSingleColumn(
    float baseX,
    float baseY,
    Size result
  ) {
  ...
  class Column extends java.util.ArrayList<LWComponent> {
    ...
    void layout(float baseX, float baseY, boolean center) {
      ...
      while (i.hasNext()) {
        LWComponent c = (LWComponent) i.next();
        if (center) c.setLocation(
          baseX + (width - c.getLocalBorderWidth()) / 2,
          y
        ); else c.setLocation(baseX, y);
        y += c.getHeight();
  ...
  protected void layoutChildrenGrid(
    float baseX,
    float baseY,
    Size result,
    int nColumn,
  ...
  public Color getRenderFillColor(DrawContext dc) {
    ...
    if (
      dc == null || dc.focal == this
    ) return super.getRenderFillColor(dc); // TextBox or presentation focal
    ...
    if (getParent() instanceof LWNode) {
      if (fillColor != null) {
        ...
        if (
          parentFill != null &&
          !parentFill.equals(Color.black) &&
          parentFill.getAlpha() != 0 &&
          fillColor.equals(parentFill)
        ) {
  ...
  private void drawNodeWithReducedLOD(
    ...
    if (isSelected()) {
      ...
      if (
        isTransparent() || renderFill.equals(getParent().getRenderFillColor(dc))
      ) hasVisibleFill = false; else dc.g.setColor(renderFill);
    }
    ...
    if (this.height * renderScale > 5) {
      ...
      if (hasVisibleFill) {
        if (mShape.getClass() == Rectangle2D.Float.class) dc.g.fill(
          mShape
        ); else dc.g.fillRect(0, 0, (int) getWidth(), (int) getHeight());
  ...
  public boolean isCollapsed() {
    if (
      COLLAPSE_IS_GLOBAL
    ) return isGlobalCollapsed; else return super.isCollapsed();
  }
  ...
  private void drawSelection(DrawContext dc) {
    ...
    if (dc.zoom < 1) dc.setAbsoluteStroke(
      SelectionStrokeWidth
    ); else dc.g.setStroke(
      new BasicStroke(getStrokeWidth() + SelectionStrokeWidth)
    );
    dc.g.draw(mShape);
  }
  ...
  private void drawNode(DrawContext dc) {
    if (dc.isLODEnabled()) {
      ...
      if (iconShowing()) canSkipIcon =
        LWIcon.FONT_ICON.getSize() * renderScale < 5; else canSkipIcon = true;
  ...
  private void drawFullNode(DrawContext dc) {
    ...
    if (
      getStrokeWidth() > 0
      /*&& !isPresentationContext() && !dc.isPresenting()*/
    ) { // old style "turn off the wrappers"
    ...
    if (
      hasLabel() && this.labelBox != null && this.labelBox.getParent() == null
    ) {
  ...
  private float childOffsetY() {
    ...
    if (iconShowing()) {
      ...
      if (DEBUG.LAYOUT) out(
        "*** childOffsetY starting with precomputed " +
        baseY +
        " to produce " +
        (baseY + ChildOffsetY)
      );
  ...
  private static final int IconHeight = VueResources.getInt(
    "node.icon.height",
    14
  );
  ...
  private static final int LabelPositionXWhenIconShowing =
    IconMargin + LabelPadLeft;
