...
public class LWGroup extends LWContainer {
      ...
      for (LWComponent c : allUniqueDescendents) {
        ...
        for (LWLink l : c.getLinks()) {
          ...
          if (DEBUG.PARENTING) out(
            "SEEING LINK " + l + " IN-PLAY=" + bothEndsInPlay
          );
  ...
  private void normalize(boolean reshape) {
    ...
    if (DEBUG.WORK) {
      ...
      out(
        "preBounds: " +
        Util.out(preBounds) +
        String.format("; dx=%+.2f dy=%+.2f dw=%+.1f dh=%+.1f", dx, dy, dw, dh)
      );
    ...
    if (dx != 0.0f || dy != 0.0f) {
      ...
      if (DEBUG.WORK) out(
        "normalizing relative children: dx=" + dx + " dy=" + dy
      );
  ...
  private Rectangle2D.Float getPreNormalBounds() {
    if (
      numChildren() < 2
    ) return LWMap.EmptyBounds; else return LWMap.getLocalBorderBounds( // if only zero or one child, we should be about to disperse...
      getChildren()
    );
  }
    ...
    public void run() {
      ...
      if (numChildren() < 2) {
        if (DEBUG.PARENTING || DEBUG.CONTAINMENT) out(
          "AUTO-DISPERSING on child count " + numChildren()
        );
        disperse();
      } else {
        normalize();
  ...
  private void requestCleanup(Object srcMsg) {
    ...
    if (um != null && !um.isUndoing() && !um.hasLastTask(this)) {
      if (DEBUG.CONTAINMENT) out(
        "addLastTask/DisperseOrNormalize; on " + srcMsg
      );
  ...
  public void addChildren(
    Collection<? extends LWComponent> iterable,
    Object context
  ) {
  ...
  public void disperse() {
    ...
    if (DEBUG.PARENTING || DEBUG.CONTAINMENT) System.out.println(
      "DISPERSING: " + this
    );
  ...
  private void translateSelection(double dx, double dy) {
    for (LWComponent c : getChildren()) {
      ...
      if (
        c.isSelected() && c.isAncestorSelected()
      ) continue; else translateOnMap(c, dx, dy);
    }
  }
  ...
  public void setLocation(float x, float y) {
    if (isForSelection) Util.printStackTrace(
      "setLocation on selection group " + x + "," + y + " " + this
  ...
  public Color getRenderFillColor(DrawContext dc) {
    if (
      TrackPathwayColor && dc != null && dc.drawPathways() && isTransparent()
    ) {
  ...
  protected LWComponent pickChild(PickContext pc, LWComponent c) {
    ...
    if (pc.pickDepth > 0) return c; else if (
      c.isPathwayOwned() && c instanceof LWSlide
    ) return c; else return this; // to allow slide icon picking
  }
  ...
  protected boolean containsImpl(final float x, final float y, PickContext pc) {
    if (
      (pc.isZoomRollover && pc.pickDepth < 1) ||
      hasDecoratedFeatures() ||
      pc.root == this
    ) {
  ...
  protected boolean intersectsImpl(final Rectangle2D rect) {
    ...
    if (hasDecoratedFeatures()) {
      ...
      for (LWComponent c : getChildren()) if (
        c.intersects(rect)
      ) return true; // todo: not in parent coords (?)
      return false;
    }
  }
