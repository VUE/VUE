...
public class LWCFilter {
  ...
  public static final int USERDATA = 5;
  ...
  public LWCFilter(Vector pEditors) {
    ...
    if ((pEditors != null) && (!pEditors.isEmpty())) {
      ...
      for (int i = 0; i < pEditors.size(); i++) {
        FilterStatementEditor fse = (FilterStatementEditor) pEditors.elementAt(
          i
        );
  ...
  public void setFilterAction(Object action) {
    if (
      action != ACTION_HIDE && action != ACTION_SHOW && action != ACTION_SELECT
    ) throw new IllegalArgumentException(
      this + " illegal filter action: " + action
    );
  ...
  public void removeStatements(Key key) {
    ...
    while (i.hasNext()) {
      ...
      if (
        ((String) statement.getKey().getKey()).equals(key.getKey().toString())
  ...
  private boolean matchStatementComponent(
    Statement statement,
    LWComponent pLWC
  ) {
    if (statement.getKey().getKey().toString().equals("Anywhere")) return (
      matchLabel(statement, pLWC) | matchNotes(statement, pLWC)
  ...
  public class LogicalStatement {
    ...
    private boolean matchAnywhere(LWComponent pLWC) {
      ...
      if (matchLabel(pLWC)) return true;
      if (matchNotes(pLWC)) return true;
      if (matchUserMapType(pLWC)) return true;
      if (matchMetaData(pLWC)) return true;
      if (matchUserProperty(pLWC, true)) return true;
    ...
    private boolean isMatch(String pStr) {
      if (pStr == null || mValue == null) return false;
      ...
      switch (getCondition()) {
        ...
        case STARTS:
          return search.startsWith(filter);
        case ENDS:
          return search.endsWith(filter);
      }

      // should never happen
      throw new IllegalStateException(
        this + " unknown condition " + getCondition()
      );
    }

    public String toString() {
      return (
        "LogicalStatement[Source: " +
        mSourceType +
        " condition: " +
        mCondition +
        " value: " +
