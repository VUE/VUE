...
class LWCInspector
  ...
  implements
    VueConstants, LWSelection.Listener, LWComponent.Listener, ActionListener {
  ...
  private JTextArea log = new JTextArea() {
    ...
    public void append(String s) {
      if (LOG_ENABLED) super.append(rows++ + " " + s + "\n");
    }
  };
  ...
  private java.util.Map<JTextField, LWComponent.Key> fieldKeys =
  ...
  public LWCInspector() {
    ...
    if (!(notesField instanceof JTextField)) notesField.setBorder(
  ...
  public void activeChanged(ActiveEvent e, LWPathway.Entry entry) {
    if (
      entry != null && entry.isPathway() && VUE.getSelection().isEmpty()
    ) loadItem(entry.pathway.getMasterSlide());
  }

  public void activeChanged(ActiveEvent e, LWComponent c) {
    if (lockBtn.isSelected()) return;
    if (c == null) loadItem(VUE.getActiveMap()); else if (
      c instanceof LWPathway
    ) loadItem(((LWPathway) c).getMasterSlide()); else loadItem(c);
  }
  ...
  private void addLabelTextRows(
    ...
    while (i.hasNext()) {
      ...
      if (field instanceof JTextField) ((JTextField) field).addActionListener(
          this
        );
      ...
      if (readOnly) {
        ...
        if (VueUtil.isMacPlatform()) field.setBackground(SystemColor.control);
      }
    }
    /*
  ...
  public void LWCChanged(LWCEvent e) {
    ...
    if (e.component != null && e.component != e.source) extra +=
    " from:" + e.component;
    if (e.getComponents() != null) extra += " list:" + e.getComponents();

    log.append(String.format("%-21s %s" + extra, e.key, e.source));
    //System.out.println(this + " " + e);
    if (this.lwc != e.getSource()) return;
  ...
  public void selectionChanged(LWSelection selection) {
    if (!lockBtn.isSelected()) setSelection(selection);
  }
  ...
  private void loadSelection(LWSelection selection) {
    ...
    String id = "";
    if (selection.allOfSameType()) id +=
    selection.first().getClass().getName(); else id += "LWComponent";
    id += "[" + selection.size() + "]";
  ...
  private void loadLabel(LWComponent lwc) {
    ...
    if (c == null || c.getAlpha() != 255) c = Color.gray;
    labelField.setBackground(c);
    if (Color.black.equals(c)) labelField.setForeground(
      Color.white
    ); else labelField.setForeground(Color.darkGray);
  ...
  private void loadItem(LWComponent lwc) {
    if (this.lwc != lwc) {
      if (this.lwc != null) this.lwc.removeLWCListener(this);
      this.lwc = lwc;
      if (this.lwc != null) {
    ...
    String id = c.getID();
    if (c.getParent() == null) id += " [PARENT IS NULL]"; else {
      id += ", #" + c.getParent().indexOf(c);
      id += " in <" + c.getParent().toName() + ">";
      id += " links:" + c.getLinks().size();
      if (c instanceof LWContainer && c.hasChildren()) id +=
      " children:" + ((LWContainer) c).getChildList().size();
    }

    idField.setText(id);
    if (lwc.getTypeToken() != lwc.getClass()) typeField.setText(
      lwc.getClass().getName() + " [" + lwc.getTypeToken() + "]"
    ); else typeField.setText(lwc.getClass().getName());

    parentField.setText(
      c.getParent() == null ? "null" : c.getParent().toString()
    );
    loadLabel(c);
    ...
    if (!c.isAutoSized()) sizeText += " userSize";
    sizeField.setText(sizeText);
    transLocField.setText(c.getZeroTransform().toString());
    bitsField.setText(
      c.getDescriptionOfSetBits() + (c.isFiltered() ? " +FILTERED" : "")
    );
    ...
    widthField.setText(
      String.format("%5.1f map(%5.1f)", c.getWidth(), c.getMapWidth())
    );
    heightField.setText(
      String.format("%5.1f map(%5.1f)", c.getHeight(), c.getMapHeight())
    );
    ...
    for (Map.Entry<JTextField, LWComponent.Key> e : fieldKeys.entrySet()) {
      ...
      try {
        if (DEBUG.PROPERTY || c.supportsProperty(key)) field.setText(
          key.getStringValue(c)
        ); else field.setText(
          "<unsupported for " + c.getClass().getName() + ">"
        );
      } catch (Throwable t) {
        if (field != null) field.setText(t.toString());
        t.printStackTrace();
      }
    }
  ...
  private LWSelection getSelection() {
    ...
    } else return VUE.getSelection();
  }

  private void setWidths(String text) throws NumberFormatException {
    float w = Float.parseFloat(text);
    for (LWComponent c : getSelection()) c.setSize(w, c.getHeight());
    //c.setAbsoluteSize(w, c.getAbsoluteHeight());
  }

  private void setHeights(String text) throws NumberFormatException {
  ...
  private void setXs(String text) throws NumberFormatException {
  ...
  private void setYs(String text) throws NumberFormatException {
    float v = Float.parseFloat(text);
    Iterator i = getSelection().iterator();
    while (i.hasNext()) {
      LWComponent c = (LWComponent) i.next();
  ...
  private void setScales(String text) throws NumberFormatException {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    try {
      ...
      else if (src == widthField) setWidths(text); else if (
        src == heightField
      ) setHeights(text); else if (src == zoomField) setScales(text); else if (
        src == resourceField
      ) c.setResource(text); else if (src == xField) setXs(text); else if (
        src == yField
      ) setYs(text);
      ...
      if (set) VUE.getUndoManager().mark(); else return;
    } catch (Exception ex) {
      System.err.println(ex);
      System.err.println(
        "LWCInspector: error setting property value [" + text + "] on " + src
      );
    }
  ...
  static class Introspector
    ...
    private void updateText() {
      ...
      for (Map.Entry<Class, Object> e : allActive.entrySet()) {
        final Class implClass = e.getValue() == null
          ? null
          : e.getValue().getClass();
        ...
        text.append(
          String.format(
            "\n%25s: %s%s\n",
            e.getKey().getName(),
            (implClass == e.getKey() || implClass == null)
              ? ""
              : ("(" + implClass.getName() + ") "),
            e.getValue()
          )
        ...
        if (DEBUG.META) addMethodData(e.getValue());
    ...
    private void addMethodData(Object instance) {
      ...
      if (
        LWComponent.class.isInstance(instance) ||
        VueTool.class.isInstance(instance)
      ) methods = clazz.getMethods(); else methods = clazz.getDeclaredMethods(); // includes private's, excludes super-class methods

      for (Method m : methods) {
        if (
          m.getReturnType() == void.class || m.getParameterTypes().length > 0
        ) continue;
        ...
        if (
          "duplicate".equals(m.getName()) || "clone".equals(m.getName())
        ) value = "not invoked:" + m.getName();

        try {
          if (value == null) value = m.invoke(instance);
    ...
    private static String shortTypeName(Class type) {
      ...
      if (name.indexOf('.') > 0) return name.substring(
        name.lastIndexOf('.') + 1,
        name.length()
      ); else return name;
    }
