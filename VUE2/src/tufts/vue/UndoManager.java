...
public class UndoManager implements LWComponent.Listener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(UndoManager.class);
  ...
  private UndoActionList UndoList = new UndoActionList(
    VueResources.getString("menu.edit.undo")
  );
  /** The list of redo actions (named groups of property changes generated from Undo's) */
  private UndoActionList RedoList = new UndoActionList(
    VueResources.getString("menu.edit.redo")
  );
  ...
  private Map<Thread, UndoMark> mThreadsWithMark = Collections.synchronizedMap(
    new HashMap()
  );
  ...
  private static class UndoAction {
    ...
    synchronized void addAttachedThread(Thread t) {
      if (attachedThreads == null) attachedThreads = new ArrayList();
    ...
    synchronized void undoAggregateUserAction() {
      if (DEBUG.UNDO) Log.debug(
        this + " undoing sequence of size " + changeCount()
      );
      ...
      if (attachedThreads != null) {
        ...
        if (DEBUG.Enabled) Log.debug(
          this +
          " making sure " +
          attachedThreads.size() +
          " attached threads are stopped"
        );
      ...
      while (i.hasPrevious()) {
        UndoItem undoItem = i.previous();
        if (undoItem.propKey != LWKey.HierarchyChanging) undoItem.undo();
      }
      //             if (hierarchyChanged)
    ...
    private String produceDisplayName() {
      if (this.name == LWKey.HierarchyChanging) return "(Hierarchy Change)"; // shouldn't see this

      String display = "";
      String uName = this.name;

      if (uName.startsWith("hier.")) uName = uName.substring(5);
      // Replace all '.' with ' ' and capitalize first letter of each word
      uName = uName.replace('-', '.');
      String[] words = uName.split("\\.");
      for (int i = 0; i < words.length; i++) {
        final String word = words[i];
        if (word.length() == 0) return null; // if seen multiple dots in a row, presume pre-formatted
        if (Character.isLowerCase(word.charAt(0))) words[i] =
  ...
  private static class UndoItem implements Comparable {
      ...
      if (propKey == LWKey.HierarchyChanging) {
        ...
        if (DEBUG.TEST) {
          ...
          try {
            ...
            if (curValue != null) undoAnimated();
          } catch (Exception e) {
            System.err.println(
              "Exception during animated undo of [" +
              propKey +
              "] on " +
              component
            );
            if (oldValue != null) System.err.println(
              "\toldValue is " + oldValue.getClass() + " " + oldValue
            );
            e.printStackTrace();
          }
        }
        ...
        if (component.isOrphan()) {
          ...
          if (DEBUG.Enabled) Log.debug(
            "SKIPPING undo item for deleted (parentless) component: " +
            component +
            "; " +
            this
          );
        } else component.undoProperty(propKey, oldValue);
      }
    }
    ...
    private void undoAnimated() {
      ...
      if (oldValue instanceof Point) animatedChange((Point) oldValue); else if (
        oldValue instanceof Point2D
      ) animatedChange((Point2D) oldValue); else if (
        oldValue instanceof Color
      ) animatedChange((Color) oldValue); else if (
        oldValue instanceof Size
      ) animatedChange((Size) oldValue); else if (
        oldValue instanceof Integer
      ) animatedChange((Integer) oldValue); else if (
        oldValue instanceof Float
      ) animatedChange((Float) oldValue); else if (
    ...
    private void animatedChange(Float endValue) {
      ...
      final float inc =
        (endValue.floatValue() - curValue.floatValue()) / segments;
      Float value;
    ...
    private void animatedChange(Double endValue) {
      ...
      final double inc =
        (endValue.doubleValue() - curValue.doubleValue()) / segments;
      Double value;
    ...
    private void animatedChange(Point2D endValue) {
      ...
      Point2D.Float value = new Point2D.Float(
        (float) curValue.getX(),
        (float) curValue.getY()
      );
    ...
    private static void undoHierarchyChange(
      final LWContainer parent,
      final Object oldValue
    ) {
      if (DEBUG.UNDO) System.out.println(
        "\trestoring children of " + parent + " to " + oldValue
      );
      ...
      if (newChildList == LWComponent.NO_CHILDREN) {
        ...
        if (oldChildList != LWComponent.NO_CHILDREN) childrenAdded.removeAll(
          oldChildList
        );
      }
      ...
      if (oldChildList == LWComponent.NO_CHILDREN) {
        ...
        if (newChildList != LWComponent.NO_CHILDREN) childrenRemoved.removeAll(
          newChildList
        );
      }
      ...
      if (parent.mChildren != LWComponent.NO_CHILDREN) {
        if (parent instanceof LWPathway) {
          ...
          Log.error(
            "LWPathway's don't have real children: " +
            parent +
            "; for children " +
            parent.mChildren
          );
      ...
      if (childrenAdded.size() > 0) {
        if (DEBUG.UNDO) Log.debug(
          "Synthetic event " + LWKey.ChildrenAdded + " " + childrenAdded
        );
        parent.notify(LWKey.ChildrenAdded, childrenAdded);
      }
      if (childrenRemoved.size() > 0) {
        if (DEBUG.UNDO) Log.debug(
          "Synthetic event " + LWKey.ChildrenRemoved + " " + childrenRemoved
        );
    ...
    public String toString() {
      ...
      if (oldValue instanceof Collection) {
        ...
        if (c.size() > 1) old = c.getClass().getName() + "{" + c.size() + "}";
      }
      return (
        "UndoItem[" +
        order +
  ...
  private static class UndoActionList extends ArrayList {
    ...
    void advance() {
      current++;
      if (current >= size()) throw new IllegalStateException(
        this + " top >= size()"
      );
    }
    ...
    public String toString() {
      return (
        "UndoActionList[" + name + " top=" + top() + " size=" + size() + "]"
      );
    }
  ...
  private boolean checkAndHandleUnmarkedChanges() {
    if (mCurrentUndo.isIncomplete()) {
      new Throwable(this + " UNMARKED CHANGES! " + mComponentChanges)
        .printStackTrace();
  ...
  public synchronized void undo() {
    ...
    if (UndoList.peek() == null) mMap.markAsSaved();
  }
  ...
  public void mark(String aggregateName) {
    ...
    if (
      name == null && mLastEvent != null
    ) name = mLastEvent.getName(); // going to need to put last event into UndoAction..
  ...
  private static class TaskMap
    extends java.util.LinkedHashMap<Object, Runnable> {
  ...
  private void addTask(TaskMap taskMap, Object taskKey, Runnable task) {
    ...
    if (mUndoUnderway) {
      Util.printStackTrace(
        this +
        "; ignoring task during undo/redo in " +
        "\n\ttaskKey: " +
        taskKey +
        "\n\t   task: " +
        task
    ...
    synchronized (taskMap) {
      ...
      if (prior != null) Util.printStackTrace(
        "over-wrote existing cleanup task (won't be run): " +
        prior +
        " taskKey: " +
        taskKey
      );
  ...
  private void runPrioritizedCleanupTasks(boolean debug) {
    if (mCurrentUndo.size() == 0) {
      ...
      if (DEBUG.Enabled) Log.debug(
        "Running cleanup tasks with an empty undo queue: " + this
      );
      debug = true;
    } else if (!debug) debug = DEBUG.WORK || DEBUG.UNDO;

    int recurseCount = 0; // first one doesn't count
    do {
      if (debug && recurseCount > 0) System.out.println(
        TERM_RED +
        "CLEANUP TASKS: extra model cleanup pass #" +
        recurseCount +
        TERM_CLEAR
      );
      if (mCleanupTasks.size() > 0) runCleanupTasks(debug, mCleanupTasks);
      if (mLastTasks.size() > 0) runCleanupTasks(debug, mLastTasks);
    } while (
      ++recurseCount < MaxRecurse &&
      (mCleanupTasks.size() > 0 || mLastTasks.size() > 0)
    );

    if (recurseCount > 1) {
      if (recurseCount >= MaxRecurse) Util.printStackTrace(
        this + " cleanup task recursion count exceeded max at " + recurseCount
      ); else Log.info(
        "note: UndoManager cleanup task recusion count reached " +
        recurseCount +
        " extra passes before model settled down."
      );
    }
  ...
  public synchronized void markChangesAsUndo(String name) {
    ...
    if (
      mCurrentUndo.size() == 0
    ) addUndoable = false; else if (name == null) { // if nothing changed, don't bother adding an UndoAction
      if (mLastEvent == null) addUndoable = false; else name =
        mLastEvent.getName();
    }
  ...
  void flush() {
    ...
    RedoList.clear();
    mComponentChanges.clear();
    if (VUE.getActiveMap() == mMap) updateGlobalActionLabels();
  }
  ...
  void attachThreadToNextMark(UndoableThread t) {
    if (t.isAlive()) throw new Error(
      t + ": not safe to attach an UndoAction to an already started thread"
    );
  ...
  private static class UndoMark {
    ...
    public String toString() {
      String s = "UndoMark[";
      if (action != manager.mCurrentUndo) s += action + " / ";
      return s + manager + "]";
    }
  }
  ...
  public static Object getKeyForNextMark(LWComponent c) {
    ...
    if (
      undoManager == null
    ) return null; else return undoManager.getKeyForNextMark();
  ...
  static void attachCurrentThreadToMark(Object undoActionKey) {
    if (undoActionKey != null) {
      ...
      if (existingMark != null) {
        ...
        if (DEBUG.UNDO || DEBUG.THREAD) Log.debug(
          "ATTACHED " + mark + " to " + thread
        );
      }
      /*
  ...
  static void detachCurrentThread(Object undoActionKey) {
    if (undoActionKey != null) {
      if (DEBUG.THREAD) Util.printStackTrace(
        "detachCurrentThread: " + Thread.currentThread()
      );
  ...
  private void checkAndHandleSelectionCleanups() {
    if (DEBUG.UNDO) Log.debug(
      "checkAndHandleSelectionCleanups: hideCleanup=" +
  ...
  public void LWCChanged(final LWCEvent e) {
    ...
    if (
      mRedoUnderway
    ) return; // ignore everything during redo

    if (
      e.key == LWKey.Repaint || e.key == LWKey.RepaintAsync
    ) return; // ignore these

    if (mUndoUnderway) {
      if (!mRedoCaptured && mCurrentUndo.size() > 0) {
        Util.printStackTrace(
          "Undo Error: have changes at start of redo record:" +
          "\n\t  current UndoAction: " +
          mCurrentUndo +
  ...
  private void recordEvent(LWCEvent e, boolean hierarchyEvent) {
    ...
    if (thread instanceof UndoableThread) {
      ...
      mThreadsWithMark.size() > 0 && mThreadsWithMark.containsKey(thread)
    ) {
      //-----------------------------------------------------------------------------
...
*/
    ...
    if (hierarchyEvent) recordUndoableChangeEvent(
      relevantUndoAction,
      perComponentChanges,
      LWKey.HierarchyChanging,
      (LWContainer) e.getSource(), // parent
      HIERARCHY_CHANGE_TAG
    ); else recordUndoableChangeEvent(
      relevantUndoAction,
      perComponentChanges,
      e.key,
      e.getComponent(),
  ...
  private static void recordUndoableChangeEvent(
    ...
    if (perComponentChanges != null) {
      ...
      if (allChangesToComponent == null) {
        ...
        alreadyStoredValue =
          (TaggedPropertyValue) allChangesToComponent.get(propertyKey);
        if (alreadyStoredValue != null) {
          if (DEBUG.UNDO) System.out.println(" (compressed)");
          compressed = true;
        } else if (
          propertyKey == LWKey.HierarchyChanging &&
          allChangesToComponent.containsKey(LWKey.Created)
        ) {
          ...
          if (DEBUG.WORK) Log.debug(
            "UndoManager: compressing hier change event for newly created component: " +
            component
          );
          if (DEBUG.UNDO) System.out.println(
            " (compressed:NEW COMPONENT IGNORES HIER CHANGES)"
          );
          compressed = true;
        }
      }
    ...
    if (compressed) {
      ...
      if (undoAction.size() > 1 && alreadyStoredValue != null) {
        ...
        UndoItem undoItem = (UndoItem) undoAction.undoSequence.get(
          alreadyStoredValue.index
        );
        if (DEBUG.UNDO && DEBUG.META) System.out.println(
          "Moving index " +
          alreadyStoredValue.index +
          " to end index " +
      ...
      if (oldValue == HIERARCHY_CHANGE_TAG) {
        ...
        if (container.mChildren == LWComponent.NO_CHILDREN) oldValue =
          LWComponent.NO_CHILDREN; else oldValue =
          ((ArrayList) container.mChildren).clone(); // TODO: impl dependency on ArrayList
      }
      if (allChangesToComponent != null) allChangesToComponent.put(
        propertyKey,
        new TaggedPropertyValue(undoAction.size(), oldValue)
      );
      undoAction.undoSequence.add(
        new UndoItem(component, propertyKey, oldValue, undoAction.eventCount)
      );
      undoAction.eventCount++;
      if (DEBUG.UNDO) {
