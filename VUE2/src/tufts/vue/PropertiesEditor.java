 ...
 *
 * @version $Revision: 1.34 $ / $Date: 2010-02-03 19:17:40 $ / $Author: mike $
 * @author  akumar03
 */
...
public class PropertiesEditor extends JPanel implements DublinCoreConstants {
  ...
  JButton addPropertyButton = new VueButton(
    VueResources.getString("button.add.label")
  );
  JButton deletePropertyButton = new VueButton(
    VueResources.getString("button.delete.label")
  );
  JLabel questionLabel = new JLabel(
    VueResources.getImageIcon("smallInfo"),
    JLabel.LEFT
  );
  ...
  private void setResourePropertiesPanel() {
    ...
    propertiesTable.addFocusListener(
      new FocusListener() {
        public void focusLost(FocusEvent e) {
          if (propertiesTable.isEditing()) {
            propertiesTable
              .getCellEditor(
                propertiesTable.getEditingRow(),
                propertiesTable.getEditingColumn()
              )
              .stopCellEditing();
          }
    ...
    propertiesTable.addKeyListener(
      new KeyAdapter() {
        public void keyPressed(KeyEvent e) {
          if (
            propertiesTable.getSelectedRow() ==
              (propertiesTable.getRowCount() - 1) &&
            e.getKeyCode() == KeyEvent.VK_ENTER &&
            tableModel.isEditable()
          ) {
    ...
    JLabel label = new JLabel(
      VueResources.getString("jlabel.resourcemetadata")
    );
    ...
    if (tableModel.isEditable()) {
      ...
      deletePropertiesButtonListener =
        new DeletePropertiesButtonListener(propertiesTable, sListener);
  ...
  public static class PropertiesTableModel extends AbstractTableModel {
    ...
    public void setProperties(PropertyMap properties) {
      ...
      if (isEditable() && m_conditions.size() == 0) addProperty(
        DC_FIELDS[0],
        PROPERTY_NOT_SET
      );
    ...
    public Object getValueAt(int row, int col) {
      ...
      if (col == 0) return cond.getProperty(); else return cond.getValue();
    }
    ...
    public void addProperty(String key, String value) {
      ...
      cond.setOperator(ComparisonOperator.eq);
      cond.setValue(value);
      if (value != PROPERTY_NOT_SET) mProperties.put(key, value);
      m_conditions.add(cond);
      fireTableDataChanged();
    }
    ...
    public void setValueAt(Object value, int row, int col) {
      ...
      if (col == 1) cond.setValue((String) value);
  ...
  public class DeletePropertiesButtonListener implements ActionListener {
    ...
    public DeletePropertiesButtonListener(
      JTable table,
      PropertiesSelectionListener sListener
    ) {
      this.table = table;
