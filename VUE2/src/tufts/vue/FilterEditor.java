...
public class FilterEditor extends JPanel {
  ...
  public static final Key keyLabel = new Key(
    "Label",
    TypeFactory.getStringType()
  );
  public static final Key keyAnywhere = new Key(
    "Anywhere",
    TypeFactory.getStringType()
  );
  public static final Key keyNotes = new Key(
    "Notes",
    TypeFactory.getStringType()
  );
  public static final JLabel triangleLabel = new JLabel(
    VueResources.getImageIcon("triangleDownIcon")
  );
  JLabel questionLabel = new JLabel(
    tufts.vue.VueResources.getImageIcon("smallInfo"),
    JLabel.LEFT
  );
  public static final String FILTER_INFO = tufts.vue.VueResources.getString(
    "info.filter.create"
  );
  JButton addButton = new tufts.vue.gui.VueButton(
    VueResources.getString("button.add.label")
  );
  JButton deleteButton = new tufts.vue.gui.VueButton(
    VueResources.getString("button.delete.label")
  );
  ...
  public void stopEditing() {
    if (filterTable.isEditing()) {
      filterTable
        .getCellEditor(
          filterTable.getEditingRow(),
          filterTable.getEditingColumn()
        )
        .stopCellEditing();
    }
  ...
  private void setFilterEditorPanel() {
    ...
    filterTable.addKeyListener(
      new KeyAdapter() {
        public void keyPressed(KeyEvent e) {
          if (
            filterTable.getSelectedRow() ==
              (filterTableModel.getRowCount() - 1) &&
    ...
    FilterSelectionListener sListener = new FilterSelectionListener(
      deleteButton,
      -1
    );
    filterTable.getSelectionModel().addListSelectionListener(sListener);
    deleteButton.addActionListener(
      new DeleteButtonListener(filterTable, sListener)
    );
    filterTable
      .getColumnModel()
      .getColumn(FilterTableModel.KEY_COL)
      .setCellEditor(new KeyCellEditor());
    filterTable
      .getColumnModel()
      .getColumn(FilterTableModel.KEY_COL)
      .setCellRenderer(new KeyCellRenderer());
    filterTable
      .getColumnModel()
      .getColumn(FilterTableModel.OPERATOR_COL)
      .setCellEditor(new OperatorCellEditor());
    filterTable
      .getColumnModel()
      .getColumn(FilterTableModel.OPERATOR_COL)
      .setCellRenderer(new OperatorCellRenderer());
  ...
  public class FilterTableModel
    ...
    implements MapFilterModel.Listener {
    ...
    public Object getValueAt(int row, int col) {
      ...
      if (col == KEY_COL) return statement.getKey(); else if (
        col == OPERATOR_COL
      ) return statement.getOperator(); else return statement.getValue();
    }
    ...
    public void setValueAt(Object value, int row, int col) {
      ...
      if (col == VALUE_COL) {
        ...
        if (value instanceof Key) {
          ...
          setValueAt(
            ((Key) value).getType().getDefaultOperator(),
            row,
            OPERATOR_COL
          );
  ...
  public class OperatorCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (value.toString().equals("")) value =
        keyAnywhere.getType().getDefaultOperator().toString();
  ...
  public class KeyCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (value.toString().equals("")) value = keyAnywhere.toString();
      JPanel panel = new JPanel(new BorderLayout());
      panel.setBorder(BorderFactory.createEmptyBorder(0, 2, 0, 0));
      panel.add(new JLabel(value.toString()), BorderLayout.CENTER);
      panel.add(triangleLabel, BorderLayout.EAST);
  ...
  public class DeleteButtonListener implements ActionListener {
    ...
    public DeleteButtonListener(
      JTable table,
      FilterSelectionListener sListener
    ) {
      this.table = table;
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (r > 0) table.setRowSelectionInterval(r - 1, r - 1); else if (
        table.getRowCount() > 0
