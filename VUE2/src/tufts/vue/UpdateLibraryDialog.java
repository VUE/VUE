...
/**
 * @version $Revision: 1.33 $ / $Date: 2010-02-03 19:17:41 $ / $Author: mike $
 * @author  akumar03
 */
import javax.swing.*;
...
public class UpdateLibraryDialog
  extends JDialog
  ...
  JButton cancelButton = new JButton(
    VueResources.getString("button.label.done")
  );
  ...
  private static String TITLE = VueResources.getString(
    "updateLibrary.dialogTitle"
  );
  private static String AVAILABLE = VueResources.getString(
    "updateLibrary.resourcesavailable"
  );
  ...
  public UpdateLibraryDialog(
    ...
    try {
      ...
      VueUtil.alert(
        this,
        VueResources.getString("dialog.instprovider.message"),
    ...
    try {
      ...
      helpButton.setToolTipText(
        VueResources.getString("updateLibrary.helpText.tooltip")
      );
  ...
  private void populate() {
    ...
    try {
      ...
      for (int i = 0; i < dataSources.length; i++) {
        installedProviderVector.addElement(
          dataSources[i].getProviderId().getIdString()
        );
      }
      ...
      while (providerIterator.hasNextProvider()) {
        ...
        if (installedProviderVector.contains(idString)) {
          ...
          if (
            !(installedProviderVector.contains(
  ...
  public void add() {
    try {
      ...
      try {
        ...
        if (proceed && (!provider.isInstalled())) {
          ...
          try {
            ...
            VueUtil.alert(
              this,
              VueResources.getString("dialog.installerror.message"),
              VueResources.getString("dialog.error.title")
            );
            return;
          } finally {
      ...
      if (proceed) {
        ...
        dataSourceList
          .getModelContents()
          .removeElement(dataSourceThatWasSelectedForUpdate);
