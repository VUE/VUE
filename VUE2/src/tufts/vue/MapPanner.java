 ...
 * visible viewport.
 *
 * @version $Revision: 1.70 $ / $Date: 2010-02-03 19:17:41 $ / $Author: mike $
 * @author Scott Fraize
 *
 */
...
public class MapPanner
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MapPanner.class);
  ...
  public void eventRaised(MapViewer.Event e) {
    if (VUE.inNativeFullScreen()) return;

    if (e.isActivationEvent() && mapViewer == null) {
      setViewer(e.viewer);
    } else if (e.viewer == this.mapViewer) {
      if (e.id == MapViewer.Event.PAN) repaint(); else if (
  ...
  private void updateZoomTitle() {
    ...
    if (this.mapViewer != null) titleInfo =
  ...
  private void setViewer(MapViewer mapViewer) {
    ...
    if (this.mapViewer != mapViewer) {
      ...
      if (mapViewer != null) setMap(mapViewer.getMap());
      repaint();
      updateZoomTitle();
    }
  }
  ...
  private void setMap(LWMap map) {
    ...
    if (this.map != map) {
      if (this.map != null) this.map.removeLWCListener(this);
      this.map = map;
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if (mapViewer.inScrollPane()) {
      ...
      if (ViewerViewportAlwaysVisible) {
        ...
        if (x < 0) x = 0; else if (x > getWidth() - 2) x = getWidth() - 2;
        if (y < 0) y = 0; else if (y > getHeight() - 2) y = getHeight() - 2;
      }
  ...
  public static DrawContext paintViewerIntoRectangle(
    Graphics g,
    final MapViewer viewer,
    final Rectangle pannerSize
  ) {
   ...
   */
  static DrawContext paintViewerIntoRectangle(
    final MapPanner panner,
    final Graphics g,
    final MapViewer viewer,
    ...
    if (viewer.getVisibleWidth() < 1 || viewer.getVisibleHeight() < 1) {
      if (DEBUG.Enabled) System.out.println(
        "MapPanner: paintViewerIntoRectangle: nothing to paint; visible size=" +
    ...
    if (ViewerViewportAlwaysVisible && viewer.inScrollPane()) {
      ...
      pannerRect = canvasRect.createUnion(allComponentBounds); else pannerRect =
        viewerRect.createUnion(allComponentBounds);
    } else pannerRect = allComponentBounds;
    ...
    double zoomFactor = ZoomTool.computeZoomFit(
      paintRect.getSize(),
      DEBUG.MARGINS ? 0 : MapMargin,
      pannerRect,
      offset
    );
    if (panner != null) panner.zoomFactor = zoomFactor;
    ...
    if (drawViewerReticle) {
      ...
      if (panner == null) dc.setAbsoluteStroke(3); else dc.setAbsoluteStroke(1);
