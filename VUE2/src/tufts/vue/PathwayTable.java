...
public class PathwayTable
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(PathwayTable.class);
  ...
  public PathwayTable(PathwayTableModel model) {
    ...
    if (showHeaders) {
      ...
      this.getTableHeader()
        .setPreferredSize(
          new Dimension(this.getTableHeader().getPreferredSize().width, 1)
        );
    ...
    for (int i = 0; i < colWidths.length; i++) {
      ...
      if (i == PathwayTableModel.COL_OPEN) col.setMaxWidth(26); else if (
        i == PathwayTableModel.COL_LABEL
  ...
  */
  ...
  private void handleValueChanged(ListSelectionEvent le) {
    ...
    if (DEBUG.PATHWAY) System.out.println(
      "PathwayTable: valueChanged: selected row " + row + ", col " + col
    );

    final LWPathway.Entry entry = tableModel.getEntry(row);
    if (DEBUG.PATHWAY) System.out.println(
      "PathwayTable: valueChanged: object at row: " + entry
    );
  ...
  private class ColorEditor
    ...
    public Component getTableCellEditorComponent(
      ...
      Component c = button.getTableCellRendererComponent(
        table,
        value,
        isSelected,
        true,
    ...
    public void mousePressed(MouseEvent e) {
      ...
      Color c = VueUtil.runColorChooser(
  ...
  private class ColorRenderer extends JPanel implements TableCellRenderer {
    ...
    private final Color TopGradient1 = new Color(
      179,
      166,
      121
...
//        	final LWPathway.Entry entryReal = getTableModel().getEntry(PathwayTable.this.getSelectedRow());
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      if (entry == null) {
        ...
        final LWPathway activePathway = VUE.getActivePathway();
        if (
          entry.pathway == activePathway &&
          entry.pathway.getCurrentEntry() == entry
        ) {
  ...
  private static Font ArrowIconFont = new Font(
    "Lucida Sans Unicode",
    Font.PLAIN,
    20
  );
  ...
  private class LabelRenderer extends DefaultTableCellRenderer {
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      if (Util.isUnixPlatform()) ArrowIconFont =
      ...
      if (entry.isPathway()) {
        ...
        if (col == PathwayTableModel.COL_OPEN) {
          ...
          if (entry.pathway.getEntries().isEmpty()) {
            if (entry.pathway.getEntries().isEmpty()) emptyString =
              VueResources.getString("pathways.empty.label");
          ...
          setFont(PathwayFont);
          if (VUE.getActiveEntry() == entry) setForeground(
            Color.black
        ...
        if (
          ...
          if (
            entry.node != null &&
            (entry.node.isFiltered() || entry.node.isHidden())
  ...
  private static final RoundRectangle2D BlobShape =
  ...
  private class GradientLabel extends JPanel {
    ...
    private void paintGradient(Graphics2D g) {
      Paint p = g.getPaint();
      if (path != null && path == VUE.getActivePathway()) g.setPaint(
        Gradient
      ); else g.setPaint(Gradient2);
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      if (entry.isPathway()) {
        ...
        if (col == PathwayTableModel.COL_VISIBLE) {
          ...
          if (
            entry.node == VUE.getActivePathway() &&
            entry.pathway.getCurrentEntry() == entry
          ) setBackground(selectedColor(entry)); else setBackground(BGColor);
        ...
        if (col == PathwayTableModel.COL_NOTES) {
          ...
          if (entry.hasVariableDisplayMode()) {
            ...
            if (obj instanceof Boolean) isMapView =
  ...
  public void drop(DropTargetDropEvent arg0) {
    ...
    try {
      entry =
        (LWPathway.Entry) transferable.getTransferData(
    ...
    if (entry != null) {
      ...
      if (
        !model
          .getPathwayForElementAt(dropRow)
          .equals(model.getPathwayForElementAt(rowAtPoint(arg0.getLocation())))
      ) {
        VueUtil.alert(
          this,
        ...
        if (
          dropIndex < 0 ||
          model.getPathwayIndexForElementAt(rowAtPoint(arg0.getLocation())) < 0
        ) {
          arg0.dropComplete(false);
          arg0.rejectDrop();
        } else {
  ...
  public void dragGestureRecognized(DragGestureEvent event) {
    ...
    try {
      ...
      if (entry.isPathway()) return; else dragSource.startDrag(
        event,
  ...
  private static final JMenuItem renamePresentation = new JMenuItem(
    VueResources.getString(context + "pathway.rename")
  );
  private static final JMenuItem playbackPresentation = new JMenuItem(
    VueResources.getString(context + "pathway.start")
  );
  private static final JMenuItem addNoteEntry = new JMenuItem(
    VueResources.getString(context + "pathway.notes")
  );
  private static final JMenuItem masterSlidePresentation = new JMenuItem(
    VueResources.getString(context + "pathway.masterslide")
  );
  private static final JMenuItem deletePresentation = new JMenuItem(
    VueResources.getString(context + "pathway.delete")
  );

  //entry menu items
  private static final JMenuItem deleteEntry = new JMenuItem(
    VueResources.getString(context + "entry.delete")
  );
  private static final JMenuItem previewEntry = new JMenuItem(
    VueResources.getString(context + "entry.preview")
  );
  private static final JMenuItem editEntry = new JMenuItem(
    VueResources.getString(context + "entry.edit")
  );
  private static final JMenuItem duplicateEntry = new JMenuItem(
    VueResources.getString(context + "entry.duplicate")
  );
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource().equals(renamePresentation)) {
      ...
      e.getSource().equals(editEntry) || e.getSource().equals(previewEntry)
    ) {
      ...
      GUI.invokeAfterAWT(
        new Runnable() {
          public void run() {
            ...
            if (dsd.getOkCanel()) VUE
              .getActivePathway()
              .remove(VUE.getActivePathway().getCurrentIndex());
          }
        }
      );
    }
  ...
  private JPopupMenu getPopup(MouseEvent e) {
    ...
    if (entry.isPathway()) {
      infoCheckBox.setText(
        VueResources.getString("pathwaytable.menu.pathwayinfo")
      );
      m.add(infoCheckBox);
      ...
      infoCheckBox.setText(
        VueResources.getString("pathwaytable.menu.slideinfo")
      );
      m.add(infoCheckBox);
      m.add(previewEntry);
      m.addSeparator();
  ...
  public void mousePressed(MouseEvent e) {
    ...
    if (entry.isPathway()) {
      if (
        col == PathwayTableModel.COL_VISIBLE ||
        col == PathwayTableModel.COL_OPEN ||
  ...
  private final boolean isDoubleClickEvent(MouseEvent e) {
    return (
      (e.getClickCount() > 1 && e.getClickCount() % 2 == 0) &&
      (e.getModifiers() & java.awt.event.InputEvent.BUTTON1_MASK) != // % 2 detects cascading double clicks (reported as a 4 click, 6 click, etc)
        0
    );
  }
