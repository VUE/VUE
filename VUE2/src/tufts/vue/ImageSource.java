...
class ImageSource {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ImageSource.class);
  ...
  public static ImageSource createIconSource(
    ImageSource is,
    ImageRep fullRep,
    java.awt.Image hardFullImage,
    int size
  ) {
  ...
  URI getIconKey(int size) {
    if (key == null) {
      ...
      if (DEBUG.IMAGE || DEBUG.WORK) Log.warn(
        "can't create icon key w/null key: " + this,
        new Throwable("HERE")
      ); else Log.warn("can't create icon key w/null key: " + this);
      return null;
    }
    if (isImageSourceForIcon()) throw new Error(
      "can't make icon key from another icon image source"
    );
  ...
  private ImageSource(
    ImageSource is,
    ImageRep softImageSource,
    java.awt.Image hardImageSource,
    int iconSize
  ) {
    if (iconSize <= 0) throw new IllegalArgumentException(
      "bad icon size " + iconSize
    );
  ...
  private ImageSource(URI cacheKey, int iconSize) {
    if (cacheKey == null) throw new NullPointerException("cacheKey");
  ...
  private ImageSource(Object original) {
    ...
    if (readable instanceof java.net.URL) {
      ...
      if (DEBUG.IMAGE) Log.debug(
        "CREATED NULL-KEY NON-CACHEABLE IMAGE SOURCE:" +
        "\n\treadable=" +
        Util.tags(readable) +
        "\n\toriginal=" +
        Util.tags(original),
  ...
  private static URI keyFromReadable(Object readable) {
    if (readable instanceof java.net.URL) {
      ...
      if (DEBUG.IMAGE) Log.debug(
        "readable " + Util.tags(readable) + " has no key; will not enter cache"
      );
      return null;
    }
  }
  ...
  void setCacheFile(File file) {
    if (_cacheFile == DO_NOT_CACHE_TO_DISK) throw new RuntimeException(
      "attempt to cache non-cacheable: " + this
    );
    _cacheFile = file;

    if (DEBUG.IMAGE) {
      if (resource != null) resource.setDebugProperty("image.cache", file);
    }
    //             if (resource != null)
  ...
  boolean useCacheFile() {
    if (
      isDiskCacheEntry() || _cacheFile == DO_NOT_CACHE_TO_DISK
    ) return false; else return true;
  }

  String debugName() {
    if (key == null) return "[null ImageSource.key]";

    try {
      // todo: put this in the image source?
      String basename = key.getPath();
      final int lastSlash = basename.lastIndexOf('/');
      if (lastSlash < (basename.length() - 2)) basename =
        basename.substring(lastSlash + 1, basename.length());
      return basename;
    } catch (Throwable t) {
      return "[" + t.toString() + "]";
    }
  ...
  public String toString() {
    ...
    //s.append("<<<");
    if (original instanceof Resource) s.append(original); else if (
      original instanceof URI
    ) {
      s.append("URI@");
      s.append(Util.tags(original.toString()));
    } else s.append(Util.tags(original));
    //s.append(">>>");
    ...
    if (readable != original) {
      ...
      if (readable instanceof File) {
        if (
          original instanceof Resource &&
          ((Resource) original).getImageSource() == readable
        ) {
          s.append("FF");
      ...
      } else s.append(Util.tags(readable));
    }
    if (iconSize > 0) {
      s.append("; ICON");
      s.append(iconSize);
  ...
  static URI makeIconKey(URI cacheKey, int size) {
    if (cacheKey == null) throw new IllegalArgumentException(
      "makeIconKey: null source key"
    );
    if (size <= 0) throw new IllegalArgumentException(
      "makeIconKey: bad size " + size
    );
    try {
      ...
      Util.printStackTrace(
        t,
        "can't make URI icon cache key from key " + cacheKey
      );
    }
    return null;
  }
