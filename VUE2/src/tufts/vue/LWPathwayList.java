 ...
 *
 * @version $Revision: 1.37 $ / $Date: 2010-02-03 19:17:41 $ / $Author: mike $
 * @author Scott Fraize
 *
 */

public class LWPathwayList
  implements LWComponent.Listener, Iterable<LWPathway> {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWPathwayList.class);
  ...
  public LWPathwayList(LWMap map) {
    ...
    LWPathway defaultPath = new LWPathway(
      map,
      VueResources.getString("pathways.defaultPathway.label")
    );
  ...
  public void setRevealer(LWPathway newRevealer) {
    if (mRevealer == newRevealer) return;
    Object old = mRevealer;
    if (mRevealer != null) mRevealer.setRevealer(false);
    mRevealer = newRevealer;
    if (newRevealer != null) newRevealer.setRevealer(true);
    notify(
      newRevealer,
      "pathway.revealer",
      new Undoable(old) {
        void undo() {
          setRevealer((LWPathway) old);
        }
      }
    );
  }
  ...
  public void setRevealerIndex(int i) {
    if (mInRestore) mRevealerIndex = i; else {
      if (i >= 0 && i < size()) mRevealer = (LWPathway) get(i); else mRevealer =
        null;
    }
  }
  ...
  void completeXMLRestore(LWMap map) {
    ...
    if (mActive == null && mPathways.size() > 0) setActivePathway(getFirst());
    mInRestore = false;
  ...
  private Object get(int i) {
    return mPathways.get(i);
  }

  public int size() {
    return mPathways.size();
  }

  public Iterator<LWPathway> iterator() {
    return mPathways.iterator();
  }

  //public List<LWPathway> getList() { return mPathways; } // todo: should be unmodifiable
  public int indexOf(Object o) {
    return mPathways.indexOf(o);
  }

  public LWPathway getFirst() {
    if (size() != 0) return (LWPathway) get(0); else return null;
  }

  public LWPathway getLast() {
    if (size() != 0) return (LWPathway) get(size() - 1); else return null;
  }
  ...
  public void remove(LWPathway p) {
    p.removeFromModel();
    if (!mPathways.remove(p)) throw new IllegalStateException(
      this + " didn't contain " + p + " for removal"
    );
    ...
    if (mActive == p) {
      if (getFirst() == null) VUE.setActive(
        LWPathway.class,
        this,
        null
      ); else setActivePathway(getFirst());
    }
    if (mRevealer == p) setRevealer(null);
  }
