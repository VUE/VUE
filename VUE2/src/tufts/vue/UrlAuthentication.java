...
public class UrlAuthentication implements edu.tufts.vue.dsm.DataSourceListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(UrlAuthentication.class);

  private static final Map<String, Map<String, String>> HostMap =
  ...
  public void changed(
    ...
    if (state == VueDataSourceManager.DS_CONFIGURED) {
      ...
      if (VueDataSourceManager.BLOCKING_OSID_LOAD) {
        for (edu.tufts.vue.dsm.DataSource ds : dataSources) scanForCredentials(
          ds
        );
      }

      if (true || DEBUG.Enabled) Log.info(
        "Done scanning for authentication keys: " + HostMap
      ); else Log.info("Done scanning for authentication keys.");
    }
  }
  ...
  public static Map<String, String> getRequestProperties(URL url) {
    ...
    if (url.getPort() > 0) key = url.getHost() + ":" + url.getPort(); else key =
      url.getHost();
  ...
  public static java.net.URLConnection getAuthenticatedConnection(URL url)
    ...
    final Map<String, String> sessionKeys =
      UrlAuthentication.getRequestProperties(url);

    if (debug) Log.debug(
      "opening URLConnection... (sessionKeys=" + sessionKeys + ") " + cleanURL
    );
    ...
    if (sessionKeys != null) {
      for (Map.Entry<String, String> e : sessionKeys.entrySet()) {
        if (debug) System.out.println(
          "\tHTTP request[" + e.getKey() + ": " + e.getValue() + "]"
        );
    ...
    if (debug) {
      ...
      for (Map.Entry<String, List<String>> e : rp.entrySet()) {
        System.out.println(
          "\toutbound HTTP header[" + e.getKey() + ": " + e.getValue() + "]"
        );
      }
    }
    ...
    if (debug) {
      ...
      if (response != null) System.out.format(
        "%20s: %s\n",
        "HTTP-RESPONSE",
        response
      );
      for (Map.Entry<String, List<String>> e : headers.entrySet()) {
        if (e.getKey() != null) System.out.format(
          "%20s: %s\n",
          e.getKey(),
          e.getValue()
        );
    ...
    try {
      if ("file".equals(url.getProtocol())) {
        if (DEBUG.IO) Log.debug(
          "Skipping auth checks for local access: " + url
        );
        return url;
      }
      final String asText = url.toString();
      ...
      final HttpURLConnection conn =
        (HttpURLConnection) cleanURL.openConnection();
