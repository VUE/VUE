 ...
 * @see LWPathway
 *
 * @version $Revision: 1.37 $ / $Date: 2010-02-03 19:17:40 $ / $Author: mike $
 * @author  Scott Fraize
 */
public class PathwayTool extends VueSimpleTool {
  ...
  public boolean handleMousePressed(MapMouseEvent e) {
    ...
    if (hit == null || activePathway == null) return false;

    if (hit.inPathway(activePathway)) activePathway.removeFirst(
      hit
    ); else activePathway.add(hit);
  ...
  private static class PathwayComboBoxModel
    extends DefaultComboBoxModel
    implements
      ActiveListener<LWMap>,
      LWComponent.Listener //implements VUE.ActiveMapListener, LWComponent.Listener
  {
    ...
    PathwayComboBoxModel() {
      ...
      if (DEBUG.TOOL) System.out.println(
        this + ": CURRENT PATHWAY AT INIT: " + current
      );
      if (current != null) setSelectedItem(current.getDisplayLabel());
    }
    ...
    private void setPathwayList(LWMap map) {
      ...
      if (map == null) pathwayList = null; else pathwayList =
        map.getPathwayList();

      if (mPathwayList != pathwayList) {
        if (mPathwayList != null) mPathwayList.removeListener(this);
        mPathwayList = pathwayList;
        if (mPathwayList != null) mPathwayList.addListener(this);
      }
      rebuildModel();
    }
    ...
    public void setSelectedItem(Object o) {
      if (rebuilding) {
        if (DEBUG.PATHWAY) System.out.println(
          this + " setSelectedItem IGNORED " + o
        );
        return;
      } else {
      ...
      if (mPathwayList != null) mPathwayList.setCurrentIndex(getIndexOf(o));
    }
