 ...
 *
 * @author Scott Fraize
 * @version $Revision: 1.18 $ / $Date: 2010-02-03 19:17:41 $ / $Author: mike $
 */
public final class MasterSlide extends LWSlide {

  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MasterSlide.class);

  static final String TitleLabel = "Slide Title Style";
  static final String TextLabel = "Slide Text Style";
  static final String URLLabel = "Link";
  ...
  public final void setParent(LWContainer parent) {
    if (parent instanceof LWPathway) {
      ...
      this + " master slide can't set parent to non pathway: " + parent
    );
  }
  ...
  void completeXMLRestore() {
    ...
    if (headerStyle == null) {
      ...
      for (LWComponent c : getChildren()) {
        ...
        if (
          "Sample Text".equals(c.getLabel()) || label.startsWith("Slide Text")
        ) {
          textStyle = c;
          ...
          Util.printStackTrace(
            this + "; Missed old-master-slide style object: " + c
          );
        }
      }
  ...
  private static final Font EditLabelFont = new Font(
    "Gill Sans",
    Font.PLAIN,
    72
  );
  private static final Color EditLabelColorDarkBG = new Color(
    255,
    255,
    255,
    64
  ...
  public void draw(DrawContext dc) {
    ...
    if (dc.focal == this) {
      ...
      if (mFillColor.brightness() < 0.5) dc.g.setColor(
        EditLabelColorDarkBG
      ); else dc.g.setColor(EditLabelColorLightBG);
      dc.g.drawString("Master Slide", 72 / 4, 72);
      if (true || DEBUG.Enabled) {
        //dc.g.setColor(Color.red);
        dc.g.drawString(
          getEntry().pathway.getLabel(),
          72 / 4,
          getHeight() - 72 / 4
        );
      }
  ...
  public String getLabel() {
    if (mXMLRestoreUnderway) return null; else {
      if (getEntry() != null && getEntry().pathway != null) return getEntry()
        .pathway.getLabel(); else return null;
    }
  }

  @Override
  public void setLabel(String label) {
    if (mXMLRestoreUnderway); // ignore
    else getEntry().pathway.setLabel(label);
  }
  ...
  protected void drawImpl(DrawContext dc) {
    drawChildren(dc);
    if (dc.focal == this) {
      headerStyle.drawLocal(dc.push());
      dc.pop();
      textStyle.drawLocal(dc.push());
      dc.pop();
      linkStyle.drawLocal(dc.push());
      dc.pop();
    }
  }

  @Override
  public java.util.List<LWComponent> getPickList(
    PickContext pc,
    java.util.List<LWComponent> stored
  ) {
    stored.clear();
