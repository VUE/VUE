...
public abstract class ImageRep implements /*ImageRef.Rep,*/Images.Listener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ImageRep.class);
  ...
  private static final Ref IMG_LOADING_AFTER_ERROR = new LoadingRef(
    "LOADING-POST-ERROR"
  );
  ...
  static ImageRep create(ImageRef ref, ImageSource src, boolean allowGC) {
    if (allowGC) return new ImageRep.Soft(
      ref,
      src
    ); else return new ImageRep.Hard(ref, src);
  }
  ...
  private static final class Hard extends ImageRep {
    ...
    Ref newRef(Image o) {
      if (o == null) Log.warn("HardRef to null " + this, new Throwable("HERE"));
  ...
  public static final ImageRep UNAVAILABLE = new ImageRep() {
    ...
    @Override
    protected Handle reconstitute(Object when) {
      error();
      return null;
    }

    @Override
    protected void cacheData(Images.Handle i, Object debug) {
      error();
    }

    @Override
    void renderRep(Graphics2D g, float width, float height) {
      fillRect(g, width, height, DEBUG.Enabled ? Color.orange : LoadingColor);
    }

    @Override
    public String toString() {
      return "REP.UNAVAILABLE";
    }
    ...
    private Object error() {
      throw new Error("constant-class");
    }
  };
  ...
  private ImageRep(ImageRef ref, ImageSource src) {
    if (src == null) throw new Error(
      "no ImageSource constructing rep for " + ref
    );
    _data = src;
    _ref = ref;
  }
  ...
  protected synchronized Handle reconstitute(final Object when) {
    if (_handle == IMG_ERROR) {
      ...
      if (DEBUG.IMAGE) debug(
        "skipping reconstitue: last load had error: " + this
      );
      return AT_ERROR;
    }
    ...
    if (DEBUG.IMAGE) debug(
      Util.TERM_CYAN + "RECONSTITUTE(" + when + ") " + Util.TERM_CLEAR + _data
    );
    ...
    if (when == LOAD_IMMEDIATE) {
      ...
      if (imageData == null) Log.warn("null on immediate request: " + this);
      ...
      if (DEBUG.Enabled && when != LOAD_NORMAL) Log.error(
        "bad recon code: " + Util.tags(when)
      );
    ...
    if (_handle == oldHandle) {
      ...
      if (DEBUG.Enabled) debug(
        "*** got immediate callback w/image, handle is now " + _handle
      );
    }
  ...
  private void debug(String s, boolean dumpStack) {
    String msg = String.format(
      ...
      ImageRef.debugSRC(_data),
      s
    );
    if (dumpStack) Log.debug(msg, new Throwable("HERE")); else Log.debug(msg);
  }

  private void setHandle(Ref r, String debug) {
    if (DEBUG.IMAGE || DEBUG.WORK) {
      String t;
      if (r instanceof NullRef && !(r instanceof ProgressRef)) t =
        r.toString(); else t = Util.tags(r);
  ...
  public void gotImageProgress(Object imageSrc, long bytesSoFar, float pct) {
    ...
    if (handle.getClass() == ProgressRef.class) {
      ...
      handle.isLoader() && pct > 0
      /*&& pct < Float.POSITIVE_INFINITY*/
    ) { // todo: should never see infinity
  ...
  public void gotImageError(Object imageSrc, String msg) {
    ...
    if (_ref == null) Log.warn(
      "rep w/out null ref: " + this + "; error=" + msg
    ); else _ref.notifyRepHasArrived(this, null);
  ...
  private void drawStatus(
    ...
    if (status == IMG_ERROR) {
      ...
      status == IMG_ERROR_MEMORY || status == IMG_LOADING_AFTER_ERROR
    ) {
      g.setColor(LowMemoryColor);
    } else if (DEBUG.Enabled) {
      if (status == IMG_LOADING) g.setColor(LoadingColor); else g.setColor(
        Color.green
      ); // no status yet
    } else {
  ...
  private void drawPartialProgress(
    Graphics2D g,
    float progress,
    float width,
    float height
  ...
  private static final class HardRef<T> implements Ref<T> {
    ...
    /*@Override*/public boolean isLoader() {
      return false;
    }
  ...
  private static final class SoftRef<T>
    ...
    @Override
    public String toString() {
      if (get() == null) return "SOFT(GC'd)"; else return "SOFT";
    }
