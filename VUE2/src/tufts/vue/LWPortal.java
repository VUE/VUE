...
public class LWPortal extends LWNode {
  ...
  protected boolean containsImpl(final float x, final float y, PickContext pc) {
    if (pc.isZoomRollover) return false; // allow picking through the portal -- never zoom portals
    else return super.containsImpl(x, y, pc);
  }
  ...
  public Color getRenderFillColor(DrawContext dc) {
    if (mFillColor.isTransparent()) return (
        getMap().mFillColor.brightness() > 0.5
      )
      ? DarkFill
      : LightFill; else return getFillColor();
  ...
  protected void drawImpl(DrawContext dc) {
    ...
    if (dc.focal == this || (printing && dc.focused == this)) {
      ...
      if (DEBUG.CONTAINMENT) {
        ...
        dc.g.setTransform(zeroTransform);
        if (true || this.stroke == STROKE_ZERO) dc.setAbsoluteStroke(
          2
        ); else dc.g.setStroke(this.stroke);
      ...
      if (dc.zoom > PathwayOnTopZoomThreshold) {
        dc.g.setStroke(
          new java.awt.BasicStroke(LWPathway.PathBorderStrokeWidth)
        );
        dc.g.draw(getZeroShape());
      } else {
