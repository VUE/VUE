...
public class Archive {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Archive.class);
  ...
  public static boolean isVueIMSCPArchive(File file) {
    ...
    try {
      ...
      return (
        zipFile.getEntry(IMSCP.MAP_FILE) != null &&
        zipFile.getEntry(IMSCP.MANIFEST_FILE) != null
      );
    } catch (Throwable t) {
      Log.warn(t);
      return false;
  ...
  public static LWMap openVuePackage(final File zipFile)
    ...
    if (
      false && canCreateFiles(parentDirectory)
    ) unpackingDir = parentDirectory.toString(); else unpackingDir = // for now, always unpack into the temp directory
      VUE.getSystemProperty("java.io.tmpdir");
    ...
    while ((entry = zin.getNextEntry()) != null) {
      ...
      if (comment != null) {
        if (comment.startsWith(MAP_ARCHIVE_KEY)) {
          ...
          mapFile = location;
          Log.info(
            "Identified map entry: " + comment + " (" + entry.getName() + ")"
          );
          //Log.info("Found map: " + entry + "; at " + location);
        } else {
          String spec = comment.substring(
            comment.indexOf(SPEC_KEY) + SPEC_KEY_LEN
          );
          Log.info("             [" + spec + "]"); // todo: revert to debug level eventually
          if (packagedResources.put(spec, location) != null) Log.warn(
            "repeated resource spec in archive! [" + spec + "]"
          );
          //Log.debug("       spec= " + spec);
        }
      } else {
    ...
    final LWMap map = ActionUtil.unmarshallMap(
      new File(mapFile),
      new ArchiveMapUnmarshalHandler(
        zipFile + "(" + mapEntry + ")",
        zipFile,
        packagedResources
      )
  ...
  private static class ArchiveMapUnmarshalHandler extends MapUnmarshalHandler {
    ...
    ArchiveMapUnmarshalHandler(
      Object source,
      File archiveFile,
      Map<String, String> resourcesFoundInPackage
    ) {
      ...
      for (Resource r : map.getAllResources()) {
        ...
        if (packageCacheFile != null) {
          ...
          if (DEBUG.Enabled) Log.debug(
            "patching packaged resource: " + packageCacheFile + "; into " + r
          );
          ...
          if (DEBUG.RESOURCE && !localPath.equals(packageCacheFile)) Log.info(
            "       localized file path: " + localPath
          );
          ...
          if (r instanceof URLResource) {
            ...
            Log.warn(
              "package file fallback for unknown resource type, impl in question: " +
              Util.tags(r) +
              "; " +
              localFile
            );
          ...
          if (DEBUG.Enabled) Log.debug(
            "No archive entry matching: " + r.getSpec()
          );
        }
      }
    }
   ...
   */
  public static String unzipEntryToFile(
    ZipInputStream zin,
    ZipEntry entry,
    String location
  ) throws IOException {
    ...
    if (location == null) {
      ...
      if (location.endsWith(File.separator)) filename =
        location + entry.getName(); else filename =
        location + File.separator + entry.getName();
    }
  ...
  public static LWMap loadVueIMSCPArchive(File file)
    throws java.io.FileNotFoundException, java.util.zip.ZipException, java.io.IOException {
    ...
    if (resourceFolder.exists() || resourceFolder.mkdir()) {
      ...
      while ((e = zin.getNextEntry()) != null) {
        ...
        if (
          !e.getName().equalsIgnoreCase(IMSCP.MAP_FILE) &&
          !e.getName().equalsIgnoreCase(IMSCP.MANIFEST_FILE)
        ) {
  ...
  private static void setComment(ZipEntry entry, String comment) {
    ...
    try {
      ...
      Log.warn(
        "Couldn't " +
        COMMENT_ENCODING +
        " encode 'extra' bytes into ZipEntry comment; " +
        entry +
        "; [" +
        comment +
        "]",
  ...
  public static String getComment(ZipEntry entry) {
    ...
    if (extra != null && extra.length > 0) {
      if (DEBUG.IO && DEBUG.META) Log.debug(
        "getComment found " + extra.length + " extra bytes"
      );
      try {
        ...
        Log.warn(
          "Couldn't " +
          COMMENT_ENCODING +
          " decode 'extra' bytes from ZipEntry comment; " +
          entry,
          t
        );
   ...
   * on future calls.
   */
  private static String generatePackageFileName(
    Resource r,
  ...
  private static String generateInformativePackageFileName(Resource r)
    ...
    if (DEBUG.IO) Log.debug(
      "Generating package file name from " + r + "; " + r.getProperties()
    );
    ...
    try {
      if (r.hasProperty(PACKAGE_FILE)) {
        ...
        if (DEBUG.IO) Log.debug(
          "Using pre-existing package file name: " + name
        );
        return name;
      }
    } catch (Throwable t) {
    ...
    if (imageSource == null) return r.getSpec(); // failsafe
    ...
    if (imageSource instanceof File) {
      ...
      if (
        r.isImage() &&
        r.hasProperty(IMAGE_FORMAT) &&
        !Resource.looksLikeImageFile(packageName)
      ) packageName += "." + r.getProperty(IMAGE_FORMAT).toLowerCase();
    } else {
      throw new IllegalArgumentException(
        "image source is neither URL or File: " + Util.tags(imageSource)
      );
    }
  ...
  private static class Item {
    ...
    public String toString() {
      return (
        "Item[" + entry.toString() + "; " + resource + "; " + dataFile + "]"
      );
    }
  }
  ...
  public static void writeArchive(LWMap map, File archive)
    ...
    final String mapName;
    if (label.endsWith(VueUtil.VueArchiveExtension)) mapName =
      label.substring(0, label.length() - 4); else mapName = label;
    ...
    for (Resource r : uniqueResources) {
      try {
        ...
        final String packageEntryName = generatePackageFileName(
          r,
          uniqueEntryNames
        );
    ...
    final ZipEntry mapEntry = new ZipEntry(
      dirName + "/" + mapName + "$map.vue"
    );
    final String comment =
      ...
      items.size() +
      ";" +
      ">"//+ "\n\tmap-name(" + mapName + ")" // /usr/bin/what terminatior
    ...
    for (Item item : items) {
      if (DEBUG.Enabled) Log.debug("writing: " + item); else Log.info(
        "writing: " + item.entry
      );
  ...
  private static void copyBytesToZip(File file, ZipOutputStream zos)
    throws java.io.IOException {
    final BufferedInputStream fis = new BufferedInputStream(
