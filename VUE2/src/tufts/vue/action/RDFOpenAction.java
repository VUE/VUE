...
public class RDFOpenAction extends VueAction {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(RDFOpenAction.class);
  ...
  public static LWMap loadMap(String fileName) {
    try {
      ...
      if (mapName.length() == 0) mapName = "RDF Import";
      ...
      while (nIterator.hasNext()) {
        ...
        if (rdfNode instanceof com.hp.hpl.jena.rdf.model.Resource) {
          com.hp.hpl.jena.rdf.model.Resource nodeResource =
            (com.hp.hpl.jena.rdf.model.Resource) rdfNode;
      ...
      while (iter.hasNext()) {
        ...
        if (stmtObject instanceof com.hp.hpl.jena.rdf.model.Resource) {
          ...
          LWNode node2 = hashMap.get(
            ((com.hp.hpl.jena.rdf.model.Resource) stmtObject).getURI()
          );
          LWLink link = new LWLink(node1, node2);
          map.add(link);
        } else if (stmtObject instanceof com.hp.hpl.jena.rdf.model.Literal) {
          tufts.vue.Resource mapResource = hashMap
            .get(stmtSubject.getURI())
            .getResource();
          if (mapResource != null) mapResource.setProperty(
            stmtProperty.getLocalName(),
            stmtObject
          );
        }
      }
      ...
      VueUtil.alert(
        ...
        fileName +
        "\"" +
        VueResources.getString("rfdopenaction.openfile.error"),
        VueResources.getString("rfdopenaction.openfile.title")
      );
      e.printStackTrace();
    }
  ...
  private static LWNode createNodeFromResource(
    ...
    try {
      ...
      if (uri != null) resource =
        tufts.vue.Resource.getFactory().get(uri); else Log.warn(
        "null URI: " + tufts.Util.tags(r)
      );
    } catch (Throwable t) {
    ...
    if (resource == null) {
      if (uri != null) node = new LWNode("Resource Error; URI [" + uri + "]");
    } else {
      ...
      if (labelDisplay.equals("LABEL")) {
        ...
        com.hp.hpl.jena.rdf.model.Property labelOf = index.createProperty(
          edu.tufts.vue.rdf.RDFIndex.VUE_ONTOLOGY,
          edu.tufts.vue.metadata.Constants.LABEL
        );
  ...
  public static final int STAGGERED_FLOW = 1;
  public static final int nodeLayout = STAGGERED_FLOW;

  private static void placeNode(LWNode node) {
    if (nodeLayout == CIRCLE) {
      double angle = Math.random() * Math.PI * 4;
      node.setLocation(
        200 + 200 * Math.cos(angle),
        200 + 200 * Math.sin(angle)
      );
    }
    if (nodeLayout == STAGGERED_FLOW) {
      ...
      if (name.length() > NODE_LABEL_TRUNCATE_LENGTH) name =
        name.substring(0, NODE_LABEL_TRUNCATE_LENGTH) + "...";
  ...
  public static void main(String args[]) throws Exception {
    ...
    VUE.parseArgs(args);
    LWMap map;
    if (file.indexOf(':') >= 0) map =
      OpenAction.loadMap(new java.net.URL(file)); else map =
      OpenAction.loadMap(file);
