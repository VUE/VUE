...
public class OpenAction extends VueAction {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(OpenAction.class);
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    try {
      File[] file = ActionUtil.openMultipleFiles(
        "Open Map",
      ...
      for (int i = 0; i < file.length; i++) displayMap(file[i]);
  ...
  public static LWMap loadMap(java.net.URL url) {
    try {
      ...
      VueUtil.alert(
        null,
        VueResources.getString("openaction.mapopen.error"),
        VueResources.getString("openaction.mapopen.title")
      );
  ...
  public static LWMap loadMap(String filename) {
    ...
    try {
      ...
      VueUtil.alert(
        ...
        filename +
        "\"" +
        VueResources.getString("openaction.mapnotfound.error"),
        VueResources.getString("openaction.mapnotfound.title")
      );
      ...
      VueUtil.alert(
        String.format(message, filename, Util.formatLines(exception, 80)),
  ...
  public static void main(String args[]) throws Exception {
    ...
    for (String arg : args) {
      ...
      Throwable tx = null;
      try {
        if (arg.indexOf(':') >= 0) map =
          OpenAction.loadMap(new java.net.URL(arg)); else map =
          OpenAction.doLoadMap(arg);
      ...
      if (map == null && tx != null) {
        if (tx.getCause() != null) result = tx.getCause(); else result = tx;
      } else result = map;
