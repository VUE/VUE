...
public class TextOpenAction extends VueAction {
  ...
  public static LWMap loadMap(String fileName) throws Exception {
    ...
    if (
      !ZOTERO_PROTOTYPE
    ) { // in zotero import each row is a node
    ...
    while ((line = reader.readLine()) != null && count < MAX_SIZE) {
      ...
      if (ZOTERO_PROTOTYPE) {
        ...
        if (parts.length > 1) links = parts[1]; else links = null;
      ...
      if (layout == CIRCLE_LAYOUT) {
        ...
        node1.setLocation(
          MAP_SIZE * (1 + Math.cos(angle)),
          MAP_SIZE * (1 + Math.sin(angle))
        );
        ...
        node1.setLocation(
          MAP_SIZE * (1 + Math.cos(angle) / fact1),
          MAP_SIZE * (1 + Math.sin(angle) / fact1)
        );
        angle = Math.random() * Math.PI * 4;
        ...
        node2.setLocation(
          MAP_SIZE * (1 + Math.cos(angle) / fact2),
          MAP_SIZE * (1 + Math.sin(angle) / fact2)
        );
      }
    ...
    if (ZOTERO_PROTOTYPE) {
      ...
      while (nodes.hasNext()) {
        ...
        if (DEBUG_LOCAL) {
          System.out.println(
            "TextOpenAction: linkslist arr length " + arr.length
          );
        }
