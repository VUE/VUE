...
public class ActionUtil {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ActionUtil.class);

  private static final String XML_MAPPING_CURRENT_VERSION_ID =
    VueResources.getString("mapping.lw.current_version");
  private static final URL XML_MAPPING_DEFAULT = VueResources.getURL(
    "mapping.lw.version_" + XML_MAPPING_CURRENT_VERSION_ID
  );
  private static final URL XML_MAPPING_UNVERSIONED = VueResources.getURL(
    "mapping.lw.version_none"
  );
  private static final URL XML_MAPPING_OLD_RESOURCES = VueResources.getURL(
    "mapping.lw.version_resource_fix"
  );

  private static final String VUE_COMMENT_START = "<!-- Do Not Remove:";
  private static final String OUTPUT_ENCODING = "US-ASCII";
  private static final String DEFAULT_WINDOWS_ENCODING = "windows-1252"; // (a.k.a Cp1252) for reading pre ASCII enforced save files from Windows
  private static final String DEFAULT_MAC_ENCODING = "UTF-8"; // "MacRoman" not supported on Windows platform
   ...
   * and right-quote).
   */
  private static final String DEFAULT_INPUT_ENCODING = "UTF-8"; // safest default input encoding
  ...
  public static File selectFile(String title, final String fileType) {
    ...
    if (
      fileType != null && !fileType.equals("export")
    ) saveChooser.setFileFilter(new VueFileFilter(fileType)); else if (
      fileType != null && fileType.equals("export")
    ) {
      saveChooser.addChoosableFileFilter(
        new VueFileFilter(VueFileFilter.JPEG_DESCRIPTION)
      );
      saveChooser.addChoosableFileFilter(
        new VueFileFilter(VueFileFilter.PNG_DESCRIPTION)
      );
      saveChooser.addChoosableFileFilter(
        new VueFileFilter(VueFileFilter.SVG_DESCRIPTION)
      );
      saveChooser.addChoosableFileFilter(
        new VueFileFilter(VueFileFilter.IMS_DESCRIPTION)
      );
      saveChooser.addChoosableFileFilter(
        new VueFileFilter(VueFileFilter.IMAGEMAP_DESCRIPTION)
      );
      //  chooser.addChoosableFileFilter(new VueFileFilter(VueFileFilter.ZIP_DESCRIPTION));
    } else {
      VueFileFilter defaultFilter = new VueFileFilter(
        VueFileFilter.VUE_DESCRIPTION
      );

      saveChooser.addChoosableFileFilter(defaultFilter);
      saveChooser.addChoosableFileFilter(
        new VueFileFilter(VueFileFilter.VPK_DESCRIPTION)
      );
      ...
      new PropertyChangeListener() {
        public void propertyChange(PropertyChangeEvent arg0) {
          if (
            ...
            VueFileChooser.FILE_FILTER_CHANGED_PROPERTY
          ) {
            adjustExtension();
          }
        }
    ...
    if (option == VueFileChooser.APPROVE_OPTION) {
       ...
       */
      String extension =
        ((VueFileFilter) saveChooser.getFileFilter()).getExtensions()[0];
      ...
      if (picked.exists()) {
        int n = VueUtil.confirm(
          null,
          VueResources.getString("replaceFile.text") +
          " \'" +
          picked.getName() +
          "\'",
          VueResources.getString("replaceFile.title"),
      ...
      if (picked != null) VueUtil.setCurrentDirectoryPath(picked.getParent());
    }

    return picked;
  }

  private static final void adjustExtension() {
    String name = null;
    ...
    if (name == null || (name != null && name.length() < 1)) baseName =
      VUE.getActiveMap().getLabel(); else baseName = name;

    if (name == null || (name != null && name.length() < 1)) {
      if (baseName.lastIndexOf(".") > 0) baseName =
    ...
    if (f.exists()) baseName =
      baseName + "-copy" + "." + extension; else baseName =
      baseName + "." + extension;
  ...
  public static File openFile(String title, String extension) {
    ...
    if (option == VueFileChooser.APPROVE_OPTION) {
      ...
      if (chooserFile == null) return null;
      final String fileName;
      final String chosenPath = chooserFile.getAbsolutePath();

      // if they type a file name w/out an extension
      if (chooserFile.getName().lastIndexOf('.') < 0) fileName =
        chosenPath + "." + extension; else fileName = chosenPath;
      ...
      if (file.exists()) {
        ...
        if (dir.exists() && dir.isDirectory()) {
          ...
          tufts.vue.VueUtil.alert(
            chooser,
            VueResources.getString("actionutil.filenotfound.error") +
            " " +
            file,
            VueResources.getString("actionutil.filenotfound.error")
          );
  ...
  public static File[] openMultipleFiles(String title, String extension) {
    ...
    if (option == VueFileChooser.APPROVE_OPTION) {
      ...
      if (chooserFile.length == 1) {
        ...
        // if they type a file name w/out an extension
        if (chooserFile[0].getName().lastIndexOf('.') < 0) fileName =
          chosenPath + "." + extension; else fileName = chosenPath;

        chooserFile[0] = new File(fileName);

        if (chooserFile[0].exists()) {
          VueUtil.setCurrentDirectoryPath(
          ...
          if (dir.exists() && dir.isDirectory()) {
            ...
            Log.debug(
              "File '" + chosenPath + "' " + file + " can't  be found."
            );
            tufts.vue.VueUtil.alert(
              chooser,
              VueResources.getString("actionutil.filenotfound.error") +
  ...
  public static Mapping getDefaultMapping() {
    ...
    if (result instanceof Exception) {
      VueUtil.alert(
        null,
        VueResources.getString("dialog.mappingfile.message") +
        "\n" +
        VueResources.getString("dialog.mappingurl.message") +
        XML_MAPPING_DEFAULT +
        "\n" +
        result,
        VueResources.getString("dialog.mappingfile.title"),
  ...
  public static Unmarshaller getDefaultUnmarshaller(
    ...
    if (mapping == null) mapping = getDefaultMapping();
    ...
    unmarshaller.setUnmarshalListener(
      new MapUnmarshalHandler(sourceName, "DEFAULT(" + sourceName + ")")
    );
    unmarshaller.setMapping(mapping);

    if (DEBUG.CASTOR || DEBUG.XML || DEBUG.IO) Log.debug(
      "got default unmarshaller for mapping " +
      mapping +
      " source " +
      sourceName
    );
  ...
  private static Mapping getMapping(URL mappingSource) {
    ...
    if (result instanceof Mapping) return (Mapping) result; else return null;
  }
  ...
  private static Object _loadMapping(
    URL mappingSource
  ) //throws java.io.IOException //, org.exolab.castor.mapping.MappingException
  {
    if (
      !DEBUG.CASTOR && LoadedMappings.containsKey(mappingSource)
    ) return (Mapping) LoadedMappings.get(mappingSource);
    Mapping mapping = new Mapping();
    if (DEBUG.IO || DEBUG.INIT) Log.debug(
      "Loading mapping " + mappingSource + "..."
    );
    try {
  ...
  public static class VueMarshalListener implements MarshalListener {
    ...
    public boolean preMarshal(Object o) {
      ...
      try {
        ...
        String key = null;
        if (
          o.getClass() == tufts.vue.PropertyEntry.class
        ) key = ((tufts.vue.PropertyEntry) o).getEntryKey(); // is final class
  ...
  public static boolean isCastorCompatible(LWMap map) {
    try {
      ...
      tufts.vue.VueUtil.alert(
        VueResources.getString("actionutil.filesave.error") + t,
        VueResources.getString("actionutil.filesave.title")
      );
  ...
  public static void marshallMap(File targetFile, LWMap map) {
    ...
    try {
      ...
      if (t instanceof WrappedMarshallException) t = t.getCause();
      Log.error(
        "marshalling: " +
        map +
        "; to " +
    ...
    if (targetFile.exists()) {
      ...
      try {
        ...
        if (
          backup.delete()
        ) Log.info("  deleted prior backup: " + backup); // Required on Win32 or rename will fail
        Log.info("renaming old to backup: " + backup);
        if (!targetFile.renameTo(backup)) Log.warn(
          "failed to make backup of " + targetFile
        );
      } catch (Throwable t) {
    ...
    if (!tmpFile.renameTo(targetFile)) {
      ...
      VueUtil.alert(
        String.format(
          Locale.getDefault(),
          VueResources.getString("actionutil.rename.error"),
          tmpFile,
          targetFile
        ),
  ...
  private static void doMarshallMap(
    final File targetFile,
    final File tmpFile,
    final LWMap map
  )
    throws java.io.IOException, org.exolab.castor.xml.MarshalException, org.exolab.castor.xml.ValidationException, org.exolab.castor.mapping.MappingException {
    ...
    try {
      ...
      Log.warn(
        "No FileDescriptor for " +
        path +
        "; failsafe sync will be skipped: " +
        t
      );
    }
    ...
    if (DEBUG.IO) {
      try {
        Log.debug(
          String.format(
            "%s; %s; encoding: \"%s\", which will represent \"%s\" XML content",
            tmpFile,
            writer,
            writer.getEncoding(),
            OUTPUT_ENCODING
   ...
   */
  private static void marshallMapToWriter(
    final Writer writer,
    final LWMap map,
    final File targetFile,
    final File tmpFile
    ...
    String name = "";
    if (targetFile != null) name = targetFile.getName(); else name =
      map.getLabel();
    if (name == null) name = "";

    final java.util.Date date = new java.util.Date();
    final String today = new java.text.SimpleDateFormat("yyyy-MM-dd")
      .format(date);

    String headerText =
      VueResources.getString("vue.version") +
      " concept-map (" +
      name +
      ") " +
      today;
    ...
    headerText =
      org.apache.commons.lang.StringEscapeUtils.escapeXml(headerText);
    ...
    writer.write(
      ...
      " VUE mapping " +
      "@version(" +
      XML_MAPPING_CURRENT_VERSION_ID +
      ")" +
      " " +
      XML_MAPPING_DEFAULT +
      " -->\n"
    ...
    writer.write(
      ...
      date +
      " by " +
      VUE.getSystemProperty("user.name") +
      " on platform " +
      VUE.getSystemProperty("os.name") +
      " " +
      VUE.getSystemProperty("os.version") +
      " in JVM " +
      VUE.getSystemProperty("java.runtime.version") +
      " -->\n"
    );
    writer.write(
      VUE_COMMENT_START +
      " Saving version " +
      tufts.vue.Version.WhatString +
      " -->\n"
    );
    ...
    try {
      ...
      writer.flush();
      if (DEBUG.Enabled) Log.debug(
        "marshalled " + map + " to " + writer + "; file=" + tmpFile
      );
    } catch (Throwable t) {
      ...
      VueUtil.alert(
        VueResources.getString("actionutil.filecorrupted.error") +
        "\n\n" +
      ...
      try {
        ...
        VueUtil.alert(
          Util.formatLines(tx.toString(), 80),
          VueResources.getString("actionutil.internalsave.title")
        );
  ...
  public static LWMap unmarshallMap(File file) throws IOException {
  ...
  public static LWMap unmarshallMap(File file, MapUnmarshalHandler handler)
    throws IOException {
    if (file.isDirectory()) throw new MapException(
      "is a directory, not a map file: " + file
    );
    if (!file.exists()) throw new FileNotFoundException("does not exist");
  ...
  private static MapReader getMapReaderForURL(
    URL url,
    ...
    if (file != null && file.isDirectory()) throw new MapException(
      "cannot open " + file + ": is directory"
    );
    ...
    try {
      if (file != null) {
        if (charsetEncoding != null) reader =
          new InputStreamReader(
            new FileInputStream(file),
            charsetEncoding
          ); else reader = new FileReader(file); // could default to UTF-8
      } else {
        // No local file was found: it must have been a remote URL

        if (allowRedirects) {
          final URL redirectURL = tufts.vue.UrlAuthentication.getRedirectedUrl(
            url,
            10
        ...
        if (charsetEncoding != null) reader =
          new InputStreamReader(
            UrlAuthentication.getAuthenticatedStream(url),
            charsetEncoding
          ); else reader =
          new InputStreamReader(UrlAuthentication.getAuthenticatedStream(url)); // could default to UTF-8
      }
    } catch (Throwable t) {
  ...
  public static LWMap unmarshallMap(java.net.URL url) throws IOException {
    return unmarshallMap(url, null);
  }

  public static LWMap unmarshallMap(
    java.net.URL url,
    MapUnmarshalHandler handler
  ) throws IOException {
    ...
    final BufferedReader reader = getMapReaderForURL(
      url,
      BOOTSTRAP_ENCODING,
      false
    )
    ...
    for (;;) {
      ...
      if (line.startsWith(VUE_COMMENT_START + " Saved")) {
        ...
        if (line.indexOf("platform Windows") > 0) {
          if (DEBUG.IO) Log.debug(
            url + " was saved in the Windows environment"
          );
        ...
        if (savingVersionIndex > 0) {
          ...
          if (savingVersion.indexOf("-->") > 10) savingVersion =
            savingVersion.substring(0, savingVersion.indexOf("-->"));
      ...
      if ((idx = line.indexOf("@version(")) >= 0) {
        ...
        if (x > 0) {
          ...
          if (DEBUG.CASTOR || DEBUG.IO) Log.debug(
            url + "; Found mapping version ID[" + versionID + "]"
          );
          if (versionID.equals(XML_MAPPING_CURRENT_VERSION_ID)) {
            mapping = getDefaultMapping();
          } else {
            URL mappingURL = VueResources.getURL(
              "mapping.lw.version_" + versionID
            );
            if (mappingURL == null) {
              Log.error(
                "Failed to find mapping for version tag [" +
                versionID +
                "], attempting default."
              );
              mapping = getDefaultMapping();
      ...
      if (firstNonCommentLine.indexOf("encoding=\"UTF-8\"") > 0) {
        ...
        if (Util.isWindowsPlatform()) {
          if (savedOnWindowsPlatform) {
            localEncoding = true;
          } else if (savedOnMacPlatform) guessedEncoding =
            DEFAULT_MAC_ENCODING; else guessedEncoding =
            DEFAULT_WINDOWS_ENCODING;
        } else if (Util.isMacPlatform()) {
          if (savedOnMacPlatform) localEncoding = true; else if (
            savedOnWindowsPlatform
          ) guessedEncoding = DEFAULT_WINDOWS_ENCODING; else guessedEncoding =
            DEFAULT_MAC_ENCODING;
        }

        if (localEncoding) guessedEncoding = Util.getDefaultPlatformEncoding();
    ...
    final String encoding = guessedEncoding == null
  ...
  private static LWMap unmarshallMap(
  ...
  throws IOException {
    ...
    for (;;) {
      ...
      if (DEBUG.META && (DEBUG.CASTOR || DEBUG.IO)) Log.debug(
        "Skipping[" + line + "]"
      );
    }
    ...
    try {
      ...
      if (mapHandler == null) mapHandler =
        new MapUnmarshalHandler(url, tufts.vue.Resource.MANAGED_UNMARSHALLING); // managed is the default
      ...
      try {
        ...
        if (
          ...
          me
              .getMessage()
              .indexOf("tufts.vue.Resource can no longer be constructed") >=
            0
        ) {
          Log.warn("ActionUtil.unmarshallMap: " + me);
          Log.warn(
            "Attempting specialized MapResource mapping for old format."
          );
          // NOTE: delicate recursion here: won't loop as long as we pass in a non-null mapping.
          return unmarshallMap(
            url,
            getMapping(XML_MAPPING_OLD_RESOURCES),
            charsetEncoding,
            false,
            savingVersion,
            mapHandler
      ...
      tufts.Util.printStackTrace(
        e,
        "Exception restoring map from [" + url + "]: " + e.getClass().getName()
      );
   ...
   */public static final class OLD_MERGE_MAP_STUB extends tufts.vue.LWMap {
    static {
      if (true) throw new Error("should-not-classload");
    }
  }
}
...
class MapUnmarshalHandler implements UnmarshalListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MapUnmarshalHandler.class);
  ...
  MapUnmarshalHandler(Object source, Object context) {
    this.source = source;
    if (context == null) this.context = CONTEXT_NONE; else this.context =
      context;
  }
  ...
  void notifyVersionOfVueThatSavedMap(String savingVersion) {
    ...
    if (map.getModelVersion() > LWMap.getCurrentModelVersion()) {
      VueUtil.alert(
        ...
        "\n" +
        "        " +
        VueResources.local("actionutil.notifyversion.vuebuilt") +
        " " +
        tufts.vue.Version.AllInfo +
        " (public v" +
        VueResources.local("vue.version") +
        ")" +
        "\n" +
        "\n" +
        ...
        String.format(
          Locale.getDefault(),
          VueResources.local("actionutil.notifyversion.versionwarning"),
          fileName
        )
      );
      ...
      if (DEBUG.DATA && DEBUG.META) map.setLabel(
        "|" + map.getModelVersion() + "| " + map.getLabel()
      );
    }
  ...
  public void fieldAdded(String name, Object parent, Object child) {
    if (DEBUG.XML) {
      ...
      if (DEBUG.META) {
        ...
        if (child instanceof String) Log.debug(
          field + Util.TERM_RED + '"' + child + '"' + Util.TERM_CLEAR
        ); else Log.debug(
          field + Util.TERM_PURPLE + Util.tags(child) + Util.TERM_CLEAR
        );
...
final class XMLObjectFactory
  extends org.exolab.castor.util.DefaultObjectFactory {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(XMLObjectFactory.class);
  ...
  public Object createInstance(
    ...
    if (_type != type) Log.info(
      "request for instance of " + _type.getName() + " yields: " + type
    );
    ...
    if (DEBUG.Enabled) {
      if (
        ...
        DEBUG.XML ||
        DEBUG.CASTOR ||
        (DEBUG.RESOURCE && DEBUG.META && o instanceof tufts.vue.Resource)
      ) {
