...
public class SaveAction extends VueAction {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(SaveAction.class);
  ...
  public void actionPerformed(ActionEvent e) {
    if (
      inSave
    ) return; // otherwise rapid Ctrl-S's will trigger multiple dialog boxes

    try {
      inSave = true;
      Log.info("Action[" + e.getActionCommand() + "] invoked...");
      if (saveMap(tufts.vue.VUE.getActiveMap(), isSaveAs(), export)) Log.info(
        "Action[" + e.getActionCommand() + "] completed."
      ); else Log.info("Action[" + e.getActionCommand() + "] aborted.");
    } finally {
      inSave = false;
    }
  }
  ...
  public static boolean saveMap(LWMap map, boolean saveAs, boolean export) {
    ...
    if (map.getSaveFileModelVersion() == 0) {
      ...
      response =
        VueUtil.option(
          VUE.getDialogParent(),
          VueResources.getFormatMessage(
            messageObject,
            "dialog.saveaction.message"
          ),
          VueResources.getFormatMessage(
            messageObject,
            "dialog.saveaction.title"
          ),
    ...
    try {
      ...
      if (name.endsWith(".rli.xml")) {
      ...
      ) ImageConversion.createActiveMapJpeg(
        file,
        VueResources.getDouble("imageExportFactor")
      ); else if (name.endsWith(".png")) ImageConversion.createActiveMapPng(
        file,
        VueResources.getDouble("imageExportFactor")
      ); else if (name.endsWith(".svg")) SVGConversion.createSVG(
        file
        ...
        try {
          while (istream.read(bytes, 0, fileLength) != -1) ostream.write(
            bytes,
            0,
            fileLength
          );
      ...
      //else if (name.endsWith(".imap"))
        ...
        hod.setVisible(true);
        if (hod.getReturnVal() > 0) new ImageMap()
          .createImageMap(file, hod.getScale(), hod.getFormat());
      }
      ...
      //             }
        ...
        final edu.tufts.vue.rdf.RDFIndex index =
          new edu.tufts.vue.rdf.RDFIndex();
        ...
        final String selectionType = VueResources.getString(
          "rdf.export.selection"
        );

        if (selectionType.equals("ALL")) {
          for (LWMap nextMap : VUE.getAllMaps()) index.indexAdd(nextMap);
        } else if (
          selectionType.equals("ACTIVE")
        ) index.indexMap(VUE.getActiveMap()); else index.indexMap( // Hardcoded to this case in VueResources.properties
          VUE.getActiveMap()
        );
      ...
      if (e != t) Log.error(
        "Exception attempting to save file " + file + ": " + e
      );

      VueUtil.alert(
        String.format(
          Locale.getDefault(),
          VueResources.getString("saveaction.savemap.error") +
          " \"%s\";\n" +
          ...
          map.getLabel(),
          file,
          Util.formatLines(e.toString(), 80)
        ),
        "Problem Saving Map"
      );
    } finally {
  ...
  private static void writeHTMLOutline(LWMap map, PrintWriter out) {
    ...
    if (title.endsWith(".vue") || title.endsWith(".htm")) title =
  ...
  private static void writeUnorderedNode(LWComponent node, PrintWriter out) {
    ...
    final LWComponent[] ordered = topChildren.toArray(
      new LWComponent[topChildren.size()]
    );
  ...
  private static void writeNode(LWComponent c, PrintWriter out) {
    if (c instanceof LWGroup || c instanceof LWSlide) writeUnorderedNode(
      c,
      out
    ); else if (c instanceof LWLink); else writeOrderedChildren(c, out);
  }
  ...
  private static void writeOrderedChildren(LWComponent node, PrintWriter out) {
    if (node.hasChildren()) {
      ...
      for (LWComponent c : node.getChildList()) {
        out.print("<li>");
        writeLabel(c, out);
        if (c.hasChildren()) writeNode(c, out);
        //writeNode(node, out);
        out.println();
      }
      out.println("</ul>");
  ...
  private static void writeLabel(LWComponent node, PrintWriter out) {
    if (node.hasResource()) out.println(
      "<a href=\"" + node.getResource() + "\">"
    );
    ...
    if (
      node.getTextColor() != null &&
      !java.awt.Color.black.equals(node.getTextColor())
    ) {
    ...
    if (didFontColor) out.print("</font>");

    if (node.hasResource()) out.print("</a>");
  }
  ...
  public static void main(String args[]) throws IOException {
    ...
    Archive.writeArchive(
      map,
      new File(
        map.getLabel().substring(0, map.getLabel().length() - 4) +
