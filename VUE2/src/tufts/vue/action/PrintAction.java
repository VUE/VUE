...
public class PrintAction extends tufts.vue.VueAction {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(PrintAction.class);

  private static PrintAction singleton;

  public static PrintAction getPrintAction() {
    if (singleton == null) singleton =
      new PrintAction(VueResources.getString("menu.file.printdot"));
    return singleton;
  }
  ...
  private PageFormat getPageFormat(PrinterJob job, Rectangle2D bounds) {
    if (pageFormat == null) {
      ...
      if (bounds.getWidth() > bounds.getHeight()) pageFormat.setOrientation(
   ...
   */
  private PageFormat getPageFormatInteractive(
    PrinterJob job,
    Rectangle2D bounds
  ) {
    PageFormat initial = getPageFormat(job, bounds);
    PageFormat result = job.pageDialog(initial);
    if (result != initial) return pageFormat = result; else return null; // dialog was canceled
  }
  ...
  private class PrintJob extends Thread implements Printable {
    ...
    private PrintJob(MapViewer viewer, boolean viewerPrint) {
      ...
      if (isPrintingView()) {
        if (focal == map) this.bounds =
          viewer.getVisibleMapBounds(); else this.bounds =
          focal.getBorderBounds();
    ...
    public int print(Graphics gc, PageFormat format, int pageIndex)
      ...
      Dimension page = new Dimension(
        (int) format.getImageableWidth() - 1,
      ...
      if (isPrintingView() && map == focal) g.clipRect(
      ...
      // render the map
      if (map == focal) map.draw(dc); else focal.draw(dc);
  ...
  private static String outpf(PageFormat p) {
    ...
    return String.format(
      "PageFormat[%s xoff=%.1fpt (%.2fin); yoff=%.1fpt (%.2fin); imageable-area: %.1fpt x %.1fpt (%.2f x %.2f inches)]",
      o[p.getOrientation()],
      p.getImageableX(),
