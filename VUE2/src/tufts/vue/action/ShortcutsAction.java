...
public class ShortcutsAction extends tufts.vue.VueAction {
  ...
  private static String keyCodeChar(int keyCode, boolean lowerCase) {
    ...
    if (keyCode == KeyEvent.VK_OPEN_BRACKET) return "["; else if (
      keyCode == KeyEvent.VK_CLOSE_BRACKET
    ) return "]"; else if (keyCode == 0) return ""; else if (
      Util.isMacLeopard()
    ) {
      if (
        keyCode == KeyEvent.VK_SPACE
      ) return "Space"; else if ( // override the little "u" glyph
        keyCode == KeyEvent.VK_BACK_QUOTE
      ) return "Back Quote"; // override the hard to see '`'
    }
  ...
  private static final String BIG_NBSP =
    ...
    NBSP +
    " " +
    NBSP//+ " " + NBSP // these if tool lines are bold on Leopard
  //+ " " + NBSP
  //+ " " + NBSP
  ;
  ...
  private static void add(int bits, Object o, String... attr) {
    ...
    if ((bits & SPAN2) != 0) html.append(" colspan=2"); else if (
      (bits & SPAN3) != 0
    ) html.append(" colspan=3");

    if (
      (bits & CENTER) != 0
    ) html.append(" align=center"); else if ((bits & RIGHT) != 0) html.append( // CENTER takes priority over RIGHT
      " align=right"
    );
    ...
    if ((bits & NO_WEST_GAP) == 0) html.append(NBSP);
    ...
    if ((bits & NO_EAST_GAP) == 0) html.append(NBSP);
  ...
  private static void addRow(int row, boolean debug) {
    ...
    if (debug) {
      ...
      if (Util.isMacPlatform()) {
        if (DEBUG.Enabled) html.append(" bgcolor=#DDDDFF"); else html.append(
          " bgcolor=#EEEEEE"
        ); // VUE-1036
      } else {
  ...
  private static void addTable(String... attr) {
    ...
    if (DEBUG.Enabled) html.append(" border=1"); else html.append(" border=0");
  ...
  private static final int SHIFT =
    Event.SHIFT_MASK + InputEvent.SHIFT_DOWN_MASK;
  ...
  private static boolean hasOnlyShift(int mods) {
    return (mods & SHIFT) == (mods | SHIFT);
  }

  private static boolean hasOnlyCtrl(int mods) {
    return (mods & CTRL) == (mods | CTRL);
  }

  private static boolean hasOnlyAlt(int mods) {
    return (mods & ALT) == (mods | ALT);
  }
  ...
  static JComponent buildShortcutsComponent() {
    if (html == null) {
      if (DEBUG.Enabled) html = new StringBuffer(65536); else html =
    ...
    for (VueTool t : VueTool.getTools()) {
      if (t.getShortcutKey() == 0) continue;
      ...
      if (DEBUG.Enabled) {
        ...
        if (downKey == 0) add(""); else add(CENTER, keyCodeChar(downKey, true));
    ...
    if (DEBUG.Enabled) {
      ...
      if (Util.isMacLeopard()) add(
        BOLD,
        "<font size=-3>Leopard<br>&nbsp;Glyphs"
      );
      ...
      if (Util.isMacLeopard()) add(
        BOLD + SPAN3 + NO_WEST_GAP,
        VueResources.getString("shortcutactions.macshortcut")
      ); else add(
        BOLD + SPAN2,
        VueResources.getString("shortcutactions.shortcutkey")
      );
    ...
    if (DEBUG.Enabled) {
      ...
      new tufts.vue.EventRaiser<AbstractButton>(
        ...
        public void dispatch(AbstractButton b) {
          if (reportKeyStroke(b.getAction(), row)) row++;
        }
      }
    ...
    row = 0;
    for (VueAction a : VueAction.getAllActions()) if (
      reportKeyStroke(a, row)
    ) row++;
  ...
  private static boolean reportKeyStroke(Action a, int row) {
    ...
    if (k == null && !(DEBUG.Enabled && DEBUG.WORK)) return false;
    ...
    if (DEBUG.Enabled) {
      if (tufts.vue.VueAction.isDupeStrokeAction(a)) addRow(
        row,
        true
      ); else addRow(row, false);
    } else {
      addRow(row);
    }
    ...
    if (
      goRight != 0 && (mods & Actions.COMMAND) != 0
    ) goRight = 0; // not for the platform primary
    ...
    if (DEBUG.Enabled) {
      ...
      if (k == null) {
        ...
        if (Util.isMacLeopard()) {
          ...
          add(
            BOLD + goRight + (DEBUG.Enabled ? 0 : CENTER),
      ...
      if (a instanceof VueAction) add(
        BOLD,
        ((VueAction) a).getPermanentActionName()
      ); else add(BOLD + ITAL, (String) a.getValue(Action.NAME));
      ...
      if (Util.isMacLeopard()) add(NO_GAP, "");

      if (a instanceof VueAction) add(
        ((VueAction) a).getPermanentActionName()
      ); else add(ITAL, (String) a.getValue(Action.NAME));
      ...
      if (row == 1 && Util.isMacLeopard()) add(NO_GAP, BIG_NBSP); // for colspan 3 in header
      //=============================================================================
  ...
  public static String getDescription(KeyStroke k) {
    if (Util.isMacLeopard()) {
      ...
      return (
        get_MacOSX_Leopard_Modifier_Glyphs(k.getModifiers()) +
        keyCodeChar(k.getKeyCode())
      ); // shortest
    } else {
      return (
