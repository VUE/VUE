...
public class ImageMap extends VueAction {
  ...
  public void act() {
    ...
    if (selectedFile != null) createImageMap(selectedFile, 1.0, "png");
  }

  public void createImageMap(File file, LWMap map, double zoom) {
    String imageLocation =
      file.getAbsolutePath().substring(0, file.getAbsolutePath().length() - 5) +
      ".png";
    String imageName =
      file.getName().substring(0, file.getName().length() - 5) + ".png";
    String fileName =
      file.getAbsolutePath().substring(0, file.getAbsolutePath().length() - 5) +
      ".html";
    ...
    if (imageLocationFile.exists()) {
      ...
      if (confirm == javax.swing.JOptionPane.NO_OPTION) {
        VueUtil.alert(
          VueResources.getString("imagemap.mapnotsaved.error"),
          VueResources.getString("imagemap.mapnotsaved.title")
        );
        return;
      }
    }
    imageDimensions =
      ImageConversion.createActiveMapPng(imageLocationFile, map, 1.0);
  ...
  public void createImageMap(File file, double zoom, String format) {
    ...
    String imageLocation =
      ...
      format;
    String imageName =
      file.getName().substring(0, file.getName().length() - 5) + "." + format;
    String fileName =
      file.getAbsolutePath().substring(0, file.getAbsolutePath().length() - 5) +
      ".html";
    ...
    if (format.equals("jpeg")) imageDimensions =
      ImageConversion.createActiveMapJpeg(
        imageLocationFile,
        zoom
      ); else imageDimensions =
      ImageConversion.createActiveMapPng(imageLocationFile, zoom);
  ...
  private Rectangle getRectNode(LWComponent node) {
    ...
    if (node.getParent() instanceof LWNode) {
      ...
      ow =
        (int) (ow -
          ((ow - ox) * (1 - Math.pow(ChildScale, (node.getDepth() - 2)))));
      /// (node.getDepth() - 2))));
      oh =
        (int) (oh -
          ((oh - oy) * (1 - Math.pow(ChildScale, (node.getDepth() - 2)))));
      /// (node.getDepth() - 2))));
  ...
  private String writeMapforContainer(
    ...
    for (LWComponent layer : reverse(map.getChildren())) {
      ...
      Iterator<LWComponent> iter = layer
        .getAllDescendents(ChildKind.PROPER, new ArrayList(), Order.DEPTH)
        .iterator();
      //comps.iterator();
      ...
      while (iter.hasNext()) {
        ...
        String type = "node";
        if (container instanceof LWGroup) type = "group";

        if (!(comp instanceof LWNode) && !(comp instanceof LWGroup)) continue;
        ...
        if (res == null) href = ""; else if (res.equals("null")) href =
          ""; else href = "href=\"" + res.getSpec() + "\" target=\"_blank\"";
        ...
        if (!comp.hasChildren()) {
          ...
          for (int i = array.length - 1; i >= 0; i--) {
            if (array[i] instanceof LWNode) {
              ...
              if (childRes == null) childHref = ""; else if (
                childRes.equals("null")
              ) childHref = ""; else childHref =
                "href=\"" + childRes.getSpec() + "\" target=\"_blank\"";
