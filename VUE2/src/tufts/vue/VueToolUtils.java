...
public class VueToolUtils {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueToolUtils.class);
  ...
  public static VueTool loadTool(String pName) {
    ...
    if (className == null) {
      Log.error(
        "loadTool[" +
        pName +
        "]; missing class key in resources: [" +
        classKey +
        "]"
      );
      return null;
    ...
    try {
      ...
      Class toolClass =
        VueToolUtils.class.getClassLoader().loadClass(className);
  ...
  public static synchronized VueTool[] loadToolsFromMap(
    String DefaultToolsKey
  ) {
  ...
  private static int getToolCount(String[] names) {
    ...
    for (int i = 0; i < names.length; i++) {
      if (!names[i].equals("separator")) toolCount++;
    }
  ...
  public static int[] getSeparatorPositions(String DefaultToolsKey) {
    ...
    separatorCount = 0;
    for (int i = 0; i < names.length; i++) {
      if (names[i].equals("separator")) separatorPositions[separatorCount++] =
        i;
    }
   ...
   **/
  public static synchronized VueTool[] loadTools(String DefaultToolsKey) {
    String[] names = VueResources.getStringArray(DefaultToolsKey);
    VueTool[] mVueTools = null;
  ...
  public static void setToolProperties(VueTool tool, String pName) {
    ...
    if (subtools != null) {
      ...
      tool.setOverlayDownIcon(
        VueResources.getImageIcon(pName + ".overlaydown")
      );
