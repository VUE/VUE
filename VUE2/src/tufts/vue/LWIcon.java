...
public abstract class LWIcon extends Rectangle2D.Float implements VueConstants {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWIcon.class);

  private static final float DefaultScale = 0.045f; // scale to apply to the absolute size of our vector based icons
  private static final Color TextColor = VueResources.getColor(
    "node.icon.color.foreground"
  );
  private static final Color FillColor = VueResources.getColor(
    "node.icon.color.fill"
  );
  public static final Font FONT_ICON = VueResources.getFont("node.icon.font");

  private static final BooleanPreference oneClickLaunchResPref =
    ...
    IconPref.addVuePrefListener(
      new VuePrefListener() {
        public void preferenceChanged(VuePrefEvent pe) {
          ...
          //     return;
          Log.info(
            "an icon display preference has changed: re-layout of all open VUE maps"
          );
  ...
  private static final Cursor RESOURCE_CURSOR = Cursor.getPredefinedCursor(
    Cursor.HAND_CURSOR
  );
  ...
  public static class Block extends Rectangle2D.Float {
    ...
    private final boolean mNoShrink; // don't let icon's get less than 100% zoom
    ...
    void draw(DrawContext dc) {
      ...
      for (LWIcon icon : mIcons) if (icon.isShowing()) icon.draw(dc);
    ...
    void checkAndHandleMouseOver(MapMouseEvent e) {
      ...
      for (LWIcon icon : mIcons) {
        ...
        if (icon instanceof LWIcon.Resource) {
          if (icon.contains(cx, cy)) {
            ...
            e
              .getViewer()
              .setTopCursor(VueToolbarController.getActiveTool().getCursor());
          }
        }
      ...
      if (tipIcon != null) {
        ...
        final Rectangle2D tipRegion = mLWC.transformZeroToMapRect(
          (Rectangle2D.Float) tipIcon.getBounds2D()
        );
        ...
        e
          .getViewer()
          .activateRolloverToolTip(e, tipComponent, avoidRegion, tipRegion);
        // }
      }
    }
  ...
  static class Resource extends LWIcon {
    ...
    private static final String NoResource = VueUtil.isMacPlatform()
      ? "---"
      : "__";
    ...
    public JComponent getToolTipComponent() {
      ...
      final boolean hasTitle =
        (r.getTitle() != null && !r.getTitle().equals(r.getLocationName()));
    ...
    void draw(DrawContext dc) {
      if (false && DEBUG.Enabled) {
        ...
        if (icon instanceof ResourceIcon) {
          ((ResourceIcon) icon).setSize(
              (int) Math.round(getWidth() - 4),
              (int) Math.round(getHeight())
            );
          icon.paintIcon(
            null,
            dc.g,
            (int) Math.round(getX() + 2),
            (int) Math.round(getY() + 2)
          );
          super.draw(dc);
          return;
        }
      ...
      if (
        true || extension == null
      ) internalLayout(); // TODO PERF: sometimes starts with boxBounds wrong...
      ...
      if (!dc.isInteractive()) {
        ...
        dc.g.setRenderingHint(
          PdfGraphics2D.HyperLinkKey.KEY_INSTANCE,
        ...
        if (g instanceof Graphics2D) {
          ...
          g2d.setComposite(
            AlphaComposite.getInstance(AlphaComposite.SRC_IN, 0.0f)
          );
  ...
  static class Notes extends LWIcon {
    ...
    private static final AffineTransform t = AffineTransform.getScaleInstance(
      scale,
      scale
    );
  ...
  static class MergeSourceMetaData extends LWIcon {
    ...
    public JComponent getToolTipComponent() {
      String html = "<html>";
      html +=
      mLWC.getMetaDataAsHTML(edu.tufts.vue.metadata.VueMetadataElement.OTHER);
      if (ttMetaDataHtml == null || !ttMetaDataHtml.equals(html)) {
        ttMetaData = new AALabel(html);
        ttMetaData.setFont(FONT_MEDIUM);
        ttMetaDataHtml = html;
  ...
  static class OntologicalMetaData extends LWIcon {
    ...
    public JComponent getToolTipComponent() {
      String html = "<html>";
      html +=
      mLWC.getMetaDataAsHTML(
        edu.tufts.vue.metadata.VueMetadataElement.ONTO_TYPE
      );
      if (ttMetaDataHtml == null || !ttMetaDataHtml.equals(html)) {
        ttMetaData = new AALabel(html);
        ttMetaData.setFont(FONT_MEDIUM);
  ...
  static class Hierarchy extends LWIcon {
    ...
    private static final Line2D line1 = new Line2D.Float(101, 16, 141, 16); // top horiz
    private static final Line2D line2 = new Line2D.Float(70, 76, 141, 76); // middle long horiz
    private static final Line2D line3 = new Line2D.Float(101, 136, 141, 136); // bottom horiz
    private static final Line2D line4 = new Line2D.Float(101, 16, 101, 136); // vertical

    private static final Rectangle2D box = new Rectangle2D.Float(0, 51, 56, 56);
    private static final Rectangle2D rect1 = new Rectangle2D.Float(
      150,
      0,
      70,
      33
    );
    private static final Rectangle2D rect2 = new Rectangle2D.Float(
      150,
      63,
      70,
      33
    );
    private static final Rectangle2D rect3 = new Rectangle2D.Float(
      150,
      122,
      70,
      33
    ...
    boolean isWanted() {
      ...
      if (IconPref.getHierarchyIconValue()) {
        if (mLWC.numChildren() == 1) {
          ...
          if (
            child0.isTextNode() || LWNode.isImageNode(mLWC)
    ...
    public JComponent getToolTipComponent() {
      if ((mLWC instanceof LWContainer) == false) return new JLabel(
        VueResources.getString("jlabel.nochild")
      );
    ...
    private String getChildHtml(LWComponent c, int indent) {
      ...
      String label = null;
      if (indent == 1) label =
        "&nbsp;<b>" + c.getDisplayLabel() + "</b>"; else label =
        c.getDisplayLabel();
