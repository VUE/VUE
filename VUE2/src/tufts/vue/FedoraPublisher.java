...
public class FedoraPublisher {
  ...
  public static final String VUE_FORMAT_URL =
    "http://vue.tufts.edu/docs/vueformat/";
  public static final String ONT_TYPE_METADATA =
    "http://vue.tufts.edu/ontology/vue.rdfs#ontoType";
  public static final String FEDORA_ONTOLOGY =
    "http://www.fedora.info/definitions/1/0/fedora-relsext-ontology.rdfs#";
  public static final String DC_URL = VueResources.getString(
    "metadata.dublincore.url"
  );
  public static final String RELS_URL =
  ...
  public static void uploadMap(edu.tufts.vue.dsm.DataSource ds, LWMap map)
    throws Exception {
    addObjectToRepository(ds, VUE_CM, map.getFile(), map, map);
  }

  public static void uploadArchive(edu.tufts.vue.dsm.DataSource ds, LWMap map)
    throws Exception {
    ...
    if (mapName.endsWith(VueUtil.VueArchiveExtension)) {
      ...
      archiveFilePath =
        archiveFilePath.substring(0, archiveFilePath.length() - 4) +
        VueUtil.VueArchiveExtension;
  ...
  public static void uploadMapAll(edu.tufts.vue.dsm.DataSource ds, LWMap map)
    throws Exception {
    ...
    File tempFile = new File(
      VueUtil.getDefaultUserFolder() + File.separator + origFile.getName()
    );
      ...
      if (
        ...
        if (resource.isLocalFile()) {
          File localFile = new File(
            resource.getSpec().replace(FILE_PREFIX, "")
          );
          addObjectToRepository(ds, OTHER_CM, localFile, component, cloneMap);
          String ingestUrl =
            HTTP +
            "://" +
            properties.getProperty("fedora22Address") +
            ":" +
            properties.getProperty("fedora22Port") +
            FEDORA_URL_PATH +
            "get/" +
            pid +
            "/" +
  ...
  private static void addObjectToRepository(
    ...
    System.setProperty(
      ...
      properties.getProperty("fedora22TrustStore")
    );
    System.setProperty(
      "javax.net.ssl.trustStorePassword",
      properties.getProperty("fedora22TrustStorePassword")
    );
    FedoraClient fc = new FedoraClient(
      ...
      properties.getProperty("fedora22Address") +
      ":" +
      properties.getProperty("fedora22SecurePort") +
      FEDORA_URL_PATH,
      properties.getProperty("fedora22UserName"),
      properties.getProperty("fedora22Password")
    );
  ...
  private static void modifyObject(
    ...
    if (!cModel.equals(VUE_CM)) {
      fc
        ...
        .modifyDatastreamByValue(
          ...
          XML_MIME_TYPE,
          RELS_URL,
          getRDFDescriptionForLWComponent(comp, map).getBytes(),
          null,
          null,
          COMMENT,
          true
    ...
    if (cModel.equals(REMOTE_CM)) {
      ...
      Uploader uploader = new Uploader(
        HTTPS,
        p.getProperty("fedora22Address"),
        Integer.parseInt(p.getProperty("fedora22SecurePort")),
        p.getProperty("fedora22UserName"),
        p.getProperty("fedora22Password")
      );
  ...
  private static String getDigitalObjectXML(
    Properties p,
    LWComponent comp,
    LWMap map,
    String cModel,
    ...
    xml.append(
      "    <foxml:property NAME=\"info:fedora/fedora-system:def/model#contentModel\" VALUE=\"" +
      cModel +
      "\"/>\n"
    );
    xml.append(
  ...
  private static String getOjectDSXML(
    Properties p,
    LWComponent comp,
    String cModel,
    File file
    ...
    System.out.println(
      "Getting object XML for " + comp.getLabel() + " file: " + file
    );
    String r = "";
    ...
    if (cModel.equals(VUE_CM)) {
      ...
      getDSXML(
        ...
        dateString,
        comp.getLabel(),
        "http://local.fedora.server/fedora/get/" + pid + "/" + dsName,
        "URL",
        mimeType
      );
    }
  ...
  public static String getDSXML(
    ...
    StringBuffer xml = new StringBuffer();
    xml.append(
      "<foxml:datastream CONTROL_GROUP=\"" +
      controlGroup +
      "\" ID=\"" +
      dsName +
    ...
    xml.append(
      "<foxml:datastreamVersion CREATED=\"" +
      dateString +
      "\" FORMAT_URI=\"http://vue.tufts.edu/docs/vueformat/\""
    );
    xml.append(
      " ID=\"" +
      dsName +
  ...
  private static String getDC(LWComponent c, String title, String identifier) {
    ...
    for (VueMetadataElement element : c.getMetadataList().getMetadata()) {
      ...
      if (element.getKey().contains(DC_URL)) {
        String key =
          "dc:" + element.getKey().substring(DC_URL.length() + 1).toLowerCase();
  ...
  private static String getRelsXML(LWComponent comp, LWMap map)
    throws Exception {
    String dateString = getDateString();
    StringBuffer xml = new StringBuffer();
    xml.append(
    ...
    xml.append(
      "<foxml:datastreamVersion CREATED=\"" +
      dateString +
      "\" ID=\"RELS-EXT.0\" LABEL=\"Relationships to other objects\" MIMETYPE=\"text/xml\">\n"
    );
    xml.append("<foxml:xmlContent>\n");
  ...
  private static String getRDFDescriptionForLWComponent(
    LWComponent comp,
    LWMap map
  ) {
    ...
    rdfDescription +=
    "<rdf:Description rdf:about=\"info:fedora/" + getFedoraPid(comp) + "\">";
    for (LWComponent c : map.getAllDescendents(LWComponent.ChildKind.PROPER)) {
      for (VueMetadataElement element : c.getMetadataList().getMetadata()) {
        if (DEBUG.RDF) System.out.println(
          "METADATA: " +
          element.getValue() +
          " key:  " +
          element.getKey() +
        ...
        if (
          element.getKey().equals(ONT_TYPE_METADATA) &&
          element.getValue().startsWith(FEDORA_ONTOLOGY) &&
          c instanceof LWLink
        ) {
          ...
          if (getFedoraPid(head).equals(getFedoraPid(comp))) {
            ...
            rdfDescription +=
            "<rel:" +
            getFedoraOntologyTerm(element.getValue()) +
    ...
    rdfDescription +=
    "<rel:IsPartOf rdf:resource=\"info:fedora/" + getFedoraPid(map) + "\" />";
  ...
  private static String getFedoraPid(LWComponent component) {
    return (
      ...
      component
        .getURIString()
        .substring(component.getURIString().lastIndexOf("/") + 1)
    );
  }
  ...
  private static String getMimeType(File file, LWComponent comp) {
    ...
    if (file != null) {
      ...
      mimeType =
        new MimetypesFileTypeMap().getContentType(comp.getResource().getSpec());
    }
    return mimeType;
  }
}
