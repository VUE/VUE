...
public abstract class Resource implements Cloneable {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Resource.class);
  ...
  public static final String PACKAGE_FILE =
    HIDDEN_RUNTIME_PREFIX + "package.file";
  public static final String PACKAGE_KEY_DEPRECATED =
    HIDDEN_PREFIX + "Packaged";
  ...
  public static class DefaultFactory implements Factory {
    ...
    public Resource get(org.xml.sax.InputSource s) {
      ...
      if (spec == null) spec = s.getPublicId();

      if (spec == null) {
        Log.error(
          "no system or public id in " +
          Util.tags(s) +
  ...
  public static Resource instance(String spec) {
    return getFactory().get(spec);
  }

  public static Resource instance(java.net.URL url) {
    return getFactory().get(url);
  }

  public static Resource instance(java.net.URI uri) {
    return getFactory().get(uri);
  }

  public static Resource instance(java.io.File file) {
    return getFactory().get(file);
  }

  public static Resource instance(osid.filing.CabinetEntry entry) {
    return getFactory().get(entry);
  }
  ...
  public static Resource instance(org.xml.sax.InputSource s) {
    return getFactory().get(s);
  }
  ...
  public long getReferenceCreated() {
    return mReferenceCreated;
  }

  public void setReferenceCreated(long created) {
    mReferenceCreated = created;
  }

  public long getAccessAttempted() {
    return mAccessAttempted;
  }

  public void setAccessAttempted(long attempted) {
    mAccessAttempted = attempted;
  }

  public Long getPersistAccessAttempted() {
    return mAccessAttempted == 0 ? null : Long.valueOf(mAccessAttempted);
  }

  public void setPersistAccessAttempted(Long l) {
    setAccessAttempted(l == null ? 0 : l.longValue());
  }

  public long getAccessSuccessful() {
    return mAccessSuccessful;
  }

  public void setAccessSuccessful(long succeeded) {
    mAccessSuccessful = succeeded;
  }

  public Long getPersistAccessSuccessful() {
    return mAccessSuccessful == 0 ? null : Long.valueOf(mAccessSuccessful);
  }

  public void setPersistAccessSuccessful(Long l) {
    setAccessSuccessful(l == null ? 0 : l.longValue());
  }
  ...
  /** for castor hacks */public Object getNull() {
    return null;
  }
  ...
  public String getName() {
    ...
    ) return getSpec(); else return title;
  }
  ...
  protected String determineDataType() {
    ...
    if (getClientType() == DIRECTORY) ext = EXTENSION_DIR; else ext =
      extractExtension();

    if (ext == null || ext == NO_EXTENSION || ext.length() == 0) {
      final String spec = getSpec();
      if (spec == null || spec == SPEC_UNSET || spec.trim().length() < 1) ext =
        EXTENSION_UNKNOWN; else if (
        spec.startsWith("http:") || spec.startsWith("https:")
      ) ext = EXTENSION_HTTP; else if (getClientType() == Resource.FILE) ext =
        "txt"; else ext = EXTENSION_DIR;
    } else {
      ext = ext.toLowerCase();
      ...
      if ("readme".equals(ext) || "msg".equals(ext)) {
        ...
        if (
          "asp".equals(ext) || // microsoft web page
          "php".equals(ext) ||
  ...
  protected String extractExtension(final String s) {
    ...
    if (s == null || s.length() == 0) lastChar = 0; else lastChar =
      s.charAt(s.length() - 1);
    ...
    if (lastChar == 0) {
      ...
      if (lastDotIdx > 1 && (s.length() - lastDotIdx) > 2) {
        ...
        if (Character.isLetterOrDigit(extTest.charAt(0))) ext = extTest;
      }
    }

    if (DEBUG.RESOURCE) Log.debug(
      "extractExtension " + this + "; [" + s + "] = [" + ext + "]"
    );
  ...
  public Icon getTinyIcon() {
    if (mTinyIcon == null) mTinyIcon = makeIcon(16, 16);
    return mTinyIcon;
  }
  ...
  public Icon getLargeIcon() {
    if (mLargeIcon == null) mLargeIcon = makeIcon(32, 128);
    return mLargeIcon;
  }
  ...
  public Image getLargeIconImage() {
    if (mLargeIcon == null) getLargeIcon();
    if (mLargeIcon != null) return mLargeIcon.getImage(); else return null;
  }

  public Image getTinyIconImage() {
    if (mTinyIcon == null) getTinyIcon();
    if (mTinyIcon != null) return mTinyIcon.getImage(); else return null;
  }
  ...
  public Image getDragImage() {
    ...
    if (!isLocalFile()) {
      ...
      if (icon instanceof ResourceIcon) image =
        ((ResourceIcon) icon).getImage();
    }
    if (image == null) image = getLargeIconImage();
   ...
   */
  public synchronized javax.swing.Icon getContentIcon(
    java.awt.Component repainter
  ) {
    //if (!isImage())
    //  return null;
  ...
  private static final String _debugFmt = Util.getJavaVersion() > 1.5
    ? _fmt1
    : _fmt0;
  ...
  public String asDebug() {
    return String.format(
      ...
      mDataFile == null
        ? Util.tags(getSpec())
        : (Util.TERM_RED + mDataFile + Util.TERM_CLEAR)
  ...
  public static boolean looksLikeLocalFilePath(String s) {
    ...
    if (Util.isWindowsPlatform()) {
      ...
      return (
        c0 == '/' || s.startsWith(java.io.File.separator)
      )//|| s.startsWith("file:")
      ;
    }
  }
  ...
  public static File toCanonicalFile(File file) {
    ...
    if (file.getPath().equals(canonical)) return file; else return new File(
      canonical
    );
  }
  ...
  public static File getLocalFileIfPresent(String urlOrPath) {
    ...
    if (file == null) {
      ...
      if (!file.exists()) file = getLocalFileIfPresent(makeURL(urlOrPath));
    }
    ...
    if (false) {
      ...
      try {
        if (url.getAuthority() != null) {
          ...
          if (!fullPath.startsWith("file:")) throw new IllegalStateException(
            "URL should already have had a file: protocol; " + url
          );
        ...
        if (!file.isAbsolute()) {
          ...
          if (DEBUG.Enabled) Log.debug(
            "ignoring non-absolute: " + Util.tags(file)
          );
          return null;
        }

        if (DEBUG.IO) Log.debug(
          Util.tags(file) + "; getLocalFileIfPresent(URL): testing"
        );
        if (!file.exists()) {
          ...
          if (fullpath.indexOf('%') >= 0) { // TODO: keep repeating this until no more % or no change after decoding
            Log.info(
              Util.tags(file) + "; claims non-existent, attempting decode:"
            );

            for (String encoding : Encodings) {
              String decoded = "<failed>";
              try {
                if (encoding == DEFAULT_ENCODING) decoded =
                  java.net.URLDecoder.decode(fullpath); else decoded =
                  java.net.URLDecoder.decode(fullpath, encoding);
              ...
              Log.info(
                String.format("Decoded to %12s: ", encoding) +
                Util.tags(decoded)
              );
            ...
            if (!exists) Log.info(
              Util.tags(file) + "; cannot find under any decoding."
            );
          }
  ...
  public static java.net.URL makeURL(final File file) {
    try {
      ...
      Log.warn(
        Util.tags(file) + " failed to convert itself to a URL",
        new Throwable()
      );
  ...
  public static java.net.URL makeURL(String s) {
    try {
      if (s != null && s.startsWith("feed:")) s = "http:" + s.substring(5);
      ...
      try {
        ...
        String msg =
          "couldn't make URL from decoded URI: " +
          Util.tags(decoded == null ? uri : decoded);
        if (DEBUG.Enabled) Log.info(msg, t); else Log.info(msg + "; " + t);
      ...
      if ((DEBUG.WORK || DEBUG.RESOURCE) && url != null) dumpURL(
        url,
        "Made URL FROM " + Util.tags(uri)
      );
  ...
  public static URI makeURI(String s) {
    ...
    try {
      if (c0 == '#' || s.startsWith("rdf:#")) {
        ...
        if (c0 == '#') uri =
          new java.net.URI("rdf", schemeSpecific, s.substring(1)); else uri =
          new java.net.URI("rdf", schemeSpecific, s.substring(5));
        Log.warn(
          "makeURI: guessed at creating " +
          Util.tags(uri) +
          " from " +
          Util.tags(s)
        );
      } else if (
        c0 == '/' || c0 == '\\' || (Character.isLetter(c0) && c1 == ':')
      ) {
        ...
        if (uri != null) uri = uri.normalize();
      }

      if (uri != null && uri.getScheme() == null) uri =
        new java.net.URI("file:" + uri.toString());
    } catch (Throwable t) {
  ...
  public static String getDump(Object o, String msg) {
    ...
    if (o instanceof URI) writeURI(w, (URI) o, msg); else if (
      o instanceof URL
    ) writeURL(w, (URL) o, msg); else if (o instanceof File) writeFile(
      w,
      (File) o,
      msg
    ); else w.print(
      "\tResource: unhandled getDump for object of type: " + Util.tags(o)
    );
  ...
  private static void dumpOut(Object o, String msg, boolean toError) {
    if (msg == null) msg =
      Util.TERM_RED + "Made " + o.getClass().getName() + ";" + Util.TERM_CLEAR;

    final String txt = getDump(o, msg);

    if (toError) Log.error(txt); else Log.debug(txt);
  }
  ...
  private static void writeURI(PrintWriter w, URI u, String msg) {
    ...
    if (u.getPort() != -1) writeField(w, "port", u.getPort());
  ...
  private static void writeField(
    ...
    if (
      (value != null && !value.equals(rawValue)) ||
