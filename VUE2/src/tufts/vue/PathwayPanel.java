 ...
 * @author  Scott Fraize
 * @version $Revision: 1.140 $ / $Date: 2010-02-03 19:17:40 $ / $Author: mike $
 */

public class PathwayPanel extends JPanel implements ActionListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(PathwayPanel.class);

  private Frame mParentFrame;

  private final VueButton btnAddSlide = new VueButton(
    "presentationDialog.button.makeSlides",
    this
  );
  private final VueButton btnMergeInto = new VueButton(
    "presentationDialog.button.mergeInto",
    this
  );
  private final VueButton btnLiveMap = new VueButton(
    "presentationDialog.button.liveMap",
    this
  );

  //edit
  private final VueButton btnEditSlides = new VueButton(
    "presentationDialog.button.preview",
    this
  );
  ...
  private final VueButton btnRefresh = new VueButton(
    "presentationDialog.button.refresh",
    this
  );

  //new
  private final VueButton btnPresentationCreate = new VueButton(
    "presentationDialog.button.add",
    this
  );
  private final VueButton btnPresentationDelete = new VueButton(
    "presentationDialog.button.delete",
    this
  );

  //filter
  private final JToggleButton btnPathwayOnly = new VueButton.Toggle(
    "presentationDialog.button.viewAll",
    this
  );
  ...
  private final JToggleButton btnShowSlides = new VueButton.Toggle(
    "presentationDialog.button.showSlides",
    this
  );
  ...
  private final VueButton btnPlay = new VueButton(
    "presentationDialog.button.play",
    this
  );
  ...
  private JLabel lblCreateSlides = new JLabel(
    VueResources.getString("presentationDialog.createslides.label")
  );
  private JLabel lblEditSlides = new JLabel(
    VueResources.getString("presentationDialog.editslides.label")
  );
  private JLabel lblViewSlide = new JLabel(
    VueResources.getString("presentationDialog.viewslide.label")
  );
  private JLabel lblNew = new JLabel(
    VueResources.getString("presentationDialog.new.label")
  );
  //private JLabel lblFilter = new JLabel(VueResources.getString("presentationDialog.filter.label"));
  private JLabel lblRefresh = new JLabel(
    VueResources.getString("presentationDialog.refresh.label")
  );
  ...
  private JLabel lblPresentationBackground = new JLabel(
    VueResources.getString("presentationDialog.presentationBackground.label")
  );
  ...
  public PathwayPanel(Frame parent) {
    ...
    final Font smallFont = defaultFont.deriveFont(
      (float) boldFont.getSize() - 2
    );
    ...
    notesArea.addKeyListener(
      new KeyAdapter() {
        public void keyPressed(KeyEvent e) {
          mNoteKeyWasPressed = true;
        }

        public void keyReleased(KeyEvent e) {
          if (e.getKeyCode() == KeyEvent.VK_ENTER) ensureNotesSaved();
        }
      }
    );
    notesArea.addFocusListener(
      new FocusAdapter() {
        public void focusLost(FocusEvent e) {
          if (DEBUG.PATHWAY) System.out.println(
            "PathwayPanel.notesArea     focusLost to " +
        ...
        public void focusGained(FocusEvent e) {
          if (DEBUG.PATHWAY) System.out.println(
            "PathwayPanel.notesArea focusGained from " +
            e.getOppositeComponent()
          );
        }
      }
    ...
    c.gridheight = 1;
    c.gridwidth = 1;
    JLabel createPres = new JLabel(
      VueResources.getString("presentationDiaaog.presentationName.text")
    );
    ...
    JLabel lblPlay = new JLabel(
      VueResources.getString("presentationDialog.playback.label")
    );
    ...
    gbc.anchor = GridBagConstraints.WEST;
    gbc.gridx = 1;
    if (VUE.getActiveMap() == null) bkgrndColorButton.selectValue(
      new Color(32, 32, 32)
    ); else bkgrndColorButton.selectValue(
      VUE.getActiveMap().getPresentationBackgroundValue()
    );
    bkgrndColorButton.setPropertyKey(LWKey.PresentationColor);
    bkgrndColorButton.addPropertyChangeListener(
    ...
    tufts.vue.VUE.addActiveListener(
      ...
      new ActiveListener() {
        public void activeChanged(ActiveEvent e) {
          ...
          if (e == null || e.active == null) return;
  ...
  private void addToolTips() {
    ...
    btnAddSlide.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "makeSlides.tooltip") +
      helpTextFooter
    );
    btnMergeInto.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "mergeInto.tooltip") +
      helpTextFooter
    );
    btnLiveMap.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "liveMap.tooltip") +
      helpTextFooter
    );

    //edit
    btnEditSlides.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "preview.tooltip") +
      helpTextFooter
    );

    //new
    btnPresentationCreate.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "add.tooltip") +
      helpTextFooter
    );
    btnPresentationDelete.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "delete.tooltip") +
      helpTextFooter
    );

    //filter
    btnPathwayOnly.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "viewAll.tooltip") +
      helpTextFooter
    );

    // hack for now as single button just to get this working:
    btnShowSlides.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "showNodes.tooltip") +
      helpTextFooter
    );
    btnPlay.setToolTipText(
      helpTextHeader +
      VueResources.getString(baseProp + "play.tooltip") +
      helpTextFooter
    );
  }
  ...
  class ImageDropDown extends JPanel {
    ...
    class ComboBoxRenderer extends JLabel implements ListCellRenderer {
      ...
      public Component getListCellRendererComponent(
        ...
        setIcon(icon);
        if (!comboList.isEditable()) setIcon(disabledIcon);
  ...
  public void activeChanged(ActiveEvent e, LWPathway pathway) {
    if (pathway == null) {
      if (exclusiveDisplay != null) ClearFilter(exclusiveDisplay.getMap());
      return;
    }
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (pathway == null && btn != btnPresentationCreate) return;
    if (btn == btnEditSlides) {
      ...
      if (
        pathway.getCurrentEntry() != null &&
        pathway.getCurrentEntry().getSlide() != null
      ) Actions.EditSlide.act(
        pathway.getCurrentEntry().getSlide()
      ); else Actions.EditMasterSlide.act(pathway.getMasterSlide());
    } else if (btn == btnPlay) {
      ...
      if (
        entry != null &&
        entry.getSlide().canSync() &&
        entry.getSlide().numChildren() == 0
      ) {
    ...
    //  else if (btn == btnElementDown) { pathway.moveCurrentDown(); }
      ...
      if (pathway.getCurrentEntry() == null) {
        final Object[] defaultOrderButtons = {
          ...
          VueResources.getString("optiondialog.deletepathway.delete"),
        };
      ...
      setFocusable(false);
      new PathwayDialog(mParentFrame, mTableModel, getLocationOnScreen())
        .setVisible(true);
      setFocusable(true);
    }
  ...
  private static void ClearFilter(LWMap map, Iterable<LWComponent> allNodes) {
    for (LWPathway pathway : map.getPathwayList()) pathway.setFiltered(false);

    for (LWComponent c : allNodes) c.setFiltered(false);
  }
  ...
  private synchronized void toggleHideEverythingButCurrentPathway(
    ...
    if (pathway == null || map == null) return;
    ...
    if (DEBUG.Enabled) Log.debug(
      "toggle; clear=" + clearFilter + "; all=" + Util.tags(allNodes)
    );
        ...
        if (entry.isPortal()) {
          ...
          for (LWComponent c : allNodes) {
            ...
            if (c.intersects(portalRect)) {
              if (
                c instanceof LWLink && ((LWLink) c).isConnectedTo(entry.node)
              ) {
  ...
  private void filterAllOutsidePathway(LWComponent node, LWPathway pathway) {
    ...
    for (LWComponent c : node.getChildren()) {
      ...
      c.setFiltered(true);
      if (c.hasChildren()) filterAllOutsidePathway(c, pathway);
    }
  }

  private void updateAddRemoveActions() {
    if (DEBUG.PATHWAY && DEBUG.META) System.out.println(
    ...
    if (selection.size() > 0 && !(selection.first() instanceof LWPathway)) {
      ...
      if (selection.size() == 1) btnAddSlide.setEnabled(
        LWPathway.isPathwayAllowed(selection.first())
      ); else btnAddSlide.setEnabled(true);
      ...
      if (!removeDone) {
        ...
        while (i.hasNext()) {
          ...
          if (c.inPathway(path)) {
            if (DEBUG.PATHWAY) System.out.println(
              this + " in selection enables remove: " + c + " on " + path
            );
            enabled = true;
            break;
          }
  ...
  public void updateEnabledStates() {
    ...
    if (VUE.getActivePathway() != null) {
      if (
        VUE.getActivePathway().getCurrentEntry() != null
      ) btnEditSlides.setEnabled(
        !VUE.getActivePathway().getCurrentEntry().isPortal()
      ); else btnEditSlides.setEnabled(true);
    } else btnEditSlides.setEnabled(false);
    //         final LWPathway pathway = VUE.getActivePathway();
