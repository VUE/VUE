...
public class MapDropTarget implements java.awt.dnd.DropTargetListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MapDropTarget.class);
  ...
  public static class DropIndication {
    ...
    java.awt.Color getColor() {
      ...
      else return VueConstants.COLOR_INDICATION;
    }
  ...
  public void dragEnter(DropTargetDragEvent e) {
    ...
    if (transfer.isDataFlavorSupported(DropHandler.DataFlavor)) mActiveHandler =
      extractData(transfer, DropHandler.DataFlavor, DropHandler.class); // will be null if not found

    final DropIndication di = getIndication(e);

    if (DEBUG.DND) out(
      "dragEnter: " + dragName(e) + "; handler=" + mActiveHandler + " " + di
    );
  ...
  public void drop(DropTargetDropEvent e) {
    try {
      ...
      if (DEBUG.DND) out(
        ...
        Util.tag(e) +
        "\n\t     sourceActions: " +
        dropName(e.getSourceActions()) +
        "\n\t        dropAction: " +
        dropName(e.getDropAction()) +
        "\n\t        dropAccept: " +
        dropName(di.acceptedAction) +
      ...
      if (DEBUG.DND) out(
        TERM_CYAN + "processTransferable: success=" + success + TERM_CLEAR
      );
  ...
  private DropIndication getIndication(DropTargetDragEvent e) {
    return getIndication(
      ...
      e.getSourceActions(),
      e.getDropAction(),
      dropToMapLocation(e.getLocation())
    );
  }
  ...
  private DropIndication getIndication(
    ...
    if (mActiveHandler != null) {
      ...
      if (hit instanceof LWImage) {
        ...
        if (hit.supportsChildren() && hit != mViewer.getFocal()) {
          ...
          if (hit instanceof LWSlide); // disable slide icon dropping for now
          // (mapToLocalLocation doesn't seem to be working in this case)
          else if (hit instanceof LWGroup); // don't allow drops into groups sitting on maps (when not the focal)
          else dropHit = hit;
        }
    ...
    if (dropAccept == ACTION_LINK) {
      if (
        (hit instanceof LWNode || hit instanceof LWImage) && hit.hasResource()
      ) {
        type = DROP_RESOURCE_RESET;
      } else {
    ...
    if (type != null) report =
      new DropIndication(type, dropAccept, hit); else report =
      new DropIndication();
  ...
  public static class DropContext {
    ...
    DropContext(
      ...
      if (hit != null && hit.supportsChildren()) hitParent =
        (LWContainer) hit; else hitParent = null;
  ...
  public static <A> A extractData(
    ...
    if (clazz.isInstance(data)) {
      ...
      Log.warn(
        "Transfer data expecting type " + clazz + "; found: " + Util.tags(data)
      );
      return null;
    }
  }
  ...
  public static Object extractData(Transferable transfer, DataFlavor flavor) {
    ...
    try {
      ...
      Util.printStackTrace(
        ex,
        "TRANSFER: Transfer lied about supporting flavor " +
        "\"" +
   ...
   */
  public boolean processTransferable(
    Transferable transfer,
    ...
    if (e != null) {
      ...
      if (DEBUG.DND) out(
        "processTransferable: (no drop event) transfer=" + transfer
      );
    }
    ...
    if (dropLocation != null) {
      ...
      if (dropTarget != null) {
        if (!dropTarget.supportsChildren() && !isLinkAction) {
          ...
          if (DEBUG.DND) out(
            "dropTarget: doesn't support children: " + dropTarget
          );
          return false;
        }
        ...
        if (mViewer.getFocal() instanceof LWMap == false) {
          ...
          if (DEBUG.DND) out(
            "warning: drop to non-map focal " + mViewer.getFocal()
          );
    ...
    final DataFlavor URLFlavor = findFlavor(
      dataFlavors,
      "application/x-java-url",
      java.net.URL.class
    );
    final DataFlavor HTMLTextFlavor = findFlavor(
      dataFlavors,
      "text/html",
    ...
    if (HTMLTextFlavor != null) {
      ...
      final String htmlText = extractData(
        transfer,
        HTMLTextFlavor,
        String.class
      );
    ...
    try {
      if (URLFlavor != null && found_HTTP_URL == null) {
        ...
        if (url != null) {
          if ("http".equals(url.getProtocol())) {
            ...
            found_HTTP_URL = url;
            Log.debug(
              "FOUND HTTP URL FLAVOR/DATA: " + URLFlavor + "; URL=" + url
            );
          }
        }
      }
      ...
      if (transfer.isDataFlavorSupported(DropHandler.DataFlavor)) {
        ...
        transfer.isDataFlavorSupported(
          edu.tufts.vue.ontology.ui.TypeList.DataFlavor
        ) &&
        (dropAction == DnDConstants.ACTION_LINK)
      ) {
        dropType = DROP_ONTOLOGY_TYPE;
        foundData =
          extractData(transfer, edu.tufts.vue.ontology.ui.TypeList.DataFlavor);
        ...
        if (foundData == null) throw new IllegalStateException(
          "null resource found"
        );

        dropType = DROP_RESOURCE_LIST;

        if (foundData instanceof List) dropItems =
          (List) foundData; else dropItems =
          Collections.singletonList(foundData);
      } else if (
        found_HTTP_URL != null &&
        !found_HTTP_URL.getHost().equals("images.google.com")
      ) {
      ...
      Util.printStackTrace(
        ex,
        "TRANSFER: Transfer data did not match expected type:" +
        "\n\tflavor=" +
        foundFlavor +
        "\n\t  type=" +
        foundData.getClass()
    ...
    if (DEBUG.Enabled) {
      ...
      Log.debug(
        TERM_CYAN +
        "\nTRANSFER: Found a supported DataFlavor among the " +
        dataFlavors.length +
        " available;" +
        "\n\t flavor: " +
        foundFlavor +
    ...
    DropContext drop = new DropContext(
      transfer,
      hitLocation,
      mViewer,
      dropItems,
    ...
    if (dropItems != null && dropItems.size() > 1); //CenterNodesOnDrop = false; // turned off 2008-09-30 for data-drops to be on-center
    // TODO: fix for non-centered lists or fix code to handle centering them
    else CenterNodesOnDrop = true;
  ...
  protected boolean processDroppedText(DropContext drop) {
    ...
    if (
      foundURL != null && foundURL.getQuery() != null
      /*&& !drop.isLinkAction*/
    ) {
    ...
    if (foundURL != null) {
      ...
      if (drop.hit != null) {
        if (overwriteResource) {
          ...
          drop.hitParent.dropChild(
            createNodeAndResource(
              drop,
              null,
              foundURL.toString(),
              properties,
      ...
      if (drop.hit == null || !processed) createNodeAndResource(
        drop,
        null,
        foundURL.toString(),
        properties,
  ...
  public abstract static class DropHandler {
    ...
    public abstract DropIndication getIndication(
      LWComponent target,
      int requestAction
    );
  ...
  private boolean processDroppedNodes(DropContext drop) {
    ...
    if (CenterNodesOnDrop) setCenterAt(
      drop.items,
      drop.location
    ); else setLocation(drop.items, drop.location);
  ...
  private boolean processDroppedOntologyType(
    DropContext drop,
    Object foundData
  ) {
    if (DEBUG.DND) out("processDroppedType");

    edu.tufts.vue.metadata.VueMetadataElement ele =
  ...
  private void processDroppedFile(File file, DropContext drop) {
    ...
    if (path.toLowerCase().endsWith(".url")) {
      ...
      if (url != null) {
        ...
        if (file.getName().length() > 4) resourceName =
          file
            .getName()
            .substring(0, file.getName().length() - 4); else resourceName =
          file.getName();

        props.put("title", resourceName);
      }
    } else if (
      path.endsWith(".textClipping") ||
      path.endsWith(".webloc") ||
      path.endsWith(".fileloc")
    ) {
      ...
      if (drop.transfer.isDataFlavorSupported(DataFlavor.stringFlavor)) {
        ...
        try {
          unicodeString =
            (String) drop.transfer.getTransferData(DataFlavor.stringFlavor);
          if (DEBUG.Enabled) out(
            "*** GOT MAC REDIRECT DATA [" + unicodeString + "]"
          );
        } catch (Exception ex) {
          ex.printStackTrace();
        }
  ...
  private LWComponent createNodeAndResource(
    DropContext drop,
    File file,
    String resourceSpec,
    Map properties,
    ...
    if (file != null) resource =
      mViewer.getMap().getResourceFactory().get(file); else resource =
      mViewer.getMap().getResourceFactory().get(resourceSpec);
  ...
  private LWComponent createNode(
    DropContext drop,
    Resource resource,
    Point2D where
  ) {
  ...
  private static int MaxNodeTitleLen = VueResources.getInt(
    "node.title.maxDefaultChars",
    50
  );
  ...
  private LWComponent createNode(
    ...
    if (DEBUG.DND) Log.debug(
      drop + "; createNode " + resource + " " + properties + " where=" + where
    );
    ...
    if (shortName.length() > MaxNodeTitleLen) shortName =
      shortName.substring(0, MaxNodeTitleLen) + "...";
    ...
    if (resource.isImage()) {
      ...
      if (DEBUG.DND || DEBUG.IMAGE) Log.debug(
        drop + "; IMAGE DROP " + resource + " " + properties
      );
      String ws = (String) properties.get("width");
      String hs = (String) properties.get("height");
      if (ws != null && hs != null) {
    ...
    if (dropImagesAsNodes) {
      shortName =
        Util.formatLines(
          shortName,
          VueResources.getInt("dataNode.labelLength")
        );
      node = NodeModeTool.createNewNode(shortName);
  ...
  private LWComponent addNodeToFocal(final LWComponent node, Point2D where) {
    ...
    if (CenterNodesOnDrop) node.setCenterAt(where); else node.setLocation(
      where
    );
        ...
        public void run() {
          if (!node.getParent().isTopLevel()) {
            ...
            if (DEBUG.Enabled) Log.debug(
              "ignoring push for non-top0level final parent of " + node
            );
            return;
          }
          ...
          if (layer != null && layer.getChildren() != null) {
            final Rectangle2D.Float clearRegion = Util.grow(
              node.getMapBounds(),
              24
            );
  ...
  private void dumpFlavors(Transferable transfer) {
    ...
    for (int i = 0; i < dataFlavors.length; i++) {
      ...
      if (flavor.getMimeType().toString().startsWith(name + ";")) name =
        ""; else name = "\"" + name + "\"";
      ...
      try {
        ...
        if (DEBUG.META) {
          if (
            flavor.getHumanPresentableName().equals("text/uri-list")
  ...
  private DataFlavor findFlavor(
    ...
    for (DataFlavor flavor : dataFlavors) {
      ...
      if (
        ...
        flavor.getRepresentationClass() == repClass
      ) return flavor;
    }
    return null;
  }
   ...
   */
  private static URL decodeSearchEngineLightBoxURL(
    final URL url,
    Map properties
  ) {
    final String query = url.getQuery();
    // special case for google image search:
    if (DEBUG.IMAGE || DEBUG.IO || DEBUG.DND) Log.debug(
      "DECODE QUERY: host " + url.getHost() + " query " + url.getQuery()
    );
    ...
    if (questionMarkIndex > 0) urlWithoutQuery =
      s.substring(0, questionMarkIndex); else urlWithoutQuery = s;

    String imageURL = (String) data.get("imgurl"); // google & yahoo
    if (imageURL == null) imageURL = (String) data.get("image_url"); // Lycos & Mamma(who are they?)
    // note: as of Aug 2005, excite gives us no option
    if (
      (imageURL == null && host.endsWith(".msn.com")) ||
      host.endsWith(".live.com")
    ) imageURL = (String) data.get("iu"); // MSN search / Live Search
    if (imageURL == null && host.endsWith(".netscape.com")) imageURL =
      (String) data.get("img"); // Netscape search
    if (imageURL == null && host.endsWith(".ask.com")) imageURL =
      (String) data.get("u"); // ask jeeves, but only from their context page
    ...
    if (
      ...
      if (DEBUG.IMAGE || DEBUG.IO || DEBUG.DND) Log.debug(
        "redirect to image search url " + imageURL
      );
      if (imageURL.indexOf(':') < 0) imageURL = "http://" + imageURL;
      redirectURL = makeURL(imageURL);
      if (redirectURL == null && !imageURL.startsWith("http://")) redirectURL =
        makeURL("http://" + imageURL);
      if (DEBUG.IMAGE || DEBUG.IO || DEBUG.DND) Log.debug(
        "redirect got URL " + redirectURL
      );
  ...
  private Point2D.Float mapToLocalLocation(
    Point2D.Float mapLocation,
    LWComponent local
  ) {
    return (Point2D.Float) local.transformMapToZeroPoint(
      mapLocation,
      new Point2D.Float()
    );
  }
  ...
  private Point2D.Float dropToFocalLocation(int x, int y) {
    final Point2D.Float mapLoc = (Point2D.Float) mViewer.screenToFocalPoint(
      x,
      y
    );
   ...
   */
  public static void setCenterAt(
    Collection<LWComponent> nodes,
    Point2D.Float mapLocation
  ) {
    if (DEBUG.DND) Log.debug(
      "setCenterAt " + mapLocation + "; " + Util.tags(nodes)
    );
  ...
  public static void setLocation(
    List<LWComponent> nodes,
    Point2D.Float mapLocation
  ) {
    if (nodes.size() == 1) {
      if (nodes.get(0).getParent() == null) nodes
        .get(0)
        .setLocation(mapLocation);
    } else {
      java.awt.geom.Rectangle2D.Float bounds = LWMap.getBounds(
        nodes.iterator()
      );
  ...
  private String readTextFlavor(DataFlavor flavor, Transferable transfer) {
    ...
    try {
      ...
      if (reader.read() != -1) System.out.println(
        "[there was more data in the reader]"
      );
    } catch (Exception e) {
  ...
  private static final String MIME_TYPE_MAC_URLN =
    "application/x-mac-ostype-75726c6e";
