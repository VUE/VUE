...
public class NodeTool
  ...
  public Class<? extends Shape>[] getAllShapeClasses() {
    ...
    int i = 0;
    for (Object o : getAllShapeValues()) classes[i++] = ((Shape) o).getClass();
    return classes;
  }
  ...
  public static class SubTool extends VueSimpleTool {
    ...
    public VueAction getShapeSetterAction() {
      if (shapeSetterAction == null) {
        shapeSetterAction =
          new Actions.LWCAction(
            getToolName(),
            new ShapeIcon(getShapeInstance())
          ) {
            void act(LWNode n) {
              n.setShapeInstance(getShapeInstance());
            }
          };
    ...
    public RectangularShape getShapeInstance() {
      if (shapeClass == null) {
        ...
        try {
          this.shapeClass =
            getClass().getClassLoader().loadClass(shapeClassName);
        } catch (Exception e) {
          e.printStackTrace();
          return null;
        }
    ...
    public RectangularShape getShape() {
      if (cachedShape == null) cachedShape = getShapeInstance();
      return cachedShape;
    }

    static final int nearestEven(double d) {
      if (
        Math.floor(d) == d && d % 2 == 1
      ) return (int) d + 1; // if exact odd integer, just increment
      if (Math.floor(d) % 2 == 0) return (int) Math.floor(
        d
    ...
    static final int nearestOdd(double d) {
      if (
        Math.floor(d) == d && d % 2 == 0
      ) return (int) d + 1; // if exact even integer, just increment
      if (Math.floor(d) % 2 == 1) return (int) Math.floor(
        d
      ); else return (int) Math.ceil(d);
    }
    ...
    private static final Color sShapeColorLight = VueUtil.factorColor(
      sShapeColor,
      1.3
    );
    ...
    static {
      ...
      if (ToolIcon.Width % 2 == 0) sWidth =
        nearestOdd(ToolIcon.Width / 2); else sWidth =
        nearestEven(ToolIcon.Width / 2);
      if (ToolIcon.Height % 2 == 0) sHeight =
        nearestOdd(ToolIcon.Height / 2); else sHeight =
        nearestEven(ToolIcon.Height / 2);
    ...
    public static class ShapeIcon implements Icon {
      ...
      public ShapeIcon(RectangularShape pShape) {
        ...
        } else mShape.setFrame(0, 0, sWidth, sHeight);
      }
  ...
  public static class OntologyNodeTool
    extends VueTool
    ...
    public boolean handleMousePressed(MapMouseEvent e) {
      ...
      if (creationNode == null) {
        VueUtil.alert(
          VueResources.getString("ontologyNodeError.message"),
          VueResources.getString("ontologyNodeError.title")
        );
        return true;
      } else return false;
    }
    ...
    public boolean handleSelectorRelease(MapMouseEvent e) {
      //LWNode node = createNode(VueResources.getString("newnode.html"), true);
      if (creationNode == null) return false;
