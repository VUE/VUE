...
public class NotePanel extends tufts.vue.gui.Widget {
  ...
  public NotePanel(String name, boolean autoAttach) {
    ...
    if (false) {
      ...
      if (tufts.Util.isMacPlatform()) {
        ...
        mTextPane.setBorder(
          new CompoundBorder(
            new MatteBorder(b, b, b, b, SystemColor.control),
            new CompoundBorder(mTextPane.getBorder(), GUI.makeSpace(insetInner))
          )
        );
      } else {
    ...
    if (autoAttach) VUE.addActiveListener(LWComponent.class, this);
  }
  ...
  private Dimension minSize(String src) {
    ...
    int MinNoteHeight =
      5 * mTextPane.getFontMetrics(mTextPane.getFont()).getHeight() +
      insets.top +
      insets.bottom;
    ...
    if (d.height < MinNoteHeight) d.height = MinNoteHeight;
  ...
  private static void setTypeName(
    ...
    if (c instanceof LWSlide && c.isPathwayOwned()) type =
      "Pathway"; else type = c.getComponentTypeLabel();
  ...
  private void load(LWComponent c) {
    attach(c);
    if (c != null) setTypeName(this, c, VueResources.getString("jlabel.notes"));
  }
    ...
    try {
      if (args.length > 0) {
        if (args[0].endsWith(".rtf")) {
          ...
          if (args.length > 1) p.mTextPane.setEditable(false);
        }
      }
