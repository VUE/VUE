...
public class MetaMap
  implements TableBag, XMLUnmarshalListener, tufts.vue.ds.Relation.Scannable {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MetaMap.class);
  ...
  private static final Map<String, Key> KEY_MAP =
  ...
  public boolean equals(Object o) {
    if (o instanceof MetaMap == false) return false;
  ...
  private static class Key {
    ...
    @Override
    public boolean equals(Object o) {
      if (KEY_DEBUG_MODE && DEBUG.META) Log.debug(
        String.format("testing [%s]=[%s]", Util.tags(this), Util.tags(o))
      );
      ...
      if (KEY_DEBUG_MODE && DEBUG.Enabled) {
        ...
        if (eq && !name.equals(o.toString())) Log.debug(
          String.format("matched mixed-case [%s] to [%s]", name, o)
        );
        return eq;
      } else {
  ...
  public synchronized void put(final String key, final Object value) {
    ...
    if (mData.put(internalKey, value)) markChange();
  }
  ...
  public synchronized void putAllStrings(final Iterable<Map.Entry> entries) {
    ...
    if (!onHold) holdChanges();
    for (Map.Entry e : entries) put(
      e.getKey().toString(),
      e.getValue().toString()
    );
    if (!onHold) releaseChanges();
  }
  ...
  private static boolean isEmpty(Object v) {
    if (v == null) return true; else if (
      v.getClass() == String.class && v.toString().length() == 0
    ) return true; else if (
  ...
  public void putNonEmpty(String key, Object value) {
    if (!isEmpty(value)) put(key, value);
  ...
  public void setNonEmpty(String key, Object value) {
    if (!isEmpty(value)) set(key, value);
    //else if (DEBUG.DATA) Log.debug("EMPTY: " + Util.tags(value));
  }
  ...
  private static Object extractFirstValue(Object o) {
    ...
    if (o instanceof Collection) {
      ...
      if (bag.isEmpty()) return null; else return bag.iterator().next();
    }
    ...
    //         }
      ...
      if (DEBUG.Enabled) Log.debug(
        "extracing 1st value as is (no collection): " + Util.tags(o)
      );
      return o;
    }
  }
  ...
  public Collection<Map.Entry<String, Object>> getPersistEntries() {
    if (mPersistEntries == null) mPersistEntries = new ArrayList();

    if (mData.size() > 0) {
      mPersistEntries.clear();
      if (DEBUG.XML) Log.debug(
        "LOADING ENTRIES n=" + mData.size() + "; in " + Util.tag(this)
      );
      ...
      for (Map.Entry e : entries()) mPersistEntries.add(new PropertyEntry(e));
    }
  ...
  public void XML_completed(Object context) {
    if (DEBUG.XML) Log.debug(
      "UNPACKING ENTRIES IN " +
      this +
      " context=" +
      context +
      "; persistEntries: " +
      Util.tags(mPersistEntries)
    );
    if (mPersistEntries == null) return;
  ...
  private void markChange() {
    if (mHoldingChanges) mChanges++; else if (
      mTableModel != null
    ) mTableModel.reload();
  }
  ...
  public synchronized java.util.Properties asProperties() {
    ...
    for (Map.Entry e : entries()) props.put(
      e.getKey().toString(),
      e.getValue().toString()
    );
  ...
  public javax.swing.table.TableModel getTableModel() {
    ...
    if (mTableModel == null) {
      synchronized (mData) { // sync on anything other than "this" just in case
        if (mTableModel == null) mTableModel = new MapModel();
      }
    }
    return mTableModel;
  }
  ...
  public synchronized void notifyListeners() {
    if (listeners != null && listeners.size() > 0) {
      ...
      if (DEBUG.RESOURCE || DEBUG.THREAD) out(
        "notifyListeners " + listeners.size() + " of " + super.toString()
      );
      Iterator i = listeners.iterator();
      while (i.hasNext()) {
        Listener l = (Listener) i.next();
        if (DEBUG.RESOURCE || DEBUG.THREAD) out(
          "notifying: " + tufts.vue.gui.GUI.namex(l)
        );
      ...
      if (DEBUG.RESOURCE || DEBUG.THREAD) out(
        "notifyListeners completed " + listeners.size()
      );
    }
  }
  ...
  public String toString() {
    ...
    try {
      ...
      if (s != null) {
        String keyFieldName = "";
        try {
          keyFieldName = s.getKeyFieldName();
        } catch (Throwable tx) {
          keyFieldName = "[" + tx + "]";
        }
        t =
  ...
  private class MapModel extends javax.swing.table.AbstractTableModel {
    ...
    private void reload() {
      ...
      if (DEBUG.RESOURCE) {
        ...
        if (DEBUG.META) out("model loaded " + mEntries);
      }
    ...
    public Object getValueAt(int row, int col) {
      ...
      if (entry == null) {
        Log.warn(
          getClass().getName(),
          new Throwable(
            "FYI: null entry at row " + row + "; col-request=" + col
          )
        );
        return null;
      } else if (col == 0) return entry.getKey();
