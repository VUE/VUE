...
public class JavaAnalysisPanel extends JPanel implements ActionListener {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(JavaAnalysisPanel.class);
  protected static final String PUBLIC_KEYWORD = "public", ABSTRACT_KEYWORD =
    "abstract", FINAL_KEYWORD = "final", CLASS_KEYWORD =
    "class", INTERFACE_KEYWORD = "interface", EXTENDS_KEYWORD =
    "extends", IMPLEMENTS_KEYWORD = "implements", METADATA_CATEGORY =
    "Java Analysis", METADATA_KEYWORD_UNDECLARED =
    "undeclared", METADATA_KEYWORD_DECLARED = "declared in ", JAVA_ANALYSIS =
    VueResources.getString("analyze.java.javaAnalysis"), HELP_TEXT =
    VueResources.getString(
      "dockWindow.JavaAnalysis.helpText"
    ), DEFAULT_FILE_CHOOSER_TITLE = VueResources.getString(
    "FileChooser.openDialogTitleText"
  ), DEFAULT_FILE_CHOOSER_OPEN_BUTTON = VueResources.getString(
    "FileChooser.openButtonText"
  ), DEFAULT_FILE_CHOOSER_OPEN_BUTTON_TOOLTIP = VueResources.getString(
    "FileChooser.openButtonToolTipText"
  ), CHOOSE_FILES = VueResources.getString(
    "analyze.java.chooseFiles"
  ), ANALYZE_CLASSES = VueResources.getString(
    "analyze.java.analyzeClasses"
  ), ANALYZE_INNER_CLASSES = VueResources.getString(
    "analyze.java.analyzeInnerClasses"
  ), ANALYZE_INTERFACES = VueResources.getString(
    "analyze.java.analyzeInterfaces"
  ), ANALYZE_INNER_INTERFACES = VueResources.getString(
    "analyze.java.analyzeInnerInterfaces"
  ), ANALYZE = VueResources.getString("analyze.java.analyze"), ANALYZE_TOOLTIP =
    VueResources.getString("analyze.java.analyzeTooltip"), STOP =
    VueResources.getString("analyze.java.stop"), PARSING =
    VueResources.getString("analyze.java.parsing"), DISPLAYING =
    VueResources.getString("analyze.java.displaying"), NO_JAVA_CLASS =
    VueResources.getString("analyze.java.noJavaClass"), JAVA_ANALYSIS_ERROR =
    VueResources.getString("analyze.java.error");
  protected static final Color DECLARED_COLOR = Color.BLACK, UNDECLARED_COLOR =
    Color.LIGHT_GRAY;
  protected static final LWComponent.StrokeStyle ABSTRACT_STROKE_STYLE =
    LWComponent.StrokeStyle.DASHED, NONABSTRACT_STROKE_STYLE =
    LWComponent.StrokeStyle.SOLID;
  protected static final float FINAL_STROKE_WIDTH = 2f, NONFINAL_STROKE_WIDTH =
    1f;
  protected static final Class<? extends RectangularShape> CLASS_SHAPE =
    Ellipse2D.Float.class, INTERFACE_SHAPE = Rectangle2D.Float.class;
  protected static final char DOLLAR_TOKEN = '$', UNDERSCORE_TOKEN =
    '_', COMMA_TOKEN = ',', SEMICOLON_TOKEN = ';', LEFT_ANGLE_TOKEN =
    '<', RIGHT_ANGLE_TOKEN = '>';
  protected static final int CLASSES_MASK = 1, INNER_CLASSES_MASK =
    2, INTERFACES_MASK = 4, INNER_INTERFACES_MASK = 8, HALF_GUTTER = 4;
  protected static final Insets HALF_GUTTER_INSETS = new Insets(
    HALF_GUTTER,
    HALF_GUTTER,
    HALF_GUTTER,
    HALF_GUTTER
  ...
  protected JCheckBox classesCheckBox = null, innerClassesCheckBox =
    null, interfacesCheckBox = null, innerInterfacesCheckBox = null;
  ...
  public JavaAnalysisPanel() {
    ...
    int checkbox_width = (int) (innerClassesCheckBox
        .getPreferredSize()
        .getHeight());
    Insets HALF_GUTTER_INSETS_INDENTED = new Insets(
      0,
      checkbox_width,
      HALF_GUTTER,
      HALF_GUTTER
  ...
  protected void analyze(int mask) {
    ...
    if (files != null) {
      ...
      int fileCount = files.length, fileIndex;
      Point2D viewerCenter = VUE.getActiveViewer().getVisibleMapCenter();
      double viewerCenterX = viewerCenter.getX(), viewerCenterY =
        viewerCenter.getY();
      ...
      for (
        fileIndex = 0;
        fileIndex < fileCount && proceedWithAnalysis;
        fileIndex++
      ) {
        ...
        if (file.isDirectory()) {
          ...
          int fileInFolderCount = filesInFolder.length, fileInFolderIndex;

          for (
            fileInFolderIndex = 0;
            fileInFolderIndex < fileInFolderCount && proceedWithAnalysis;
            fileInFolderIndex++
          ) {
            ...
            if (!fileInFolder.isDirectory()) {
              foundCount =
                parseFile(fileInFolder, mask, viewerCenterX, viewerCenterY);

              if (foundCount > 0) {
                totalFoundCount += foundCount;
              } else {
                VueUtil.alert(
                  String.format(NO_JAVA_CLASS, fileInFolder.getName()),
                  JAVA_ANALYSIS
                );
              }
            }
          ...
          if (foundCount > 0) {
            ...
            VueUtil.alert(
              String.format(NO_JAVA_CLASS, file.getName()),
              JAVA_ANALYSIS
            );
          }
        }
  ...
  protected int parseSourceFile(
    ...
    boolean analyzeClasses =
      (mask & CLASSES_MASK) == CLASSES_MASK, analyzeInnerClasses =
      (mask & INNER_CLASSES_MASK) == INNER_CLASSES_MASK, analyzeInterfaces =
      (mask & INTERFACES_MASK) == INTERFACES_MASK, analyzeInnerInterfaces =
      (mask & INNER_INTERFACES_MASK) == INNER_INTERFACES_MASK, isPublic =
      false, isAbstract = false, isFinal = false;

    FileReader stream = new FileReader(file);
    BufferedReader reader = new BufferedReader(stream);
    StreamTokenizer tokenizer = new StreamTokenizer(reader);
    String sourceName = filename.replace(
      JavaSourceFileFilter.FILE_EXTENSION,
      ""
    );

    if (DEBUG) {
      Log.info(
        "parseSourceFile() analyzing " +
        filename +
        " for" +
        (analyzeClasses
            ? " classes" +
            (analyzeInnerClasses ? " (including inner classes)" : "")
            : "") +
        (analyzeInterfaces
            ? (analyzeClasses ? "," : "") +
            " interfaces" +
            ((analyzeInnerInterfaces ? " (including inner interfaces)" : ""))
            : "") +
        "."
      );
    }
        ...
        if (tokenizer.sval.equals(PUBLIC_KEYWORD)) {
          ...
          if (analyzeClasses) {
            if (tokenizer.nextToken() == StreamTokenizer.TT_WORD) {
              String className = tokenizer.sval, extendsClassName = null;
  ...
  protected void skipGeneric(StreamTokenizer tokenizer)
    throws java.io.IOException {
  ...
  protected int parseClassFile(
    ...
    boolean analyzeClasses =
      ...
      (mask & INNER_CLASSES_MASK) == INNER_CLASSES_MASK, analyzeInterfaces =
      (mask & INTERFACES_MASK) == INTERFACES_MASK, analyzeInnerInterfaces =
      (mask & INNER_INTERFACES_MASK) == INNER_INTERFACES_MASK;

    if (DEBUG) {
      Log.info(
        "parseClassFile() analyzing " +
        filename +
        " for" +
        (analyzeClasses
            ? " classes" +
    ...
    if (classInfo.isJavaClass) {
      ...
      String classNameWithoutPackage = classRef.nameWithoutPackage, sourceName =
        attributeInfo.sourceName;
      ...
      if (accessFlags.isInterface) {
        ...
        if (analyzeClasses && (analyzeInnerClasses || isNotInner)) {
          newClass(
            ...
            classRef.name,
            superclassRef.name,
            (analyzeInterfaces ? interfaces.names : null),
            x,
            y
          );
        }
  ...
  protected void newClass(
    ...
    ArrayList<String> implementsInterfaceNames,
    double x,
    double y
  ) {
    ...
    if (implementsInterfaceNames != null) {
      ...
      while (iterator.hasNext()) {
        ...
        LWNode implementsNode = findOrCreateInterfaceNode(
        ...
        if (DEBUG) {
          debugMessage.append(
            implementsNode.label + (iterator.hasNext() ? ", " : "")
          );
        }
      }
    }
    ...
    if (DEBUG) {
      debugMessage.append(
        sourceName != null ? "(declared in " + sourceName + ")." : "."
      );
  ...
  protected void newInterface(
    ...
    ArrayList<String> extendsInterfaceNames,
    double x,
    double y
  ) {
    StringBuffer debugMessage = null;
    ...
    if (extendsInterfaceNames != null) {
      ...
      while (iterator.hasNext()) {
        ...
        LWNode extendsNode = findOrCreateInterfaceNode(
        ...
        if (DEBUG) {
          debugMessage.append(
            extendsNode.label + (iterator.hasNext() ? ", " : "")
          );
        }
      }
    }
  ...
  protected LWNode findOrCreateInterfaceNode(
    String interfaceName,
    double x,
    double y
  ) {
    ...
    if (interfaceNode == null) {
      ...
      // visible part of the map.
      interfaceNode =
        newNode(interfaceName, x, y, INTERFACE_SHAPE, INTERFACE_KEYWORD);
    }

    return interfaceNode;
  }

  protected LWNode newNode(
    String nodeName,
    double x,
    double y,
    Class<? extends RectangularShape> shape,
    String metadata
  ) {
  ...
  protected void setIsDeclared(
    ...
    metadataList.removeCategoryType(
      METADATA_CATEGORY,
      METADATA_KEYWORD_UNDECLARED
    );
    ...
    if (
      isAbstract && !metadataList.contains(METADATA_CATEGORY, ABSTRACT_KEYWORD)
    ) {
    ...
    if (sourceName != null) {
      metadataList.add(
        METADATA_CATEGORY,
        METADATA_KEYWORD_DECLARED + sourceName
      );
    }
  }
  ...
  protected LWLink findOrCreateLink(
    LWNode node,
    LWNode parentNode,
    String label
  ) {
    ...
    for (LWLink existingLink : node.getLinks()) {
      if (
        existingLink.hasEndpoint(parentNode) &&
        existingLink.getLabel().equals(label)
      ) {
        link = existingLink;
        break;
      }
  ...
  protected File[] showFileChooser() {
    ...
    try {
      ...
      if (
        chooser.showOpenDialog(VUE.getDialogParent()) ==
      ...
      chooser.setApproveButtonToolTipText(
        DEFAULT_FILE_CHOOSER_OPEN_BUTTON_TOOLTIP
      );
    }
  ...
  public void actionPerformed(ActionEvent event) {
    ...
    if (source == classesCheckBox || source == interfacesCheckBox) {
      analyzeButton.setEnabled(
        classesCheckBox.isSelected() || interfacesCheckBox.isSelected()
      );
      ...
      int analysisMask =
        (classesCheckBox.isSelected() ? CLASSES_MASK : 0) |
        (innerClassesCheckBox.isEnabled() && innerClassesCheckBox.isSelected()
            ? INNER_CLASSES_MASK
            : 0) |
        (interfacesCheckBox.isSelected() ? INTERFACES_MASK : 0) |
        (innerInterfacesCheckBox.isEnabled() &&
            innerInterfacesCheckBox.isSelected()
            ? INNER_INTERFACES_MASK
            : 0);
  ...
  protected static void addToGridBag(
    Container container,
    Component component,
    int gridX,
    int gridY,
  ...
  private class ClassInfo {
    ...
    ClassInfo(DataInputStream stream) throws java.io.IOException {
      ...
      if (isJavaClass) {
        short compilerMinorVersion = stream.readShort(), compilerMajorVersion =
          stream.readShort();
        if (DEBUG) {
    ...
    ConstantPool(DataInputStream stream) throws java.io.IOException {
      ...
      for (int index = 1; index <= constantPoolCount; index++) {
        ...
        switch (type) {
          ...
          default:
            VueUtil.alert(
              "Unknown constant pool type: '" + type + "'",
              JAVA_ANALYSIS_ERROR
            );
        }
  ...
  private class ConstantPoolItemFieldRef extends ConstantPoolItem {
    ...
    ConstantPoolItemFieldRef(DataInputStream stream)
      throws java.io.IOException {
    ...
    public String toString() {
      return (
        "classIndex " +
        Short.toString(classIndex) +
        ", nameAndTypeIndex " +
        Short.toString(nameAndTypeIndex)
      );
    }
  }
  ...
  private class ConstantPoolItemMethodRef extends ConstantPoolItem {
    ...
    ConstantPoolItemMethodRef(DataInputStream stream)
      throws java.io.IOException {
      classIndex = stream.readShort();
      nameAndTypeIndex = stream.readShort();
    }
  ...
  private class ConstantPoolItemInterfaceMethodRef extends ConstantPoolItem {
    ...
    ConstantPoolItemInterfaceMethodRef(DataInputStream stream)
      throws java.io.IOException {
      classIndex = stream.readShort();
      nameAndTypeIndex = stream.readShort();
    }
  ...
  private class ConstantPoolItemNameAndType extends ConstantPoolItem {
    ...
    ConstantPoolItemNameAndType(DataInputStream stream)
      throws java.io.IOException {
      nameIndex = stream.readShort();
    ...
    public String toString() {
      return (
        "nameIndex " +
        Short.toString(nameIndex) +
        ", descriptorIndex " +
        Short.toString(descriptorIndex)
      );
    }
  }
  ...
  private class AccessFlags {
    ...
    protected static final int ACC_PUBLIC = 0x0001, ACC_FINAL =
      0x0010, ACC_SUPER = 0x0020, ACC_INTERFACE = 0x0200, ACC_ABSTRACT = 0x0400;
    public boolean isInterface, isPublic, isAbstract, isFinal;
  ...
  private class ClassRef {
    ...
    public String name = null, nameWithoutPackage = null;

    ClassRef(DataInputStream stream, ConstantPool pool)
      throws java.io.IOException {
      short classIndex = stream.readShort();

      ConstantPoolItemClass classItem =
        (ConstantPoolItemClass) pool.items[classIndex];
      ConstantPoolItemUTF8 classNameItem =
        (ConstantPoolItemUTF8) pool.items[classItem.nameIndex];
      ...
      nameWithoutPackage =
        (packageNameLength > 0 ? name.substring(packageNameLength) : name);
  ...
  private class SuperclassRef {
    ...
    SuperclassRef(DataInputStream stream, ConstantPool pool)
      throws java.io.IOException {
      short classIndex = stream.readShort();
  ...
  private class Interfaces {
    ...
    Interfaces(DataInputStream stream, ConstantPool pool)
      throws java.io.IOException {
      ...
      if (interfaceCount > 0) {
        ...
        for (int index = 0; index < interfaceCount; index++) {
          ...
          ConstantPoolItemClass interfaceItem =
            (ConstantPoolItemClass) pool.items[interfaceIndex];
          ConstantPoolItemUTF8 interfaceNameItem =
            (ConstantPoolItemUTF8) pool.items[interfaceItem.nameIndex];
  ...
  private class FieldInfo {
    ...
    FieldInfo(DataInputStream stream) throws java.io.IOException {
      ...
      for (int fieldIndex = 0; fieldIndex < fieldCount; fieldIndex++) {
        short accessFlags = stream.readShort(), nameIndex =
          stream.readShort(), descriptorIndex =
          stream.readShort(), attributeCount = stream.readShort();
  ...
  private class AttributeInfo {
    ...
    AttributeInfo(DataInputStream stream, ConstantPool pool)
      throws java.io.IOException {
      ...
      for (
        ...
        if (
          ((ConstantPoolItemUTF8) pool.items[nameIndex]).value.equals(
              ATTRIBUTE_SOURCE_FILE
            )
        ) {
          short sourceFileIndex = stream.readShort();
          ConstantPoolItemUTF8 sourceNameItem =
            (ConstantPoolItemUTF8) pool.items[sourceFileIndex];

          sourceName =
            sourceNameItem.value.replace(
              JavaSourceFileFilter.FILE_EXTENSION,
              ""
            );
  ...
  protected class JavaSourceFileFilter
  ...
  protected class JavaClassFileFilter
    extends javax.swing.filechooser.FileFilter
    implements FileFilter {
  ...
  protected class JavaArchiveFileFilter
    extends javax.swing.filechooser.FileFilter
    implements FileFilter {
