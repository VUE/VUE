 ...
 *
 */
public class MapInspectorPanel extends JPanel implements ActiveListener<LWMap> {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MapInspectorPanel.class);
  ...
  public static final String dcCreator =
    "http://purl.org/dc/elements/1.1/#Creator";
  public static final String dcDescription =
  ...
  public MapInspectorPanel(DockWindow w) {
    ...
    mDescriptionPanel.setName(
      VueResources.getString("mapinspectorpanel.description")
    );
    ...
    if (DEBUG.TWITTER) mapInfoStack.addPane(mTwitterPanel, 0f);
    ...
    Widget.setHelpAction(
      metadataPanel,
      VueResources.getString("dockWindow.MapInfo.MapKeywords.helpText")
    );
  ...
  public void activeChanged(ActiveEvent<LWMap> e) {
    ...
    if (
      e.active == null && mAuthorEditor != null && mDescriptionPanel != null
    ) {
  ...
  public class InfoPanel
    extends JPanel
    implements PropertyChangeListener, FocusListener, LWComponent.Listener {

    //JTextField mTitleEditor = null;
    //JText Field mAuthorEditor = null;
    //VueTextPane mAuthorEditor = null;
    JLabel mDate = null, mObjectStats = null, mConnectivityStats = null;

    // VUE-1001
    //JLabel mLocation = null;
    PropertyPanel mPropPanel = null;
    PropertiesEditor propertiesEditor = null;
    ColorMenuButton mMapColor = new ColorMenuButton(
      VueResources.getColorArray("fillColorValues"),
      true
    );

    public InfoPanel() {
      mMapColor.setToolTipText(
        VueResources.getString("mapinspectorpanel.mapcolor")
      );
      ...
      mPropPanel.addProperty(
        VueResources.getString("mapinspectorpanel.created"),
        mDate
      );
      mPropPanel.addProperty(
        VueResources.getString("mapinspectorpanel.creator"),
        mAuthorEditor
      ); //added through metadata
      mPropPanel.addProperty(
        VueResources.getString("mapinspectorpanel.background"),
        mMapColor
      );
      mPropPanel.addProperty(
        VueResources.getString("mapinspectorpanel.objects"),
        mObjectStats
      );
      mPropPanel.addProperty(
        VueResources.getString("mapinspectorpanel.connectivity"),
        mConnectivityStats
      );
      ...
      mAuthorEditor.addFocusListener(
        new FocusAdapter() {
          public void focusLost(FocusEvent e) {
            ...
            vme.setType(
              edu.tufts.vue.metadata.VueMetadataElement.RESOURCE_CATEGORY
            );
            if (
              currentMap.getMetadataList().findRCategory(dcCreator) != -1
            ) mMap.getMetadataList().replaceValueForKey(vme); else mMap
              .getMetadataList()
              .getMetadata()
              .add(vme);
          }
        ...
        .addFocusListener(
          new FocusAdapter() {
            public void focusLost(FocusEvent e) {
              ...
              String[] pairedValue = {
                dcDescription,
                LWComponent.escapeWhitespace(
                  mDescriptionPanel.getTextPane().getText()
                ),
              };
              vme.setObject(pairedValue);
              vme.setType(
              ...
              if (
                currentMap.getMetadataList().findRCategory(dcDescription) != -1
              ) mMap.getMetadataList().replaceValueForKey(vme); else mMap
                .getMetadataList()
                .getMetadata()
                .add(vme);
    ...
    public void updatePanel(LWMap pMap) {
      ...
      edu.tufts.vue.metadata.VueMetadataElement creator = mMap
        .getMetadataList()
        .get(dcCreator);
      String creatorValue = "";
      if (creator != null) {
      ...
      edu.tufts.vue.metadata.VueMetadataElement description = mMap
        .getMetadataList()
        .get(dcDescription);
      if (description != null) {
        ...
        mDescriptionPanel
          .getTextPane()
          .setText(LWComponent.unEscapeWhitespace(descriptionValue));
      }
    ...
    public void LWCChanged(LWCEvent event) {
      if (
        event.key == LWKey.LinkAdded ||
        event.key == LWKey.LinkRemoved ||
        event.key == LWKey.Created ||
        event.key == LWKey.Deleting
      ) {
        updateStats();
      }
    }
  ...
  public class PathwayPane extends JPanel {
    ...
    public PathwayPane() {
      ...
      mPathScrollPane.setVerticalScrollBarPolicy(
        JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED
      );
      mPathScrollPane.setHorizontalScrollBarPolicy(
  ...
  public static void main(String args[]) {
    ...
    DockWindow w = GUI.createDockWindow(
      VueResources.getString("dockWindow.mapinspector.title"),
      inspector
    );
    w.setVisible(true);
    if (args.length > 1) VueUtil.displayComponent(
      new VueTextPane(map, LWKey.Notes, null)
    );
  }
}
