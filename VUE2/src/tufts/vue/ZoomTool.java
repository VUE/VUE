 ...
 *
 */
public class ZoomTool extends VueTool implements VueConstants {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ZoomTool.class);

  private static final int ZOOM_MANUAL = -1;
  private static final double[] ZoomDefaults = {
  ...
  private static final int ZOOM_FIT_PAD = 20; // make this is > SelectionStrokeWidth & SelectionHandleSize
  private static final double MAX_ZOOM = ZoomDefaults[ZoomDefaults.length - 1];
  ...
  */
  ...
  public boolean supportsDraggedSelector(MapMouseEvent e) {
    if (
      false && e.getPicked() != null
    ) return false; // is causing a pick traversal for ever mouse drag event: too slow
  ...
  public boolean handleMousePressed(MapMouseEvent e) {
    ...
    if (isZoomInMode() || isZoomOutMode()) {
      if (zoomedTo != null) zoomedTo = null;
    ...
    if (disableToggleZoom) return false;
    ...
    if (zoomedTo != null && !e.isShiftDown()) {
      ...
      if (oldFocal != null) {
        ...
        if (
          tufts.vue.gui.FullScreen.inWorkingFullScreen()
        ) ZoomTool.setZoomFitRegion(
          tufts.vue.gui.FullScreen.getObscuredViewer(),
          VUE.getActiveMap().getTempBounds()
        );
        ...
        if (VUE.getActiveMap().getTempBounds() != null) {
          ...
          if (tufts.vue.gui.FullScreen.inWorkingFullScreen()) setZoomFit(
            tufts.vue.gui.FullScreen.getObscuredViewer(),
            true
          );
          setZoomFit(viewer, true);
      ...
      if (picked instanceof LWSlide) {
        if (zoomedTo == picked) {
          if (VUE.getActiveViewer().getFocal() instanceof LWSlide) return false;
          ...
          if (zoomedTo == null) {
            ...
            VUE
              .getActiveMap()
              .setTempUserOrigin(VUE.getActiveViewer().getOriginLocation());
            VUE
              .getActiveMap()
              .setTempBounds(VUE.getActiveViewer().getVisibleMapBounds());
          }
          if (tufts.vue.gui.FullScreen.inWorkingFullScreen()) zoomToSlide(
            tufts.vue.gui.FullScreen.getObscuredViewer(),
            (LWSlide) picked
          );
        ...
        if (e.isControlDown()) zoomBounds = picked.getFanBounds(); else {
          if (!e.isShiftDown()) {
            VUE
              .getActiveMap()
              .setTempZoom(VUE.getActiveViewer().getZoomFactor());
        ...
        if (tufts.vue.gui.FullScreen.inWorkingFullScreen()) setZoomFitRegion(
          tufts.vue.gui.FullScreen.getObscuredViewer(),
          zoomBounds,
          0,
          true
        ...
        setZoomFitRegion(viewer, zoomBounds, 0, true);
  ...
  public DrawContext getDrawContext(DrawContext dc) {
    if (zoomedTo instanceof LWSlide); else dc.skipDraw = zoomedTo; // don't draw now: force on top later
    return dc;
  }

  @Override
  public void handlePostDraw(DrawContext dc, MapViewer viewer) {
    if (zoomedTo instanceof LWSlide) {} else if (zoomedTo != null) {
  ...
  public PickContext initPick(PickContext pc, float x, float y) {
    if (zoomedTo != null) pc.pickDepth = zoomedTo.getPickLevel() + 1;
    return pc;
  }
  ...
  public boolean handleMouseReleased(MapMouseEvent e) {
    ...
    if (
      e.isShiftDown() || e.getButton() != MouseEvent.BUTTON1
      //|| toolKeyEvent != null && toolKeyEvent.isShiftDown()
    ) {
      if (isZoomOutMode() || disableToggleZoom) setZoomBigger(p);
      //    else if (isZoomOutToMapMode())
      //    	setZoom(1.0);
      else setZoomSmaller(p);
    } else {
      ...
      if (box != null && box.width > 10 && box.height > 10) {
        ...
        if (isZoomOutMode()) setZoomSmaller(p);
        //      else if (isZoomOutToMapMode())
        //    	setZoom(1.0);
        else setZoomBigger(p);
      }
    }
    return true;
  }
  ...
  private static void setZoom(
    ...
    boolean adjustViewport,
    Point2D focus,
    boolean reset
  ) {
    if (newZoomFactor < 0.001) newZoomFactor = 0.001; else if (
      newZoomFactor > 100000
    ) newZoomFactor = 100000;

    // this is much simpler as the viewer now handles adjusting for the focal point
    if (focus == DONT_FOCUS) focus = null;
    ...
    //}
      ...
      // viewport.
      if (DEBUG.SCROLL) System.out.println(
        "VISIBLE CENTER " + viewer.getVisibleCenter()
      );
...
*/
  ...
  public static void setZoomFitRegion(
    ...
    if (mapRegion == null) {
      new Throwable("setZoomFitRegion: mapRegion is null for " + viewer)
        .printStackTrace();
      return;
    }
    if (viewer == null) throw new NullPointerException(
      "null viewer in setZoomFitRegion"
    );

    final Point2D.Float offset = new Point2D.Float();
    final double newZoom = computeZoomFit(
      viewer.getVisibleSize(),
      borderGap,
      mapRegion,
      offset
    ...
    if (!viewer.canAnimate()) {
      Point2D center = new Point2D.Double(
        mapRegion.getCenterX(),
        mapRegion.getCenterY()
      );
      //             if (newZoom > MaxZoom)
      //                 newZoom = MaxZoom;
          ...
          ((VUE.getActiveViewer() == null) ||
            (VUE.getActiveViewer() != null &&
              (VUE.getActiveViewer().getFocal() == null ||
                VUE.getActiveViewer().getFocal() instanceof LWMap))) &&
          fitAndCenterMap)
      ) viewer.setZoomFactor(1, false, center, true); else viewer.setZoomFactor(
        newZoom,
        false,
        center,
        true
      ...
      if (animate) animatedZoomTo(viewer, newZoom, offset.x, offset.y);
  ...
  private static void doAnimatedZoomTo(
    ...
    for (int i = 1; i < frames; i++) {
      ...
      if (DEBUG.VIEWER || DEBUG.PAINT) Log.debug(
        String.format("zoomAnimate frame %2d/%d %.1f%%", i, frames, zoom * 100)
      );
  ...
  public static void setZoomFit(MapViewer viewer, boolean animate) {
    ...
    setZoomFitRegion(
      viewer,
      viewer.getDisplayableMapBounds(),
      DEBUG.MARGINS ? 0 : ZOOM_FIT_PAD,
      animate
    );
  ...
  public static void setZoomOutFit() {
    final Point2D.Float offset = new Point2D.Float();
    double zoomFactor = computeZoomFit(
      VUE.getActiveViewer().getVisibleSize(),
      0f,
      VUE.getActiveViewer().getDisplayableMapBounds(),
      offset
    );
    if (zoomFactor < 1) {
      setZoomFit();
  ...
  public static double computeZoomFit(
    Dimension2D viewport,
    float borderGap,
    Rectangle2D bounds,
    Point2D outgoingOffset
  ) {
    return computeZoomFit(
      viewport,
      borderGap,
      bounds,
      outgoingOffset,
  ...
  public static double computeZoomFit(
    ...
    if (
      viewport.getWidth() <= 0 || viewport.getHeight() <= 0 || bounds.isEmpty()
    ) {
      return 1.0;
    } else {
      return computeZoomFitImpl(
        viewport,
        borderGap,
        bounds,
        outgoingOffset,
  ...
  private static double computeZoomFitImpl(
    final Dimension2D viewport,
    final float borderGap,
    Rectangle2D bounds,
    ...
    if (outgoingOffset != null) {
      ...
      if (centerSmallerDimensionInViewport) {
        if (centerVertical) offsetY -=
        (viewHeight - bounds.getHeight() * newZoom) / 2; else offsetX -= // center horizontal
        (viewWidth - bounds.getWidth() * newZoom) / 2;
      }
