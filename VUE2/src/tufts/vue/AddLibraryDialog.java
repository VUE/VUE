...
public class AddLibraryDialog
  extends SizeRestrictedDialog
  implements ListSelectionListener, ActionListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(AddLibraryDialog.class);
  ...
  JButton addButton = new JButton(
    VueResources.getString("addLibrary.ok.label")
  );
  JButton cancelButton = new JButton(
    VueResources.getString("addLibrary.cancel.label")
  );
  ...
  private static String MY_COMPUTER = VueResources.getString(
    "addLibrary.mycomputer.label"
  );
  private static String ZOTERO = VueResources.getString(
    "addLibrary.zotero.label"
  );
  private static String ZOTERO_DESCRIPTION = VueResources.getString(
    "addLibrary.zotero.description"
  );
  private static String MY_COMPUTER_DESCRIPTION = VueResources.getString(
    "addLibrary.addbrowsecontrol"
  );
  private static String MY_SAVED_CONTENT = VueResources.getString(
    "addLibrary.mysavedcontent"
  );
  private static String MY_SAVED_CONTENT_DESCRIPTION = VueResources.getString(
    "addLibrary.savedcontent"
  );
  private static String DS_FTP = VueResources.getString("addLibrary.ftp");
  private static String DS_FTP_DESCRIPTION = VueResources.getString(
    "addLibrary.ftpsite"
  );
  private static String DS_RSS = VueResources.getString("addLibrary.rssfeed");
  private static String DS_RSS_DESCRIPTION = VueResources.getString(
    "addLibrary.rssfeedsite"
  );

  private static String TITLE = VueResources.getString(
    "addLibrary.addaresource"
  );
  private static String AVAILABLE = VueResources.getString(
    "addLibrary.resourcesavailable"
  );

  private static String LOADING = VueResources.getString(
    "addLibrary.loading.label"
  );
  ...
  public AddLibraryDialog(DataSourceList dataSourceList) {
    ...
    try {
      ...
      VueUtil.alert(
        this,
        VueResources.getString("dialog.error.message"),
        VueResources.getString("dialog.error.title")
      );
    }

    try {
      VueLabel helpButton = new VueLabel(
        VueResources.getImageIcon("addLibrary.helpIcon")
      );
      ...
      listJsp.setVerticalScrollBarPolicy(
        javax.swing.JScrollPane.VERTICAL_SCROLLBAR_ALWAYS
      );
      listJsp.setHorizontalScrollBarPolicy(
        javax.swing.JScrollPane.HORIZONTAL_SCROLLBAR_NEVER
      );
      listJsp.setPreferredSize(new Dimension(300, 180));

      descriptionTextArea.setText(
        VueResources.getString("addLibraryDailog.textarea")
      );
      descriptionJsp = new JScrollPane(descriptionTextArea);
      descriptionJsp.setVerticalScrollBarPolicy(
        javax.swing.JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED
      );
      descriptionJsp.setHorizontalScrollBarPolicy(
        javax.swing.JScrollPane.HORIZONTAL_SCROLLBAR_NEVER
      );
  ...
  private void populate() {
    ...
    try {
      ...
      // System.out.println("In Add Library Dialog, asking Provider for list of
      // Providers");
      ...
      while (providerIterator.hasNextProvider()) {
        org.osid.provider.Provider nextProvider =
          providerIterator.getNextProvider();
        // only latest
      ...
      while (providerIterator.hasNextProvider()) {
        ...
        if (
          (nextProvider.needsUpdate() == false) &&
    ...
    if (index != -1) {
      try {
        if (((JList) lse.getSource()).getSelectedValue() instanceof String) {
          ...
          org.osid.provider.Provider p =
            (org.osid.provider.Provider) (((JList) lse.getSource()).getSelectedValue());
    ...
    try {
      ...
      if (o instanceof String) {
        ...
        if (s.equals(MY_COMPUTER)) {
          ...
          xml =
            "<?xml version=\"1.0\" encoding=\"UTF-8\"?><configuration><field><key>name</key><title>Name</title><description>Name for this datasource</description><default>DEFAULT_NAME</default><mandatory>true</mandatory><maxChars></maxChars><ui>0</ui></field></configuration>";
          String name = ds.getDisplayName();
          xml = xml.replaceFirst("DEFAULT_NAME", name);
          this.oldDataSource = ds;
        } /*
        ...
        try {
                ...
                SwingUtilities.invokeLater(
                  new Runnable() {
                    public void run() {
                      ...
                      AddLibraryDialog.this.getRootPane()
          ...
          if (proceed && (!provider.isInstalled())) {
            ...
            try {
              ...
              VueUtil.alert(
                this,
                VueResources.getString("dialog.installerror.messaged"),
                VueResources.getString("dialog.error.title")
              );
              // cancelButton.requestFocus();
              SwingUtilities.invokeLater(
          ...
          if (proceed) {
            ...
            try {
              ...
              ds =
                new edu.tufts.vue.dsm.impl.VueDataSource(
                  factory.getIdManagerInstance().createId(),
                  provider.getId(),
                  true
                );
            } catch (Throwable t) {
              VueUtil.alert(
                this,
                VueResources.getString("dialog.loadfailed.message"),
                VueResources.getString("dialog.error.title")
              );
              return;
            }
          ...
          VueUtil.alert(
            this,
            t.getMessage(),
            VueResources.getString("dialog.osidinstall.title")
          );
          t.printStackTrace();
          // cancelButton.requestFocus();
          SwingUtilities.invokeLater(
      ...
      if (xml != null) {
        edu.tufts.vue.ui.ConfigurationUI cui =
          new edu.tufts.vue.ui.ConfigurationUI(
        ...
        if (
          ...
          if (s != null) {
            if (s.equals(MY_COMPUTER)) {
              ...
              java.util.Properties p = cui.getProperties();
              RemoteFileDataSource ds =
                (RemoteFileDataSource) this.oldDataSource;
              ds.setDisplayName(p.getProperty("name"));
              ...
              try {
                ...
                proceed = false;
                VueUtil.alert(
                  VueResources.getString("dialog.connectftp.message"),
                  VueResources.getString("dialog.connectionerror.title")
                );
                ex.printStackTrace();
            ...
            try {
                ...
                new Runnable() {
                  public void run() {
                    try {
                      ...
                      // DSM handles the synchronization itself -- locking hear can deadlock -- SMF
                      // 2007-11-19
              ...
              VueUtil.alert(
                this,
                VueResources.getString("dialog.addresourceerror.message"),
                VueResources.getString("dialog.addresourceerror.title")
              );
              t2.printStackTrace();
            } finally {
              GUI.clearWaitCursor();
  ...
  public void actionPerformed(ActionEvent ae) {
    if (
      ae
        .getActionCommand()
        .equals(VueResources.getString("addLibrary.ok.label"))
    ) {
