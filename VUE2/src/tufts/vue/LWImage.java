...
public class LWImage extends LWComponent implements ImageRef.Listener {
  ...
  public LWImage duplicate(CopyContext cc) {
    ...
    if (!hasFlag(Flag.UNSIZED)) newImage.clearFlag(Flag.UNSIZED);
  ...
  private void updateNodeIconStatus(LWContainer parent) {
    ...
    if (DEBUG.IMAGE) out(
      "updateNodeIconStatus " + mImageRef + "; parent=" + parent
    );
    ...
    if (selected && !wasSelected && hasImageError() && hasResource()) {
      ...
      addCleanupTask(
        new Runnable() {
          public void run() {
            if (
              hasResource() && VUE.getSelection().only() == LWImage.this
  ...
  public void setResource(Resource r) {
    ...
    if (mXMLRestoreUnderway || r == null) {
      ...
      if (r == null) setFlag(Flag.UNSIZED);
    } else {
      initRef(r);
    }
  }
  ...
  public synchronized void imageRefUpdate(Object cause) {
    ...
    if (DEBUG.IMAGE || DEBUG.WORK) out(
      "imageRefUpdate: cause=" + Util.tags(cause)
    );
    ...
    if (hasFlag(Flag.UNSIZED) && ref().fullPixelSize() != ImageRef.ZERO_SIZE) {
      ...
      if (DEBUG.IMAGE) out(
        "imageRefUpdate: SIZE IS UNSET; pixels=" +
        Util.tags(ref().fullPixelSize())
      );
      ...
      if (isNodeIcon()) autoShape(); else setToNaturalSize(); // PROBLEM?: shouldn't be undoable, tho it's already working that way because of general setSizeImpl?
    }
  ...
  private int[] getResourceImageSize(Resource r) {
    if (r == null || r.getProperties() == null) return null;

    if (DEBUG.IMAGE) out(
      "checking for resource size props in: " + r.getProperties().asProperties()
    );
  ...
  @Override
  protected synchronized void setSizeImpl(
    float w,
    float h,
    final boolean internal
  ) {
    if (DEBUG.Enabled) out(
      "setSizeImpl " + w + "x" + h + "; internal=" + internal
    );
    ...
    if (internal) {
      if (!hasFlag(Flag.UNSIZED)) {
        if (DEBUG.Enabled) Log.error(
          "ATTEMPT TO DE-VALIDATE SIZE! " + this,
          new Throwable("HERE")
        );
        //setFlag(Flag.UNSIZED);
      }
  ...
  protected void userSetSize(float width, float height, MapMouseEvent e) {
    ...
    if (DEBUG.IMAGE) out(
      "userSetSize " +
      Util.fmt(new Point2D.Float(width, height)) +
      "; aspect=" +
      aspect +
      "; e=" +
      e
    ...
    if (e != null && e.isShiftDown()) {
      ...
      Log.error(
        "unhandled userSetSize w/aspect " +
        aspect +
        ": " +
        width +
        "x" +
  ...
  private ImageRef refLoaded() {
    if (mImageRef == ImageRef.EMPTY && hasResource()) return initRef(
      getResource()
    ); else return ref();
  }
  ...
  private ImageRef initRef(Resource r) {
    if (DEBUG.IMAGE || DEBUG.WORK) {
      ...
      if (!java.awt.EventQueue.isDispatchThread()) out(
        "initRef: NOT ON AWT: " + Thread.currentThread()
      );
    }

    if (mImageRef != ImageRef.EMPTY && mImageRef.source().original == r) {
      Log.info(
        "re-init of same image source: " + this + "; " + r,
        new Throwable("HERE")
      );
    }
  ...
  private void guessAtBestSize(Resource r) {
    ...
    if (fullSize != ImageRef.ZERO_SIZE) {
      ...
      if (suggestSize != null) {
        ...
        if (isNodeIcon()) guess =
          Images.fitInto(DefaultIconMaxSide, guess); else if (
