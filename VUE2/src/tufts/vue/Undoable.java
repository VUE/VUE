...
abstract class Undoable {
  ...
  void undo() {
    if (old instanceof Boolean) undo(((Boolean) old).booleanValue()); else if (
      old instanceof Integer
    ) undo(((Integer) old).intValue()); else if (old instanceof String) undo(
      ((String) old)
    ); else if (old instanceof Float) undo(
      ((Float) old).floatValue()
    ); else if (old instanceof Double) undo(
      ((Double) old).doubleValue()
    ); else if (old instanceof Object[]) undo(
      (Object[]) old
  ...
  class TypeException extends RuntimeException {
    ...
    TypeException() {
      this(
        Undoable.this +
        ": must override undo(<type> arg) with argument matching old value type " +
        old.getClass()
      );
    }
  }
  ...
  public String toString() {
    ...
    String s =
      ...
      getClass().getName() +
      (m == null ? "" : ("/" + m.getName()));
    if (old instanceof Object[]) s +=
    "{" + java.util.Arrays.asList((Object[]) old) + "}"; else if (
      old != null
    ) s += "[" + old + "]";
    return s;
  }
