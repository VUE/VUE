...
public class LWLink
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWLink.class);
  ...
  public static final Font DEFAULT_FONT = VueResources.getFont("link.font");
  public static final Color DEFAULT_LABEL_COLOR = java.awt.Color.darkGray;
  ...
  /** both endpoints have arrows */public static final int ARROW_BOTH =
    ARROW_HEAD + ARROW_TAIL;

  /** @deprecated -- use ARROW_HEAD */public static final int ARROW_EP1 =
    ARROW_HEAD;
  /** @deprecated -- use ARROW_TAIL */public static final int ARROW_EP2 =
    ARROW_TAIL;

  // todo: create set of arrow types
  private static final float ArrowBase = 5;
  private static final RectangularShape HeadShape =
    new tufts.vue.shape.Triangle2D(0, 0, ArrowBase, ArrowBase * 1.3);
  private static final RectangularShape TailShape =
    new tufts.vue.shape.Triangle2D(0, 0, ArrowBase, ArrowBase * 1.3);
  ...
  private static final class End extends Point2D.Float {
    ...
    void setPoint(
      ...
      if (isConnected()) {
        ...
        if (um != null && !um.isUndoing()) {
          if (DEBUG.Enabled) Util.printClassTrace(
            "tufts.vue",
            this +
            "; setting pixel point for connected link: " +
            newX +
            "," +
            newY +
            "; " +
        ...
        } else; // this is needed during undo
      }
    ...
    public String toString() {
      String s = "End[";
      if (pruned) s += "PRUNED "; else s += "-open- ";
  ...
  private transient LWIcon.Block mIconBlock = new LWIcon.Block(
    this,
    11,
    9,
    Color.darkGray,
  ...
  public static final Key KEY_LinkArrows = new Key<LWLink, Object>(
    "link.arrows",
    "vue-head;vue-tail"
  ) {
    ...
    public boolean setValueFromCSS(LWLink c, String cssKey, String cssValue) {
      ...
      if (cssKey.equals("vue-tail")) {
        if (cssValue.equals("on")) {
          ...
          cssValue.equals("off") && (!vueHeadOnFromCSS || vueHeadOffFromCSS)
        ) {
          c.setArrowState(ARROW_NONE);
        }
  ...
  private final IntProperty mArrowState = new IntProperty(
    KEY_LinkArrows,
    ARROW_TAIL
  ) {
    void onChange() {
  ...
  public static final Key KEY_LinkShape = new Key<LWLink, Integer>(
    "link.shape"
  ) { // do we want this to be a KeyType.STYLE? could argue either way...
    @Override
  ...
  public static final Key KEY_LinkHeadPoint = new Key<LWLink, Point2D>(
    "link.head.location"
  ) {
    @Override
    public void setValue(LWLink l, Point2D val) {
      l.setHeadPoint(val);
    }

    @Override
    public Point2D getValue(LWLink l) {
      return l.getHeadPoint();
    }
  };
  public static final Key KEY_LinkTailPoint = new Key<LWLink, Point2D>(
    "link.tail.location"
  ) {
    @Override
    public void setValue(LWLink l, Point2D val) {
  ...
  public static final Key KEY_HeadPruned = new Key<LWLink, Boolean>(
    "prune.link.head"
  ) {
    @Override
    public void setValue(LWLink l, Boolean b) {
      l.setHeadUserPruned(b);
    }

    @Override
    public Boolean getValue(LWLink l) {
      return l.head.pruned;
    }
  };
  public static final Key KEY_TailPruned = new Key<LWLink, Boolean>(
    "prune.link.tail"
  ) {
    @Override
    public void setValue(LWLink l, Boolean b) {
      l.setTailUserPruned(b);
    }

    @Override
    public Boolean getValue(LWLink l) {
      return l.tail.pruned;
    }
  };

  private static final String Key_Control_0 = "link.control.0";
  private static final String Key_Control_1 = "link.control.1";
  ...
  public Object getPropertyValue(Object key) {
    ...
    if (key == Key_Control_0) return getCtrlPoint0(); else if (
      key == Key_Control_1
    ) return getCtrlPoint1(); else return super.getPropertyValue(key);
  }

  @Override
  public void setPropertyImpl(Object key, Object val, Object context) {
    ...
    } else if (key == Key_Control_0) setCtrlPoint0((Point2D) val); else if (
      key == Key_Control_1
    ) setCtrlPoint1((Point2D) val); else super.setPropertyImpl(
      key,
      val,
      context
    );
  ...
  public LWComponent getHead() {
    if (
      head.node == null || head.node.isHidden(HideCause.PRUNE)
    ) return null; else return head.node;
  }

  /** @return the component connected at the tail end, or null if none or of it's pruned*/
  public LWComponent getTail() {
    if (
      tail.node == null || tail.node.isHidden(HideCause.PRUNE)
    ) return null; else return tail.node;
  }
  ...
  void disconnectFrom(LWComponent c) {
    if (head.node == c) setHead(null); else if (tail.node == c) setTail(
      null
    ); else throw new IllegalArgumentException(
      this + " cannot disconnect: not connected to " + c
    );
  }
  ...
  public void controlPointPressed(int index, MapMouseEvent e) {
    if (DEBUG.LINK || DEBUG.MOUSE) debug(
      "controlPointPressed " + index + "\n\thead: " + head + "\n\ttail: " + tail
    );
    boolean acted = true;
    ...
    if (PruneControlsEnabled) {
      if (index == CHead && head.hasNode()) {
        if (head.isPruning() || tail.hasNode()) toggleHeadPrune();
      } else if (index == CTail && tail.hasNode()) {
        if (tail.isPruning() || head.hasNode()) toggleTailPrune();
      }
    } else acted = false;
  ...
  boolean isPrunedBelow(LWComponent node) {
    if (node == head.node) return head.isPruning(); else if (
      node == tail.node
    ) return tail.isPruning(); else {
  ...
  private void pruneToggle(final boolean hide, Collection<LWComponent> bag) {
    for (LWComponent c : bag) {
      if (
        c != this
      ) pruneNode(c, hide); // never hide us (the source of the prune)
    }
  }
  ...
  public Collection<? extends LWComponent> getConnected() {
    ...
    if (head.hasNode() && !head.isPruning()) bag.add(head.node);
    if (tail.hasNode() && !tail.isPruning()) bag.add(tail.node);

    if (links.size() > 0) bag.addAll(links);
  ...
  public Rectangle2D.Float getFanBounds(Rectangle2D.Float r) {
    ...
    if (head.hasNode()) rect.add(head.node.getBounds());
    if (tail.hasNode()) rect.add(tail.node.getBounds());
    return rect;
  }
  ...
  public void draw(DrawContext dc) {
    if (dc.focal == this) {
      ...
      if (getParent() != null) getParent().isTopLevel();

      if (head.hasNode()) {
        if (
          head.node.getParent() == getParent() ||
  ...
  private void setControllerLocation(
    int index,
    float mapX,
    float mapY,
    MapMouseEvent e
    ...
    } else local = e.getLocalPoint(this);
    ...
    if (index == CHead && !head.isPruning()) {
      ...
      if (e != null) LinkTool.setMapIndicationIfOverValidTarget(
        tail.node,
        this,
        e
      );
      ...
      if (mCurveControls == 1) {
        ...
        if (index == CCurve1) setCtrlPoint0(
          (float) p.getX() + dx,
          (float) p.getY() + dy
        ); else setCtrlPoint1((float) p.getX() + dx, (float) p.getY() + dy);
      }
    }
  }
  ...
  public void controlPointDropped(int index, MapMouseEvent e) {
    ...
    if (DEBUG.MOUSE) System.out.println(
      "LWLink: control point " + index + " dropped on " + dropTarget
    );
    if (dropTarget != null && !e.isShiftDown()) {
      if (
        index == CHead && head.node == null && tail.node != dropTarget
      ) setHead(dropTarget); else if (
        index == CTail && tail.node == null && head.node != dropTarget
      ) setTail(dropTarget);
  ...
  private final LWSelection.Controller[] mControlPoints =
    new LWSelection.Controller[MAX_CONTROL];

  private static final RectangularShape ConnectCtrlShape = new Ellipse2D.Float(
    0,
    0,
    9,
    9
  );
  private static final RectangularShape CurveCtrlShape = new Ellipse2D.Float(
    0,
    0,
    8,
    8
  );
  private static final RectangularShape PruneCtrlShape = new Rectangle2D.Float(
    0,
    0,
    8,
    8
  ...
  private static class ConnectCtrl extends LWSelection.Controller {
    ...
    ConnectCtrl(Point2D.Float mapLoc, End end, boolean farConnect) {
      ...
      if (PruneControlsEnabled && end.isConnected() && farConnect) setColor(
        end.isPruning() ? Color.red : COLOR_SELECTION
      ); else setColor(end.isConnected() ? null : COLOR_SELECTION_HANDLE);
    }

    public final RectangularShape getShape() {
      return ConnectCtrlShape;
    }
  }
  ...
  private static class CurveCtrl extends LWSelection.Controller {
    ...
    CurveCtrl(Point2D p, float epx, float epy) {
      //super(p);
      super((float) (p.getX() + epx) / 2, (float) (p.getY() + epy) / 2);
      setColor(COLOR_SELECTION_CONTROL);
    }

    public final RectangularShape getShape() {
      return CurveCtrlShape;
    }
  }
  ...
  private static final boolean EXCLUDE_CONNECTED = true;
  private static final boolean INCLUDE_CONNECTED = false;
  private static final boolean INCLUDE_ENDPOINTS = true;
  private static final boolean EXCLUDE_ENDPOINTS = false;
  private static final boolean INCLUDE_PRUNES = true;
  private static final boolean EXCLUDE_PRUNES = false;
  ...
  private LWSelection.Controller[] getControls(
    ...
    if (excludeConnected && head.isConnected()) {
      ...
      mControlPoints[CHead] =
        new ConnectCtrl(mapHead, head, tail.isConnected());
      if (DEBUG.BOXES) mControlPoints[CHead].setColor(Color.green); // mark the head
    } else {
      mControlPoints[CHead] = null;
    }

    if (excludeConnected && tail.isConnected()) mControlPoints[CTail] =
      null; else if (
      endpointDrags && !tail.hasPrunedNode()
    ) mControlPoints[CTail] =
      new ConnectCtrl(
        mapTail,
        tail,
        head.isConnected()
      ); else mControlPoints[CTail] = null;
    ...
    if (mCurveControls == 1) {
      mControlPoints[CCurve1] =
        new CurveCtrl(mapTx.transform(mQuad.getCtrlPt(), null));
      mControlPoints[CCurve2] = null;
    } else if (mCurveControls == 2) {
      mControlPoints[CCurve1] =
        new CurveCtrl(
          mapTx.transform(mCubic.getCtrlP1(), null),
          mapHead.x,
          mapHead.y
        );
      mControlPoints[CCurve2] =
        new CurveCtrl(
          mapTx.transform(mCubic.getCtrlP2(), null),
          mapTail.x,
          mapTail.y
        );
    } else {
  ...
  public void run() {
    ...
    if (
      head.hasNode() && head.node.getLayer() == getLayer()
    ) LWContainer.ensureLinkPaintsOverAllAncestors(this, head.node);
    if (
      tail.hasNode() && tail.node.getLayer() == getLayer()
  ...
  private void ensureDataBitsSet(String relation) {
    if (relation != null) {
      ...
      if (relation.startsWith("COUNT:")) setFlag(Flag.DATA_COUNT);
    }
  }
  ...
  private boolean reparentBasedOnEndpoints() {
    ...
    if (getParent().isTopLevel() && commonAncestor.isTopLevel()) return false;

    if (DEBUG.LINK) out(
      Util.TERM_GREEN +
      "REPARENTING TO NEW COMMON ANCESTOR: " +
      commonAncestor +
      Util.TERM_CLEAR
    );
  ...
  private LWComponent findCommonEndpointAncestor() {
    if (head.node == null) {
      if (tail.node == null) return null; else return tail.node.getParent();
    } else if (tail.node == null) {
      if (head.node == null) return null; else return head.node.getParent();
    }

    // These are some quick-check cases we can test
    // w/out having to generate the ancestor lists:

    if (head.node.parent == tail.node.parent) return head.node.parent;

    if (head.node.parent == tail.node.parent.parent) return head.node.parent;

    if (tail.node.parent == head.node.parent.parent) return tail.node.parent;
    ...
    if (DEBUG.PARENTING && DEBUG.META) out(
      Util.TERM_RED +
      "checking for common ancestor:" +
      "\n\t      in link: " +
      this +
      "\n\t         head: " +
      head.node +
    ...
    for (LWComponent ha : headAncestors) for (LWComponent ta : tailAncestors) if (
      ta == ha
    ) return ta;
  ...
  public void setControlCount(int newControlCount) {
    ...
    if (newControlCount > 2) throw new IllegalArgumentException(
      "LWLink: max 2 control points " + newControlCount
    );

    if (mCurveControls == newControlCount) return;
  ...
  public Point2D getCtrlPoint0() {
    if (mCurveControls == 0) return null; else if (
      mCurveControls == 2
    ) return mCubic.getCtrlP1(); else return mQuad.getCtrlPt();
  }
  ...
  public void setCtrlPoint0(float x, float y) {
    ...
    if (mCurveControls == 0) {
      ...
      if (DEBUG.UNDO) System.out.println(
        "implied curved link by setting control point 0 " + this
      );
    }
    Object old;
    if (mCurveControls == 2) {
  ...
  public void setCtrlPoint1(float x, float y) {
    ...
    if (mCurveControls < 2) {
      setControlCount(2);
      if (DEBUG.UNDO) System.out.println(
        "implied cubic curved link by setting a control point 1 " + this
      );
    }
  ...
  protected void removeFromModel() {
    if (head.pruned) toggleHeadPrune();
    if (tail.pruned) toggleTailPrune();
  ...
  private LWComponent parentInParentChildLink() {
    if (head.node == null || tail.node == null) return null;
    if (head.node.getParent() == tail.node) return tail.node;
    if (tail.node.getParent() == head.node) return head.node;
    return null;
  }

  /** Is this link between a parent and a child? */
  public boolean isParentChildLink() {
    if (head.node == null || tail.node == null) return false;
    return (
      head.node.getParent() == tail.node || tail.node.getParent() == head.node
    );
  ...
  public boolean isConnectedTo(LWComponent c) {
    if (c == null) return false; else return (
      hasEndpoint(c) || super.isConnectedTo(c)
    );
  }
  ...
  public LWComponent getFarPoint(LWComponent source) {
    if (head.node == source) return tail.node; else if (
      tail.node == source
    ) return head.node; else throw new IllegalArgumentException(
      "bad farpoint: " + source + " not connected to " + this
    );
  }
  ...
  public LWComponent getFarNavPoint(LWComponent source) {
    ...
    if (getHead() == source) {
      if (arrows == ARROW_NONE || (arrows & ARROW_TAIL) != 0) return getTail();
    } else if (getTail() == source) {
      if (arrows == ARROW_NONE || (arrows & ARROW_HEAD) != 0) return getHead();
    } else throw new IllegalArgumentException(
      "bad farpoint: " + source + " not connected to " + this
    );
    return null;
  ...
  public boolean isNestedLink() {
    ...
    if (head.node == null || tail.node == null) return (
      getParent() instanceof LWNode
    );
    if (
      head.node.getParent() == tail.node || tail.node.getParent() == head.node
    ) return true;
    return (
      head.node.getParent() == tail.node.getParent() &&
      head.node.getParent() instanceof LWNode
    );
  }

  public void mouseOver(MapMouseEvent e) {
    if (
      e.getViewer().getZoomFactor() > ICON_BLOCK_LOD_ZOOM &&
      mIconBlock.isShowing()
    ) mIconBlock.checkAndHandleMouseOver(e);
  }

  private class SegIterator
    implements Iterator<Line2D.Float>, Iterable<Line2D.Float> {
    ...
    public Iterator<Line2D.Float> iterator() {
      return this;
    }
  }
  ...
  protected boolean intersectsImpl(Rectangle2D mapRect) {
    ...
    if (DEBUG.LINK && mXMLRestoreUnderway) {
      ...
      if (!localRect.equals(mapRect)) System.out.println(
        "\t" + Util.fmt(mapRect) + " to:" + "\n\t" + Util.fmt(localRect)
      );
    }
    ...
    if (mCurve != null) {
      for (Line2D seg : new SegIterator()) if (
        seg.intersects(localRect)
      ) return true;
    } else {
      if (localRect.intersectsLine(mLine)) return true;
    }

    if (mIconBlock.isShowing() && mIconBlock.intersects(localRect)) return true;

    if (labelBox != null && hasLabel()) return labelBox.boxIntersects(
      localRect
    ); else return false;
  }
  ...
  protected float pickDistance(float x, float y, PickContext pc) {
    ...
    if (isCurrentlyPruned()) return pickPruneDistance(
      x,
      y,
      pc
    ); else return pickLineDistance(x, y, pc);
  }

  private float pickPruneDistance(float x, float y, PickContext pc) {
    final float distSq;

    if (head.pruned) distSq = (float) head.distanceSq(x, y); else if (
      tail.pruned
    ) distSq = (float) tail.distanceSq(x, y); else distSq = -1;

    if (distSq <= PruneDotHitRadiusSq) return 0; else return distSq;
  }
  ...
  private float pickLineDistance(float x, float y, PickContext pc) {
    ...
    if (mCurve != null) {
      ...
      for (Line2D seg : new SegIterator()) {
        ...
        if (distSq <= hitDistSq) return 0; else if (
          distSq < minDistSq
        ) minDistSq = distSq;
      }
    } else {
      final float distSq = (float) mLine.ptSegDistSq(x, y);
      if (distSq <= hitDistSq) return 0; else minDistSq = distSq;
    }

    if (!isNestedLink()) {
      if (mIconBlock.contains(x, y)) return 0; else if (
        hasLabel() && labelBox != null && labelBox.boxContains(x, y)
      ) return 0;
    }
  ...
  public void setLocation(float x, float y) {
    ...
    if (DEBUG.CONTAINMENT || DEBUG.LINK) out(
      String.format("             setLocation %+.1f,%+.1f", x, y)
    );
  ...
  private void translateAllPoints(float dx, float dy, boolean onUndo) {
    if (DEBUG.CONTAINMENT) out(
      String.format(
        "           map translate %+.1f,%+.1f onUndo=%s",
        dx,
        dy,
        onUndo
      )
    ...
    if (head.node == null) setHeadPoint(head.x + dx, head.y + dy);

    if (tail.node == null) setTailPoint(tail.x + dx, tail.y + dy);

    if (onUndo) {
      if (DEBUG.LINK) out(
        "skipping ctrl point translate on undo; relying on ctrl point undo events"
      );
      ...
      if (mCurveControls == 1) {
        setCtrlPoint0(mQuad.ctrlx + dx, mQuad.ctrly + dy);
      } else if (mCurveControls == 2) {
        setCtrlPoint0(mCubic.ctrlx1 + dx, mCubic.ctrly1 + dy);
        setCtrlPoint1(mCubic.ctrlx2 + dx, mCubic.ctrly2 + dy);
      }
    }
  }
  ...
  void notifyEndpointMoved(LWComponent movingSrc, LWComponent end) {
    ...
    if (
      mRecompute ||
      (movingSrc != null &&
        hasAncestor(movingSrc) &&
        end.hasAncestor(movingSrc))
    ) {
    ...
    if (DEBUG.CONTAINMENT) {
      if (DEBUG.LINK && DEBUG.WORK) {
        System.out.format(
          "notifyEndpointMoved %-70s movingSrc=%s end=%s wasDirty=%s nowMarked=%s\n",
          this,
          movingSrc,
          end,
          wasDirty,
        ...
        if (!wasDirty && !mRecompute) System.err.print("|"); else if (
          !wasDirty && mRecompute
        ) System.err.print(";"); else System.err.print(":");
      }
  ...
  private void initCurveControlPoints() {
    ...
    if (DEBUG.LINK) out(
      "rotHeadINIT " + head.rotation + " rotTailINIT " + tail.rotation
    );

    if (mCurveControls == 2) axisOffset = axisLen / 4; else axisOffset =
      axisLen / 3; // do this via a log: grows slowing with length increaase

    //out("axisLen " + axisLen + " offset " + axisOffset);

    final AffineTransform centerLeft = AffineTransform.getTranslateInstance(
      mCenterX,
      mCenterY
    );
    ...
    if (reverse) centerLeft.translate(
      +axisOffset * further,
      0
    ); else centerLeft.translate(-axisOffset * further, 0);
  ...
  public Shape getZeroShape() {
    if (mRecompute) computeLink();
    if (mCurveControls > 0) return mCurve; else return mLine;
  }
  ...
  void setLabel0(String newLabel, boolean setDocument) {
    ...
    if (mXMLRestoreUnderway); // do nothing
    else if (getParent() == null) mRecompute = true; // mark for later
    else computeLink(); // recompute now
  }
  ...
  private void computeLink() {
    if (mXMLRestoreUnderway) {
      if (DEBUG.LINK) Util.printStackTrace(
        "computeLink attempted during restore " + this
      );
      return;
    }
    ...
    if (mCurveControls > 0 && mCurve == null) initCurveControlPoints();
    ...
    if (head.hasNode()) {
      ...
      if (
        !mXMLRestoreUnderway &&
        head.node instanceof LWLink &&
        ((LWLink) head.node).mRecompute
    ...
    if (tail.hasNode()) {
      ...
      if (
        !mXMLRestoreUnderway &&
        tail.node instanceof LWLink &&
        ((LWLink) tail.node).mRecompute
    ...
    if (headShape != null) {
      ...
      final float[] result = VueUtil.computeIntersection(
        head.x,
        head.y,
        srcX,
        srcY,
    ...
    if (mCurveControls > 0) curveBounds = computeCurvedLink();

    //---------------------------------------------------------------------------------------------------
    // Compute rotations for arrows or for moving linearly along the link
    //---------------------------------------------------------------------------------------------------

    if (DEBUG.LINK && DEBUG.META) out(
      "head " +
      head.x +
      "," +
      head.y +
    ...
    if (mCurveControls == 1) {
      head.rotation =
        computeVerticalRotation(head.x, head.y, mQuad.ctrlx, mQuad.ctrly);
      tail.rotation =
        computeVerticalRotation(tail.x, tail.y, mQuad.ctrlx, mQuad.ctrly);
    } else if (mCurveControls == 2) {
      head.rotation =
        computeVerticalRotation(head.x, head.y, mCubic.ctrlx1, mCubic.ctrly1);
      tail.rotation =
        computeVerticalRotation(tail.x, tail.y, mCubic.ctrlx2, mCubic.ctrly2);
    } else {
      head.rotation =
        computeVerticalRotation(mLine.x1, mLine.y1, mLine.x2, mLine.y2);
      tail.rotation = head.rotation + Math.PI; // can just flip head rotation: add 180 degrees
    }

    if (DEBUG.LINK && DEBUG.META) out(
      "rotHead0 " + head.rotation + " rotTail0 " + tail.rotation
    );
    ...
    if (getStrokeWidth() > 0) grow(bounds, getStrokeWidth() / 2f);

    if (labelBox != null && hasLabel()) bounds.add(labelBox.getBoxBounds());

    if (mIconBlock.isShowing()) bounds.add(mIconBlock);
  ...
  private Rectangle2D.Float computeCurvedLink() {
    ...
    if (mCurveControls == 1) {
      ...
      if (badCurve = badCurve(mQuad)) Log.warn(
        this + "; bad curve: " + Util.fmt(mQuad)
      );

      if (IncludeControlPointsInBounds) bounds.add(mQuad.ctrlx, mQuad.ctrly);
    } else if (mCurveControls == 2) {
      mCubic.x1 = head.x;
      mCubic.y1 = head.y;
      mCubic.x2 = tail.x;
      ...
      if (badCurve = badCurve(mCubic)) Log.warn(
        this + "; bad curve: " + Util.fmt(mCubic)
      );

      if (IncludeControlPointsInBounds) {
        // Add the centers of the two control lines, where we put the controllers.
        bounds.add((mCubic.ctrlx1 + head.x) / 2, (mCubic.ctrly1 + head.y) / 2);
        bounds.add((mCubic.ctrlx2 + tail.x) / 2, (mCubic.ctrly2 + tail.y) / 2);
      }
    } else badCurve = false;
    ...
    if (badCurve || mCurve.getBounds().isEmpty()) {
      if (!badCurve) Log.warn(
        this + "; empty curve " + Util.fmt(mCurve) + " " + mCurve.getBounds()
      );
      //tufts.Util.printStackTrace("empty curve " + mCurve + " " + mCurve.getBounds());
      return bounds;
    }

    if (mPoints == null) mPoints = new float[16];
    mLastPoint = 0;
    ...
    final PathIterator i = new FlatteningPathIterator(
      mCurve.getPathIterator(null),
      1f
    );
  ...
  private double computeVerticalRotation(
    double x1,
    double y1,
    double x2,
    double y2
  ...
  public void setArrowState(int arrowState) {
    mArrowState.set(arrowState);
  }

  public int getArrowState() {
    return mArrowState.get();
  }

  public void rotateArrowState() {
    int newState = getArrowState() + 1;
    if (newState > ARROW_BOTH) newState = ARROW_NONE;
  ...
  private void drawArrows(DrawContext dc) {
    ...
    if (mStrokeStyle.get() == StrokeStyle.SOLID) dc.g.setStroke(
      this.stroke
    ); else dc.g.setStroke(StrokeStyle.SOLID.makeStroke(mStrokeWidth.get()));
    ...
    if ((mArrowState.get() & ARROW_HEAD) != 0) {
      ...
      if (scale != 1) dc.g.scale(scale, scale);
  ...
  private static final float PruneDotHitRadiusSq =
    PruneDotHitRadius * PruneDotHitRadius;

  private static final RectangularShape PruneDot =
    new java.awt.geom.Ellipse2D.Float(0, 0, PruneDotSize, PruneDotSize);
  ...
  private void drawLink(DrawContext dc) {
    ...
    if (mArrowState.get() != 0) {
      if (dc.zoom <= 0.125 && dc.isLODEnabled()); // don't draw arrows
      else drawArrows(dc);
    }
    ...
    if (strokeWidth <= 0) strokeWidth = 0.5f;
    ...
    if (stroke == STROKE_ZERO) { // mStrokeWidth.get() was 0
      ...
      if (curScale > 1) strokeWidth /= curScale;
    ...
    if (!isNestedLink()) drawLinkDecorations(dc);
  ...
  private void drawStroke(DrawContext dc) {
    if (mCurve == null) {
      ...
      if (
        dc.isInteractive() &&
        (isSelected() || DEBUG.BOXES || DEBUG.CONTAINMENT) &&
        !dc.isBrowsing()
      ) {
        //-------------------------------------------------------
  ...
  public Color getRenderFillColor(DrawContext dc) {
    if (
      dc != null && dc.isInteractive() && isSelected()
    ) return COLOR_HIGHLIGHT; else return super.getRenderFillColor(dc);
    //         if (dc != null) {
  ...
  private void drawLabel(DrawContext dc) {
    ...
    if (DEBUG.LINK && DEBUG.META) {
      ...
      dc.g.drawString(
        String.format("txtBounds: %s", Util.out(tbounds)),
        0,
        y += inc
      );
      dc.g.drawString(
      ...
      dc.g.drawString(
        String.format("     head: %+4.0f,%+4.0f", head.x, head.y),
        0,
        y += inc
      );
      dc.g.drawString(
        String.format("     tail: %+4.0f,%+4.0f", tail.x, tail.y),
        0,
        y += inc
      );
  ...
  protected void layoutImpl(Object triggerKey) {
    ...
    if (labelBox == null && hasLabel()) getLabelBox(); // make sure labelBox is set if we have a label
    ...
    if (iconBlockShowing) {
      ...
      if (putBelow) {
        ...
        if (hasLabel()) iby =
          labelBox.getBoxY() + labelBox.getBoxHeight(); else iby =
          (float) (cy - mIconBlock.getHeight() / 2f);
        // we're seeing a sub-pixel gap -- this should fix
        iby -= 0.5;
      } else {
        // for at right
        if (hasLabel()) ibx = (float) lx + labelBox.getBoxWidth(); else ibx =
          (float) (cx - mIconBlock.getWidth() / 2);
  ...
  public void initTextBoxLocation(TextBox textBox) {
    ...
    out(
      "setboxcenter " +
      Util.fmt(new Point2D.Float(getZeroCenterX(), getZeroCenterY()))
    );
  ...
  public LWLink duplicate(CopyContext cc) {
    ...
    if (mCurveControls > 0) {
      ...
      if (mCurveControls > 1) link.setCtrlPoint1(getCtrlPoint1());
    }
    //computeLink();
  ...
  public String paramString() {
    ...
    if (getControlCount() == 1) s +=
    String.format(" (%.0f,%.0f)", mQuad.ctrlx, mQuad.ctrly); else if (
      getControlCount() == 2
    ) s +=
    String.format(
      " (%.0f,%.0f & %.0f,%.0f)",
    ...
    if (head.pruned) s += " X-HEAD";
    if (tail.pruned) s += " X-TAIL";
    return s;
  }

  /** @deprecated -- use getHead */public LWComponent getComponent1() {
    return getHead();
  }

  /** @deprecated -- use getTail */public LWComponent getComponent2() {
    return getTail();
  }

  /** @deprecated -- use setHeadPoint */public void setStartPoint(
    float x,
    float y
  ) {
    setHeadPoint(x, y);
  ...
  /** @deprecated -- use setTailPoint */public void setEndPoint(
    float x,
    float y
  ) {
    setTailPoint(x, y);
  ...
  /** @deprecated -- use getHeadPoint */public Point2D getPoint1() {
    return getHeadPoint();
  }

  /** @deprecated -- use getTailPoint */public Point2D getPoint2() {
    return getTailPoint();
  }
