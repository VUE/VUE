...
public class Toolbox {
   ...
   */
  public static double angleBetween2PointsCoord(
    Point2D p,
    Point2D q,
    boolean javaCoord
  ) {
  ...
  public static void findChildrenToDepth(
    ...
    Collection<LWComponent> comps,
    int depth,
    boolean expandIncoming,
    boolean expandOutgoing,
    Hashtable<LWComponent, Integer> alreadyVisited
  ) {
    for (LWComponent comp : comps) {
      ...
      if (
        alreadyVisitedAtLevel == null ||
        alreadyVisitedAtLevel.intValue() < depth
      ) {
        alreadyVisited.put(comp, new Integer(depth));

        boolean compIsUserSelected = userSelection.contains(comp), compIsLink =
          (comp.getClass() == LWLink.class);
