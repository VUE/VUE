...
public class NumericField extends JTextField {
  ...
  public void setValue(int pValue) {
    if (
      (pValue < 0) && (mMode == POSITIVE_INTEGER || mMode == POSITIVE_NUMERIC)
    ) {
      //throw new IllegalArgumentException(pValue + " is < 0, field requires positive value");
      throw new IllegalArgumentException(
        "NumericFieldF - Requires Positive Value"
      );
    } else {
  ...
  public void setValue(double pValue) {
    if (mMode == INTEGER || mMode == POSITIVE_INTEGER) {
      ...
      throw new IllegalArgumentException(
        "NumericField - Requires Positive Value"
      );
    } else {
  ...
  private class NumericDocument extends PlainDocument {
    ...
    public void insertString(int offset, String str, AttributeSet a)
      ...
      for (int n = 0; n < source.length; n++) {
        ...
        if (
          c == '.' &&
          ((text = mDocNumericField.getText()) != null) &&
          text.indexOf('.') != -1
        ) return;
      }
