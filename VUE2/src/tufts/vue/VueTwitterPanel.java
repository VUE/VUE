...
public class VueTwitterPanel
  extends JPanel
  implements ItemListener, ActionListener {

  private static final String FROM_LAYER = "From";
  //private final static String TO_LAYER="To";
  private static final String GEO_LAYER = "Geo";
  private static final String ANALYSIS_LAYER = "Analysis";
  private static final String TWEETS_LAYER = "Tweets";
  private static final long serialVersionUID = 1L;

  private static final Color TweetNodeColor = VueResources.getColor(
    "twitter.tweet.color",
    Color.gray
  );
  private static final float TweetNodeStrokeWidth = VueResources.getInt(
    "twitter.tweet.stroke.width",
    0
  );
  private static final Color TweetNodeStrokeColor = VueResources.getColor(
    "twitter.tweet.stroke.color",
    Color.black
  );
  private static final Font TweetNodeFont = VueResources.getFont(
    "twitter.tweet.font"
  );

  private static final Color ClusterNodeTextColor = VueResources.getColor(
    "node.dataValue.text.color",
    Color.black
  );
  private static final Font ClusterNodeFont = VueResources.getFont(
    "node.dataValue.font"
  );
  private static final Color[] ClusterNodeDataColors =
    VueResources.getColorArray("node.dataValue.color.cycle");
  private static int NextColor = 0;

  private JTextField mSearchFieldEditor = null;
  private PropertyPanel mPropPanel = null;
  private JButton followButton = null;
  private final JRadioButton scatterCheckbox = new JRadioButton(
    VueResources.getString("twitter.scatter")
  );
  private final JRadioButton fromCheckbox = new JRadioButton(
    VueResources.getString("twitter.from")
  );
  ...
  private final JRadioButton analysisCheckbox = new JRadioButton(
    VueResources.getString("twitter.analysis")
  );
  private TwitterThread twitterThread = null;
  private final ButtonGroup group = new ButtonGroup();

  private final HashMap<String, LWMap.Layer> layerList = new HashMap<
    String,
    LWMap.Layer
  >();
  ...
  public VueTwitterPanel() {
    ...
    mPropPanel.addProperty(
      VueResources.getString("twitter.searchField"),
      mSearchFieldEditor
    ); //added through metadata
    JLabel titleLabel = new JLabel(VueResources.getString("twitter.title"));
    followButton =
      new JButton(VueResources.getString("twitter.button.activate"));
  ...
  public void itemStateChanged(ItemEvent e) {
    ...
    if (e.getStateChange() == ItemEvent.SELECTED) twitterThread.layout(null);
  }
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource().equals(followButton)) {
      ...
      if (
        followButton
          .getText()
          .equals(VueResources.getString("twitter.button.activate"))
      ) {
        t = new Thread(twitterThread);
        t.start();
        followButton.setText(
          VueResources.getString("twitter.button.deactivate")
        );
      } else {
        t.stop();
    ...
    if (!layersInit) {
      ...
      for (LWComponent layer : reverse(map.getChildren())) {
        if (layer instanceof LWMap.Layer) {
          ...
          if (
            !layer.getLabel().equals(TWEETS_LAYER)
  ...
  class TwitterThread implements Runnable {
    ...
    private ScrollingTabularLayout scrollTableLayout =
      new ScrollingTabularLayout();
    ...
    public void run() {
      ...
      while (true) {
        try {
          ...
          if (lastTweet > 0) q.setSinceId(lastTweet);
          ...
          for (Tweet tweet : tweets) {
            ...
            if (tweet.getGeoLocation() != null) mlist.add(
              "geolocation",
            ...
            javax.swing.SwingUtilities.invokeLater(
              new Runnable() {
                public void run() {
                  ...
                  final LWMap.Layer analysisLayer = layerList.get(
                    ANALYSIS_LAYER
                  );
    ...
    public LWComponent getClusterStyleNode() {
      ...
      if (++NextColor >= ClusterNodeDataColors.length) NextColor = 0;
        ...
        javax.swing.SwingUtilities.invokeLater(
          new Runnable() {
            public void run() {
              ...
              final List<LWComponent> l = new ArrayList<LWComponent>();
              Iterable<LWNode> nodeIterator = fromLayer.getChildrenOfType(
                LWNode.class
              );
              for (LWNode c : nodeIterator) {
                l.add(c);
        ...
        javax.swing.SwingUtilities.invokeLater(
          new Runnable() {
            public void run() {
              ...
              final List<LWComponent> l = new ArrayList<LWComponent>();
              Iterable<LWNode> nodeIterator = analysisLayer.getChildrenOfType(
                LWNode.class
              );
              for (LWNode c : nodeIterator) {
                l.add(c);
