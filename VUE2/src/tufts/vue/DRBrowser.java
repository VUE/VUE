...
public class DRBrowser extends ContentBrowser {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DRBrowser.class);
  ...
  final JComponent searchPane = new Widget(
    VueResources.getString("action.search")
  ) {
    ...
    protected void addImpl(Component c, Object constraints, int idx) {
      ...
      if (c instanceof JComponent) jc = (JComponent) c;
      if (constraints == SEARCH_EDITOR) {
        ...
        if (jc != null) jc.setBorder(GUI.WidgetInsetBorder);
        ...
        tufts.Util.printStackTrace(
          "illegal search pane constraints: " + constraints
        );
      }
  ...
  final JPanel librariesPane = new Widget(
    VueResources.getString("drbrowser.resources")
  );
  final Widget browsePane = new Widget(
    VueResources.getString("button.browse.label")
  );
  final Widget resultsPane = new Widget(
    VueResources.getString("jlabel.searchresult")
  );
  ...
  public DRBrowser(boolean delayedLoading, DockWindow resourceDock) {
    ...
    if (delayedLoading) {
      final String LoadingMessage = VueResources.getString(
        "dockWindow.Resources.loading.label"
      );
      ...
      if (Util.isMacLeopardOrLater()) {
        ...
        loadingComponent =
          new DataSourceViewer.StatusLabel(LoadingMessage, true, true);
    ...
    JLabel please = new JLabel(
      VueResources.getString("jlabel.searchableresource"),
      JLabel.CENTER
    );
  ...
  public void loadDataSourceViewer() {
    ...
    try {
      ...
      if (loadingComponent != null) librariesPane.remove(loadingComponent);
