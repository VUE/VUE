...
public class Images {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Images.class);
  ...
  public static void setLowMemory(Object cause) {
    synchronized (Images.class) {
      if (LOW_MEMORY_COUNT == 0) {
        Log.info(
          Util.TERM_PURPLE +
          "entering low-memory conditions, cause=" +
          Util.tags(cause) +
          Util.TERM_CLEAR
        );
      } else {
  ...
  public static VueAction ClearCacheAction = new VueAction(
    "Empty Image Cache"
  ) {
    public void act() {
      RawCache.clear();
  ...
  public static Handle getImageHandle(
    Object imageSRC,
    Images.Listener listener
  ) {
    return getImage(imageSRC, listener, LOAD_NORMAL);
  }

  public static Handle getImageImmediately(
    Object imageSRC,
    Images.Listener listener
  ) {
   ...
   * @return the Image if immediately available, null if the listener will be called back.
   **/
  private static Handle getImage(
    Object imageSRC,
    Images.Listener listener,
    Object when
  ) {
    try {
      ...
      if (listener != null) listener.gotImageError(imageSRC, t.toString());
    }
    return null;
  }
  ...
  private static class CacheEntry {
    ...
    CacheEntry(Image image, File cacheFile, Map<String, ?> props) {
      if (
        image == null && cacheFile == null
      ) throw new IllegalArgumentException(
        "CacheEntry: at least one of image or file must be non null"
      );
      if (image != null) this.ref = new SoftReference(image); else this.ref =
        null;
      this.file = cacheFile;
      this.data = props;
    ...
    Handle getHandle() {
      ...
      if (i != null) return new Handle(i, data); else return null;
    }
  ...
  static String keyToCacheFileName(
    URI key
  ...
  private static class ListenerRelay implements Listener {
    ...
    protected final void relaySize(
      Listener l,
      Object src,
      int w,
      int h,
      ...
      if (l != null) {
        try {
          if (DEBUG.IMAGE && l instanceof ListenerRelay == false) out(
            Util.TERM_CYAN +
            "relay SIZE " +
            w +
            "x" +
    ...
    protected final void relayProgress(
      Listener l,
      Object src,
      long bytes,
      float pct
      ...
      if (l != null) {
        try {
          if (
            DEBUG.IMAGE && DEBUG.META && l instanceof ListenerRelay == false
          ) out(
            String.format("relay PROGRESS %.2f %5d to %s", pct, bytes, tag(l))
          );
    ...
    protected final void relayImage(Listener l, Object src, Handle handle) {
      if (l != null) {
        try {
          if (DEBUG.IMAGE && l instanceof ListenerRelay == false) out(
            Util.TERM_CYAN + "relay IMAGE to " + tag(l) + Util.TERM_CLEAR
          );
    ...
    protected final void relayError(Listener l, Object src, String msg) {
      if (l != null) {
        try {
          if (DEBUG.IMAGE && l instanceof ListenerRelay == false) out(
            "relay ERROR to " + tag(head) + "; is=" + src
          );
    ...
    boolean hasListener(Listener l) {
      if (head == l || tail == l) return true; else if (
        tail instanceof ListenerRelay
      ) return ((ListenerRelay) tail).hasListener(l); else return false;
    }
  ...
  /*public*/static class Progress {
    ...
    public Image image() {
      return handle == null ? null : handle.image;
    }
  ...
  private static final class CachingRelayer
    extends Progress
    implements Listener {
    ...
    public synchronized void gotImageSize(
      Object imageSrc,
      int w,
      int h,
      long byteSize,
      ...
      if (sourceSize != null) this.sourceSize = sourceSize;
      chain.gotImageSize(imageSrc, w, h, byteSize, sourceSize);
    }

    public synchronized void gotImageProgress(
      Object imageSrc,
      long bytesSoFar,
      float _p
    ) {
      ...
      if (byteSize > 0) chain.gotImageProgress(
        imageSrc,
        bytesSoFar,
        percentProgress()
      );
    ...
    private float percentProgress() {
      ...
      if (byteSize > 0) return (
        (float) bytesSoFar / (float) byteSize
      ); else return 0;
    }

    void addListener(Listener newListener) {
      if (chain.addListener(newListener)) deliverPartialResults(newListener);
    }
    ...
    private void deliverPartialResults(Listener l) {
      ...
      if (bytesSoFar > 0) chain.relayProgress(
        l,
        imageSRC.original,
        bytesSoFar,
        percentProgress()
      ...
      if (handle != null) chain.relayImage(l, imageSRC.original, handle);

      if (errorMsg != null) {
        if (handle != null) Log.warn(
          "had both image and error: " + errorMsg + "; for " + l
        );
  ...
  private static Handle getCachedOrCallbackOnLoad(
    ...
    if (imageSource instanceof Image) {
      if (DEBUG.Enabled) Log.info(
        "image source was an instance of Image",
        new Throwable(Util.tags(imageSource))
      );
      ...
      if (listener != null) listener.gotImage(
        imageSource, // same as image
        handle
      );
    ...
    if (DEBUG.IMAGE || DEBUG.WORK) Log.debug(
      "fetching for " +
      Util.tags(when) +
      " to listener " +
      Util.tag(listener) +
    ...
    if (cacheEntry instanceof Loader) {
      ...
      if (when == LOAD_NORMAL) loader.raisePriority();
      ...
      if (when != LOAD_IMMEDIATE) {
        ...
        // may now be loading.
        Log.warn(
          "no listener for non-cached content: caller in the dark as to the presence of image content: " +
          imageSRC
        );
        return null;
      }
      ...
      if (handle == null) Log.warn(
        "Zealous GC: image tossed immediately " + imageSRC
      );
    } else if (cacheEntry instanceof Handle) {
      handle = (Handle) cacheEntry;
    } else if (cacheEntry instanceof Image) {
      Log.warn(
        "DEPRECATED USE OF CACHE ENTRY -- USE HANDLE; " + imageSRC,
        new Throwable("HERE")
      );
      //cachedImage = (Image) cacheEntry;
      handle = new Handle((Image) cacheEntry);
    } else {
      if (DEBUG.IMAGE) Log.warn(
        "unhandled fetch result: " + Util.tags(cacheEntry),
        new Throwable("HERE")
      ); else Log.warn("unhandled fetch result: " + Util.tags(cacheEntry));
  ...
  private static class ImmediatelyReducablePool implements Runnable {
    ...
    private static final org.apache.log4j.Logger Log =
      org.apache.log4j.Logger.getLogger(ImmediatelyReducablePool.class);
    ...
    public void run() {
      ...
      for (;;) {
        synchronized (this) {
          if (oldPool == _poolInShutdown) Log.error(
            "should never happen: re-waiting on the same pool " + oldPool
          );
      ...
      Log.info(
        "resizer terminating, pool at size = 1, no further shrinkages possible"
      );
    }

    private void reduceNextPoolSize() {
      if (_nextSmallerPoolSize > 1) _nextSmallerPoolSize /=
      2; else _nextSmallerPoolSize = 1;
    }
    ...
    public synchronized void shrinkIfPossible(boolean firstLowMemory) {
      if (_nextSmallerPoolSize < 1) {
        if (DELAYED_ICONS && firstLowMemory) resortQueue();
        return;
      }
      ...
      if (_pool != null) {
        ...
        Log.warn(
          "stacked OutOfMemoryError conditions: next pool size reduced to " +
          _nextSmallerPoolSize
        );
      }
    }
  ...
  private static final class TaskQueue<E> extends SynchronousQueue<E> {
    ...
    private synchronized E popNext() {
      ...
      if (q1.size() > 0) return q1.removeFirst(); // pop front (was inserted FIFO)
      else if (q2.size() > 0) return q2.removeFirst(); // pop front (was inserted LIFO)
      else return q3.poll(); // pop front (was inserted FIFO), but return null if empty
    }
    ...
    synchronized void jumpQueue(List src, Task task, Object newPri) {
      ...
      if (!src.remove(task)) {
        ...
        if (true) {
          debug(
            "queue did not contain: " +
            task +
            "\n\t      was not in queue: " +
            Util.tags(src) +
            "\n\trequested new priority: " +
            newPri
  ...
  private static final ThreadFactory ImageThreadFactory = new ThreadFactory() {
    ...
    public Thread newThread(Runnable r) {
      ...
      try {
        ...
        Log.warn(
          "failed to set image thread priority to " + ImageThreadPriority,
          t
        );
      }
      count++;
      if (DEBUG.Enabled) it.setName("IPX-" + count); else it.setName(
        "imageProcessor-" + count
      );
  ...
  static {
    ...
    if (DEBUG.SINGLE_THREAD) {
      if (cores == 1) useCores = 2; else useCores = 1;
    } else {
      useCores = cores;
    }
  ...
  private static class Task extends Loader {
    ...
    static Loader create(ImageSource imageSRC, Listener listener, Object when) {
      ...
      if (imageSRC.key == null) {
        if (DEBUG.IMAGE || DEBUG.WORK) Util.printStackTrace(
          "attempting to load cache w/null key: " + imageSRC
        ); else Log.warn("attempting to load cache w/null key: " + imageSRC);
      }
    ...
    private Task(ImageSource is, Listener relay, Object pri) {
      super(is, relay);
      priority = pri;
      if (DEBUG.IMAGE && relay == null) Log.debug(
        this +
        "; nobody currently listening: image may be quietly cached: " +
        imageSRC
      );
    }

    void raisePriority() {
      if (!isRunning()) TaskQueue.raiseTaskPriority(this);
    }
  ...
  private static final class IconTask extends Task {
    ...
    IconTask(ImageSource is, Listener relay) {
      ...
      if (DEBUG.Enabled && relay == null) Log.debug(
        this +
        "; nobody currently listening: image may be quietly cached: " +
        imageSRC
      );
  ...
  private static final class LoadThread extends Loader {
    ...
    LoadThread(ImageSource imageSRC, Listener firstRelay) {
      ...
      thread =
        new ImgThread(this, String.format("ImgLoader-%02d", LoaderCount++));
   ...
   * one was provided.
   */
  private static Object getCachedOrKickLoad(
    ImageSource imageSRC,
    Images.Listener listener,
    Object when
  ) {
  ...
  private static void markCacheAsLoading(URI key, Loader loader) {
    ...
    if (
      old != null &&
      old instanceof CacheEntry &&
      !((CacheEntry) old).isPreloadedDiskEntry()
    ) {
      Log.warn(
        "blew away existing cache content:\n\told: " +
        Util.tags(old) +
        "\n\tfor: " +
        Util.tags(loader)
      );
  ...
  private static Object getCacheContentsWithAutoFlush(ImageSource imageSRC) {
    if (imageSRC.key == null) return null;
    ...
    if (DEBUG.IMAGE) out(
      "found cache entry for key " + tag(imageSRC.key) + ": " + entry
    );

    if (entry instanceof Loader) {
      if (DEBUG.IMAGE) out(
        "Image is loading into the cache via already existing Loader..."
      );
    ...
    if (cachedImage != null) {
      ...
      if (ce.file.canRead()) {
        ...
        emptyEntry = false;
      } else Log.warn("cache file no longer available: " + ce.file);
    }
    ...
    if (cachedImage == null) return null; else return new Handle(
      cachedImage,
      ce.data
    );
    //return cachedImage;
  ...
  private static Handle createAndCacheIcon(
    Listener listener,
    ...
    if (badReadable) throw new Error(
      "iconSource had no image content in readable: " + iconSource
    );

    if (hardImage == null) {
      Log.warn(
        "hard image GC'd before icon creation, forcing low-memory conditions (" +
        iconSource +
        ")"
      );
    ...
    final Dimension originalSize = new Dimension(
      hardImage.getWidth(null),
      hardImage.getHeight(null)
    );

    final Handle iconHandle = createIcon(hardImage, iconSource.iconSize);
    ...
    if (cacheFile != null && iconHandle.image != null) cacheIconToDisk(
      iconSource.key,
      (RenderedImage) iconHandle.image,
      cacheFile,
      originalSize
    );
    return iconHandle;
  ...
  private static boolean cacheIconToDisk(
    ...
    final Iterator<ImageWriter> writers = ImageIO.getImageWritersByFormatName(
      "png"
    );
  ...
  private static final String GENERIC_META_DATA =
  ...
  private static void writeWithComments(
    ImageWriter writer,
    RenderedImage image,
    File file,
    Dimension originalSize
  ) throws IOException {
    if (DEBUG.Enabled || DEBUG.IMAGE || DEBUG.IO) Log.debug(
    ...
    if (DEBUG.IMAGE && DEBUG.IO) {
      Log.debug(
        " FORMAT-NAMES: " + Arrays.asList(meta_data.getMetadataFormatNames())
      );
    ...
    addData(
      metaTree,
      "sourceSize",
      String.format("%d,%d", originalSize.width, originalSize.height)
    );
    ...
    final IIOImage io_image = new IIOImage(
      image,
  ...
  private static void addData(
    ...
    if (DEBUG.IO || DEBUG.IMAGE) Log.debug(
      "addData; " + keyword + "=" + comment
    );
  ...
  private static Map<String, Object> extractVueImageMetaData(
    IIOMetadata metaData
  ) {
    //final org.w3c.dom.Node metaTree
    final IIOMetadataNode metaTree = (IIOMetadataNode) metaData.getAsTree(
      GENERIC_META_DATA
    );

    if (DEBUG.IMAGE && DEBUG.IO) {
      Log.debug(
        " FORMAT-NAMES: " + Arrays.asList(metaData.getMetadataFormatNames())
      );
      Log.debug(
        "GOT META-DATA: " +
        Util.tags(metaData) +
  ...
  private static Map<String, Object> extractVueImageMetaData(
    IIOMetadataNode metaTree
  ) {
    Map result = Collections.EMPTY_MAP;
    for (org.w3c.dom.Node n : Util.iterable(
      metaTree.getElementsByTagName("TextEntry")
    )) {
      ...
      for (org.w3c.dom.Node a : Util.iterable(n.getAttributes())) {
        if (DEBUG.IO && DEBUG.DATA) Log.debug(
          "FOUND TEXT ATTRIB " + Util.tags(a)
        );
        ...
        if (text != null && text.startsWith(VUE_IMAGE_META_VALUE_PREFIX)) {
          ...
          if (DEBUG.IO || DEBUG.DATA) Log.debug(
            String.format("VUE-DATA: key=%s value=%s", key, Util.tags(value))
          );
          ...
          if (result == Collections.EMPTY_MAP) result = new HashMap(4);
  ...
  private static Handle loadImageAndCache(
    final ImageSource imageSRC,
    final Listener relay
  ) {
    Handle imageData = null;

    if (imageSRC.resource != null) imageSRC.resource
      .getProperties()
      .holdChanges();
    ...
    try {
      ...
      if (relay != null) {
        ...
        if (t instanceof java.io.FileNotFoundException) {
          ...
          t instanceof java.lang.IllegalArgumentException &&
          t.getMessage().startsWith("LUT has improper length")
        ) {
        ...
        if (dumpTrace) Log.warn(imageSRC + ":", t); else Log.warn(
          imageSRC + ": " + t
        );
      }
    ...
    if (imageSRC.useCacheFile()) {
      if (imageData != null) {
        ...
        if (imageSRC.hasCacheFile()) permanentCacheFile =
          ensurePermanentCacheFile(imageSRC.getCacheFile());

        if (DEBUG.IMAGE) out(
          "getting cache lock for storing result; " + imageSRC
        );
        RawCache.put(
          imageSRC.key,
          new CacheEntry(imageData, permanentCacheFile)
        );
        ...
        if (permanentCacheFile != null) imageSRC.setCacheFile(
          permanentCacheFile
        );
      ...
      if (imageData.image != null) RawCache.put(
        imageSRC.key,
        new CacheEntry(imageData, null)
      );
  ...
  private static Handle readImageInAvailableMemory(
    ImageSource imageSRC,
    Listener listener
    ...
    do {
      try {
        ...
        if (Thread.currentThread() instanceof ImgThread) {
          ...
          if (DEBUG.Enabled) Log.info(
            "EOM; consider re-kicking a LoadTask for " + imageSRC
          );
  ...
  private static File makeCacheFile(URI key, boolean temporary)
    ...
    if (temporary) cacheName = "." + keyToCacheFileName(key); else cacheName =
      keyToCacheFileName(key);
    ...
    if (cacheDir != null) {
      ...
      try {
        ...
        if (DEBUG.Enabled) Util.printStackTrace(e, msg); else Log.warn(msg, e);
        return null;
      }
      if (!file.canWrite()) {
  ...
  private static File ensurePermanentCacheFile(File file) {
    try {
      ...
      tufts.Util.printStackTrace(
        t,
        "Unable to create permanent cache file from tmp " + file
      );
    }
    return null;
  }
  ...
  public static final class Handle {
    ...
    Handle(Image i, Map<String, ?> data) {
      if (i == null) throw new NullPointerException("null image");
  ...
  private static Handle readAndCreateImage(
    ImageSource imageSRC,
    Images.Listener listener
    ...
    if (imageSRC.hasCacheFile()) {
      ...
      if (DEBUG.IMAGE && imageSRC.resource != null) {
        imageSRC.resource.setDebugProperty(
          "image.cache",
          imageSRC.getCacheFile()
        );
      ...
      if (imageSRC.resource != null) {
        imageSRC.resource.setProperty(
          CONTENT_SIZE,
          imageSRC.getCacheFile().length()
        );
        // java has no creation date for Files!  Well, last modified good enough...
        setDateValue(
          imageSRC.resource,
          CONTENT_ASOF,
          imageSRC.getCacheFile().lastModified()
        );
      }
      ...
      do {
        final URLConnection conn = UrlAuthentication.getAuthenticatedConnection(
          url
        );
        ...
        if (!imageSRC.useCacheFile()) {
          ...
          if (imageSRC.hasCacheFile()) {
            try {
              imageSRC.readable =
                new FileBackedImageInputStream(
                  urlStream,
                  tmpCacheFile,
                  listener
                );
      ...
      if (imageSRC.resource != null) setResourceMetaData(
        imageSRC.resource,
        (File) imageSRC.readable
      );
    }
    ...
    if (inputStream == null) throw new ImageException(
      "Can't Access [" + imageSRC.readable + "]"
    ); // e,g., local file permission denied
    ...
    if (reader == null) {
      badStream:{
        if (FirstFailure) {
          ...
          FirstFailure = false;
          Log.warn(
            "No reader found: first failure, rescanning for codecs: " + imageSRC
          );
          ...
          if (DEBUG.Enabled) tufts.Util.printStackTrace(
            "first failure: " + imageSRC
          );
          ImageIO.scanForPlugins();
          reader = getDecoder(inputStream, imageSRC);
          if (reader != null) break badStream;
        }
    ...
    if (w == 0 || h == 0) throw new ImageException(
      "invalid size: width=" + w + "; height=" + h
    );

    if (imageSRC.resource != null) {
      if (DEBUG.IMAGE || DEBUG.THREAD || DEBUG.RESOURCE) out(
        "setting resource image.* meta-data for " + imageSRC.resource
      );
    ...
    if (DEBUG.IMAGE || DEBUG.IO) out(
      "reading " + imageSRC + "; " + reader + "..."
    );
      ...
      if (listener != null && !ignoreMetadata) {
        ...
        try {
          ...
          if (sourceSize != null) {
            ...
            if (DEBUG.IO || DEBUG.IMAGE) Log.debug(
              String.format("parsed meta-data source size: %dx%d", sw, sh)
            );
            ...
            Log.warn(
              "failed to find sourceSize in meta-data: " +
              imageData +
              " for " +
              imageSRC
            );
  ...
  private void testImageInspect(
    ...
    try {
      ...
      if (thumbs > 0) Log.info("thumbs: " + thumbs + "; " + imageSRC);
    } catch (Throwable t) {
      Log.debug("getNumThumbnails", t);
    }
    if (DEBUG.WORK && image != null) {
      String[] tryProps = new String[] {
        "name",
        "title",
        "description",
        "comment",
      ...
      for (int i = 0; i < tryProps.length; i++) {
        ...
        if (
          p != null && p != java.awt.Image.UndefinedProperty
  ...
  static Handle createIcon(Image source, int maxSide) {
    ...
    //         }
    if (DEBUG.Enabled) Log.debug(
      "image pre-scaled: " + Util.tags(source) + " -> toMaxSide " + maxSide
    );
  ...
  public static tufts.vue.Size fitInto(float maxSide, float srcW, float srcH) {
    ...
    if (srcW < 1 || srcH < 1) {
      Log.error(
        "fitInto " + maxSide + ": bad source size " + srcW + "x" + srcH
      );
  ...
  private static Handle produceDrawnIcon(Image source, int maxSide) {
    final java.awt.Dimension size = fitInto(
      maxSide,
    ...
    if (source instanceof BufferedImage) transparency =
      ((BufferedImage) source).getTransparency(); else transparency =
      Transparency.OPAQUE;
    ...
    if (ALLOW_HIGH_QUALITY_ICONS/*&& DrawContext.isImageQualityRequested()*/) {
      ...
      // Amazing.
      iconSource =
        source.getScaledInstance(size.width, size.height, Image.SCALE_SMOOTH);
      ...
      if (iconSource instanceof sun.awt.image.ToolkitImage) { // will probably always be a ToolkitImage
        ...
        transparency =
          ((sun.awt.image.ToolkitImage) iconSource).getColorModel()
            .getTransparency();
      }
    ...
    final Image icon = tufts.vue.gui.GUI
    ...
    if (iconSource != source) iconSource.flush(); // crucial to release the memory consumed
  ...
  private static ImageReader getDecoder(
    ...
    while (iri.hasNext()) {
      ...
      if (DEBUG.IMAGE) {
        out(
          ...
          formatName +
          "; provider=" +
          Util.tags(ir.getOriginatingProvider())
        );
      }
      idx++;
    }
  ...
  private static String tag(Object o) {
    if (o instanceof java.awt.Component) return tufts.vue.gui.GUI.name(
      o
    ); else if (o instanceof LWComponent) return o.toString();
    //return ((LWComponent)o).getDiagnosticLabel();
    else return Util.tags(o);
    //         String s = Util.tag(o);
    //         s += "[";
  ...
  public static void main(String args[]) throws Exception {
    ...
    Object imageSRC;
    if (args[0].startsWith("http:") || args[0].startsWith("file:")) imageSRC =
      url = new java.net.URL(filename); else imageSRC =
...
class FileBackedImageInputStream extends ImageInputStreamImpl {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(FileBackedImageInputStream.class);
   ...
   */
  public FileBackedImageInputStream(
    InputStream stream,
    File file,
    Images.Listener listener
  ) throws IOException, Images.ImageException {
    if (stream == null || file == null) throw new IllegalArgumentException(
      "FileBackedImageInputStream: stream or file is null"
    );
    ...
    if (true) {
      ...
      if (DEBUG.IMAGE) {
        Log.debug(
          String.format(
            "ContentHead; got=%d; streamPos=%d; length=%d [%s]",
            got,
            streamPos,
            length,
            contentHead.trim()
      ...
      final String matcher = trimmed
        .substring(0, Math.min(16, trimmed.length()))
        .toUpperCase();
      ...
      if (matcher.startsWith("<HTML>") || matcher.startsWith("<!DOCTYPE")) {
        if (DEBUG.IMAGE) Log.warn(
          "Stream " +
          stream +
          " contains HTML, not image data: [" +
          contentHead.trim() +
          "]"
        ); else Log.info(
          "Stream contains HTML, not image data: see cache file for HTML sample:\n\t" +
          file
        );
  ...
  private long readUntil(long pos) throws IOException {
    ...
    while (len > 0) {
      ...
      final int nbytes = stream.read(
        streamBuf,
        0,
        (int) Math.min(len, (long) BUFFER_LENGTH)
      );
      if (nbytes == -1) {
        if (DEBUG.IMAGE && DEBUG.IO) System.err.println(
          "<EOF @ " + length + ">"
        );
        foundEOF = true;
        return length;
      ...
      if (DEBUG.IO) Log.debug(
        String.format("+%4d bytes; %7d total", nbytes, length + nbytes)
      );
      cache.write(streamBuf, 0, nbytes);
      len -= nbytes;
      length += nbytes;
      //System.out.println("READ TO " + length);
      if (listener != null) listener.gotImageProgress(stream, length, -1);
    }
