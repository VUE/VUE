...
public class VueAction extends javax.swing.AbstractAction {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueAction.class);
  ...
  static void setAllActionsIgnored(final boolean disabled) {
    ...
    for (VueAction a : AllActionList) {
      ...
      if (DEBUG.EVENTS && DEBUG.META && disabled && enabled) Log.debug(
        "setAllActionsIgnored override; always enabled: " + a
      );
  ...
  public static boolean isDupeStrokeAction(Action a) {
    if (DupeStrokeActions != null) return DupeStrokeActions.contains(
      a
    ); else return false;
  }

  public static void checkForDupeStrokes() {
    if (AllStrokes != null) return; // already checked

    for (VueAction a : AllActionList) trackForDupeStrokes(
      a,
  ...
  private static void trackForDupeStrokes(VueAction a, KeyStroke keyStroke) {
    ...
    if (keyStroke != null) {
      ...
      if (existingAction != null) {
        final KeyStroke existingStroke = (KeyStroke) existingAction.getValue(
          ACCELERATOR_KEY
        );
        if (existingStroke.equals(keyStroke)) {
          Util.printStackTrace(
            "WARNING; DUPLICATE KEYSTROKE: " +
            keyStroke +
            "; conflicting actions:" +
            "\n\t" +
            Util.tags(existingStroke) +
            "; " +
            Util.tags(existingAction) +
            "\n\t" +
            Util.tags(keyStroke) +
            "; " +
            Util.tags(a)
  ...
  public VueAction(
    ...
    if (shortDescription != null) putValue(
      SHORT_DESCRIPTION,
      shortDescription
    ); else putValue(SHORT_DESCRIPTION, name);
    ...
    if (isSelectionWatcher()) getSelectionWatchers().add(this);

    if (DEBUG.Enabled) trackForDupeStrokes(this, keyStroke);
  }
  ...
  public void trackToggler(AbstractButton toggler) {
    ...
    if (list == null) putValue(CHECKBOX_LIST, list = new java.util.ArrayList());
  ...
  protected void updateTogglers(boolean state) {
    final List<AbstractButton> toggles = (List<AbstractButton>) getValue(
      CHECKBOX_LIST
    );
      ...
      if (EnableSmallIconsForMenus) {
        icon =
          VueResources.getImageIconResource(
            "/toolbarButtonGraphics/" + iconSpec.substring(1) + "16.gif"
          );
        setSmallIcon(icon);
      }
      icon =
        VueResources.getImageIconResource(
          "/toolbarButtonGraphics/" + iconSpec.substring(1) + "24.gif"
        );
  ...
  private void setSmallIcon(Icon icon) {
    if (EnableSmallIconsForMenus) {
      if (icon != null) {
        if (icon != tufts.vue.gui.GUI.NO_ICON) putValue(SMALL_ICON, icon);
      }
    }
  }
  ...
  public void actionPerformed(ActionEvent ae) {
    ...
    try {
      ...
      synchronized (System.err) {
        System.err.println(
          "*** VueAction: exception during action [" + getActionName() + "]"
        );
  ...
  public String getUndoName(ActionEvent e, Throwable exception) {
    ...
    if (undoName == null) undoName = e.getActionCommand();
    if (undoName == null) undoName = getActionName();
    if (exception != null) {
      if (DEBUG.Enabled) undoName += " (!" + exception + ")"; else undoName +=
      " (!)";
    }
    return undoName;
  }
  ...
  public String getKeyStrokeDescription() {
    ...
    if (
      keyStroke != null
    ) return tufts.vue.action.ShortcutsAction.getDescription(
      keyStroke
    ); else return "Menu item: " + getActionName();
  }
  ...
  private static final Collection<VueAction> _selectionWatchers =
      ...
      .addListener(
        new LWSelection.Listener() {
          ...
          public String toString() {
            return (
              "Global Handler for " +
              _selectionWatchers.size() +
              " " +
              VueAction.class.getSimpleName() +
              "s"
            );
          }
        }
  ...
  private static void updateSelectionWatchers(
    final Collection<VueAction> watchers,
    final LWSelection s
  ) {
  ...
  protected boolean enabled() {
    return VUE.getActiveViewer() != null;
  }
  ...
  public boolean isUserEnabled() {
    if (
      allIgnored && !overrideIgnoreAllActions()
    ) return false; else return enabled();
  }
