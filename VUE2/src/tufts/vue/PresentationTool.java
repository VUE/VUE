 ...
 *
 */
public class PresentationTool extends VueTool implements LWComponent.Listener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(PresentationTool.class);

  private static final boolean AnimateAcrossMap = false;
  private static final boolean AnimateInOutMap = false;
  private static final boolean AnimateTransitions =
    AnimateAcrossMap || AnimateInOutMap;
  ...
  private static final BooleanPreference ChooseNodesOverSlides =
    BooleanPreference.create(
      ...
      VueResources.getMessageString(
        "preference.slidenodeview.descriptiontwo",
        obj
      ),
  ...
  public static final BooleanPreference AutoZoomPreference =
    BooleanPreference.create(
      edu.tufts.vue.preferences.PreferenceConstants.PRESENTATION_CATEGORY,
      "autoZoom",
  ...
  private final ImageButton ZoomButton = new ImageButton(
    "zoomOut",
    VueResources.getImageIcon("pathwayTool.zoomOutImageIcon")
  ) {
    void doAction() {
      ...
      if (
        ...
        !(VUE.getActiveViewer() instanceof tufts.vue.ui.SlideViewer)
      ) { // total SlideViewer hack...
        ZoomButton.setIcon(
          VueResources.getImageIcon("pathwayTool.zoomBackImageIcon")
        );
  ...
  private final ImageButton ExitButton = new ImageButton(
    "exit",
    VueResources.getImageIcon("pathwayTool.exitImageIcon")
  ) {
    void doAction() {
      if (VUE.inFullScreen()) VUE.toggleFullScreen(false, true);
      VUE.setActive(
        VueTool.class,
        this,
        VueTool.getInstance(SelectionTool.class)
      );
    }
  ...
  public static void ResumePresentation() {
    if (
      !VUE.inNativeFullScreen() && !VueUtil.isUnixPlatform()
    ) VUE.toggleFullScreen(true); else if (
      !VUE.inNativeFullScreen() && VueUtil.isUnixPlatform()
    ) {
  ...
  private final ImageButton ResumeButton = new ImageButton(
    "resume",
    VueResources.getImageIcon("pathwayTool.resumeImageIcon")
  ) {
    void doAction() {
    ...
    void setVisible(boolean visible) {
      ...
      if (visible) Actions.ToggleFullScreen.setActionName(
        ResumeActionName
      ); else Actions.ToggleFullScreen.revertActionName();
    }
  };
  ...
  private static volatile boolean mFadeEffect = true, mShowOverview =
    DEBUG.NAV, mShowNavNodes, mForceShowNavNodes, mDidAutoShowNavNodes, mScreenBlanked;
  ...
  private static class Page {
      ...
      if (c instanceof LWSlide) {
        if (((LWSlide) c).getPathwayEntry() != null) {
          ...
          if (DEBUG.Enabled) Util.printStackTrace(
            "creating page for slide w/out entry: " + c
          );
          entry = null;
          node = c;
        }
        ...
        if ( // mPathway != null // if we have a current pathway
          !LWPathway.isShowingSlideIcons() &&
          ChooseNodesOverSlides.isFalse() &&
          // and slide icons are turned off
          c instanceof
            LWNode && // and this is a node
          c.hasEntries()
        ) { // and it has slide views available (is on any pathway)
          ...
          if (
            slideInsteadOfNode == null && fallbackCount == 1
    ...
    public LWComponent getOriginalMapNode() {
      if (node != null) return node; else if (entry != null) return entry.node;
      return null;
    }

    public boolean isMapView() {
      if (entry == null) return (
        node != null && node.getAncestorOfType(LWSlide.class) == null
      ); else return entry.isMapView();
    }
    ...
    public boolean onPathway(LWPathway p) {
      if (p == null) return false; else if (node != null) return node.inPathway(
        p
      ); else if (entry != null) return entry.pathway == p; else return false;
    }
    ...
    public boolean inPathway(LWPathway p) {
      if (p == null) return false; else if (
        node != null && !node.inPathway(p)
      ) {
    ...
    public String getDisplayLabel() {
      if (node != null) return node.getDisplayLabel(); else if (
        entry != null
      ) return entry.getLabel(); else {
    ...
    public String toString() {
      //String s = "PAGE: ";
      String s = "";
      if (node != null) s += node; else s += entry;
      return s;
    }
  ...
  private static class PageQueue<T> extends ArrayList<T> {
    ...
    public void rollBack() {
      if (index > 0) index--; else throw new IndexOutOfBoundsException(
        "index=" + index
      );
    }
    ...
    public void rollForward() {
      if (
        index < lastIndex()
      ) index++; else throw new IndexOutOfBoundsException(
    ...
    public T popPrev() {
      if (!hasPrev()) throw new NullPointerException(
        this + " empty; can't pop"
      );
      return get(--index);
    }
    ...
    public T popNext() {
      if (!hasNext()) throw new NullPointerException(
        this + "index=" + index + "; size=" + size() + "; already at end"
      );
      return get(++index);
    }
  }
  ...
  private static final Font NavFontItalic = new Font(
    "SansSerif",
    Font.ITALIC,
    16
  );
  private static final Font NavFontDebug = new Font(
    "SansSerif",
    Font.PLAIN,
    10
  );
  private static final Font NavFontBoldDebug = new Font(
    "SansSerif",
    Font.BOLD,
    10
  );
  ...
  private final class NavNode extends LWNode {
    ...
    private final class JumpBox extends java.awt.geom.RoundRectangle2D.Float {
      ...
      Page getPage() {
        return entry == null
          ? new Page(page.getOriginalMapNode())
          : new Page(entry);
      }
      ...
      void draw(DrawContext dc) {
        ...
        if (entry == null) {
          ...
          page.entry != null && page.entry.pathway == entry.pathway
        ) dc.g.setStroke(STROKE_THREE); else dc.g.setStroke(STROKE_ONE);
    ...
    private List<JumpBox> createPathwayJumpBoxes(LWComponent node) {
      ...
      if (isOffEdge) rightEdge +=
      (NavLayout.InsetOffEdge - NavLayout.InsetIndent);
      ...
      if (mJumpBoxMask == null) mJumpBoxMask = new Rectangle2D.Float();
    ...
    NavNode(
      ...
      if (destinationPage == null) throw new IllegalArgumentException(
        getClass() + "; destination page is null"
      );
      ...
      if (page.node != null && page.node instanceof LWMap) label =
        "Map View"; else label = page.getDisplayLabel();
      ...
      if (style == STANDOUT) setFont(NavFontBold); else if (
        style == ITALIC
      ) setFont(NavFontItalic); else setFont(NavFont); // style == NORMAL
        ...
        if (mJumpBoxes != null) {
          ...
          for (JumpBox box : mJumpBoxes) {
            if (box.contains(localX, localY)) {
              if (DEBUG.PRESENT || DEBUG.PICK) Log.debug(
                "hit entry box " + box
              );
              return box.getPage();
            }
    ...
    public void draw(DrawContext dc) {
      ...
      if (DEBUG.Enabled) {
        ...
        if (page.entry != null) {
          ...
          if (page.node != null) dc.g.drawString(
            "NODE: " + page.node,
            -100,
            y += 10
          );
          ...
          if (page.entry != null) dc.g.drawString(
            "ENTRY: " + page.entry,
            -100,
            y += 10
          );
    ...
    private void drawPathwayJumpBoxes(DrawContext dc) {
      if (DEBUG.PRESENT) dc.g.setColor(Color.lightGray); else dc.g.setColor(
        NavFillColor
      );
      ...
      for (JumpBox box : mJumpBoxes) box.draw(dc);
    }
    ...
    @Override
    protected final float relativeLabelX() {
      return -NavNodeX + 10;
    }
  }
  ...
  public PresentationTool() {
    super();
    if (singleton != null) new Throwable(
      "Warning: mulitple instances of " + this
    )
      .printStackTrace();
    singleton = this;
  ...
  public void activeChanged(ActiveEvent e, LWPathway.Entry entry) {
    ...
    if (entry == null) {
      ...
      if (!entry.isPathway()) setEntry(entry, BACKING_UP);
    }
  }
  ...
  public JPanel createToolPanel() {
    ...
    JPanel p = new JPanel();
    mStartButton =
      new JButton(VueResources.getString("presentationTool.startButton.label"));
  ...
  private void add(JPanel p, AbstractButton c) {
    ...
    if (c instanceof JCheckBox) c.setContentAreaFilled(false);
    p.add(c, 0);
  ...
  private boolean onCurrentPathway() {
    ...
    if (
      p == null || p == NO_PAGE || mPathway == null
    ) return false; else return p.onPathway(mPathway);
  ...
  private boolean inCurrentPathway() {
    ...
    if (
      page == null || page == NO_PAGE || mPathway == null
    ) return false; else return page.inPathway(mPathway);
  }

  private void revisitPrior() {
    revisitPrior(false);
  }

  private void revisitPrior(boolean allTheWay) {
    if (DEBUG.PRESENT) out(
      String.format("revisitPrior%s", allTheWay ? " (AllTheWay)" : "")
    );

    if (allTheWay) setPage(mVisited.jumpFirst()); else if (
      mVisited.hasPrev()
    ) setPage(mVisited.popPrev(), BACKING_UP);
  }

  private void revisitNext() {
    revisitNext(false);
  }

  private void revisitNext(boolean allTheWay) {
    if (allTheWay) setPage(mVisited.jumpLast()); else if (
      mVisited.hasNext()
  ...
  private boolean goForward(boolean allTheWay) {
    return goForward(allTheWay, false);
  }
  ...
  private boolean goForward(boolean allTheWay, boolean guessing) {
    ...
    if (mCurrentPage == NO_PAGE && mNextPage == null) {
      ...
      if (guessing) {
        ...
        if (DEBUG.NAV) revisitNext(allTheWay); else wentForward = false;
      }
    }
  ...
  public PickContext initPick(PickContext pc, float x, float y) {
    ...
    if (mFocal instanceof LWMap == false) pc.pickDepth = 1;
    return pc;
  }
  ...
  public boolean handleKeyPressed(java.awt.event.KeyEvent e) {
    ...
    switch (keyCode) {
      ...
      case KeyEvent.VK_ENTER:
        ...
        break;
      case KeyEvent.VK_R:
        if (ResumeButton.isVisible()) ResumeButton.doAction(); else setPage(
          mLastStartPathwayPage
        );
        break;
      case KeyEvent.VK_SPACE:
        if (e.isShiftDown()) goBackward(false); else doDefaultForwardAction();
        break;
      case KeyEvent.VK_DOWN:
        if (DEBUG.NAV) {
          if (mVisited.hasNext()) revisitNext(amplified); else goForward(
            amplified
          );
          break;
    ...
    switch (keyChar) {
      ...
      case '=': // allow "non-shift-plus"
        if (
          isOverviewVisible() &&
          OverviewMapSizeIndex < OverviewMapScales.length - 1
        ) {
  ...
  private static float[] OverviewMapScales = {
    8,
    6,
    4,
    3,
  ...
  private void drawOverviewMap(DrawContext dc) {
    ...
    final Rectangle overview = new Rectangle(
      0,
      0,
    ...
    if (overviewMapFraction == 1 || DEBUG.Enabled) dc.g.setColor(
      OverviewMapFillAlpha
    ); else dc.g.setColor(OverviewMapFill);
    dc.g.fill(overview);
    ...
    if (standout != null) {
      ...
      if (inSlide != null) standout = inSlide;
      //standout = slide.getSourceNode();
    } else inSlide = null;

    if (inSlide != null && inSlide.getEntry() != null) pathway =
      inSlide.getEntry().pathway; else if (entry != null) pathway =
      entry.pathway; else pathway = null;
    ...
    mNavMapDC =
      MapPanner.paintViewerIntoRectangle(
        null,
        pannerGC,
        viewer,
        overview,
  ...
  public boolean handleMouseMoved(MapMouseEvent e) {
    ...
    if (DEBUG.MOUSE && DEBUG.META) {
      Log.debug(
        String.format(
          "CURX %d; Max %d; On pixel: %d, off pixel %d",
          e.getX(),
          width,
    ...
    if (DEBUG.Enabled && isOverviewVisible()) debugTrackNavMapMouseOver(e);
    ...
    if (e.getX() > MouseRightActivationPixel) {
      ...
      if (DEBUG.MOUSE && oldShowNav) Log.debug(
        "nav nodes should already be visible"
      );
    } else {
    ...
    if (oldShowNav != mShowNavNodes) repaint("mouseMove nav display change");
  ...
  private LWComponent getOverviewMapHit(MapMouseEvent e) {
    if (e.getX() < mNavMapX || e.getY() < mNavMapY) return null;

    //out("over map at " + e.getPoint());
    Point2D.Float mapPoint = new Point2D.Float(
      e.getX() - mNavMapX,
      e.getY() - mNavMapY
    );
    ...
    final MapViewer viewer = e.getViewer();
    final PickContext pc = new PickContext(
      viewer.getLastDC(),
      mapPoint.x,
      mapPoint.y
    );
  ...
  private void toggleFocal(MapMouseEvent e) {
    ...
    if (mCurrentPage.isMap()) setPage(mLastPage); else focusUp(e, TO_MAP);
  }
  ...
  public boolean handleMousePressed(MapMouseEvent e) {
    ...
    if (isOverviewVisible()) {
      ...
      if (hit != null && hit != HIT_OVERVIEW) {
        setPage(hit);
        if (e.isShiftDown()) mShowOverview = false;
        return true;
      }
    }

    if (hit == null && checkForNavNodeClick(e)) return true;

    //if (e.isShiftDown())
    //    return false;

    if (hit == null) hit = e.getPicked();

    if (DEBUG.PRESENT) out("[" + e.paramString() + "] hit=" + hit);

    if (
      hit == HIT_OVERVIEW
    ) return true; // only needed in earlier design: should never hit this now
    ...
    if (hit != null) {
      ...
      if (hit.isExternalResourceLinkForPresentations()) {
        ...
        if (hit.handleSingleClick(e)) return true;
      }
  ...
  private void doDefaultForwardAction() {
    ...
    if (mVisited.hasPrev() && mCurrentPage.entry == null) {
      ...
      if (
        DEBUG.Enabled && mFocal != mCurrentPage.getPresentationFocal()
      ) Util.printStackTrace(
        "mFocal != curPagePresFocal; " +
        mFocal +
        " != " +
        mCurrentPage.getPresentationFocal()
      );
      final LWComponent lastFocal = mVisited.prev().getPresentationFocal();
      boolean lastWasAncestor = mFocal.hasAncestor(lastFocal);
      if (DEBUG.PRESENT) out(
        "LAST FOCAL: " + lastFocal + "; ANCESTOR=" + lastWasAncestor
      );
      if (lastWasAncestor) {
        revisitPrior();
        return;
  ...
  public void handleToolSelection(boolean selected, VueTool fromTool) {
    ...
    if (VUE.getSelection().size() == 1) mNextPage =
      VUE.getSelection().first(); else mNextPage = null;
  ...
  public void startPresentation(LWSelection selection) {
    ...
    try {
      if (pathway != null && pathway.length() > 0) {
        ...
        loadPathway(null);
        //mPathwayIndex = 0;
        if (selection.size() > 0) setPage(VUE.getSelection().first());
  ...
  private LWPathway pickStartingPathway(LWSelection selection) {
    ...
    if (pathway != null) {
      ...
      if (pathway != null && !pathway.isVisible()) {
        if (!pathway.isLocked() && pathway.hasEntries()) pathway.setVisible(
          true
        ); else pathway = null; // non-pathway presentation mode
      }
    }
  ...
  private LWPathway.Entry nextPathwayEntry(
    String direction,
    boolean allTheWay
  ) {
    if (mLastPathwayPage == null) return null;

    final Entry entry = mLastPathwayPage.entry;

    if (direction == FORWARD) {
      if (allTheWay) return entry.pathway.getLast(); else return entry.next();
    } else { // direction == BACKWARD
      if (allTheWay) return entry.pathway.getFirst(); else return entry.prev();
    }
  }
  ...
  public void LWCChanged(LWCEvent e) {
    ...
    if (e.key == LWKey.Repaint || e.source instanceof LWMap) repaint(
      e.toString()
    );
  }
  ...
  private void recordPageTransition(final Page page, boolean recordBackup) {
    ...
    if (page.equals(mVisited.prev())) {
      ...
      if (page.isMap()); // don't record map views
      else mVisited.push(page);
    }
    ...
    if (page.getPresentationFocal() instanceof LWSlide) mLastSlidePage = page;

    if (page.onPathway()) {
      if (page.pathway() != mPathway) loadPathway(page.pathway());
  ...
  private void setPage(LWComponent destination) {
    if (destination != null) setPage(new Page(destination), RECORD_BACKUP);
  }
  ...
  private void setPage(final Page page, boolean recordBackup) {
    ...
    if (page.equals(mCurrentPage)) {
      if (DEBUG.Enabled) Log.debug(
        "current page matches new page: " + mCurrentPage + "; requested=" + page
      );
      return;
    }
    ...
    if (page.entry != null && page.entry.isPathway()) {
      ...
      if (DEBUG.Enabled) Util.printStackTrace(
        "present master slide attempt: " + page.entry
      );
      return;
    }
  ...
  public boolean handleFocalSwitch(
    ...
    if (!pageLoadingUnderway) recordPageTransition(new Page(newFocal), true);
    ...
    if (mFadeEffect && !animate) {
      if (!startUnderway) makeInvisible();
      viewer.loadFocal(mFocal, FIT_FOCAL, NO_ANIMATE);
      if (!startUnderway) fadeUpLater();
    } else {
    ...
    if (AnimateTransitions) {
      ...
      if (oldFocal.hasAncestor(newFocal)) {
        ...
        oldFocal instanceof LWSlide || newFocal instanceof LWSlide
      ...
      if (animatingFocal != null) {
        ...
        mFocal = newFocal;
        if (animatingFocal == newFocal) loaded = true;
      ...
      if (
        animatingFocal instanceof LWSlide && newFocal == animatingFocal
      ) animateToFocal(viewer, newFocal, true); else animateToFocal(
        viewer,
        newFocal,
        false
      );
  ...
  private void animateToFocal(
    MapViewer viewer,
    LWComponent newFocal,
    boolean forceRawSlideBounds
  ) {
    ...
    if (forceRawSlideBounds) focalBounds =
      newFocal.getBounds(); else focalBounds = getFocalBounds(newFocal);
  ...
  public DrawContext getDrawContext(DrawContext dc) {
    ...
    dc.setDrawPathways(
      dc.isAnimating() ||
      mCurrentPage == null ||
      mCurrentPage.node instanceof LWMap
    );
  ...
  public void handlePreDraw(DrawContext dc, MapViewer viewer) {
    ...
    if (mPathway != null) {
      if (mCurrentPage != null) {
        ...
        if (DEBUG.PRESENT) out(
          "handlePreDraw: FILLING FOR ENTRY: " + mCurrentPage.entry
        );
        if (mCurrentPage.entry != null) {
          dc.fillBackground(mCurrentPage.entry.getFullScreenFillColor(dc));
          //if (!mCurrentPage.entry.hasSlide())
          if (
            mCurrentPage.entry.isMapView() || !mCurrentPage.entry.hasSlide()
          ) drawMaster = true; // For VUE-967
        } else {
  ...
  public void handlePostDraw(DrawContext dc, MapViewer viewer) {
    if (DEBUG.PRESENT) out(
      "handlePostDraw " + viewer + "; showNav=" + mShowNavNodes
    );
    ...
    if (mShowNavNodes) {
      ...
      if (true || dc.isInteractive()) {
        drawNavNodes(dc.push());
        dc.pop();
      } else out("NON-INTERACTIVE (no NavNodes)");
    ...
    if (true || dc.isInteractive()) {
      if (ResumeButton.isVisible()) {
        dc.setFrameDrawing();
        ResumeButton.setLocation(
          30,
          dc.frame.height - (ResumeButton.height + 70)
        );
        ...
        // only really need to set location when this tool actives, but just
        // in case the screen size should change...
        ZoomButton.setLocation(
          dc.frame.width - 80,
          dc.frame.height - (ZoomButton.height + 20)
        );
        ZoomButton.draw(dc);
      }
    }
    ...
    if (DEBUG.NAV) {
      ...
      dc.g.drawString(
        "CurPageFocal: " + mCurrentPage.getPresentationFocal(),
        10,
        y += 15
      );
      y += 5;
      ...
      int i = 0;
      for (Page p : mVisited) {
        if (p == mVisited.current()) dc.g.setColor(
          Color.green
        ); else dc.g.setColor(Color.gray);
  ...
  private class NavLayout {
    ...
    void recordUnique(Page page) {
      if (page != null) recordUnique(page.getOriginalMapNode());
    }

    void recordUnique(LWComponent c) {
      if (c != null) unique.add(c);
    }
    ...
    void addIfUnique(Page page, String debug) {
      ...
      if (!repeat || DEBUG.PRESENT) {
        ...
        add(nn);
        if (DEBUG.PRESENT && repeat) nn.setFillColor(null);
      }
    }
  ...
  private void makeNavNodes(Rectangle frame) {
    ...
    if (mCurrentPage == NO_PAGE || mCurrentPage == null) return;
    ...
    if (curEntry != null && curEntry.prev() != null) prev =
      new Page(curEntry.prev()); else prev = null;

    // always add the current pathway at the top
    if (
      mLastStartPathwayPage != null &&
      mLastStartPathwayPage.entry != mCurrentPage.entry
    ) layout.setLastPathway(mLastStartPathwayPage);

    if (
      prev != null &&
      !prev.equals(mLastStartPathwayPage) &&
      !prev.equals(mCurrentPage)
    ...
    if (curEntry != null) {
      ...
      if (nextEntryThisPath != null) layout.add(
        new Page(nextEntryThisPath),
        "next-path"
      );
    }
    ...
    for (LWPathway path : mapNode.getPathways()) {
      if (path.isDrawn() && path != mPathway) {
        ...
        if (nextPathEntry != null) layout.addIfUnique(
          new Page(nextPathEntry),
          "other-path"
        );
      }
    }
...
//             mNextPage = null;
  ...
  /** @return false */
  public boolean supportsResizeControls() {
    return false;
  }

  /** @return false */
  public boolean supportsDrag(InputEvent e) {
    return false;
  }
  ...
  /** @return false */
  public boolean supportsEditActions() {
    return false;
  }
  ...
  public boolean permitsToolChange() {
    if (VUE.inNativeFullScreen()) return false; else if (
      VUE.inWorkingFullScreen() && ResumeButton.isVisible()
