...
public class TuftsDLAuthZ {
  ...
  public static final String VISITOR_LIST = new String(
    "http://www.nolaria.org/vue_visitors.txt"
  );
  public static final String ADMIN_LIST = new String(
    "http://www.nolaria.org/vue_admins.txt"
  );
  ...
  public TuftsDLAuthZ() throws osid.OsidException {
    ...
    sharedMgr =
      (tufts.oki.shared.SharedManager) osid.OsidLoader.getManager(
        "osid.shared.SharedManager",
        "tufts.oki.shared",
        myOwner
      );
    authNMgr =
      (tufts.oki.authentication.AuthenticationManager) osid.OsidLoader.getManager(
        "osid.authentication.AuthenticationManager",
        "tufts.oki.authentication",
        myOwner
      );
    authNMgr.initManager(sharedMgr);
    authZMgr =
      (tufts.oki.authorization.AuthorizationManager) osid.OsidLoader.getManager(
        "osid.authorization.AuthorizationManager",
        "tufts.oki.authorization",
        myOwner
      );
  ...
  private void initQualifierTypes() {
    qualifierTypes.put(
      TuftsDLAuthZ.ASSET,
      new VueQualifierType(
        VueQualifierType.QUALIFIER_TYPE_KEY + "Fedora.Asset",
        "A Fedora Asset."
      )
    );
    qualifierTypes.put(
      TuftsDLAuthZ.OWNED_ASSET,
      new VueQualifierType(
        VueQualifierType.QUALIFIER_TYPE_KEY + "Fedora.OwnedAsset",
        "An owned Fedora Asset."
      )
    );
  }
  ...
  private void initFunctionTypes() {
    functionTypes.put(
      TuftsDLAuthZ.AUTH_SEARCH,
      new VueFunctionType(
        VueFunctionType.FUNCTION_TYPE_KEY + "Fedora.Search",
        "Search a Fedora repository."
      )
    );
    functionTypes.put(
      TuftsDLAuthZ.AUTH_VIEW,
      new VueFunctionType(
        VueFunctionType.FUNCTION_TYPE_KEY + "Fedora.View",
        "View an asset in a Fedora repository."
      )
    );
    functionTypes.put(
      TuftsDLAuthZ.AUTH_UPDATE,
      new VueFunctionType(
        VueFunctionType.FUNCTION_TYPE_KEY + "Fedora.Update",
        "Update an asset in a Fedora repository."
      )
    );
    functionTypes.put(
      TuftsDLAuthZ.AUTH_ADD,
      new VueFunctionType(
        VueFunctionType.FUNCTION_TYPE_KEY + "Fedora.Add",
        "Add a new asset to a Fedora repository."
      )
    );
    functionTypes.put(
      TuftsDLAuthZ.AUTH_DELETE,
      new VueFunctionType(
        VueFunctionType.FUNCTION_TYPE_KEY + "Fedora.Delete",
        "Delete an asset in a Fedora repository."
      )
    );
  }
  ...
  private void initQualifiers() {
    try {
      root =
        (VueQualifier) authZMgr.createRootQualifier(
          sharedMgr.createId(),
          "Tufts DL Asset",
          "A Tufts digital library asset.",
          (osid.shared.Type) qualifierTypes.get("asset"),
          (osid.shared.Id) null
        );
      qualifiers.put(TuftsDLAuthZ.ASSET, root);
      qualifiers.put(
        TuftsDLAuthZ.OWNED_ASSET,
        authZMgr.createQualifier(
          sharedMgr.createId(),
          "Tufts DL Owned Asset",
          "An owned Tufts digital library asset.",
          (osid.shared.Type) qualifierTypes.get("owned-asset"),
          root.getId()
        )
      );
  ...
  private void initFunctions() {
    try {
      functions.put(
        TuftsDLAuthZ.AUTH_SEARCH,
        authZMgr.createFunction(
          sharedMgr.createId(),
          "Search",
          "Search the Tufts DL.",
          (osid.shared.Type) functionTypes.get("search"),
          root.getId()
        )
      );
      functions.put(
        ...
        authZMgr.createFunction(
          sharedMgr.createId(),
          "View",
          "View an asset in the Tufts DL.",
          (osid.shared.Type) functionTypes.get("view"),
          root.getId()
        )
      );
      functions.put(
        ...
        authZMgr.createFunction(
          sharedMgr.createId(),
          "Update",
          "Update an asset in the Tufts DL.",
          (osid.shared.Type) functionTypes.get("update"),
          root.getId()
        )
      );
      functions.put(
        ...
        authZMgr.createFunction(
          sharedMgr.createId(),
          "Add",
          "Add an asset to the Tufts DL.",
          (osid.shared.Type) functionTypes.get("add"),
          root.getId()
        )
      );
      functions.put(
        ...
        authZMgr.createFunction(
          sharedMgr.createId(),
          "Delete",
          "Delete an asset from the Tufts DL.",
          (osid.shared.Type) functionTypes.get("delete"),
          root.getId()
        )
      );
    } catch (osid.shared.SharedException ex) {}
  ...
  private void initAgentTypes() {
    agentTypes.put(
      "visitor",
      new VueAgentType(
        VueAgentType.AGENT_TYPE_KEY + "Tufts.visitor",
        "A person of type visitor."
      )
    );
    agentTypes.put(
      "student",
      new VueAgentType(
        VueAgentType.AGENT_TYPE_KEY + "Tufts.student",
        "A person of type student."
      )
    );
    agentTypes.put(
      "administrator",
      new VueAgentType(
        VueAgentType.AGENT_TYPE_KEY + "Tufts.administrator",
        "A person of type administrator."
      )
    );
  }
   ...
   *  @author Mark Norton
   */
  public osid.shared.Agent authorizeUser(String username, String password)
    ...
    osid.shared.Type ldapType =
      (osid.shared.Type) new tufts.oki.authentication.AuthenticationLDAP();
    ...
    try {
      ...
      try {
        user =
          sharedMgr.createAgent(
            username,
            (osid.shared.Type) agentTypes.get("student")
          );
      ...
      if (isVisitor(username)) {
        ...
        try {
          user =
            sharedMgr.createAgent(
              username,
              (osid.shared.Type) agentTypes.get("visitor")
            );
    ...
    if (isAdmin(username)) {
      ...
      try {
        user =
          sharedMgr.createAgent(
            username,
            (osid.shared.Type) agentTypes.get("administrator")
          );
  ...
  private boolean isVisitor(String username) {
    ...
    try {
      ...
      for (int i = 0; i < admins.size(); i++) {
        ...
        if (
          username.compareTo(((String) admins.elementAt(i)).trim()) == 0
        ) return true;
      }
  ...
  private void authorizeUser(osid.shared.Agent user) {
    ...
    if (userType.isEqual((osid.shared.Type) agentTypes.get("visitor"))) {
      try {
        this.authZMgr.createAuthorization(
            userId,
            ((osid.authorization.Function) functions.get(
                TuftsDLAuthZ.AUTH_SEARCH
              )).getId(),
            ((osid.authorization.Qualifier) qualifiers.get(
                TuftsDLAuthZ.ASSET
              )).getId()
          );
        this.authZMgr.createAuthorization(
    ...
    if (userType.isEqual((osid.shared.Type) agentTypes.get("student"))) {
      try {
        ...
        ftn =
          ((osid.authorization.Function) functions.get(TuftsDLAuthZ.AUTH_VIEW));
        this.authZMgr.createAuthorization(
            userId,
            ftn.getId(),
            ((osid.authorization.Qualifier) qualifiers.get(
    ...
    if (userType.isEqual((osid.shared.Type) agentTypes.get("administrator"))) {
      try {
        ...
        System.out.println(
          "authorizeUser - ftn id: " +
          ftn.getId().getIdString() +
          " type: " +
          ftn.getFunctionType().getKeyword()
        );
        this.authZMgr.createAuthorization(
            userId,
            ftn.getId(),
        ...
        ftn =
          ((osid.authorization.Function) functions.get(TuftsDLAuthZ.AUTH_ADD));
        System.out.println(
          "authorizeUser - ftn id: " +
          ftn.getId().getIdString() +
          " type: " +
      ...
      } catch (osid.authorization.AuthorizationException ex2) {} catch (
        osid.shared.SharedException ex3
      ) {}
    }
  }
   ...
   *  @author Mark Norton
   */
  public osid.authorization.AuthorizationIterator getAuthorizations(
    osid.shared.Agent user
  ) {
    osid.shared.Id userId = null;
    try {
      userId = user.getId();
  ...
  public boolean isAuthorized(
    ...
    osid.authorization.Function ftn =
      (osid.authorization.Function) functions.get(operation);
    osid.authorization.Qualifier qual =
      (osid.authorization.Qualifier) qualifiers.get(assetType);
    osid.shared.Id userId = null;
    ...
    try {
      ...
      ftnId = ftn.getId();
      qualId = qual.getId();
    } catch (osid.authorization.AuthorizationException ex1) {} catch (
      osid.shared.SharedException ex2
    ) {}
