...
public class RemoteFileDataSource extends BrowseDataSource {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(RemoteFileDataSource.class);
  ...
  public void setConfiguration(java.util.Properties p) {
    ...
    String val = null;
    try {
      if ((val = p.getProperty("username")) != null) setUserName(val);
    } catch (Throwable t) {
      Log.error("val=" + val, t);
    }
    try {
      if ((val = p.getProperty("password")) != null) setPassword(val);
    } catch (Throwable t) {
      Log.error("val=" + val, t);
    }
  }
  ...
  protected JComponent buildResourceViewer() {
    ...
    try {
      ...
      manager.createClient(
        ...
        this.getUserName(),
        this.getPassword()
      ); // make a connection to the ftp site
      RemoteCabinetEntryIterator rootCabs =
        (RemoteCabinetEntryIterator) manager.listRoots();
