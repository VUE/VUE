...
public class PropertyMap
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(PropertyMap.class);
  ...
  private static boolean hasContent(Object v) {
    if (v == null) return false; else if (
  ...
  public Object getValue(Object k) {
    ...
    if (o instanceof Reference) {
      o = ((Reference) o).get();
      if (DEBUG.Enabled && o == null) Log.debug("value was GC'd for key: " + k);
    }
   ...
   */
  public synchronized String addProperty(
    ...
    while (containsKey(key)) key =
      String.format("%s.%03d", desiredKey, index++);
    put(key, value);
    return key;
  }

  public synchronized String addIfContent(
    final String desiredKey,
    Object value
  ) {
    if (hasContent(value)) return addProperty(
      desiredKey,
      value
    ); else return null;
  }
  ...
  public javax.swing.table.TableModel getTableModel() {
    ...
    if (mTableModel == null) {
      synchronized (mTableModel_LOCK) {
        if (mTableModel == null) mTableModel = new SortedMapModel();
      }
    }
    return mTableModel;
  }
  ...
  private static class Entry implements Comparable {
    ...
    public int compareTo(Object o) {
      if (priority) return Short.MIN_VALUE; else if (
        ((Entry) o).priority
      ) return Short.MAX_VALUE; else return key.compareTo(((Entry) o).key);
    }
  ...
  private class SortedMapModel extends javax.swing.table.AbstractTableModel {
    ...
    private void reload() {
      ...
      if (DEBUG.RESOURCE) out(
        "SortedMapModel: reload " + mEntries.length + " items"
      );
      ...
      if (DEBUG.RESOURCE) {
        ...
        if (DEBUG.META) out("model loaded " + Arrays.asList(mEntries));
      }
    ...
    public Object getValueAt(int row, int col) {
      if (row > mEntries.length) {
        tufts.Util.printStackTrace(
          "SortedMapModel has only " +
          mEntries.length +
          " entries, attempt to access row " +
          row
        );
        return "<empty>";
      }
      ...
      if (entry == null) {
        ...
        return null;
      } else if (col == 0) return entry.key; else return entry.value;
    }
