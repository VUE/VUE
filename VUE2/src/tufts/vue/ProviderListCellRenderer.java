...
public class ProviderListCellRenderer extends DefaultListCellRenderer {
  ...
  private final Icon checkedIcon = VueResources.getImageIcon(
    "addLibrary.checkMarkIcon"
  );
  ...
  private Color AlternateRowColor = VueResources.getColor(
  ...
  public Component getListCellRendererComponent(
    ...
    if (selected) {
      ...
      if (index % 2 == 0) bg = list.getBackground(); else bg =
        AlternateRowColor;
    }
    ...
    /*   if (index == waitingMode)
        ...
        */
    if (checklist.size() == 0 || checklist.size() <= index) checklist.add(
      index,
      new Boolean(false)
    );

    if (
      ((Boolean) checklist.get(index)).booleanValue() &&
      mRow.getComponent(1) != waitLabel
    ) {
      mRow.remove(1);
      ...
      try {
        ...
        while (propertiesIterator.hasNextProperties()) {
          org.osid.shared.Properties props =
            propertiesIterator.nextProperties();
          ...
          while (objectIterator.hasNextObject()) {
            ...
            try {
              if (key.equals("icon16x16")) {
                String path = factory.getResourcePath(
    ...
    /* if (value == ProviderList.IndicatedDragOverValue)
        ...
        else */if (selected) mLabel.setForeground(Color.black);
  ...
  public void clearAllChecked() {
    for (int i = 0; i < checklist.size(); i++) checklist.set(
      i,
      new Boolean(false)
    );
  }
