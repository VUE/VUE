...
public class ImageRef {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ImageRef.class);
  private static final int PIXEL_THRESHOLD_FOR_ICON_GENERATION =
    (2 * DEFAULT_ICON_SIZE) * (2 * DEFAULT_ICON_SIZE);
  /**
   ...
   */
  private static final int PIXEL_THRESHOLD_FOR_ICON_DRAWING =
    DEFAULT_ICON_SIZE * 2;
  ...
  private static final java.awt.Color LoadingOverlay = new java.awt.Color(
    128,
    128,
    128,
    128
  );
  private static final java.awt.Color LoadingOverlayWhite = new java.awt.Color(
    255,
    255,
    255,
    128
  );
  private static final java.awt.Color LoadingOverlayBlack = new java.awt.Color(
    0,
    0,
    0,
    128
  );
  private static final java.awt.Color DebugRed = new java.awt.Color(
    255,
    0,
    0,
    128
  );
  private static final java.awt.Color DebugGreen = new java.awt.Color(
    0,
    255,
    0,
    128
  );
  private static final java.awt.Color DebugBlue = new java.awt.Color(
    0,
    0,
    255,
    128
  );
  private static final java.awt.Color DebugYellow = new java.awt.Color(
    255,
    255,
    0,
    128
  ...
  private ImageRep pickRepToDraw(
    final ImageRep desired,
    final ImageRep backup
  ) {
    ...
    if (
      desired.available()
    ) return desired; else if (backup.available()) return backup; else if ( // the most common case at runtime
      desired == backup
    ) {
      ...
      if (desired != ImageRep.UNAVAILABLE) {
        // should never happen
        Log.error(
          "desired == backup != UNAVAILABLE: " + desired,
          new Throwable("HERE")
        );
        reload();
      }
      return ImageRep.UNAVAILABLE;
    } else if (desired.loading()) return desired; else if (
      backup.loading()
    ) return backup; else if (desired.hasError()) return desired; else if (
      backup.hasError()
    ) return backup; else return ImageRep.UNAVAILABLE;
  }

  //private static final double ASSUMED_PRINTER_DPI = 600;

  private ImageRep getIdealRep(
    DrawContext dc,
    float width,
    float height,
    Image[] GC_lock
  ...
  static void drawBetterRepAvailableIndicator(
    Graphics2D g,
    float width,
    float height
  ) {
  ...
  private void drawDebugStatus(
    ...
    final java.awt.geom.Rectangle2D.Float r =
  ...
  public void notifyRepHasArrived(
    final ImageRep freshRep,
    final Images.Handle hardImageRef
  ) {
    ...
    if (freshRep == _full && _icon == UNAVAILABLE && hardImageRef != null) {
      ...
      freshRep == _icon && (_full == UNAVAILABLE || _full.size() == ZERO_SIZE)
    ) {
  ...
  private ImageRep createRuntimeScaledIconRep(
    final ImageRep full,
    final Image hardFullImageRef
  ) {
    ...
    final ImageRep icon = ImageRep.create(
      this,
  ...
  static String debugSRC(ImageSource s) {
    if (s == null) return "[null ImageSource]"; else return s.debugName();
  }
  ...
  @Override
  public boolean equals(Object o) {
    if (o instanceof ImageRef) return (
      ((ImageRef) o)._source.original == _source.original
    ); else return false;
  }
