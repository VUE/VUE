...
public class SearchTextField extends JTextField implements FocusListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(SearchTextField.class);
  ...
  private static final String SEARCH_WORD = VueResources.local(
    "search.popup.search"
  );
  private static final String _DEFAULT_TEXT = VueResources.local(
    "search.text.default"
  );
  ...
  SearchTextField() {
    ...
    if (
      !DRAW_OVER_STATUS || Util.isMacTiger() || isWindows
  ...
  private String getRawText() {
    if (
      inputField == this
    ) return super.getText(); else return inputField.getText();
  }

  private void resetForInput() {
    // Clear friendly gray reminder note, get ready for input:
    inputField.setText(MINIMUM_TEXT);
    if (!DRAW_OVER_STATUS) inputField.setForeground(Color.black);
  }
  ...
  private String getStatusMessage() {
    ...
    if (
      searchType == searcheveryWhereMenuItem
    ) return DEFAULT_TEXT; else return (
      SEARCH_WORD + " " + searchType.getLabel()
    );
  }

  private void updateMessage(String src) {
    if (DEBUG.SEARCH) Log.debug(
      "updateMessage src=" + src + "; inUpdate=" + inUpdate
    );
    if (!inUpdate) {
      if (DRAW_OVER_STATUS) repaint(); // display via draw-over -- better as hides the 'x' button
      else inputField.setText(getStatusMessage()); // display via input text
    }
  }

  public static void updateSearchType() {
    if (!Singleton.hasInput()) Singleton.updateMessage("remoteUpdate");
  }

  private JMenuItem getSelectedSearchTypeMenuItem() {
    if (
      searcheveryWhereMenuItem.isSelected()
    ) return searcheveryWhereMenuItem; else if (
      labelMenuItem.isSelected()
    ) return labelMenuItem; else if (
      keywordMenuItem.isSelected()
    ) return keywordMenuItem; else if (
      categoriesMenuItem.isSelected()
    ) return categoriesMenuItem; else if (
      categoryKeywordMenuItem.isSelected()
    ) return categoryKeywordMenuItem; else return searcheveryWhereMenuItem;
  }
  ...
  private void runConfiguredSearch() {
    if (labelMenuItem.isSelected()) runSearch(SEARCH_ONLY_LABELS); else if (
      keywordMenuItem.isSelected()
    ) runSearch(SEARCH_ONLY_KEYWORDS); else runSearch(SEARCH_EVERYTHING);
  ...
  private final MouseAdapter MacMouseListener = new MouseAdapter() {
  ...
  private void initForPlatformMacTiger() {
    ...
    if (!DRAW_OVER_STATUS) DEFAULT_TEXT = MINIMUM_TEXT + DEFAULT_TEXT;
  ...
  private synchronized void initForPlatformWindows() {
        ...
        public void mouseClicked(MouseEvent e) {
          if (
            subTextField
              .getText()
              .trim()
              .equals(VueResources.local("search.text.default"))
          ) {
    ...
    textField.addMouseListener(
      new MouseAdapter() {
        ...
        public void mouseReleased(MouseEvent e) {
          if (e.isPopupTrigger()) {
            if ((e.getX() < 23)) {} else if (
              e.getX() < textField.getWidth() - 23
            ) {
              createEditPopupMenu();
              editPopup.show(e.getComponent(), e.getX() + 5, e.getY());
            } else {}
          ...
          if ((e.getX() < 23)) {
            ...
            if (
              getText().trim().equals(VueResources.local("search.text.default"))
            ) {
              setText("");
            ...
            if (DEBUG.Enabled) Log.debug(
              "Windows attempting to launch actions separately",
              new Throwable("HERE")
            );
  ...
  class PopupActionListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent actionEvent) {
      ...
      if (DEBUG.SEARCH || DEBUG.FOCUS) Log.debug(
        "actionPerformed " + GUI.name(actionEvent)
      );
      if ("search.popup.select.all".equals(action)) {
        inputField.grabFocus();
        inputField.selectAll();
      } else if ("search.popup.cut".equals(action)) {
        inputField.cut();
      } else if ("search.popup.copy".equals(action)) {
        inputField.copy();
      } else if ("search.popup.paste".equals(action)) {
        if (!hasInput()) resetForInput(); // be sure to first clear any grey messaging
        inputField.paste();
        runConfiguredSearch();
      } else if ("search.popup.clear".equals(action)) {
        inputField.setText(MINIMUM_TEXT);
      } else if ("search.popup.searcheverywhere".equals(action)) {
        setSearchEverywhereAction();
      } else if ("search.popup.edit.search.settings".equals(action)) {
        setEditSettingsAction();
      } else if ("search.popup.labels".equals(action)) {
        setLabelSettingsAction();
      } else if ("search.popup.keywords".equals(action)) {
        setKeywordSettingsAction();
      } else if ("search.popup.reset".equals(action)) {
        setResetSettingsAction();
      } else if ("search.popup.categories".equals(action)) {
        setCategorySettingsAction();
      } else if ("searchgui.categories_keywords".equals(action)) {
        setKeywordCategorySettingsAction();
      }
    }
  }
  ...
  private void updateMetaDataGUI(SearchType domain) {
    if (DEBUG.Enabled) Log.debug(
      "no longer syncing search-fields type with crap-tastic MetadataSearchMainGUI; domain=" +
      domain
    );
    // //VUE.getMetadataSearchMainGUI().setVisible(false);
    // if (typeKey != null) {
  ...
  private void runSearch(SearchType domain) {
    if (DRAW_OVER_STATUS) repaint();
    updateMetaDataGUI(domain);
    if (hasInput()) fireSearch(domain);
  }
  ...
  private void setLabelSettingsAction() {
    runSearch(SEARCH_ONLY_LABELS);
  }

  private void setKeywordSettingsAction() {
    runSearch(SEARCH_ONLY_KEYWORDS);
  }
  ...
  private void createEditPopupMenu() {
    if (editPopup == null) {
      ...
      JMenuItem clearMenuItem = new JMenuItem(
        VueResources.local("search.popup.clear")
      );
      clearMenuItem.addActionListener(actionListener);
      editPopup.add(clearMenuItem);
      JMenuItem resetSettingsMenuItem = new JMenuItem(
        VueResources.local("search.popup.reset")
      );
  ...
  private static final Image arrowImg = VueResources.getImage(
    "search.downarrowicon"
  );
  private static final Image clearImg = VueResources.getImage(
    "search.closeicon"
  );
  private static final Image searchImg = VueResources.getImage(
    "search.searchicon"
  );
  private static final Image searchOVImg = VueResources.getImage(
    "search.searchicon.ov"
  );
  private static final Image searchTigerImg = VueResources.getImage(
    "search.tiger.searchicon"
  );
