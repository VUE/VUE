...
public class SakaiExport {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(SakaiExport.class);

  private static final org.osid.shared.Type _collectionAssetType =
    new edu.tufts.vue.util.Type("sakaiproject.org", "asset", "siteCollection");
  private static final org.osid.shared.Type _sakaiRepositoryType =
  ...
  public static boolean isSakaiSource(edu.tufts.vue.dsm.DataSource ds)
    ...
    final String rname =
      (repository == null ? "<null!>" : repository.getDisplayName());
    if (DEBUG.DR) Log.debug(
      "Examining data source: " + ds + "; repository=" + rname
    );
    ...
    if (ds.supportsUpdate() && repository != null) {
      ...
      if (DEBUG.DR) Log.info(
        "checking type " + repository.getType().getAuthority()
      );
  ...
  public java.util.List<edu.tufts.vue.dsm.DataSource> getSakaiDataSources()
    throws org.osid.repository.RepositoryException {
    final java.util.List<edu.tufts.vue.dsm.DataSource> sakaiSources =
      new java.util.ArrayList();

    for (edu.tufts.vue.dsm.DataSource ds : _dsm.getDataSources()) if (
      isSakaiSource(ds)
    ) sakaiSources.add(ds);
  ...
  public SakaiCollection[] getCollections(
    ...
    if (dataSource.supportsUpdate()) {
      ...
      org.osid.repository.AssetIterator assetIterator =
        repository.getAssetsByType(_collectionAssetType);
      while (assetIterator.hasNextAsset()) {
        collectionVector.addElement(
          new SakaiCollection(assetIterator.nextAsset())
        );
      }
      // convert to array for return
