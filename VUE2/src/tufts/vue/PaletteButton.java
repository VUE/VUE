...
public class PaletteButton extends JRadioButton {
  ...
  public void removePaletteItem(PaletteButtonItem pItem) {
    if (mItems != null) {
      ...
      boolean found = false;
      int slot = 0;
      if (len > 1) newItems = new PaletteButtonItem[len - 1];
  ...
  public void setPaletteButtonItems(PaletteButtonItem[] pItems) {
    if (DEBUG.INIT) System.out.println(
      this + " setPaletteButtonItems n=" + pItems.length
    );
    mItems = pItems;
    buildPalette();
  }
  ...
  protected void buildPalette() {
    ...
    int cols = 0;
    while (
      mColThreshold[cols] < numItems && cols < mColThreshold.length
    ) cols++;
    int rows = (numItems + (numItems % cols)) / cols;

    if (
      rows < 3 && GUI.isMacBrushedMetal() && VueUtil.getJavaVersion() < 1.5f
      /*&& tiger */
    ) { // total hack for now
   ...
   * the given trigger.
   */
  public static class PalettePopupMenuHandler
    ...
    public void mousePressed(MouseEvent e) {
      ...
      if (
        (now - mLastHidden > 100) && (mMenu.getComponentCount() > 1)
    ...
    public int getMenuX(Component trigger) {
      return 0;
    }

    /** get menu Y location relative to trigger: default is trigger height (places below trigger) */
    public int getMenuY(Component trigger) {
      return trigger.getHeight();
    }
  ...
  public void setPropertiesFromItem(AbstractButton pItem) {
    ...
    if (pItem.getToolTipText() != null) this.setToolTipText(
        pItem.getToolTipText()
      );
  ...
  public void paint(java.awt.Graphics g) {
    ...
    } else if ((mPopup != null) && (!mPopup.isVisible())) { // Use provided popup overlay  icons
  ...
  public class PBPopupMenu extends JPopupMenu {
    ...
    public PBPopupMenu(int rows, int cols) {
      ...
      grid.setHgap(0);
      if (DEBUG.INIT) System.out.println(
        "*** CREATED GRID LAYOUT " + grid.getRows() + "x" + grid.getColumns()
      );
      setLayout(grid);
    }
    ...
    public void menuSelectionChanged(boolean isIncluded) {
      if (DEBUG.Enabled) System.out.println(
        this + " menuSelectionChanged included=" + isIncluded
      );
