...
public class MapTypeDialog
  ...
  private void initializeComponents() {
    ...
    JLabel listTitle = new JLabel(
      VueResources.getString("list.usertype.label")
    );
    Box listControls = Box.createHorizontalBox();
    mAddTypeButton =
      createButton(VueResources.getString("button.newtype.label"));
    mRemoveTypeButton =
      createButton(VueResources.getString("button.removetype.label"));
    ...
    mAddPropertyButton =
      createButton(VueResources.getString("button.add.label"));
    mRemovePropertyButton =
      createButton(VueResources.getString("button.remove.label"));
    mUpButton = createButton(VueResources.getString("button.nudgeup.label"));
    mDownButton =
      createButton(VueResources.getString("button.nudgedown.label"));
    ...
    tablePanel.setVerticalScrollBarPolicy(
      JScrollPane.VERTICAL_SCROLLBAR_ALWAYS
    );
    tablePanel.setHorizontalScrollBarPolicy(
      JScrollPane.HORIZONTAL_SCROLLBAR_NEVER
    );
  ...
  private void initPropertyTable() {
    String[] columns = {
      VueResources.getString("column.property.name"),
      VueResources.getString("column.type.name"),
      VueResources.getString("column.value.name"),
    };
  ...
  public JButton createButton(String pText) {
    JButton button = null;
    if (pText == null) button = new JButton(); else button = new JButton(pText);
  ...
  public void doAddType() {
    ...
    name =
      (String) VueUtil.input(VueResources.getString("dialog.newuser.title"));
    if (name != null) {
  ...
  public void doAddProperty() {
    ...
    if (name != null) {
      ...
      if (mCurType != null) mCurType.addUserProperty(prop);
  ...
  public class PropertyTableModel extends AbstractTableModel {
    ...
    public int getRowCount() {
      ...
      if (mModelType != null) {
        ...
        if (names != null) rows = names.length;
      }
      return rows;
    }
    ...
    public Object getValueAt(int row, int column) {
      ...
      if (mModelType != null) {
        ...
        if (column == 0) value = rows[row].getDisplayName();
        if (column == 1) {
