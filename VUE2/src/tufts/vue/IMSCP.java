...
public class IMSCP {
  ...
  public void putEntry(String entryName, File file)
    throws ZipException, IOException {
    byte[] buf = new byte[1024]; // buffer for reading file
    int len;
    if (zos == null) throw new ZipException(
      "IMSCP.putEntry(File file): ZipOutputstream not initialized"
    );
    try {
      ...
      if (
        !ex.getMessage().startsWith("duplicate")
      ) throw ex; // ignoring duplicate entries
    }
  }
