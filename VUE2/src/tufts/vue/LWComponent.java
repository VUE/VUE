...
public class LWComponent implements VueConstants, XMLUnmarshalListener {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWComponent.class);
  ...
  public enum HideCause {
    ...
    DEFAULT/** we've been hidden by link pruning */
    ,

    PRUNE/** another layer is set to be the exclusive layer */ // (CAUSE_PERSIST),
    ,

    LAYER_EXCLUDED/** we're a member of a pathway that hides when the pathway hides, and all pathways we're on are hidden */
    ,
    ...
    COLLAPSED/** we're an LWImage that's a node-icon image, and we're hidden */
    ,
  ...
  public enum Flag {
    ...
    COLLAPSED/** for links: this is data-relation link */
    ,

    DATA_LINK/** for links: this is data-relation link and ALSO a data-count link */
    ,

    DATA_COUNT/** currently used for marking LWImage's as being node-icons */
    ,

    ICON/** for subclasses that want to distinguish between a default size and a validated size (e.g., LWImage)
     * "default size" could actually mean any suggested or invalid size before a final definite size */
    ,

    UNSIZED/** lets us know this is in the process of duplicating */
    ,

    DUPLICATING/** this is part of a pruned map sub-graph */
    ,

    PRUNED/** this is link with head pruned */
    ,

    PRUNE_HEAD/** this is link with tail pruned */
    ,
  ...
  public enum State {
    ...
    HAS_AUTO_CLUSTERED/** a map/layer has been auto-clustered by a data-drop */
    ,
  ...
  public static final java.util.List<LWComponent> NO_CHILDREN =
  ...
  static void toggleGlobalCollapsed() {
    if (!COLLAPSE_IS_GLOBAL) throw new Error("disabled");
  ...
  private final transient AffineTransform _zeroTransform =
    new AffineTransform();
  ...
  protected final transient LWChangeSupport mChangeSupport =
    new LWChangeSupport(this);
  ...
  public static final Comparator GridSorter = new Comparator<LWComponent>() {
    public int compare(LWComponent c1, LWComponent c2) {
      if (c1.y == c2.y) return XSorter.compare(
        c1,
        c2
      ); else return YSorter.compare(c1, c2);
    }
  };
  ...
  public LWComponent() {
    ...
    if (mSupportedPropertyKeys == 0) {
      ...
      if (DEBUG.INIT || DEBUG.STYLE) Util.printStackTrace(
        "ZERO PROPERTY BITS IN " + Util.tag(this)
      );
    } else {
      // not on by default:
  ...
  public boolean supportsProperty(Object key) {
    if (key instanceof Key) return supportsProperty(
      (Key) key
    ); else return false;
  }
  ...
  protected void disablePropertyTypes(KeyType type) {
    for (Key key : Key.AllKeys) if (
      key.type == type ||
      (type == KeyType.STYLE && key.type == KeyType.SUB_STYLE)
    ) disableProperty(key);
  }
  ...
  public void copyStyle(LWComponent styleSource, long permittedPropertyBits) {
    if (DEBUG.STYLE || styleSource == null) {
      System.out.println(
        "COPY STYLE of " +
        Util.tags(styleSource) +
        " ==>> " +
        Util.tags(this) +
        " permitBits=" +
        Long.bitCount(permittedPropertyBits)
      );
    }
    if (styleSource == null) return;
    for (Key key : Key.AllKeys) //if (key.isStyleProperty && styleSource.supportsProperty(key) && (permittedPropertyBits & key.bit) != 0)
    if (
      styleSource.isStyling(key) && (permittedPropertyBits & key.bit) != 0
  ...
  public void copyProperties(LWComponent source, long propertyBits) {
    if (DEBUG.STYLE) System.out.println(
      ...
      this +
      " bits=" +
      Long.bitCount(propertyBits)
    );
    for (Key key : Key.AllKeys) if (
      (propertyBits & key.bit) != 0 && source.supportsProperty(key)
  ...
  public void applyCSS(edu.tufts.vue.style.Style cssStyle) {
    ...
    for (Map.Entry<String, String> se : cssStyle.getAttributes().entrySet()) {
      ...
      if (!applied) System.err.println("UNHANDLED");
    }
  }
  ...
  public static class Key<TSubclass extends LWComponent, TValue> {
    ...
    private static final java.util.Map<Class, Long> ClassProperties =
    ...
    private static long PartialPropertyMaskForClass(Class clazz) {
      final Long bitsForClass = ClassProperties.get(clazz); // property bits for this class
      if (bitsForClass == null) return 0L; else return bitsForClass;
    }
    ...
    protected Key(String name, String cssName, KeyType keyType) {
      ...
      if (InstanceCount >= Long.SIZE) {
        ...
        tufts.Util.printStackTrace(
          ...
          ": " +
          InstanceCount +
          "th key created -- need to re-implement (try BitSet)"
        );
      } else this.bit = 1 << InstanceCount;
      AllKeys.add(this);
      ...
      if (DEBUG.INIT || DEBUG.STYLE) Log.debug(
        String.format(
          "KEY %-20s %-11s %-22s bit#%2d; %25s now has %2d properties",
          name,
      ...
      try {
        if (propertySlot == NO_SLOT_PROVIDED) {
          if (DEBUG.META) Log.error(
            ...
            c == null
              ? null
              : c.getClass() + "; has no property of type: " + this
          );
          return null;
        } else return (TValue) propertySlot.get();
      } catch (Throwable t) {
        if (DEBUG.META) tufts.Util.printStackTrace(
          new Throwable(t),
          this + ": property slot get() failed " + propertySlot
        ); else Log.warn(
          this + ": property slot get() failed " + propertySlot + " " + t
        );
    ...
    private final void setValueBySlot(TSubclass c, TValue value) {
      ...
      if (slot == null || slot == NO_SLOT_PROVIDED) return;
    ...
    private Property getSlotSafely(TSubclass c) {
      ...
      try {
        ...
        tufts.Util.printStackTrace(
          new Throwable(t),
          this + ": bad slot? unimplemented get/setValue?"
        );
        return null;
      }
    ...
    String getStringValue(TSubclass c) {
      ...
      if (slot == NO_SLOT_PROVIDED || slot == null) {
        ...
        try {
          ...
          final String msg =
            "Property not supported(getStringValue): " + this + " on\t" + c;
          if (DEBUG.META) tufts.Util.printStackTrace(e, msg); else Log.warn(
            msg + "; " + e
          );
    ...
    void setStringValue(TSubclass c, String stringValue) {
      ...
      if (slot != NO_SLOT_PROVIDED) {
        ...
        if (v instanceof String) stringSet(c, stringValue); else if (
          v instanceof Integer
        ) stringSet(c, Integer.valueOf(stringValue)); else if (
          v instanceof Long
        ) stringSet(c, Long.valueOf(stringValue)); else if (
          v instanceof Float
        ) stringSet(c, Float.valueOf(stringValue)); else if (
          v instanceof Double
        ) stringSet(c, Double.valueOf(stringValue)); else if (
          v instanceof Boolean
        ) stringSet(c, Boolean.valueOf(stringValue)); else Log.error(
          this +
          ":setValue(" +
          stringValue +
          "); no slot provided for parsing string value",
          new Throwable("HERE")
        );
      }
    }
    ...
    boolean setValueFromCSS(TSubclass c, String cssKey, String cssValue) {
      ...
      if (slot == Key.NO_SLOT_PROVIDED) {
        c.out(
          "Can't auto-apply CSS Style property to non-slotted key: " +
          cssName +
          " -> " +
          this
        );
    ...
    boolean valueEquals(TSubclass c, Object otherValue) {
      ...
      return (
        value == otherValue || (otherValue != null && otherValue.equals(value))
      );
    }

    void copyValue(TSubclass source, TSubclass target) {
      if (!source.supportsProperty(this)) {
        if (DEBUG.STYLE && DEBUG.META) System.err.println(
          "   COPY-VALUE: " +
          this +
          "; source doesn't support this property; " +
          source
        );
        ...
        if (DEBUG.STYLE && DEBUG.META) System.err.println(
          "   COPY-VALUE: " +
          this +
          "; target doesn't support this property; " +
          target
        );
      } else {
        ...
        if (
          ...
          if (DEBUG.STYLE) System.out.format(
            "    COPY-VALUE: %s %s%-15s%s %-40s -> %s over (%s)\n",
            source,
            TERM_PURPLE,
            this.name,
            TERM_CLEAR,
  ...
  protected abstract class Property<T> {
    ...
    void take(T o) {
      this.value = o;
      if (DEBUG.TOOL) System.out.printf(
        "     TAKING: %-30s -> %s\n",
        vtag(key, o, this),
        LWComponent.this
      );
    ...
    void setFromCSS(String cssKey, String value) {
      throw new UnsupportedOperationException(
        this + " unimplemented setFromCSS " + cssKey + " = " + value
      );
  ...
  public abstract class NumberProperty<T> extends Property<T> {
    ...
    void setFromCSS(String cssKey, String value) {
      if (value.endsWith("pt") || value.endsWith("px")) setBy(
        value.substring(0, value.length() - 2)
      ); else throw new IllegalArgumentException(
        "unhandled CSS number conversion for [" + value + "]"
      );
    }
  }
  ...
  public class FontProperty extends Property<java.awt.Font> {
    ...
    final String asString() {
      ...
      if (LWComponent.this.mFontUnderline.get().equals("underline")) strStyle =
        strStyle.concat("underline");
  ...
  public class CSSFontStyleProperty extends IntProperty {
    ...
    void setFromCSS(String cssKey, String value) {
      ...
      if ("italic".equalsIgnoreCase(value)) set(java.awt.Font.ITALIC); else if (
        "bold".equalsIgnoreCase(value)
      ) set(java.awt.Font.BOLD); else set(0);
    }
  }
  ...
  public class ColorProperty extends Property<java.awt.Color> {
    ...
    void setAllowAlpha(boolean allow) {
      if (allow) fixedAlpha = NO_ALPHA_SET; else fixedAlpha =
        ALPHA_NOT_PERMITTED;
    }
    ...
    void take(Color c) {
      if (
        fixedAlpha < NO_ALPHA_SET && (c == null || c.getAlpha() != 0xFF)
      ) throw new PropertyValueVeto(
        key +
        "; color with translucence: " +
        c +
        " alpha=" +
        c.getAlpha() +
        " not allowed on " +
  ...
  private static String ColorToDebugString(final Color c) {
    ...
    if (s == null) return "#null00"; else return s;
  }
  ...
  public static String ColorToString(final Color c) {
    ...
    if (
      c.getAlpha() == 255
    ) return String.format( // opaque: only bother to save hue info
      "#%06X",
      c.getRGB() & 0xFFFFFF
    ); else return String.format("#%08X", c.getRGB());
  }
  ...
  public static final Key KEY_StrokeColor = new Key(
    "stroke.color",
    "border-color"
  ) {
    final Property getSlot(LWComponent c) {
  ...
  public static final Key KEY_StrokeWidth = new Key(
    "stroke.width",
    "stroke-width"
  ) {
    final Property getSlot(LWComponent c) {
  ...
  public static final Key KEY_FontSize = new Key(
    "font.size",
    KeyType.SUB_STYLE
  ) {
    final Property getSlot(LWComponent c) {
  ...
  public static final Key KEY_FontStyle = new Key(
    "font.style",
    KeyType.SUB_STYLE
  ) {
    final Property getSlot(LWComponent c) {
  ...
  public static final Key KEY_FontUnderline = new Key(
    "font.underline",
    KeyType.SUB_STYLE
  ) {
    final Property getSlot(LWComponent c) {
  ...
  public static final Key KEY_FontName = new Key(
    "font.name",
    KeyType.SUB_STYLE
  ) {
    final Property getSlot(LWComponent c) {
  ...
  public static final Key KEY_Collapsed = new Key<LWComponent, Boolean>(
    "collapsed"
  ) {
    @Override
  ...
  public final ColorProperty mTextColor = new ColorProperty(
    KEY_TextColor,
    java.awt.Color.black
  ) {
    //{ color = java.awt.Color.black; } // default value
    void onChange() {
      if (labelBox != null) labelBox.copyStyle(LWComponent.this); // todo better: handle thru style.textColor notification?
    }
  };
  public final ColorProperty mStrokeColor = new ColorProperty(
    KEY_StrokeColor,
  ...
  public final EnumProperty<Alignment> mAlignment = new EnumProperty(
    KEY_Alignment,
    Alignment.LEFT
  ) {
    void onChange() {
  ...
  public final EnumProperty<StrokeStyle> mStrokeStyle = new EnumProperty(
    KEY_StrokeStyle,
    StrokeStyle.SOLID
  ) {
    void onChange() {
  ...
  public enum StrokeStyle {
    ...
    public BasicStroke makeStroke(float width) {
      if (this == SOLID) return new BasicStroke(
        width,
      ...
      else return new BasicStroke(
        width,
        BasicStroke.CAP_BUTT, // anything else will mess with the dash pattern
        BasicStroke.JOIN_BEVEL,
        10f//, 0f // miter-limit // miter-limit
        ,
        dashPattern,
        0f
      ); // dash-phase (offset to start of pattern -- apparently pixels, not index)
    }
  ...
  private void rebuildStroke() {
    ...
    if (width > 0) this.stroke =
      mStrokeStyle.get().makeStroke(width); else this.stroke = STROKE_ZERO;
  ...
  public final IntProperty mFontStyle = new CSSFontStyleProperty(
    KEY_FontStyle
  ) {
    void onChange() {
      rebuildFont();
  ...
  public final StringProperty mFontName = new CSSFontFamilyProperty(
    KEY_FontName
  ) {
    void onChange() {
      rebuildFont();
  ...
  public final StringProperty mFontUnderline = new StringProperty(
    KEY_FontUnderline
  ) {
  ...
  public static final Key KEY_Label = new Key<LWComponent, String>(
    "label",
    KeyType.DATA
  ) {
    @Override
    public void setValueWithContext(LWComponent c, String val, Object context) {
      if (context == PROPERTY_SET_UNDO) c.setLabelImpl(
        val,
        true,
        false
      ); else c.setLabel(val);
  ...
  public static final Key KEY_Notes = new Key<LWComponent, String>(
    "notes",
    KeyType.DATA
  ) {
    @Override
  ...
  public void setClientData(Object key, Object o) {
    if (mClientData == null) {
      if (
        o == null
      ) return; else mClientData = new HashMap(2); // storing null means remove value
    }
    ...
    if (o == null) {
      ...
      if (mClientData.size() == 0) mClientData = null;
    } else {
  ...
  public <A> A getClientData(Class<A> classKey, String subKey) {
    ...
    if (classKey == Boolean.class && o == null) return (A) Boolean.FALSE; // special hack for null == boolean false
    else return (A) o;
  }
  ...
  public Object getPropertyValue(final Object key) {
    if (key instanceof Key) {
      ...
      try {
        ...
        String msg =
          "Property not supported(getPropertyValue): " +
          key +
          " on " +
          this +
          " (returned null)";
    ...
    Log.warn(
      this +
      " getPropertyValue; unsupported property [" +
      key +
      "] (returning null)"
    );
  ...
  protected void setPropertyImpl(
    final Object key,
    final Object val,
    final Object context
  ) {
    if (DEBUG.TOOL || DEBUG.UNDO) Log.debug(
      "setPropertyImpl[" + context + "] " + this + " " + vtag(key, val, null)
    );
    ...
    else if (key == KEY_LabelFormat) setLabelFormat((String) val); else if (
      key == LWKey.DataUpdate
    ) setDataMap((MetaMap) val); else if (key == LWKey.Scale) setScale(
      (Double) val
    ); else if (key == LWKey.Resource) setResource((Resource) val); else if (
      key == LWKey.Location
    ) {
  ...
  public static class LinkPatcher {
    ...
    private java.util.Map<LWComponent, LWComponent> mCopies =
      new java.util.IdentityHashMap();
    private java.util.Map<LWComponent, LWComponent> mOriginals =
      new java.util.IdentityHashMap();
    ...
    public void reconnectLinks() {
      ...
      for (LWComponent c : mCopies.values()) {
        ...
        if (DEBUG.DND) Log.debug(
          "LinkPatcher: reconnecting " +
          linkCopy +
          " endpoints:" +
          "\n\t" +
          headCopy +
  ...
  public static class CopyContext {
    ...
    void reset() {
      if (patcher != null) patcher.reset();
    }

    void complete() {
      if (patcher != null) patcher.reconnectLinks();
    }
  }
  ...
  protected <Ts extends LWComponent> Ts duplicateTo(Ts c, CopyContext cc) {
    ...
    if (c instanceof LWText) {
      c.label = this.label;
      ((LWText) c).getRichLabelBox()
        .setText(((LWText) this).getRichLabelBox().getRichText());
    } else {
      c.setLabelImpl(this.label, true, false);
      c.getLabelBox().setSize(getLabelBox().getSize());
    }

    if (hasResource()) c.setResource(getResource());
    if (hasNotes()) c.setNotes(getNotes());

    if (cc.patcher != null) cc.patcher.track(this, c);
  ...
  protected void copyMetaData(final LWComponent source) {
    if (source.metadataList != null) {
      ...
      final List<VueMetadataElement> srcVMD = source
        .getMetadataList()
        .getMetadata();
      final List<VueMetadataElement> targetVMD =
        this.getMetadataList().getMetadata();
      for (VueMetadataElement vme : srcVMD) targetVMD.add(vme);
    }
    // duplicate data-set data
    if (source.mDataMap != null) mDataMap = source.mDataMap.clone();
  }

  protected boolean isPresentationContext() {
    if (true) return false; // turned off for now
    if (parent == null) return false; // this means presentation nodes will report wrong sizes during restores...
    else return parent.isPresentationContext();
  }
  ...
  protected String getNextUniqueID() {
    if (getParent() == null) {
      ...
      if (DEBUG.PARENTING) out(
        "getNextUniqueID: returning null for current orphan"
      );
      return null;
    } else return getParent().getNextUniqueID();
  }
  ...
  public UndoManager getUndoManager() {
    ...
    if (map == null) return null; else return map.getUndoManager();
  }
    ...
    if (um != null) {
      if (um.isUndoing()) {
        if (DEBUG.WORK || DEBUG.UNDO) System.out.println(
          "Ignoring cleanup task during undo: " + task + " for " + this
        );
      } else if (um.hasCleanupTask(taskKey)) {
        if (DEBUG.WORK || DEBUG.UNDO) System.out.println(
          "Ignoring duplicate cleanup task: " + task + " for " + this
        );
      } else {
  ...
  public MetaMap getPersistDataMap() {
    if (mXMLRestoreUnderway) return getDataMap(); else return mDataMap; // if null on save, nothing to persist
  }
  ...
  public Field getDataValueField() {
    if (mDataMap == null || mDataMap.getSchema() == null) return null;

    String fieldName = getDataValueFieldName();

    if (fieldName != null) return mDataMap
      .getSchema()
      .getField(fieldName); else return null;
  }
  ...
  public boolean isDataValueNode(Field field) {
    return (
      getDataSchema() == field.getSchema() &&
      field.getName().equals(getDataValueFieldName())
    );
  }
  ...
  public Schema getDataSchema() {
    if (mDataMap != null) return mDataMap.getSchema(); else return null;
  }
  ...
  public MetadataList getMetadataList() {
    if (metadataList == null) metadataList = new MetadataList();
    return metadataList;
  }
  ...
  public MetadataList getXMLmetadataList() {
    if (mXMLRestoreUnderway) {
      ...
      // persist underway:
      if (
        metadataList != null && metadataList.isEmpty()
      ) return null; else return metadataList;
    }
  }

  /** see edu.tufts.vue.metadata.VueMetadataElement for metadata types **/
  public boolean hasMetaData(int type) {
    if (metadataList != null) return metadataList.hasMetadata(
      type
    ); else return false;
  ...
  public String getMetaDataAsHTML() {
    return getMetaDataAsHTML(
      edu.tufts.vue.metadata.VueMetadataElement.CATEGORY
    );
  }

  /** see edu.tufts.vue.metadata.VueMetadataElement for metadata types */
  public String getMetaDataAsHTML(int type) {
    if (metadataList != null) return metadataList.getMetadataAsHTML(
      type
    ); else return "";
  }
  ...
  public void setID(String ID) {
    if (this.ID != null) throw new IllegalStateException(
      "Can't set ID to [" + ID + "], already set on " + this
    );
  ...
  public void setLabelFormat(String s) {
    if (s == mLabelFormat || (s != null && s.equals(mLabelFormat))) return;
  ...
  protected void setLabelImpl(
    ...
    if (this.label != null && this.label.equals(newLabel)) return;

    Object old = this.label;

    if (newLabel == null || newLabel.length() == 0) {
      this.label = null;
      if (labelBox != null) labelBox.setText("");
    } else {
      this.label = newLabel;
      if (DEBUG.TEXT || DEBUG.DATA) out(
        "setLabelImpl textSet " + Util.tags(newLabel)
      );
      ...
      if (this.labelBox == null) {
        ...
        try {
          ...
          Log.error(
            String.format(
              "failed to set label '%s' on %s in %s",
              newLabel,
              Util.tags(getLabelBox()),
              this
            ),
            t
          );
  ...
  public void wrapLabelToWidth(final int charWidth) {
    ...
    if (wrappedLabel != existingLabel) {
      if (DEBUG.Enabled) Log.debug(
        this + " wrapped to: " + Util.tags(wrappedLabel)
      );
      setLabelImpl(wrappedLabel, true, false);
    }
  }

  private static final int MaxLabelLineLength = VueResources.getInt(
    "dataNode.labelLength",
    50
  );
  ...
  private String fillLabelFormat(final String fmt) {
    ...
    for (String s : parts) {
      ...
      try {
        ...
        Log.error(
          "exception processing replacement braces " +
          Util.tags(s) +
          " in format " +
          Util.tags(fmt) +
          " for " +
      ...
      if (noValueFound) {
        // leave un-touched
        if (part > 0) buf.append('$'); // restore the '$' we split on
        buf.append(s);
      }
      part++;
    }
    ...
    if (DEBUG.TEXT || DEBUG.DATA) {
      if (anyReplacement) Log.debug(
        this +
        " FILL made from " +
        Util.tags(fmt) +
        "->" +
        Util.tags(buf.toString())
      ); else Log.debug(
        this + " FILL; NO REPLACEMENTS MADE in " + Util.tags(fmt)
      );
    }
  ...
  private String findLabelFormatDataValue(String key) {
    ...
    if (value == null && hasResource()) value = getResource().getProperty(key);
  ...
  protected tufts.vue.TextBox getLabelBox() {
    try {
      if (this.labelBox == null) {
        synchronized (this) {
          if (this.labelBox == null) this.labelBox =
            new tufts.vue.TextBox(this, this.label);
        }
      }
    } catch (Throwable t) {
  ...
  public void setNotes(String pNotes) {
    ...
    if (pNotes == null) {
      ...
      if (trimmed.length() > 0) this.notes = pNotes; else this.notes = null;
    }
    layout();
  ...
  public Resource.Factory getResourceFactory() {
    ...
    if (
      map == null
    ) return Resource.getFactory(); else return map.getResourceFactory();
  }
  ...
  public String getDisplayLabel() {
    ...
    } else return getUniqueComponentTypeLabel();
  }
  ...
  public String getDiagnosticLabel() {
    if (hasLabel()) {
      return (
        getUniqueComponentTypeLabel() + ": " + getLabel().replace('\n', ' ')
      );
    } else return getUniqueComponentTypeLabel();
  }
  ...
  public String getComponentTypeLabel() {
    ...
    if (name.startsWith("tufts.vue.LW")) return name.substring(12); else if (
      name.startsWith("tufts.vue.")
    ) return name.substring(10); else return name;
  }

  String toName() {
    if (getLabel() == null) return getDisplayLabel(); else return (
      getComponentTypeLabel() + "[" + getLabel() + "]"
    );
  }
  ...
  public NodeFilter getXMLnodeFilter() {
    if (mXMLRestoreUnderway) {
      ...
      nodeFilter == NEEDS_NODE_FILTER ||
      (nodeFilter != null && nodeFilter.size() < 1)
    ) {
      return null;
    } else return nodeFilter;
  }
  ...
  public boolean canLinkTo(LWComponent target) {
    return (
      canLinkToImpl(target) && (target == null || target.canLinkToImpl(this))
    );
  }
  ...
  public boolean inVisiblePathway() {
    if (inPathway()) for (LWPathway p : mPathways) if (p.isDrawn()) return true;
    return false;
  }
  ...
  public boolean inPathway(LWPathway path) {
    if (mPathways == null || path == null) return false;

    for (LWPathway p : mPathways) if (p == path) return true;
  ...
  public boolean inDrawnPathway() {
    if (mPathways == null) return false;

    for (LWPathway p : mPathways) if (
      p.isVisible() && !p.isRevealer()
    ) return true;
  ...
  protected void addEntryRef(LWPathway.Entry e) {
    ...
    if (!mEntries.contains(e)) mEntries.add(e);
  ...
  protected void removeEntryRef(LWPathway.Entry e) {
    ...
    if (!mEntries.remove(e)) Util.printStackTrace(
      this + "; Warning: didn't contain entry " + e
    );
    removePathwayRef(e.pathway);
  }

  private void addPathwayRef(LWPathway p) {
    if (mPathways == null) mPathways = new ArrayList();
    if (!mPathways.contains(p)) {
      ...
      if (
        !hasFlag(Flag.UNDELETING) && LWIcon.IconPref.getPathwayIconValue()
      ) layout();
    }
    //notify("pathway.add");
  }
  ...
  private void removePathwayRef(LWPathway p) {
    if (mPathways == null) {
      if (DEBUG.META) tufts.Util.printStackTrace(
        "attempt to remove non-existent pathwayRef to " + p + " in " + this
      );
      return;
    }
    mPathways.remove(p);
    ...
    for (HideCause cause : HideCause.values()) if (
      cause.type == CAUSE_PATHWAY
    ) clearHidden(cause);
  ...
  public final void layout() {
    if (mXMLRestoreUnderway == false) layout(LAYOUT_DEFAULT);
  }
  ...
  public Color getFinalFillColor(DrawContext dc) {
    if (mFillColor.isTransparent()) {
      ...
      if (getParent() != null) return getParent()
        .getFinalFillColor(dc); else if (
        dc != null
      ) return dc.getBackgroundFill(); else return null;
    } else return getFillColor();
  }

  public static Color getContrastColor(Color c) {
    if (c != null) {
      if (c.equals(Color.black)) return Color.darkGray; else return c.darker();
    } else {
  ...
  public Color getPriorityPathwayColor(DrawContext dc) {
    ...
    if (exclusive != null) return exclusive.getColor(); else if (
      inPathway(VUE.getActivePathway()) && VUE.getActivePathway().isDrawn()
    ) return VUE.getActivePathway().getColor(); else return null;
  ...
  public LWMap.Layer getLayer() {
    if (parent == null) return null; else if (
      parent instanceof LWMap.Layer
    ) return (LWMap.Layer) parent; else return parent.getLayer();
  }
  ...
  public LWMap.Layer getPersistLayer() {
    if (
      parent instanceof LWMap.Layer
    ) return (LWMap.Layer) parent; else return null;
  }
  ...
  public int getDepth() {
    if (parent == null) return 0; else return parent.getDepth() + 1;
  }

  public int getIndex() {
    if (parent == null) return -1; else return parent.indexOf(this);
  }

  protected void setParent(LWContainer newParent) {
    if (DEBUG.UNDO) System.err.println(
      "*** SET-PARENT: " + newParent + " for " + this
    );
    ...
    if (newParent.hasAncestor(this)) {
      Util.printStackTrace(
        "ATTEMPTED PARENT LOOP " +
        this +
        " can't make a child our parent: " +
        newParent
      );
      return;
    }
  ...
  protected void addSyncClient(LWComponent c) {
    if (mSyncClients == null) mSyncClients = new HashSet();
    mSyncClients.add(c);
  }
  ...
  public void setStyle(LWComponent parentStyle) {
    ...
    if (parentStyle != null) {
      ...
      if (
        !mXMLRestoreUnderway
      ) copyStyle(parentStyle); // we can skip the copy during restore
    }
  }
  ...
  /** @deprecated: tmp back compat only */public void setParentStyle(
    LWComponent c
  ) {
    setStyle(c);
  }

  /** @deprecated: tmp back compat only */public Boolean getPersistIsStyleParent() {
    return null;
  }

  /** @deprecated: tmp back compat only */public void setPersistIsStyleParent(
    Boolean b
  ) {
    setPersistIsStyle(b);
  }

  /** @deprecated: tmp back compat only */public LWComponent getParentStyle() {
    return null;
  }
  ...
  protected LWComponent defaultDropTarget(PickContext pc) {
    ...
    if (
      pc.dropping instanceof LWComponent
    ) return getParent(); else return this;
  }
  ...
  public boolean isManagedLocation() {
    return (
      (parent != null && parent.isManagingChildLocations()) ||
  ...
  public void addChildren(
    Collection<? extends LWComponent> children,
    Object context
  ) {
    Util.printStackTrace(
      this +
      ": can't take children; ignored: " +
      Util.tags(children) +
      "; context=" +
      context
    );
  ...
  private final class SlideIconIter
    implements Iterator<LWSlide>, Iterable<LWSlide> {
    ...
    public LWSlide next() {
      if (nextSlide == null) {
        if (DEBUG.Enabled) Util.printStackTrace(
          this + "; next at end of SlideIconIter; entries=" + mEntries
        );
        return null;
      }
  ...
  private final Iterable<LWSlide> seenSlideIcons(DrawContext dc) {
    ...
    if (mEntries.size() == 1) {
      ...
      if (e.hasVisibleSlide()) return Util.iterable(
        e.getSlide()
      ); else return Util.EmptyIterable;
    } else {
   ...
   */
  public List<LWComponent> getPickList(
    PickContext pc,
    List<LWComponent> stored
  ) {
    Iterable<LWSlide> seenSlides = null;
    if (
      pc.root != this &&
      hasEntries() &&
      (seenSlides = seenSlideIcons(pc.dc)) != Util.EmptyIterable
    ) {
      // todo performance: see LWTraversal for comments: change impl to return a ReverseListIterator, etc.a
      stored.clear();
      if (!isCollapsed()) stored.addAll(getChildren());
      for (LWSlide s : seenSlides) stored.add(s);
      return stored;
    } else return (List) getChildren();
  }
  ...
  public Collection<LWComponent> getAllDescendents(final ChildKind kind) {
    if (
      kind == ChildKind.PROPER
    ) return java.util.Collections.EMPTY_LIST; else return getAllDescendents(
      kind,
  ...
  public final Collection<LWComponent> getAllDescendents(
  ...
  public <A extends LWComponent> Iterable<A> getDescendentsOfType(
    Class<A> clazz
  ) {
    return getDescendentsOfType(ChildKind.PROPER, clazz);
  }
  ...
  void removeLinkRef(LWLink link) {
    ...
    if (mLinks == null || !mLinks.remove(link)) Log.warn(
      "removeLinkRef: " + this + " didn't contain " + link
    );
  ...
  public List<LWLink> getIncomingLinks() {
    ...
    for (LWLink link : getLinks()) {
      ...
      boolean arrowHead = arrowState == LWLink.ARROW_HEAD, arrowTail =
        arrowState == LWLink.ARROW_TAIL, arrowNonDirectional =
        arrowState == LWLink.ARROW_BOTH || arrowState == LWLink.ARROW_NONE;
  ...
  public Collection<LWComponent> getLinked(
    Collection<LWLink> links,
    Collection<LWComponent> bag
  ) {
    for (LWLink link : links) {
      final LWComponent head = link.getHead();
      final LWComponent tail = link.getTail();
      if (head == tail); // ignore circular links
      else if (head == this && tail != null) bag.add(tail); else if (
        tail == this && head != null
      ) bag.add(head);
    }
    return bag;
  ...
  public Collection<LWComponent> getClustered() {
    ...
    int dcl = 0;
    for (LWLink l : links) if (l.isDataCountLink()) dcl++;

    if (dcl == 0 || dcl == links.size()) return getLinked(
      links,
      new HashSet(links.size())
    ); else return getLinked(
      getPriorityDataLinks(getLinks()),
      new HashSet(dcl)
    );
  }

  /** @return list will only contain LWLink */
  private static Collection<LWLink> getPriorityDataLinks(
    Collection<LWLink> links
  ) {
    ...
    for (LWLink l : links) {
      if (l.isDataCountLink()); //countDataLinks.add(l);
      else normalDataLinks.add(l);
    }
   ...
   */
  public final Collection<LWComponent> getLinkChain(
    Collection bag,
    LWComponent backstop
  ) {
  ...
  private Collection<LWComponent> getLinkChainImpl(
    Collection bag,
    LWComponent backstop,
    int depth
  ) {
  ...
  public Rectangle2D.Float getFanBounds(Rectangle2D.Float rect) {
    if (rect == null) rect = getMapBounds(); else rect.setRect(getMapBounds());
    ...
    for (LWLink link : getLinks()) {
      ...
      if (head != this) {
        if (head != null) rect.add(head.getPaintBounds());
      } else if (tail != this) {
        if (tail != null) rect.add(tail.getPaintBounds());
      }
    }
    return rect;
  }
...
*/
  ...
  public int countLinksTo(LWComponent c) {
    if (c == null || mLinks == null) return 0;

    int count = 0;
    for (LWLink link : mLinks) if (link.hasEndpoint(c)) count++;
    return count;
  }

  /** @return true if there are any links between us and the given component */
  public boolean hasLinkTo(LWComponent c) {
    if (c == null || mLinks == null) return false;

    for (LWLink link : mLinks) if (link.hasEndpoint(c)) return true;
    return false;
  }
  ...
  public boolean hasDirectedLinkTo(LWComponent c) {
    ...
    for (LWLink link : mLinks) {
      ...
      if (link.hasEndpoint(c) && tail.equals(c)) return true;
    }
    return false;
  }
  ...
  public int countCurvedLinksTo(LWComponent c) {
    int count = 0;
    for (LWLink link : getLinks()) if (
      link.hasEndpoint(c) && link.isCurved()
    ) count++;
    return count;
  }
  ...
  protected LWComponent getParentWithParent(LWContainer parent) {
    if (getParent() == parent) return this;
  ...
  public boolean hasAncestor(LWComponent c) {
    ...
    if (parent == null) return false; else if (
      c == parent
    ) return true; else return parent.hasAncestor(c);
  }
  ...
  public <T extends LWComponent> T getParentOfType(
    Class<T> clazz,
    LWComponent root
  ) {
    LWComponent parent = getParent();
    if (parent == null) return null; else return parent.getAncestorOfType(
      clazz,
      root
    );
  }
  ...
  /** never ascend above root */
  public <T extends LWComponent> T getAncestorOfType(
    Class<T> clazz,
    LWComponent root
  ) {
    if (clazz.isInstance(this)) return (T) this; else if (
      this == root
    ) return null; else return getParentOfType(clazz, root);
  }
  ...
  public LWComponent getTopMostAncestorOfType(Class clazz, LWComponent root) {
    ...
    if (nextAncestor != null) {
      for (;;) {
        ...
        if (nextAncestor != null) topAncestor = nextAncestor; else break;
  ...
  public double getMapScale() {
    if (getParent() == null) return getScale(); else return (
      getParent().getMapScale() * getScale()
    );
  }

  /** Convenience for returning float */public final float getScaleF() {
    return (float) getScale();
  }

  /** Convenience for returning float */public final float getMapScaleF() {
  ...
  protected void setLocation(
    float x,
    float y,
    LWComponent hearableEventSource,
    ...
    if (hearableEventSource != this) hearableEventSource.notifyProxy(
      new LWCEvent(hearableEventSource, this, LWKey.Location, oldValue)
    ); else notify(LWKey.Location, oldValue); //if (hearableEventSource != null) // if null, skip event delivery
    ...
    if (issueMapLocationChangeCalls) {
      ...
      final double scale;
      if (parent != null) scale = parent.getMapScale(); // we move within the scale of our parent
      else scale = 1.0;
      ...
      //if (!linkNotificationDisabled)
      if (updatingLinks()) updateConnectedLinks(this);
    }
  }
  ...
  protected void updateConnectedLinks(LWComponent movingSrc) {
    ...
    if (updatingLinks()) if (
      mLinks != null && mLinks.size() > 0
    ) for (LWLink link : mLinks) link.notifyEndpointMoved(movingSrc, this);
  }
  ...
  protected void notifyMapLocationChanged(
    LWComponent movingSrc,
    double mdx,
    double mdy
  ) {
    //if (!linkNotificationDisabled) // todo: if still end up using this feature, need to pass this bit on down to children
    if (updatingLinks()) updateConnectedLinks(movingSrc);
  }

  protected void notifyMapScaleChanged(
    double oldParentMapScale,
    double newParentMapScale
  ) {}
  ...
  public void notifyHierarchyChanged() {
    if (
      mLinks != null && mLinks.size() > 0
    ) for (LWLink link : mLinks) link.notifyEndpointHierarchyChanged(this);
  }
  ...
  protected void setSizeImpl(float w, float h, boolean internal) {
    ...
    if (!internal && !javax.swing.SwingUtilities.isEventDispatchThread()) {
      ...
      if (DEBUG.THREAD || DEBUG.UNDO || DEBUG.EVENTS) Log.info(
        "skipping undo on non-AWT size change: " +
        this +
        "; newSize=" +
        w +
        "x" +
    ...
    final Object old = skipUndo
      ? LWCEvent.NO_OLD_VALUE
    ...
    takeSize(w, h);
    if (isLaidOut()) getParent().layout();
    updateConnectedLinks(null);
    if (!quiet && isAutoSized()) notify(LWKey.Size, old); // technically only needed if is user-sized (otherwise layout code handles this)
  }

  public static Size ConstrainToAspect(double aspect, double w, double h) {
    if (DEBUG.IMAGE) Log.debug(
      "constrainToAspect " + aspect + " " + w + "x" + h
    );
  ...
  protected void getLinkConnectionCenterRelativeTo(
    Point2D.Float point,
    LWContainer relative
  ) {
    ...
    if (relative == this) {
      if (DEBUG.Enabled) Util.printStackTrace(
        "debug: " +
        this +
        " is computing link connetion center relative to itself"
      );
        ...
        if (relative != null && !hasAncestor(relative)) {
          ...
          if (getClass().getEnclosingClass() != LinkTool.LinkModeTool.class) {
            ...
            String msg =
              "non-ancestor: " +
              relative +
              " used as parent-relative of " +
              this;
            if (DEBUG.META) Util.printStackTrace(msg); else Log.debug(msg);
          }
        }
      }
  ...
  protected double getAncestorX(LWContainer ancestor) {
    if (
      ancestor == parent
    ) return getX(); else if (parent == null) { // quick check for the common case
      Util.printStackTrace("didn't find ancestor " + ancestor + " for " + this);
      return getX();
    } else return parent.getAncestorX(ancestor) + getX() * parent.getMapScale();
  }

  protected double getAncestorY(LWContainer ancestor) {
    if (
      ancestor == parent
    ) return getY(); else if (parent == null) { // quick check for the common case
      Util.printStackTrace("didn't find ancestor " + ancestor + " for " + this);
      return getY();
    } else return parent.getAncestorY(ancestor) + getY() * parent.getMapScale();
  }
  ...
  public Shape getZeroShape() {
    if (mZeroBounds == null) mZeroBounds = new Rectangle2D.Float();
  ...
  public Rectangle2D.Float getPaintBounds() {
    if (LWPathway.isShowingSlideIcons() && inDrawnPathway()) {
      Rectangle2D.Float b = addStrokeToBounds(
        getMapBounds(),
        LWPathway.PathBorderStrokeWidth
      );
      ...
      return b;
    } else return addStrokeToBounds(getMapBounds(), 0);
  }
  ...
  private Rectangle2D.Float addStrokeToBounds(
    Rectangle2D.Float r,
    float extra
  ) {
  ...
  protected AffineTransform getRelativeTransform(LWContainer ancestor) {
    if (parent == ancestor || parent == null) return transformDownA(
      new AffineTransform()
    ); else return transformDownA(parent.getRelativeTransform(ancestor));
  }
  ...
  protected AffineTransform transformDownA(final AffineTransform a) {
    if (mTemporaryTransform != null) {
      ...
      if (this.scale != 1) a.scale(this.scale, this.scale);
    }
   ...
   */
  public Point2D transformMapToZeroPoint(
    Point2D.Float mapPoint,
    Point2D.Float zeroPoint
  ) {
  ...
  protected Point2D transformZeroToMapPoint(
    Point2D.Float zeroPoint,
    Point2D.Float mapPoint
  ) {
    // This doesn't work if we're a link!
   ...
   */
  public Rectangle2D transformZeroToMapRect(
    Rectangle2D zeroRect,
    Rectangle2D mapRect
  ) {
    final AffineTransform tx = getZeroTransform();
    final double[] points = new double[4];
    ...
    if (mapRect == null) mapRect = new Rectangle2D.Float();
  ...
  protected boolean intersectsImpl(Rectangle2D mapRect) {
    ...
    if (DEBUG.PAINT && DEBUG.PICK) System.out.println(
      "INTERSECTS " +
      fmt(mapRect) +
      " " +
      (hit ? "YES" : "NO ") +
  ...
  protected Object requiresPaintImpl(DrawContext dc) {
    if (dc.skipDraw == this) return null;

    // always draw the focal
    if (dc.focal == this) return "isFocal";

    if (isZoomedFocus()) return null;
    ...
    if (dc.isClipOptimized()) {
      ...
      if (intersects(dc.getMasterClipRect())) return "inClipRegion";
  ...
  public final boolean contains(float x, float y, PickContext pc) {
    if (containsImpl(x, y, pc)) return true;
  ...
  private Point2D.Float getSlideIconStackLocation() {
    ...
    // If shape is small, try and keep it from overlapping too much (esp the label)
    if (xoff < getWidth() / 2f) xoff = getWidth() / 2f;
    if (yoff < getHeight() * 0.75f) yoff = getHeight() * 0.75f;
  ...
  public void drawLocal(DrawContext dc) {
    ...
    final AffineTransform zeroTransform = DEBUG.BOXES
      ? dc.g.getTransform()
      : null;

    //if (dc.focal == this || dc.isFocused()) // prevents slide icons from appearing in portals
    if (dc.focal == this) drawZero(dc); else drawZeroDecorated(dc, true);

    if (DEBUG.BOXES) drawDebugInfo(dc, zeroTransform);
  }

  private void drawDebugInfo(DrawContext dc, AffineTransform zeroTransform) {
    if (this instanceof LWLink) return;
  ...
  public final void drawZero(DrawContext dc) {
    final AffineTransform zeroTransform = DEBUG.PDF
      ? dc.g.getTransform()
      : null;
    ...
    try {
      if (!isTopLevel()) { // e.g., isn't a Layer, which is never selected
        ...
        if (VUE.getInteractionToolsPanel() != null) {
          ...
          if (alpha != 1 && !selectedOrParent()) dc.setAlpha(alpha); // fade nodes not in selection
        }
      }
    ...
    if (DEBUG.PDF && DEBUG.META && this instanceof LWLink == false) {
      ...
      if (c1 == null || !c1.equals(c2)) dc.g.drawString(fmt(c2), 0, 20);
    }
  }
  ...
  public void drawFit(DrawContext dc, Rectangle2D frame, int borderGap) {
    ...
    final double zoom = ZoomTool.computeZoomFit(
      size,
      borderGap,
  ...
  protected final void drawZeroDecorated(DrawContext dc, boolean drawSlides) {
    ...
    if (drawSlides && mEntries != null && !isFiltered()) drawSlideIconStack(dc);
    //else
  ...
  protected static final BasicStroke SlideIconPathwayStroke = new BasicStroke(
    (float) (LWPathway.PathBorderStrokeWidth / SlideIconScale),
  ...
  public synchronized void addLWCListener(
    Listener listener,
    LWComponent.Key singleEventKey
  ) {
  ...
  public static void swapLWCListener(
    Listener listener,
    LWComponent oldSource,
    LWComponent newSource
  ) {
    if (oldSource != null) oldSource.removeLWCListener(listener);
    if (newSource != null) newSource.addLWCListener(listener);
  }
  ...
  private boolean isStyling(Key key) {
    return (
      supportsProperty(key) &&
      //&& (key.isStyleProperty() || key == LWKey.Label);
      (key.isStyleProperty() ||
        (key == LWKey.Label && hasFlag(Flag.DATA_STYLE)))
    );
  }
  ...
  protected synchronized void notifyLWCListeners(LWCEvent e) {
    ...
    if (hasFlag(Flag.EVENT_SILENT)) return;
    ...
    if (e.component == this && e.key instanceof Key) {
      ...
      if (isStyle() && isStyling(key)) updateStyleWatchers(key, e);
  ...
  protected void doSyncUpdate(Key key) {
    ...
    if (mSyncSource != null) {
      Log.debug(
        "[" + key + "] UPDATING SYNC SOURCE: " + this + " -> " + mSyncSource
      );
      if (!mSyncSource.isDeleted()) key.copyValue(this, mSyncSource);
      ...
      for (LWComponent c : mSyncClients) {
        ...
        if (!c.isDeleted()) key.copyValue(this, c);
      }
    }
  }
  ...
  protected void updateStyleWatchers(Key key, LWCEvent e) {
    ...
    if (DEBUG.Enabled) out(
      "\nSTYLE OBJECT UPDATING STYLED CHILDREN with " +
      key +
      "; value " +
      Util.tags(e.getOldValue())
    );
    ...
    for (LWComponent dest : findPotentialStyleWatchers()) {
      ...
      if (
        dest.mParentStyle == this && dest.supportsProperty(key) && dest != this
      ) {
        ...
        try {
          if (key.isStyleProperty()) {
            if (key.valueEquals(dest, e.getOldValue())) {
              ...
              System.err.println(
                " SKIP-USER-SET: " +
                dest +
                "; target has user-override value: " +
                Util.tags(key.getValue(dest))
              );
            }
          } else {
            if (DEBUG.STYLE) Log.debug(
              "DATA-STYLE COPY " + key + " -> " + dest
            );
            key.copyValue(this, dest);
          }
        } catch (Throwable t) {
          tufts.Util.printStackTrace(
            t,
            "Failed to copy value from " + e + " old=" + e.getOldValue()
          );
        }
      }
    }
  ...
  private Collection<LWComponent> findPotentialStyleWatchers() {
    final LWMap map;
    if (getParent() == null) //map = getClientData(LWMap.class, "styledMap");
  ...
  protected void notify(String what, LWComponent contents) {
    if (alive()) notifyLWCListeners(new LWCEvent(this, contents, what));
  }
  ...
  protected void notify(String what, Object oldValue) {
    ...
    //             return;
    if (alive()) notifyLWCListeners(new LWCEvent(this, this, what, oldValue));
  }
  ...
  protected void notify(Key key, Object oldValue) {
    if (alive()) notifyLWCListeners(new LWCEvent(this, this, key, oldValue));
  }

  protected void notify(Key key, boolean oldValue) {
    if (alive()) notify(key, oldValue ? Boolean.TRUE : Boolean.FALSE);
  }
   ...
   */
  public void delete() {
    if (!isDeleted()) getParent().deleteChildPermanently(this); else Log.debug(
      "attempt to delete already deleted: " + this
    );
  }
  ...
  protected void removeFromModel() {
    if (isDeleted()) {
      if (DEBUG.PARENTING || DEBUG.EVENTS) out(
        this + " removeFromModel(lwc): ignoring (already removed)"
      );
      return;
    }
  ...
  protected boolean selectedOrParent() {
    return parent == null
      ? isSelected()
      : (isSelected() || parent.selectedOrParent());
  ...
  public void setState(State s, boolean on) {
    ...
    if (on) mState |= s.bit; else mState &= ~s.bit;
    ...
    if (mState != old) notify(KEY_State, Integer.valueOf(old));
  }
  ...
  public boolean isCollapsed() {
    if (COLLAPSE_IS_GLOBAL) return false; // LWNode overrides
    //return isGlobalCollapsed;
    else return hasFlag(Flag.COLLAPSED);
  }
  ...
  public boolean isAncestorCollapsed() {
    ...
    if (parent != null) {
      if (
        parent.isCollapsed()
      ) return true; else return parent.isAncestorCollapsed();
    } else return false;
  }
  ...
  String getDescriptionOfSetBits() {
    String s = "";
    if (mHideBits != 0) s +=
    getDescriptionOfSetBits(HideCause.class, mHideBits);
    if (mFlags != 0) {
      if (s.length() > 0) s += "; ";
  ...
  public void setHidden(HideCause cause, boolean hide) {
    if (hide) setHidden(cause); else clearHidden(cause);
  }
  ...
  private void setHideBits(int bits) {
    ...
    mHideBits = bits;
    if (wasHidden != isHidden()) notify(LWKey.Hidden);
    //notify(LWKey.Hidden, wasHidden); // if we need it to be undoable
  }
  ...
  public boolean hasDraws() {
    if (isFiltered()) return hasChildren(); else return isVisible();
  }
  ...
  public void addObject(final Object o) {
    ...
    final String name;
    if (o instanceof String) name = "String["; else name = Util.tag(o);

    if (s.length() > 1024) Log.info(
      this +
      " ignoring XML: " +
      name +
      "[" +
      ...
      "...x" +
      s.length()
    ); else Log.info(this + " ignoring XML: " + name + s + "]");
  }
  ...
  public void XML_fieldAdded(Object context, String name, Object child) {
    if (DEBUG.XML && DEBUG.META) out(
      "XML_fieldAdded <" + name + "> = " + child
    );
  }

  /** interface {@link XMLUnmarshalListener} */
  public void XML_addNotify(Object context, String name, Object parent) {
    if (DEBUG.XML && DEBUG.META) tufts.Util.printClassTrace(
      "tufts.vue",
      "XML_addNotify; name=" +
      name +
      "\n\tparent: " +
  ...
  protected Rectangle2D.Float getImageBounds() {
    final Rectangle2D.Float bounds = (Rectangle2D.Float) getPaintBounds()
      .clone();

    int growth = 1; // just in case / rounding errors

    if (this instanceof LWMap) growth += 15;

    if (growth > 0) grow(bounds, growth);

    return bounds;
  }

  private static double computeZoomAndSize(
  ...
  public BufferedImage createImage(
    ...
    final double usedZoom = computeZoomAndSize(
      bounds,
      maxSize,
      zoomRequest,
      imageSize
    ...
    if (width >= 512 || height >= 512) Log.info(
      "creating large image: " +
      imageSize +
      " = approx " +
      Util.abbrevBytes(width * height * 4)
    );
    ...
    if (
      ...
      if (DEBUG.DND || DEBUG.IMAGE) out(
        TERM_RED + "created image: " + mImageBuffer + TERM_CLEAR
      ); else Log.info("created image " + mImageBuffer);
    }

    drawImage(
      (Graphics2D) mImageBuffer.getGraphics(),
      alpha,
      maxSize,
      fillColor,
      zoomRequest
  ...
  public void drawImage(
    ...
    final double zoom = computeZoomAndSize(
      bounds,
      maxSize,
      zoomRequest,
      fillSize
    ...
    if (fillColor != null) {
      ...
      //             }
      if (alpha != OPAQUE) dc.setAlpha(alpha, AlphaComposite.SRC); // erase any underlying in cache
    ...
    if (zoom != 1.0) dc.g.scale(zoom, zoom);
    ...
    if (DEBUG.IMAGE && DEBUG.META) {
      ...
      dc.g.fillRect(
        -Short.MAX_VALUE / 2,
        -Short.MAX_VALUE / 2, // larger values than this can blow out internal GC code and we get nothing
        Short.MAX_VALUE,
        Short.MAX_VALUE
      );
    }
  ...
  public String toString() {
    String typeName = getClass().getSimpleName();
    if (typeName.startsWith("LW")) typeName = typeName.substring(2);
    String label = "";
    String s;
    if (getLabel() != null) {
    ...
    s += label;
    if (getResource() != null) s += " " + getResource().getSpec();
    //s += " <" + getResource() + ">";
    s += "]";
    return s;
  }
  ...
  public static void main(String args[]) throws Exception {
    ...
    java.util.Set<String> sortedKeys = new java.util.TreeSet<String>(
      edu.tufts.vue.style.StyleMap.keySet()
    );
  ...
  protected static final org.apache.log4j.Logger LWLog =
    org.apache.log4j.Logger.getLogger(LW.class);
}
/*
