...
public class ResourceIcon
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(ResourceIcon.class);
  ...
  synchronized void loadResource(Resource r) {
    if (DEBUG.RESOURCE || DEBUG.IMAGE) out(
      "loadResource: " + Util.tag(r) + " " + r
    );
  ...
  /*@Override*/public synchronized void run() {
    //loadPreview(mPreviewData);
    if (!isLoading) loadResource(mResource);
  }
  ...
  /*@Override*/public void paintIcon(Component c, Graphics g, int x, int y) {
    ...
    if (expandToFit) {
      ...
      if (DrawBorder) maxImageSize =
        new Dimension(
          fitWidth - BorderSpace * 2,
          fitHeight - BorderSpace * 2
        ); else maxImageSize = new Dimension(fitWidth, fitHeight);

      if (DEBUG.IMAGE && DEBUG.META) out(
        "paintIcon; into " + GUI.name(maxImageSize)
      );
    }
    ...
    if (mImage == NoImage && expandToFit) {
      ...
      if (CropToSquare) {
        ...
        int smallestAxis = mImageWidth > mImageHeight
          ? mImageHeight
          : mImageWidth;
      ...
      zoomFit =
        ZoomTool.computeZoomFit(maxImageSize, 0, imageBounds, null, false);
    ...
    if (DEBUG.IMAGE && DEBUG.META) out(
      "paintIcon; " + Util.tag(mImage) + " as " + drawW + "x" + drawH
    );
