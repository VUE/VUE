...
public class AssociationsPane extends Widget {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(AssociationsPane.class);
  static final int BUTTON_WIDTH = 20;
  static final String CHOOSE_FIELD = VueResources.getString(
    "associationsPane.chooseField"
  );
  ...
  public AssociationsPane(String name) {
    ...
    try {
      ...
      deleteAssociationAction =
        new AbstractAction(
          VueResources.getString("associationsPane.deleteassociation")
        ) {
          static final long serialVersionUID = 1;
      ...
      associationsTable =
        new JTable(new AssociationsTableModel()) {
          static final long serialVersionUID = 1;
      ...
      associationsTable
        .getSelectionModel()
        .addListSelectionListener(new AssociationsListSelectionListener());

      Font tableFont = GUI.LabelFace;

      associationsTable.setFont(tableFont);
      associationsTable.setRowHeight(
        (int) (1.5 * associationsTable.getFontMetrics(tableFont).getHeight())
      );
  ...
  public void setActions() {
    ...
    setHelpAction(
      this,
      VueResources.getString("dockWindow.Datasources.associationsPane.helpText")
    );
    enableMenuActions();
  }

  public void enableMenuActions() {
    int deleteCount = associationsTable.getSelectedRowCount(), deleteRow =
      associationsTable.getSelectedRow();
    ...
    deleteAssociationAction.setEnabled(
      deleteCount > 1 ||
      (deleteCount == 1 && deleteRow != Association.getCount())
    );
  ...
  public void deleteAssociation() {
    AssociationsTableModel model =
      ((AssociationsTableModel) associationsTable.getModel());
    int toDelete[] = associationsTable.getSelectedRows(), deleteCount =
      toDelete.length, lastRow = associationsTable.getRowCount() - 1;
  ...
  public boolean dropAssociation(Transferable transfer, int row, int column) {
    ...
    if (column == 1 || column == 3) {
      try {
        ...
        Log.error(
          "exception processing drop " + transfer + " at " + row + "," + column,
          t
        );
      }
    }
  ...
  public void toggleAssociation() {
    if (
      associationsTable.getSelectedRowCount() == 1 &&
  ...
  protected class AssociationsTableModel
    extends AbstractTableModel
    implements Association.Listener {
    ...
    public Class getColumnClass(int column) {
      if (getRowCount() > 0) return getValueAt(0, column)
        .getClass(); else return null;
    }
      ...
      switch (column) {
        case COL_ENABLED:
          result =
            (row == lastRow
                ? Boolean.FALSE
                : (Association.get(row).isEnabled()
                    ? Boolean.TRUE
                    : Boolean.FALSE));
          break;
        case COL_FIELD_LEFT:
          result =
            (row == lastRow ? tmpField0 : Association.get(row).getLeft());
          break;
        case COL_EQUALS:
          result = "=";
          break;
        case COL_FIELD_RIGHT:
          result =
            (row == lastRow ? tmpField1 : Association.get(row).getRight());
          break;
      }
  ...
  protected class AssociationsDropTarget extends DropTarget {
    ...
    public void drop(DropTargetDropEvent event) {
      Point dropLocation = event.getLocation();
      int row = associationsTable.rowAtPoint(dropLocation), column =
        associationsTable.columnAtPoint(dropLocation);

      if (row == Association.getCount()) {
        event.acceptDrop(DnDConstants.ACTION_COPY);
        event.dropComplete(
          dropAssociation(event.getTransferable(), row, column)
        );
      }
    }
  ...
  protected class AssociationsListSelectionListener
