...
public class PreviewPane
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(PreviewPane.class);
  ...
  private final JLabel StatusLabel = new JLabel(
    VueResources.getString("jlabel.status"),
    JLabel.CENTER
  );
    ...
    addMouseListener(
      new java.awt.event.MouseAdapter() {
        public void mouseClicked(MouseEvent me) {
          if (
            mResource != null && me.getClickCount() == 2
        ...
        public void mouseDragged(MouseEvent me) {
          if (mResource != null) {
            GUI.startSystemDrag(
              ...
              me,
              mImage,
              new GUI.ResourceTransfer(mResource)
            );
  ...
  public void mouseWheelMoved(MouseWheelEvent e) {
    if (GUI.noModifierKeysDown(e)) return;
    ...
    if (rotation < 0) height += MinHeight; else if (rotation > 0) height -=
    MinHeight;

    if (height < MinHeight) height = MinHeight; else if (
      height > MaxHeight
    ) height = MaxHeight;
  ...
  private void showLoadingStatus(Object data) {
    if (DEBUG.Enabled) StatusLabel.setText("" + data); else StatusLabel.setText(
  ...
  synchronized void loadResource(Resource r) {
    ...
    mResource = r;
    if (r != null) mPreviewData = r.getPreview(); else mPreviewData = null;
    mImage = null;
  ...
  private void loadPreview(Object previewData) {
    ...
    if (false/*&& r.getIcon() != null*/) { // these not currently valid from Osid2AssetResource (size=-1x-1)
      ...
      if (previewData instanceof Image) displayImage(
        (Image) previewData
      ); else loadImage(previewData);
    } else {
      displayImage(NoImage);
    }
  }
  ...
  public synchronized void gotImage(Object imageSrc, Images.Handle handle) {
    if (imageSrc != mPreviewData && !imageSrc.equals(mPreviewData)) {
      if (DEBUG.Enabled) Log.info(
        "skipping: image source != mPreviewData;\n\t" +
        Util.tags(imageSrc) +
        "\n\t" +
  ...
  public void paintComponent(Graphics g) {
    ...
    if (mImage == null) {
      if (LazyLoadOnPaint) {
        if (!isLoading && mPreviewData != null) {
          ...
          synchronized (this) {
            if (!isLoading && mPreviewData != null) VUE.invokeAfterAWT(
              PreviewPane.this
            ); // load the preview
          }
        }
    ...
    if (mImage == NoImage) {
      ...
      zoomFit = ZoomTool.computeZoomFit(getSize(), 0, imageBounds, null, false);

      final double maxZoom;

      if (mImageWidth == 128 && mImageHeight == 128) maxZoom = 1; // if looks like an icon, don't scale
      else if (getHeight() > MinHeight) maxZoom = 2; // if preview has been expanded, allow to scale up
      else maxZoom = 1; // default: scale never greater than 100%

      if (zoomFit > 1 && zoomFit < 2) netZoom = 1;
      //             if (zoomFit > 1 && zoomFit <= 1.25)
      //                 netZoom = 1;
      //             else if (zoomFit > 2 && zoomFit < 3)
      //                 netZoom = 2;
      else if (zoomFit > maxZoom) netZoom = maxZoom; else netZoom = zoomFit;
    }
    ...
    int xoff = 0;
    int yoff = 0;
    if (drawW != w) xoff = (w - drawW) / 2;
    if (drawH != h) yoff = (h - drawH) / 2;

    //if (DEBUG.IMAGE) out("painting " + Util.tag(mImage) + "; into " + drawW + "x" + drawH);
    if (DEBUG.IMAGE) out(
      String.format(
        "painting %s into %dx%d; zoomFit=%.1f%%; netZoom=%.1f%%;",
        Util.tags(mImage),
        drawW,
        drawH,
        zoomFit * 100,
