...
public class InspectorPane
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(InspectorPane.class);
  ...
  private final OntologicalMembershipPane ontologicalMetadata =
    new OntologicalMembershipPane();

  //-------------------------------------------------------
  // Resource panes
  //-------------------------------------------------------
  private final MetaDataPane mResourceMetaData = new MetaDataPane(
    "Resource Properties",
    false
  );
  private final MetaDataPane mDataSetData = new MetaDataPane(
    "Data Set Fields",
    false
  );
  ...
  public InspectorPane() {
    ...
    new Pane(
      VueResources.getString("inspectorpane.label"),
      mLabelPane,
      EXACT_SIZE,
      INFO + NOTES + KEYWORD
    );
    new Pane(
      VueResources.getString("inspectorpane.contentpreviews"),
      mPreview,
      EXACT_SIZE,
      RESOURCE
    );
    new Pane(
      VueResources.getString("inspectorpane.datasetfields"),
      mDataSetData,
      EXACT_SIZE,
      INFO + NOTES + KEYWORD + DATA
    );
    new Pane(
      VueResources.getString("inspectorpane.contentinfo"),
      mResourceMetaData,
      EXACT_SIZE,
      RESOURCE
    );
    new Pane(
      VueResources.getString("inspectorpane.contentsummary"),
      mDescription,
      0.5f,
      RESOURCE + DATA
    );
    new Pane(
      VueResources.getString("nodeNotesTabName"),
      mNotes,
      EXACT_SIZE,
      INFO + NOTES
    );
    new Pane(
      VueResources.getString("inspectorpane.pathwaynotes"),
      mPathwayNotes,
      EXPANDER,
      INFO + NOTES
    );
    new Pane(
      VueResources.getString("jlabel.keyword"),
      mKeywords,
      EXACT_SIZE,
      KEYWORD
    );
    new Pane(
      VueResources.getString(
        "combobox.mergepropertychoices.ontologicalmembership"
      ),
      ontologicalMetadata,
      EXACT_SIZE,
      0
    ...
    for (Pane p : Pane.AllPanes) stack.addPane(p.name, p.widget, p.size);
    ...
    Widget.setHelpAction(
      mLabelPane,
      VueResources.getString("dockWindow.Info.summaryPane.helpText")
    );
    Widget.setHelpAction(
      mPreview,
      VueResources.getString("dockWindow.Info.previewPane.helpText")
    );
    Widget.setHelpAction(
      mResourceMetaData,
      VueResources.getString("dockWindow.Info.resourcePane.helpText")
    );
    Widget.setHelpAction(
      mNotes,
      VueResources.getString("dockWindow.Info.notesPane.helpText")
    );
    Widget.setHelpAction(
      mKeywords,
      VueResources.getString("dockWindow.Info.userPane.helpText")
    );
    Widget.setHelpAction(
      ontologicalMetadata,
      VueResources.getString("dockWindow.Info.ontologicalMetadata.helpText")
    );
  ...
  public void addNotify() {
    ...
    if (mScrollPane == null) {
      mScrollPane =
        (JScrollPane) javax.swing.SwingUtilities.getAncestorOfClass(
          JScrollPane.class,
          this
        );
  ...
  public void activeChanged(
    ...
    if (
      ...
      ((ActiveEvent) e.source).type == LWComponent.class
    ) {
  ...
  public void activeChanged(
    final tufts.vue.ActiveEvent _e,
    ...
    if (index < 1 || entry.pathway.isShowingSlides()) {
      ...
      mPathwayNotes.setTitle(
        "Pathway Notes (" + entry.pathway.getLabel() + ": #" + index + ")"
      );
      //          }
  ...
  public void selectionChanged(final LWSelection s) {
    ...
    if (s.size() == 0) {
      hideAll();
      mSelectionInfo.setText(
        VueResources.getString("inspectionpane.nothingselected.tooltip")
      );
  ...
  private static final String ItemsSelectedFmt = VueResources.getString(
    "infowindow.itemselected"
  );
  ...
  private static final String SelectionStatsFmt = VueResources.local(
    "mapinspectorpanel.objectStats.format"
  );
  private static final String WordNodes = VueResources.local(
    "mapinspectorpanel.objectStats.nodes"
  );
  private static final String WordLinks = VueResources.local(
    "mapinspectorpanel.objectStats.links"
  );
  private static final String WordGroups = VueResources.local(
    "mapinspectorpanel.objectStats.groups"
  );
  private static final String WordImages = "Images:";

  private static final String DescriptionFormat =
    "<html>%s&nbsp;%d&nbsp;&nbsp;&nbsp; %s&nbsp;%d&nbsp;&nbsp;&nbsp; %s&nbsp;%d</html>";
  ...
  protected static String getSelectionDescription(final LWSelection s) {
    ...
    if (nodeCount > 0) {
      b.append(Gap).append(WordNodes).append(SP).append(nodeCount).append(Post);
    }
    if (linkCount > 0) {
      b.append(Gap).append(WordLinks).append(SP).append(linkCount).append(Post);
    }
    if (imageCount > 0) {
      b
        .append(Gap)
  ...
  private void loadAllNodePanes(LWComponent c) {
    ...
    if (c instanceof LWSlide && c.isPathwayOwned() && c.hasChildren()) {
      search:try {
        final LWComponent titleStyle =
          ((LWSlide) c).getEntry().pathway.getMasterSlide().getTitleStyle();

        if (titleStyle == null) break search;
    ...
    if (slideTitle == null) mLabelPane.loadLabel(c); else mLabelPane.loadLabel(
      slideTitle,
      c
    );
    ...
    if (DEBUG.Enabled) setTitleItem(
      c.getUniqueComponentTypeLabel()
    ); else setTitleItem(c.getComponentTypeLabel());
  }
  ...
  private static void setTypeName(
    ...
    if (c == null) type = null; else if (DEBUG.Enabled) type =
      c.getUniqueComponentTypeLabel(); else type = c.getComponentTypeLabel();

    String title;
    if (suffix != null) {
      if (type == null) title = suffix; else title = type + " " + suffix;
    } else title = type;
  ...
  private static final String DESCRIPTION_VIEWER_KEY =
    Resource.HIDDEN_RUNTIME_PREFIX + "ipCache";

  private boolean loadContentSummary(Resource r, MetaMap data) {
    JComponent descriptionView =
      (r == null
          ? null
          : (JComponent) r.getPropertyValue(DESCRIPTION_VIEWER_KEY));
    ...
    if (descriptionView == null) {
      try {
        ...
        if (descriptionView != null) gotView = true;
      } catch (Throwable t) {
      ...
      if (gotView && r != null) r.setProperty(
        DESCRIPTION_VIEWER_KEY,
        new java.lang.ref.SoftReference(descriptionView)
      );
      // r.putSoft
      // r.data.putSoft
    }
  ...
  private static String findProperty(Resource r, MetaMap data, String... keys) {
    for (String k : keys) {
      ...
      if (data != null) {
        ...
        if (Character.isUpperCase(k.charAt(0))) value =
          data.getString(k.toLowerCase());
        if (value != null) return value;
      }
      if (r != null) {
        value = r.getProperty(k);
  ...
  private static JComponent buildSummary(final Resource r, final MetaMap data) {
    ...
    final String desc = findProperty(
      r,
      data,
      "descriptionSummary",
      "Description",
    ...
    if (desc != null || r == null) {
      ...
      else htmlText.setText(summary);

      if (DEBUG.DATA && data != null) data.put(
        "$reformatted",
        "[" + summary + "]"
      );
    } else { //if (r != null) {
  ...
  private static String buildSummaryFromResource(final Resource r) {
    ...
    try {
      if (r.isLocalFile()) {
        if (r.getSpec().startsWith("file:")) b.append(
          r.getSpec()
        ); else b.append("file://" + r.getSpec());
      } else b.append(r.getSpec());
  ...
  private static String buildSummaryWithDescription(
    final Resource r,
    final MetaMap data,
    String desc
  ) {
    final StringBuilder buf = new StringBuilder(
      128 + (desc == null ? 0 : desc.length())
    );

    if (desc != null) desc = cleanForTextPaneAndTrim(desc);

    String title = findProperty(r, data, "Title");
    if (title == null && r != null) title = r.getTitle();
    ...
    if (desc == null || desc.indexOf("<style") < 0) {
      ...
      final String published = findProperty(
        r,
        data,
        "Published",
        "pubDate",
      ...
      if (desc != null) {
        ...
        if (!desc.startsWith("<p>")) buf.append("<p>\n");
      }
    }
    ...
    if (link != null && Resource.looksLikeURLorFile(link)) {
      if (hasDesc) buf.append("<p>"); else if (
        !buf.toString().endsWith("<br>")
      ) buf.append("<br>");
      //if (DEBUG.DR && hasDesc) buf.append("hadDescription:");
      buf.append(
        String.format("<a href=\"%s\">%s</a><br>", link, Util.decodeURL(link))
      );
    }
    ...
    if (
      image != null &&
      Resource.looksLikeURLorFile(image) &&
      Resource.looksLikeImageFile(image)
    ) {
      buf.append(
        String.format(
          "<center><a href=\"%s\"><img src=\"%s\"></a>",
          image,
          image
        )
      );
  ...
  private void hideAllPanes() {
    for (Pane p : Pane.AllPanes) Widget.setHidden(p.widget, true);
  }
  ...
  private void expandCollapsePanes(int type) {
    for (Pane p : Pane.AllPanes) {
      if (Widget.isHidden(p.widget)) continue;

      if ((p.bits & type) != 0) {
        if (!Widget.isExpanded(p.widget)) Widget.setExpanded(p.widget, true);
      } else {
        if (Widget.isExpanded(p.widget)) Widget.setExpanded(p.widget, false);
      }
    }
  }
  ...
  private void displayPanes(int type) {
    ...
    for (Pane p : Pane.AllPanes) {
      if ((p.bits & type) != 0) {
        ...
        anyVisible = true;
      } else Widget.setHidden(p.widget, true);
    }

    if (anyVisible) Widget.setHidden(mSelectionInfo, true);
  }
  ...
  private class Preview extends tufts.vue.ui.PreviewPane {
    ...
    void loadResource(Resource r) {
      ...
      if (title == null) title = r.getProperty("title");
      // TODO: resource property lookups should be case insensitive
      if (title == null) title = r.getProperty("Title");
      if (title == null) title = "Content Preview";
  ...
  public class UserMetaData extends Widget {
    ...
    public void refresh() {
      ...
      if (userMetadataEditor != null) userMetadataEditor.listChanged();
    }
    ...
    void loadKeywords(LWComponent c) {
      ...
      if (c != null && c.hasMetaData()) mKeywords.setTitle(
        "Keywords (" + c.getMetadataList().getCategoryListSize() + ")"
      ); else mKeywords.setTitle("Keywords");
    }
    //         @Override
  ...
  public class LabelPane extends tufts.Util.JPanelAA {
    ...
    private final VueTextPane labelValue = new VueTextPane() {
      ...
      protected void applyText(String text) {
        ...
        if (selection != null && text != null && text.trim().length() > 0) {
          ...
          if (dataStyle != null) VUE.markUndo(
            selection.size() + " Data Labels"
          ); else VUE.markUndo(selection.size() + " Labels");
        }
      }
    };
    ...
    LabelPane() {
      ...
      if (Util.isMacPlatform()) {
        ...
        labelValue.setBorder(
          new CompoundBorder(
            new MatteBorder(so, so, so, so, SystemColor.control),
            new CompoundBorder(
              labelValue.getBorder(),
    ...
    void loadLabel(LWSelection s) {
      if (DEBUG.Enabled) Log.debug(
        "LabelPane LOADING SELECTION w/style " + s.getStyleRecord()
      );
      ...
      if (s.getStyleRecord() != null) {
        ...
        setName(
          String.format(
            VueResources.getString("infowindow.multiplelabel"),
            s.size()
          )
        );
      }
  ...
  public class SummaryPane extends tufts.Util.JPanelAA implements Runnable {
    ...
    SummaryPane() {
      ...
      JScrollPane labelScroller = new JScrollPane(
        labelValue,
        JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
        JScrollPane.HORIZONTAL_SCROLLBAR_NEVER
      );
      int lineHeight = labelValue
        .getFontMetrics(labelValue.getFont())
        .getHeight();
    ...
    void load(LWComponent c) {
      ...
      if (c instanceof LWText) labelValue.setEditable(
        false
      ); else labelValue.setEditable(true);
  ...
  private final Insets labelInsets = new Insets(
    topPad,
    0,
    botPad,
    GUI.LabelGapRight
  );
  private final Insets fieldInsets = new Insets(
    topPad,
    0,
    botPad,
    GUI.FieldGapRight
  ...
  private void addLabelTextRows(
    ...
    for (int i = 0; i < labels.length; i++) {
      ...
      if (labelFace != null) GUI.apply(labelFace, label);

      if (field instanceof JTextComponent) {
        if (field instanceof JTextField) centerLabelVertically = true;
        ...
        if (fieldFace != null) GUI.apply(fieldFace, field);
      }
      ...
      if (centerLabelVertically) c.anchor =
        GridBagConstraints.EAST; else c.anchor = GridBagConstraints.NORTHEAST;
    ...
    } else defaultExpander.setOpaque(false);
  ...
  private void loadText(JTextComponent c, String text) {
    ...
    if (hasText != text && !hasText.equals(text)) c.setText(text);
  }
  ...
  public void searchPerformed(SearchEvent evt) {
    ...
    if (
      VUE.getSelection().size() > 0 && VUE.getActiveResource() == null
    ) return;
