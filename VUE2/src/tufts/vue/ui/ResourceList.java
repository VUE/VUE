 ...
 *
 * @version $Revision: 1.30 $ / $Date: 2010-02-03 19:16:31 $ / $Author: mike $
 */
public class ResourceList
  extends JList
  implements
    DragGestureListener,
    /*tufts.vue.ResourceSelection.Listener,*/MouseListener,
    ActionListener {

  public static final Color DividerColor = VueResources.getColor(
    "ui.resourceList.dividerColor",
    204,
    204,
    204
  );
  ...
  private static ImageIcon DragIcon = tufts.vue.VueResources.getImageIcon(
    "favorites.leafIcon"
  );
  ...
  private static class MsgLabel extends JLabel {
    ...
    MsgLabel(String txt) {
      ...
      setBorder(
        new CompoundBorder(
          new MatteBorder(0, 0, 1, 0, DividerColor),
  ...
  private JLabel mLessLabel = new MsgLabel(
    String.format(
      Locale.getDefault(),
      VueResources.getString("resourcelist.showless"),
      PreviewItems
    )
  );
    ...
    public Object getElementAt(int index) {
      if (isMasking) {
        if (index == PreviewItems) {
          return mMoreLabel;
        } else if (index > PreviewItems) return "MASKED INDEX " + index; // should never see this
      } else if (index == size()) {
        return mLessLabel;
      }
  ...
  public ResourceList(Collection resourceBag, String name) {
    ...
    if (resourceBag.size() > PreviewModelSize) {
      ...
      mMoreLabel =
        new MsgLabel(
          String.format(
            Locale.getDefault(),
            VueResources.getString("resourcelist.showmore"),
            (resourceBag.size() - PreviewItems)
          )
        );

      isMasking = true;
    } else mDataModel = new javax.swing.DefaultListModel();
    ...
    while (i.hasNext()) mDataModel.addElement(i.next());
        ...
        public void valueChanged(ListSelectionEvent e) {
          if (DEBUG.RESOURCE || DEBUG.SELECTION || DEBUG.FOCUS) {
            ...
            if (e.getValueIsAdjusting()) System.out.println(
              ResourceList.this + " isAdjusting: ignoring"
            );
          }

          if (e.getValueIsAdjusting()) return;

          if (DEBUG.SELECTION) tufts.Util.printStackTrace(
            "ResourceList valueChanged " + ResourceList.this
          );
          if (isMaskingModel) {
            if (
              isMasking && getSelectedIndex() >= PreviewItems
            ) ((MaskingModel) mDataModel).expand(); else if (
              !isMasking && getSelectedIndex() == mDataModel.size()
            ) {
          ...
          if (getPicked() != null) tufts.vue.VUE.setActive(
            Resource.class,
            ResourceList.this,
            getPicked()
          );
    ...
    addMouseListener(
      new tufts.vue.MouseAdapter("resourceList") {
        public void mouseClicked(java.awt.event.MouseEvent e) {
          if (e.getClickCount() == 2) {
            ...
            if (
              isMaskingModel && getSelectedValue() == mLessLabel
      ...
      new java.awt.event.MouseMotionAdapter() {
        public void mouseDragged(java.awt.event.MouseEvent me) {
          ...
          if (picked != null) {
            ...
            if (getSelectedValue() instanceof ResourceIcon) image =
              ((ResourceIcon) o).getImage(); // todo: more generic on Resource class
            ...
            GUI.startSystemDrag(
              ...
              me,
              image,
              new GUI.ResourceTransfer(picked)
            );
          }
        }
      }
  ...
  public void activeChanged(
    ...
    final tufts.vue.Resource resource
  ) {
  ...
  private Resource getPicked() {
    Object o = getSelectedValue();
    if (o instanceof Resource) return (Resource) o; else if (
      o instanceof ResourceIcon
    ) return ((ResourceIcon) o).getResource(); else return null;
  ...
  public void dragGestureRecognized(DragGestureEvent e) {
    if (getSelectedIndex() != -1) {
      Resource r = getPicked();
      if (DEBUG.DND || DEBUG.SELECTION) System.out.println(
        "ResourceList: startDrag: " + r
      );
      e.startDrag(
  ...
  public String toString() {
    String tag;
    if (mName == null) tag = ""; else tag = "[" + mName + "]";
  ...
  private class RowRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(
      ...
      if (value == mMoreLabel) return mMoreLabel; else if (
        value == mLessLabel
      ) return mLessLabel;
      ...
      if (false) setText("<HTML>" + r.getTitle()); else setText(r.getTitle());
      /*  if (isSelected) {
  ...
  private final JMenuItem launchResource = new JMenuItem(
    VueResources.getString("menu.popup.resource.launchresource")
  );
  private final WindowDisplayAction infoAction = new WindowDisplayAction(
    VUE.getInfoDock()
  );
  private final JCheckBoxMenuItem infoCheckBox = new JCheckBoxMenuItem(
    infoAction
  );
  private final JMenuItem addToMap = new JMenuItem(
    VueResources.getString("menu.popup.resource.addtomap")
  );
  private final JMenuItem addToNode = new JMenuItem(
    VueResources.getString("menu.popup.resource.addtoselectednode")
  );
  private final JMenuItem addToSlide = new JMenuItem(
    VueResources.getString("menu.popup.resource.addtoslide")
  );
  private final JMenuItem addToSavedContent = new JMenuItem(
    VueResources.getString("menu.popup.resource.addtomysavedcontent")
  );
  ...
  private JPopupMenu getPopup(MouseEvent e) {
    if (m == null) {
      ...
      infoCheckBox.setLabel(
        VueResources.getString("menu.popup.resource.resourceinfo")
      );
    ...
    if (c != null && c instanceof LWNode) {
      ...
      if (c.hasResource()) addToNode.setLabel(
        VueResources.getString("menu.popup.resource.replaceresourceonnode")
      );
      ...
      addToNode.setEnabled(false);
      addToSlide.setEnabled(false);
      if (c != null && c.hasResource()) addToNode.setLabel(
        VueResources.getString("menu.popup.resource.addtoselectednode")
      );
    }
    return m;
  ...
  private static final int MAX_LABEL_LINE_LENGTH = 30;
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource().equals(launchResource)) {
      ...
      if (true) tufts.vue.VueUtil.setXYByClustering(
        end
      ); else setXYByClustering(end);
      ...
      ((FavoritesWindow) repository.getResourceViewer()).getFavoritesTree()
        .addResource(o.getResource());
    }
  }
  ...
  public static void setXYByClustering(LWNode node) {
    Iterator<LWComponent> i = VUE
      .getActiveMap()
      .getAllDescendents(LWContainer.ChildKind.PROPER)
      .iterator();
    float xNumerator = 0;
    float yNumerator = 0;
    float denominator = 0;
