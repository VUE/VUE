...
public class MetaDataPane
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MetaDataPane.class);
  ...
  public void addNotify() {
    ...
    if (mScrollPane == null) mScrollPane =
  ...
  static {
    ...
    if (EasyReading1) {
      ...
      if (DEBUG.DR) ValueFace = dataFace; else ValueFace = GUI.ValueFace;
    }
  }
  ...
  private boolean ensureSlots(int minSlots) {
    ...
    } else curSlots = mLabels.length;

    if (minSlots <= curSlots) return false;

    int maxSlots;
    if (curSlots == 0) maxSlots = minSlots; else maxSlots = minSlots + 4;
    ...
    if (Util.isMacPlatform()) alternatingColor =
      Color.white; else alternatingColor = new Color(250, 250, 250);
    ...
    for (int i = 0; i < mLabels.length; i++) {
      ...
      if (Util.isWindowsPlatform()) mValues[i].setBorder(
          WindowsPlatformAdjustBorder
        );
      ...
      if (EasyReading2) {
        if (i % 2 != 0) {
          ...
          if (Util.isMacPlatform()) mValues[i].setBorder(
              macAdjustBorder
            ); else mValues[i].setBorder(winAdjustBorder);
        }
  ...
  private void loadPropertiesAWT(TableBag propertyMap) {
    if (DEBUG.THREAD || DEBUG.RESOURCE || DEBUG.IMAGE) out(
      "loadPropertiesAWT: " + propertyMap.size() + " key/value pairs"
    );
    ...
    try {
      ...
      if (mProperties != propertyMap) {
        if (mProperties != null) mProperties.removeListener(this);
  ...
  private void updateDisplayAWT(final TableBag properties) {
    ...
    if (DEBUG.THREAD || DEBUG.RESOURCE) out(
      "updateDisplay: " + properties.size() + " key/value pairs"
    );

    if (DEBUG.SCROLL) Log.debug(
      "scroll model listeners: " +
      Arrays.asList(
        ((DefaultBoundedRangeModel) mScrollPane
            .getVerticalScrollBar()
            .getModel()).getListeners(ChangeListener.class)
      )
    );

    if (inScroll) mScrollPane.getVerticalScrollBar().setValueIsAdjusting(true);

    mGridBag.setPaintDisabled(true);

    try {
      if (DEBUG.RESOURCE || DEBUG.THREAD) out(
        "updateDisplay: getTableModel() on " + tufts.Util.tag(properties)
      );
      ...
      loadAllRows(
        model,
        properties instanceof MetaMap ? (MetaMap) properties : null
      );

      if (DEBUG.Enabled) setTitle(mLabel + " (" + properties.size() + ")");
  ...
  private String valueToText(final Object value) {
    if (value == null) return "(empty)"; else if (
      value instanceof java.awt.Component
    ) return GUI.name(value); else if (
      value instanceof java.lang.ref.Reference
    ) return (
      "[" +
      value.getClass().getSimpleName() +
      "] " +
      valueToText(((java.lang.ref.Reference) value).get())
    ); // note recursion
  ...
  private void loadAllRows(TableModel model, MetaMap dataMap) {
    ...
    if (Util.getJavaVersion() > 1.5) {
      ...
      if (rows > 512) maxRow = 512; else maxRow = rows;
    }
  ...
  private boolean loadRow(
    ...
    if (DEBUG.RESOURCE && DEBUG.META) out(
      "adding row " + row + " " + labelText + "=[" + valueText + "]"
    );
    ...
    if (
      mLastLabel != null &&
      mLastValue != null &&
      labelText.startsWith(mLastLabel + "@")
    ) {
      ...
      mLastValue.setText(
        String.format("%s [%s=%s]", mLastValue.getText(), attributeLabel, value)
      );
    ...
    if (EasyReading2) {
      ...
      //             }
        ...
        txt =
          "<html>" +
          labelText.substring(0, half) +
          "<br>" +
          labelText.substring(half, len);
      } else {
        if (DEBUG.DR || DEBUG.DATA) txt = labelText; else txt =
          Util.upperCaseWords(labelText);
      }
    ...
    if (DEBUG.Enabled) {
      ...
      if (value instanceof String == false) field.setForeground(ObjectColor);
    }

    // if field has at least one space, use word wrap
    if (valueText.indexOf(' ') >= 0) field.setWrapStyleWord(
      true
    ); else field.setWrapStyleWord(false);
  ...
  public static int getWrappedLines(JTextComponent c) {
    ...
    if (linesOfText == 0) linesOfText = 1;
    return linesOfText;
  }
