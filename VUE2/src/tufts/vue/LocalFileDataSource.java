...
public class LocalFileDataSource
  extends BrowseDataSource
  implements Publishable {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LocalFileDataSource.class);
  ...
  protected JComponent buildResourceViewer() {
    ...
    if (this.getAddress().length() > 0) {
      ...
      LocalCabinet rootNode = LocalCabinet.instance(
        this.getAddress(),
        agent,
        null
      );
  ...
  private void installDesktopFolders(Vector cabVector) {
    ...
    if (home.exists() && home.canRead()) {
      ...
      for (int i = 0; i < dirs.length; i++) {
        ...
        if (dir.exists() && dir.canRead() && dir.isDirectory()) { // TODO: create workaround for Vista bug
          ...
          CabinetResource r = CabinetResource.create(
            LocalCabinet.instance(dir, agent, null)
          );
      ...
      if (added == 0 || tufts.Util.isWindowsPlatform() == false) {
        CabinetResource r = CabinetResource.create(
          LocalCabinet.instance(home, agent, null)
        );
        String title = "Home";
    ...
    if (volumes != null && volumes.exists() && volumes.canRead()) {
      ...
      for (int i = 0; i < vols.length; i++) {
        ...
        CabinetResource r = CabinetResource.create(
          LocalCabinet.instance(v, agent, null)
        );
    ...
    try {
      ...
      if (!tufts.Util.isWindowsPlatform()) fsview =
        FileSystemView.getFileSystemView();

      final LocalFilingManager manager = getLocalFilingManager(); // get a filing manager

      LocalCabinetEntryIterator rootCabs =
        (LocalCabinetEntryIterator) manager.listRoots();
      while (rootCabs.hasNext()) {
        ...
        if (rootNode instanceof LocalCabinet) {
          ...
          String sysName = null;
          if (!tufts.Util.isWindowsPlatform()) sysName =
            fsview.getSystemDisplayName(f); else sysName = f.toString();
      ...
      VueUtil.alert(
        null,
        ex.getMessage(),
        VueResources.getString("dialog.settingreserroe.title")
      );
    }
  }
  ...
  public int[] getPublishableModes() {
    int modes[] = {
      Publishable.PUBLISH_MAP,
      Publishable.PUBLISH_CMAP,
      Publishable.PUBLISH_ZIP,
    };
    return modes;
  }
  ...
  public void publish(int mode, LWMap map) throws IOException {
    System.out.println(
      "ZIP File: " + map.getFile() + "," + map + ", mode:" + mode
    );
    if (mode == Publishable.PUBLISH_MAP) publishMap(map); else if (
      mode == Publishable.PUBLISH_CMAP
    ) publishCMap(map); else if (mode == Publishable.PUBLISH_ALL) publishAll(
      map
    ); else if (mode == Publishable.PUBLISH_ZIP) publishZip(map);
  }

  private void publishMap(LWMap map) throws IOException {
    File savedMap = PublishUtil.saveMap(map);
    InputStream istream = new BufferedInputStream(
      new FileInputStream(savedMap)
    );
  ...
  private void publishCMap(LWMap map) throws IOException {
    try {
      ...
      VueUtil.alert(
        VUE.getDialogParent(),
        VueResources.getString("dialog.export.message") + ex.getMessage(),
        VueResources.getString("dialog.export.title"),
  ...
  private void publishZip(LWMap map) {
    try {
      if (map.getFile() == null) {
        VueUtil.alert(
          VueResources.getString("dialog.mapsave.message"),
  ...
  private void publishAll(LWMap map) {
    VueUtil.alert(
      VUE.getDialogParent(),
      VueResources.getString("dialog.exportall.message"),
      VueResources.getString("dialog.export.title"),
