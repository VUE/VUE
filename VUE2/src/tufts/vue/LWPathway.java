 ...
 */
public class LWPathway extends LWContainer implements LWComponent.Listener {
  ...
  private java.util.List<Entry> mSecureEntries =
    java.util.Collections.unmodifiableList(mEntries);

  /** for backward compat with old style saved pathways -- the ordered list of member node ID's */
  private transient java.util.List<String> mOldStyleMemberIDList =
    new java.util.ArrayList();
  /** for backward compat with old style saved pathways (pre Feb/March 2006) */
  private ArrayList<LWPathwayElementProperty> mOldStyleProperties =
    new ArrayList();
  ...
  private static final BooleanPreference AutoNodeToSlideNotesPref =
  ...
  public static class Entry implements Transferable {
    ...
    private Entry(LWPathway pathway, Entry partial) {
      ...
      if (slide != null) slide.setPathwayEntry(this);
      this.isMapView = partial.isMapView;
      this.notes = partial.notes;
      if (isOffMapSlide() && slide != null) slide.enableProperty(LWKey.Label);
      syncNodeEntryRef();
    }
    ...
    private void syncNodeEntryRef() {
      if (node != null && node instanceof LWPathway == false) node.addEntryRef(
        this
      );
    }
    ...
    public String getLabel() {
      if (node != null) return node.getDisplayLabel(); else return getSlide()
        .getLabel();
    }

    public void setLabel(String s) {
      if (node != null) node.setLabel(s); else getSlide().setLabel(s);
    }
    ...
    public Color getFullScreenFillColor(DrawContext dc) {
      if (isMapView()) return pathway
        .getMasterSlide()
        .getFillColor(); else return getSlide().getRenderFillColor(dc);
    }
    ...
    private LWSlide getSlide(boolean force) {
      if (isMapView()) {
        if (force || node.supportsSlide()) {
          ...
          if (mapSlide == null) mapSlide = new MapSlide(this);
          return mapSlide;
        } else return null;
      }

      if (slide == null || slide instanceof MapSlide) buildSlide();
      if (node != null && slide.parent != node) {
        if (node instanceof LWContainer) slide.setParent(
          (LWContainer) node
        ); else Util.printStackTrace(
          "Non-containers can't have slides: " + node + " can't own " + slide
        );
      }
      return slide;
    }
    ...
    public void setPersistSlide(LWSlide s) {
      if (isMapView) {
        if (DEBUG.Enabled) Log.info(
          "skipping restoring slide for an entry marked as map-view: " + this
        );
    ...
    public void setMapView(boolean asMapView) {
      if (isMapView == asMapView) return;

      final boolean wasMapView = isMapView;
      isMapView = asMapView;

      if (restoreUnderway()) return;
      ...
      if (wasMapView) {
        ...
        } else if (node != null) node.notify(this, MAP_VIEW_CHANGED);
      } else {
    ...
    public final boolean isMapView() {
      ...
      if (node.supportsSlide()) return isMapView; else return true;
    }

    public boolean canProvideSlide() {
      if (isMapView()) return node.supportsSlide(); else return true;
    }
    ...
    public final boolean isPortal() {
      return (
        (node instanceof LWPortal) || (node != null && node.isTranslucent())
      );
    }
    ...
    public void setNotes(String s) {
      if (notes == s || (notes != null && notes.equals(s))) return;
      final String oldNotes = notes;
      notes = s;
      if (
        pathway != null
      ) pathway.notify( // will be null during restore
        "pathway.entry.notes",
        new Undoable() {
          void undo() {
            setNotes(oldNotes);
    ...
    public String toString() {
      ...
      if (isMapView()) s += "MAP-VIEW"; else s +=
      (slide == null ? "<null-LWSlide>" : slide.getLabel());
    ...
    public boolean isDataFlavorSupported(
      java.awt.datatransfer.DataFlavor arg0
    ) {
      if (arg0.isFlavorRemoteObjectType()) return true; else return false;
    }
  }
  ...
  public final Icon mSlideIcon = new Icon() {
    public void paintIcon(
      ...
      final Color color = Util.alphaMix(
        AlphaWhite,
        getMasterSlide().getFillColor()
      );
      g.setColor(color);
   ...
   */
  private final Entry mOurEntry = new Entry(LWPathway.this, LWPathway.this) {
    @Override
    public boolean canProvideSlide() {
      return false;
    }
  ...
  private static final int PathwayAlpha = (int) (255f *
      (VueResources.getInt("pathway.alpha.percent", 50) / 100f) +
    0.5);
  ...
  public Entry getFirstEntry(LWComponent c) {
    for (Entry e : mEntries) if (e.node == c) return e;
    return null;
  }
  ...
  void setRevealer(boolean t) {
    throw new UnsupportedOperationException(
      "re-implement reveal functionality"
    );
    //         mRevealer = t;
  ...
  @Override
  public final void setFiltered(boolean t) {
  ...
  private static Color getNextColor() {
    if (sColorIndex >= ColorTable.length) sColorIndex = 0;
  ...
  public int count(LWComponent c) {
    int count = 0;
    for (Entry e : mEntries) if (e.node == c) count++;
    return count;
  }
  ...
  public Entry getEntry(int index) {
    if (index >= length() || index < 0) return null; else return mEntries.get(
      index
    );
  }

  public int getEntryIndex(Entry e) {
    if (e.pathway != this) Log.warn(
      "fetching entry index for non-member of ths pathway: " + e
    );
  ...
  public boolean moveCurrentUp() {
    if (mCurrentIndex <= 0) return false;
  ...
  public boolean moveCurrentDown() {
    if (mCurrentIndex == length() - 1) return false;
  ...
  public void add(Iterator<LWComponent> i) {
    ...
    while (i.hasNext()) {
      ...
      if (AutoNodeToSlideNotesPref.isTrue()) e.setNotes(c.getNotes());
    ...
    if (addCount > 0) {
      ...
      if (addCount == 1) newIndex = newEntries.size() - 1;
  ...
  private void removeEntries(
    final Iterator<LWComponent> toRemove,
    int index,
    boolean isDeleting
  ) {
    if (DEBUG.PATHWAY || DEBUG.PARENTING) out(
      "removeEntries " +
      toRemove +
      " index=" +
      index +
    ...
    if (didRemove) setEntries("pathway.remove", newEntries, NO_INDEX_CHANGE);
  }

  private synchronized void removeIndex(int index, boolean isDeleting) {
    if (DEBUG.PATHWAY || DEBUG.PARENTING) out(
      "removeIndex " + index + " isDeleting=" + isDeleting
    );
  ...
  private void setEntries(
    ...
    for (Entry e : oldEntries) if (!newEntries.contains(e)) e.removeFromModel();

    for (Entry e : newEntries) e.ensureModel();

    final int oldIndex = mCurrentIndex;

    if (newIndex >= -1) setIndex(newIndex);
    //mCurrentIndex = newIndex;
    else if (mCurrentIndex >= newEntries.size()) setIndex(
      newEntries.size() - 1
    );
  ...
  public void setPrevious() {
    if (mCurrentIndex > 0) setIndex(mCurrentIndex - 1);
  }

  public void setNext() {
    if (mCurrentIndex < (length() - 1)) setIndex(mCurrentIndex + 1);
  }
  ...
  protected void removeFromModel() {
    ...
    for (Entry e : mEntries) e.removeFromModel();
  }

  @Override
  protected void restoreToModel() {
    super.restoreToModel();
    for (Entry e : mEntries) e.ensureModel();
  }

  @Override
  public void preCacheContent() {
    if (DEBUG.Enabled) Log.debug(
      "preCacheContent: not currently enabled for " + getClass()
    );
  ...
  public MasterSlide getMasterSlide() {
    if (mMasterSlide == null && !mXMLRestoreUnderway) mMasterSlide =
      buildMasterSlide();
    return mMasterSlide;
  }
  ...
  private static void getMasterStyle() {
    System.out.println(
      "MASTER SLIDE CSS: " + VueResources.getURL("masterSlide.css")
    );
  ...
  static final class MapSlide extends LWSlide {
    ...
    public String getLabel() {
      return (
        "View of " +
        getEntry().node.getDisplayLabel() +
        " in " +
        getEntry().pathway.getDisplayLabel()
      );
    }
  ...
  protected void addChildImpl(LWComponent c, Object context) {
    throw new UnsupportedOperationException("add:" + context + "; " + c);
  }
  ...
  @Override
  public java.util.List<LWComponent> getChildList() {
    if (DEBUG.XML || DEBUG.PATHWAY) out(
      "getChildList returning EMPTY, as always"
    );
    return java.util.Collections.EMPTY_LIST;
  }
  ...
  public java.util.List<LWComponent> getChildren() {
    if (DEBUG.XML || DEBUG.PATHWAY) out(
      "getChildren returning EMPTY, as always"
    );
  ...
  public LWComponent getChild(int index) {
    throw new UnsupportedOperationException(
      "pathways don't have proper children"
    );
  }
  ...
  void completeXMLRestore(LWMap map) {
    if (DEBUG.INIT || DEBUG.IO || DEBUG.XML) Log.debug(
      this + " completeXMLRestore, map=" + map
    );
    setParent(map);
    ...
    for (Entry e : mEntries) newEntries.add(new Entry(this, e));
    ...
    if (mOldStyleMemberIDList.size() > 0) {
      final Collection<LWComponent> allRestored = map.getAllDescendents(
        ChildKind.ANY
      );
      for (String id : mOldStyleMemberIDList) {
        LWComponent c = map.findByID(allRestored, id);
        if (DEBUG.XML || DEBUG.PATHWAY) out(
          "RESTORING old-style path element " + c
        );
    ...
    for (LWPathwayElementProperty pep : mOldStyleProperties) {
      for (Entry e : findEntriesWithNodeID(pep.getElementID())) {
        if (DEBUG.XML || DEBUG.PATHWAY) out(
          "RESTORING old style notes for " + e
        );
  ...
  private List<Entry> findEntriesWithNodeID(String ID) {
    ...
    for (Entry e : mEntries) if (
      e.node != null && e.node.getID().equals(ID)
    ) matches.add(e);
  ...
  public java.util.List<String> getElementIDList() {
    if (DEBUG.XML || DEBUG.PATHWAY) out(
      "getElementIDList: " + mOldStyleMemberIDList
    );
    if (mXMLRestoreUnderway) return mOldStyleMemberIDList; else return null; // we no longer save this way: only read in
  }

  /** for persistance: XML save/restore only for old-style (non Entry) pathway code */
  public java.util.List getElementPropertyList() {
    if (mXMLRestoreUnderway) return mOldStyleProperties; else return null; // we no longer save this way: only read in
  }

  private static final boolean PathwayAsDots = true;
  private static final int ConnectorStrokeWidth = VueResources.getInt(
    "pathway.stroke.width",
    5
  );
  ...
  private static final float DotSize = VueResources.getInt(
    "pathway.dotSize",
    20
  );
  ...
  private static final BasicStroke ConnectorStrokePlain = new BasicStroke(
    ConnectorStrokeWidth,
    BasicStroke.CAP_ROUND,
    BasicStroke.JOIN_BEVEL
  );

  private static final BasicStroke ConnectorStrokeActive = new BasicStroke(
    10,
    BasicStroke.CAP_ROUND,
    BasicStroke.JOIN_BEVEL, // ignored: always straight (no bends)
    0f, // ignored (mitre-limit)
  ...
  private static final BasicStroke PathBorderStroke = new BasicStroke(
    PathBorderStrokeWidth,
    BasicStroke.CAP_ROUND,
    BasicStroke.JOIN_ROUND
  );

  private static final BasicStroke PathBorderStrokeDashed = new BasicStroke(
    PathBorderStrokeWidth,
  ...
  private static final BasicStroke PathBorderStrokeDashed2 = new BasicStroke(
    PathBorderStrokeWidth,
    BasicStroke.CAP_BUTT,
    BasicStroke.JOIN_ROUND,
    10f, // mitre-limit
  ...
  public static void decorateOver(
    final LWComponent node,
    final DrawContext dc
  ) {
    if (PathwayAsDots || node instanceof LWLink) drawPathwayDot(node, dc);
  }

  public static void decorateUnder(
    final LWComponent node,
    final DrawContext dc
  ) {
    if (PathwayAsDots) return;

    if (
      node instanceof LWLink ||
      node instanceof LWPortal ||
      node instanceof LWGroup
  ...
  private static void drawPathwayBorder(LWComponent node, DrawContext dc) {
    ...
    dc.g.setStroke(
      new BasicStroke(
        PathBorderStrokeWidth + node.getStrokeWidth(),
        BasicStroke.CAP_ROUND,
        BasicStroke.JOIN_ROUND
      )
    );
  ...
  private static void drawPathwayDot(LWComponent node, DrawContext dc) {
    ...
    if (visiblePathMemberships > 1) {
      ...
      dc.g.setColor(onlyPathway.getStrokeColor());
      final RectangularShape dot = new java.awt.geom.Ellipse2D.Float(
        0,
        0,
        DotSize,
        DotSize
  ...
  private static void setConnectionPoint(
    Line2D.Float line,
    boolean end,
    LWComponent c
  ) {
    ...
    for (Entry e : mEntries) {
      ...
      if (last != null && last.isDrawn() && next.isDrawn()) {
        ...
        if (PathwayAsDots) {
          ...
          VueUtil.clipEnds(
            connector,
            DotRadius * Math.min(last.getMapScale(), next.getMapScale())
          );
        } else {
