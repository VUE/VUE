...
public class PathwayTableModel
  ...
  static final String[] ColumnNames = { "A", "B", "C", "D", "E", "F" };
  ...
  private void setMap(LWMap map) {
    if (mMap == map) return;
    if (mMap != null) mMap.getPathwayList().removeListener(this);
    mMap = map;
    if (mMap != null) mMap.getPathwayList().addListener(this);
  ...
  public void LWCChanged(LWCEvent e) {
    ...
    if (e.key instanceof LWComponent.Key) {
      if (e.getKey().type == LWComponent.KeyType.DATA) fireChange = true;
    } else {
  ...
  Iterator<LWPathway> getPathwayIterator() {
    return mMap == null
      ? VueUtil.EmptyIterator
      : mMap.getPathwayList().iterator();
  }
...
*/
  ...
  LWPathway getPathwayForElementAt(int pRow) {
    ...
    throw new IllegalArgumentException(
      "Couldn't find any element at row " + pRow
    );
  }
  ...
  int getPathwayIndexForElementAt(int pRow) {
    ...
    while (i.hasNext()) {
      LWPathway p = (LWPathway) i.next();
      if (row++ == pRow) return -1;
      if (p.isOpen()) {
        for (int index = 0; index < p.length(); index++) if (
          row++ == pRow
        ) return index;
      }
    }
  ...
  LWPathway.Entry getEntry(int pRow) {
    ...
    while (i.hasNext()) {
      LWPathway p = i.next();
      if (row++ == pRow) return p.asEntry();
      if (p.isOpen()) {
        for (LWPathway.Entry e : p.getEntries()) {
          if (row++ == pRow) return e;
        }
      }
    }
    return null;
  ...
  int getRow(LWPathway.Entry findEntry) {
    ...
    while (i.hasNext()) {
      LWPathway p = i.next();
      if (findEntry.isPathway() && findEntry.pathway == p) return row;
      row++;
      if (p.isOpen()) {
        for (LWPathway.Entry e : p.getEntries()) {
          if (e == findEntry) return row;
          row++;
        }
      }
    }
  ...
  public Class getColumnClass(int col) {
    if (col == COL_COLOR) return Color.class; else if (
      col == COL_VISIBLE || col == COL_NOTES || col == COL_LOCKEDnMAPVIEW
    ) return ImageIcon.class; else if ( // || col == COL_MAPVIEW)
      col == COL_LABEL || col == COL_OPEN
    ) return Object.class;
  ...
  public boolean isCellEditable(int row, int col) {
    if (getPathwayList() != null) {
      ...
      if (entry.pathway.isLocked()) return false;
      ...
      if (entry.isPathway()) return col == COL_COLOR || col == COL_LABEL; // || col == COL_REVEALER;
    }
    return false;
  }
  ...
  public boolean containsPathwayNamed(String label) {
    ...
    while (i.hasNext()) {
      LWPathway p = (LWPathway) i.next();
      if (p.getLabel().equals(label)) return true;
    }
    return false;
  }
  ...
  public synchronized Object getValueAt(int row, int col) {
    ...
    if (entry.isPathway()) {
      ...
      try {
        switch (col) {
          case COL_VISIBLE:
            return p.isVisible() ? Boolean.TRUE : Boolean.FALSE;
          case COL_COLOR:
            return p.getStrokeColor();
          case COL_OPEN:
            return p.isOpen() ? Boolean.TRUE : Boolean.FALSE;
          case COL_LABEL:
            return p.getDisplayLabel();
          case COL_NOTES:
            return p.hasNotes() ? Boolean.TRUE : Boolean.FALSE;
          case COL_LOCKEDnMAPVIEW:
            return p.isLocked() ? Boolean.TRUE : Boolean.FALSE;
        ...
        e.printStackTrace();
        System.err.println(
          "exception in the table model, setting pathway cell:" + e
        );
      }
    } else {
      try {
        if (col == COL_LABEL) return entry.getLabel(); else if (
          col == COL_LOCKEDnMAPVIEW
        ) return entry.isMapView() ? Boolean.TRUE : Boolean.FALSE;
      } catch (Exception e) {
        e.printStackTrace();
        System.err.println(
          "exception in the table model, setting pathway element cell:" + e
        );
      }
    }
    return null;
  ...
  public void setValueAt(Object aValue, int row, int col) {
    if (DEBUG.PATHWAY) System.out.println(
      this + " setValutAt " + row + "," + col + " " + aValue
    );
    ...
    if (entry.isPathway()) {
      LWPathway p = entry.pathway;
      boolean bool = false;
      if (aValue instanceof Boolean) bool = ((Boolean) aValue).booleanValue();

      if (col == COL_VISIBLE) {
        p.setVisible(!p.isVisible());
      } // not proper (must use aValue to be proper)
      else if (col == COL_COLOR) {
        p.setStrokeColor((Color) aValue);
      } // proper
      else if (col == COL_OPEN) {
        p.setOpen(!p.isOpen());
      } // not proper
      else if (col == COL_LABEL) {
        entry.setLabel((String) aValue);
      } // proper
      ...
      if (col == COL_LABEL) entry.setLabel((String) aValue); else if (
        col == COL_LOCKEDnMAPVIEW
      ) {
