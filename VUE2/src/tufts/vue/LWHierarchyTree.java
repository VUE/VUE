...
public class LWHierarchyTree extends JPanel implements TreeModelListener {
    ...
    tree.addTreeSelectionListener(
      new TreeSelectionListener() {
        public void valueChanged(TreeSelectionEvent e) {
          ...
          if (treeNode == null) selectedNode = null; else {
            //retrieves the LWComponent associated with the selected tree node
            selectedNode =
              (tufts.oki.hierarchy.HierarchyNode) treeNode.getUserObject();
  ...
  public void setHierarchyModel(
    tufts.oki.hierarchy.HierarchyViewHierarchyModel hierarchyModel
  ) {
    if (hierarchyModel != null) {
      DefaultTreeModel model = hierarchyModel.getTreeModel();
      model.addTreeModelListener(this);
      tree.setModel(model);
    } else tree.setModel(null);
  }
}
