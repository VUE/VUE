 ...
 *
 * @version $Revision: 1.42 $ / $Date: 2010-02-03 19:17:40 $ / $Author: mike $
 * @author Mark Norton
 */
public class CabinetResource extends URLResource {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(CabinetResource.class);

  // Filing metadata property keywords.
  public static final String MD_NAME =
    DublinCoreConstants.DC_FIELDS[DublinCoreConstants.DC_TITLE];
  public static final String MD_OWNER =
    DublinCoreConstants.DC_FIELDS[DublinCoreConstants.DC_CREATOR];
  public static final String MD_MIME =
    DublinCoreConstants.DC_FIELDS[DublinCoreConstants.DC_TYPE];
  ...
  // private final transient osid.filing.CabinetEntry entry; // This is not
  // marshalled.
  ...
  public CabinetResource() {
    ...
    // this.entry = null; // entries are currently never marshalled, so we don't
  ...
  private CabinetResource(final osid.filing.CabinetEntry e) {
    ...
    if (e instanceof tufts.oki.remoteFiling.RemoteCabinetEntry) initFrom(
      (RemoteCabinetEntry) e
    ); else if (
      e instanceof tufts.oki.localFiling.LocalByteStore
    ) setSpecByKnownFile(((LocalByteStore) e).getFile(), false); else if (
      e instanceof tufts.oki.localFiling.LocalCabinet
    ) setSpecByKnownFile(((LocalCabinet) e).getFile(), true);
  }
  ...
  static CabinetResource create(final osid.filing.CabinetEntry entry) {
    ...
    if (DEBUG.RESOURCE) Log.debug("created: " + r + "; from: " + entry);
    // else
  ...
  // if (DEBUG.RESOURCE) Log.debug("created: " + r + "; from: " +
  // tufts.Util.tags(file));
  // return r;
  // }
  ...
  // String name = file.getName(); // Get the filename with out path.
  // if(name.lastIndexOf('.')> -1)
  // this.extension = name.substring (name.lastIndexOf ('.')+1); // Extract
  // extention.
  // else if(name.length() > 0)
  // this.extension = "none";
  // else
  // return null; // this is case where there is no file. useful for castor
  // save/restore
  // }
  ...
  // /**
  // * Return the metadata properties associated with this object. Metadata is
  // extracted
  // * from the various flavors of CabinetEntry and collected into a Properties
  // object,
  // * which is returned. Each metadata element has a property keyword defined
  // above.
  // *
  // * @author Mark Norton
  // */
  ...
  // // props.setProperty(CabinetResource.MD_TIME,
  // cab.getLastAccessedTime().toString());
  // }
  ...
  // props.setProperty(CabinetResource.MD_NAME, bs.getDisplayName());
  // //props.setProperty(CabinetResource.MD_TIME,
  // Long.toString(bs.getLastAccessedTime().getTimeInMillis()));
  // props.setProperty(CabinetResource.MD_OWNER, bs.getOwner().getDisplayName());
  // /**
  // if (bs.isReadable())
  // props.setProperty(CabinetResource.MD_READ, "true");
  ...
  // /*
  // * Return the resource specification. For cabinet resources, this is URL of
  // either
  ...
  // if (e instanceof tufts.oki.remoteFiling.RemoteByteStore)
  // setSpec(((RemoteByteStore)e).getUrl());
  // else if (e instanceof tufts.oki.remoteFiling.RemoteCabinet)
  // setSpec(((RemoteCabinet)e).getUrl());
  // else if (e instanceof tufts.oki.localFiling.LocalByteStore)
  // setSpecByKnownFile(((LocalByteStore)e).getFile(), false);
  // else if (e instanceof tufts.oki.localFiling.LocalCabinet)
  // setSpecByKnownFile(((LocalCabinet)e).getFile(), true);
  ...
  private void openRemoteByteStore()
    throws IOException, osid.filing.FilingException {
