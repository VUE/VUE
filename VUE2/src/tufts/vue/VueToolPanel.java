...
public class VueToolPanel extends JPanel {
  ...
  public VueToolPanel() {
    ...
    setOpaque(false);
    if (GUI.isMacBrushedMetal()) setBorder(new EmptyBorder(0, 3, 2, 10)); //tlbr
    else setBorder(new EmptyBorder(0, 3, 0, 10)); //tlbr
  ...
  public void createToolButton(
    PaletteButton pButton,
    boolean addToMainToolbar
  ) {
    if (debug) pButton.setBackground(Color.magenta); else {
      GUI.applyToolbarColor(pButton);
    }

    if (addToMainToolbar) add(pButton);
  ...
  public void addTools(VueTool[] pTools, int[] separators) {
    ...
    for (int i = 0; i < pTools.length + separators.length; i++) {
      if (
        (separatorIndex < separators.length) && i == separators[separatorIndex]
      ) {
        addSeparator();
        separatorIndex++;
      } else addTool(pTools[i - separatorIndex], true);
    }
  }
  ...
  public void setSelectedTool(VueTool pTool) {
    if (pTool != null) {
      if (pTool.getParentTool() != null) {
        ...
        PaletteButton button = (PaletteButton) mToolButtons.get(
          parentTool.getID()
        );

        //PaletteButtonItem[] items = button.getPaletteButtonItems();
        button.setPropertiesFromItem(
          (PaletteButton) mToolButtons.get(pTool.getID())
        );

        parentTool.setSelectedSubTool(pTool);
        //button.setContext( pTool);
        if (button != null) mButtonGroup.setSelected(button.getModel(), true);
  ...
  protected PaletteButton createPaletteButton(VueTool pTool) {
    ...
    if (pTool.hasSubTools()) {
      ...
      button.setOverlayIcons(
        pTool.getOverlayUpIcon(),
        pTool.getOverlayDownIcon()
      );
      ...
      button.setIcons(
        pTool.getIcon(),
        pTool.getDownIcon(),
        pTool.getSelectedIcon(),
        pTool.getDisabledIcon(),
        pTool.getRolloverIcon()
      );
    }
  ...
  private void debug(String pStr) {
    if (sDebug) System.out.println("VueToolPanel - " + pStr);
  }
}
