...
public class TextBox
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(TextBox.class);
  ...
  TextBox(LWComponent lwc, String text) {
    if (DEBUG.TEXT && DEBUG.LAYOUT) tufts.Util.printClassTrace(
      "tufts.vue.",
      "NEW TextBox, txt=" + text
    );
    ...
    if (
      VueUtil.isWindowsPlatform() && SelectionColor != null
  ...
  public void addNotify() {
    ...
    if (getText().length() < 1) setText("<label>");
    keyWasPressed = false;
    Dimension size = getSize();
    super.addNotify();
    // note: we get a a flash/move if we add the border before the super.addNotify()
    if (false && TestDebug); //setBorder(javax.swing.BorderFactory.createLineBorder(Color.green));
    else {
    ...
    if (background == null && nodeParent != null) background =
      nodeParent.getRenderFillColor(null); // todo: only handles 1 level transparent embed!
    ...
    int w = getWidth() + 1;
    if (w < preferred.width) mKeepTextWidth = true; else mKeepTextWidth = false;
    if (TestDebug || DEBUG.TEXT) out(
      "width+1=" +
      w +
      " < preferred.width=" +
      preferred.width +
  ...
  public void removeNotify() {
    ...
    } else mFirstAfterAddNotify = false;
    ...
    if (wasOpaque != isOpaque()) setOpaque(wasOpaque);
    if (TestDebug || DEBUG.TEXT) out(
      "*** REMOVENOTIFY end: insets=" + getInsets()
    );
  }
  ...
  private static void setFontAttributes(
    MutableAttributeSet a,
    Font f,
    LWComponent c
  ) {
    if (DEBUG.TEXT) System.out.println("setFontAttribnutes " + f);
    ...
    if (c != null) {
      ...
      if (s.equals("underline")) StyleConstants.setUnderline(
        a,
        true
      ); else StyleConstants.setUnderline(a, false);
    }
  }
  ...
  void copyStyle(LWComponent c) {
    ...
    SimpleAttributeSet a = new SimpleAttributeSet();
    if (
      TestHarness || (c instanceof LWNode && ((LWNode) c).isTextNode())
    ) StyleConstants.setAlignment(
      a,
      StyleConstants.ALIGN_LEFT
    ); else StyleConstants.setAlignment(a, StyleConstants.ALIGN_CENTER);
  ...
  private void computeMinimumWidth(Font font, String text) {
    mMaxCharWidth =
      (float) font.getMaxCharBounds(DefaultFontContext).getWidth();
    try {
  ...
  private float maxWordWidth(Font font, String text) {
    if (text == null || text.length() == 0) return mMaxCharWidth;

    if (
      text.length() > 512
    ) return mMaxCharWidth * BigWordLen; // provide a rough figure if string is long

    if (
      text.indexOf(' ') < 0 && text.indexOf('\n') < 0
    ) return (float) font.getStringBounds(text, DefaultFontContext).getWidth(); // if no spaces, specal case no wrapping
    ...
    for (int i = 0; i <= len; i++) {
      ...
      if (DebugWord) out(
        "char[" + c + "] ci=" + curRunIdx + " cl=" + curRunLen
      );

      // add '/' as word break character if no whitespace?
      if (c == 0 || Character.isWhitespace(c) || c == '.' || c == ','); // treat as a word break
      else continue;
      ...
      if (c != 0) { // if we're not at the end
        try {
          ...
          do {
            ...
            if (DebugWord) out(
              "char{" + whiteChar + "} ci=" + curRunIdx + " cl=" + curRunLen
            );
      ...
      if (DebugWord) out(
        "word[" +
        text.substring(curRunIdx, curRunIdx + curRunLen) +
        "] w=" +
        wordWidth
      );
      ...
      if (wordWidth > maxWidth) {
        if (c == 0 && curRunIdx == 0) {
          ...
          return wordWidth < mMaxCharWidth * BigWordLen
            ? wordWidth
            : mMaxCharWidth * BigWordLen;
        } else {
          maxWidth = wordWidth;
          maxRunIdx = curRunIdx;
          maxRunLen = curRunLen;
          if (DebugWord) out(
            "MI=" + curRunIdx + " ML=" + curRunLen + " w=" + wordWidth
          );
        }
      }
      curRunIdx = i + 1;
    ...
    if (DebugWord || DEBUG.TEXT) out(
      "maxWord[" +
      text.substring(maxRunIdx, maxRunIdx + maxRunLen) +
      "] w=" +
      maxWidth
    );
  ...
  public void doLayout() {
    if (getParent() instanceof MapViewer) {
      ...
      if (false) out("skipping size fix-up"); else {
        if (WrapText) adjustSizeDynamically(); else {
          setSize(getPreferredSize());
          setSize(getPreferredSize());
        }
      }
    } else {
      if (!TestHarness) new Throwable(this + " UNPARENTED doLayout")
        .printStackTrace();
    }
    //super.layout();
  }
  ...
  private void adjustSize(boolean keepTextWidth) {
    ...
    if (TestDebug || DEBUG.TEXT) out(
      "adjustTo",
      newSize,
      keepTextWidth ? "(keep width)" : "(expand width)"
    );
    setSize(newSize);
  ...
  private Container removeAsEdit() {
    ...
    if (parent != null) parent.remove(this); else out(
      "FAILED TO FIND PARENT ATTEMPTING TO REMOVE SELF"
    );
    return parent;
  }
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
      ...
      e.consume();
      String t = getText();
      if (e.isShiftDown()) setText(t.toUpperCase()); // upper whole string
      else setText(Character.toTitleCase(t.charAt(0)) + t.substring(1)); // upper first char
    } else keyWasPressed = true;
  ...
  public void focusLost(FocusEvent e) {
    if (TestDebug || DEBUG.FOCUS) outc(
      "focusLost to " + e.getOppositeComponent()
    );
    if (TestHarness == false && getParent() != null) getParent().remove(this);
    if (keyWasPressed) { // TODO: as per VueTextField, need to handle drag & drop detect
      ...
      if (TestDebug || DEBUG.FOCUS) out(
        "key was pressed; setting label to: [" + getText() + "]"
      );
  ...
  public void focusGained(FocusEvent e) {
    if (TestDebug || DEBUG.FOCUS) outc(
      "focusGained from " + e.getOppositeComponent()
    );
  }
  ...
  public Dimension getPreferredSize() {
    ...
    if (getParent() != null) s.width += 1; // leave room for cursor, which at least on mac gets clipped if at EOL
  ...
  public void reshape(int x, int y, int w, int h) {
    if (TestDebug || DEBUG.TEXT) {
      ...
      if (DEBUG.META && change) new Throwable("reshape").printStackTrace();
    }
    super.reshape(x, y, w, h);
    if (TestDebug || DEBUG.TEXT) {
      Rectangle b = getBounds();
      if (b.x != x || b.y != y || b.width != w || b.height != h) out(
        "BADBOUNDS " + tufts.Util.out(b)
      );
    }
  }
  ...
  public void setBoxCenter(float x, float y) {
    setBoxLocation(x - getBoxWidth() / 2, y - getBoxHeight() / 2);
  }
  ...
  public void paintComponent(Graphics g) {
    if (TestDebug || DEBUG.TEXT) out(
      "paintComponent @ " + getX() + "," + getY() + " parent=" + getParent()
    );

    final MapViewer viewer =
      (MapViewer) javax.swing.SwingUtilities.getAncestorOfClass(
        MapViewer.class,
        this
      );
    if (viewer != null) ((Graphics2D) g).setRenderingHint(
  ...
  public void draw(DrawContext dc) {
    ...
    if (getParent() != null) System.err.println(
      "Warning: 2nd draw of an AWT drawn component!"
    );
  ...
  public void changedUpdate(DocumentEvent de) {
    if (TestDebug || DEBUG.TEXT) out(
      "changeUpdate " + de.getType() + " len=" + de.getLength()
    );
    handleChange();
  }
  ...
  private static class TestPanel extends javax.swing.JPanel {
    ...
    public void paint(Graphics g) {
      ...
      g.drawString(
        d.width + "x" + d.height + " preferred size",
        box.getX(),
        box.getY() - 2
      );
      g.setColor(Color.blue);
      g.drawString(
        box.getWidth() + "x" + box.getHeight() + " size",
        box.getX(),
        box.getY() + box.getHeight() + 11
      );
    }
  }
  ...
  public static void main(String args[]) {
    ...
    tufts.vue.gui.DockWindow w = GUI.createDockWindow(
      "TextBox Resize Test",
      new TestPanel(box)
    );
    w.setVisible(true);
  ...
  private final JMenuItem copyItem = new JMenuItem(
    VueResources.getString("richtextbox.menu.copy")
  );
  private final JMenuItem pasteItem = new JMenuItem(
    VueResources.getString("richtextbox.menu.paste")
  );

  private JPopupMenu getPopup(MouseEvent e) {
    if (m == null) {
      m =
        new JPopupMenu(VueResources.getString("richtextbox.menu.textboxmenu"));
