...
public class AnalyzerAction extends Actions.LWCAction {
	...
	public static final AnalyzerAction calais = new AnalyzerAction(new OpenCalaisAnalyzer(),
			VueResources.getString("analyzeaction.usingopencalais"), null);
	public static final AutoTaggerAction calaisAutoTagger = new AutoTaggerAction(new OpenCalaisAnalyzer(),
			VueResources.getString("analyzeaction.usingopencalais"), null);
	public static final SemanticMapAction calaisMapAction = new SemanticMapAction(new OpenCalaisAnalyzer(),
			VueResources.getString("analyzeaction.usingopencalais"), null);

	private static final LWComponentAnalyzer alchemyAnalyzer = new AlchemyAnalyzerAPIKeyGuarder();
	private static final AnalyzerAction alchemy = new AnalyzerAction(alchemyAnalyzer,
			VueResources.getString("analyzeaction.usingalchemyapi"), null);
	private static final AutoTaggerAction alchemyAutoTagger = new AutoTaggerAction(alchemyAnalyzer,
			VueResources.getString("analyzeaction.usingalchemyapi"), null);
	private static final SemanticMapAction alchemyMapAction = new SemanticMapAction(alchemyAnalyzer,
			VueResources.getString("analyzeaction.usingalchemyapi"), null);
	...
	public static final Action luckyImageAction = new LWCAction(VueResources.getString("analyzeaction.luckyimage")) {
		public void act(LWComponent c) {
			JSONArray _ja;
			try {
				...
				for (int i = 0; i < resultCount; i++) {
					...
					String big = "http://farm" + farm + ".static.flickr.com/" + server + "/" + id + "_" + secret
							+ ".jpg";
		...
		public void act(LWComponent c) {
			try {
				...
				try {
					...
					VueUtil.alert(VueResources.getString("dialog.semanticmaperror.message"),
							VueResources.getString("dialog.analyzeerror.title"));
					e.printStackTrace();
					return;
				}
				...
				if (list.isEmpty()) {
					VueUtil.alert(VueResources.getString("dialog.semanticmaperror.noresults"),
							VueResources.getString("dialog.analyzeerror.title"));
					return;
	...
	static private class AlchemyAnalyzerAPIKeyGuarder extends AlchemyAnalyzer {
		...
		public List<AnalyzerResult> analyze(LWComponent c, boolean tryFallback) {
			return (IsAlchemyAPIKeySet() || keyRequest()) ? super.analyze(c, tryFallback)
					: new java.util.ArrayList<AnalyzerResult>();
		}

		@Override
		public List<AnalyzerResult> analyze(LWComponent c) {
			return (IsAlchemyAPIKeySet() || keyRequest()) ? super.analyze(c)
					: new java.util.ArrayList<AnalyzerResult>();
		}
		...
		private boolean keyRequest() {
			String key = (String) edu.tufts.vue.preferences.implementations.AlchemyAPIPreference.getInstance()
					.getValue();
