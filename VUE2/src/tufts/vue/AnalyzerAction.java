...
public class AnalyzerAction extends Actions.LWCAction {
  ...
  private AnalyzerAction(
    LWComponentAnalyzer analyzer,
    String name,
    KeyStroke keyStroke
  ) {
  ...
  boolean enabledFor(LWSelection s) {
    return (s.size() == 1 && s.first() instanceof LWNode); // todo: a have capability check (free-layout? !isLaidOut() ?)
  }

  private static final int maxLabelLineLength = 30;
  private static final Color[] ValueNodeDataColors = VueResources.getColorArray(
    "node.dataValue.color.cycle"
  );

  public static void addResultsToMap(
    List<Resource> resourceList,
    LWComponent centerComponent,
    int colorSelection
  ) {
    // *** MAP BASED SEARCHING ***
  ...
  public void act(LWComponent c) {
    // if (VUE.getDRBrowser().getDataSourceViewer().mMapBasedSearchThreads.size() >
    // 0)
    // {
    ...
    if (selectionCount == 0) {
      final ShowAgainDialog sad = new ShowAgainDialog(
        VUE.getApplicationFrame(),
        "noResourceSelected",
        VueResources.local("noResourceSelected.title"),
        "OK",
        null
      );
      JPanel panel = new JPanel(new GridLayout(1, 1));
      String label = Util.formatLines(
        VueResources.local("noResourceSelected.message"),
        30
      );
     ...
     * if (l.getValue() !=null && l.getValue().trim() != " " &&
    ...
    // if (!hasResults)
    // {
    // VueUtil.alert(VueResources.getString("dialog.analyzeerror.message"),
    // VueResources.getString("dialog.analyzeerror.title"));
  ...
  // public static final AnalyzerAction yahoo = new AnalyzerAction(new
  // YahooAnalyzer(),VueResources.getString("analyzeaction.yahoocontent"),null);
  public static final AnalyzerAction calais = new AnalyzerAction(
    new OpenCalaisAnalyzer(),
    VueResources.getString("analyzeaction.usingopencalais"),
    null
  );
  public static final AutoTaggerAction calaisAutoTagger = new AutoTaggerAction(
    new OpenCalaisAnalyzer(),
    VueResources.getString("analyzeaction.usingopencalais"),
    null
  );
  public static final SemanticMapAction calaisMapAction = new SemanticMapAction(
    new OpenCalaisAnalyzer(),
    VueResources.getString("analyzeaction.usingopencalais"),
    null
  );

  private static final LWComponentAnalyzer alchemyAnalyzer =
    new AlchemyAnalyzerAPIKeyGuarder();
  private static final AnalyzerAction alchemy = new AnalyzerAction(
    alchemyAnalyzer,
    VueResources.getString("analyzeaction.usingalchemyapi"),
    null
  );
  private static final AutoTaggerAction alchemyAutoTagger =
    new AutoTaggerAction(
      alchemyAnalyzer,
      VueResources.getString("analyzeaction.usingalchemyapi"),
      null
    );
  private static final SemanticMapAction alchemyMapAction =
    new SemanticMapAction(
      alchemyAnalyzer,
      VueResources.getString("analyzeaction.usingalchemyapi"),
      null
    );
  ...
  public static final Action luckyImageAction = new LWCAction(
    VueResources.getString("analyzeaction.luckyimage")
  ) {
    public void act(LWComponent c) {
      JSONArray _ja;
      try {
        ...
        GetMethod method = new GetMethod(
          "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=093a05d30b0be54c8b18d227fec80b6b&tag_mode=any&sort=relevance&per_page=1&format=json&nojsoncallback=1&text=" +
          URLEncoder.encode(c.getLabel())
        );
  ...
  public static final Action[] ANALYZER_ACTIONS = { calais };
    ...
    public void act(LWComponent c) {
      try {
        ...
        try {
          ...
          VueUtil.alert(
            VueResources.getString("dialog.semanticmaperror.message"),
            VueResources.getString("dialog.analyzeerror.title")
          );
          e.printStackTrace();
          return;
        ...
        if (list.isEmpty()) {
          VueUtil.alert(
            VueResources.getString("dialog.semanticmaperror.noresults"),
            VueResources.getString("dialog.analyzeerror.title")
          );
          return;
        }
        ...
        while (categoryChildren != null) {
          ...
          while (nodeIterator.hasNext()) {
            ...
            while (subCatIt.hasNext()) {
              ...
              if (
                res.getSubtypes() == null || res.getSubtypes().size() <= layerIx
              ) {
                ...
                if (rel < 0.25) mlist.add("relevance", "Low"); else if (
                  rel > 0.25 && rel < 0.50
                ) mlist.add("relevance", "Medium"); else if (
                  rel > 0.50 && rel < 0.75
                ) mlist.add("relevance", "High"); else mlist.add(
                  "relevance",
                  "Essential"
                );
                ...
                if (subtypeNode == null) {
                  ...
                  tufts.vue.LWLink link = new tufts.vue.LWLink(
                    joinNode,
                    subtypeNode
                  );
                ...
                Collection subtypeCollection = newCategoryChildren.get(
                  subtypeNode
                );
  ...
  static class AutoTaggerAction extends Actions.LWCAction {
    ...
    public void act(LWComponent c) {
      ...
      if (!hasResults) {
        VueUtil.alert(
          VueResources.getString("dialog.analyzeerror.message"),
          VueResources.getString("dialog.analyzeerror.title")
        );
      }
  ...
  private static class AlchemyAnalyzerAPIKeyGuarder extends AlchemyAnalyzer {

    @Override
    public List<AnalyzerResult> analyze(LWComponent c, boolean tryFallback) {
      return (IsAlchemyAPIKeySet() || keyRequest())
        ? super.analyze(c, tryFallback)
        : new java.util.ArrayList<AnalyzerResult>();
    }
    ...
    public Multimap<String, AnalyzerResult> analyzeResource(LWComponent c)
      throws Exception {
      if (!IsAlchemyAPIKeySet()) keyRequest();
      return super.analyzeResource(c);
    }

    private boolean keyRequest() {
      String key =
        (String) edu.tufts.vue.preferences.implementations.AlchemyAPIPreference
          .getInstance()
          .getValue();
      if (null == key || "".equals(key)) {
        String title = VueResources.local("dialog.alchemyapikey.title"), lable =
          VueResources.local("dialog.alchemyapikey.label");

        key =
          (String) VueUtil.input(
            VUE.getApplicationFrame(),
            lable,
            title,
        ...
        if (null == key || key.length() <= 0) return false;
      }
