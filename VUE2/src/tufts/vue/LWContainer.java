...
public abstract class LWContainer extends LWComponent {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(LWContainer.class);
  ...
  public LWComponent getChild(int index) {
    if (mChildren.size() > index) return mChildren.get(index); else return null;
  }
  ...
  protected void notifyMapLocationChanged(
    ...
    if (hasChildren()) {
      for (LWComponent c : getChildren()) c.notifyMapLocationChanged(
        src,
        mdx,
        mdy
      ); // is overcalling updateConnectedLinks (+1 for each depth!), but it's cheap
    }
  }
  ...
  protected void updateConnectedLinks(LWComponent movingSrc) {
    ...
    if (updatingLinks()) {
      ...
      for (LWComponent c : getChildren()) c.updateConnectedLinks(movingSrc);
    }
  }
  ...
  public void reparentTo(
    final LWContainer newParent,
    Collection<LWComponent> possibleChildren
  ) {
    if (newParent == null) {
      Log.warn(
        this + "; reparentTo: null new parent reparenting " + possibleChildren
      );
      return;
    }
    if (newParent == this) {
      ...
      Log.warn(
        this +
        "; reparentTo: attempting to reparent back to ourself: " +
        possibleChildren
      );
      return;
    }
    ...
    for (LWComponent c : possibleChildren) {
      if (c.getParent() == this) reparenting.add(c);
    }
  ...
  public void takeAllChildren(LWContainer source) {
    if (source == this) throw new IllegalArgumentException(
      "takeAllChildren: source is target: " + this
    );
  ...
  protected Collection<? extends LWComponent> sortForIncomingZOrder(
    ...
    if (toAdd.size() == 0) return Collections.EMPTY_LIST;
  ...
  public void addChildren(
    ...
    if (toAdd == null || toAdd.size() < 1) return;

    notify(LWKey.HierarchyChanging);

    if (mChildren == NO_CHILDREN) mChildren = new ArrayList(toAdd.size());

    if (toAdd.size() > 1 && context != ADD_PRESORTED) toAdd =
      sortForIncomingZOrder(toAdd);
    ...
    for (LWComponent c : toAdd) {
      ...
      try {
        ...
        if (c.getCreated() == 0) c.setCreated(timestamp);
  ...
  public void notifyHierarchyChanged() {
    ...
    for (LWComponent c : getChildren()) c.notifyHierarchyChanged();
  }
  ...
  protected void addChildImpl(LWComponent c, Object context) {
    ...
    if (c == this) throw new Error("attempt to add self as child");

    if (context == ADD_MERGE && c.hasAncestor(this)) return;
  ...
  protected void localizeCoordinates(
    LWComponent c,
    ...
    if (getParent() == null) {
      ...
      if (DEBUG.Enabled) Log.warn(
        "coordinate localization in unparented (new?) LWContainer: " +
        this +
        "; for " +
        c
      );
    ...
    if (DEBUG.PARENTING || DEBUG.CONTAINMENT) out(
      "localizing coordinates: " + c + " oldParent=" + oldParent
    );

    final LWComponent eventSource;

    // c.getMap() should == getMap() at this point; setParent to this LWContainer has been done above
    if (c.getMap() != getMap()) Util.printStackTrace(
      "different maps? " + c.getMap() + " != " + getMap()
    );
    ...
    if (c.getID() != null && c.getMap() == null) {
      ...
      if (oldParent == null || oldParent.getMap() == null) {
        if (DEBUG.Enabled) Util.printStackTrace(
          "FYI: no event source for: " +
          c +
          ";\n\t           localizing new parent: " +
          this +
          ";\n\toldParent is not in model either: " +
          oldParent +
          ";\n\tlocation events will not be available for UNDO"
        );
        eventSource = c;
      } else eventSource = oldParent; // if the component has no map, it's not in the model yet, and nobody can hear it.
    } else eventSource = c;
    ...
    if (DEBUG.Enabled || DEBUG.PARENTING || DEBUG.CONTAINMENT) out(
      "         now localized: " + c
    );
  }
  ...
  protected void removeChildren(
    ...
    for (LWComponent c : iterable) {
      if (c.getParent() == this) {
        ...
        if (permanent) c.removeFromModel();
      } else if (!permanent) {
  ...
  protected void removeChildImpl(LWComponent c) {
    ...
    if (mChildren == NO_CHILDREN) {
      Util.printStackTrace(
        this + "; null child list is null trying to remove: " + c
      );
      return;
    }
    if (isDeleted()) {
      // just in case:
      new Throwable(this + " FYI: ZOMBIE PARENT DELETING CHILD " + c)
        .printStackTrace();
      return;
    }
  ...
  protected void removeChildrenFromModel() {
    ...
    for (LWComponent c : copy(getChildren())) c.removeFromModel();
  }
  ...
  protected void prepareToRemoveFromModel() {
    ...
    if (
      mChildren == VUE.ModelSelection
    ) throw new IllegalStateException("attempted to delete selection"); // todo: tmp debug
  }
  ...
  private void ensureLinksPaintOnTopOfAllParents() {
    ...
    for (LWComponent c : getChildren()) {
      ...
      if (c instanceof LWContainer) ensureLinksPaintOnTopOfAllParents(
        (LWContainer) c
      );
    }
  }
  ...
  private static void ensureLinksPaintOnTopOfAllParents(LWComponent component) {
    for (LWLink link : component.getLinks()) ensureLinkPaintsOverAllAncestors(
      link,
      component
    );
  }

  static void ensureLinkPaintsOverAllAncestors(
    LWLink link,
    LWComponent component
  ) {
    LWContainer parent1 = null;
    LWContainer parent2 = null;
    if (link.getHead() != null) parent1 = link.getHead().getParent();
    if (link.getTail() != null) parent2 = link.getTail().getParent();
    ...
    if (link.isParentChildLink()) return;
    /*
    ...
    if (commonParent == null) {
      System.out.println(
        "ELPOTOAP: ignoring link with no parent: " + link + " for " + component
      );
      return;
    }
    if (commonParent != component.getParent()) {
      ...
      LWComponent topMostParentThatIsSiblingOfLink =
        component.getParentWithParent(commonParent);
      if (topMostParentThatIsSiblingOfLink == null) {
        // this could happen for stuff in cutbuffer w/out parent?
        String msg =
          "FYI; ELPOTOAP couldn't find common parent for " + component;
        if (DEBUG.LINK) tufts.Util.printStackTrace(msg); else Log.info(msg);
      } else commonParent.ensurePaintSequence(
        topMostParentThatIsSiblingOfLink,
        link
      );
    }
  }
  ...
  public Collection<LWComponent> getAllDescendents(final ChildKind kind) {
    return getAllDescendents(
      kind,
      new java.util.ArrayList(numChildren() * 2),
      Order.TREE
    );
  }
  ...
  public Collection<LWComponent> getAllDescendents(
    ...
    if (DEBUG.PARENTING) Log.debug(
      "getAllDescendents " + kind + "," + order + "; in=" + Util.tags(bag)
    );
  ...
  protected int indexOf(Object c) {
    if (isDeleted()) throw new IllegalStateException(
      "*** Attempting to get index of a child of a deleted component!" +
      "\n\tdeleted parent=" +
      this +
      "\n\tseeking index of child=" +
      c
    );
  ...
  >() {
    public int compare(LWComponent c1, LWComponent c2) {
      ...
      if (parent1 == null) return Short.MIN_VALUE; else if (
        parent2 == null
      ) return Short.MAX_VALUE;
      if (parent1 == parent2) return (
        parent1.indexOf(c1) - parent1.indexOf(c2)
      ); else return parent1.getDepth() - parent2.getDepth();
    }
  };
  ...
  public static final Comparator ZOrderSorter = new Comparator<LWComponent>() {
    public int compare(LWComponent c1, LWComponent c2) {
      ...
      if (parent1 == parent2) return (
        parent1.getChildren().indexOf(c1) - parent2.getChildren().indexOf(c2)
      ); else return 0;
  ...
  protected static LWComponent[] sort(
    Collection<? extends LWComponent> bag,
  ...
  public boolean bringToFront(LWComponent c) {
    ...
    if (idx < 0 || idx == idxLast) return false;
  ...
  void ensurePaintSequence(LWComponent onBottom, LWComponent onTop) {
    if (mChildren == NO_CHILDREN) {
      Util.printStackTrace(
        "no children " + this + "; bot=" + onBottom + "; top=" + onTop
      );
      return;
    }
    ...
    if (onBottom.getParent() != this || onTop.getParent() != this) {
      System.out.println(
        this +
        "ensurePaintSequence: both aren't children " +
        onBottom +
        " " +
        onTop
      );
    ...
    if (bottomIndex < 0 || topIndex < 0) {
      if (DEBUG.Enabled) Util.printStackTrace(
        this +
        "ensurePaintSequence: both aren't in list! " +
        bottomIndex +
        " " +
        topIndex
      );
    ...
    if (topIndex == (bottomIndex - 1)) {
      if (DEBUG.PARENTING) out(
        "ensurePaintSequence: swapping adjacents " + onTop
      );
      notify(LWKey.HierarchyChanging);
      ...
      if (DEBUG.PARENTING) out(
        "ensurePaintSequence: re-inserting after botIndex " + onTop
      );
      notify(LWKey.HierarchyChanging);
      mChildren.remove(topIndex);
      // don't forget that after above remove the indexes have all been shifted down one
      if (bottomIndex >= mChildren.size()) mChildren.add(
        onTop
      ); else mChildren.add(bottomIndex, onTop);
      notify("hier.sequence");
    } else {
      if (DEBUG.PARENTING) out(
        "ensurePaintSequence: already sequenced: " + onTop
      );
    }
    //if (DEBUG.PARENTING) System.out.println("ensurepaintsequence: " + onBottom + " " + onTop);
  ...
  public LWContainer duplicate(CopyContext cc) {
    ...
    if (isPatcherOwner) cc.patcher.reconnectLinks();
  ...
  public String paramString() {
    if (hasChildren()) return (
      super.paramString() + " chld=" + numChildren()
