...
public class Screen {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Screen.class);
  ...
  public static Screen getScreen(int n) {
    final GraphicsDevice[] devices = genv().getScreenDevices();
    for (int i = 0; i < devices.length; i++) {
      if (i == n) return Screen.create(devices[i]);
    }
    return null;
  }
  ...
  public static GraphicsDevice getDeviceForWindow(Window w) {
    if (w == null) return getDefaultDevice();

    final GraphicsDevice[] devices = genv().getScreenDevices();

    if (devices.length < 2) return getDefaultDevice();
  ...
  public static GraphicsDevice getDeviceForRegion(final Rectangle region) {
    ...
    ) return getDefaultDevice(); else return getDeviceForRegion(
      genv().getScreenDevices(),
      region
    );
  }

  private static GraphicsDevice getDeviceForRegion(
    final GraphicsDevice[] devices,
    final Rectangle region
  ) {
    if (
      devices.length < 2 || region == null || region.isEmpty()
  ...
  public static GraphicsDevice getDeviceForPoint(final Point point) {
    if (point == null) return getDefaultDevice();
    ...
    for (int i = 0; i < devices.length; i++) {
      ...
      try {
        GraphicsConfiguration config = device.getDefaultConfiguration();
        if (config.getBounds().contains(point)) return device;
      } catch (Throwable t) {
        Log.error("scanning device: " + Util.tags(device));
      }
    }
  ...
  public Rectangle getVisibleBounds() {
    return new Rectangle(
      ...
      width,
      height - (topIn - top)
    ); // margin contents are exposed as mutable, so we re-compute margin.top
  }
  ...
  public boolean inTop(int y) {
    return y >= top && y <= topIn;
  }

  public boolean inLeft(int x) {
    return x >= left && x <= leftIn;
  }

  public boolean inBottom(int y) {
    return y <= bottom && y >= bottomIn;
  }

  public boolean inRight(int x) {
    return x <= right && x >= rightIn;
  }
  ...
  public String toString() {
    StringBuilder s = new StringBuilder(
      String.format(
        ...
        x,
        y,
        (device == null ? "n/a" : Util.quote(device.getIDstring()))
      )
    );
    ...
    if (margin.hashCode() == 0) {
      ...
      if (margin.top != 0) s.append(" top=" + margin.top); else s.append(' ');
  ...
  public static Insets boundsToInsets(Rectangle r) {
    return new Insets(r.y, r.x, r.y + r.height, r.x + r.width);
  }
  ...
  public static void dump(java.io.PrintStream ps) {
    ...
    for (int i = 0; i < screens.length; i++) {
      ps.format(
        ...
        i,
        screens[i],
        Util.fmt(screens[i].getMaxWindowBounds())
      );
    }
  }
}
