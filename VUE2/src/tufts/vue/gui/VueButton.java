...
public class VueButton
  extends JButton //public class VueButton extends com.jidesoft.swing.JideButton
//implements com.jidesoft.swing.Alignable // JIDE
{
  ...
  public VueButton(Action a) {
    ...
    if (largeIcon != null) setIcon(largeIcon);
    init((String) a.getValue(Action.ACTION_COMMAND_KEY));
  }

  public void setAsToolbarButton(boolean tb) {
    isToolbarButton = tb;
    if (isToolbarButton) super.setText(null);
  }

  public void setText(String text) {
    if (isToolbarButton) setToolTipText(text); else super.setText(text);
  }
  ...
  private static void init(AbstractButton b, String key) {
    ...
    if (key == null) {
      ...
      if (b.getName() == null) b.setName(b.getText());
  ...
  public static void installResourceConfiguration(
    AbstractButton b,
    String key
  ) {
    Icon i = null;
    ...
    if ((i = VueResources.getImageIcon(key + kRAW)) != null) {
      ...
      VueButtonIcon.installGenerated(
        ...
        i2,
        i3,
        VueResources.getSize(key + kSIZE)
      );
    } else {
      if ((i = VueResources.getImageIcon(key + kSELECTED)) != null) {
        b.setSelectedIcon(i);
      }
      if ((i = VueResources.getImageIcon(key + kUP)) != null) b.setIcon(i);
      if (
        (i = VueResources.getImageIcon(key + kDOWN)) != null
      ) b.setPressedIcon(i);
      if (
        (i = VueResources.getImageIcon(key + kDISABLED)) != null
      ) b.setDisabledIcon(i);
    }
  ...
  public String toString() {
    String label;
    if (key == null) {
      if (getAction() == null) label = "txt=" + getText(); else label =
        getAction().toString();
    } else label = key;
