...
public class StrokeMenuButton extends ComboBoxMenuButton<Float> {
  ...
  private static int sIconWidth = 24;
  private static int sIconHeight = 16;

  public StrokeMenuButton(
    float[] pValues,
    String[] pMenuNames,
    boolean pGenerateIcons,
    boolean pHasCustom
  ) {
    Float[] values = new Float[pValues.length];
    for (int i = 0; i < pValues.length; i++) values[i] = new Float(pValues[i]);
  ...
  public void displayValue(Float width) {
    ...
    if (width == null) mCurrentValue = new Float(1); else mCurrentValue = width;
