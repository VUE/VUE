...
public class DashBorder extends AbstractBorder {
  ...
  public void paintBorder(
    ...
    g.setColor(colorFGX);
    if (top) g.fillRect(x, x, width, thickness); // top
    if (bottom) g.fillRect(x, y + height - thickness, width, thickness); // bottom
    if (left) g.fillRect(x, y, thickness, height); // left
    if (right) g.fillRect(x + width - thickness, y, thickness, height); // right
