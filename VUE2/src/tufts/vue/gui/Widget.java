...
public class Widget extends javax.swing.JPanel {
  ...
  public static void setHidden(JComponent c, boolean hidden) {
    ...
    if (c instanceof Widget) ((Widget) c).setHidden(hidden); else setHiddenImpl(
      c,
      hidden
    );
  }
  ...
  protected static void setHiddenImpl(JComponent c, boolean hidden) {
    if (DEBUG.WIDGET) System.out.println(
      GUI.name(c) + " Widget.setHidden " + hidden
    );
  ...
  public static void setWantsScroller(JComponent c, boolean scroller) {
    if (DEBUG.WIDGET) System.out.println(
      GUI.name(c) + " Widget.setWantsScroller " + scroller
    );
  ...
  public static void setWantsScrollerAlways(JComponent c, boolean scroller) {
    if (DEBUG.WIDGET) System.out.println(
      GUI.name(c) + " Widget.setWantsScrollerAlways " + scroller
    );
  ...
  public static void setExpanded(JComponent c, boolean expanded) {
    // make sure instance method called in case it was overridden
    if (c instanceof Widget) ((Widget) c).setExpanded(
        expanded
      ); else setExpandedImpl(c, expanded);
  }
  ...
  protected static void setExpandedImpl(JComponent c, boolean expand) {
    if (DEBUG.WIDGET) System.out.println(
      GUI.name(c) + " Widget.setExpanded " + expand
    );
    ...
    if (expand && !tufts.vue.VUE.isStartupUnderway()) {
      if (isBooleanTrue(c, HIDDEN_KEY) || !c.isVisible()) setHidden(c, false);
      if (
        !DockWindow.AllWindowsHidden() &&
        !tufts.vue.VUE.inNativeFullScreen() &&
  ...
  public static void setMiscAction(
    JComponent c,
  ...
  public static void setMenuActions(
    JComponent c,
    javax.swing.Action[] actions
  ) {
    if (DEBUG.WIDGET) System.out.println(
      GUI.name(c) + " Widget.setMenuActions " + java.util.Arrays.asList(actions)
    );
  ...
  public static boolean isWidget(JComponent c) {
    return (
      c != null &&
      (c instanceof Widget || c.getClientProperty(EXPANSION_KEY) != null)
    );
  }
   ...
   */
  protected static boolean setBoolean(
    JComponent c,
    String key,
    boolean newValue
  ) {
    ...
    if (current != newValue) {
      if (DEBUG.WIDGET) System.out.println(
        GUI.name(c) + " WIDGET-CHANGE " + key + "=" + newValue
      );
  ...
  public Widget(String title, JComponent content) {
    ...
    if (DEBUG.BOXES) setBorder(
      new javax.swing.border.LineBorder(java.awt.Color.blue, 4)
    );
  }
  ...
  public static void setLoading(JComponent c, boolean loading) {
    // make sure instance method called in case it was overridden
    if (c instanceof Widget) ((Widget) c).setLoading(
        loading
      ); else setLoadingImpl(c, loading);
  }

  protected static void setLoadingImpl(JComponent c, boolean loading) {
    if (DEBUG.WIDGET) System.out.println(
      GUI.name(c) + " Widget.setLoading " + loading
    );
