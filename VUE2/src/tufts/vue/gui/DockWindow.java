...
public class DockWindow
  implements
    MouseListener,
    MouseMotionListener//, FocusManager.MouseInterceptor
    ,
    java.beans.PropertyChangeListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DockWindow.class);

  public static final int DefaultWidth = 300;
  private static boolean AllVisible = true;

  static final LinkedList<DockWindow> AllWindows = new LinkedList();
  static final char RightArrowChar = 0x25B8; // unicode
  static final char DownArrowChar = 0x25BE; // unicode
  //final static String RightArrow = "" + RightArrowChar;
  //final static String DownArrow = "" + DownArrowChar;

  public static final int ToolbarHeight = VueResources.getInt(
    "gui.dockToolbar.height",
    70
  );
  ...
  private static final Color TopGradientColor = VueResources.getColor(
    "gui.dockWindow.title.background.top"
  ), BottomGradientColor = VueResources.getColor(
    "gui.dockWindow.title.background.bottom"
  );
  ...
  private static final int TitleHeight = VueResources.getInt(
    "gui.dockWindow.title.height",
    19
  );
  private static final int ResizeCornerSize = VueResources.getInt(
    "gui.dockWindow.resizeCorner.size",
    12
  );
  private static final Font TitleFont = VueResources.getFont(
    "gui.dockWindow.title.font",
    tufts.vue.VueConstants.FONT_MEDIUM
  );
  private static final int MacAquaMetalMinHeight = 37; // Mac Aqua Brushed Metal window size bug
  ...
  private final class FramePeer extends JFrame implements Peer {
    ...
    FramePeer(String title, boolean alwaysOnTop, boolean decorated) {
      ...
      if (Util.isMacLeopard()) getRootPane()
    ...
    public void addNotify() {
      DockWindow.this.addNotify();
    }

    @Override
    public void validate() {
      DockWindow.this.validate();
    }

    @Override
    public void invalidate() {
      DockWindow.this.invalidate();
    }

    @Override
    public void setVisible(boolean v) {
      DockWindow.this.setVisible(v);
    }
    ...
    public void setBounds(int x, int y, int w, int h) {
      if (
        _peer == null
      ) super.setBounds(x, y, w, h); else DockWindow.this.setBounds(x, y, w, h); // can happen during init
    }

    public void peer_addNotify() {
      super.addNotify();
    }

    public void peer_validate() {
      super.validate();
    }

    public void peer_invalidate() {
      super.invalidate();
    }
    ...
    public void peer_setBounds(int x, int y, int w, int h) {
      super.setBounds(x, y, w, h);
    }

    public String toString() {
      return GUI.name(this);
    }

    public DockWindow getDock() {
      return DockWindow.this;
    }
  }
  ...
  private final class DialogPeer extends JDialog implements Peer {
    ...
    DialogPeer(
      ...
      if (
        Util.isMacLeopard()
      ) getRootPane().putClientProperty("Window.style", "small"); // will only have effect when decorated
  ...
  public DockWindow(
    ...
    if (Util.isUnixPlatform()) {
      _peer =
        new DialogPeer(VUE.getApplicationFrame(), title, !ON_TOP, !DECORATED);
     ...
     */
    if (
      Util.isWindowsPlatform() && Util.getJavaVersion() < 1.6
    ) _win.setBackground(Color.lightGray);

    if (CollapsedHeight == 0) staticInit();
    ...
    wpp =
      WindowPropertiesPreference.create(
        "windows",
    ...
    if (true) _peer.setContentPane(mContentPane); else _peer.setContentPane(
      new Box(BoxLayout.Y_AXIS)
    );

    setResizeEnabled(!isToolbar);

    if (DEBUG.INIT || DEBUG.DOCK) out(
      "constructed (child of " + GUI.name(owner) + ")"
    );
    ...
    if (content != null) {
      setContent(content);
    } else; //pack(); // ensure peer's created
    ...
    if (owner == null) Log.warn(
      "NULL OWNER: " + Util.tags(this),
  ...
  public void setFocusable(boolean t) {
    _win.setFocusable(t);
  }

  public void setFocusableWindowState(boolean t) {
    _win.setFocusableWindowState(t);
  }
  ...
  public void setContent(JComponent c) {
    if (DEBUG.DOCK || DEBUG.WIDGET || DEBUG.INIT) out(
      "adding " + GUI.name(c) + " to " + GUI.name(getContentPanel())
    );
    ...
    if (hadContent) getContent().removePropertyChangeListener(this);

    mMinContentSize = c.getMinimumSize();
    if (DEBUG.DOCK) GUI.dumpSizes(c, this + ":setContent");
    mContentPane.setWidget(
      c,
      Widget.wantsScroller(c),
      Widget.wantsScrollerAlways(c)
    );
    ...
    if (c instanceof JTabbedPane) {
      ...
      for (int tabIndex = 0; tabIndex < tabCount; tabIndex++) {
        ...
        if (tab instanceof JScrollPane) toListen =
          ((JScrollPane) (tab)).getViewport().getView(); else toListen = tab;
      ...
      if (c instanceof JScrollPane) toListen =
        ((JScrollPane) c).getViewport().getView();
      if (toListen == null) toListen = c;
  ...
  public JComponent getContent() {
    if (
      mContentPane.mContent.getComponentCount() > 0
    ) return (JComponent) mContentPane.mContent.getComponent(
      0
    ); else return null;
  }
  ...
  public void propertyChange(java.beans.PropertyChangeEvent e) {
    ...
    if (key == Widget.EXPANSION_KEY) {
      ...
      if (!mWasVisible) {
        ...
        if (hide) dismiss(); else setRolledUp(false, isVisible(), true);
      }
      ...
      String auxTitle = auxTitle = (String) e.getNewValue();
      if (auxTitle != null && auxTitle.length() > 0) setAuxTitle(
        ": " + auxTitle
      ); else setTitle(mBaseTitle, "");
    }
  }
  ...
  private static void staticInit() {
    ...
    if (
      OverrideMacAquaBrushedMetal && Util.getJavaVersion() >= 1.5f
    ...
    if (WindowBorder != null) {
      ...
      if (
        isMacAqua
      ) CollapsedHeightVisible -= 1; else; // overlap by one pixel // overlap by 2 pixels // not till we can fix up border overlap flashing; CollapsedHeightVisible -= 2;
      if (!isMacAquaMetal) CollapsedHeight += bi.top + bi.bottom;
    }
  ...
  private Border getWindowBorder() {
    if (isToolbar && isMacAqua) return null; else return WindowBorder;
  }

  /** @return a border, if any, for the entire DockWindow (null if none) */
  private Border getContentBorder(JComponent c) {
    if (isToolbar) return null;

    if (ContentBorder == null) {
      if (DEBUG.BOXES) return (
        ContentBorder = new LineBorder(Color.orange, 4)
      ); else return null;
  ...
  private static Component firstChild(Component component) {
    if (component instanceof Container) {
      ...
      if (c.getComponentCount() > 0) return c.getComponent(0);
    }
    return null;
  }
    ...
    if (isMacAqua && (MacWindowShadowEnabled || isMacAquaMetal)) {
      ...
      if (DEBUG.BOXES) {
        ...
        return new CompoundBorder(
          new MatteBorder(0, 1, 1, 1, new Color(204, 204, 204)),
  ...
  private static void dumpGC(Graphics g) {
    ...
    if (g instanceof sun.java2d.SunGraphics2D) sd =
      ((sun.java2d.SunGraphics2D) g).surfaceData;
  ...
  public void validate() {
    ...
    if (mAnimatingReshape) {
      ...
      if (mResizeCorner != null) {
        ...
        // south east
        mResizeCorner.setLocation(
          width - ResizeCornerSize,
          height - ResizeCornerSize
        );

        // south west
        if (mResizeCorner2 != null) mResizeCorner2.setLocation(
          0,
          height - ResizeCornerSize / 2
        );
      }
    }
  }
  ...
  public void addNotify() {
    ...
    if (isToolbar) {
      ...
      mContentPane.setPreferredSize(
        new Dimension(_win.getPreferredSize().width, ToolbarHeight)
      );
    }
  }

  private DockWindow getFirstVisibleParent() {
    if (isVisible()) return this; else if (
      mParentWhenHidden != null
    ) return mParentWhenHidden.getFirstVisibleParent(); else return null;
  }
  ...
  public static void ToggleAllVisible() {
    if (!AllVisible) ShowPreviouslyHiddenWindows(); else HideAllWindows();
  }
  ...
  public static synchronized void HideAllWindows() {
  ...
  public static synchronized void RestoreAllWindowStates() {
    if (DEBUG.DOCK) Log.debug(
      "-----------RestoreAllDockWindowStates-----------"
    );
    for (DockWindow dw : AllWindows) {
  ...
  public static synchronized void SaveAllWindowStates() {
  ...
  public static synchronized void DeactivateAllWindows() {
    if (DEBUG.Enabled) Log.debug("deactivate all");
    AllVisible = false;
    for (DockWindow dw : AllWindows) {
      dw.mWasVisible = dw.isVisible();
      if (dw.mWasVisible) dw.superSetVisible(false);
    }
  }
  ...
  public static synchronized void ShowPreviouslyHiddenWindows() {
    ...
    if (Util.isMacLeopard()) {
      ...
      for (DockWindow dw : AllWindows) {
        if (dw.mWasVisible) dw.superSetVisible(true); // non-Leopard platforms automatically toFront on this
        dw.mWasVisible = false;
      }
    }
    ...
    GUI.invokeAfterAWT(
      new Runnable() {
        public void run() {
          ...
          if (viewer != null) {
            if (DEBUG.DOCK) Log.debug(
              "focusRequest to return focus to " + viewer
            );
            viewer.requestFocus();
  ...
  public static synchronized void ImmediatelyRepaintAllWindows() {
  ...
  private void keepOnScreen() {
    ...
    if (keepOnScreen(r)) setSize(r.width, r.height);
  }
  ...
  public void setStackVisible(boolean show) {
    ...
    if (mChild != null) mChild.setStackVisible(show);
    if (!show) {
  ...
  protected void setVisible(boolean show, boolean autoUnrollOnShow) {
    ...
    if (show) {
      if (autoUnrollOnShow && isRolledUp()) setRolledUp(false); else if (
        false && mUnrolledShape != null
      ) // need to show before we do this!  Will need to tweak us so that's okay to do.
      setShapeAnimated(
        getX(),
        getY(),
        mUnrolledShape.width,
        mUnrolledShape.height
      );
  ...
  private void updateOnVisibilityChange() {
    if (DEBUG.DOCK) out(
      "updateOnVisibilityChange; mShowing=" +
      mShowing +
      " visible=" +
      isVisible()
    );
    ...
    if (mShowing == false) {
      ...
      if (mChild != null) {
        if (mParent != null) mParent.setChild(mChild); else removeChild();
  ...
  public void saveWindowProperties() {
    ...
    if (isRolledUp) size =
      new Dimension(
        (int) mUnrolledShape.getWidth(),
        (int) mUnrolledShape.getHeight()
      ); else size = getSize();
  ...
  public void positionWindowFromProperties() {
    ...
    if (wpp.isWindowVisible() || wpp.isRolledUp()) {
      if (p.x > -1 && isPointFullyOnScreen(p, size, screenSize)) {
        if (
          isToolbar
        ) setLocation(p.x, p.y); else setBounds( // ignore size on toolbars: they always get their designed size
          p.x,
          p.y,
          size.width,
          size.height
        ...
        if (wpp.isRolledUp()) {
          ...
          showRolledUp();
        } else setVisible(wpp.isWindowVisible());
  ...
  public static void flickerAnchorDock() {
    if (VUE.usingAnchorDock() == false) return;
  ...
  private boolean isPointFullyOnScreen(
    Point p,
    Dimension size,
    ...
    if (
      (rightCorner <= screenSize.getWidth()) &&
      (bottomCorner <= screenSize.getHeight())
  ...
  private void setTitle(String title, String suffix) {
    if (
      mTitleName != null && mTitleName.equals(title) && suffix == null
    ) return;
    ...
    final int suffixLen =
      (suffix == null ? 0 : GUI.stringLength(TitleFont, suffix));
    ...
    if (suffix == null) fullTitle = title; else fullTitle = title + suffix;
    ...
    if (mContentPane != null && mContentPane.mTitle != null) {
      ...
      if (suffixLen > baseLen) mMinTitleWidth -= suffixLen;
    }
    repaint();
  }
  ...
  private int minUnrolledHeight(int height) {
    int absoluteMin =
      TitleHeight + getBorderSize().height + mMinContentSize.height;

    if (absoluteMin < TitleHeight + ResizeCornerSize) absoluteMin =
      TitleHeight + ResizeCornerSize;

    if (height < absoluteMin) return absoluteMin; else return height;
  }
  ...
  private int minUnrolledWidth(int requestedWidth) {
    ...
    if (absoluteMin < mMinTitleWidth) absoluteMin = mMinTitleWidth;

    if (stackTop.isStackOwner && stackTop != this) {
      if (absoluteMin < stackTop.getWidth()) absoluteMin = stackTop.getWidth();
    }

    if (
      requestedWidth < absoluteMin
    ) return absoluteMin; else return requestedWidth;
  }
  ...
  private static final String ANCHOR_TITLE = VueResources.getString(
    "dockWindow.vueleopard.title"
  );
  ...
  public void setBounds(int x, int y, int width, int height) {
    ...
    if (DEBUG.DOCK) out(
      "setBounds " + x + "," + y + " " + width + "x" + height
    );

    if (
      DEBUG.Enabled && width == 0 && mTitleName != null
    ) Util.printStackTrace( // mTitle only null during <init>
      this + " zero width setBounds " + x + "," + y + " " + width + "x" + height
    );
    ...
    } else _peer.peer_setBounds(x, y, width, height);
  ...
  private int minRolledHeight() {
    if (wantsSidewaysRollup()) return getWidth(); else return CollapsedHeight;
  }

  private int minRolledWidth() {
    if (mDockRegion != null) return mDockRegion.getRolledWidth(
      this
    ); else return minUnrolledWidth(getWidth());
  }

  private int XminRolledWidth() {
    if (mDockRegion != null) return mDockRegion.getRolledWidth(this);
    ...
    final int parentWidth = mParent == null
      ? Integer.MAX_VALUE
      : mParent.getWidth();
    final int childWidth = mChild == null
      ? Integer.MAX_VALUE
      : mChild.getWidth();

    // smallest of the two:
    return minUnrolledWidth(
      parentWidth < childWidth ? parentWidth : childWidth
    );
  }

  private int XgetRolledWidth() {
    if (mParent == null && mChild == null) return 180;
  ...
  private int getStackedWidth() {
    ...
    if (mUnrolledShape != null) return mUnrolledShape.width < curWidth
      ? mUnrolledShape.width
      : curWidth; else return curWidth;
  }
  ...
  public boolean atScreenBottom() {
    ...
    if (GS == null || getY() < GS.top + GS.height / 2) return false;
  ...
  private void setRolledUp(
    boolean makeRolledUp,
    boolean animate,
    ...
    if (isRolledUp == makeRolledUp || isToolbar) return;
    ...
    if (makeRolledUp) {
      ...
      if (
        atScreenBottom() && mParent == null && !atScreenTop() // don't roll-down in stacks for now
      ...
      if (animate) setShapeAnimated(
        rolledX,
        rolledY,
        rolledWidth,
        rolledHeight
      ); else setBounds(rolledX, rolledY, rolledWidth, rolledHeight);

      getContentPanel().setVisible(false);

      if (wantsSidewaysRollup()) mContentPane.setVerticalTitle(true);
      ...
      if (
        animate
      ) GUI.postEvent( // only during normal use: not during init
        new ComponentEvent(_win, ComponentEvent.COMPONENT_HIDDEN) {}
      );
    } else {
      if (
        SidewaysRollup
      ) mContentPane.setVerticalTitle(false); // ensure vertical cleared
    ...
    if (mDockPrev != null) mDockPrev.repaintTitle();
    if (mDockNext != null) mDockNext.repaintTitle();
    /*
        if (mParent != null)
  ...
  private void setShapeAnimated(int x, int y, int width, int height) {
    ...
    try {
      if (moved && resized) {
        for (int i = 1; i < steps; i++) setBounds(
          ix.next(),
          iy.next(),
          iw.next(),
          ih.next()
        ...
        for (int i = 1; i < steps; i++) setBounds(x, y, iw.next(), ih.next());
      } else if (moved) {
        for (int i = 1; i < steps; i++) setBounds(
          ix.next(),
          iy.next(),
      ...
      GUI.invokeAfterAWT(
        new Runnable() {
          public void run() {
            ...
            if (DEBUG.DOCK) out(
              "setShapeAnimated: AWT eventQueue cleared, animating stopped."
            );
  ...
  private void dragToConstrained(int x, int y, boolean relaxed) {
    ...
    if (relaxed) {
      ...
      p = new Point(x, y);
    } else p = getConstrainedLocation(x, y);
  ...
  private static class Edge {
    ...
    boolean inRangeOf(Edge e) {
      if (false && isLip) return (
        max > e.min && e.max > min && e.min >= min
      ); else return max > e.min && e.max > min;
    }
    ...
    public String toString() {
      String s = "[" + axis;
      if (min == Integer.MIN_VALUE) return s + "]"; else if (
        isLip || lipDir != 0
      ) return s + " LIP " + min + "+" + lipDir + "]"; else return (
        s + " len " + (max - min) + ":" + min + "-" + max + "]"
      );
    }
  }
  ...
  private static class EdgeBox {
    ...
    public EdgeBox(final Component given) {
      if (given == null) {
        ...
        if (c.getParent() instanceof JViewport) c = (JViewport) c.getParent();
        //if (c.getParent() instanceof JScrollPane)
        //c = (JScrollPane) c.getParent();

        Point loc = new Point(c.getX(), c.getY());

        if (c instanceof Window == false) SwingUtilities.convertPointToScreen(
          loc,
          c
        );
        ...
        if (DEBUG.EDGE) System.out.println(
          this +
          " for " +
          GUI.name(c) +
          " parent=" +
          GUI.name(c.getParent()) +
          " from " +
          given
        );
      }
  ...
  private static class EdgeArray {
    ...
    void add(Edge e) {
      if (e != null) {
        if (DEBUG.EDGE && DEBUG.META) System.out.println(
          "Added " + name + "\tedge" + e
        );
        array[length++] = e;
      }
  ...
  private static void addEdges(EdgeBox b, DockWindow dw) {
    ...
    if (
      dw == null ||
      dw.mChild == null ||
      dw.getWidth() != dw.mChild.getWidth() ||
      dw.mChild.mMouseWasPressed
    ) StickyTopEdges.add(b.bottom);

    if (
      dw == null || dw.mParent == null || dw.getWidth() != dw.mParent.getWidth()
    ...
    if (b.top != null) //e.top.addLip(b.left.axis);
    StickyLeftEdges.add(new Edge(b.left.axis, b.top.axis, b.top.axis - 1));

    if (b.bottom != null) StickyLeftEdges.add(
      new Edge(b.left.axis, b.bottom.axis, b.bottom.axis + 1)
    );
    ...
    if (b.top != null) {
      if (false) {
        ...
        StickyTopEdges.add(
          new Edge(b.top.axis, b.right.axis, b.right.axis + 1)
        );
      } else {
  ...
  private static void addEdges(Component c) {
    if (c != null) addEdges(new EdgeBox(c), null);
  }
  ...
  private static void addEdges(DockWindow dw) {
    if (false) {
      ...
      if (dw.mParent != null) edges.top = null;
      ...
      if (dw.mChild != null && !dw.mChild.mMouseWasPressed) edges.bottom = null;
  ...
  public static void raiseAll() {
    ...
    for (DockWindow dw : AllWindows) dw.toFront();
  }
  ...
  private static void addScreenEdges(Screen screen) {
    ...
    StickyTopEdges.add(
      new Edge(screen.top + screen.margin.top, screen.left, screen.right)
    );
    ...
    StickyBottomEdges.add(
      new Edge(screen.bottom - screen.margin.bottom, screen.left, screen.right)
    );
  }
  ...
  private static void refreshScreenEdges(DockWindow mover) {
    ...
    for (Screen s : screens) addScreenEdges(s);
    ...
    for (DockWindow dw : AllWindows) {
      ...
      if (dw == mover || !dw.isVisible() || mover.hasDescendant(dw)) continue;
  ...
  private DockWindow getStackTop() {
    if (mParent == null) return this; else return mParent.getStackTop();
  }

  private DockWindow getStackBottom() {
    if (mChild == null) return this; else return mChild.getStackBottom();
  }
  ...
  public boolean hasDescendant(DockWindow child) {
    if (mChild == null) return false; else if (mChild == child) {
      if (DEBUG.DOCK) out("found descendant " + child);
      return true;
    } else return mChild.hasDescendant(child);
  }

  /** @return total stack height BELOW us, or just our height if no children under us */
  public int getStackHeight() {
    if (mChild == null) return getVisibleHeight(); else return (
      getVisibleHeight() + mChild.getStackHeight()
    );
  }
  ...
  private Point getConstrainedLocation(int x, int y) {
    ...
    if (mMovingStackHeight > 0) movingBounds.height = mMovingStackHeight;
  ...
  private static boolean computeClosestEdge(
    Edge movingEdge,
  ...
  private static boolean mergeWithResult(
    Edge edge,
    Edge movingEdge,
    EdgeHit result
  ) {
    if (edge.axis < 0) return false;

    if (!edge.inRangeOf(movingEdge)) return false;
  ...
  private Point getConstrainedXY(int x, int y, EdgeBox movingBox) {
    ...
    boolean matchedRight = computeClosestEdge(
      movingBox.right,
      StickyRightEdges,
      Xresult
    );
    ...
    boolean matchedBottom = computeClosestEdge(
      movingBox.bottom,
      StickyBottomEdges,
      Yresult
    );
    ...
    if (Xresult.delta < StickyDistance) {
      if (matchedRight) x = Xresult.edge.axis - movingBox.width; else x =
        Xresult.edge.axis;
    ...
    if (Yresult.delta < StickyDistance) {
      if (matchedBottom) y = Yresult.edge.axis - movingBox.height; else y =
        Yresult.edge.axis;
    }
  ...
  private DockWindow getNearWindow(int localMouseX, int localMouseY) {
    ...
    for (DockWindow dw : AllWindows) {
      if (!dw.isVisible() || dw == this) continue;

      // only dock like windows together for now
      if (isToolbar != dw.isToolbar) continue;
  ...
  public boolean interceptMousePress(MouseEvent e) {
    ...
    if (!Util.isUnixPlatform()) raiseStack();
  ...
  public void mousePressed(MouseEvent e) {
    ...
    if (e.getSource().getClass() == ResizeCorner.class) {
      if (!isRolledUp()) mDragSizeStart = getSize(); // we'll be resizing the window
    ...
    if (DEBUG.MOUSE) out(
      "mDragStartScreen=" + mDragStartScreen + " mDragStart=" + mDragStart
    );
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if (e.getSource().getClass() == ResizeCorner.class) e =
      SwingUtilities.convertMouseEvent(e.getComponent(), e, _win);
  ...
  private void dragMoveWindow(MouseEvent e) {
    if (mWindowDragUnderway == false) {
      if (!dragStartWindowMove(e)) return;
    }
    ...
    if (isMac) {
      if (GUI.hasMultipleScreens()) {
        ...
        if (
          false && y < 22
        ) y = 22; // can never have a screen above otherwise
      } else if (y < 22) {
        y = 22;
      }
    }
  ...
  private boolean dragStartWindowMove(MouseEvent e) {
    ...
    if (Math.abs(dx) < 5 && Math.abs(dy) < 5) {
      if (DEBUG.MOUSE) out(
        "delaying drag start with dx=" + dx + " dy=" + dy + " screen=" + screen
      );
      return false;
    }
    ...
    if (isStackTop()) mMovingStackHeight =
      getStackHeight(); else mMovingStackHeight = 0;

    //if (isMacAqua) MacOSX.setAlpha(this, 0.75f);

    if (e.isShiftDown()) {
      // remove from any stack it's in and drag free
      if (mParent != null) mParent.removeChild();
      if (mChild != null) setChild(null);
    } else {
      if (isMac) {
        ...
        if (e.isAltDown()) attachSiblingsForMoving();
      }
    }
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (
      e.getSource().getClass() == ResizeCorner.class
    ) e = SwingUtilities.convertMouseEvent(mResizeCorner, e, _win); // should never be needed, but just in case

    if (mWindowDragUnderway) dropWindow(e); else if (
      !isToolbar
    ...
    if (mWasStickingRight != mStickingRight) updateAllChildLocations();
  }

  private void handleMouseClicked(MouseEvent e) {
    if (DEBUG.MOUSE) out("handleMouseClicked: " + GUI.name(e));

    if (e.getSource().getClass() == ResizeCorner.class) return;
    ...
    if (GUI.isDoubleClick(e) && titleBarHit(e)) {
      if (DEBUG.Enabled) {
        ...
        } else setMaximizeVertical(e.isShiftDown());
      } else {
        if (isRolledUp()) {
          setRolledUp(false);
  ...
  private void dropWindow(MouseEvent e) {
    ...
    if (dockRegion != null) {
      ...
      if (near != null && near.atScreenBottom()) near = null;
    }
  ...
  private void updateWindowShadow() {
    if (isMac && Util.isMacCocoaSupported()) {
      ...
      if (DEBUG.DOCK) out(
        "updateWindowShadow: docked=" + isDocked() + " rolled=" + isRolledUp()
      );
  ...
  private void setChild(DockWindow newChild, boolean updateChildren) {
    ...
    if (newChild == mParent) throw new Error(
      this + " cannot setChild on parent " + mParent
    );
    ...
    if (mChild == newChild) {
      ...
      if (newChild.mParent != this) throw new Error(
        "DockWindow child/parent inconsistency"
      );
      if (isMac) updateChildLocation(); else updateAllChildLocations();
    } else {
      ...
      if (hasDescendant(newChild)) {
        Util.printStackTrace(
          "is already descendent of " + this + " " + newChild
        );
        return;
      }
      ...
      if (updateChildren) {
        if (isMac) {
          ...
          // placed wrong.
          if (reparented) updateAllChildLocations(); else updateChildLocation();
      ...
      if (!newChild.isRolledUp()) newChild.keepOnScreen();
    }
  ...
  private void _updateChildLocation(
    boolean allChildren,
    int upcomingHeight,
    int upcomingY
  ) {
    if (StackDepth - 1 > AllWindows.size()) throw new LoopError(
      "updateChildLocation"
    );
    ...
    if (mChild != null) {
      ...
      if (CollapsedHeight != CollapsedHeightVisible) {
        ...
        if (isRolledUp()) y -=
        (CollapsedHeight - CollapsedHeightVisible); else y--;
      }
      ...
      if (y >= mChild.getY()) {
        ...
        if (allChildren) mChild.updateAllChildLocations(); // recurse down the chain
      } else {
        if (allChildren) mChild.updateAllChildLocations(mChild.getHeight(), y); // recurse down the chain
        mChild.superSetLocation(x, y);
      }
    }
  ...
  public int getVisibleHeight() {
    if (mChild != null && CollapsedHeight != CollapsedHeightVisible) {
      if (isRolledUp()) return (
        getHeight() - (CollapsedHeight - CollapsedHeightVisible)
      ); else return getHeight() - 1;
  ...
  private void _raiseChildren() {
    ...
    if (isMac && MacWindowShadowEnabled) GUI.invokeAfterAWT(
      new Runnable() {
        public void run() {
          updateWindowShadow();
        }
  ...
  public void suggestLocation(int x, int y) {
    Rectangle limit = GraphicsEnvironment
      .getLocalGraphicsEnvironment()
      .getMaximumWindowBounds();
    limit.width += limit.x;
    limit.height += limit.y;
    //out("suggesting location " + x + "," + y + " in limits " + limit);
    if (x + getWidth() > limit.width) x = limit.width - getWidth();
    if (x < limit.x && x != 0) x = limit.x;
    if (y + getHeight() > limit.height) y = limit.height - getHeight();
  ...
  public String toString() {
    ...
    String s = "[" + mTitleName;
    if (true || mChild == null) return s + "]"; else return (
      s + " ->" + mChild.mTitleName + "]"
    );
  }

  public static class ScrollableWidthTracker
    extends JPanel
    ...
    public Dimension getPreferredScrollableViewportSize() {
      ...
      if (DEBUG.DOCK || DEBUG.SCROLL) Log.debug(
        GUI.name(tracked) + " viewportSize " + GUI.name(d)
      );
      return d;
    }
    ...
    public String toString() {
      return (
        ...
        GUI.name(tracked) +
        "; psvs=" +
        Util.fmt(getPreferredScrollableViewportSize()) +
        "]"
      );
    }
  }
  ...
  private class ContentPane extends JPanel {
    ...
    public ContentPane(String title, boolean asToolbar) {
      ...
      if (true || asToolbar) {
        ...
        if (asToolbar) titleConstraints =
          BorderLayout.WEST; else titleConstraints = BorderLayout.NORTH;
      ...
      if (isMac && isToolbar) setBorder(
        BorderFactory.createLineBorder(Color.black)
      ); else setBorder(getWindowBorder());
    ...
    void setCloseButtonVisible(boolean visible) {
      if (mTitle != null) mTitle.setCloseButtonVisible(visible);
    }
    ...
    private void changeAll(JComponent root) {
      new EventRaiser<JComponent>(this, JComponent.class) {
        protected void visit(Component c) {
          if (DEBUG.INIT) {
            if (targetClass.isInstance(c)) {
              System.out.format(
                "\tDockWindow(%s) making transparent: ",
                DockWindow.this.mTitleName
              );
              dispatchSafely(c);
        ...
        public void dispatch(JComponent c) {
          if (c instanceof javax.swing.text.JTextComponent) return;
    ...
    public void paintComponent(Graphics g) {
      if (gradientBG) paintBackgroundGradient(g); else super.paintComponent(g);
    }
    ...
    private void installTitlePanel(String title, boolean asToolbar) {
      ...
      if (asToolbar && !isMacAqua) add(
        new Gripper(),
        BorderLayout.WEST
      ); else add(mTitle, titleConstraints);
    }
    /*
  ...
  private class Gripper extends javax.swing.Box {
    ...
    Gripper() {
      ...
      if (true) setOpaque(false); else setBackground(Color.darkGray); // looks like crap
      ...
      if (showCloseBtn) add(closeButton);
    ...
    public void paintComponent(Graphics g) {
      ...
      if (true) paintGripper((Graphics2D) g); else super.paintComponent(g);
    }
    ...
    private void paintGripper(Graphics2D g) {
      ...
      GradientPaint mGradient = new GradientPaint(
        0,
        0,
        BottomGradientColor,
        width,
  ...
  private class TitlePanel extends javax.swing.Box {
    ...
    TitlePanel(String title) {
      ...
      mLabel.setForeground(
        VueResources.getColor(
          "gui.dockWindow.title.foreground",
          SystemColor.activeCaptionText
        )
      );

      Color iconColor = VueResources.getColor(
        "gui.dockWindow.title.disclosureIcon.color",
      ...
      final String helpTextHeader = VueResources.getString(
        "dockWindow.helpTextHeader"
      );
      final String helpTextFooter = VueResources.getString(
        "dockWindow.helpTextFooter"
      );
      final String helpText =
        helpTextHeader + "No help for " + getName() + "." + helpTextFooter;
      ...
      if (helpText != null && helpText.length() > 0) {
        if (false && Util.isMacLeopard()) {
          ...
          mHelpButton =
            new VueLabel(
              VueResources.getImageIconResource(
                "/tufts/vue/images/btn_help_top.gif"
              )
            );
        }
      ...
      if (isMacAqua) {
        ...
        // add(mLabel);
        add(Box.createGlue());
        if (mHelpButton != null) add(mHelpButton);
      } else {
      ...
      if (isGradientTitle) installGradient(false);
    }
    ...
    void setMenuActions(Action[] actions) {
      ...
      if (count == 0) {
        ...
        if (mMenuButton == null) {
          ...
          add(mMenuButton, -1);
        } else mMenuButton.setMenuActions(actions);
      }
    }

    void setVertical(boolean vertical) {
      if (isGradientTitle) installGradient(vertical);
      mCloseButton.setVisible(!vertical);
      isVertical = vertical;
    }

    private void installGradient(boolean vertical) {
      if (vertical) mGradient =
        new GradientPaint(
          getHeight(),
          0,
          TopGradientColor,
          0,
          0,
          false ? Color.gray : BottomGradientColor
        ); else mGradient =
        new GradientPaint(
          0,
          0,
    ...
    private void paintGradientEtc(Graphics g) {
      ...
      if (isGradientTitle) ((Graphics2D) g).setPaint(
          mGradient
  ...
  private class MenuButton
    ...
    MenuButton(Action[] actions) {
      ...
      setRolloverIcon(
        VueResources.getImageIcon("dockWindow.panner.menu.raw.over")
      );
      setName(DockWindow.this._win.getName());
    ...
    void setMenuActions(Action[] actions) {
      ...
      new GUI.PopupMenuHandler(this, GUI.buildMenu(actions)) {
        ...
        public int getMenuX(Component c) {
          return c.getWidth();
        }

        public int getMenuY(Component c) {
          return -getY();
        } // 0 in parent
      };
    ...
    private void clearMenuActions() {
      ...
      for (int i = 0; i < ml.length; i++) {
        if (ml[i] instanceof GUI.PopupMenuHandler) removeMouseListener(ml[i]);
      }
    }
  ...
  private static class CloseButton extends JLabel {
    ...
    public CloseButton(final DockWindow dockWindow) {
      ...
      addMouseListener(
        new tufts.vue.MouseAdapter(getClass()) {
          public void mouseEntered(MouseEvent e) {
            setRollover(true);
          }
          ...
          public void mouseClicked(MouseEvent e) {
            if (
              visible && e.getClickCount() != 0 && e.getClickCount() != 2
            ) dockWindow.dismiss();
          }
        }
      );
  ...
  private static class ResizeCorner extends javax.swing.JComponent {
    ...
    ResizeCorner(DockWindow mouseListener, Object corner) {
      ...
      if (mCorner == SOUTH_EAST) setSize(
        ResizeCornerSize,
        ResizeCornerSize
      ); else setSize(ResizeCornerSize / 2, ResizeCornerSize / 2);
      ...
      if (mCorner == NORTH_EAST) cursorID = Cursor.NE_RESIZE_CURSOR; else if (
        mCorner == SOUTH_WEST
      ) cursorID = Cursor.SW_RESIZE_CURSOR; else if (
        mCorner == NORTH_WEST
      ...
      if (isMacAqua && !isMacAquaMetal) setForeground(
        Color.lightGray
      ); else setForeground(Color.gray);
    }

    public void paintComponent(Graphics g) {
      if (mCorner == SOUTH_EAST) paintResizeCorner((Graphics2D) g);
    ...
    private void paintResizeCorner(Graphics2D g) {
      if (DEBUG.PAINT) System.out.println(
        "ResizeCorner paint " + g.getClipBounds()
      );
  ...
  public static void main(String args[]) {
    ...
    stack.addPane(
      "My Picture",
      new JLabel(new ImageIcon(VueResources.getURL("splashScreen")))
    );
