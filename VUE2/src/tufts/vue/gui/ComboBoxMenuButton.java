...
/**
 * @version $Revision: 1.14 $ / $Date: 2010-02-03 19:15:46 $ / $Author: mike $
 */
...
public abstract class ComboBoxMenuButton<T>
  extends JComboBox
  ...
  public T getMenuValueAt(int index) {
    ...
    if (item instanceof Action) return (T) ((Action) item).getValue(
        VALUE_KEY
      ); else return (T) item;
  }
  ...
  protected Icon getIconForValue(Object value) {
    ...
    if (mIconCache != null) icon = mIconCache.get(value);
    ...
    if (mIconCache == null) mIconCache = new HashMap();
  ...
  protected void handleMenuSelection(ItemEvent e) {
    ...
    if (e.getItem() instanceof Action) handleValueSelection(
      (T) ((Action) e.getItem()).getValue(VALUE_KEY)
    ); else handleValueSelection((T) e.getItem());
  }
