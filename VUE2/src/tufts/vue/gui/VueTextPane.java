...
public class VueTextPane extends JTextPane implements LWComponent.Listener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueTextPane.class);
  ...
  public VueTextPane(LWComponent c, Object propertyKey, String undoName) {
    ...
    if (c != null && propertyKey != null) attachProperty(c, propertyKey);
    setUndoName(undoName);

    GUI.installBorder(this);

    if (propertyKey != null) setName(propertyKey.toString()); else if (
      undoName != null
    ) setName(undoName);
  }
  ...
  public void setFocusTraversalKeys(int id, java.util.Set keystrokes) {
    if (DEBUG.FOCUS) System.out.println(
      this + " ignoring setFocusTraversalKeys " + id + " " + keystrokes
    );
  }
  ...
  public void setText(String s) {
    if (DEBUG.TEXT) debug(
      "setText: curText=" + Util.tags(getText()) + " newText=" + Util.tags(s)
    );
    super.setText(s);
  }
  ...
  protected void processKeyEvent(KeyEvent e) {
    if (DEBUG.KEYS && e.getID() == KeyEvent.KEY_PRESSED) Log.debug(
      "processKeyEvent " + GUI.name(e) + "; " + this
    );
  ...
  public void attachProperty(LWComponent c, Object key) {
    if (c == null || key == null) throw new IllegalArgumentException(
      "component=" + c + " propertyKey=" + key + " neither can be null"
    );
    saveText("attach");
    if (lwc == c && propertyKey == key) return;
    if (lwc != null) lwc.removeLWCListener(this);
    lwc = c;
    propertyKey = key;
    loadPropertyValue();
    if (DEBUG.TEXT || DEBUG.EVENTS) debug(
      "attachProperty " + Util.tags(key) + "; " + c
    );
  ...
  protected void saveText(Object src) {
    ...
    if (keyWasPressed || !currentText.equals(loadedText)) {
      ...
      if (DEBUG.EVENTS) debug(
        String.format("saveText: apply %s -> %s", Util.tags(propertyKey), lwc)
      );
  ...
  protected void applyText(String text) {
    ...
    if (DEBUG.EVENTS || DEBUG.TEXT) debug(
      String.format(
        "applyText: '%.16s'... to: %s -> %s",
        text,
        Util.tags(propertyKey),
        lwc
      )
    );
    lwc.setProperty(propertyKey, text);
    loadedText = text;
    if (undoName != null) VUE.markUndo(undoName); else VUE.markUndo();
  }
    ...
    if (lwc != null) {
      try {
        ...
        throw new IllegalArgumentException(
          "VueTextPane only handles properties of type String"
        );
      }
  ...
  public void addNotify() {
    ...
    if (getContentType().equalsIgnoreCase("text/rtf")) {
      ...
      // Add CTRL-B for Bold
      KeyStroke boldStroke = KeyStroke.getKeyStroke(
        KeyEvent.VK_B,
        COMMAND,
        false
      );
      ...
      KeyStroke italicStroke = KeyStroke.getKeyStroke(
        KeyEvent.VK_I,
        COMMAND,
        false
      );
      ...
      KeyStroke underlineStroke = KeyStroke.getKeyStroke(
        KeyEvent.VK_U,
        COMMAND,
        false
      );
  ...
  public static void main(String args[]) {
    ...
    DockWindow w = GUI.createDockWindow(
      VueResources.getString("dockWindow.vueTextpanetest.title")
    );
    ...
    w.setContent(panel);
    w.setVisible(true);
    VueUtil.displayComponent(
      new VueTextPane(new LWMap("Test Map"), LWKey.Notes, null)
    );
  }
