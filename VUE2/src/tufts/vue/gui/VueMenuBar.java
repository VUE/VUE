 ...
 * @author Scott Fraize
 */
public class VueMenuBar
  extends javax.swing.JMenuBar
  implements java.awt.event.FocusListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(VueMenuBar.class);
...
*/
  ...
  private static class VueMenu extends JMenu {
    ...
    VueMenu(String name) {
       ...
       */
      if (Util.isWindowsPlatform() || Util.isUnixPlatform()) this.getPopupMenu()
        .setUI(new VuePopupMenuUI());
    }
  ...
  private static JCheckBoxMenuItem makeCheckBox(
    VueAction a,
    ...
    if (a.getKeyStroke() != null) item.setAccelerator(a.getKeyStroke());
  ...
  private final JMenu windowMenu = makeMenu(
    VueResources.getFormatMessage(arguments, "menu.windows")
  );
  ...
  private static final java.util.concurrent.atomic.AtomicInteger PublishRebuildCount =
  ...
  public VueMenuBar() {
    ...
    final JMenu recentlyOpenedMenu = makeMenu(
      VueResources.getString("menu.windiws.openrecent")
    );
    ...
    final JMenu presentationMenu = makeMenu(
      VueResources.getString("menu.pathway.label")
    );
    final JMenu analysisMenu = makeMenu(
      VueResources.getString("menu.analysis")
    );
    ...
    final JMenu notesMenu = new JMenu(
      VueResources.getString("menu.pathways.handoutsandnotes")
    );
    playbackMenu =
      new JMenu(VueResources.getString("menu.playback.play.label"));
    playbackMenu.setEnabled(false);
    ////////////////////////////////////////////////////////////////////////////////////
    // Initialize Actions
    ////////////////////////////////////////////////////////////////////////////////////

    final JMenuItem splitScreenItem = new JCheckBoxMenuItem(
      Actions.ToggleSplitScreen
    );
    final JMenuItem toggleLinksItem = new JCheckBoxMenuItem(
      Actions.ToggleLinks
    );
    final JMenuItem toggleSlideIconsItem = makeLinkedCheckBox(
      Actions.ToggleSlideIcons
    );
    final JMenuItem togglePruningItem = new JCheckBoxMenuItem(
      Actions.TogglePruning
    );
    ...
    saveAction =
      new SaveAction(VueResources.getString("menu.windows.save"), false);
    saveAction.setEnabled(false);
    saveAsAction =
      new SaveAction(VueResources.getString("menu.windows.saveas"));
    saveAsAction.setEnabled(false);
    //final SaveAction exportAction = new SaveAction("Export ...",true,true);
    final OpenAction openAction = new OpenAction(
      VueResources.getString("menu.windows.open")
    );
    final OpenURLAction openFromURLAction = new OpenURLAction(
      VueResources.getString("menu.windows.openfromurl")
    );
    final ExitAction exitAction = new ExitAction(
      Util.isMacPlatform()
        ? VueResources.getString("menu.windows.quit")
        : VueResources.getString("menu.windows.exit")
    );
    ...
    final CreateCM createCMAction = new CreateCM(
      VueResources.getString("menu.windows.conanalysis")
    );
    final AnalyzeCM analyzeCMAction = new AnalyzeCM(
      VueResources.getString("menu.windows.mergemaps")
    );
        ...
        new edu.tufts.vue.dsm.DataSourceListener() {
          public void changed(
            ...
            if (state == VueDataSourceManager.DS_CONFIGURED) {
              ...
              state == VueDataSourceManager.DS_ALL_CONFIGURED &&
              VUE.BLOCKING_OSID_LOAD
            ) {
            ...
            for (int i = 0; i < dataSource.length; i++) {
              try {
                final org.osid.repository.Repository r =
                  dataSource[i].getRepository();
                if (r == null) {
                  if (DEBUG.DR) Log.debug(
                    "null repository in " + dataSource[i]
                  );
                  continue;
                }
                if (
                  r
                    .getType()
                    .isEqual(
                      edu.tufts.vue.dsm.DataSourceTypes.FEDORA_REPOSITORY_TYPE
                    )
                ) {
                  if (!fedoraFlag) {
                    typeActions.add(
                      PublishActionFactory.createPublishAction(
                        edu.tufts.vue.dsm.DataSourceTypes.FEDORA_REPOSITORY_TYPE
                      )
                    );
                    fedoraFlag = true;
                  ...
                  publishActions.add(
                    PublishActionFactory.createPublishAction(dataSource[i])
                  );
                  count++;
                } else if (
                  r
                    .getType()
                    .isEqual(
                      edu.tufts.vue.dsm.DataSourceTypes.SAKAI_REPOSITORY_TYPE
                    )
                ) {
                  if (!sakaiFlag) {
                    typeActions.add(
              ...
              new Runnable() {
                public void run() {
                  ...
                  if (rebuildCount < curCount) {
                    if (DEBUG.INIT || DEBUG.DR) Log.debug(
                      "skipping rebuild; runnable count " +
                      rebuildCount +
                      " < " +
                      curCount
                    );
                  ...
                  if (typeActions.size() > 0) {
                    for (Action a : typeActions) publishMenu.add(a);

                    if (publishActions.size() > 0) {
                      publishMenu.addSeparator();
                      for (Action a : publishActions) publishMenu.add(a);
                    }
                  } else {
                    ...
                    publishMenu.add(
                      createWindowItem(
                        VUE.getContentDock(),
                        0,
                        VueResources.getString("menu.publisher.resourcewindow")
                      )
                    );
                  }
    ...
    fileMenu
      .add(openAction)
      .setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, metaMask));
    fileMenu.add(openFromURLAction);
    fileMenu.add(Actions.CloseMap);
    fileMenu.addSeparator();
    fileMenu
      .add(saveAction)
      .setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, metaMask));
    fileMenu
      .add(saveAsAction)
      .setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_S, metaMask + Event.SHIFT_MASK)
      );
    ...
    pdfExportMenu =
      new JMenu(VueResources.getString("menu.windows.exporthandouts"));
    pdfExportMenu.setEnabled(false);
    //pdfExportMenu.add(Actions.MapAsPDF);
    final JMenuItem fullPageSlideNotesItem = new JMenuItem(
      Actions.FullPageSlideNotes
    );
    ...
    final JMenuItem speakerNotesOutlineItem = new JMenuItem(
      Actions.SpeakerNotesOutline
    );
    final JMenuItem nodeNotes4Item = new JMenuItem(Actions.NodeNotes4);
    final JMenuItem nodeNotesOutlineItem = new JMenuItem(
      Actions.NodeNotesOutline
    );
    ...
    fileMenu
      .add(printAction)
      .setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P, metaMask));
    fileMenu
      .add(printAction)
      .setText(VueResources.getString("menu.windows.printvisible"));
    // if (!VUE.isApplet())
    ...
    if (DEBUG.IMAGE) editMenu.add(Images.ClearCacheAction);
    ...
    if (DEBUG.TWITTER) viewMenu.add(viewKiosk);
    ...
    if (LWComponent.COLLAPSE_IS_GLOBAL) viewMenu.add(
      makeCheckBox(Actions.ToggleGlobalCollapse)
    ); else viewMenu.add(Actions.Collapse);
    ...
    final JMenuItem removeResourceItem = new JMenuItem(
      Actions.RemoveResourceAction
    );
    final JMenuItem removeResourceKeepImageItem = new JMenuItem(
      Actions.RemoveResourceKeepImageAction
    );
          ...
          if (c instanceof LWNode) {
            if ((c).hasResource()) {
              ...
              if (c.hasChildren()) {
                ...
                if (!enabled) removeResourceKeepImageItem.setEnabled(false);
              }

              addFileItem.setLabel(
                VueResources.getString(
                  "mapViewer.componentMenu.replaceFile.label"
                )
              );
              addURLItem.setLabel(
                VueResources.getString(
                  "mapViewer.componentMenu.replaceURL.label"
                )
              );
            } else {
    ...
    presentationMenu.addMenuListener(
      new MenuListener() {
        public void menuCanceled(MenuEvent e) {
          /* no op	*/
        }
      ...
      new MenuListener() {
        ...
        public void menuSelected(MenuEvent arg0) {
          ...
          while (i.hasNext()) {
            ...
            if (path.getEntries().isEmpty()) menuItem.setEnabled(false);
    ...
    presentationMenu.addMenuListener(
      new MenuListener() {
        ...
        private void handleActivation() {
          LWPathway p = VUE.getActivePathway();
          if (p == null || p.length() == 0) notesMenu.setEnabled(
            false
          ); else notesMenu.setEnabled(true);
        }
      }
    );
    ////////////////////////////////////////////////////////////////////////////////////
    ...
    analysisMenu.add(
      createWindowItem(
        JavaAnalysisPanel.getJavaAnalysisDock(),
        0,
        VueResources.getString("menu.windows.java")
      )
    );
    //}
    ...
    helpMenu.add(
      new ShowURLAction(
        VueResources.getString("helpMenu.userGuide.label"),
        VueResources.getString("helpMenu.userGuide.url")
      )
    );
    helpMenu.add(
      new ShowURLAction(
        VueResources.getString("helpMenu.feedback.label"),
        VueResources.getString("helpMenu.feedback.url")
      )
    );
    ...
    if (RootMenuBar == null) RootMenuBar = this;
  }
  ...
  public void rebuildWindowsMenu() {
    ...
    if (VUE.getFormatDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getFormatDock(),
        KeyEvent.VK_1,
        VueResources.getString("menu.windows.formatpalette")
      )
    );
    windowMenu.addSeparator();
    if (VUE.getInfoDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getInfoDock(),
        KeyEvent.VK_2,
        VueResources.getString("menu.windows.info")
      )
    );
    ...
    if (VUE.getInteractionToolsDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getInteractionToolsDock(),
        KeyEvent.VK_4,
        VueResources.getString("dockWindow.interactionTools.title")
      )
    );
    if (VUE.getLayersDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getLayersDock(),
        KeyEvent.VK_5,
        VueResources.getString("menu.windows.layers")
      )
    );
    if (VUE.getMapInfoDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getMapInfoDock(),
        KeyEvent.VK_6,
        VueResources.getString("menu.windows.mapinfo")
      )
    );
    if (VUE.getOutlineDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getOutlineDock(),
        KeyEvent.VK_7,
        VueResources.getString("menu.windows.outline")
      )
    );
    if (VUE.getPannerDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getPannerDock(),
        0,
        VueResources.getString("menu.windows.panner")
      )
    );
    if (VUE.getPresentationDock() != null) windowMenu.add(
      createWindowItem(
        VUE.getPresentationDock(),
        KeyEvent.VK_8,
        VueResources.getString("menu.windows.pathways")
      )
    );
    if (VUE.getMetadataSearchMainGUI() != null) windowMenu.add(
      createWindowItem(
        VUE.getMetadataSearchMainGUI(),
        KeyEvent.VK_9,
        VueResources.getString("menu.windows.search")
      )
    );
    ...
    if (VUE.getFloatingZoomDock() != null) {
      fullScreenToolbarItem =
        createWindowItem(
          VUE.getFloatingZoomDock(),
          KeyEvent.VK_0,
          VueResources.getString("menu.windows.fullscreentoolbar")
        );
  ...
  public void activeChanged(tufts.vue.ActiveEvent e, LWMap map) {
    ...
    if (VUE.isStartupUnderway()) return;
    ...
    for (int i = 0; i < tp.getTabCount(); i++) {
      ...
      JMenuItem item = new JMenuItem(
        new VueAction(m.getSaveFileModelVersion() + " | " + m.getLabel()) {
          public void act() {
      ...
      if (m == map) item.setEnabled(false);
  ...
  public JCheckBoxMenuItem createWindowItem(
    DockWindow dock,
    int accelKey,
    String text
  ) {
    ...
    if (dock != null) {
      ...
      final KeyStroke acceleratorKey = KeyStroke.getKeyStroke(
        accelKey,
        Actions.COMMAND
      );
      if (accelKey > 0) windowAction.putValue(
        Action.ACCELERATOR_KEY,
        acceleratorKey
      );
      if (text != null) windowAction.setTitle(text);
  ...
  protected void rebuildRecentlyOpenedItems(
    JMenu fileMenu,
    JMenu recentlyOpenedMenu,
    RecentlyOpenedFilesManager rofm
  ) {
    if (rofm.getFileListSize() > 0) {
      ...
      while (i.hasNext()) {
        ...
        if (f.exists()) recentlyOpenedMenu.add(
          new RecentOpenAction(f)
        ); else f = null;
      }
  ...
  public void processKeyEvent(KeyEvent e) {
    if (!e.isConsumed()) super.processKeyEvent(e); else Log.debug(
      "processKeyEvent: already consumed " + GUI.name(e)
    );
  }
  ...
  protected boolean processKeyBinding(
    ...
    if (e.isConsumed()) Log.debug("GOT CONSUMED " + ks);

    if (
      !pressed
    ) return false; // we only ever handle on key-press
    ...
    final boolean didAction = super.processKeyBinding(
      ks,
      e,
      condition,
      pressed
    ...
    if (DEBUG.KEYS) {
      String used = didAction ? "CONSUMED " : "NOACTION ";
      Log.debug("processKeyBinding " + used + "[" + ks + "] " + GUI.name(e));
    }
    if (didAction) e.consume();
  ...
  public static JMenu buildMenu(
    ...
    final JMenu menu = new JMenu(
      VueResources.getString(localizationKey, localizationKey)
    );
    buildMenu(menu, actions);
    if (!enabled) menu.setEnabled(false);
    return menu;
  }
  ...
  public static JMenu buildMenu(JMenu menu, Action[] actions) {
    for (int i = 0; i < actions.length; i++) {
      Action a = actions[i];
      if (a == null) menu.addSeparator(); else menu.add(a);
    }
    return menu;
  }
  ...
  private static class RecentOpenAction extends VueAction {
    ...
    public RecentOpenAction(File f) {
      super(f.getName());
      if (DEBUG.Enabled) putValue(
        SHORT_DESCRIPTION,
        f.toString()
      ); else putValue(SHORT_DESCRIPTION, "");
      file = f;
    }
  ...
  private static class ShowLogAction extends VueAction {
    ...
    private void buildGUI() {
      ...
      panel.add(
        new JButton(VueResources.getString("button.submitrep.label")) {
          @Override
      ...
      errorDock =
        GUI.createDockWindow(VueResources.getString("menu.help.vuelog"), panel);
