...
public class AlignmentDropDown extends JPanel {
  ...
  public AlignmentDropDown() {
    ...
    images[0] =
      (ImageIcon) VueResources.getIcon("list.button.leftalignment.raw");
    images[1] =
      (ImageIcon) VueResources.getIcon("list.button.centeralignment.raw");
    images[2] =
      (ImageIcon) VueResources.getIcon("list.button.rightalignment.raw");
    ...
    this.setPreferredSize(
        new Dimension(alignList.getWidth() - 10, alignList.getHeight())
      );
  }

  public void quietlySelectIndex(int p) {
    ActionListener[] l = alignList.getActionListeners();
    for (int i = 0; i < l.length; i++) alignList.removeActionListener(l[i]);
    alignList.setSelectedItem(new Integer(p));
    for (int i = 0; i < l.length; i++) alignList.addActionListener(l[i]);
  }
  ...
  private static final AlphaComposite DisabledTransparency =
    AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.4f);

  public void paint(Graphics g) {
    if (!alignList.isEnabled()) ((java.awt.Graphics2D) g).setComposite(
