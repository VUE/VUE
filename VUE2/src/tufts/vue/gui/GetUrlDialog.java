...
public class GetUrlDialog extends JDialog implements ActionListener {
  ...
  JTextField fUrlField = AutoCompleteDocument.createAutoCompleteTextField(
    roum.getRecentlyOpendUrlsArray()
  );
  ...
  public GetUrlDialog() {
    ...
    super(
      ...
      VueResources.getString("dialog.openurl.title"),
      true
    );
    ...
    JLabel label = new JLabel(
      VueResources.getString("openurl.label"),
      SwingConstants.LEFT
    );
    addGB(label, 0, 0);
    ...
    location.x =
      Math.max(0, Math.min(location.x, screenSize.width - getSize().width));
    location.y =
      Math.max(0, Math.min(location.y, screenSize.height - getSize().height));
