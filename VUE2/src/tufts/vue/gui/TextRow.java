...
public class TextRow {
  ...
  private static final boolean FunkyMacTextBounds =
    VueUtil.isMacPlatform() && VueUtil.getMacMRJVersion() < 232;
  /** default FontRenderContext: anti-alias=true and fractional-metrics=false */
  private static final FontRenderContext DefaultFontContext =
  ...
  private TextRow(
    ...
    this.text = text;
    if (row == null) mRow = new TextLayout(text, font, frc); else mRow = row;
    ...
    if (bounds == null) {
      ...
      if (DEBUG.TEXT) System.out.println(
        "TextRow[" + text + "] rowbnds=" + tufts.Util.fmt(row.getBounds())
      );
    ...
    if (DEBUG.TEXT) System.out.println(
      "TextRow[" + text + "]  bounds=" + tufts.Util.fmt(bounds)
    );
  }

  public TextRow(String text, Font font, FontRenderContext frc) {
    this(text, font, frc, null, null);
  }
