...
public class FullScreen {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(FullScreen.class);
  ...
  public static class FSWindow extends javax.swing.JWindow {
    ...
    FSWindow() {
      ...
      if (GUI.isMacBrushedMetal()) setName(GUI.OVERRIDE_REDIRECT); // so no background flashing of the texture
      else setName(FULLSCREEN_NAME);
    ...
    private void setOffScreen() {
      if (DEBUG.DOCK && DEBUG.META) {
        ...
        if (Util.isMacLeopard()) {
          ...
          if (Color.black.equals(c)) setBackground(
            Color.darkGray
    ...
    public void paint(Graphics g) {
      ...
      if (DEBUG.Enabled) Log.debug(
        "paint " + this + ";  hidden=" + isHidden + "; offscreen=" + offscreen
      );
    ...
    private void setMenuBarEnabled(boolean enabled) {
      ...
      if (enabled) getRootPane()
        .setJMenuBar(getMainMenuBar()); else getRootPane().setJMenuBar(null);
    }
  }
  ...
  public static void dropFromNativeToWorking() {
    ...
    if (device.getFullScreenWindow() != null) {
      ...
      Log.debug(
        "clearing native full screen window:" +
        "\n\t  controlling device: " +
        device +
        "\n\tcur device FS window: " +
  ...
  public static synchronized void toggleFullScreen(boolean goNative) {
    ...
    if (fullScreenWorking) {
      if (goNative && !inNativeFullScreen()) enterFullScreenMode(
        true
    ...
    if (!inWorkingFullScreen()) {
      tufts.vue.VueToolbarController
        .getController()
        .setSelectedTool(
          tufts.vue.VueToolbarController.getController().getSelectedTool()
        );
    } else if (!goNative) {
  ...
  private static synchronized void enterFullScreenMode(final boolean goNative) {
    NDC.push("[FS->]");
    ...
    if (activeViewer != null) _activeFocal = activeViewer.getFocal();
    if (_activeFocal == null && activeMap != null) _activeFocal = activeMap;
    ...
    if (FullScreenWindow == null) {
      ...
      FullScreenWindow
        .getContentPane()
        .add(FullScreenViewer = new MapViewer(null, VIEWER_NAME));
    ...
    if (useNativeMode) {
      ...
      if (goNative) {
        if (
          ExtraDockWindowHiding && !DockWindow.AllWindowsHidden()
          /*&& !GUI.hasMultipleScreens()*/
        ) {
          nativeModeHidAllDockWindows = true;
          DockWindow.HideAllWindows();
        }
    ...
    if (
      ...
      activeViewer != null &&
      activeMap != null &&
      !(VUE.getActiveViewer().getFocal() instanceof tufts.vue.LWSlide)
    ) {
      ...
      ZoomTool.setZoomFitRegion(
        FullScreenViewer,
        activeViewer.getVisibleMapBounds()
      );
      //	else
    ...
    if (
      !goNative &&
      (Util.isUnixPlatform() ||
        (DockWindow.useManagedWindowHacks() && Util.isMacPlatform()))
    ) {
    ...
    GUI.invokeAfterAWT(
      new Runnable() {
        public void run() {
          if (DEBUG.PRESENT) Log.debug(
            "AWT thread activeTool.handleFullScreen for " + activeTool
          );
  ...
  private static synchronized void exitFullScreenMode() {
    NDC.push("[<-FS]");
    ...
    final Rectangle2D fullScreenVisibleMapBounds =
      FullScreenViewer.getVisibleMapBounds();
      ...
      GUI.invokeAfterAWT(
        new Runnable() {
          public void run() {
            ...
            // displayed
            FullScreenLastActiveViewer.grabVueApplicationFocus(
              "FullScreen.exit",
              null
            );
          }
      ...
      new Runnable() {
        public void run() {
          VueToolbarController
            .getActiveTool()
            .handleFullScreen(false, wasNative);
          ...
          try {
            if (!wasNative && returnViewer != null) {
              ...
              if (fullScreenMap == returnViewer.getMap()) {
                if (
                  returnViewer.getFocal() != fullScreenFocal
              ...
              if (
                returnViewer.getWidth() != returnViewer.getVisibleWidth() ||
                returnViewer.getHeight() != returnViewer.getVisibleHeight()
              ) {
                if (DEBUG.Enabled) Log.debug(
                  "size mismatch (viewer probably scroll-enabled) re-zoom"
                );
                ZoomTool.setZoomFitRegion(
                  returnViewer,
