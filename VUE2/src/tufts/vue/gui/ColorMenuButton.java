...
public class ColorMenuButton
  ...
  private final RecentlyUsedColorsManager colorManager =
    RecentlyUsedColorsManager.getInstance();
  /**
  ...
  public ColorMenuButton(Color[] pColors, boolean pHasCustom) {
    ...
    // use frame
    if (tufts.Util.isUnixPlatform()) popupWindow =
      new JWindow(); else popupWindow = new JFrame();
    if (tufts.Util.isWindowsPlatform()) popupWindow.setAlwaysOnTop(true);
    popupWindow.setName(COLOR_POPUP_NAME);
    if (!tufts.Util.isUnixPlatform()) ((JFrame) popupWindow).setUndecorated(
        true
      );
    //    popupWindow.setAlwaysOnTop(true);

    Component c = null;
    if (!tufts.Util.isUnixPlatform()) c =
      ((JFrame) popupWindow).getGlassPane(); else c =
      ((JWindow) popupWindow).getGlassPane();
    c.setVisible(true);
    c.addMouseListener(
      new MouseAdapter() {
        public void mousePressed(final MouseEvent e) {
          ...
          if (!tufts.Util.isUnixPlatform()) contentPane =
            ((JFrame) popupWindow).getContentPane(); else contentPane =
            ((JWindow) popupWindow).getContentPane();
        ...
        public void mouseExited(final MouseEvent e) {
          SwingUtilities.invokeLater(
            new Runnable() {
              public void run() {
                if (popupWindow.isVisible()) doClick();
              }
            }
          );
        }
      ...
      new WindowFocusListener() {
        ...
        public void windowLostFocus(WindowEvent e) {
          ...
          if (
            ...
            ((e.getOppositeWindow().getClass() == VueFrame.class))
          ) {
            return;
          }
  ...
  public JPanel buildMenu(Color[] colors, boolean custom) {
    ...
    for (int i = 0; i < colors.length; i++) {
      ...
      if (
        c != null &&
        blobColor != null &&
        blobColor.getRed() == c.getRed() &&
        blobColor.getBlue() == c.getBlue() &&
        blobColor.getGreen() == c.getGreen() &&
        blobColor.getAlpha() == c.getAlpha()
      ) {
    ...
    item.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          ...
          colorManager.updateRecentlyUsedColors(
  ...
  public void setColor(Color c) {
    ...
    if (c == null && mPropertyKey == null) return;
    if (c == null || c.getAlpha() == 0) setToolTipText("No Fill"); else if (
      c.getAlpha() == 255
    ) setToolTipText(
      String.format("RGB: %d,%d,%d", c.getRed(), c.getGreen(), c.getBlue())
    ); else setToolTipText(
      String.format(
        "RGB: %d,%d,%d @ %.0f%%",
        c.getRed(),
    ...
    if (i instanceof BlobIcon) ((BlobIcon) i).setColor(c);
    //    if (c == null)
  ...
  public void setButtonIcon(Icon i) {
    if (DEBUG.BOXES || DEBUG.TOOL) System.out.println(
      this + " setButtonIcon " + i
    );
  ...
  private void _setIcon(Icon i) {
    ...
    if (true || !GUI.isMacAqua()) {
      if (false) VueButtonIcon.installGenerated(
        this,
        i,
        d
      ); else VueButtonIcon.installGenerated(this, new MenuProxyIcon(i), d);
  ...
  protected void handleValueSelection(Color newPropertyValue) {
    if (DEBUG.TOOL) System.out.println(
      this + " handleValueSelection: newPropertyValue=" + newPropertyValue
    );
    ...
    ) return; // could be result of custom chooser
    ...
    if (newPropertyValue instanceof Action) {
      System.out.println(
        "Skipping setPropertyValue & firePropertyChanged for Action " +
        newPropertyValue
      );
    } else {
  ...
  protected void firePropertyChanged(Object oldValue, Object newValue) {
    if (getPropertyKey() != null) {
      ...
      if (listeners.length > 0) {
        PropertyChangeEvent event = new LWPropertyChangeEvent(
          this,
          getPropertyKey(),
          oldValue,
          newValue
        ...
        for (int i = 0; i < listeners.length; i++) {
          if (DEBUG.TOOL && (DEBUG.EVENTS || DEBUG.META)) System.out.println(
            this + " fires " + event + " to " + listeners[i]
          );
  ...
  protected Object runCustomChooser() {
    if (tufts.Util.isWindowsPlatform()) popupWindow.setAlwaysOnTop(false);
    Color c = tufts.vue.VueUtil.runColorChooser(
      "Select Custom Color",
      getColor(),
      this
    );
  ...
  /*private static String[] sTestNames = { "Black",
                                           ...
                                           "Green",
                                           "Blue" };*/
  private static Color[] sTestColors = {
    new Color(0, 0, 0),
    ...
    if (!popupWindow.isVisible()) {
      ...
      if (false) {
        ...
        location.translate(
          0,
          getHeight() +
          (getBorder() == null ? 0 : getBorder().getBorderInsets(this).bottom)
        );
      } else {
      ...
      GUI.keepLocationOnScreen(
        location,
        new Dimension(popupWindow.getWidth(), popupWindow.getHeight())
      );
