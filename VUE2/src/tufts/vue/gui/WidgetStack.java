 ...
 * @author Scott Fraize
 */
public class WidgetStack extends Widget implements Scrollable {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(WidgetStack.class);
  ...
  public WidgetStack(String name) {
    ...
    if (DEBUG.BOXES) setBorder(
      new javax.swing.border.LineBorder(Color.cyan, 4)
    );
  ...
  public void addPane(
    ...
    if (
      !isBooleanTrue(widget, TITLE_HIDDEN_KEY) && !title.startsWith("_")
    ...
    if (false) {
      ...
      if (
        widget.getBorder() == null && !(widget instanceof JScrollPane)
  ...
  public void addNotify() {
    ...
    super.addNotify();
    if (mExpanderCount == 0) if (DEBUG.WIDGET) out("no vertical expanders");
  ...
  private Dimension sizeTrack(String src) {
    final JComponent sizeTrack = (JComponent) getClientProperty(
      "VUE.sizeTrack"
    );
    if (sizeTrack != null) {
  ...
  private void updateDefaultExpander() {
    ...
    } else mDefaultExpander.setVisible(false);
  }
  ...
  private WidgetTitle findFirstOpenExpander() {
    ...
    while (i.hasNext()) {
      ...
      if (w.isExpander && w.mExpanded && w.isVisible()) return w;
    }
    return null;
  }

  public static final int TitleHeight = VueResources.getInt(
    "gui.widget.title.height",
    18
  );
  public static final Color TopGradient = VueResources.getColor(
    "gui.widget.title.background.top",
    108,
    149,
    221
  ), BottomGradient = VueResources.getColor(
    "gui.widget.title.background.bottom",
    80,
    123,
    197
  );
  ...
  private static final GradientPaint GradientEmbedded = new GradientPaint(
    0,
    0,
    new Color(79, 154, 240),
    0,
    TitleHeight,
    new Color(0, 133, 246)
  );
  ...
  class WidgetTitle extends Box implements java.beans.PropertyChangeListener {
    ...
    public WidgetTitle(String label, JComponent widget, boolean isExpander) {
      ...
      if (localName == null) localName = label;
      ...
      //name for this.
      if (
        localName.startsWith("Searching ") || localName.startsWith("Loading ")
      ...
      if (localName != null) {
        ...
        if (mTopColor != null) {
          Color botColor = VueResources.getColor(
            localKey + ".background.bottom",
            BottomGradient
          );
          mGradient =
            new GradientPaint(0, 0, mTopColor, 0, TitleHeight, botColor);
        } else {
          mGradient = Gradient;
        }
        mTitle.setText(VueResources.getString(localKey + ".text", label));
      } else mGradient = Gradient;

      if (mTopColor == null) mTopColor = TopGradient;
      ...
      addMouseListener(
        new tufts.vue.MouseAdapter(label) {
          ...
          public void mousePressed(MouseEvent e) {
            if (!isLocked) mIcon.setForeground(mTopColor.brighter());
          }

          public void mouseReleased(MouseEvent e) {
            if (!isLocked) mIcon.setForeground(Color.white);
          }
        }
      );
    ...
    public void addNotify() {
      super.addNotify();
      mEmbeddedStack =
        (SwingUtilities.getAncestorOfClass(WidgetStack.class, getParent()) !=
          null);
    }
    ...
    private void handleMouseClicked() {
      ...
      if (doExpand || DEBUG.WIDGET) {
        ...
        final JScrollPane s = (JScrollPane) SwingUtilities.getAncestorOfClass(
          JScrollPane.class,
          this
        );
        if (s != null) {
        ...
        if (!doExpand) return; // only get here if DEBUG.WIDGET
    ...
    public void propertyChange(java.beans.PropertyChangeEvent e) {
      ...
      if (DEBUG.WIDGET && (true || !key.equals("ancestor"))) out(
        GUI.name(e.getSource()) +
        " property \"" +
        key +
        "\" newValue=[" +
        GUI.name(e.getNewValue()) +
        "]"
      );
    ...
    private void handleWidgetDisplayChange(boolean visible) {
      ...
      if (visible) mExpandersOpen++; else mExpandersOpen--;

      if (DEBUG.WIDGET) out("VISIBLE EXPANDER COUNT: " + mExpandersOpen);

      if (mExpandersOpen < 0 || mExpandersOpen > mExpanderCount) Log.warn(
        this + "; handleWidgetDisplayChange=" + visible,
        new IllegalStateException(
          "WidgetStack: expanders claimed open: " +
          mExpandersOpen +
          ", expander count=" +
          mExpanderCount
        )
    ...
    private void setWidgetHidden(boolean hide) {
      ...
      if (mTitleVisible && isVisible() == !hide) return;
    ...
    private void paintGradient(Graphics2D g) {
      ...
      if (false && mEmbeddedStack) g.setPaint(
        GradientEmbedded
      ); else g.setPaint(mGradient);
  ...
  class MenuButton extends JButton {
    ...
    protected void paintComponent(Graphics g) {
      ...
      int w = getWidth();
      int h = getHeight();
      Image centerImg = VueResources
        .getImageIcon("dockWindow.panner.menu.raw.center")
        .getImage();
      Image leftImg = VueResources
        .getImageIcon("dockWindow.panner.menu.raw.leftcap")
        .getImage();
      Image rightImg = VueResources
        .getImageIcon("dockWindow.panner.menu.raw.rightcap")
        .getImage();
      Font macFont = new Font("Lucinda Grande", Font.BOLD, 11);
      Font windowsFont = new Font("Lucida Sans Unicode", Font.BOLD, 11);
      boolean isWindows = VueUtil.isWindowsPlatform();
      ...
      if (mouse_over) {
        centerImg =
          VueResources
            .getImageIcon("dockWindow.panner.menu.raw.center_ov")
            .getImage();
        leftImg =
          VueResources
            .getImageIcon("dockWindow.panner.menu.raw.leftcap.hover")
            .getImage();
        rightImg =
          VueResources
            .getImageIcon("dockWindow.panner.menu.raw.rightcap.hover")
            .getImage();
      }
      if (isWindows) {
        ...
        g.drawImage(
          ...
          w - 11,
          h / 2 - 7,
          rightImg.getWidth(null),
          rightImg.getHeight(null),
          this
        );
        g.setFont(font);
  ...
  static class MiscActionButton extends JLabel implements MouseListener {
    ...
    public void setIconPrefix(String prefix) {
      iconChar = prefix;
      if (iconChar != null) setIcon(
        VueResources.getImageIcon(iconChar + ".raw")
      );
    }
  ...
  public static void main(String args[]) {
    ...
    GUI
      .createDockWindow(
        VueResources.getString("dockWindow.WidgetStackTest.title"),
        s
      )
      .setVisible(true);
  }
