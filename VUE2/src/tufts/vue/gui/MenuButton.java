...
// menu-popups would automatically work also.
{
  ...
  public MenuButton() {
    ...
    addMouseListener(
      new tufts.vue.MouseAdapter(toString()) {
        public void mousePressed(MouseEvent e) {
          if (!MenuButton.this.isEnabled()) return;
          /*
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (actionAreaClicked) firePropertySetter();
  }
  ...
  protected void buildMenu(
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      if (icon == null) icon = makeIcon(value);
      if (icon != null) item.setIcon(icon);
      if (names != null) item.setText(names[i]);
    ...
    if (createCustom) {
      JMenuItem item = new JMenuItem(
        VueResources.getString("menubutton.custom")
      ); // todo: more control over this item
  ...
  protected void buildMenu(Class<T> enumType) {
    ...
    if (values == null) throw new Error(
      "no enum constants for (not an enum?) " + enumType
    );
    String[] names = new String[values.length];
    int i = 0;
    for (T e : values) names[i++] = e.toString();
  ...
  protected void handleMenuSelection(ActionEvent e) {
    ...
    handleValueSelection(
      ((JComponent) e.getSource()).getClientProperty(ValueKey)
    );
  }
  ...
  protected void firePropertySetter() {
    ...
    if (o instanceof Action) {
      if (o instanceof VueAction) ((VueAction) o).fire(this); else {
        Action a = (Action) o;
        a.actionPerformed(
          new ActionEvent(this, 0, (String) a.getValue(Action.NAME))
        );
      }
    } else {
