...
class DockRegion {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DockRegion.class);
  ...
  DockRegion(int y, int gravity, String name) {
    ...
    if (
      ...
      "illegal DockRegion gravity " + gravity
    );
  ...
  static DockRegion findRegion(Window w) {
    if (w.isVisible()) return findRegion(
      w.getY(),
      w.getHeight()
    ); else return null;
  }
  ...
  private static void clearAll() {
    Iterator i = sAllRegions.iterator();
    while (i.hasNext()) ((DockRegion) i.next()).mDockedWindows.clear();
  }
  ...
  static void assignAllMembers() {
    ...
    for (DockWindow dockWindow : DockWindow.AllWindows) {
      ...
      if (dockRegion != null) dockRegion.mDockedWindows.add(dockWindow);
    }
  ...
  int getRolledWidth(DockWindow dw) {
    ...
    if (rightSibling == null) return dw.getWidth();
    //return GUI.GScreenWidth - dw.getX();
    else return rightSibling.getX() - dw.getX();
  }
  ...
  private void updateRolledWidths() {
    ...
    while (i.hasNext()) {
      DockWindow dw = (DockWindow) i.next();
      if (dw.isRolledUp()) dw.setSize(getRolledWidth(dw), dw.getHeight());
    }
  }
  ...
  private void sortByX() {
    ...
    for (int k = 0; k < windows.length; k++) {
      ...
      if (k > 0) prev = (DockWindow) windows[k - 1];
      if (k + 1 < windows.length) next =
        (DockWindow) windows[k + 1]; else next = null;
  ...
  private DockWindow getSiblingToRight(DockWindow dw) {
    ...
    if (mDockedWindows.size() > next) return (DockWindow) mDockedWindows.get(
      next
    ); else return null;
  }
