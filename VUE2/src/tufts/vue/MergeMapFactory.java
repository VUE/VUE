...
public class MergeMapFactory {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MergeMapFactory.class);
  ...
  public MergeMapFactory(
    ...
    boolean bma = false;
    for (LWMap m : activeMaps) if (m == baseMap) {
      bma = true;
      break;
    }
  ...
  public LWMap createAsVoteMerge() {
    return createMergeMap(true);
  }

  public LWMap createAsWeightMerge() {
    return createMergeMap(false);
  }

  private LWMap createMergeMap(boolean asVote) {
    if (alreadyUsed) throw new Error(getClass() + ": already used");
    alreadyUsed = true;

    if (excludeNodesOnBaseMap && !includeSecondaryMaps) throw new Error(
      "merge-map would have no contents in this configuration"
    );

    if (asVote) createAggregateAndFillMap(
      VoteAggregate.class
    ); else createAggregateAndFillMap(WeightAggregate.class);
  ...
  public void setFilterOnBaseMap(boolean doFilter) {
    includeSecondaryMaps = !doFilter;
  }

  public void setExcludeNodesFromBaseMap(boolean doExclude) {
    excludeNodesOnBaseMap = doExclude;
  }
  ...
  public boolean isBaseMapActive() {
    return !baseMapInactive;
  }

  public void setNodeIntervalBoundaries(List<Double> nib) {
    if (DEBUG.MERGE) {
      Log.debug("nodeIntervals:");
      tufts.Util.dump(nib);
    }
  ...
  private List<LWMap> computeActiveMaps() {
    ...
    for (LWMap map : maps) {
      if (statusIter.hasNext()) {
        if (statusIter.next().booleanValue()) actives.add(
          map
  ...
  private void createAggregateAndFillMap(final Class clazz) {
    ...
    if (excludeNodesOnBaseMap || baseMapInactive) {
      if (DEBUG.MERGE) Log.debug(
        "excluding base-map nodes from merge; active=" + !baseMapInactive
      );
    } else mergeInNodes(baseMap, voteAggregate);
    ...
    // list.
    final Collection<LWComponent> allMergedNodes = new ArrayList(
      outputMap.getChild(0).getChildren()
    );

    for (LWComponent c : allMergedNodes) if (isMergeSkipped(c)) Log.warn(
      "unexpected content in merge results: " + c
    );

    Log.info("pre-link content: " + tufts.Util.tags(allMergedNodes));

    if (voteAggregate != null) installLinksForVotes(
      allMergedNodes,
      cms,
      voteAggregate
    ); else installLinksAndStylesForWeights(
      allMergedNodes,
      cms,
      weightAggregate
    );
  ...
  private static boolean isMergeSkipped(LWComponent c) {
    if (c == null || c.hasFlag(Flag.ICON)) return true;
  ...
  private void mergeInNodes(
    ...
    for (LWComponent srcNode : sourceMap.getAllDescendents(ChildKind.PROPER)) {
      if (isMergeSkipped(srcNode)) continue;
      ...
      //-----------------------------------------------------------------------------
      if (isVoting && !voteAggregate.isNodeVotedIn(mergeKey)) continue;

      if (excludeNodesOnBaseMap && baseMapKeys.contains(mergeKey)); // skip this srcNode
      else copyInNode(sourceMap, srcNode, mergeKey);
    }
  }
  ...
  private void copyInLink(
    ConnectivityMatrixList cms,
    Object headKey,
    Object tailKey,
    LWLink link
  ) {
  ...
  private void installLinksAndStylesForWeights(
    ...
    for (LWComponent node : allMergedNodes) {
      ...
      if (styleColor != null) node.setTextColor(Style.hexToColor(styleColor));
    }
    ...
    for (LWComponent head : allMergedNodes) {
      ...
      for (LWComponent tail : allMergedNodes) {
        ...
        link.setStrokeColor(
          Style.hexToColor(linkStyle.getAttribute("background"))
        );
  ...
  private void annotateNodeSource(
    LWMap sourceMap,
    LWComponent sourceNode,
    LWComponent newMergeNode
  ) {
    annotate(
      new StringBuilder(annotationPartForMap(sourceMap)),
      sourceNode,
      newMergeNode
    );
  }
  ...
  private void annotate(
    ...
    if (target instanceof LWImage || DEBUG.TEST) { // images have no rollovers to show the merge data
      ...
      if (counter == null) counter = target.putClientData(new Counter());
      counter.count++;
      final String notesAnno = String.format(
        "[in:%d:%s]",
        counter.count,
        anno.toString()
      );
  ...
  private static final class LinksCache extends ArrayList<LWLink> {
    ...
    /**/private LinksCache(LWMap map) {
      ...
      for (LWLink link : map.getDescendentsOfType(LWLink.class)) add(link);
   ...
   * maps. */
  private void annotateLinkSources(
    final ConnectivityMatrixList sources,
    final Object key1,
    final Object key2,
    final LWLink newLink
  ) {
    ...
    for (ConnectivityMatrix matrix : sources) {
      if (matrix.getConnection(key1, key2) <= 0) continue;
    ...
    if (matchCount > 1) newLink.setStrokeWidth(matchCount); else if (
      matchCount <= 0
    ) Log.warn("no sources for: " + newLink);
  }

  private static void putMergeAnnotation(
    final LWComponent mergeComponent,
  ...
  private void annotateMap(boolean asVote) {
    ...
    p.println(
      "Merge property: " + edu.tufts.vue.compare.Util.getMergeProperty()
    );
    ...
    for (LWMap map : activeMaps) {
      if (map != baseMap) p.println("Source: " + map.getDisplayLabel());
    }
  ...
  public String toString() {
    return getClass().getSimpleName() + "=" + outputMap.getLabel();
  }
}
