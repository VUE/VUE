...
public class DataSourceListCellRenderer
  extends DefaultListCellRenderer { // implements ActionListener

  public static final int FirstColumn = 28;

  private final Icon myComputerIcon = VueResources.getImageIcon(
    "dataSourceMyComputer"
  );
  private final Icon savedResourcesIcon = VueResources.getImageIcon(
    "dataSourceSavedResources"
  );
  ...
  private Border DividerBorder = new DashBorder(
    Color.LIGHT_GRAY,
    false,
    true,
    false,
  ...
  private final Color AlternateRowColor = VueResources.getColor(
    "gui.dataSourceList.alternateRowColor",
    237,
    243,
    253
  );
  ...
  private static final Color DataSourcesColor = VueResources.getColor(
    "gui.widget.title.resultsPane.background.top"
  );
  ...
  public Component getListCellRendererComponent(
    ...
    if (value instanceof edu.tufts.vue.dsm.DataSource) {
      ...
      // (edu.tufts.vue.dsm.DataSource)value;
      edu.tufts.vue.dsm.impl.VueDataSource ds =
        (edu.tufts.vue.dsm.impl.VueDataSource) value;
      ...
      if (ds.isAvailable()) {
        ...
        isLoading = true;
        if (DEBUG.Enabled) mLabel.setForeground(
          Color.gray
        ); else mLabel.setForeground(Color.black);
      }
      ...
      if (value instanceof LocalFileDataSource) mIconLabel.setIcon(
        myComputerIcon
      ); else if (value instanceof FavoritesDataSource) mIconLabel.setIcon(
        savedResourcesIcon
      ); else if (value instanceof RemoteFileDataSource) mIconLabel.setIcon(
        remoteIcon
      ); else if (
        value instanceof edu.tufts.vue.rss.RSSDataSource
      ) mIconLabel.setIcon(rssIcon); else mIconLabel.setIcon(null);
    } else {
      System.out.println(
        "DataSourceList: unhandled data source: " + tufts.Util.tags(value)
      );
      mRow.setBorder(DividerBorder);
      mCheckBox.setVisibility(false);
    ...
    if (
      !isLoading && value == DataSourceList.IndicatedDragOverValue
  ...
  public void actionPerformed(ActionEvent e) {
    GetLibraryInfoDialog getLibraryInfoDialog = new GetLibraryInfoDialog(
      infoDataSource
    );
   ...
   * super.getListCellRendererComponent(list,((DataSource)value).getDisplayName(),
   * index, iss, chf);
   * } else if (value instanceof edu.tufts.vue.dsm.DataSource) {
   * super.getListCellRendererComponent(list,((edu.tufts.vue.dsm.DataSource)value)
   * .getRepositoryDisplayName(), index, iss, chf);
   * }
   *
   *
   * if (value instanceof String) {
  ...
  // adjust the spacing within the name panel (name and status) so labels are
  // flush with edges
  /*
   * JLabel displayNameLabel = new JLabel(displayName);
   * double displayNameWidth;
   * while ((displayNameWidth = displayNameLabel.getPreferredSize().getWidth()) >
   * 155) {
