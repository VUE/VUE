...
class Slides {
  ...
  protected static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(Slides.class);
  ...
  private static LWNode createNode(LWComponent node, Resource r) {
    if (!r.equals(node.getResource())) Log.warn(
      "createNode: resource not on node: " + node + "; r=" + r
    );

    String label;

    if (node.hasLabel()) label = node.getLabel(); else label = r.getTitle();
  ...
  private static List<LWComponent> getContentToCopy(LWComponent node) {
    ...
    while (i.hasNext()) {
      ...
      ) i.remove(); else if (c instanceof LWLink) i.remove();
    }
  ...
  private static class TitledImage {
    ...
    TitledImage(LWComponent imageNode, LWImage i) {
      ...
      if (imageNode != null) {
        String txt;
        if (imageNode.hasLabel()) txt = imageNode.getLabel(); else txt =
          i.getLabel();
        if (txt == null || txt.length() == 0) txt = "Image";
  ...
  private static LWSlide buildPathwaySlide(
    final LWSlide slide,
    final LWPathway.Entry entry
  ) {
    //final LWSlide slide = CreatePathwaySlide(entry);
    final LWNode title = NodeModeTool.buildTextNode(
      entry.node.getDisplayLabel()
    );
    final MasterSlide master = entry.pathway.getMasterSlide();
    final CopyContext cc = new CopyContext(false);
    ...
    for (LWComponent c : getContentToCopy(entry.node)) {
      if (LWNode.isImageNode(c)) {
        ...
        if (copy instanceof LWImage) images.add(
          new TitledImage((LWImage) c, (LWImage) copy)
        ); else text.add(copy);
      }
    }
    ...
    if (LWNode.isImageNode(entry.node)) {
      if (images.size() == 0) images.add(
        new TitledImage(
          (LWComponent) null,
          ((LWNode) entry.node).getImage().duplicate(cc)
        )
      ); else images.add(
        new TitledImage(
          entry.node,
    ...
    for (TitledImage t : images) {
      ...
      added.add(t.image);
      if (t.title != null) added.add(t.title); // keep titles over images (add after)
    }
    ...
    if (DEBUG.PRESENT || DEBUG.STYLE) Log.debug(
      "LAYING OUT CHILDREN in " + slide + "; parent=" + slide.getParent()
    );

    slide.setSize(SlideWidth, SlideHeight);

    if (text.size() > 1) layoutText(slide, text);
    ...
    if (images.size() > 0) layoutImages(slide, images);
  ...
  private static void layoutImages(LWSlide slide, List<TitledImage> images) {
    ...
    if (images.size() == 1) {
      ...
      image.setLocation(
        (slide.getWidth() - image.getWidth()) / 2,
      ...
      if (label != null) {
        label.setLocation(
          image.getX() + (image.getWidth() - label.getWidth()) / 2,
      ...
      final float maxImageHeight =
        (imageRegionHeight -
          (((images.size() - 1) * verticalMargin) +
            images.size() * labelHeight)) /
        images.size();
  ...
  protected static void synchronizeResources(final LWSlide slide, Sync type) {
    ...
    if (!slide.canSync()) {
      Util.printStackTrace(
        "Sync not permitted: " + slide + " type(" + type + ")"
      );
      return;
    }
    ...
    if (
      slide.numChildren() == 0 &&
      slide.getEntry() != null &&
      (type == Sync.ALL || type == Sync.TO_SLIDE)
    ) {
    ...
    if (DEBUG.Enabled) outf(
      "NODE/SILDE SYNCHRONIZATION; type(%s) ---\n\t NODE: %s\n\tSLIDE: %s",
      type,
      node,
      slide
    );
    ...
    if (node.hasResource()) {
      if (nodeUnique.add(node.getResource())) nodeSources.put(
        node.getResource(),
        node
      );
    }
    for (LWComponent c : node.getAllDescendents()) {
      if (c.hasResource()) {
        if (nodeUnique.add(c.getResource())) nodeSources.put(
          c.getResource(),
          c
        );
      }
    ...
    if (DEBUG.Enabled) {
      for (Resource r : nodeUnique) outf(
        "%40s: %s",
    ...
    for (LWComponent c : slide.getAllDescendents()) {
      if (c.hasResource()) {
        ...
        if (nodeUnique.contains(r)) {
          ...
          if (DEBUG.Enabled) outf(
            "%40s: %s",
            "ALREADY ON NODE, IGNORE FOR SLIDE",
            Util.tags(r)
          );
        } else {
    ...
    if (type == Sync.ALL || type == Sync.TO_SLIDE) {
      ...
      for (Resource r : nodeUnique) {
        ...
        if (false && r.isImage()) {
          ...
          if (LWNode.isImageNode(newNode)) ((LWNode) newNode).getImage()
            .setSyncSource(nodeSources.get(r));
        }
        added.add(newNode);
      }
      ...
      if (added.size() > 0) {
        ...
        float x = slide.getWidth() / 4, y = SlideMargin;
