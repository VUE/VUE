...
/**
 * * A wrapper for Fedora Object implementation of osid.dr.Asset which can be
 * used as the user object in a
...
public class AssetResource extends URLResource {
  ...
  public static final int DC_NAMESPACE_LENGTH = 3; // (dc:) the namespacepresent in metadata fields. Beginning is
  // chopped off for clean rendering
  public static final String DISSEMINATION_ID = "getDublinCore";
  public static final String DC_FIELD_ID = "fedora.disseminationURL";
  public static final String DISSEMINATION_INFOSTRUCTURE_ID =
    "fedora.dissemination";
  ...
  private Asset asset;
  private CastorFedoraObject castorFedoraObject; // stripped version of fedora object for saving and restoring in

  // castor will work only with this implementation of DR API.
  ...
  public void setAsset(Asset asset)
    ...
    setSpec(
      asset
        .getInfoField(new tufts.oki.dr.fedora.PID(VUE_DEFAULT_VIEW_ID))
        .getValue()
        .toString()
    );
    setClientType(Resource.ASSET_FEDORA);
  ...
  private void setPropertiesByAsset() {
    try {
      if (
        !(asset.getAssetType().getKeyword().equals("fedora:BDEF") ||
          asset.getAssetType().getKeyword().equals("fedora:BMECH"))
      ) {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        factory.setValidating(false);
        // InputStream dublinCoreInputStream = new
        // ByteArrayInputStream(((MIMETypedStream)(asset.getInfoField(new
        // PID("getDublinCore")).getValue())).getStream());
        URL dcUrl = getDCUrl();
        if (dcUrl != null) {
          InputStream dublinCoreInputStream = dcUrl.openStream();
          Document document = factory
            .newDocumentBuilder()
            .parse(dublinCoreInputStream);
          for (int i = 0; i < dcFields.length; i++) {
            NodeList list = document.getElementsByTagName(
              DC_NAMESPACE + dcFields[i]
            );
            if (list != null && list.getLength() != 0) {
              // only picks the first element
              if (list.item(0).getFirstChild() != null) setProperty(
                dcFields[i],
  ...
  private URL getDCUrl() {
    ...
    try {
      boolean flag = true;
      osid.dr.InfoFieldIterator i = asset
        .getInfoRecord(new PID(DISSEMINATION_ID))
        .getInfoFields();
  ...
  public static AbstractAction getFedoraAction(
    osid.dr.InfoRecord infoRecord,
    osid.dr.DigitalRepository dr
  ) throws osid.dr.DigitalRepositoryException {
    final DR mDR = (DR) dr;
    final tufts.oki.dr.fedora.InfoRecord mInfoRecord =
      (tufts.oki.dr.fedora.InfoRecord) infoRecord;

    try {
      AbstractAction fedoraAction = new AbstractAction(
        infoRecord.getId().getIdString()
      ) {
        public void actionPerformed(java.awt.event.ActionEvent actionEvent) {
          try {
            // String fedoraUrl =
            // mDR.getFedoraProperties().getProperty("url.fedora.get","http://vue-dl.tccs..tufts.edu:8080/fedora/get");
            String fedoraUrl = mInfoRecord
              .getInfoField(
                new PID(
                  FedoraUtils.getFedoraProperty(
                    mDR,
                    "DisseminationURLInfoPartId"
                  )
                )
              )
              .getValue()
      ...
      throw new osid.dr.DigitalRepositoryException(
        "FedoraUtils.getFedoraAction " + ex.getMessage()
      );
    }
  }
}
