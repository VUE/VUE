...
public class PublishUtil implements tufts.vue.DublinCoreConstants {
  ...
  public static final String RESOURCE_FOLDER =
    "resource:resources" + File.separator;
  ...
  public static File createZip(LWMap map, Vector resourceVector)
    throws IOException, URISyntaxException, CloneNotSupportedException, ZipException {
    System.out.println("Creating a zip file");
    IMSCP imscp = new IMSCP();
    try {
      if (map.getFile() == null) {
        VueUtil.alert(
          VueResources.getString("dialog.exporterror.message"),
          VueResources.getString("dialog.exporterror.title")
        );
        return null;
      }
      String saveEntry =
  ...
  public static File createIMSCP(Vector resourceVector)
    ...
    Properties props = tufts.vue.VUE
      .getActiveMap()
      .getMetadata()
      .asProperties();
    ...
    while (i.hasNext()) {
      ...
      if (b.booleanValue()) {
        ...
        IMSCPResources +=
        getResourceTag(r.getProperties().asProperties(), entry, resourceCount);
        IMSCPOrganization +=
        getItemTag(
          "ITEM" + resourceCount,
        ...
        replaceResource(
          ...
          Resource
            .getFactory()
            .get(IMSCP.RESOURCE_FILES + File.separatorChar + file.getName())
        );
    ...
    IMSManifest =
      IMSManifest.replaceAll(IMSCP_MANIFEST_METADATA, IMSCPMetadata).trim();
    IMSManifest =
      IMSManifest.replaceAll(IMSCP_MANIFEST_ORGANIZATION, IMSCPOrganization);
    IMSManifest =
      IMSManifest.replaceAll(IMSCP_MANIFEST_RESOURCES, IMSCPResources);

    File IMSManifestFile = File.createTempFile("imsmanifest", ".xml");
    BufferedOutputStream fos = new BufferedOutputStream(
      new FileOutputStream(IMSManifestFile)
    );
  ...
  public static String getResourceTag(
    Properties dcFields,
    String entry,
    int resourceCount
  ) {
  ...
  public static String readRawManifest() {
    ...
    try {
      BufferedInputStream fis = new BufferedInputStream(
        VueResources.getURL("imsmanifest").openStream()
      );
