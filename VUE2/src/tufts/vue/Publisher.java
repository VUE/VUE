...
public class Publisher
  extends JDialog
  implements ActionListener, tufts.vue.DublinCoreConstants {
  ...
  private static final String DUPLICATE_OBJ_ERR_MESG = VueResources.getString(
    "dialog.mapexists.message"
  );
  private static final String DUPLICATE_OBJ_ERR_TITLE = VueResources.getString(
    "dialog.mapexists.title"
  );

  public static final String[] PUBLISH_INFORMATION = {
    VueResources.getString("publisher.publishinfo0"),
  ...
  public static final String[] MODE_LABELS = {
    VueResources.getString("publisher.maponly"),
    VueResources.getString("publisher.mapandresources"),
    VueResources.getString("publisher.vpk"),
  };

  private static final String NEXT_BUTTON_TEXT = VueResources.getString(
    "button.next.label"
  );
  private static final String BACK_BUTTON_TEXT =
    "< " + VueResources.getString("button.back.label");
  private static final String FINISH_BUTTON_TEXT = VueResources.getString(
    "button.finish.label"
  );
  private static final String CANCEL_BUTTON_TEXT = VueResources.getString(
    "button.cancel.lable"
  );
  private static final String PUBLISH_BUTTON_TEXT = VueResources.getString(
    "button.publish.label"
  );
  private static final String DONE_BUTTON_TEXT = VueResources.getString(
    "button.done.label"
  );
  ...
  org.osid.shared.Type fedoraDataSourceType =
  ...
  public boolean isMapSaved() {
    if (VUE.getActiveMap() == null) {
      VueUtil.alert(
        VueResources.getString("dialog.mapnotsave.message"),
        VueResources.getString("dialog.mapnotsave.title")
      );
      return false;
  ...
  private void setUpRepositorySelectionPanel() {
    ...
    JLabel repositoryLabel = new JLabel(
      VueResources.getString("jlabel.repositoryinstance")
    );
  ...
  private void setUpWorkspaceSelectionPanel() {
    ...
    JLabel wLabel = new JLabel(
      VueResources.getString("jlabel.selectworkspace") +
      ((edu.tufts.vue.dsm.DataSource) repList.getSelectedValue()).getRepositoryDisplayName()
    );
    wLabel.setBorder(BorderFactory.createEmptyBorder(15, 10, 0, 0));
    wPanel.add(wLabel, BorderLayout.NORTH);
    edu.tufts.vue.dsm.DataSource selectedDataSource =
      (edu.tufts.vue.dsm.DataSource) repList.getSelectedValue();
    wTree = new JTree(getWorkSpaceTreeModel(selectedDataSource));
    wTree
      .getSelectionModel()
      .setSelectionMode(TreeSelectionModel.SINGLE_TREE_SELECTION);
    wTree.addTreeSelectionListener(
  ...
  private void setUpModeSelectionPanel() {
    ...
    JLabel publishLabel = new JLabel(
      VueResources.getString("jlabel.publishas")
    );
  ...
  private void setUpPublishPanel() {
    JLabel pLabel = new JLabel(
      VueResources.getString("jlabel.publishingto") +
      ((edu.tufts.vue.dsm.DataSource) repList.getSelectedValue()).getRepositoryDisplayName(),
      VueResources.getImageIcon("dsv.statuspanel.waitIcon"),
      JLabel.CENTER
    );
    ...
    if (e.getActionCommand().equals(CANCEL_BUTTON_TEXT)) {
      ...
      if (
        ...
        DefaultMutableTreeNode treeNode =
          (DefaultMutableTreeNode) _tp.getLastPathComponent();
        String fileName = VUE.getActiveMap().getFile().getName();
        org.osid.repository.Asset asset =
          ((SakaiSiteUserObject) (treeNode.getUserObject())).getAsset();
        /**
         * int confirm = 0;
         * try {
      ...
      if (repList.getSelectedValue() == null) {
        alert(
          this,
          VueResources.getString("dialog.norepository.message"),
          VueResources.getString("dialog.norepository.title")
        );
        return;
      }
      getContentPane().remove(rPanel);
    ...
    try {
      if (
        ds
          .getRepository()
          .getType()
          .isEqual(edu.tufts.vue.dsm.DataSourceTypes.FEDORA_REPOSITORY_TYPE)
      ) {
        if (publishMapRButton.isSelected()) FedoraPublisher.uploadMap(
          ds,
          VUE.getActiveMap()
        ); else if (
          publishMapAllRButton.isSelected()
        ) FedoraPublisher.uploadMapAll(ds, VUE.getActiveMap()); else if (
          publishZipRButton.isSelected()
        ) FedoraPublisher.uploadArchive(ds, VUE.getActiveMap()); else alert(
          VUE.getDialogParent(),
          VueResources.getString("dialog.publishnotsupported.message"),
          VueResources.getString("dialog.publishnotsupported.title")
        );
      } else if (
        ds
          .getRepository()
          .getType()
          .isEqual(edu.tufts.vue.dsm.DataSourceTypes.SAKAI_REPOSITORY_TYPE)
      ) {
        DefaultMutableTreeNode treeNode =
          (DefaultMutableTreeNode) _tp.getLastPathComponent();
        String siteId =
          ((SakaiSiteUserObject) (treeNode.getUserObject())).getId();
        ...
        if (isFilePresent(asset, fileName)) {
          confirm =
            VueUtil.confirm(
              this,
              DUPLICATE_OBJ_ERR_MESG,
              DUPLICATE_OBJ_ERR_TITLE,
      ...
      alert(
        this,
        VueResources.getString("dialog.notauthorized.message") +
  ...
  private DefaultTreeModel getWorkSpaceTreeModel(
    edu.tufts.vue.dsm.DataSource dataSource
  ) {
    String ROOT_LABEL = "Sites";
    javax.swing.tree.DefaultMutableTreeNode root =
      new javax.swing.tree.DefaultMutableTreeNode(ROOT_LABEL);
    ...
    try {
      ...
      while (assetIterator.hasNextAsset()) {
        ...
        if (
          asset
            .getAssetType()
            .isEqual(DataSourceTypes.SAKAI_COLLECTION_ASSET_TYPE)
        ) {
  ...
  private void addWorkSiteNode(
    ...
    if (
      !asset.getAssetType().isEqual(DataSourceTypes.SAKAI_COLLECTION_ASSET_TYPE)
    ) {
      return;
    }
    ...
    DefaultMutableTreeNode nextTreeNode = new DefaultMutableTreeNode(
      userObject
    );
    treeModel.insertNodeInto(nextTreeNode, parentNode, 0);

    org.osid.repository.AssetIterator assetIterator = asset.getAssetsByType(
      DataSourceTypes.SAKAI_COLLECTION_ASSET_TYPE
    );
    logger.debug("asset is " + asset.getDisplayName());
  ...
  class DatasourceListCellRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(
      ...
      JLabel label = (JLabel) super.getListCellRendererComponent(
        list,
        (value instanceof edu.tufts.vue.dsm.DataSource
            ? ((edu.tufts.vue.dsm.DataSource) value).getRepositoryDisplayName()
            : value),
        index,
        isSelected,
        cellHasFocus
  ...
  > getPublishableDatasources(org.osid.shared.Type type) {
    edu.tufts.vue.dsm.DataSource[] datasources =
      edu.tufts.vue.dsm.impl.VueDataSourceManager
        .getInstance()
        .getDataSources();
    java.util.List<edu.tufts.vue.dsm.DataSource> resourceList = new ArrayList<
      edu.tufts.vue.dsm.DataSource
    >();
    for (int i = 0; i < datasources.length; i++) {
      try {
        if (datasources[i].getRepository() != null) {
          if (
            (datasources[i].getRepository().getType() != null) &&
            (datasources[i].getRepository().getType().isEqual(type))
          ) {
  ...
  public static boolean isFilePresent(
    ...
    while (assetIterator.hasNextAsset()) {
      ...
      if (
        (SakaiPublisher
            .makeSakaiFolderFromVueMap(fileName)
            .equals(childAsset.getDisplayName()))
      ) {
        return true;
      }
    }
