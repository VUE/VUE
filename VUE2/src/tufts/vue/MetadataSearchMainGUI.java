 ...
 *
 */
public class MetadataSearchMainGUI
  extends JPanel
  implements ActiveListener<LWMap> {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MetadataSearchMainGUI.class);
  ...
  private static final Object[] SearchDomainsAll =
    SearchAction.SearchType.All();
  private static final Object[] SearchDomains = DEBUG.TEST
    ? SearchDomainsLimited
    : SearchDomainsAll;

  private static final Object[] ResultActionsAll = SearchAction.ResultOp.All();
  private static final Object[] ResultActionsLimited = {
    SearchAction.RA_SELECT,
    SearchAction.RA_SHOW,
    SearchAction.RA_HIDE,
  };
  private static final Object[] ResultActions = DEBUG.TEST
    ? ResultActionsLimited
    : ResultActionsAll;
  private static final Object[] ResultActionsMultiMap = {
    SearchAction.RA_COPY,
  };

  private static final Object[] LogicOps = {
    SearchAction.Operator.AND,
    SearchAction.Operator.OR,
  };

  private static final boolean DEBUG_LOCAL = false;

  public static final int BUTTON_COL_WIDTH = 24;
  ...
  private final String SAVED_SEARCH_STR = VueResources.local(
    "searchgui.savedsearches"
  );
  private final String SAVE_SEARCH_STR = VueResources.local(
    "searchgui.savesearch"
  );
  private final String RUN_SEARCH_STR = VueResources.local(
    "searchgui.runsearch"
  );
  ...
  private void runSaveAction(ActionEvent e) {
    if (searchTermsTbl.isEditing()) searchTermsTbl
      .getCellEditor()
      .stopCellEditing();
  ...
  private String getSearchName(int rowCount) {
    String inputValue = (String) VueUtil.input(
      null,
      VueResources.local("searchgui.entersearchname"),
      null,
  ...
  public MetadataSearchMainGUI(DockWindow w) {
    ...
    saveSearchAction =
      new AbstractAction(SAVE_SEARCH_STR) {
    ...
    runSearchAction =
      new AbstractAction(RUN_SEARCH_STR) {
        public void actionPerformed(ActionEvent e) {
          runSavedSearch();
        }
      };
  ...
  private Object getChosen(final JComponent jc) {
    ...
    if (jc instanceof javax.swing.JComboBox) {
      ...
      final ButtonGroup group = (ButtonGroup) jc.getClientProperty(
        ButtonGroup.class
      );
      if (group != null) {
        ...
        Log.debug(
          "getChosen: buttonModel.getSelectedObjects: " + Util.tags(objs)
        );
  ...
  private JComboBox getComboBox(Object[] choices, ItemListener listener) {
    ...
    if (true) combo.setPreferredSize(combo.getMinimumSize());
  ...
  public class MetaSearchPanel
    ...
    final ItemListener searchTypesListener = new ItemListener() {
      public void itemStateChanged(ItemEvent ie) {
        ...
        } else Log.warn("Unknown selected search-type: " + Util.tags(selected));
    ...
    final ItemListener locationChoiceListener = new ItemListener() {
      public void itemStateChanged(ItemEvent e) {
        if (e.getStateChange() == ItemEvent.SELECTED) {
          ...
          if (
            SEARCH_SCOPE_ALL_OPEN_MAPS.equals(e.getItem())
          ) optionsPanel.switchChoices(
            RESULTS,
            ResultActionsMultiMap
          ); else optionsPanel.switchChoices(RESULTS, ResultActions);
        }
      }
    };
    ...
    private static final int HALF_GUTTER = 4;
    private static final int GUTTER = HALF_GUTTER * 2;

    final Insets labelInsets = new Insets(
      HALF_GUTTER,
      HALF_GUTTER,
      HALF_GUTTER,
      0
    ), textFieldInsets = new Insets(
      HALF_GUTTER,
      HALF_GUTTER,
      HALF_GUTTER,
      HALF_GUTTER
    ...
    public MetaSearchPanel() {
      ...
      labelScope =
        new JLabel(VueResources.local("searchgui.maps"), SwingConstants.RIGHT);
      //labelDomain = new JLabel(VueResources.local("searchgui.searchtype"), SwingConstants.RIGHT); // too long
      labelDomain =
        new JLabel(
          VueResources.local("searchgui.search") + ":",
          SwingConstants.RIGHT
        ); // short & sweet
      labelResult =
        new JLabel(
          GUI.ensureColon(VueResources.local("searchgui.results")),
          SwingConstants.RIGHT
        );

      choiceScope =
        getChooser(SearchScopes, locationChoiceListener, "searchScope");
      choiceDomain =
        getChooser(SearchDomains, searchTypesListener, "searchDomain");
      choiceResult =
        getChooser(ResultActions, resultsTypeListener, "resultAction");
      ...
      searchTermsTbl.addMouseListener(
        new java.awt.event.MouseAdapter() {
          public void mouseReleased(java.awt.event.MouseEvent evt) {
            if (evt.getX() > searchTermsTbl.getWidth() - BUTTON_COL_WIDTH) {
              ...
              if (
                searchTermsTbl.getSelectedColumn() == buttonColumn &&
      ...
      saveButton.addActionListener(
        new ActionListener() { // TODO: CHRIST: MORE REPEATED CODE???
          public void actionPerformed(ActionEvent e) {
      ...
      searchButton.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            ...
            MetadataSearchMainGUI.this.searchType =
              (SearchType) getChosen(choiceDomain);
            MetadataSearchMainGUI.this.termsAction =
              new SearchAction(searchTerms);
      ...
      searchPanelGBC.insets =
        new Insets(HALF_GUTTER, HALF_GUTTER, HALF_GUTTER, HALF_GUTTER);
      ...
      topPanelGBC.insets =
        new java.awt.Insets(HALF_GUTTER, HALF_GUTTER, 0, HALF_GUTTER);
      topPanelGBC.gridx = 0;
      ...
      topPanelGBC.gridy = 3;
      topPanelGBC.insets =
        new java.awt.Insets(0, HALF_GUTTER, HALF_GUTTER, HALF_GUTTER);
  ...
  public class MetadataPanel
    ...
    public MetadataPanel() {
      ...
      searchResultTbl.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
      searchResultTbl.setDefaultRenderer(
        java.lang.Object.class,
        new SavedSearchTableRenderer(searchResultModel)
      );
      //searchResultTbl.setCellEditor( new SearchResultTableEditor(new JCheckBox()));

      ((DefaultCellEditor) searchResultTbl.getDefaultEditor(
          java.lang.Object.class
        )).setClickCountToStart(1);
  ...
  public void adjustHeaderTableColumnModel() {
    if (searchHeaderTbl == null) return;
    ...
    if (searchHeaderTbl.getModel().getColumnCount() == 3) {
      searchHeaderTbl
        .getColumnModel()
        .getColumn(0)
        .setCellRenderer(new SearchTermsTableHeaderRenderer());
      searchHeaderTbl
        .getColumnModel()
        .getColumn(1)
        .setCellRenderer(new ComboBoxAndOrRenderer(andOrCmbBox));
      searchHeaderTbl
        .getColumnModel()
        .getColumn(2)
        .setCellRenderer(new SearchTermsTableHeaderRenderer());
      searchHeaderTbl
        .getColumnModel()
        .getColumn(1)
        .setCellEditor(new ComboBoxAndOrEditor(andOrCmbBox));
      searchHeaderTbl
        .getColumnModel()
        .getColumn(2)
        .setCellEditor(new AddButtonTableCellEditor());
  ...
  static final JLabel EmptyLabel = new JLabel(DEBUG.BOXES ? "(hid)" : "");
    ...
    public Component getTableCellEditorComponent(
      ...
      if (vColIndex == 1 || vColIndex == 2) {
        SearchTermsTableModel model =
          (SearchTermsTableModel) searchTermsTbl.getModel();
  ...
  public class AddButtonTableCellEditor
  ...
  public class ComboBoxAndOrRenderer implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (searchTermsTbl.getModel().getRowCount() > 1) {
        ...
        if (Util.isMacLeopard()) combo.putClientProperty(
  ...
  public void setCategorySearch() {
    ...
    SearchTermsTableModel model =
      (SearchTermsTableModel) searchTermsTbl.getModel();
    SearchHeaderTableModel headerModel =
      (SearchHeaderTableModel) searchHeaderTbl.getModel();
  ...
  class SearchTermsTableHeaderRenderer extends DefaultTableCellRenderer {
    ...
    public java.awt.Component getTableCellRendererComponent(
      ...
      if (col == headerButtonColumn) {
        ...
        table.getModel().getColumnCount() == 3 && col == headerValueColumn
      ) {
        ...
        (table.getModel().getColumnCount() == 4) && col == headerCategoryColumn
      ) {
        comp.setText(VueResources.local("advancedSearch.category"));
      } else if (
        (table.getModel().getColumnCount() == 4) && col == headerValueColumn
      ) {
        comp.setText(VueResources.local("advancedSearch.keywords"));
      }
  ...
  public java.awt.Component createRendererComponent(
    ...
    if (col == (valueColumn)) {
      ...
      buttonLabel.setIcon(
        tufts.vue.VueResources.getIcon("metadata.editor.delete.up")
      );
  ...
  class OptionsPanel extends JPanel {
    ...
    public void switchChoices(int i, Object[] choices) {
      ...
      for (int j = 0; j < choices.length; j++) box.addItem(choices[j]);
  ...
  class PopupListener extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent e) {
      //Bug #: 1302
      if (
        /*e.getClickCount()==2 &&*/e.getX() > (searchResultTbl.getWidth() - 40)
      ) {
        runSavedSearch();
      }
      showPopup(e);
  ...
  private static final String ResetLabel =
    VueResources.local("button.reset.label") + " (";
  private static final String RedoLabel =
    VueResources.local("action.redo") + " (";

  public void updateEnabled() {
    if (
      loadedMap != null && loadedMap.hasClientData(SearchAction.RevertableState)
    ) {
      int hideCount = (Integer) loadedMap.getClientData(
        SearchAction.RevertableState
      );
      ...
      GUI.invokeAfterAWT(
        new Runnable() {
          public void run() { // delay hiding it a tad
