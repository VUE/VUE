...
public class MapViewer
  extends TimedASComponent //javax.swing.JComponent
  implements
    VueConstants,
    FocusListener,
    LWComponent.Listener,
    LWSelection.Listener//, VueToolSelectionListener
    //, DragGestureListener
    //, DragSourceListener
    ,
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(MapViewer.class);

  private static final int RolloverAutoZoomDelay = VueResources.getInt(
    "mapViewer.rolloverAutoZoomDelay"
  );
  ...
  public static final class Event {
    ...
    public String toString() {
      ...
      if (id == DISPLAYED) name = "DISPLAYED"; else if (id == HIDDEN) name =
        ...
        id == ZOOM
      ) name = " ZOOM    "; else if (id == FOCUSED) name =
        " FOCUSED "; else if (id == VIEWS_CHANGED) name = "VIEWS_CHANGED";
      return String.format(
        "MapViewer$Event@%06x[%s %s]",
        hashCode(),
        name,
        viewer.getDiagName()
      );
  ...
  private static final EventHandler<Event> ViewerEventSource =
    EventHandler.getHandler(MapViewer.Event.class);
  ...
  protected final LWGroup draggedSelectionGroup = LWGroup.createTemporary(
    VUE.ModelSelection
  );
  ...
  private final VueTool HandTool = VueTool.getInstance(
    tufts.vue.HandTool.class
  );
  private final VueTool LinkTool = VueTool.getInstance(
    tufts.vue.LinkTool.class
  );
  ...
  private final VueTool NodeModeTool = VueTool.getInstance(
    tufts.vue.NodeTool.NodeModeTool.class
  );
  private final VueTool RichTextTool = VueTool.getInstance(
    tufts.vue.RichTextTool.class
  );
  private final VueTool BrowseTool = VueTool.getInstance(
    tufts.vue.SelectionTool.Browse.class
  );
  private final VueTool ToolPresentation = VueTool.getInstance(
    tufts.vue.PresentationTool.class
  );
  ...
  private static final BooleanPreference oneClickNodePref =
    BooleanPreference.create(
  ...
  public MapViewer(LWMap map, String instanceName) {
    ...
    if (map != null) {
      ...
      //-------------------------------------------------------
      if (!AutoZoomToMapOnLoad && map.getUserZoom() != 1.0) setZoomFactor(
        getMap().getUserZoom(),
        false,
        null,
        false
  ...
  private boolean isAutoZoomEnabled() {
    if (AutoZoomEnabled) {
      ...
      ) return AutoZoomEnabledInPresentations; else return true;
    } else return false;
  }
  ...
  public void addNotify() {
    ...
    if (inScrollPane) {
      if (
        getParent() instanceof MapViewport == false
      ) throw new IllegalStateException(
        "MapViewer will only work in ScrollPane if using MapViewport"
      );
      ...
      GUI.MouseWheelRelay.addScrollPaneIntercept(
  ...
  private void activateTool(VueTool tool, boolean temporary) {
    ...
    if (DEBUG.FOCUS && VUE.getActiveViewer() == this) out(
      "activateTool: " + tool.getID()
    );
    ...
    ) activeTool = subTool; else activeTool = tool;
    ...
    ) repaint(); else if ( // in case we change tool via kbd shortcut in the middle of a drag
      (oldTool != null && oldTool.hasDecorations()) || tool.hasDecorations()
    ) repaint();
  ...
  private final Runnable ViewChangeTracker = new Runnable() {
    public void run() {
      synchronized (MapViewer.this) {
        if (DEBUG.PRESENT) debug(
          "ViewChangeTracker: countDown -" + mViewChangeCount
        );
        if (--mViewChangeCount <= 0) {
          if (DEBUG.PRESENT) debug(
            "ViewChangeTracker: countdown  " +
            mViewChangeCount +
            " <= 0, firing..."
          );
          attemptRecording();
    ...
    private void attemptRecording() {
      ...
      try {
        ...
        Log.error(
          MapViewer.this + "; ViewChangeTracker, count=" + mViewChangeCount,
          t
        );
      } finally {
        // should already be at 0, but just in case
        if (DEBUG.SCROLL) debug(
          "view change count reset to 0 (recording test completed)"
        );
        mViewChangeCount = 0;
      }
    }
  ...
  private boolean viewIsChanging() {
    ...
    if (DEBUG.PRESENT || DEBUG.SCROLL) out(
      "anim=" +
      isAnimating +
      ", restoring=" +
      mViewRestoring +
  ...
  private boolean viewIsTrackable() {
    if (activeTool == ToolPresentation) return false; else if (
      inScrollPane && (mViewport.getWidth() <= 0 || mViewport.getHeight() <= 0)
  ...
  protected void fireViewerEvent(int id, String cause) {
    if (VUE.getActiveViewer() == this); // always raise the event if we're the active viewer
    else if (id == Event.HIDDEN); // always raise hidden, no matter who's raising it
    else if (id == Event.ZOOM && VUE.multipleMapsVisible()); // everyone raises ZOOM if multiple maps open (so each map tabbed pane can update it's title)
    else return; // don't raise the event
  ...
  private void adjustCanvasSize(
    ...
    final boolean trimNorthWest,
    final boolean trimSouthEast
  ) {
  ...
  private void doAdjustCanvasSize(
    boolean expand,
    boolean trimNorthWest,
    boolean trimSouthEast
  ) {
    if (mFocal.hasContent()) mViewport.adjustSize(
      expand,
      trimNorthWest,
      trimSouthEast
    ); else mViewport.adjustSize(false, true, true);
  }
  ...
  private void takeZoomFactor(final double zoom) {
    ...
    if (mFocal == mMap && mMap != null) mMap.setUserZoom(mZoomFactor);
  ...
  protected void setZoomFactor(
    ...
    final boolean centerOnAnchor
  ) { // recenter the map by moving the focusAnchor to the center of the screen (?)
    ...
    if (!resetScrollBars) {
      if (inScrollPane) {
        if (!centerOnAnchor) {
          ...
          screenPositionOfMapAnchor =
            new Point2D.Double(
              canvasPosition.getX() + canvasOffset.x,
    ...
    if (inScrollPane) {
      ...
      if (focusAnchor != null && offset != null) setMapOriginOffset(
        offset.x,
        offset.y
      );
    }
  ...
  protected void setMapOriginOffsetImpl(
    float panelX,
    float panelY,
    boolean update
  ) {
    ...
    //if (mMap == mFocal) {
    if (true) {
      if (mMap == mFocal && VUE.getActiveViewer() == this) mMap.setUserOrigin(
        panelX,
        panelY
      );
      if (!inScrollPane && update) {
        repaint();
        if (mMap == mFocal) fireViewerEvent(Event.PAN, "setMapOriginOffset");
      }
    }
  }
  ...
  Point2D mapToScreenPoint2D(Point2D p) {
    return new Point2D.Double(
      mapToScreenX2D(p.getX()),
      mapToScreenY2D(p.getY())
    );
  }

  int mapToScreenDim(double dim) {
    if (dim > 0) return (int) (0.5 +
      (dim * mZoomFactor)); else return (int) (0.5 + (-dim * mZoomFactor));
  }
  ...
  Rectangle mapToScreenRect(Rectangle2D mapRect) {
    ...
    screenRect.x =
      (int) Math.floor(mapRect.getX() * mZoomFactor - getOriginX());
    screenRect.y =
      (int) Math.floor(mapRect.getY() * mZoomFactor - getOriginY());
  ...
  Rectangle2D.Float mapToScreenRect2D(Rectangle2D mapRect) {
    if (
      mapRect.getWidth() < 0 || mapRect.getHeight() < 0
    ) throw new IllegalArgumentException("mapDim<0");
  ...
  Rectangle2D screenToMapRect(Rectangle screenRect) {
    if (
      ...
      "screenDim<0 " + screenRect + " in " + this
    );
    Rectangle2D mapRect = new Rectangle2D.Float();
    mapRect.setRect(
      screenToMapX(screenRect.x),
  ...
  private Point2D viewportToCanvasPoint(double x, double y) {
    if (inScrollPane) return new Point2D.Double(
      x - getX(),
      y - getY()
  ...
  private static final float SelectionStrokeMargin = SelectionStrokeWidth / 2;
  ...
  public void reshape(int x, int y, int w, int h) {
    final boolean ignore =
      getX() == x && getY() == y && getWidth() == w && getHeight() == h;
    ...
    if (
      DEBUG.SCROLL || DEBUG.PAINT || DEBUG.EVENTS || DEBUG.FOCUS || DEBUG.VIEWER
    ) {
      out(
        "reshape",
        w + " x " + h + " " + x + "," + y + (ignore ? " (IGNORING)" : "")
      );
    ...
    if (
      DEBUG.VIEWER || (ignore && hasActiveTextEdit())
  ...
  protected void fitToFocal(boolean animate) {
    ...
    if (
      (getVisibleWidth() == 0 || getVisibleHeight() == 0) ||
      (getVisibleWidth() == 1 && getVisibleHeight() == 1)
    ) {
      if (DEBUG.PRESENT) out(
        "requesting delayed autoZoom; visSize=" + getVisibleSize()
      );
  ...
  private void doFitToFocal(boolean animate) {
    ...
    if (DEBUG.PRESENT || DEBUG.VIEWER) out(
      "fitToFocal",
      mFocal + " at " + zoomBounds
    );

    if (
      animate &&
      mLastFocal != null &&
      mLastFocal instanceof LWMap == false &&
      canAnimate()
    ) {
    ...
    if (ScrollBarHiding && mFocal instanceof LWMap) {
      ...
      VUE.invokeAfterAWT(
        new Runnable() {
          public void run() {
            ZoomTool.setZoomFit(MapViewer.this);
          }
        }
      );
    } else {
      ...
      if (mFocal != mMap) {
        mFocalMapLocation.setLocation(
          mFocal.getMapXPrecise(),
          mFocal.getMapYPrecise()
        );
  ...
  public void ensureMapVisible() {
    if (mMap == mFocal && mMap != null && mMap.hasContent()) {
      ...
      try {
        ...
        currentlyVisibleObjects =
          computeSelection(getVisibleMapBounds()).size();
      } catch (Throwable t) {
        Log.info(t);
      }
  ...
  private boolean isDisplayed() {
    ...
    } else return getWidth() > 0 && getHeight() > 0;
  }
  ...
  public LWComponent getDropFocal() {
    if (getFocal() instanceof LWMap) {
      if (((LWMap) getFocal()).getActiveLayer() == null) {
        String msg =
          "getDropFocal: NO ACTIVE LAYER FOR FOCAL: " +
          getFocal() +
          " in " +
          this;
        if (DEBUG.Enabled) Util.printStackTrace(msg); else Log.warn(msg);
        return getFocal();
      }
      return ((LWMap) getFocal()).getActiveLayer();
    } else return getFocal();
  }
  ...
  public void switchFocal(LWComponent newFocal, boolean animate) {
    if (DEBUG.PRESENT || DEBUG.VIEWER || DEBUG.WORK) out(
      "switchFocal",
      newFocal
    );
    if (
      activeTool != null && activeTool.handleFocalSwitch(this, mFocal, newFocal)
    ) {
      if (DEBUG.PRESENT || DEBUG.VIEWER || DEBUG.WORK) out(
        "switchFocal",
        "activeTool handled: " + activeTool
      ); // the active tool has handled the focal loading and any desired auto-fit
    } else {
      if (DEBUG.PRESENT || DEBUG.VIEWER || DEBUG.WORK) out(
        "switchFocal",
        "vanilla load " + newFocal
      );
  ...
  public void loadFocal(
    ...
    if (mFocal == focal) {
      if (fitToFocal) fitToFocal(animate);
      return;
    }

    final boolean loadingWasSet = mFocalLoading;

    if (!loadingWasSet) setLoading(true);
    ...
    if (mFocal != null) {
      ...
      //autoZoom = true;
    } else if (!(focal instanceof LWMap)); //autoZoom = true;
    ...
    if (mFocal != null) {
      mMap = mFocal.getMap();
      if (mMap == null) tufts.Util.printStackTrace(
        "no map in focal! " + mFocal
      );
    ...
    if (mMap != null) {
      ...
      } else mUndoManager = mMap.getUndoManager();
    }
    ...
    if (
      VueSelection != null && VUE.getActiveViewer() == this
    ) VueSelection.setSelectionSourceFocal(focal);

    if (focal == null) repaint();

    if (!loadingWasSet) setLoading(false);
      ...
      VUE.invokeAfterAWT(
        new Runnable() {
          public void run() {
            //Log.debug("map=" + map);
            if (
              map.getClientData(Boolean.class, MAP_REQUESTED_CACHING) == false
            ) {
        ...
        new Runnable() {
          public void run() {
            ...
            for (LWComponent c : toCache) {
              ...
              if (Images.lowMemoryConditions()) break;
            }
          }
        }
      );
  ...
  protected boolean popFocal(boolean toTopLevel, boolean animate) {
    if (DEBUG.PRESENT || DEBUG.WORK) out(
      "popFocal",
      "up from " + mFocal + "; toTop=" + toTopLevel
    );
    if (mFocal == null) return false;
    ...
    if (parent instanceof LWPathway) switchFocal(
      parent.getMap(),
      animate
    ); else if (
      mFocal instanceof LWSlide && mFocal.isPathwayOwned()
    ) switchFocal(parent.getMap(), animate); else if (parent != null) {
      if (parent instanceof LWMap.Layer) parent = parent.getMap();
  ...
  private void RR(Rectangle r) {
    if (OPTIMIZED_REPAINT) super.repaint(
      0,
      r.x,
      r.y,
      r.width,
  ...
  private void repaintMapRegionAdjusted(Rectangle2D mapRect) {
    if (OPTIMIZED_REPAINT) repaint(
      growForSelection(mapToScreenRect(mapRect))
    ); else repaint();
  }
  ...
  private Rectangle growForSelection(Rectangle r) {
    return growRect(r, SelectionHandleSize);
  }

  private Rectangle growForSelection(Rectangle r, int pad) {
    return growRect(r, SelectionHandleSize + pad);
  }
  ...
  public void selectionChanged(LWSelection s) {
    ...
    if (s.isEmpty() && inScrollPane) {
      InputMap im = mapScrollPane.getInputMap(
  ...
  public void repaintSelection() {
    if (paintedSelectionBounds != null) RR(paintedSelectionBounds);
    if (VueSelection != null) {
      Rectangle2D newBounds = VueSelection.getBounds();
      if (newBounds != null) repaintMapRegionAdjusted(newBounds);
    }
  }
  ...
  private boolean isBoundsEvent(Object k) {
    ...
    return k != LWKey.HierarchyChanging && k != LWKey.ChildrenRemoved;
    /*
        return k == LWKey.Size
            || k == LWKey.Location
            || k == LWKey.Frame
  ...
  public void activeChanged(ActiveEvent e, LWPathway.Entry entry) {
    ...
    if (
      activeTool != ToolPresentation &&
      VUE.getActiveViewer() == this &&
      mFocal instanceof LWMap == false &&
      entry != null
    ) {
  ...
  private final class CursoredStack<T> extends ArrayList<T> {
    ...
    public T popBackward() {
      if (atHead()) return null; else return get(--cursor);
    }

    public T popForward() {
      if (atTail()) return null; else return get(++cursor);
    }
    ...
    public void push(T item) {
      if (!atTail()) flushForward();
      super.add(item);
      cursor = size() - 1;
      if (DEBUG.Enabled) debug(
        TERM_PURPLE +
        String.format("RECORDED %2d: ", cursor) +
        item +
        TERM_CLEAR
      );
    }
  ...
  private boolean isCloseMapRegion(
    ...
    if (zoom1 == zoom2) {
      ...
      return Math.abs(dpx) < pixMax && Math.abs(dpy) < pixMax;
    }

    if (DEBUG.PRESENT) out(
      String.format(
        "deltaPixels: (%.3f|%.3f) %.1f,%.1f %.1fx%.1f",
        zoom1,
        zoom2,
        dpx,
        dpy,
  ...
  private class View {
    ...
    final boolean isSameAsCurrentView() {
      if (
        ViewsRespectScrollBars
      ) return isSameAsCurrentViewImpl(); else return isCloseMapRegion(
        mZoomFactor,
        zoom,
        mapRegion,
        getVisibleMapBounds(),
    ...
    boolean isSameAsCurrentViewImpl() {
      if (zoom != mZoomFactor || focal != mFocal) return false;
  ...
  public void viewBackward() {
    if (!mViews.atHead()) restoreView(mViews.popBackward());
  }

  public void viewForward() {
    if (!mViews.atTail()) restoreView(mViews.popForward());
  }
  ...
  private View produceView() {
    ...
    if (inScrollPane) return new ScrollView(); else return new View();
  }

  private void restoreView(final View view) {
    if (DEBUG.Enabled) debug(
      TERM_YELLOW +
      String.format(" RESTORE %2d: ", mViews.cursor) +
      view +
      TERM_CLEAR
    );
    mViewRestoring = true;
    ...
    if (DEBUG.Enabled) debug(
      TERM_YELLOW +
      String.format("RESTORED %2d: ", mViews.cursor) +
      view +
      TERM_CLEAR
    );
    repaint();
  ...
  private void recordViewIfChanged() {
    if (DEBUG.Enabled && viewIsChanging()) throw new Error(
      "can't record a view while changing"
    ); // shouldn't happen
    ...
    if (DEBUG.PRESENT) debug(
      String.format("lastView %2d: ", mViews.cursor) + topOfStackView
    );

    if (topOfStackView.isSameAsCurrentView()) {
      if (DEBUG.PRESENT) debug("sameView XX: " + produceView()); // do nothing -- the view hasn't significantly changed
    } else {
  ...
  public void LWCChanged(LWCEvent e) {
    ...
    if (e.key == LWKey.RepaintRegion) {
      ...
      if (e.component == getFocal()) {
        ...
        if (e.component.isSelected()) {
          ...
          // bother in that case.
          final Rectangle selectionBounds = mapToScreenRect(
            e.component.getBounds()
          );
    ...
    if (e.key == LWKey.Deleting) {
      if (mRollover == e.getComponent()) clearRollover();
  ...
  private void setIndicated(LWComponent c, DropIndication di) {
    ...
    if (
      indication == c && (di == null || mDI == null || di.isSame(mDI))
    ) return;
    ...
    //c.setIndicated(true);
    if (OPTIMIZED_REPAINT) repaintMapRegion(
      indication.getBounds()
    ); else repaint();
  ...
  protected PickContext getPickContext(float x, float y, float w, float h) {
    final PickContext pc = initPickContext(
      new PickContext(mDC, new java.awt.geom.Rectangle2D.Float(x, y, w, h))
    );
  ...
  public List<LWComponent> pickNode(
    float mapX,
    float mapY,
    float width,
    float height
  ...
  private void runRolloverTask() {
    ...
    if (hit != null) setRollover(hit); // todo: try checking allowsRollover and triggering the clear & see if helps with the flashing cases
    else clearRollover();
  ...
  private boolean allowsZoomedRollover(LWComponent c) {
    ...
    ) return false; else if (mRollover != null && c.hasAncestor(mRollover)) {
  ...
  void setRollover(LWComponent c) {
    if (mRollover == c || !allowsZoomedRollover(c)) return;

    if (DEBUG.PICK) out("**SET ROLLOVER " + c);

    if (mRollover != null) clearRollover();
  ...
  private void setZoomedFocus(LWComponent c) {
    ...
    if (c == null) {
      ...
      if (zt != null) {
        if (mRollover != null) {
          if (DEBUG.Enabled) Log.warn(
            "fallback clear of zoomed-focus on " + mRollover
          );
          mRollover.setZoomedFocus(null);
        }
        mRollover = c;
  ...
  private AffineTransform computeZoomFocusTransform(LWComponent c) {
    ...
    testTX.translate(
      c.getMapX() + halfWidth * localScale,
  ...
  private double computeZoomFocusFactor(LWComponent c) {
    ...
    if (mapZoom <= 1.0) {
      ...
      if (c instanceof LWSlide) netZoom = normalZoom / 2; else netZoom =
        normalZoom * 2;
      if (netZoom > 4) {
        netZoom = normalZoom;
        if (netZoom > 6) netZoom = 8;
      }
    } else if (mapZoom > 2.5) {
      if (DEBUG.Enabled) out(
        "computeZoomFocusFactor: skipped -- overzoom at map zoom " + mapZoom
      );
      return 0;
    } else {
      if (c instanceof LWSlide) netZoom = 0.5; else netZoom = 2;
    }
    ...
    final double maxZoom = ZoomTool.computeZoomFit(
      visibleMapSize,
      0,
      mapBounds,
      null
    ...
    if (netZoom > maxZoom) {
      if (DEBUG.Enabled) out(
        "**SET ROLLOVER; maxZoom exceeded with default of " + netZoom
      );
      netZoom = maxZoom;
    }
  ...
  private static final Border ToolTipBorder = new CompoundBorder(
  ...
  void activateRolloverToolTip(
    ...
    if (pJComponent == null) throw new IllegalArgumentException(
      "JComponent is null"
    );
    ...
    if (sTipPopup != null) sTipPopup.hide();
    ...
    synchronized (sTipLock) {
      ...
      if (sTipComponent.getName() == null) sTipComponent.setName(
        "VUE-POPUP-COMPONENT"
      );
  ...
  private void lockMediumWeightPopup(Component c, boolean done) {
    ...
    if (done) {
      if (DEBUG.FOCUS) out(
        "skipping " + Util.out(c.getBounds()) + "\t" + GUI.name(c)
      );
    } else {
  ...
  public void clearTip() {
    synchronized (sTipLock) {
      sTipComponent = null;
      if (sTipPopup != null) {
        if (DEBUG.ROLLOVER && DEBUG.META) new Throwable("CLEARTIP")
          .printStackTrace();
        sTipPopup.hide();
        sTipPopup = null;
      }
  ...
  private static final Line2D Xaxis = new Line2D.Float(
    MinCoord,
    0,
    MaxCoord,
    0
  );
  private static final Line2D Yaxis = new Line2D.Float(
    0,
    MinCoord,
    0,
    MaxCoord
  ...
  protected boolean skipAllPainting() {
    return (
      VUE.inFullScreen() && instanceName != tufts.vue.gui.FullScreen.VIEWER_NAME
  ...
  public void repaint() { // heavy-duty debug
    if (!isDisplayed()) {
      if (DEBUG.Enabled && (DEBUG.VIEWER || DEBUG.EVENTS || DEBUG.IMAGE)) out(
        "not-displayed: skipping repaint"
      );
      return;
    }

    int frCount = 0;
    if (mFastPainting) frCount = mFastRequests.incrementAndGet();
    ...
    if (DEBUG.PAINT) {
      if (DEBUG.META) {
        ...
        String msg = "";
        if (mFastPainting) msg = "; fastPaintRequests +" + frCount;
        out(
          TERM_RED +
          "REPAINT ISSUED at " +
  ...
  private final java.util.concurrent.atomic.AtomicInteger mFastRequests =
  ...
  public void paint(Graphics g) {
    ...
    if (DEBUG.Enabled) {
      ...
      if (DEBUG.PAINT || DEBUG.SCROLL || DEBUG.PRESENT) pout(
        String.format(
          "PAINT =>[%d]%s rawClip=%s",
          mPaints,
    ...
    try {
      ...
      if (VueSelection != null) Log.error(
        "*paint* VueSelection.first: " + VueSelection.first()
      );
      Log.error("*paint* Graphics: " + g);
      Log.error(
        "*paint* Graphics transform: " + ((Graphics2D) g).getTransform()
      );
    }
    ...
    if (mPaints == 0) {
      if (inScrollPane) adjustCanvasSize(); // need for intial scroll-bar sizes if bigger than viewport on startup
  ...
  private void scheduleQualityPaintIfNeeded(
    final Graphics g,
    final int fastRequestsAtStartOfPaint
  ) {
    final int newFastPaintsWhilePainting =
      (mFastRequests.get() - fastRequestsAtStartOfPaint);

    if (mFastRequests.get() > 0) {
      if (DEBUG.PAINT) pout(
        "consuming " + mFastRequests.get() + " fast paint requests"
      );
      mFastRequests.set(0);
    }
    ...
    if (newFastPaintsWhilePainting > 0) {
      ...
      if (DEBUG.PAINT) pout(
        "fast-paint requests since started painting: " +
        newFastPaintsWhilePainting +
    ...
    if (DEBUG.PAINT) pout(
      "scheduling smooth repaint if it falls as paint count " + paintAtCount
    );
    if (mLastTask != null) {
        ...
        new TimerTask() {
          public void run() {
            ...
            if (mPaintsStarted != paintAtCount) {
              if (DEBUG.PAINT) pout(
                "skipping smooth paint @" +
                paintAtCount +
                "; already painting at count: " +
                mPaints
              );
              return;
            }
            ...
            if (DEBUG.PAINT) pout(
              "triggering smooth paint @" +
              paintAtCount +
              " (no fast requests or paints since scheduling)"
            );
            ...
            if (mFastPainting) {
              ...
              out(
                TERM_RED +
                "*** FAST PAINTING SET IN SLOW-PAINT REQUEST ***" +
                TERM_CLEAR
              );
            }
            repaint();
  ...
  public void setLoading(boolean loading) {
    ...
    if (mFocalLoading == loading) return;
  ...
  private static final String NewMapMessage = VueResources.getString(
    "vue.main.newmap",
    "NEW MAP"
  );
  ...
  protected String getEmptyMessage() {
    if (mFocalLoading) {
      ...
      if (
        mMap != null && mMap.isModified()
      ) return "Empty Map"; else return NewMapMessage;
    } else {
      if (mFocal != null) return (
        "Empty " + mFocal.getComponentTypeLabel()
      ); else return "Empty";
    }
  }
  ...
  protected DrawContext getDrawContext(Graphics2D g) {
    ...
    final DrawContext dc = new DrawContext(
      g,
      getZoomFactor(),
      -getOriginX(),
      -getOriginY(),
    ...
    if (isAnimating) {
      ...
      if (DEBUG.Enabled) {
        if (DEBUG_SKIP_ANTIALIAS != UNSET_BOOL) {
          ...
          if (DEBUG_SKIP_ANTIALIAS) dc.disableAntiAlias(true);
        }

        if (DEBUG_RENDER_QUALITY != UNSET_BOOL) dc.setImageQuality(
          dc.g,
          DEBUG_RENDER_QUALITY
        );
        if (DEBUG_FONT_METRICS != UNSET_BOOL) dc.setFontQuality(
          dc.g,
          DEBUG_FONT_METRICS
        );
  ...
  private boolean dragSelectorEnabled(MapMouseEvent mme) {
    if (
      mFocal instanceof LWPathway.MapSlide
    ) return false; else return activeTool.supportsDraggedSelector(mme);
  }
  ...
  public void paintComponent(final Graphics incomingGC) {
    ...
    if (
      ...
      if (indication instanceof MasterSlide) {
        ...
        indication instanceof LWSlide &&
        indication.isPathwayOwned() &&
    ...
    if (
      s == null ||
      s.isEmpty() ||
      !activeTool.supportsResizeControls() ||
      isAnimating
    ) {
      ...
      if (remoteFocal == null) {
        if (DEBUG.Enabled) out(
          "SKIPPING RESIZE CONTROL; null selection focal (claims not to be tied to this viewer)"
        );
      } else if (getFocal() != remoteFocal && remoteFocal.isMapVirtual()) {
        if (DEBUG.Enabled) out(
          "SKIPPING RESIZE CONTROL; SELECTION & VIEWER REPORT DIFFERENT FOCALS w/MapVirtual selection focal;" +
          "\n\tselection focal: " +
          remoteFocal +
          "\n\t   viewer focal: " +
          getFocal()
    ...
    if (
      hasActiveTextEdit()
    ) super.paintChildren(incomingGC); // This is a real Swing JComponent
  }
  ...
  private void drawIndication(DrawContext dc) {
    ...
    //             icolor = mDI.getColor();
    if (mDI == null) icolor =
      Util.alphaColor(COLOR_INDICATION, 0.5f); else icolor =
      Util.alphaColor(mDI.getColor(), 0.5f);
    ...
    if (indication instanceof LWLink) {
      ...
      if (indication.getStrokeWidth() * 2 > minStroke) dc.g.setStroke(
        new BasicStroke(indication.getStrokeWidth() * 2)
      ); else dc.g.setStroke(new BasicStroke((float) minStroke));
      ...
      final RectangularShape dataSplash = new java.awt.geom.Ellipse2D.Float(
        -wide / 2 + width / 2,
        -tall / 2 + height / 2,
        wide,
        tall
  ...
  protected Color getBackgroundFillColor(DrawContext dc) {
    ...
    if (dc.isPresenting() && !inScrollPane()) {
      ...
      if (entry != null) bgFill =
        entry.getFullScreenFillColor(dc); else bgFill = DefaultFillColor;
      ...
      if (mMap == null) {
        ...
        if (mFocal instanceof LWGroup) bgFill =
          mFocal.getFinalFillColor(dc); else bgFill = DefaultFocalFillColor;
      } else bgFill = mMap.getFillColor();
  ...
  protected void drawFocalImpl(DrawContext dc) {
    ...
    if (DEBUG.VIEWER && mRollover != null) mRollover.updateConnectedLinks(null);
    ...
    if (mRollover != null) {
      if (dc.skipDraw != null) Log.warn(
        this + "; TOSSING EXISTING SKIP-DRAW FOR ZOOMED FOCUS: " + dc.skipDraw
      );
  ...
  private static final AlphaComposite ZoomTransparency =
  ...
  private DrawContext drawZoomedFocus(LWComponent zoomed, DrawContext dc) {
    ...
    if (DEBUG.VIEWER) dc.g.setComposite(ZoomTransparency);
    ...
    if (
      zoomed.hasChildren() &&
      zoomed.isTransparent() &&
      !(zoomed instanceof LWSlide)
    ) {
      ...
      if (DEBUG.BOXES) dc.g.setColor(Color.red); else dc.g.setColor(fill);
    ...
    try {
      ...
      if (removedEdit) {
        try {
          ...
          // before repainting.
          //repaint();
          if (VUE.getActiveViewer() == this) requestFocus();
        } finally {
  ...
  void cancelLabelEdit() {
    if (activeTextEdit != null) remove(activeTextEdit);
    if (activeRichTextEdit != null) {
      ...
      VUE
        ...
        .getTextPropsPane()
        .getFontEditorPanel()
        .updateFormatControlsTB(activeRichTextEdit);
    }
  }
  ...
  void activateLabelEdit(LWComponent lwc) {
    if (activeTextEdit != null && activeTextEdit.getLWC() == lwc) return;
    if (
      activeRichTextEdit != null && activeRichTextEdit.getLWC() == lwc
    ) return;
    if (!lwc.supportsUserLabel() || !lwc.supportsProperty(LWKey.Label)) return;

    if (!activeTool.supportsEditLabel()) return;
    ...
    if (lwc instanceof LWText) {
      ...
      if (activeRichTextEdit.getText().length() < 1) activeRichTextEdit.setText(
        VueResources.getString("mapviewer.label.tooltip")
      );
    } else {
      activeTextEdit = lwc.getLabelBox();
      activeTextEdit.saveCurrentText();
      if (activeTextEdit.getText().length() < 1) activeTextEdit.setText(
        VueResources.getString("mapviewer.label.tooltip")
      );
    }

    Point2D.Float point = null;
    if (lwc instanceof LWText) point =
      activeRichTextEdit.getBoxPoint(); else point =
      activeTextEdit.getBoxPoint();
    ...
    if (DEBUG.LAYOUT) System.out.println(
      activeTextEdit + " back from addNotify"
    );
    if (lwc instanceof LWText) {
    ...
    if (DEBUG.LAYOUT) System.out.println(
      activeTextEdit + " back from requestFocus"
    );
  }
  ...
  private void drawSelectorBox(DrawContext dc, Rectangle r) {
    ...
    if (activeTool.supportsXORSelectorDrawing()) dc.g.setXORMode(
      COLOR_SELECTION_DRAG
    ); // using XOR may also be working around below clip-edge bug
    else dc.g.setColor(COLOR_SELECTION_DRAG);
  ...
  private static final AlphaComposite HiddenTransparency =
  ...
  protected void drawSelection(DrawContext dc, final LWSelection selection) {
    if (
      selection.only() == mFocal ||
      selection.first() instanceof MasterSlide ||
      selection.first() instanceof LWMap.Layer
    ) {
    ...
    if (selection.size() > 100) {
    ...
    } else for (LWComponent c : selection) {
      //             if (c instanceof LWSlide && !c.isMoveable()) {
      ...
      if (mFocalParent != null) {
        ...
        if (!c.hasAncestor(mFocalParent)) {
          ...
          if (c == mFocal) {
            ...
            if (DEBUG.Enabled) out(
              c + " in selection doesn't have ancestor " + mFocalParent
            );
          }
      ...
      if (c.isDrawn()) atLeastOneVisible = true;
      ...
      if (true || sDragUnderway) {
        if (c instanceof LWMap && !DEBUG.CONTAINMENT) continue;

        if (c instanceof LWLink && ((LWLink) c).isCurrentlyPruned()); // skip
        else drawSelectionGhost(dc, c);

        // Hack for groups in groups: show the parent group:
        final LWComponent parent = c.getParent();

        if (
          parent instanceof LWGroup &&
          !parent.isSelected() &&
          (parentGroups == null || !parentGroups.contains(parent))
        ) {
    ...
    if (!drawSelectorBoxInThisViewer || !atLeastOneVisible) {
      resizeControl.active = false;
      if (!drawSelectorBoxInThisViewer) out(
        "selection contents not for this viewer"
      );
      return;
    }
    ...
    if (DEBUG.SELECTION) out(
      "paintedSelectionBounds " + paintedSelectionBounds
    );
    ...
    if (
      !TEST &&
      selection.size() == 1 &&
      (!only.isMoveable() || !only.supportsUserResize())
    ) {
      resizeControl.active = false;
      ...
      if (
        !DEBUG.CONTAINMENT &&
        /* !ResizeControl.LOCAL_RESIZE && */selection.size() > 1 &&
        mFocal instanceof LWSlide
      ) {
        ...
        final boolean deepSelection =
          ...
          (selection.allHaveSameParentOfType(LWSlide.class) &&
            first.getParent().isMoveable());
    ...
    if (selection.size() > 50) {
      // do nothing
    } else for (LWSelection.ControlListener cl : selection.getControlListeners()) {
      ...
      for (int i = points.length - 1; i >= 0; i--) {
        ...
        if (
          false && sDragUnderway
        ) fillColor = Color.red; // hilight the dragging control // don't do for ALL controls: just the active one...
        else fillColor = ctrl.getColor();
        if (fillColor != null) {
          dc.g.setColor(fillColor);
          dc.g.fill(shape);
        }
        if (fillColor == COLOR_SELECTION) dc.g.setColor(Color.white); // ensure border contrast
        else dc.g.setColor(COLOR_SELECTION);
        dc.g.draw(shape);
    ...
    if (
      DEBUG.VIEWER || DEBUG.LAYOUT || DEBUG.CONTAINMENT
    ) resizeControl.drawDebug(dc);
    /*
  ...
  static final int SelectionHandleSize = VueResources.getInt(
    "mapViewer.selection.handleSize"
  ); // fill size
  static final int CHS = VueResources.getInt(
    "mapViewer.selection.componentHandleSize"
  ); // fill size
  ...
  private void drawSelectionHandleCentered(
    Graphics2D g,
    float x,
    float y,
    Color fillColor,
  ...
  private void drawSelectionHandle(
    ...
    if (DEBUG.BOXES) {
      ...
      if (index < 0) g.drawString(
        "cp" + -(index + 1),
        x,
        y - 2
      ); else g.drawString("sp" + index, x, y - 2);
    }
  ...
  protected void selectionToggle(LWComponent c) {
    ...
    if (activeTool == BrowseTool && VueSelection.only() == c) {
      ...
      if (c instanceof LWLink) {
        ...
        if (link.getHead() != null) VueSelection.add(link.getHead());
        if (link.getTail() != null) VueSelection.add(link.getTail());
      }
    } else {
      if (c.isSelected()) selectionRemove(c); else selectionAdd(c);
    }
  }
  ...
  private JPopupMenu buildMultiSelectionPopup() {
    ...
    infoCheckBox.setLabel(
      VueResources.getString("mapViewer.componentMenu.selInfo.label")
    );
    infoCheckBox.setSelected(VUE.getInfoDock().isShowing());
    sMultiPopup.add(infoCheckBox);

    WindowDisplayAction formatAction = new WindowDisplayAction(
      VUE.getFormatDock()
    );
    JCheckBoxMenuItem formatBox = new JCheckBoxMenuItem(formatAction);
    formatBox.setSelected(VUE.getFormatDock().isShowing());
    formatAction.setTitle(
      VueResources.getString("mapViewer.componentMenu.format.label")
    );
    ...
    sMultiPopup.add(
      GUI.buildMenu(
        VueResources.getString("analyzeaction.autotag"),
    ...
    sMultiPopup.add(
      GUI.buildMenu(
        VueResources.getString("menu.image"),
        Actions.IMAGE_MENU_ACTIONS
      )
    );
    sMultiPopup.add(
      GUI.buildMenu(
        VueResources.getString("menu.align"),
        Actions.ALIGN_MENU_ACTIONS
      )
    );
    sMultiPopup.add(
      GUI.buildMenu(
        VueResources.getString("menu.arrange"),
        Actions.ARRANGE_MENU_ACTIONS
      )
    );
    sMultiPopup.add(
      GUI.buildMenu(
        VueResources.getString("menu.layout"),
        LayoutAction.LAYOUT_ACTIONS
      )
    );

    sMultiPopup.addSeparator();
    Component multiAddPathway = sMultiPopup.add(Actions.AddPathwayItem);
    boolean enablePathway =
      getMap().getPathwayList().getActivePathway() != null &&
  ...
  private static final WindowDisplayAction infoAction = new WindowDisplayAction(
    VUE.getInfoDock()
  );
  private static final JCheckBoxMenuItem infoCheckBox = new JCheckBoxMenuItem(
    infoAction
  );
  ...
  private void buildSingleSelectionNodePopup(LWComponent c) {
    ...
    infoCheckBox.setLabel(
      VueResources.getString("mapViewer.componentMenu.nodeInfo.label")
    );
    ...
    sRemoveResourceKeepImageItem =
      sSinglePopup.add(Actions.RemoveResourceKeepImageAction);
    ...
    sSinglePopup.add(
      GUI.buildMenu(
        VueResources.getString("analyzeaction.performmapbased"),
        AnalyzerAction.RESOURCES_ACTIONS
      )
    );
    sSinglePopup.add(
      GUI.buildMenu(
        VueResources.getString("analyzeaction.semanticmap"),
    ...
    if (r == null) {
      ...
      sAddFileItem.setLabel(
        VueResources.getString("mapViewer.componentMenu.addFile.label")
      );
      sAddURLItem.setLabel(
        VueResources.getString("mapViewer.componentMenu.addURL.label")
      );
    } else {
      sRemoveResourceItem.setEnabled(true);
      sAddFileItem.setLabel(
        VueResources.getString("mapViewer.componentMenu.replaceFile.label")
      );
      sAddURLItem.setLabel(
        VueResources.getString("mapViewer.componentMenu.replaceURL.label")
      );
      ...
      if (c.hasChildren()) {
        ...
        if (!enabled) sRemoveResourceKeepImageItem.setEnabled(false);
      } else sRemoveResourceKeepImageItem.setEnabled(false);
    }
  }
  ...
  private void buildSingleSelectionLinkPopup(LWComponent c) {
    ...
    infoCheckBox.setLabel(
      VueResources.getString("mapViewer.componentMenu.linkInfo.label")
    );
    ...
    sSinglePopup.add(
      GUI.buildMenu(
        VueResources.getString("menu.link"),
        Actions.LINK_MENU_ACTIONS
      )
    );
    sSinglePopup.add(
      GUI.buildMenu(
  ...
  private void buildSingleSelectionSlideEditingPopup() {
    ...
    infoCheckBox.setLabel(
      VueResources.getString("mapViewer.componentMenu.slideInfo.label")
    );
    if (VUE.getInfoDock().isShowing()) infoCheckBox.setSelected(true);
    sSinglePopup.add(infoCheckBox);
    sSinglePopup.add(formatBox);
    if (!slide.getMasterSlide().equals(slide)) {
  ...
  private void buildSingleSelectionPortalPopup() {
    infoCheckBox.setLabel(
      VueResources.getString("mapViewer.componentMenu.portalInfo.label")
    );
    if (VUE.getInfoDock().isShowing()) infoCheckBox.setSelected(true);
    sSinglePopup.add(infoCheckBox);
    sSinglePopup.add(Actions.LaunchPresentation);
  ...
  private void buildSingleSelectionGroupPopup() {
    infoCheckBox.setLabel(
      VueResources.getString("mapViewer.componentMenu.groupInfo.label")
    );
    infoCheckBox.setSelected(VUE.getInfoDock().isShowing());
    sSinglePopup.add(infoCheckBox);
  ...
  private void buildSingleSelectionImagePopup() {
    infoCheckBox.setLabel(
      VueResources.getString("mapViewer.componentMenu.imageInfo.label")
    );
    if (VUE.getInfoDock().isShowing()) infoCheckBox.setSelected(true);
    sSinglePopup.add(infoCheckBox);
    sSinglePopup.addSeparator();
  ...
  private JMenu syncMenu = new JMenu(
    VueResources.getString("mapViewer.componentMenu.syncMenu.label")
  );
  private JMenu arrangeMenu = new JMenu(
    VueResources.getString("mapViewer.componentMenu.arrangeMenu.label")
  );

  private JPopupMenu getSingleSelectionPopup(LWComponent c) {
    if (c == null) c = VueSelection.first(); // should be only thing in selection
    ...
    if (c instanceof LWNode) {
      if (
        c instanceof LWPortal
      ) buildSingleSelectionPortalPopup(); else buildSingleSelectionNodePopup(
        c
      );
      ...
      if (
        VUE.getActiveViewer().getFocal().equals(c)
      ) buildSingleSelectionSlideEditingPopup(); else buildSingleSelectionSlideIconPopup();
  ...
  private JPopupMenu getMapPopup() {
    if (sMapPopup == null) {
      sMapPopup =
        new JPopupMenu(
          VueResources.getString("mapViewer.componentMenu.mapMenu")
        );

      sMapPopup.add(
        new VueAction(VueResources.getString("mapViewer.mapMenu.info.label")) {
  ...
  public void keyPressed(KeyEvent e) {
    ...
    switch (keyCode) {
      ...
      case KeyEvent.VK_BACK_SPACE:
        ...
        if (!e.isConsumed() && Actions.Delete.enabled()) Actions.Delete.fire(
          e
        ); else handled = false;
        break;
      case KeyEvent.VK_ENTER:
        if (
          !Actions.Rename.isUserEnabled() && // messy: encoding that we know Rename uses ENTER here...
          !(mFocal instanceof LWMap) &&
          !(this instanceof tufts.vue.ui.SlideViewer)
        ) { // total SlideViewer hack...
      ...
      case KeyEvent.VK_ESCAPE: // general abort
        ...
        if (draggedSelectorBox != null) {
          ...
          if (
            activeTool instanceof PresentationTool
          ) activateTool(VueTool.getInstance(SelectionTool.class)); // todo: need to do this in a more centralized location...
        } else handled = false;
        break;
      case KeyEvent.VK_BACK_SLASH:
        ...
        if (GUI.anyModifierKeysDown(e) || VUE.inFullScreen()) {
          ...
          handled = false;
        } else //VUE.toggleFullScreen(false, true);
        Actions.ToggleFullScreen.fire(e);
        break;
      // fallthru:
    ...
    if (!handled) {
      ...
      if (handled) {
        if (DEBUG.KEYS) out(
          e.paramString() + "; key handled by current tool: " + activeTool
        );
      }
    }
    ...
    if (
      isPathwayEntryMode() &&
      VueSelection != null &&
      (VueSelection.isEmpty() || VueSelection.first() == mFocal)
    ) {
        ...
        case KeyEvent.VK_RIGHT:
          if (
            ...
            if (e.isControlDown()) {
              ...
              if (selected.getParent() instanceof LWNode) {
                ...
                while (depth <= maxDepth && !done) {
                  ...
                  Toolbox.findChildrenToDepth(
                    ...
                    true,
                    true,
                    new Hashtable<LWComponent, Integer>()
                  );
                  ...
                  for (LWComponent comp : deepSelection) {
                    if (
                      ...
                      Point2D p1 = new Point2D.Double(
                        selected.getMapCenterX(),
                        selected.getMapCenterY() * -1
                      );
                      Point2D p2 = new Point2D.Double(
                        comp.getMapCenterX(),
                        comp.getMapCenterY() * -1
                      );
                  ...
                  if (
                    Toolbox.angleDifference(directionAlpha, nearestAlpha) <=
                    Math.PI / 4
                  ) {
              ...
              !e.isAltDown() && !e.isMetaDown() && !activeTool.supportsDrag(e)
            ) {
              final int dir;

              if (keyCode == VK_UP || keyCode == VK_LEFT) dir = -1; else dir =
                1;
              ...
              if (selected.getParent() instanceof LWNode) { // todo: more abstract test for ordered children
                ...
                if (
                  newIndex >= 0 && newIndex < selected.getParent().numChildren()
              ...
              if (e.isShiftDown()) {
                if (keyCode == KeyEvent.VK_UP) Actions.BigNudgeUp.fire(
                  e
                ); else if (
                  keyCode == KeyEvent.VK_DOWN
                ) Actions.BigNudgeDown.fire(e); else if (
                  keyCode == KeyEvent.VK_LEFT
                ) Actions.BigNudgeLeft.fire(e); else if (
                  keyCode == KeyEvent.VK_RIGHT
                ) Actions.BigNudgeRight.fire(e);
              } else {
                if (keyCode == KeyEvent.VK_UP) Actions.NudgeUp.fire(
                  e
                ); else if (keyCode == KeyEvent.VK_DOWN) Actions.NudgeDown.fire(
                  e
                ); else if (keyCode == KeyEvent.VK_LEFT) Actions.NudgeLeft.fire(
                  e
                ); else if (
                  keyCode == KeyEvent.VK_RIGHT
                ) Actions.NudgeRight.fire(e);
              }
            }
          } else {
            handled = false;
    ...
    if (activeTool.permitsToolChange()) {
      ...
      if (
        keyChar != 0 &&
        GUI.noModifierKeysDown(e) &&
        (!sDragUnderway || isDraggingSelectorBox)
      ) {
        for (VueTool tool : VueTool.getTools()) {
          if (
            tool.getShortcutKey() == keyChar ||
            tool.getBackwardCompatShortcutKey() == keyChar
          ) {
      ...
      if (
        ...
        if (tempTool != null) {
          ...
          } else activateTool(tempTool, true);
        }
      }
    }
    ...
    if (
      ...
      boolean did = true;
      if (!DEBUG.Enabled) did = false; else if (c == 'A') {
        DEBUG_SKIP_ANTIALIAS = !DEBUG_SKIP_ANTIALIAS;
        if (DEBUG_SKIP_ANTIALIAS) AA_ON =
          RenderingHints.VALUE_ANTIALIAS_OFF; else AA_ON =
          RenderingHints.VALUE_ANTIALIAS_ON;
      } else if (c == 'B') {
        DEBUG.BOXES = !DEBUG.BOXES;
      } else if (c == 'C') {
        DEBUG.CONTAINMENT = !DEBUG.CONTAINMENT;
      } else if (c == 'D') {
        DEBUG.DOCK = !DEBUG.DOCK;
      } else if (c == 'E') {
        DEBUG.EVENTS = !DEBUG.EVENTS;
      } else if (c == 'F') {
        DEBUG.FOCUS = !DEBUG.FOCUS;
      }
      //else if (c == 'F') { DEBUG_FINDPARENT_OFF = !DEBUG_FINDPARENT_OFF; }
        ...
        DEBUG.MOUSE = !DEBUG.MOUSE;
      } else if (c == 'm') {
        DEBUG.MARGINS = !DEBUG.MARGINS;
      }
      ...
      //else if (c == 'r') { DEBUG.ROLLOVER = !DEBUG.ROLLOVER; } // 'r' now taken by tool rapid-proto
        ...
        DEBUG.PRESENT = !DEBUG.PRESENT;
      } else if (c == '&') {
        DEBUG_FONT_METRICS = !DEBUG_FONT_METRICS;
      } else if (c == '^') {
        DEBUG.DR = !DEBUG.DR;
      } else if (c == '+') {
        ...
        DEBUG.PATHWAY = !DEBUG.PATHWAY;
      } else if (c == '}') {
        DEBUG.PARENTING = !DEBUG.PARENTING;
      } else if (c == '>') {
        DEBUG.DND = !DEBUG.DND;
      } else if (c == '<') {
        DEBUG.PICK = !DEBUG.PICK;
      } else if (c == '=') {
        DEBUG.THREAD = !DEBUG.THREAD;
      } else if (c == ';') {
        DEBUG.LINK = !DEBUG.LINK;
      } else if (c == '(') {
        DEBUG.setAllEnabled(true);
      } else if (c == ')') {
        DEBUG.setAllEnabled(false);
      } else if (c == '|') {
      ...
      //else if (c == '~') { if (e.isMetaDown()) { out("debug abort."); System.exit(-1);} }
        ...
        DEBUG.DYNAMIC_UPDATE = !DEBUG.DYNAMIC_UPDATE;
      } else if (c == '*') {
      ...
      //else if (c == '|') { VUE.toggleFullScreen(true); // native full screen mode }
        ...
        if (DebugInspector == null) {
          DebugInspector =
            GUI.createDockWindow(
              VueResources.getString("dockWindow.inspector.title"),
              new LWCInspector()
            );
          DebugIntrospector =
            GUI.createDockWindow(
        ...
        if (debugPanner == null) debugPanner =
          new DockWindow("Panner", new MapPanner());
  ...
  private void handleKeyReleased(KeyEvent e) {
    ...
    if (!activeTool.handleKeyReleased(e)) {
      if (tempToolKeyDown == e.getKeyCode()) {
        ...
        if (sDragUnderway) tempToolKeyReleased =
          true; else revertTemporaryTool();
      }
    }
  }
  ...
  private boolean checkAndHandleControlPointPress(MapMouseEvent e) {
    ...
    while (icl.hasNext()) {
      if (
        checkAndHandleControlListenerHits(
          (LWSelection.ControlListener) icl.next(),
          e,
          true
        )
      ) return true;
    ...
    if (resizeControl.active) {
      if (
        checkAndHandleControlListenerHits(resizeControl, e, false)
      ) return true;
    }
    return false;
  }

  private boolean checkAndHandleControlListenerHits(
  ...
  public void eventRaised(FocusManager.GlobalMouseEvent e) {
    ...
    if (e.event.getID() == MouseEvent.MOUSE_RELEASED) trackViewChanges(
      "globalMouseRelease"
    );
  }
  ...
  public void mousePressed(MouseEvent e) {
    ...
    if (DEBUG.FOCUS) System.out.println(
      "\tmouse-pressed active text edit=" + mLabelEditWasActiveAtMousePress
    );
    ...
    if (activeTool == HandTool) {
      ...
      if (inScrollPane) viewportAtDragStart =
        mViewport.getViewPosition(); else viewportAtDragStart = null;
    ...
    if (activeTool.supportsSelection()) {
      ...
      if (
        hitComponent == null &&
        activeTool instanceof ComboModeTool &&
        VUE.getSelection() != null &&
        VUE.getSelection().isEmpty()
      ) {
        //VUE-1597
        /*
      ...
      if (DEBUG.MOUSE && hitComponent != null) System.out.println(
        "\t    on " +
        hitComponent +
        "\n" +
        "\tparent " +
        hitComponent.getParent()
      );
      // if a LWSlide picked, animate zoom into it, and then load as focal
      mme.setPicked(hitComponent);

      if (
        !(hitComponent instanceof LWNode) &&
        !(hitComponent instanceof LWText) &&
        !(hitComponent instanceof LWLink) &&
        !(hitComponent instanceof LWImage) &&
    ...
    if (GUI.isMenuPopup(e) && !activeTool.usesRightClick()) {
      if (hitComponent != null && !hitComponent.isSelected()) selectionSet(
        justSelected = hitComponent
      );
      ...
      hitComponent != null ||
      (hitComponent == null &&
        tempToolPendingActivation instanceof ComboModeTool)
    ) { // check focal in case of on-slide
      ...
      if (mme.getDragRequest() != null) {
        ...
        e.isShiftDown() || (Util.isWindowsPlatform() && e.isControlDown())
      ) {
        //-------------------------------------------------------
    ...
    if (dragComponent == null) {
      if (dragSelectorEnabled(mme)) isDraggingSelectorBox =
        true; else isDraggingSelectorBox = false;
    }

    if (dragComponent != null) dragOffset.setLocation(
      dragComponent.getX() - mapX,
  ...
  private void displayContextMenu(MouseEvent e, LWComponent hitComponent) {
    ...
    PopupFactory.setSharedInstance(
      new VuePopupFactory(PopupFactory.getSharedInstance())
    );
    ...
    if (VueSelection.isEmpty() || VueSelection.only() instanceof LWMap) {
      ...
      getSingleSelectionPopup(hitComponent)
  ...
  private void dragResizeSelectorBox(int screenX, int screenY) {
    ...
    if (draggedSelectorBox != null) repaintRect = draggedSelectorBox;
    ...
    if (repaintRect == null) repaintRect =
      new Rectangle(draggedSelectorBox); else repaintRect.add(
      draggedSelectorBox
    );
    ...
    if (OPTIMIZED_REPAINT) //paintImmediately(repaintRect);
    repaint(repaintRect);
  ...
  public void mouseWheelMoved(MouseWheelEvent e) {
    ...
    if (DEBUG.MOUSE) Log.debug(
      "mouseWheelMoved meta=" +
      e.isMetaDown() +
      " alt=" +
      e.isAltDown() +
      " shift=" +
    ...
    if (eventSource instanceof JScrollPane) {
      Point viewPosition =
        ((JScrollPane) eventSource).getViewport().getViewPosition();
    ...
    if (e.isMetaDown() || e.isAltDown()) {
      final Point2D mouseLocation = new Point2D.Float(
        viewer.screenToMapX(e.getX() + scrollX),
        viewer.screenToMapY(e.getY() + scrollY)
      );
      ...
      tufts.vue.ZoomTool.setZoom(
        mZoomFactor * zoomFactorAdjustor,
        mouseLocation
      );
    } else {
  ...
  private final Runnable PanLeft = new Runnable() {
    public void run() {
      panScrollRegion(2, 0);
    }
  };
  private final Runnable PanRight = new Runnable() {
    public void run() {
      panScrollRegion(0, 2);
    }
  };
  ...
  private class ClearTipTask extends TimerTask {
    ...
    ClearTipTask() {
      ...
      if (DEBUG.FOCUS || DEBUG.THREAD) out(
        "ClearTipTask: scheduled for " +
        GUI.name(tipWhenTimerStarted) +
        " #" +
        tipDisplayInstance
      );
    }
    ...
    public void run() {
      synchronized (sTipLock) {
        ...
        if (DEBUG.FOCUS) out(
          "ClearTipTask: over nothing timeout for " +
          "#" +
          tipDisplayInstance +
          " during instance #" +
          sTipDisplayInstance
        ...
        if (mMouseHasEnteredToolTip == false) {
          ...
          if (sTipDisplayInstance == tipDisplayInstance) {
            if (DEBUG.FOCUS) out(
              "ClearTipTask: clearing tip " + sTipDisplayInstance
            );
            viewer.clearTip();
          }
        } else if (DEBUG.FOCUS) out(
          "ClearTipTask: mouse entered tooltip (it exited map)"
        );
      }
    }
  }
  ...
  public void mouseMoved(MouseEvent e) {
    if (DEBUG_MOUSE_MOTION) System.out.println(
      "[" + e.paramString() + "] on " + e.getSource().getClass().getName()
    );
    lastMouseX = e.getX();
    lastMouseY = e.getY();
    ...
    if (hit != sLastMouseOver) {
      ...
      if (DEBUG.ROLLOVER || DEBUG.FOCUS) System.out.println(
        "  mouseMoved: transition from " + sLastMouseOver
      );
    ...
    if (activeTool.handleMouseMoved(mme)) {
      ...
      if (hit == sLastMouseOver) hit.mouseMoved(mme); else hit.mouseEntered(
        mme
      );
    } else {
    ...
    if (DEBUG.PICK && !handled) {
      if (hit != null) setIndicated(hit); else clearIndicated();
    }
    ...
    if (isAutoZoomEnabled() && RolloverAutoZoomDelay > 0) {
      if (!sDragUnderway && !hasActiveTextEdit()/* && DEBUG_TIMER_ROLLOVER */) {
        if (RolloverAutoZoomDelay > 10 && mRollover == null) {
          if (rolloverTask != null) rolloverTask.cancel();
  ...
  public void mouseExited(MouseEvent e) {
    ...
    if (sLastMouseOver != null && sLastMouseOver == mRollover) clearRollover();
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if (DEBUG.MOUSE && DEBUG.DND) System.out.println(
      "[" + e.paramString() + "] on " + e.getSource().getClass().getName()
    );

    if (mouseConsumed) return;
    ...
    if (mouseWasDragged == false) {
      ...
      if (dragComponent == null && dragControl == null) viewer.setAutoscrolls(
        false
      );
      ...
      if (inScrollPane) lastDrag =
        SwingUtilities.convertPoint(MapViewer.this, lastDrag, getParent());
    }
    sDragUnderway = true;
    ...
    if (activeTool == HandTool) {
      ...
      if (originAtDragStart != null) {
        // if we pan, our canvas location might change, offsetting mouse coord each time
        if (inScrollPane) currentMousePosition =
      ...
      } else System.err.println("null originAtDragStart -- drag skipped!");
      return;
    }
    ...
    if (!e.getComponent().contains(screenX, screenY)) {
      ...
      if (screenX < 0) screenX = 0; else if (screenX >= getWidth()) screenX =
        getWidth() - 1;
      if (screenY < 0) screenY = 0; else if (screenY >= getHeight()) screenY =
        getHeight() - 1;
    }

    final float mapX = screenToMapX(screenX);
    final float mapY = screenToMapY(screenY);
    final MapMouseEvent mme = new MapMouseEvent(
      e,
      mapX,
      mapY,
      draggedSelectorBox,
    ...
    if (
      !dragSelectorEnabled(mme) && !activeTool.supportsResizeControls()
    ) return;
    ...
    if (dragControl != null) {
      ...
      dragComponent.setMapLocation(mapX + dragOffset.x, mapY + dragOffset.y);
    ...
    if (activeTool.handleMouseDragged(mme)) {} else if (
      !DEBUG_FINDPARENT_OFF &&
      ...
      if (VueSelection.size() == 1) pc.dropping =
        VueSelection.first(); else pc.dropping = dragComponent;
      ...
      if (over != null && isDropRequest(e)) {
        if (isValidParentTarget(VueSelection, over)) {
          ...
          over instanceof LWSlide == false &&
          isValidParentTarget(VueSelection, over.getParent())
        ) {
    ...
    if (dragComponent == null && dragControl == null) return;
    ...
    if (OPTIMIZED_REPAINT == false) {
      ...
      if (
        dragControl != null && dragControl instanceof LWComponent
      ...
      if (movingComponent instanceof LWLink) { // only happens thru a dragControl
        ...
        if (movingComponent != null) i = movingComponent.getLinked().iterator();
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    //else if (mouseDragInitiated && (indication == null || indication instanceof LWContainer)) {
      ...
      mouseWasDragged &&
      (indication == null || indication instanceof LWContainer)
    ) {
      ...
      if (dragComponent == null || !isDropRequest(e)) {
        ...
        processViewerLocalMoveAndDrop(
          e,
          (LWContainer) indication,
          getSelection()
        );
      }
    }
    ...
    // for performance)
    if (mouseWasDragged && mUndoManager != null) mUndoManager.mark("Drag");

    if (
      draggedSelectorBox != null && !dragSelectorEnabled(mme)
    ) System.err.println(
      "Illegal state warning: we've drawn a selector box w/out tool that supports it!"
    );
    ...
    if (draggedSelectorBox != null && dragSelectorEnabled(mme)) {
      ...
      if (activeTool instanceof NodeTool.OntologyNodeTool) {
        if (
          draggedSelectorBox.width > 0 || draggedSelectorBox.height > 0
        ) handled = activeTool.handleSelectorRelease(mme);
      } else {
        if (
          draggedSelectorBox.width > 10 && draggedSelectorBox.height > 10
        ) handled = activeTool.handleSelectorRelease(mme);
      }
      ...
      if (!handled && activeTool.supportsSelection()) {
        ...
        if (e.isShiftDown()) selectionToggle(list); else selectionSet(list);
      }
      ...
      // bounds cache hack
      if (!VueSelection.isEmpty()) draggedSelectionGroup.useSelection(
        VueSelection
      );
    ...
    if (mUndoManager != null) mUndoManager.mark(); // in case anything happened
    ...
    // of the screen.
    if (mouseWasDragged && dragComponent != null) scrollToVisible(
      dragComponent,
      6
    );
   ...
   */
  private boolean processViewerLocalMoveAndDrop(
    final MouseEvent e,
    final LWContainer target,
    final LWSelection selection
  ) {
    if (DEBUG.EVENTS || DEBUG.DND) System.out.println(
      TERM_GREEN +
      "\nINTERNAL MAP MOUSE DROP EVENT in " +
      this +
      "\n\t event: " +
      e +
      "\n\ttarget: " +
    ...
    if (target != null && !target.supportsChildren()) return false;
    ...
    for (LWComponent dropped : selection) {
      ...
      if (!parent.supportsChildren()) { // old condition?
        if (DEBUG.Enabled) Util.printStackTrace(
          "ignoring, current parent does not support children: " + dropped
        );
        continue;
      }
      ...
      //-------------------------------------------------------
      if (DEBUG.PARENTING || DEBUG.DND) out(
        "processDrop",
        "new parent: " + target + "; " + dropped
      );
    ...
    if (moveList.size() > 0) {
      ...
      for (LWComponent c : moveList) parents.add(c.getParent());
      ...
      for (LWContainer parent : parents) {
        ...
        if (newParent != parent || parent.isManagingChildLocations()) {
          ...
          if (DEBUG.DND) out(
            "processDrop",
            "no reparenting needed into " + newParent
          );
        }
      }
    ...
    for (LWContainer c : putbacks) c.layout();

    return moved;
  }

  private LWContainer reparentTarget(
    ...
    if (picked == null) {
      ...
      if (focal == null || target == null) return null;
      // never let rise above the focal
      if (focal.getDepth() > target.getDepth()) {
        if (focal instanceof LWContainer) target =
          (LWContainer) focal; else target = null;
      }
    } else target = picked;
  ...
  private final int SYSTEM_DRAG_MODIFIER = VueUtil.isMacPlatform()
  ...
  private final boolean isSystemDragStart(MouseEvent e) {
    ...
    if (!activeTool.supportsDrag(e)) return false;

    if (
      VueSelection != null &&
      VueSelection.size() == 1 &&
      VueSelection.first().supportsCopyOnDrag()
    ) return true; else return (
      !e.isPopupTrigger() &&
      e.getButton() <= 1 &&
      (e.getModifiers() & GUI.ALL_MODIFIER_KEYS_MASK) == SYSTEM_DRAG_MODIFIER
    );
  }

  private final boolean isDoubleClickEvent(MouseEvent e) {
    return (
      !activeToolAteMousePress &&
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    if (!mouseConsumed && hitOnSelectionHandle == false) {
      ...
      if (!activeTool.handleMouseClicked(me)) processMouseClick(me);
    }
    ...
    if (isSingleClickEvent(e)) {
      ...
      if (hit != null && hit != mFocal && !(hit instanceof LWGroup)) {
        ...
        if (!mLabelEditWasActiveAtMousePress) {
          if (
            activeTool == NodeModeTool && oneClickNodePref.isTrue()
          ) Actions.NewNode.fire(e); else if (
            activeTool == TextTool
          ) Actions.NewRichText.fire(e); else if (
            activeTool == RichTextTool
  ...
  public boolean isValidParentTarget(LWSelection s, LWComponent parentTarget) {
    ...
    return false;
    if (parentTarget == dragComponent) return false;
    if (parentTarget.getParent() == dragComponent) return false;
    if (parentTarget.isTopLevel()) return false;
    if (s.size() == 1) {
      if (!s.first().supportsReparenting()) return false;
    ...
    if (
      parentTarget instanceof LWSlide &&
      (mFocal != parentTarget || !parentTarget.isPathwayOwned())
    ) {
  ...
  private final Runnable focusIndicatorRepaint = new Runnable() {
    public void run() {
  ...
  private void becomeActiveViewer() {
    ...
    if (activeViewer != this) {
      ...
      if (activeViewer != null) oldActiveMap = activeViewer.getMap();
      VUE.setActive(MapViewer.class, this, this);
      if (mFocal != null) mFocal
        .getChangeSupport()
        .setPriorityListener(this); else Log.warn(
        "Active viewer has no focal: " + this
      );
  ...
  public void grabVueApplicationFocus(String from, ComponentEvent event) {
    ...
    if (id == FocusEvent.FOCUS_GAINED) requestFocus = false; else if (
      id == MouseEvent.MOUSE_ENTERED && hasActiveTextEdit()
    ) requestFocus = false; else if (
      id == MouseEvent.MOUSE_PRESSED && GUI.isMenuPopup(event)
    ) requestFocus = false; else if (
      focusOwner == this || focusOwner == getRootPane()
    ) {
  ...
  public void focusGained(FocusEvent e) {
    ...
    if (DEBUG.FOCUS) out(
      "focusGained (from " +
      GUI.name(e.getOppositeComponent()) +
      ") parentWindow=" +
      parentWindow
    );
  ...
  public void focusLost(FocusEvent e) {
    if (DEBUG.FOCUS) out(
      "focusLost (to " + GUI.name(e.getOppositeComponent()) + ")"
    );
    ...
    if (VueUtil.isMacPlatform()) {
      ...
      String opName = null;
      if (lostTo != null) opName = lostTo.getName();
  ...
  private void repaintFocusIndicator() {
    if (MapScrollPane.UseMacFocusBorder && inScrollPane && GUI.isMacAqua()) {
      ...
      if (DEBUG.FOCUS) out(
        "repaintFocusIndicator " + scrollPane.getClass().getName()
      );
      // this is slow because the whole map must also repaint
      scrollPane.repaint();
    } else mFocusIndicator.repaint();
  }
  ...
  static void installExampleNodes(LWMap map) {
    ...
    if (true) {
      ...
      LWNode parent = new LWNode(
        "PARENT CENTER *x",
        0,
        0,
        new RectangularPoly2D.Diamond()
      );
      parent.setStrokeWidth(0);
      parent.setFillColor(Color.lightGray);
      map.addNode(parent);
      map
        .addNode(new LWNode("child", 0, 0, new Rectangle2D.Float()))
        .setStrokeWidth(0);
  ...
  protected void out(String method, Object msg) {
    if (method.charAt(0) == '@') tufts.Util.printStackTrace(method);
    Log.debug(
      String.format(
        "  <%s>[%s] %18s: %s",
        instanceName,
  ...
  private String fmt(Point2D p) {
    return Util.fmt(p);
  }

  private String fmt(Rectangle2D r) {
    return String.format(
      "%7.1f,%-7.1f %7.1fx%-7.1f]",
      r.getX(),
      r.getY(),
      r.getWidth(),
      r.getHeight()
  ...
  private String fmt(Dimension d) {
    return d.width + " x " + d.height;
  }

  // poor choice of name: backward compat for now
  private String out(Dimension d) {
    return fmt(d);
  }

  private String out(Point2D p) {
    return fmt(p);
  }

  private String out(Rectangle2D r) {
    return fmt(r);
  }

  private boolean DEBUG_MOUSE_MOTION = VueResources.getBool(
    "mapViewer.debug.mouse_motion"
  ); //todo: make command line -D override these
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < args.length; i++) {
      if (args[i].equals("-zoom")) test_zoom = true; else if (
        args[i].equals("-node")
      ) test_node = true; else if (args[i].equals("-panner")) show_panner =
        true; else if (args[i].equals("-scroll")) use_scroller = true; else if (
        args[i].equals("-menu")
      ) use_menu = true; else if (args[i].equals("-map")) test_map = true;
    }
    ...
    if (test_map) VUE.installExampleMap(map); else map.add(
      new LWNode(
        VueResources.getString("newnode.html"),
  ...
  protected void drawViewerDebug(DrawContext dc) {
    ...
    if (true) {
      ...
      if (inScrollPane) {
        g.drawString(
          "viewport----pos " + out(mViewport.getViewPosition()),
          x,
          y += 15
        );
      }
      if (getMap() != null) g.drawString(
        "map-canvas-size " + out(mapToScreenDim(getMap().getBounds())),
        x,
        y += 15
      );
      if (mFocal == null) g.drawString(
        "map-canvas-adju (NULL FOCAL)",
        x,
        y += 15
      ); else g.drawString(
        "map-canvas-adju " + out(mapToScreenDim(getContentBounds())),
        x,
        y += 15
      );
