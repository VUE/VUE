...
public class URLResource extends Resource implements XMLUnmarshalListener {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(URLResource.class);
  ...
  private void setSpecByFile(File file, Object knownType) {
    if (file == null) {
      Log.error(
        "setSpecByFile",
        new IllegalArgumentException("null java.io.File")
      );
      return;
    }
    ...
    if (DEBUG.RESOURCE && DEBUG.META && "/".equals(fileSpec)) {
      Util.printStackTrace(
        "Root FileSystem Resource created from: " + Util.tags(file)
      );
    }
  }
  ...
  private Object setDataFile(File file, Object type) {
    ...
    if (
      type == FILE_DIRECTORY || (type == FILE_UNKNOWN && file.isDirectory())
    ) {
      if (DEBUG.RESOURCE && DEBUG.META) out(
        "setDataFile: ignoring directory: " + file
      );
  ...
  public void setSpec(final String newSpec) {
    ...
    if (DEBUG.RESOURCE) dumpField(
      TERM_CYAN + "setSpec------------------------" + TERM_CLEAR,
      newSpec
    );

    if (newSpec == null) throw new IllegalArgumentException(
      Util.tags(this) + "; setSpec: null value"
    );
  ...
  public void XML_completed(Object context) {
    ...
    if (context != MANAGED_UNMARSHALLING) {
      if (DEBUG.RESOURCE && DEBUG.META) out(
        "XML_completed: unmanaged (immediate) init in context " +
        Util.tags(context) +
        "; " +
        this
      );
  ...
  private void loadXMLProperties() {
    ...
    for (KVEntry entry : mXMLpropertyList) {
      ...
      if (DEBUG.Enabled) {
        ...
        if (lowKey.startsWith("subject.")) key = "Subject"; else if (
          lowKey.startsWith("keywords.")
        ) key = "Keywords";
      }
      ...
      try {
        ...
        Log.error(
          this + "; loadXMLProperties: " + Util.tags(mXMLpropertyList),
          t
        );
      }
    }
  ...
  protected String extractExtension() {
    if (mURL != null) return super.extractExtension(
      mURL.getPath()
    ); else return super.extractExtension();
  }
  ...
  private void parseAndInit() {
    ...
    if (getClientType() == Resource.NONE) {
      if (isLocalFile()) {
        if (mFile != null && mFile.isDirectory()) setClientType(
          Resource.DIRECTORY
        ); else setClientType(Resource.FILE);
    ...
    if (getClientType() != Resource.DIRECTORY && !isImage()) {
      ...
      if (mFile != null) setAsImage(looksLikeImageFile(mFile.getName())); // this just a minor optimization
      else setAsImage(looksLikeImageFile(this.spec)); // this is the default
    ...
    if (!hasProperty(CONTENT_TYPE) && mURL != null) setProperty(
      CONTENT_TYPE,
  ...
  private java.net.URI toAbsoluteURI() {
    if (mFile != null) return toCanonicalFile(mFile).toURI(); else if (
      mURL != null
    ) return makeURI(mURL); else return makeURI(getSpec());
  }
  ...
  private URI findRelativeURI(URI root) {
    ...
    if (
      root.getScheme() == null || !root.getScheme().equals(absURI.getScheme())
    ) {
      //if (DEBUG.Enabled) out("differing schemes: " + root + " - " + absURI + "; can't be relative");
      if (DEBUG.RESOURCE) Log.info(
        this +
        "; scheme=" +
        absURI.getScheme() +
        "; different scheme: " +
    ...
    if (!absURI.isAbsolute()) Log.warn(
      "findRelativeURI: non-absolute URI: " + absURI
    );
  ...
  public void restoreRelativeTo(URI root) {
    ...
    if (absoluteURI != null) {
      ...
      out_error(
        "failed to find relative " + relative + "; in " + root + " for " + this
      );
    }
  }
  ...
  public boolean dataHasChanged() {
    ...
    if (mDataFile != null) file = mDataFile; // in case user edits a package file
    else file = mFile;
  ...
  public String getLocationName() {
    ...
    if (archive == null) {
      ...
      if (mDataFile != null) name = mDataFile.getName(); else if (
        mTitle != null
      ) name = mTitle; else name = getSpec();
  ...
  private Object getBrowseReference() {
    if (mURL != null) return mURL; else if (
      mFile != null
    ) return mFile; else if (
      mDataFile != null
    ) return mDataFile; else return getSpec();
  }
...
*/
  ...
  public java.util.List getPropertyList() {
    if (mRestoreUnderway == false) {
      if (
        mProperties.size() == 0
      ) return null; // a hack for castor to work

      mXMLpropertyList = new ArrayList(mProperties.size());

      //for (Map.Entry<String,?> e : mProperties.entries())
      for (Map.Entry e : mProperties.entries()) mXMLpropertyList.add(
        new PropertyEntry(e)
      );
    ...
    if (DEBUG.CASTOR) System.out.println(
      this + " getPropertyList " + mXMLpropertyList
    );
  ...
  public void XML_addNotify(Object context, String name, Object parent) {
    if (DEBUG.CASTOR) System.out.println(
      this + " XML ADDNOTIFY as \"" + name + "\" to parent " + parent
    );
  }
  ...
  protected String determineDataType() {
    ...
    if (spec.endsWith("=jpeg")) {
      ...
      spec != SPEC_UNSET && spec.startsWith("http") && spec.contains("fedora")
    ) { // fix for fedora url
        ...
        if (type == null || type.length() < 1) {
          try {
            ...
            if (DEBUG.Enabled) out(
              "polling actual HTTP server for content-type: " + url
            );
            /**
            ...
            if (type != null && type.length() > 0) setProperty(
              CONTENT_TYPE,
              type
            );
          } catch (Throwable t) {
  ...
  private static boolean isHTML(final Resource r) {
    ...
    return (
      !s.endsWith(".vue") && isHtmlMimeType(r.getProperty("url.contentType"))
  ...
  public Object getPreview() {
    if (isCached() && isImage()) return this; else if (
      mURL_ThumbData != null
    ) return mURL_ThumbData; else if (
      mURL_ImageData != null
    ) return mURL_ImageData; else if (
      isImage()
    ) // returning "this" is a bit unclean: done so that Images.java can put meta-data back into us
    return this; else if (
      isLocalFile() ||
      getClientType() == Resource.FILE ||
      getClientType() == Resource.DIRECTORY
    ) {
      return getFileIconImage();
    } else if (
      mURL != null && !isLocalFile()
    ) //else if (getClientType() == Resource.URL && !isLocalFile())
    {
      //System.out.println("mURL : " + mURL);

      if (
        mURL.toString().toLowerCase().endsWith(VueUtil.VueExtension)
      ) return VueResources.getBufferedImage(
        "vueIcon64x64"
      ); else return getThumbshotURL(mURL);
      //             if (mThumbShot == null) {
  ...
  private URL getThumbshotURL(URL url) {
    ...
    ); else return makeURL(THUMBSHOT_FETCH + url);
  }
  ...
  public void scanForMetaDataAsync(
    final LWComponent c,
    final boolean setLabelFromTitle
  ) {
    //if (!isHTML() && !isImage()) {
  ...
  private void _scanForMetaData(URL _url) throws java.io.IOException {
    if (DEBUG.Enabled) System.out.println(
      this + " _scanForMetaData: xml props " + mXMLpropertyList
    );
  ...
  private static final Pattern HTML_Title_Regex = Pattern.compile(
    ".*<\\s*title[^>]*>\\s*([^<]+)", // hacked for lang=he constructs, but too broad
    Pattern.MULTILINE | Pattern.DOTALL | Pattern.CASE_INSENSITIVE
  );

  private static final Pattern Content_Charset_Regex = Pattern.compile(
    ".*charset\\s*=\\s*([^\">\\s]+)",
    Pattern.MULTILINE | Pattern.DOTALL | Pattern.CASE_INSENSITIVE
  );
  ...
  private Properties scrapeHTMLmetaData(
    ...
    if (DEBUG.DND) System.err.println(
    ...
    if (!isHTML()) { // we only currently handle HTML
      if (DEBUG.Enabled) System.err.println(
        "*** contentType [" +
        contentType +
        "] not HTML; skipping title extraction"
      );
      return metaData;
    }
    ...
    try {
      final BufferedInputStream bufStream = new BufferedInputStream(
        byteStream,
        maxSearchBytes
      );
      ...
      if (DEBUG.DND && DEBUG.META) System.out.println(
        "*** HTML-STRING[" + html + "]"
      );
      ...
      if (cm.lookingAt()) {
        charset = cm.group(1);
        if (DEBUG.DND) System.err.println(
          "*** found HTML specified charset [" + charset + "]"
        );
        setProperty("charset", charset);
      }

      if (charset == null && contentEncoding != null) {
        if (DEBUG.DND || true) System.err.println(
          "*** no charset found: using contentEncoding charset " +
          contentEncoding
        );
      ...
      if (charset != null) {
        bufStream.reset();
        InputStreamReader decodedStream = new InputStreamReader(
          bufStream,
          charset
        );
        //InputStreamReader decodedStream = new InputStreamReader(new ByteArrayInputStream(byteBuffer), charset);
        if (true || DEBUG.DND) System.out.println(
          "*** decoding bytes into characters with official encoding " +
        ...
        decodedStream.close();
        if (true || DEBUG.DND) System.err.println(
          "*** " + decodedChars + " characters decoded using " + charset
        );
        decodedHTML = new String(decoded, 0, decodedChars);
      } else decodedHTML = html; // we'll just have to go with the default platform charset...
    ...
    if (DEBUG.DND || DEBUG.Enabled) System.err.println(
      "*** scrapeHTMLmetaData returning [" + metaData + "]"
    );
    return metaData;
  }
