 ...
 * as well as requesting browse components and loading them into the browse
 * pane.
 * Also handles the big task of kicking off parallel searches for multiple
 * sources
...
public class DataSourceViewer
  ...
  implements
    KeyListener, edu.tufts.vue.fsm.event.SearchListener, ActionListener {

  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(DataSourceViewer.class);
  ...
  public static final org.osid.shared.Type favoritesRepositoryType =
    new edu.tufts.vue.util.Type("edu.tufts", "favorites", "Favorites");
  JPopupMenu popup;
  ...
  // private java.awt.Dimension resultSetPanelDimensions = new
  // java.awt.Dimension(400,200);
  // private javax.swing.JPanel resultSetPanel = new javax.swing.JPanel();

  org.osid.shared.Type searchType = new edu.tufts.vue.util.Type(
    "mit.edu",
    "search",
    "keyword"
  );
  org.osid.shared.Type thumbnailType = new edu.tufts.vue.util.Type(
    "mit.edu",
    "partStructure",
    "thumbnail"
  );
  ...
  private final java.util.List<SearchThread> mSearchThreads =
    java.util.Collections.synchronizedList(
      new java.util.LinkedList<SearchThread>()
    );
  private final java.util.List<MapBasedSearchThread> mMapBasedSearchThreads =
    java.util.Collections.synchronizedList(
      new java.util.LinkedList<MapBasedSearchThread>()
    );
  ...
  public DataSourceViewer(DRBrowser drBrowser) {
    ...
    if (true) {
      ...
      tabs.add(
        VueResources.getString("jtab.data"),
        new JLabel(VueResources.getString("jlabel.data"))
      );
      tabs.add(VueResources.getString("jtab.resource"), dataSourceList);
      add(tabs);
    }

    // TODO: can any of these (beyond refresh), ever have an effect? These
    // properties
    ...
    Widget.setHelpAction(
      DRB.librariesPane,
      VueResources.getString("dockWindow.Resources.libraryPane.helpText")
    );
    // dockWindow.addButton is not a localized string in this context its just a
    // property that is later resolved.
    Widget.setMiscAction(
      DRB.librariesPane,
      new MiscActionMouseListener(),
      "dockWindow.addButton"
    );
    // Widget.setHelpAction(DRB.browsePane,VueResources.getString("dockWindow.Resources.browsePane.helpText"));
    // There is no such property and if there was, the question mark would be blue
    // instead of brown.
    Widget.setHelpAction(
      DRB.resultsPane,
      VueResources.getString("dockWindow.Resources.resultsPane.helpText")
    );
    Widget.setHelpAction(
      DRB.searchPane,
      VueResources.getString("dockWindow.Resources.searchPane.helpText")
    );

    Widget.setRefreshAction(
      DRB.browsePane,
      new MouseAdapter() {
    ...
    editInfoDockWindow.setLocation(
      DRB.dockWindow.getX() + DRB.dockWindow.getWidth(),
      DRB.dockWindow.getY()
    );

    NoConfig.setMinimumSize(new Dimension(100, 50));
    configMetaData.setName(
      VueResources.getString("datasourceviewer.name.contentdescription")
    );
  ...
  // Log.info("CONFIGURED");
  // if (singleton != null) {
  // // TODO: VueDataSource accessors used during painting (in AWT thread) not
  // fully thread-safe
  // // against the above assignRepositoryConfiguration -- we're relying on luck
  // at the moment.
  // singleton.repaint();
  // } else
  ...
  private void loadBrowseableDataSources() {
    ...
    try {
      ...
      VueUtil.alert(
        VueResources.getString("dialog.loadresourceerror.message"),
        VueResources.getString("dialog.loadresourceerror.title")
      );
    }
  }
  ...
  private void loadOSIDDataSources() {
    ...
    try {
      ...
      Log.error(t);
      VueUtil.alert(
        VueResources.getString("dialog.loadosiderror.message") + "\n" + t,
        VueResources.getString("dialog.loadosiderror.title")
      );
    }

    federatedSearchManager =
      edu.tufts.vue.fsm.impl.VueFederatedSearchManager.getInstance();
    sourcesAndTypesManager =
      edu.tufts.vue.fsm.impl.VueSourcesAndTypesManager.getInstance();
    if (DEBUG.Enabled) Log.debug(
      "sourcesAndTypesManager: " + Util.tags(sourcesAndTypesManager)
    );
    ...
    if (
      activeDataSource == null && dataSources != null && dataSources.length > 0
  ...
  JPopupMenu m = null;
  private static final JMenuItem aboutResource = new JMenuItem(
    VueResources.getString("datasourcehandler.aboutthisresources")
  );
  // private static final JMenuItem configureResource = new JMenuItem("Configure
  // Resource");
  private static final JMenuItem deleteResource = new JMenuItem(
    VueResources.getString("datasourcehandler.deleteresource")
  );
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource().equals(aboutResource)) {
      ...
      if (dataSourceList.getModel().getElementAt(index) instanceof DataSource) {
        displayEditOrInfo(
          (DataSource) dataSourceList.getModel().getElementAt(index)
        );
      } else {
        displayEditOrInfo(
          (edu.tufts.vue.dsm.DataSource) dataSourceList
            .getModel()
            .getElementAt(index)
        );
      }
  ...
  private JPopupMenu getPopup(MouseEvent e) {
    if (m == null) {
      m =
        new JPopupMenu(
          VueResources.getString("datasourcehandle.menu.datasource")
        );
  ...
  private void addListeners() {
    ...
    dataSourceList.addListSelectionListener(
        ...
        public void valueChanged(ListSelectionEvent e) {
          ...
          if (o != null) {
            ...
            if (o instanceof tufts.vue.DataSource) {
              ...
              edu.tufts.vue.dsm.DataSource ds =
                (edu.tufts.vue.dsm.DataSource) o;
              DataSourceViewer.this.setActiveDataSource(ds);
              refreshEditInfo(ds);
            } else {
    ...
    dataSourceList.addMouseListener(
      new MouseAdapter() {
        public void mouseClicked(MouseEvent e) {
          if (e.getClickCount() == 2) {
            if (activeDataSource instanceof DataSource) {
              ...
              displayEditOrInfo(
                (edu.tufts.vue.dsm.DataSource) activeDataSource
              );
            }
          } else {
            Point pt = e.getPoint();
            if (
              (activeDataSource instanceof edu.tufts.vue.dsm.DataSource) &&
              (pt.x <= 40)
            ) {
              ...
              if (DEBUG.DR) Log.debug(
                "DataSource " +
                ds +
                " [" +
                ds.getProviderDisplayName() +
                "] inclusion: " +
                included
              );
  ...
  protected void displayInBrowsePane(JComponent viewer, boolean priority) {
    if (DEBUG.Enabled) Log.debug(
      "displayInBrowsePane: " + browserDS + "; " + GUI.name(viewer)
    );

    String title =
      VueResources.getString("button.browse.label") +
      ": " +
      browserDS.getDisplayName();
  ...
  public void setActiveDataSource(final tufts.vue.DataSource ds) {
    // if (DEBUG.Enabled) Log.debug("setActiveDataSource: " + ds, new
    // Throwable("FYI"));
  ...
  public void setPopup() {
    ...
    checkForUpdatesAction =
      new AbstractAction(
        VueResources.getString("datasourcehandler.updateresources")
      ) {
        public void actionPerformed(ActionEvent e) {
          try {
            edu.tufts.vue.dsm.OsidFactory factory =
              edu.tufts.vue.dsm.impl.VueOsidFactory.getInstance();
            org.osid.provider.ProviderIterator providerIterator =
              factory.getProvidersNeedingUpdate();
            if (providerIterator.hasNextProvider()) {
              if (updateLibraryDialog == null) {
                updateLibraryDialog =
                  new UpdateLibraryDialog(
                    dataSourceList,
    ...
    addLibraryAction =
      new AbstractAction(
        VueResources.getString("datasourcehandler.addresources")
      ) {
        public void actionPerformed(ActionEvent e) {
          try {
            ...
            if (ds != null) {
              ...
              edu.tufts.vue.dsm.DataSource ds1 =
                addLibraryDialog.getNewDataSource();
              if (ds1 != null) {
        ...
        public void actionPerformed(ActionEvent e) {
          ...
          if (o != null) {
            ...
            if (o instanceof edu.tufts.vue.dsm.DataSource) {
              ...
              DefaultListModel listComponents =
                dataSourceList.getModelContents();
              int index = dataSourceList.getModelContents().indexOf(ds);
              int instanceIndex = 0;
              for (int p = 0; p < index; p++) {
                edu.tufts.vue.dsm.impl.VueDataSource o2 =
                  (edu.tufts.vue.dsm.impl.VueDataSource) listComponents.get(p);

                if (
                  o2.getRepositoryDisplayName() == ds.getRepositoryDisplayName()
                ) instanceIndex++;
              }
              ...
              if (
                VueUtil.confirm(
                  ...
                  String.format(
                    Locale.getDefault(),
                    VueResources.getString("datasource.dialog.message"),
                    displayName
                  ),
                ...
                if (widgetStack != null) {
                  ...
                  for (int i = 0; i < comps.length; i++) {
                    ...
                    if (
                      (compName != null) &&
                      (compName.indexOf(displayName) != -1)
                    ) {
                      if (
                        (found == instanceIndex) || (found == instanceIndex + 1)
  ...
  private boolean checkValidUser(String userName, String password, int type) {
    if (type == 3) {
      try {
        ...
        if (
          tuftsDL.isAuthorized(user, TuftsDLAuthZ.AUTH_VIEW)
        ) return true; else return false;
      } catch (Exception ex) {
        VueUtil.alert(
          null,
          VueResources.getString("dialog.checkvaliduser.message") + ex,
          VueResources.getString("dialog.checkvaliduser.title")
        );
        ex.printStackTrace();
        return false;
      }
  ...
  private static final Collection<tufts.vue.DataSource> oldStyleDataSources =
    new ArrayList();
  ...
  private void loadOldStyleDataSources() {
    ...
    boolean init = true;
    File f = new File(
      VueUtil.getDefaultUserFolder().getAbsolutePath() +
      File.separatorChar +
      VueResources.getString("save.datasources")
    );
    if (DEBUG.DR) Log.debug("Data source file: " + f.getAbsolutePath());
    if (!f.exists()) {
      if (DEBUG.DR) System.out.println(
        "Loading default DataSources (does not exist: " + f + ")"
      );
      ...
      try {
        if (DEBUG.DR) Log.debug(
          "Loading saved DataSources from " + f + "; unmarshalling..."
        );
        VUE.diagPush("XML");
        ...
        while (!(dataSources.isEmpty())) {
          ...
          if (!ds.getTypeName().equals(XmlDataSource.TYPE_NAME)) {
            ...
            try {
              ...
              System.out.println(
                "DataSourceViewer.loadOldStyleDataSources" + ex
              );
            }
          }
        }
  ...
  public static void cacheDataSourceViewers() {
    if (singleton != null) singleton.cacheViewers();
  }
  ...
  private void loadDefaultDataSources() {
    try {
      ...
      DataSource ds1 = new FavoritesDataSource(
        VueResources.getString("addLibrary.mysavedcontent")
      );
      dataSourceList.addOrdered(ds1);
      // dataSourceList.getModelContents().addElement(breakTag);
      DataSource ds2 = new LocalFileDataSource(
        VueResources.getString("addLibrary.mycomputer"),
        ""
      );
  ...
  // // ImageIcon icon = new
  // ImageIcon(Toolkit.getDefaultToolkit().getImage((String)part.getValue()));
  ...
  private synchronized void stopAllSearches() {
    ...
    synchronized (mSearchThreads) {
      if (DEBUG.DR) Log.debug(
        "STOPPING ALL ACTIVE SEARCHES; count=" + mSearchThreads.size()
      );
    ...
    synchronized (mMapBasedSearchThreads) {
      ...
      for (Thread t : mMapBasedSearchThreads) t.interrupt();
    }
  }
  ...
  private class MapBasedSearchThread extends Thread {
    ...
    public MapBasedSearchThread(
      org.osid.repository.Repository r,
      String searchString,
      ...
      if (listOfLists == null) listOfLists =
      ...
      mResultPane =
        new Widget(
          VueResources.getString("datasource.dialog.searching") +
          " " +
          mRepositoryName
        );
      ...
      mStatusLabel =
        new StatusLabel(
          VueResources.getString("resource.dialog.searchingfor") +
          " " +
          mSearchString +
          " ...",
          false
      ...
      if (DEBUG.DR) Log.debug(
        "created search thread for: " + mRepositoryName + " \t" + mRepository
      );
    }
    ...
    public void run() {
      ...
      try {
        ...
        AssetIterator asIt = mRepository.getAssetsBySearch(
          mSearchCriteria,
          mSearchType,
          mSearchProperties
        );
        ...
        GUI.invokeAfterAWT(
          new Runnable() {
            public void run() {
              mResultPane.setTitle(
                VueResources.getString("searchgui.results") +
                ": " +
                mRepositoryName
              );
      ...
      if (stopped()) {
        GUI.clearWaitCursor();
        if (DEBUG.DR) Log.debug(
          "DELAYED STOP; server returned, run completed."
        );
        return;
      }
      ...
      if (DEBUG.DR) Log.debug(
        "RUN COMPLETED, stillActive=" + mSearchThreads.size()
      );
      ...
      if (mMapBasedSearchThreads.size() == 0) {
        ...
        if (DEBUG.DR) Log.debug(
          "ALL SEARCHES COMPLETED for \"" + mSearchCriteria + "\""
        );
        if (
          queryEditor instanceof edu.tufts.vue.ui.DefaultQueryEditor
    ...
    private String translateRepositoryException(String msg) {
      String s;
      if (
        msg.equals(org.osid.repository.RepositoryException.OPERATION_FAILED)
      ) {
        s = VueResources.getString("repositoryexception.operationfailed");
      } else if (
        msg.equals(org.osid.repository.RepositoryException.PERMISSION_DENIED)
      ) {
        s = VueResources.getString("repositoryexception.permissiondenied");
      } else if (
        msg.equals(org.osid.repository.RepositoryException.CONFIGURATION_ERROR)
      ) {
        ...
        new Runnable() {
          public void run() {
            ...
            mStatusLabel.setText(
              VueResources.getString("datasourcehandle.searchstopped.tooltip")
            );
          }
        }
      );
    ...
    private void adjustQuery() throws org.osid.repository.RepositoryException {
      ...
      edu.tufts.vue.fsm.QueryAdjuster adjuster =
        federatedSearchManager.getQueryAdjusterForRepository(
          mRepository.getId()
        );
      if (adjuster != null) {
        edu.tufts.vue.fsm.Query q = adjuster.adjustQuery(
          mRepository,
          mSearchCriteria,
          mSearchType,
          mSearchProperties
    ...
    private void processMapBasedSearchResultsAndDisplay(
      org.osid.repository.AssetIterator assetIterator
    ) throws org.osid.repository.RepositoryException {
      if (stopped()) return;

      if (DEBUG.DR) Log.debug(
        "processing AssetIterator: " + Util.tags(assetIterator)
      );
      ...
      if (assetIterator != null) {
        try {
          while (assetIterator.hasNextAsset() && (resultCount < maxResult)) {
            ...
            if (asset == null) {
              Log.warn(
                "null asset in " + mRepositoryName + "; " + assetIterator
              );
              continue;
            }
            try {
              ...
              Log.warn(
                "Failed to create resource for asset: " + Util.tags(asset),
                t
              );
            }
          }
          ...
          if (resourceList.size() < 1) {
            if (
              t instanceof RepositoryException
            ) throw (RepositoryException) t; else throw new RuntimeException(
              "processing assets for " + mRepositoryName,
              t
            );
          } else {
            ...
            Util.printStackTrace(
              t,
              "processing asset iterator for " + mRepositoryName
            );
          }
        }

        if (DEBUG.DR) Log.debug(
          "done processing AssetIterator; count=" + resultCount
        );
      }

      String name =
        VueResources.getString("searchgui.results") + ": " + mRepositoryName;

      if (DEBUG.DR) {
        if (resultCount > maxResult) Log.debug(
          name + "; returned a total of " + resultCount + " matches"
        );
      ...
      if (resourceList.size() == 0) {
        if (assetIterator == null) results =
          new StatusLabel("Empty results for " + mSearchString); else results =
          new StatusLabel("No results for " + mSearchString);
      } else {
      ...
      GUI.invokeAfterAWT(
        new Runnable() {
          public void run() {
            ...
            if (resourceList.size() > 0) AnalyzerAction.addResultsToMap(
              resourceList,
              mCenterComponent,
              mMapBasedSearchThreads.size()
            );
          }
        }
      );
   ...
   */
  private synchronized void performParallelSearchesAndMapResults(
    LWComponent c
  ) {
    final String searchString =
      "\"" + queryEditor.getSearchDisplayName() + "\"";
    final WidgetStack resultsStack = new WidgetStack(
      "searchResults " + searchString
    );
    final org.osid.repository.Repository[] repositories =
      sourcesAndTypesManager.getRepositoriesToSearch();
    ...
    final org.osid.shared.Properties searchProperties =
      queryEditor.getProperties();
    ...
    for (int i = 0; i < repositories.length; i++) {
      ...
      try {
        ...
        if (DEBUG.Enabled) VueUtil.alert(
          VueResources.getString("dialog.searcherror.message"),
          t
        ); else VueUtil.alert(
          t.getMessage(),
          VueResources.getString("dialog.searcherror.message")
  ...
  public static class StatusLabel extends JPanel {
    ...
    private static final Icon SlowSpinningIcon = VueResources.getIcon(
      "dsv.statuspanel.waitIcon"
    );
    ...
    public StatusLabel(String s, boolean center, boolean useIcon) {
      ...
      if (!useIcon && Util.isMacLeopard()) {
        ...
        // match icon height so search box doesn't change height when "no results" comes
        // back
  ...
  private class SearchThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        // INVOKE THE SEARCH, and immediately hand off to processResultsAndDisplay
        processResultsAndDisplay(
          mRepository.getAssetsBySearch(
            mSearchCriteria,
            mSearchType,
            mSearchProperties
          )
    ...
    private void processResultsAndDisplay(
      org.osid.repository.AssetIterator assetIterator
    ) throws org.osid.repository.RepositoryException {
      if (stopped()) return;
  ...
  private synchronized void performParallelSearchesAndDisplayResults() {
    ...
    // pending results area entirely while the search is running (we just see the
    // spinner)
    final String searchString =
      "\"" + queryEditor.getSearchDisplayName() + "\"";
    final WidgetStack resultsStack = new WidgetStack(
  ...
  // //final String dockTitle = "Search Results for \"" +
  // queryEditor.getSearchDisplayName() + "\"";
  // final String searchString = "\"" + queryEditor.getSearchDisplayName() + "\"";

  // /*
  // Store our results since we will fill a panel with each repository's results
  // and one with all.
  // We can't get the iterator contents again, without re-doing the search.

  // We know the repositories we searched. Some may have returned results, others
  // may not. We will
  // make a vector for each set of results with a parallel vector of repository
  // ids.
  // */
  ...
  // org.osid.repository.Repository[] repositories =
  // sourcesAndTypesManager.getRepositoriesToSearch();
  // edu.tufts.vue.dsm.DataSource[] dataSources =
  // sourcesAndTypesManager.getDataSourcesToSearch(); // will be same length

  // final WidgetStack resultsStack = new WidgetStack("searchResults " +
  // searchString);
  ...
  // resultPanes[i].add(new StatusLabel("Searching for " + searchString + " ...",
  // true));
  ...
  // if (DEBUG.DR) {
  // Log.debug("Searching criteria [" + searchCriteria + "] in selected
  // repositories. SearchProps=" + queryEditor.getProperties());
  // }
  ...
  // for (int i=0; i < dataSources.length; i++) {
  // org.osid.repository.AssetIterator assetIterator =
  // resultSetManager.getAssets(dataSources[i].getId().getIdString());
  // int counter = 0;
  ...
  // Osid2AssetResource resource = new Osid2AssetResource(nextAsset,
  // this.context);
  // v.add(resource);
  // }
  // }
  ...
  static PropertyMap buildPropertyMap(final edu.tufts.vue.dsm.DataSource ds) {
    ...
    try {
      ...
      if (r != null) {
        ...
        map.addProperty(
          "Type",
          edu.tufts.vue.util.Utilities.typeToString(r.getType())
        );
      } else {
      ...
      map.addProperty(
        "Release Date",
        edu.tufts.vue.util.Utilities.dateToString(ds.getReleaseDate())
      );
      ...
      String supportsUpd = ds.supportsUpdate()
        ? "The Resource Supports Updating"
      ...
      if (r != null) {
        ...
        if (typeIterator.hasNextType()) assetTypes.append(
          edu.tufts.vue.util.Utilities.typeToString(typeIterator.nextType())
        );
        ...
        if (typeIterator.hasNextType()) searchTypes.append(
          edu.tufts.vue.util.Utilities.typeToString(typeIterator.nextType())
        );
        while (typeIterator.hasNextType()) {
          searchTypes.append(", ");
  ...
  private static final JLabel NoConfig = new JLabel(
    VueResources.getString("jlabel.noconfig"),
    JLabel.CENTER
  );

  private final MetaDataPane configMetaData = new MetaDataPane(
    "Config Properties",
    true
  );
  ...
  private void positionEditInfoWindow() {
    ...
    if (
      (DRB.dockWindow.getX() +
        DRB.dockWindow.getWidth() +
        editInfoDockWindow.getWidth()) <
      screenSize.getWidth()
    ) editInfoDockWindow.setLocation(
      DRB.dockWindow.getX() + DRB.dockWindow.getWidth(),
      DRB.dockWindow.getY()
    ); else editInfoDockWindow.setLocation(
      DRB.dockWindow.getX() - editInfoDockWindow.getWidth(),
      DRB.dockWindow.getY()
    );
  }
  ...
  // always "Configuration: <type>", e.g., OSID, LocalFileDataSource,
  // RSSDataSource, etc.

  private static DockWindow _buildWindow() {
    final DockWindow dw = GUI.createDockWindow(
      VueResources.getString("dockWindow.resource.title")
    );
  ...
  private void refreshEditInfo(edu.tufts.vue.dsm.DataSource ds, boolean force) {
    ...
    if (force || editInfoDockWindow.isVisible()) {
      ...
      if (
        DEBUG.Enabled
      ) // name = "Configuration: " + ds.getClass().getName(); // always
      // edu.tufts.vue.dsm.impl.VueDataSource
      name =
        VueResources.getString("optiondialog.configuration.message") +
        ": " +
        ds.getRepository(); else name =
        VueResources.getString("optiondialog.configuration.message");
      ...
      JLabel label = new JLabel(
        VueResources.getString("jlabel.widgetstack"),
        JLabel.CENTER
      );
      if (DEBUG.BOXES) {
  ...
  public static Vector getPublishableDataSources(int i) {
    ...
    if (dataSourceList != null) {
      ...
      while (e.hasMoreElements()) {
        ...
        if (mDataSource instanceof Publishable) mDataSources.add(mDataSource);
      }
    }
    return mDataSources;
  }
  ...
  /**
   * returns the default favorites resources. This is will be used to add
   * favorites and perform search
   */
    ...
    try {
      for (int i = 0; i < model.size(); i++) {
        ...
        if (o instanceof edu.tufts.vue.dsm.DataSource) {
          ...
          if (repository == null) {
            Log.warn(
              "looking for default favorites: repository unavailable in: " +
              Util.tags(o)
            );
  ...
  public static void saveDataSourceViewer() {
     ...
     * File f = new
     * File(VueUtil.getDefaultUserFolder().getAbsolutePath()+File.separatorChar+
     * VueResources.getString("save.datasources"));
     * Vector sDataSources = new Vector();
     * if (DEBUG.DR) Log.debug("saveDataSourceViewer: found " + size +
     * " dataSources: scanning for local's to save...");
     * for (int i = 0; i<size; i++) {
     * Object item = dataSourceList.getModel().getElementAt(i);
     * if (DEBUG.DR) System.err.print("\tsaveDataSourceViewer: item " + i + " is " +
     * tufts.Util.tag(item) + "[" + item + "]...");
     ...
     * try {
     * if (DEBUG.DR)
     * Log.debug("saveDataSourceViewer: creating new SaveDataSourceViewer");
     * SaveDataSourceViewer sViewer= new SaveDataSourceViewer(sDataSources);
     * if (DEBUG.DR) Log.debug("saveDataSourceViewer: marshallMap: saving " +
     * sViewer + " to " + f);
  ...
  public void keyPressed(KeyEvent e) {
    ...
    // // todo: not very useful! Need to change the model in the
    // VueDataSourceManager,
