...
public class DataSourceViewer extends ContentViewer
    ...
    public static final org.osid.shared.Type favoritesRepositoryType = new edu.tufts.vue.util.Type("edu.tufts",
            "favorites", "Favorites");
    JPopupMenu popup;
    ...
    private final java.util.List<SearchThread> mSearchThreads = java.util.Collections
            .synchronizedList(new java.util.LinkedList<SearchThread>());
    private final java.util.List<MapBasedSearchThread> mMapBasedSearchThreads = java.util.Collections
            .synchronizedList(new java.util.LinkedList<MapBasedSearchThread>());
    ...
    private void loadBrowseableDataSources() {
        ...
        try {
            ...
            VueUtil.alert(VueResources.getString("dialog.loadresourceerror.message"),
    ...
    private void loadOSIDDataSources() {
        ...
        if (DEBUG.Enabled)
            Log.debug("sourcesAndTypesManager: " + Util.tags(sourcesAndTypesManager));
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    if ((activeDataSource instanceof edu.tufts.vue.dsm.DataSource) && (pt.x <= 40)) {
                        ...
                        edu.tufts.vue.dsm.DataSource ds = (edu.tufts.vue.dsm.DataSource) dataSourceList.getModel()
                                .getElementAt(index);
                        boolean included = !ds.isIncludedInSearch();
                        if (DEBUG.DR)
                            Log.debug("DataSource " + ds + " [" + ds.getProviderDisplayName() + "] inclusion: "
                                    + included);
                            ...
                            public void run() {
                                ...
                                try {
                                    synchronized (dataSourceManager) {
                                        if (DEBUG.DR)
                                            Log.debug("DataSourceManager saving...");
                                        dataSourceManager.save();
                                        if (DEBUG.DR)
                                            Log.debug("DataSourceManager saved.");
                                    }
                                } catch (Throwable t) {
                ...
                if (o != null) {
                    ...
                    if (o instanceof edu.tufts.vue.dsm.DataSource) {
                        ...
                        for (int p = 0; p < index; p++) {
                            edu.tufts.vue.dsm.impl.VueDataSource o2 = (edu.tufts.vue.dsm.impl.VueDataSource) listComponents
                                    .get(p);
    ...
    private boolean checkValidUser(String userName, String password, int type) {
        if (type == 3) {
            try {
                ...
                VueUtil.alert(null, VueResources.getString("dialog.checkvaliduser.message") + ex,
    ...
    private void loadOldStyleDataSources() {
        ...
        boolean init = true;
        File f = new File(VueUtil.getDefaultUserFolder().getAbsolutePath() + File.separatorChar
                + VueResources.getString("save.datasources"));
        if (DEBUG.DR)
            Log.debug("Data source file: " + f.getAbsolutePath());
        if (!f.exists()) {
            if (DEBUG.DR)
                System.out.println("Loading default DataSources (does not exist: " + f + ")");
            loadDefaultDataSources();
        } else {
            int type;
            try {
                if (DEBUG.DR)
                    Log.debug("Loading saved DataSources from " + f + "; unmarshalling...");
                VUE.diagPush("XML");
                final SaveDataSourceViewer dataSourceContainer = unMarshallMap(f);
                VUE.diagPop();
                if (DEBUG.DR)
                    Log.debug("Unmarshalling completed from " + f);
                ...
                if (DEBUG.DR)
                    Log.debug("Found " + dataSources.size() + " DataSources: " + dataSources);
                int i = 0;
                while (!(dataSources.isEmpty())) {
                    ...
                    if (!ds.getTypeName().equals(XmlDataSource.TYPE_NAME)) {
                        i++;
                        if (DEBUG.DR)
                            Log.debug(String.format("#%02d: loading %s ", i, Util.tags(ds)));
                        // VUE.diagPush("#" + i);
                        ...
                        try {
                            ...
                            System.out.println("DataSourceViewer.loadOldStyleDataSources" + ex);
                        }
                    }
                }
            } catch (Exception ex) {
    ...
    private synchronized void stopAllSearches() {
        ...
        synchronized (mSearchThreads) {
            if (DEBUG.DR)
                Log.debug("STOPPING ALL ACTIVE SEARCHES; count=" + mSearchThreads.size());
    ...
    private class MapBasedSearchThread extends Thread {
        ...
        public MapBasedSearchThread(org.osid.repository.Repository r,
            ...
            mStatusLabel = new StatusLabel(
                    VueResources.getString("resource.dialog.searchingfor") + " " + mSearchString + " ...", false);
            mResultPane.add(mStatusLabel);

            if (DEBUG.DR)
                Log.debug("created search thread for: " + mRepositoryName + " \t" + mRepository);
        }
        ...
        public void run() {
            ...
            if (stopped()) {
                GUI.clearWaitCursor();
                if (DEBUG.DR)
                    Log.debug("DELAYED STOP; server returned, run completed.");
                return;
            }

            mMapBasedSearchThreads.remove(this);
            if (DEBUG.DR)
                Log.debug("RUN COMPLETED, stillActive=" + mSearchThreads.size());
            ...
            if (mMapBasedSearchThreads.size() == 0) {
                ...
                if (DEBUG.DR)
                    Log.debug("ALL SEARCHES COMPLETED for \"" + mSearchCriteria + "\"");
        ...
        private void processMapBasedSearchResultsAndDisplay(org.osid.repository.AssetIterator assetIterator)
            ...
            if (DEBUG.DR)
                Log.debug("processing AssetIterator: " + Util.tags(assetIterator));
            ...
            if (assetIterator != null) {
                ...
                if (DEBUG.DR)
                    Log.debug("done processing AssetIterator; count=" + resultCount);
            }
        ...
        public void run() {
            ...
            try {
                ...
                processResultsAndDisplay(
                        mRepository.getAssetsBySearch(mSearchCriteria, mSearchType, mSearchProperties));
