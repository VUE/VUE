...
public class PresentationNotes {
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(PresentationNotes.class);

  private static FontMapper dfm = null;

  private static FontMapper getFontMapper() {
    if (dfm != null) return dfm; else {
  ...
  public static void createMapAsPDF(File file, LWMap map) {
    ...
    try {
      ...
      Dimension page = new Dimension(
        (int) document.getPageSize().getWidth() - 70,
      ...
      DrawContext dc = new DrawContext(
        ...
        map,
        false
      ); // todo: absolute links shouldn't be spec'd here
  ...
  public static void createPresentationSlidesDeck(File file) {
    ...
    try {
      ...
      for (LWPathway.Entry entry : VUE.getActivePathway().getEntries()) {
        ...
        final PdfTemplate template = PdfTemplate.createTemplate(
          writer,
          fillWidth,
          fillHeight
        );
        final PdfGraphics2D graphics = (PdfGraphics2D) template.createGraphics(
          fillWidth,
          fillHeight,
          getFontMapper(),
          false,
        ...
        try {
          if (DEBUG.Enabled) Log.debug(
            "painted " + DrawContext.getDebug() + " to " + dc
          );
          ...
          if (DEBUG.PDF) {
            final String dcDesc =
              ...
              String.format(
                " scale=%.1f%%",
                dc.g.getTransform().getScaleX() * 100
              );
            dc.setRawDrawing();
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        if (component instanceof LWNode) {
          ...
          //	PdfTemplate tp = cb.createTemplate(document.getPageSize().width()-80, document.getPageSize().height()-80);
          double scale = ZoomTool.computeZoomFit(
            page,
            15,
            bounds,
            offset,
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        if (c instanceof LWLink) {
          ...
          if (linkLabel == null || linkLabel.length() == 0) linkLabel = "Link";

          Paragraph p = new Paragraph(
            entryCount + ".  " + linkLabel.replaceAll("\\n", "")
          );
          f = p.getFont();
          f.setStyle(Font.BOLD);
          f.setSize(14f);
  ...
  private static void outlineChildNode(
    Document d,
    float indentation,
    LWNode n,
    int entryCount
    ...
    if (nodeLabel == null || nodeLabel.length() == 0) nodeLabel = "Node";

    Paragraph p = new Paragraph(
      entryCount + ".  " + n.getLabel().replaceAll("\\n", "")
    );
    Font f = p.getFont();
    f.setStyle(Font.BOLD);
    f.setSize(14f);
