...
public class FavoritesWindow
  ...
  private static final org.apache.log4j.Logger Log =
    org.apache.log4j.Logger.getLogger(FavoritesWindow.class);

  public static final String NEW_FAVORITES = VueResources.getString(
    "dialog.favoriteswindow.newfFavoritesfolder"
  );
  public static final String ADD_FAVORITES = VueResources.getString(
    "dialog.favoriteswindow.addfavoritesfolder"
  );
  public static final String REMOVE_FAVORITES = VueResources.getString(
    "dialog.favoriteswindow.removefavoritesfolder"
  );
  public static final String RESOURCE_INFO = VueResources.getString(
    "dialog.favoriteswindow.resourceinfo"
  );
  public static final String OPEN_RESOURCE = VueResources.getString(
    "dialog.favoriteswindow.openresource"
  );
  public static final String ADD_TO_MAP = VueResources.getString(
    "dialog.favoriteswindow.addtomap"
  );
  public static final String ADD_TO_NODE = VueResources.getString(
    "dialog.favoriteswindow.addtoselectednode"
  );
  public static final String REPLACE_ON_NODE = VueResources.getString(
    "dialog.favoriteswindow.replaceresourceonnode"
  );
  public static final String ADD_TO_SLIDE = VueResources.getString(
    "dialog.favoriteswindow.addtoslide"
  );
  public static final String REMOVE_RESOURCE = VueResources.getString(
    "dialog.favoriteswindow.removeresource"
  );
  public static final String CONFIRM_DEL_RESOURCE = VueResources.getString(
    "dialog.favoriteswindow.deleteresource"
  );
  public static final String TITLE_DEL_RESOURCE = VueResources.getString(
    "dialog.favoriteswindow.delresourceconf"
  );
  ...
  protected static final int MAX_LABEL_LINE_LENGTH = 30;
  ...
  protected JMenuItem addToNodeMenuItem, addToSlideMenuItem;
  ...
  public FavoritesWindow(String displayName, String saveFile) {
    ...
    if (!fileOpen) {
      ...
      URLResource favResource = (URLResource) Resource
        .getFactory()
        .get(displayName);
  ...
  public Action getDisplayAction(String displayName) {
    if (displayAction == null) displayAction = new DisplayAction(displayName);
  ...
  public void enablePopupMenu() {
    ...
    boolean isNode = (comp != null && comp instanceof LWNode), isSlide =
      (comp != null && comp instanceof LWSlide), hasResource =
      isNode && comp.hasResource();
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() instanceof JMenuItem) {
      ...
      if (tp == null) {
        resNode = (ResourceNode) model.getRoot();
      } else {
        resNode = (ResourceNode) tp.getLastPathComponent();
      }
      if (
        e.getActionCommand().toString().equals(REMOVE_RESOURCE) |
        e.getActionCommand().toString().equals(REMOVE_FAVORITES)
      ) {
        if (!resNode.isRoot()) {
          if (
            VueUtil.confirm(this, CONFIRM_DEL_RESOURCE, TITLE_DEL_RESOURCE) ==
            JOptionPane.OK_OPTION
          ) {
            TreePath parent = tp.getParentPath();
            model.removeNodeFromParent(resNode);
            if (parent.getPathCount() != 1) favoritesTree.setSelectionPath(
              parent
            ); else favoritesTree.setSelectionRow(DEFAULT_SELECTION_ROW);
          }
        }
      } else if (e.getActionCommand().toString().equals(ADD_TO_MAP)) {
        String label = Util.formatLines(
          resNode.getResource().getName(),
        ...
        ); else ResourceList.setXYByClustering(end);
      } else if (
        e.getActionCommand().toString().equals(ADD_TO_NODE) ||
        ...
        if (resNode instanceof FileNode) {
          ((FileNode) resNode).displayContent();
        } else {
  ...
  public static void marshallFavorites(File file, SaveVueJTree favoritesTree) {
    ...
    try {
      ...
      marshaller.setMarshalListener(
        new tufts.vue.action.ActionUtil.VueMarshalListener()
      );
  ...
  public void performSearch() {
    ...
    if (!searchString.equals("")) {
      ...
      DefaultTreeModel serTreeModel =
        (DefaultTreeModel) serResultTree.getModel();
  ...
  public boolean compareNode(
    String searchString,
    ResourceNode Node,
    ResourceNode serRoot,
  ...
  class PopupListener extends MouseAdapter {
    ...
    public void mouseClicked(MouseEvent e) {
      if (vtree.getSelectionPath() != null) {
        DefaultMutableTreeNode selTreeNode = (DefaultMutableTreeNode) vtree
          .getSelectionPath()
          .getLastPathComponent();
        DefaultMutableTreeNode treeNode = (DefaultMutableTreeNode) vtree
        ...
        if (e.getClickCount() == 2) {
          if (
            ((DefaultMutableTreeNode) vmodel.getRoot()).getChildCount() == 1
          ) vtree.clearSelection();
          ...
          if ((selTreeNode instanceof ResourceNode)) {
            ...
            if (DEBUG.Enabled) System.out.println(
              "instance of favorites node- res class" + tufts.Util.tags(uo)
            );

            if (uo instanceof Resource) ((Resource) uo).displayContent();
          }
        }
      }
      showPopup(e);
    ...
    private void showPopup(MouseEvent e) {
      if (e.isPopupTrigger()) {
        if (vtree.getSelectionPath() != null) {
          if (
            vtree.getSelectionPath().getLastPathComponent() instanceof
            FavoritesNode
          ) {
  ...
  public static void main(String args[]) {
    ...
    FavoritesWindow fw = new FavoritesWindow(
      "http://www.tufts.edu",
      "favorites.xml"
    );
    fw.setVisible(true);
