...
public class LWCEvent {
    ...
    if (component == null && components != null && components.size() > 0) {
      if (DEBUG.Enabled && components.size() > 1) {
        tufts.Util.printStackTrace(
          this +
          "RETURNING FIRST ONLY WHEN IT CONTAINS " +
          components.size() +
          "\n" +
          components +
          "\n"
  ...
  public LWComponent onlyComponent() {
    if (
      component == null && components != null
    ) return null; else return component;
  }
  ...
  public String getName() {
    if (this.key instanceof LWComponent.Key) return (
      (LWComponent.Key) key
    ).name; else return (String) this.key;
  }

  /** If the key is a proper Key, return it, otherwise, return null */
  public LWComponent.Key getKey() {
    if (
      this.key instanceof LWComponent.Key
    ) return (LWComponent.Key) key; else return null;
  }
  ...
  public Object getOldValue() {
    if (oldValue instanceof Undoable) return (
      (Undoable) oldValue
    ).old; else return this.oldValue;
  }
  ...
  public String paramString() {
    ...
    if (oldValue != null && oldValue != NO_OLD_VALUE) buf.append(
      " (" + oldValue + ")"
    );
