...
public class CabSyncViewer extends JFrame {
  ...
  private static final Cursor dragCursor = Cursor.getPredefinedCursor(
    Cursor.HAND_CURSOR
  );
  ...
  class DragTable extends JTable {
    ...
    protected void processMouseEvent(MouseEvent e) {
      ...
      if (id == MouseEvent.MOUSE_ENTERED) {
        ...
        // zone, start dragging...
        if (
          r >= getSelectionModel().getMinSelectionIndex() &&
          r <= getSelectionModel().getMaxSelectionIndex()
        ) startDragging(); else super.processMouseEvent(e);
  ...
  class SyncListTreeNode extends DefaultMutableTreeNode {
    ...
    SyncListTableModel getDataModel() {
      if (this.dataModel == null) {
        if (log.d) log.debug(
          this,
          "getDataModel: creating SyncListTableModel for " + getSyncList()
        );
    ...
    SyncListTableModel getRemoteDataModel() {
      if (this.remoteDataModel == null) {
        if (log.d) log.debug(
          this,
          "getRemoteDataModel: creating SyncListTableModel for " +
          getRemoteSyncList()
        );
  ...
  static class BooleanRenderer extends JCheckBox implements TableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
      ...
      if (value != null) {
        if (value instanceof Boolean) setSelected(
          ((Boolean) value).booleanValue()
        ); else log.errout(
          "NOT BOOLEAN: " + value.getClass().getName() + ": " + value
        );
      }
      return this;
    }
  ...
  static final int COL_MODTIME = 2;
  static final int COL_STATUS = 3;
  static final int COL_DOSYNC = 4;
  static final int COL_OWNER = 5;
  ...
  class SyncEntryRenderer extends DefaultTableCellRenderer {
    ...
    public SyncEntryRenderer(int index) {
      col = index;
      if (col == COL_MODTIME || col == COL_SIZE) setHorizontalAlignment(
    ...
    public Component getTableCellRendererComponent(
      ...
      if (value == null) return super.getTableCellRendererComponent(
        table,
        "<null>",
        isSelected,
        hasFocus,
      ...
      if (isGhost) setForeground(ghostColor); else setForeground(defaultColor);

      try {
        if (col == COL_NAME) {
          String label = se.getName();
          if (se.isCabinet()) label += "/";
          if (!isGhost) {
          ...
          //if (isGhost) value = ""; else
          if (isBS) value = new Long(bs.length()); else value = "";
        } else if (col == COL_STATUS) value = " " + se.getStatusDescription();
  ...
  class SyncListTableModel
    ...
    public void stateChanged(ChangeEvent e) {
      ...
      if (e instanceof SyncListChangeEvent) {
        ...
        if (se.changes > 0) refreshModel(); else fireTableDataChanged(); // se.updates>0
        if (
          se.tosync > 0 && !syncList.isSyncRunning() && syncList.isAutoSync()
        ) {
    ...
    public SyncListTCModel getTableColumnModel() {
      if (columnModel == null) columnModel = new SyncListTCModel(true);
      return columnModel;
    }
    ...
    public boolean isCellEditable(int rIndex, int cIndex) {
      if (cIndex == COL_DOSYNC) {
        ...
        if (se.isGhost()) return syncList.isImportAll() == false;
        return true;
      }
      return false;
    }
    ...
    public void setValueAt(Object aValue, int rIndex, int cIndex) {
      ...
      SyncEntry se = getEntryAt(rIndex);
      if (se == null) log.errout(
        "setValueAt, no SyncEntry at row " + rIndex
      ); else syncList.setSyncRequested(se, ((Boolean) aValue).booleanValue());
    }
    ...
    private Object getColumnData(SyncEntry se, int col) throws FilingException {
      Object data = null;

      try {
        if (col == COL_DOSYNC) {
          if (syncList.isImportAll()) data = Boolean.TRUE; else data =
            new Boolean(se.isSyncRequested());
        } else data = se;
      } catch (Exception e) {
        log.errout(e);
        data = "<" + e.getMessage() + ">";
    ...
    public void refreshModel(SyncList sl) {
      ...
      while (i.hasNext()) {
        SyncEntry se = (SyncEntry) i.next();
        if (showGhosts || !se.isGhost()) entries.add(se);
      }
      setSyncList(sl);
    ...
    private void setSyncList(SyncList sl) {
      if (syncList != sl) {
        if (syncList != null) syncList.removeChangeListener(this);
        syncList = sl;
  ...
  public CabSyncViewer(SyncList syncList) {
    ...
    MouseInputAdapter mouseListener = new MouseInputAdapter() {
      ...
      public void mousePressed(MouseEvent e) {
        ...
        // name of the cabinet)
        if (c != remoteTable) remoteTable.clearSelection();
        if (c != localTable) localTable.clearSelection();
      ...
      public void mouseEntered(MouseEvent e) {
        if (e.getComponent() == leftView) localTable.containsMouse =
          true; else if (
          e.getComponent() == rightView
  ...
  private String abbrevName(String s) {
    ...
    if (base == null) return s; else return base + s;
  }
    ...
    jtree.addTreeSelectionListener(
      new TreeSelectionListener() {
        public void valueChanged(TreeSelectionEvent e) {
          SyncListTreeNode syncNode =
            (SyncListTreeNode) jtree.getLastSelectedPathComponent();
    ...
    jtree.addTreeWillExpandListener(
      new TreeWillExpandListener() {
        public void treeWillExpand(TreeExpansionEvent e) {
          ...
          SyncListTreeNode syncNode =
            (SyncListTreeNode) path.getLastPathComponent();
          if (syncNode == null) return;
  ...
  private static Color getStatusColor(SyncEntry se) {
    switch (se.getStatus()) {
      case SyncList.STATUS_NEW_REMOTE:
        return ghostColor;
      case SyncList.STATUS_CHANGED_REMOTE:
        return Color.red;
      case SyncList.STATUS_NEW_LOCAL:
      case SyncList.STATUS_CHANGED_LOCAL:
        return darkGreen;
      case SyncList.STATUS_CONFLICT:
        return Color.orange;
    }
    return defaultColor;
  }
  ...
  private Font startupMessageFont = new Font(
    "serif",
  ...
  class CSAction extends AbstractAction {
    ...
    private boolean actionMatch(ActionEvent ae, CSAction a) {
      if (a != null) return ae.getActionCommand().equals(a.getActionCommand());
      return false;
    }

    public void actionPerformed(ActionEvent ae) {
      if (log.d) log.debug(
        ae.getActionCommand() + " ActionEvent=" + ae.getSource()
      );
      // handle the menu items
      if (actionMatch(ae, aCopy)) doCopy(); else if (
        actionMatch(ae, aPaste)
      ) doPaste(); else if (actionMatch(ae, aClear)) doClear(); else if (
        actionMatch(ae, aSelectAll)
      ) doSelectAll(); else if (
        actionMatch(ae, aAutoSync)
      ) syncList.setAutoSync(tbAutoSync.isSelected()); else if (
        actionMatch(ae, aSyncSelected)
      ) doSyncSelected(); else if (
        actionMatch(ae, aMarkSync)
      ) doMarkSelectionForSync(Boolean.TRUE); else if (
        actionMatch(ae, aUnmarkSync)
      ) doMarkSelectionForSync(Boolean.FALSE);
      // handle the tool-bar buttons
      else if (actionMatch(ae, aRefresh)) {
        if (ae.getSource() == tbRefresh) syncList.refresh(
          ae.getModifiers() != 0
        ); //modifiers is always 0 no matter what keys we hold!
        else tbRefresh.doClick();
      } else if (actionMatch(ae, aUploadOnly)) doAsyncSync(
        SyncList.SYNC_UPLOADS_ONLY
      ); else if (actionMatch(ae, aDownloadOnly)) doAsyncSync(
        SyncList.SYNC_DOWNLOADS_ONLY
      ); else if (actionMatch(ae, aSync)) doAsyncSync(
        SyncList.SYNC_ALL
      ); else if (actionMatch(ae, aCancel)) doCancel(); else if (
        actionMatch(ae, aQuit)
      ) doQuit(); else if (ae.getSource() == leftImportAll) {
  ...
  public void setPollInterval(int seconds) {
    ...
    if (syncList.getPollInterval() <= 0) aAutoSync.setEnabled(
      false
    ); else aAutoSync.setEnabled(true);
  }
  ...
  public void addEditMenuItems() {
    ...
    miPaste.setAccelerator(
      KeyStroke.getKeyStroke(KeyEvent.VK_V, Event.META_MASK)
    );
    editMenu.add(miPaste).setEnabled(false);

    miClear = new JMenuItem(aClear);
    editMenu.add(miClear);
    miClear.setAccelerator(
      KeyStroke.getKeyStroke(KeyEvent.VK_D, Event.META_MASK)
    );
  ...
  private Runnable syncThreadCode = new Runnable() {
    public void run() {
      ...
      if (syncException != null) {
        ...
        } else if (e instanceof SyncException) JOptionPane.showMessageDialog(
          null,
  ...
  public synchronized void doAutoSync() {
    if (syncList.isSyncRunning()) return;
  ...
  protected void syncSelected(JTable table, boolean exportsOnly) {
    ...
    for (int i = 0; i < rows.length; i++) {
      ...
      if (exportsOnly) {
         ...
         */
        if (
          !(se.getStatus() == SyncList.STATUS_NEW_LOCAL ||
      ...
      if (remoteSource) localEntry = syncList.addRemote(se); else localEntry =
        se;

      int changes = 0;
      try {
        if (syncList.doSynchronizeEntry(localEntry)) changes++;
      } catch (FilingException e) {
        ...
        if (changes > 0) syncList.saveProperties();
      }
    }
  }
  ...
  public void doMarkSelectionForSync(Boolean b) {
    JTable table = getFocusedTable();
    if (table != localTable || !leftTCModel.isSyncShowing()) return;
    TableModel tm = table.getModel();
  ...
  protected JTable getFocusedTable() {
    ...
    if (fo != null) {
      ...
      if (fo == remoteTable) return remoteTable;
    }
    return localTable;
  }
  ...
  protected int[] getSelectedRows() {
    ...
    if (log.d) {
      ...
      for (int i = 0; i < rows.length; i++) log.debug("r" + rows[i]);
    }
    return rows;
  }
  ...
  public static final String USAGE =
    "Usage: CabSyncViewer <local-cabinet> <remote-cabinet> [-poll seconds] [-show_ghosts]";
  ...
  public static void main(String args[]) {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (a.equals("-poll")) {
        ...
        System.exit(0);
      } else if (a.equals("-show_ghosts")) A_SHOW_GHOSTS = true;
      //            else if (a.equals("-always_sync_all"))
      //todo
      else if (!a.startsWith("-")) dirs.add(a);
    }
