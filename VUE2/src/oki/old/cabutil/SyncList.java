...
public class SyncList implements Runnable {
  ...
  public SyncList(
    ...
    if (log.d) log.debug(
      "new SyncList local=" + localCab + ", remote=" + remoteCab
    );
  ...
  public void run() {
    ...
    while (polling) {
      ...
      if (!syncRunning) {
        ...
        if (elapsed >= sleepInterval) doAutoRefresh(); else sleepInterval -=
        elapsed;
      }
    }
  ...
  synchronized void doAutoRefresh() {
    if (syncRunning || refreshRunning) return;
  ...
  private void setEntryDoSkip(SyncEntry se, boolean doSkip) {
    ...
    if (doSkip) props.remove(key); else props.setProperty(key, "t");
  }
  ...
  private void readProperties() {
    if (!isLocalList) throw new RuntimeException(
      "SyncList: remote list attempted properties access"
    );

    this.propsBS = getPropsByteStore();
    if (propsBS == null) log.errout(
      "coudn't find a properties store in: " + this
    ); else {
      try {
        InputStream in = new JavaInputStreamAdapter(
          propsBS.getOkiInputStream()
        );
        this.props.load(in);
        in.close();
  ...
  public void saveProperties() {
    ...
    try {
      OutputStream out = new JavaOutputStreamAdapter(
        propsBS.getOkiOutputStream()
      );
  ...
  public String getLocalCabinetName() {
    if (localCabinet == null) return "<lc>"; else return getCabinetName(
      localCabinet
    );
  }

  public String getRemoteCabinetName() {
    if (remoteCabinet == null) return "<rc>"; else return getCabinetName(
      remoteCabinet
    );
  }
  ...
  static String getCabinetName(Cabinet cab) {
    ...
    try {
      ...
      if (owner != null) {
        ...
        if (ra != null) s += ra.getHost() + ":" + ra.getPort() + "/";
      }
  ...
  public void setSyncRequested(SyncEntry rse, boolean doRequest) {
    ...
    if (se.isSyncRequested() != doRequest) {
      ...
      if (se.isGhost() && doRequest == false) // will end up removing the ghost
      refresh();
    }
  }
  ...
  public synchronized SyncEntry addRemote(SyncEntry remoteEntry) {
    if (!isLocalList) throw new RuntimeException("addRemote on remote list");
    ...
    if (updates > 0 || changes > 0) notifyChangeListeners(
      new SyncListChangeEvent(localMatch, updates, changes)
    );
  ...
  public void refresh(boolean force) {
    ...
    try {
      refreshRunning = true;
      if (isLocalList) doRefresh(force); else getRemoteList().doRefresh(force);
    } finally {
      refreshRunning = false;
    }
  }
  ...
  private synchronized void doRefresh(boolean force) {
    ...
    if (isLocalList) readProperties(); // just in case somebody else has been syncing here...
    ...
    while (i.hasNext()) {
      ...
      if (!locked) {
        if (remoteMatch == null) {
          ...
          if (importAll) {
            ...
            remoteChanges++;
          } else if (log.d) log.debug("no remote ghost for " + localEntry);
        }
      ...
      if (status != STATUS_CURRENT && status != STATUS_UNKNOWN) tosync++;
      ...
      if (
        remoteMatch != null && remoteMatch.setStatus(opposingStatus[status])
      ) {
        updates++;
    ...
    if (importAll) {
      ...
      while (i.hasNext()) {
        ...
        if (remoteEntry.isGhost()) continue;
        ...
        if (localMatch == null) {
          ...
          changes++;
          if (remoteEntry.setStatus(STATUS_NEW_LOCAL)) updates++;
        }
      }
    }
    ...
    if (
      ...
      if (log.d) log.debug(
        ...
        " updates, " +
        tosync +
        " tosync, notifying listeners"
      );
      notifyChangeListeners(
        new SyncListChangeEvent(this, updates, changes, tosync)
      );
      remoteList.notifyChangeListeners(
        new SyncListChangeEvent(this, remoteUpdates, remoteChanges, tosync)
      );
    }
  ...
  private synchronized SyncListChangeEvent rescan() {
    ...
    while (i.hasNext()) {
      ...
      } else if (log.d) log.debug(" exists " + se);
    }
    ...
    while (i.hasNext()) {
      ...
      if (
        ceName.equals(PropsFileName) || ceName.endsWith(IncomingFileExtension)
      ...
      if (ic == '~' || ic == '.' || ceName.endsWith("~")) continue;
      ...
      if (se != null) {
        ...
        if (se.isGhost()) {
           ...
           */
          if (visibleGhosts) slev.updates++; else slev.changes++;
          //
          if (log.d) log.debug("GHOST FILL " + se);
          se.setCabinetEntry(ce);
          se.setIsGhost(false);
          se.setStatus(STATUS_UNKNOWN);
        } else if (log.d) log.debug("confirm " + se);
      } else {
        /*
        ...
        if (this.isLocalList) {
          ...
          if (dateStr != null) //se.setLastSyncTime(Date.parse(dateStr));
          try {
        ...
        if (se.isCabinet()) if (log.d) log.debug("TODO: new SyncList" + se);
  ...
  protected int computeStatus(SyncEntry localEntry, SyncEntry remoteEntry) {
    ...
    try {
      if (localEntry.isByteStore() && remoteEntry.isByteStore()) {
        ...
        if (remoteChange && localChange) return STATUS_CONFLICT; else if (
          localChange
        ) return STATUS_CHANGED_LOCAL; else if (
          remoteChange
        ) return STATUS_CHANGED_REMOTE; else {
          long ll = localByteStore.length();
          long rl = remoteByteStore.length();
          if (ll != rl) {
            log.errout(
              "*SYNC ANOMALY: apparently current, yet sizes different: " +
              ll +
              " != " +
              rl
            );
  ...
  public void notifyChangeListeners(Object src) {
    ...
    while (i.hasNext()) {
      ...
      ChangeEvent event;
      if (src instanceof ChangeEvent) event = (ChangeEvent) src; else event =
        new ChangeEvent(src);
  ...
  public int doSynchronize() throws FilingException {
    return doSynchronize(SYNC_ALL);
  }

  public synchronized int doSynchronize(int syncType) throws FilingException {
    int changes = 0;
    syncRunning = true;
    try {
  ...
  private synchronized int doSyncType(int syncType) throws FilingException {
    if (!isLocalList) throw new RuntimeException(
      "SyncList: remote list attempted sync"
    );
    ...
    while (i.hasNext()) {
      ...
      if (!importAll && !localEntry.isSyncRequested()) continue;

      int s = localEntry.getStatus();
      if (syncType == SYNC_UPLOADS_ONLY) {
        if (s == STATUS_NEW_REMOTE || s == STATUS_CHANGED_REMOTE) continue;
      } else if (syncType == SYNC_DOWNLOADS_ONLY) {
        if (s == STATUS_NEW_LOCAL || s == STATUS_CHANGED_LOCAL) continue;
      } else if (s == STATUS_CURRENT) continue;

      if (doSynchronizeEntry(localEntry)) updates++;
    }
    /*
     * if any locks were obtained, release them.
     */
  ...
  boolean obtainCabinetLock(Cabinet c) {
    ...
    try {
      OutputStream os = new JavaOutputStreamAdapter(
        lockBS.getOkiOutputStream()
      );
  ...
  public boolean doSynchronizeEntry(SyncEntry localEntry)
    ...
    switch (syncStatus) {
      ...
      case STATUS_CHANGED_REMOTE:
        ...
        //            try {
        CabinetEntry remoteCE = getRemoteCabinet()
          .getCabinetEntry(localEntry.getName());
    ...
    if (change) {
      ...
      if (visibleGhosts) {
        ...
        notifyChangeListeners(
          new SyncListChangeEvent(localEntry, 1, wasLocalGhost ? 1 : 0)
        );
        remoteList.notifyChangeListeners(
          new SyncListChangeEvent(remoteEntry, 1, wasRemoteGhost ? 1 : 0)
        );
      }
    }
  ...
  /**
     ...
     */
  private CabinetEntry copyCabinetEntry(
    Cabinet destCabinet,
    CabinetEntry sourceEntry,
    String actionName,
    String loc
  ) throws FilingException {
    if (!sourceEntry.canRead()) throw new FilingException(
      "copyCabinetEntry: can't read source " + sourceEntry
    );

    CabinetEntry newCabinetEntry = null;

    // Lfs filing BUG: this test doesn't seem to be working...
    if (sourceEntry.getParent().equals(destCabinet)) log.errout(
      "ack! incestous copy! cab=" + destCabinet + ", entry=" + sourceEntry
    );
    ...
    if (sourceEntry.isByteStore()) {
      ...
      if (takeAction) {
        ...
        if (!obtainCabinetLock(destCabinet)) {
          throw new LockException(
            readLock(destCabinet),
            "cannont copy '" +
            sourceEntry.getName() +
            "' to " +
            destCabinet.getPath() +
            ": destination cabinet is locked"
          );
        }
        ...
        if (srcLen != destLen) {
          throw new FilingIOException(
            "copy failed: sizes differ:" +
            "\n\t" +
            sourceBS +
            "=" +
            srcLen +
        ...
        if (!newCabinetEntry.rename(sourceName)) throw new FilingException(
          "rename of " + newCabinetEntry + " to " + sourceName + " failed"
        );
      }
      ...
      if (takeAction) newCabinetEntry =
        destCabinet.createCabinet(sourceEntry.getName());
      // TODO: Need to recurse... somewhere.
    } else {
      log.errout(
        new Exception("copyCabinetEntry: neither ByteStore or Cabinet!")
      );
    }
  ...
  private void actionOut(String s) {
    if (log.v) log.outln((takeAction ? "" : "(Skipped) ") + s);
  }
  //-----------------------------------------------------------------------------
  // END OF SyncList
  //-----------------------------------------------------------------------------
