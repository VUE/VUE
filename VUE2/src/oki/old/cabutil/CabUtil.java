...
public class CabUtil {
  ...
  public static final CapabilityType FSReadWrite = new CapabilityType(
    "MIT",
    "readwrite"
  );
  public static final CapabilityType FSMetaCaching = new CapabilityType(
    "MIT",
    "metaDataCache"
  );
  public static final CapabilityType FSLocalAccess = new CapabilityType(
    "MIT",
    "localAccess"
  );
  public static final CapabilityType FSRemoteAccess = new CapabilityType(
    "MIT",
    "remoteAccess"
  );
  ...
  public static CabinetEntry getCabinetEntry(String path, boolean isCabinet) {
    if (path.indexOf(':') >= 0) {
      ...
      } else return getRfsCabinetEntryFromURL(url, isCabinet);
    } else {
   ...
   */
  public static CabinetEntry getEntryFromJavaFile(
    File file,
    boolean isCabinet
  ) {
    ...
    if (lfsFactory == null) lfsFactory = getLfsCabinetFactory();
    ...
    try {
      ...
      if (isCabinet) entry = lfsFactory.getCabinet(id); else entry =
        lfsFactory.getByteStore(id);
    } catch (Exception e) {
      errout(e);
      return null;
    }
    ...
    if (verbose) outln(
      "Got local file '" + file + "' as CabinetEntry " + entry
    );
  ...
  public static Cabinet getRfsCabinetFromDirectory(String path) {
    ...
    try {
      cabinet =
        rfsFactory.getCabinet(rfsFactory.idFromString(directory.getPath()));
    } catch (Exception e) {
      errout(e);
      return null;
    }
    ...
    if (verbose) outln(
      "Got remote dir [" + directory + "] thru cabinet [" + cabinet + "]"
    );
  ...
  public static CabinetFactory getLfsCabinetFactory() {
    ...
    try {
       ...
       */
      if (NEW_FILESYSTEMS) lfsFactory =
        fileFactory.getCabinetFactoryByType(
          new org.okip.service.filing.impl.rfs.RfsType()
        ); else lfsFactory =
        fileFactory.getCabinetFactoryByType(
          new org.okip.service.filing.impl.localfilesystem.LfsType()
        );
      if (false && verbose) { // listRoots debug
        ...
        for (int i = 0; i < roots.length; i++) System.err.println(
          "root cabinet #" + i + ": " + roots[i]
        );
      }
      return lfsFactory;
    } catch (Exception e) {
  ...
  private static Factory getFilingServiceFactory() {
    ...
    try {
      fileFactory =
        (Factory) FactoryManager.getDefaultFactory(
          "org.okip.service.filing.api",
          getUserAgent()
        );
    } catch (Exception e) {
      errout(e);
  ...
  public static CabinetFactory getRfsCabinetFactory(Agent agent) {
    ...
    try {
      ...
      if (USE_CACHING_FILESYSTEM) props.put(
        FSMetaCaching,
        Boolean.TRUE
      ); else props.put(FSMetaCaching, Boolean.FALSE);
      ...
      if (matches.length == 0 && USE_CACHING_FILESYSTEM) {
        if (verbose) outln(
          "couldn't find factory that supports " + FSMetaCaching
        );
      ...
      if (verbose) outln(
        "got RfsFactory [" + rfsFactory + "] that matched properties " + props
      );
  ...
  private static void ensureSecurityManager() {
    ...
    if (sysProps.get("java.security.policy") == null) {
      ...
      if (verbose) outln(
        "defaulted: java.security.policy=" +
        sysProps.get("java.security.policy")
      );
    }
  ...
  public static CabinetEntry getRfsCabinetEntryFromURL(
    URL url,
    boolean cabinet
  ) {
    if (!securityManagerSet) ensureSecurityManager();

    CabinetFactory rfsFactory = getRfsCabinetFactory(new RemoteAgent(url));

    outln(
      "Fetching RFS cabinet from URL: [" +
      url +
      "] thru " +
      rfsFactory.getClass()
    );
    ...
    try {
      if (cabinet) entry =
        rfsFactory.getCabinet(
          rfsFactory.idFromString(url.getPath())
        ); else entry =
        rfsFactory.getByteStore(rfsFactory.idFromString(url.getPath()));
    } catch (Exception e) {
      errout(e);
      return null;
    }
  ...
  private static void getAllCabinetFactories() {
    ...
    try {
      ...
      Factory fileFactory = (Factory) FactoryManager.getDefaultFactory(
        "org.okip.service.filing.api",
        null
      );

      List cfs = fileFactory.getAvailableCabinetFactories(
        (org.okip.service.shared.api.Agent) null
      );
  ...
  private static CabinetFactory getCabinetFactoryByProps() {
    try {
      ...
      desiredProperties.put(
        new CapabilityType("MIT", "readwrite"),
        new Boolean(true)
      );
      CabinetFactory cabinetFactories[] = fileFactory.getCabinetFactory(
        desiredProperties
      );

      outln(
        "got " +
        cabinetFactories.length +
        " cabinetFactory matching properties " +
        desiredProperties
      );
      if (cabinetFactories.length <= 0) {
        errout(
          "unable to obtain CabinetFactory with properties [" +
          desiredProperties +
          "]"
        );
        return null;
      ...
      outln(
        "returning cabinetFactory " +
        cabinetFactory +
        " PROPS=" +
        cabinetFactory.getProperties()
      );
  ...
  public static void printCabinet(Cabinet cabinet) {
    ...
    try {
      ...
      while (i.hasNext()) printCabinetEntry((CabinetEntry) i.next());
    } catch (FilingException e) {
      System.err.println(e);
    }
  }

  public static void printCabinetEntry(CabinetEntry ce) throws FilingException {
  ...
  public static void printCabinetEntry(CabinetEntry ce, PrintStream ps)
    ...
    char type = '?';
    if (ce.isCabinet()) type = 'd'; else if (ce.isByteStore()) type = '-';
    ps.print(type);
    ...
    if (ce.isByteStore()) {
      try {
        ...
        long len = bs.length();
        for (int i = 100000; i > 1; i /= 10) if (len < i) ps.print(' ');
        ps.print(" " + len + " bytes");
  ...
  public static ArrayList parseCommandLine(String args[]) {
    ...
    if (
      System.getProperty("CABUTIL_OLDFS", "false").equalsIgnoreCase("true")
    ) USE_CACHING_FILESYSTEM = NEW_FILESYSTEMS = false;

    ArrayList words = new ArrayList();
    for (int i = 0; i < args.length; i++) {
      String a = args[i];
      if (a.equals("-laf_java")) laf =
        javax.swing.UIManager.getCrossPlatformLookAndFeelClassName(); else if (
        a.equals("-laf_windows")
      ) laf = "com.sun.java.swing.plaf.windows.WindowsLookAndFeel"; else if (
        a.equals("-verbose")
      ) verbose = true; else if (a.equals("-newfs")) {
        USE_CACHING_FILESYSTEM = true;
        NEW_FILESYSTEMS = true;
      } else if (a.equals("-oldfs")) {
        USE_CACHING_FILESYSTEM = false;
        NEW_FILESYSTEMS = false;
      } else if (a.equals("-debug")) verbose = true; else if (
        a.charAt(0) != '-'
      ) words.add(a);
    }
    if (NEW_FILESYSTEMS) errout(
      "Using the new filesystem implementation."
    ); else errout("Using OLD filesystem implementations.");
