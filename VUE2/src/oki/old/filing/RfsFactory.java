...
public class RfsFactory
  extends org.okip.service.shared.api.PRBReader
  ...
  private static transient int RFS_IO_BUFFER_SIZE = 16384; // io buffer size in bytes
  private static transient int RFS_MAX_CACHE_AGE = 7 * 1000; // maximum allowable age of cache data in milliseconds
  private static transient String ROMI_API = "org.okip.util.romi.client.api";
  private static transient String ROMI_IMPL = "edu.mit.is.util.romi.client";
  private static transient String ROMI_CLIENT_HOST = "oki1.mit.edu";
  private static transient int ROMI_CLIENT_PORT = 1898;
  private static transient String ROMI_SERVER_NAME =
    "edu.mit.is.util.romi.server.Server";
  private static transient boolean trace = false;
  ...
  private static transient boolean propertiesRead = false;
  ...
  private void readProperties() {
    ...
    if ((value = getClassProperty("ROMI_API")) != null) ROMI_API = value;
    if ((value = getClassProperty("ROMI_IMPL")) != null) ROMI_IMPL = value;
    if (
      (value = getClassProperty("ROMI_CLIENT_HOST")) != null
    ) ROMI_CLIENT_HOST = value;
    if (
      (intValue = getClassInteger("ROMI_CLIENT_PORT")) >= 0
    ) ROMI_CLIENT_PORT = intValue;
    if (
      (value = getClassProperty("ROMI_CLIENT_SERVER")) != null
    ) ROMI_SERVER_NAME = value;
    if (
      (intValue = getClassInteger("RFS_MAX_CACHE_AGE")) >= 0
    ) RFS_MAX_CACHE_AGE = intValue;
    if (
      (intValue = getClassInteger("RFS_IO_BUFFER_SIZE")) > 0
  ...
  public void setOwner(org.okip.service.shared.api.Agent owner) {
    ...
    if (owner != null) {
      ...
      this.remoteAgent =
        (RemoteAgent) getOwner()
          .getProxy(org.okip.util.agents.RemoteAgent.class);
    }
  }
   ...
   *
   */
  public Cabinet[] listRoots() throws FilingException {
    if (this.getClient() != null) return (Cabinet[]) this.invoke(
        this,
        "listRoots"
      ); else return listRootsLocally();
  }

  protected Cabinet[] listRootsLocally() throws FilingException {
   ...
   */
  public ID idFromString(String id) throws FilingException {
    return new RfsID(id);
  }
  ...
  public Cabinet getCabinet(ID id) throws FilingException {
    RfsCabinet existingCabinet = (RfsCabinet) cabinets.get(id.toString());
    if (existingCabinet != null) return existingCabinet; else {
   ...
   */
  public ByteStore getByteStore(ID id) throws FilingException {
   ...
   *
   */
  public Cabinet createRootCabinet() throws FilingException {
    try {
   ...
   * the ByteStore.
   */
  public Cabinet getCabinetFromJar(ByteStore jar) throws FilingException {
  ...
  private static final transient java.util.Map properties =
  ...
  public org.okip.util.romi.client.api.Client getClient()
    ...
    if (client == null && remoteAgent != null) {
      try {
        setClient(
          createClient(remoteAgent.getHost(), remoteAgent.getPort(), getOwner())
        );
      } catch (org.okip.service.shared.api.Exception e) {
        throw new FilingException(
          e,
          "Failed to create client for RemoteAgent " + remoteAgent
        );
      }
    }
    return client;
   ...
   */
  public org.okip.util.romi.client.api.Client createClient(
    String host,
    int port,
    Agent agent
    ...
    clientFactory =
      (org.okip.util.romi.client.api.Factory) org.okip.service.shared.api.FactoryManager.getFactory(
        ROMI_API,
        ROMI_IMPL,
        agent
      );

    if (host == null || host.length() == 0) host = ROMI_CLIENT_HOST;
    if (port < 0) port = ROMI_CLIENT_PORT;
    try {
      return clientFactory.createClient(host, port, ROMI_SERVER_NAME);
    } catch (org.okip.service.shared.api.Exception e) {
      throw new FilingException(
        e,
        "failed to create client: host=" +
        host +
        " port=" +
        port +
        " server=" +
  ...
  public char getSeparatorChar() throws FilingException {
    if (separatorChar != 0) return separatorChar;
    if (this.getClient() != null) {
      try {
        return (
          separatorChar =
            ((Character) invoke(this, "getSeparatorChar")).charValue()
        );
      } catch (Exception e) {
        throw new FilingException(e);
      }
    } else return java.io.File.separatorChar;
  }

  private static String argparse(Object o) {
    String s;
    if (o == null) s = "null"; else if (o instanceof byte[]) s =
      "byte[" + ((byte[]) o).length + "]"; else if (o instanceof RfsEntry) s =
      ((RfsEntry) o).getNameInternal(); else if (o instanceof String) s =
      "\"" + o + "\""; else s = o.toString();
    return s;
  }
  ...
  protected Object invoke(
    ...
    try {
      if (trace) {
        String msg =
          "INVOKE " + object.getClass().getName() + "::" + methodName + "(";
        for (int i = 0; i < args.length; i++) {
          if (i > 0) msg += ", ";
          msg += argparse(args[i]);
        }
        msg += ")";
        if (object instanceof RfsEntry) msg +=
        " <" + ((RfsEntry) object).getNameInternal() + ">"; else msg +=
        " <" + object + ">";
       ...
       */
      Object returnValue =
        this.getClient().invoke(object, methodName, argTypes, args);
      ...
      if (ce.getTarget() instanceof FilingException) {
        ...
        throw new FilingException(
          ce.getTarget(),
          "unexpected remote exception: " + ce.getTarget()
        );
      }
