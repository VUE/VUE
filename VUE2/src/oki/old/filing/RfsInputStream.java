...
public class RfsInputStream implements OkiInputStream {
  ...
  protected RfsInputStream(RfsByteStore rfsEntry) throws FilingException {
    ...
    if (this.idStr == null) throw new FilingException("null filename");
  }

  private java.io.FileInputStream getInputStream() throws FilingException {
    if (!localPassthru) throw new FilingException(
      "RfsInputStream internal error: attempt to get local stream remotely"
    );

    try {
      if (this.inStream == null) this.inStream =
        new java.io.FileInputStream(rfsEntry.idStr);
  ...
  private DataBlock getBuffer() throws FilingException {
    if (curBuffer != null && bufPos < curBuffer.length) return curBuffer;
    ...
    if (inBuf == null) inBuf =
      new RfsInputBuffer(
        ...
        fileStart,
        bufferSize
      ); else inBuf.setStartPosition(fileStart);

    if (filePos < fileStart) throw new FilingException("filePos<fileStart");

    bufPos = (int) (filePos - fileStart);
    curBuffer = inBuf.readBuffer();

    if (curBuffer == null || curBuffer.containsEOF) noMoreBuffers = true;

    if (
      bufPos < 0 || (curBuffer != null && bufPos >= curBuffer.length)
    ) throw new FilingException(
      "bad bufPos=" + bufPos + ">=" + curBuffer.length
    );
  ...
  protected DataBlock readMax() throws FilingException {
    if (localPassthru) {
      try {
        ...
        if (got <= 0) nullbuffer(); else curBuffer.length = got;
      } catch (java.io.IOException e) {
        throw new FilingException(e);
      }
    } else {
      if (getBuffer() == null) return null;
      if (bufPos != 0) throw new FilingException(
        "readMax must not follow other read calls (bp=" + bufPos + ")"
      );
   ...
   */
  public int read(byte[] b, int off, int max) throws FilingException {
    if (localPassthru) {
      try {
   ...
   */
  public int read(byte[] b) throws FilingException {
   ...
   */
  public int read() throws FilingException {
    if (localPassthru) {
      try {
  ...
  public long skip(long n) throws FilingException {
    if (localPassthru) {
      try {
   ...
   */
  public long available() throws FilingException {
    if (localPassthru) {
      try {
    ...
    if (curBuffer == null) return 0; else return curBuffer.length - bufPos;
  }
  ...
  public java.io.InputStream getNativeInputStream() throws FilingException {
    throw new UnsupportedFilingOperationException(
      "getNativeInputStream not supported"
    );
  }

  public void mark(int readlimit) throws FilingException {
    throw new UnsupportedFilingOperationException("mark not supported");
  }

  public void reset() throws FilingException {
  ...
  public String toString() {
    ...
    if (localPassthru) s += " LOCAL";
    if (curBuffer == null) s += " <nullbuf>"; else if (
      curBuffer.length <= 16
