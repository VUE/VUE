...
public class RfsEntryCache implements java.io.Serializable {
  ...
  protected RfsEntryCache(RfsEntry rfsEntry) throws FilingException {
    setEntry(rfsEntry);
  }
  ...
  protected void setEntry(RfsEntry entry) throws FilingException {
    this.rfsEntry = entry;
    this.maxCacheAge = rfsEntry.factory.getMaxCacheAge();
    this.name = rfsEntry.getName();
    if (entry.isByteStore()) this.bits |= IS_BYTESTORE; else if (
      entry.isCabinet()
  ...
  boolean canRead() throws FilingException {
    ensureCache();
    return (this.bits & CAN_READ) != 0;
  }

  boolean canWrite() throws FilingException {
    ensureCache();
  ...
  boolean exists() throws FilingException {
    ...
    if (isCurrent()) return !this.isStale; else {
      bits |= TESTING_EXIST;
      refreshCache();
      bits &= ~TESTING_EXIST;
    }
  ...
  void ensureCache() throws FilingException {
    if (isStale) throw new FilingException(
      "'" +
      name +
      "' - stale RFS entry (no longer exists): " +
      rfsEntry.getPath()
    );
    if (!isCurrent()) refreshCache();
  }
  ...
  protected void refreshCache() throws FilingException {
    if (!rfsEntry.hasClient) throw new FilingException(
      "attempted update of RfsEntryCache without a client"
    );

    RfsEntryCache c;
    try {
      c =
        (RfsEntryCache) rfsEntry.factory.invoke(
          this,
          "refreshCacheRemotely",
          String.class,
          rfsEntry.idStr
        );
      if (c == null) {
  ...
  public RfsEntryCache refreshCacheRemotely(String path)
    ...
    try {
      ...
      if ((bits & TESTING_EXIST) != 0) return null; else throw e;
    }
    this.bits = 0;
    initFromFile(file);
    return this;
  ...
  public String toString() {
    ...
    if (lastRefresh > 0) s +=
    " age=" + (System.currentTimeMillis() - lastRefresh);
    if (isStale) s += " STALE";
    s += "]";
    return s;
  }
