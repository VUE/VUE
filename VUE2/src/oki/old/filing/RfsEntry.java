...
public abstract class RfsEntry
  ...
  protected void initEntry(
    ...
    } else if (hasClient && factory.getMaxCacheAge() > 0) this.cache =
      new RfsEntryCache(this);
    if (create) createFile();
  }

  protected java.io.File getLocalFile() throws FilingException {
    if (hasClient) throw new FilingException(
      "attempted local access to a remote RfsEntry"
    );

    if (this.file == null) return (
      this.file = getEntryFile(this.idStr, isCabinet())
  ...
  protected static java.io.File getEntryFile(String path, boolean isCabinet)
    ...
    try {
       ...
       * here.
       */
      if (!file.exists()) throw new NotFoundException(path + ": doesn't exist");

      if (isCabinet) {
        if (!file.isDirectory()) {
          String msg = " not found as directory: ";
          if (file.isFile()) msg += "is file"; else msg +=
          "is of unhandled type";
          throw new NameCollisionException(path + msg);
        }
      } else if (!file.isFile()) {
        String msg = " not found as file: ";
        if (file.isDirectory()) msg += "is directory"; else msg +=
        "is of unhandled type";
        throw new NameCollisionException(path + msg);
      }
  ...
  protected void createFile() throws FilingException {
    if (hasClient) {
      RfsEntryCache rfsData = (RfsEntryCache) factory.invoke(
        this,
        "createFileRemotely"
      );
      if (this.cache != null) this.cache.copyUpdate(rfsData);
    } else createFileLocally();
  }

  public RfsEntryCache createFileRemotely() throws FilingException {
    createFileLocally();
   ...
   */
  protected void createFileLocally() throws FilingException {
    try {
      ...
      if (this.isByteStore()) {
        ...
        try {
          ...
          ) throw new FilingPermissionDeniedException(
            e,
            exMsg
          ); else throw new FilingIOException(e, exMsg);
        }
        if (alreadyExists) throw new NameCollisionException(
          exMsg + ": already exists"
        );
      } else if (this.isCabinet()) {
        String exMsg = "directory '" + this.idStr + "' could not be created";
        if (file.exists()) throw new NameCollisionException(
          exMsg + ": already exists"
        );
        try {
          if (!this.file.mkdir()) throw new FilingIOException(exMsg);
        } catch (java.security.AccessControlException e) {
          if (
            e.getMessage().indexOf("ermission") >= 0
          ) throw new FilingPermissionDeniedException(
            e,
            exMsg
          ); else throw new FilingIOException(e, exMsg);
        }
      } else throw new FilingException(
        "tried to create RfsEntry of unknown type"
      );
    } catch (FilingException e) {
      throw e;
    } catch (Exception e) {
   ...
   * @return ID
   */
  public ID getID() throws FilingException {
    if (this.id != null) return this.id;

    if (hasClient) return (
      this.id = (RfsID) factory.invoke(this, "getID")
    ); else {
      String cPath;
      try {
   ...
   *
   */
  public boolean exists() throws FilingException {
    try {
      if (hasClient) {
        if (cache != null) return cache.exists(); else return (
          (Boolean) factory.invoke(this, "exists")
        ).booleanValue();
      } else return existsLocally();
  ...
  public boolean rename(String newName) throws FilingException {
    if (hasClient) {
      if (
        ...
        this.idStr =
          getParentPath(this.idStr) + factory.getSeparatorChar() + newName;
        this.id = null; // make sure this gets rebuilt
        String oldName = getName();
        this.name = newName;
        if (this.cache != null) this.cache.setEntry(this);
        if (this.parent != null) this.parent.notifyRename(this, oldName);
        return true;
      }
    } else return renameLocally(newName);
    return false;
  }
   ...
   */
  private boolean renameLocally(String newName) throws FilingException {
    java.io.File f = new java.io.File(this.idStr);
  ...
  public long getLastModifiedTime() throws FilingException {
    if (hasClient) {
      if (cache != null) return cache.getLastModifiedTime(); else return (
        (Long) factory.invoke(this, "getLastModifiedTime")
      ).longValue();
    } else return getLastModifiedTimeLocally();
  }

  protected long getLastModifiedTimeLocally() throws FilingException {
    long lastModified = getLocalFile().lastModified();

    if (lastModified == 0) throw new FilingIOException(
      "invalid lastModified time"
   ...
   */
  public boolean setLastModifiedTime(long time) throws FilingException {
    boolean tv = setLastModifiedTimeRemotely(new Long(time)).booleanValue();
    if (cache != null) cache.invalidate();
    return tv;
  }

  public Boolean setLastModifiedTimeRemotely(Long time) throws FilingException {
    if (hasClient) return (Boolean) factory.invoke(
      this,
      "setLastModifiedTimeRemotely",
      Long.class,
      time
    ); else return new Boolean(setLastModifiedTimeLocally(time.longValue()));
  }

  private boolean setLastModifiedTimeLocally(long time) throws FilingException {
    if (time < 0L) throw new FilingException(
    ...
    if (!success) {
      if (!this.file.canWrite()) throw new FilingPermissionDeniedException(
        "cannot set lastModifiedTime"
      ); else throw new FilingIOException("IO Error setting lastModifiedTime");
    } else {
      return true;
    }
  }
   ...
   *
   * @throws FilingIOException - if an IO error occurs.  */
  public long getLastAccessedTime() throws FilingException {
   ...
   */
  public long getCreatedTime() throws FilingException {
    return this.getLastModifiedTime();
  }
   ...
   */
  public boolean delete() throws FilingException {
    if (hasClient) {
      Boolean bv = (Boolean) factory.invoke(this, "delete");
      if (bv.booleanValue()) {
        if (cache != null) cache.markStale(0);
        if (parent != null) parent.notifyDelete(this);
        return true;
      } else return false;
    } else return deleteLocally();
  }

  private boolean deleteLocally() throws FilingException {
    getLocalFile();

    if (isCabinet() && this.file.list().length > 0) throw new FilingException(
      "cannot delete cabinet: is not empty"
    );
    ...
    if (!success) {
      if (!this.file.canWrite()) throw new FilingPermissionDeniedException(
        "read only"
      ); else throw new FilingIOException("IO Error on deleting");
    } else return true;
  }
   ...
   */
  public org.okip.service.shared.api.Agent getOwner() throws FilingException {
  ...
  public void setOwner(org.okip.service.shared.api.Agent owner)
    ...
    throw new UnsupportedFilingOperationException(
      "Entry Ownership not implemented"
    );
  }
   ...
   *
   */
  public boolean canRead() throws FilingException {
    if (hasClient) {
      if (cache != null) return cache.canRead(); else return (
        (Boolean) factory.invoke(this, "canRead")
      ).booleanValue();
    } else return canReadLocally();
  }

  protected boolean canReadLocally() throws FilingException {
    getLocalFile();
   ...
   */
  public boolean setReadOnly() throws FilingException {
    if (hasClient) {
      if (cache != null) cache.invalidate();
  ...
  private boolean setReadOnlyLocally() throws FilingException {
    getLocalFile();
    ...
    if (!success) {
      if (!this.file.canWrite()) throw new FilingPermissionDeniedException(
        "cannot set ReadOnly"
      ); else throw new FilingIOException("IO Error setting ReadOnly");
    } else return true;
  }
   ...
   *
   */
  public boolean canWrite() throws FilingException {
    if (hasClient) {
      if (cache != null) return cache.canWrite(); else return (
        (Boolean) factory.invoke(this, "canWrite")
      ).booleanValue();
    } else return canWriteLocally();
  }

  protected boolean canWriteLocally() throws FilingException {
    getLocalFile();
   ...
   */
  public boolean setWritable() throws FilingException {
    throw new UnsupportedFilingOperationException(
      "Cannot set an entry Writable in this implementation"
    );
  }
   ...
   */
  public Cabinet getParent() throws FilingException {
  ...
  private Cabinet getParentLocally() throws FilingException {
    getLocalFile();
    try {
   ...
   * @return name
   */
  public String getName() throws FilingException {
    if (name != null) return name;
    if (cache != null) return name = cache.getName();
    if (file != null) return name = file.getName();
    return name = getBaseName(this.idStr);
  }

  protected String getNameInternal() {
    if (name != null) return name;
    if (cache != null) return cache.name;
  ...
  protected String getBaseName(String path) throws FilingException {
    if (path.length() <= 1) return path;
    char separator;
    if (hasClient) separator = factory.getSeparatorChar(); else separator =
      java.io.File.separatorChar;
    // strip all separator chars off end of string
    while (
      path.length() > 1 && path.charAt(path.length() - 1) == separator
    ) path = path.substring(0, path.length() - 1);
    if (path.length() == 1) return path;
    // now make sure we're only taking the basename
    int i = path.lastIndexOf(separator);
    if (i >= 0) return path.substring(i + 1); else return path;
  }

  protected String getParentPath(String path) throws FilingException {
    char separator = factory.getSeparatorChar();
    // strip all separator chars off end of string
    while (path.charAt(path.length() - 1) == separator) path =
      path.substring(0, path.length() - 1);
    // now take the root parent path
    int i = path.lastIndexOf(separator);
    if (i > 0) return path.substring(0, i); else return "" + separator;
  }
  ...
  public String toString() {
    ...
    } else s += "idStr='" + this.idStr + "'";

    if (this.id != null) {
      if (!this.id.toString().equals(idStr)) s += " id='" + this.id + "'";
    }
    s += " " + factory;
    if (this.cache != null) s += " " + this.cache;
    return s;
  }
}
