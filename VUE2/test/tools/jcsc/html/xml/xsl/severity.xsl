...
<xsl:variable name="categoryName"><xsl:value-of select="jcsc/rulelist/severity"/></xsl:variable>
...
<xsl:template match="severity">
  <h4>
  <xsl:if test="not(. ='All Severities')">
     Severity <xsl:value-of select="."/>
  </xsl:if>

  <xsl:if test=". ='All Severities'">
    <xsl:value-of select="."/>
    <br/>
  </xsl:if>
  </h4>
...
<xsl:template match="rule">
  <!-- <xsl:value-of select="./severity"/> - --><xsl:element name="a">
