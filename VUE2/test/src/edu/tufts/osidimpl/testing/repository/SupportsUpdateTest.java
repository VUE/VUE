...
public class SupportsUpdateTest extends TestCase
{
	public SupportsUpdateTest(org.osid.repository.RepositoryManager repositoryManager, org.w3c.dom.Document document)
		throws org.osid.repository.RepositoryException, org.xml.sax.SAXParseException
	{
		// are there repositories to test for?
		org.w3c.dom.NodeList repositoriesNodeList = document.getElementsByTagName(OsidTester.SUPPORTS_UPDATING_TAG);
				...
				try {
					...
					try {
						...
						if (expected.equals("true")) {
							...
							System.out.println("PASSED: Supports Update for Repository " + idString);
						} else {
							try {
								org.osid.repository.AssetIterator ai = repository.getAssets();
								if (ai.hasNextAsset()) {
									fail("FAILED: Supports Update for Repository " + idString);
								} else {
									System.out.println("PASSED: Supports Update for Repository " + idString);
								}
