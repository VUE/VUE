...
public class UpdateContentTest extends TestCase
{
	public UpdateContentTest(org.osid.repository.RepositoryManager repositoryManager, org.w3c.dom.Document document)
		throws org.osid.repository.RepositoryException, org.xml.sax.SAXParseException
	{
		try {
			...
			org.w3c.dom.NodeList assetsNodeList = document.getElementsByTagName(OsidTester.CONTENT_UPDATE_TAG);
			...
			for (int i=0; i < numTests; i++) {
				...
				if ((assetIdString != null) && (assetIdString.trim().length() > 0)) {
					...
					// now test the asset
					boolean passed = cu.testAssetAfterUpload(repositoryManager,asset);
					if (passed) {
